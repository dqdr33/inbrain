...
Inbrain is a personal knowledge brain and GStack mod for agent platforms. Pluggable
engines: PGLite (embedded Postgres via WASM, zero-config default) or Postgres + pgvector
+ hybrid search in a managed Supabase instance. `inbrain init` defaults to PGLite;
suggests Supabase for 1000+ files. GStack teaches agents how to code. Inbrain teaches
...
inbrain aims to be the **next Postgres for memory**: the most well-tested, widest-coverage,
...
against this bar. "inbrain is best" is a WHOLE-SYSTEM claim — proven across the full
BrainBench suite (retrieval, longmemeval, calibration, …) — not by any single feature.
When scoping an eval, prove the FEATURE delivers value to inbrain users; do not waste it
proving that inbrain's particular algorithm beats some other algorithm (a research
bake-off, off-mission).

## Two organizational axes (read this first)

Inbrain knowledge is organized along two orthogonal axes. Users AND agents must
...
- **Brain** — WHICH DATABASE. Your personal brain is `host`. You can mount
  ...
  via `inbrain mounts add` (v0.19+). Routing: `--brain`, `INBRAIN_BRAIN_ID`,
  `.inbrain-mount` dotfile.
- **Source** — WHICH REPO INSIDE THE DATABASE. A brain can hold many sources
  ...
  `--source`, `INBRAIN_SOURCE`, `.inbrain-source` dotfile.
...
pattern inbrain ships for its own skills: thin router in `skills/RESOLVER.md`, fat
detail on demand.)
...
| the CLI surface (commands + flags) | `inbrain --help` / `inbrain --tools-json`, plus the relevant `KEY_FILES.md` entry |
...
inbrain exists to fix. The rules that keep it from recurring:
...
Inbrain ships three named search modes that bundle the search-lite knobs from
...
**Cost anchors (downstream agent input cost — inbrain itself is rounding error).**
...
picker copy in `inbrain init` carries the same matrix verbatim** — update
...
`inbrain eval replay` and `inbrain eval longmemeval` test the same mode-affected
...
**Three CLI surfaces:**
    ...
    inbrain search modes              # what is running, with per-knob attribution
    inbrain search modes --reset      # clear search.* overrides (mode bundle wins)
    inbrain search stats [--days N]   # cache hit rate, intent mix, budget drops
    inbrain search tune [--apply]     # data-driven recommendations

The install picker fires inside `inbrain init` AFTER `engine.initSchema()`
...
Every metric printed by any `inbrain eval *` or `inbrain search stats` command
...
Per-run records land at `<repo>/.inbrain-evals/eval-results.jsonl` per
`[CDX-23]`. The user's personal `~/.inbrain` brain is NEVER touched —
audit trail lives in the source repo's git history.

## Skills

Read the skill files in `skills/` before doing brain operations. Inbrain ships 29 skills
...
work (shell jobs via `inbrain jobs submit shell`, LLM subagents via `inbrain agent run`) ...
the prior `inbrain-jobs` skill was merged in, Preconditions are shared, and trigger
...
`inbrain sync` is resumable and converges under pool exhaustion + repeated kills.
...
|---|---|---|
| `INBRAIN_SYNC_CHECKPOINT_EVERY` | 1000 | Flush the checkpoint every N drained files. |
| `INBRAIN_SYNC_CHECKPOINT_SECONDS` | 10 | Also flush every N seconds (whichever comes first) — bounds worst-case loss regardless of throughput. Flush also fires after the first file. |
| `INBRAIN_SYNC_MAX_CHECKPOINT_FAILURES` | 3 | Consecutive failed flushes (each already retried ~12s) before the run aborts with `reason: 'checkpoint_unavailable'` instead of importing work it can never bank. |
| `INBRAIN_SYNC_YIELD_EVERY` | 64 | Yield the event loop (`setTimeout(0)`, NOT `setImmediate` — Bun starves the timers phase under a tight setImmediate loop) every N files so the lock-refresh `setInterval` heartbeat fires mid-import. |
| `INBRAIN_LOCK_STEAL_GRACE_SECONDS` | derived (~600 at 30min TTL) | A holder that refreshed within this window is NOT stolen even if its TTL lapsed (starved-but-alive). Dead holders stop refreshing, age past the grace, and become stealable; TTL stays the backstop. |

## Build

`bun build --compile --outfile bin/inbrain src/cli.ts`
...
| `package.json` | Bun/npm package version. `inbrain --version` reads it via the compiled binary's bundled package metadata. CI version-gate cross-checks this against `VERSION` and fails if they drift. | `"version": "0.31.4.1"` |
...
BRANCH=$(git branch --show-current)        # e.g. inbrain-ai/gstack-requests
case "$BRANCH" in
...
If MISMATCH (branch is `inbrain-ai/foo` but workspace is `puebla-v4`):
...
git branch -m inbrain-ai/<workspace-name>
git push -u origin inbrain-ai/<workspace-name>
...
`inbrain-ai/gstack-requests` produced PR #1439 that Conductor wouldn't
display. Renamed to `inbrain-ai/puebla-v4`; recreated as #1440.
...
Why: inbrain runs a personal knowledge brain containing notes on real people and
real companies (YC founders, portfolio companies, funds, investors, meeting
attendees). When a doc copies a query like `inbrain graph diana-hu --depth 2` or
...
- "If `inbrain doctor` still flags anything after upgrade, the message names each
...
walk away knowing "inbrain at version X has table Y readable by anon key until
