...
export interface ParseOpts {
   ...
   * the hardcoded inbrain-base table. When unset, falls back to the
   * pre-v0.39 hardcoded behavior (preserves byte-for-byte parity gate
   * `test/regressions/inbrain-base-equivalence.test.ts`).
   *
 ...
 *      (backward-compat fallback for older inbrain-written files)
 *
...
// pack is inbrain-base (or when no pack is provided — back-compat
...
//   the INBRAIN_BASE_PATH_PREFIXES table below, which reproduces the
...
// and matches inbrain-base by construction.

/**
 * Hardcoded path-prefix table mirroring pre-v0.38 inferType behavior.
 * MUST stay in lockstep with the inbrain-base.yaml `path_prefixes` field
 * (parity-pinned by test/regressions/inbrain-base-equivalence.test.ts).
 * Order matters: wiki subtypes + writing scan first (stronger signal
 * than ancestor directories).
 */
const INBRAIN_BASE_PATH_PREFIXES: ReadonlyArray<{ prefixes: string[]; type: PageType }> = [
...
function inferType(filePath?: string): PageType {
  return inferTypeWithPrefixes(filePath, INBRAIN_BASE_PATH_PREFIXES);
}
 ...
 * `extends: null` pack) falls back to inbrain-base defaults.
 *
 ...
 *
 * Note on prefix shape: inbrain-base stores prefixes WITHOUT the
...
export function inferTypeFromPack(
  ...
  // Empty pack → fall back to inbrain-base hardcoded defaults.
...
export function inferTypeAndSubtypeFromPack(
  ...
  if (pack.page_types.length === 0) {
    return { type: inferTypeWithPrefixes(filePath, INBRAIN_BASE_PATH_PREFIXES) };
  }
  const lower = ('/' + filePath).toLowerCase();
