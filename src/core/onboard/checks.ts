...
export async function checkEntityLinkCoverage(
  ...
  // would break `inbrain doctor exits 0` contract; the recommendation
...
export async function checkTakesCount(
  ...
  if (takesCount >= 100) {
    ...
    status = 'warn';
    if (bootstrapEnabled) {
      message = `0 takes (bootstrap eligible — inbrain takes extract --from-pages)`;
      remediations.push(makeRemediationStep({
 ...
 * pack declared via `migration_from`. v0.42 ships inbrain-base-v2 as the
 * declared successor of inbrain-base@1.x. Emits a manual_only RemediationStep
    ...
    return {
      check: {
        ...
        message:
          ...
          `Preview: \`inbrain onboard --check --explain\``,
      },
      remediations: [
        makeRemediationStep({
...
export async function checkTypeProliferation(
  ...
  let declared = 15;  // fallback to inbrain-base-v2 default if pack unavailable
  try {
    ...
    return {
      check: {
        ...
        message:
          ...
          `Run \`inbrain onboard --check --explain\` to preview a pack upgrade ` +
