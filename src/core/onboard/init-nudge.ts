...
//
// Bypass: INBRAIN_NO_ONBOARD_NUDGE=1 short-circuits. Non-TTY default
...
export async function runInitNudge(engine: BrainEngine): Promise<void> {
  try {
    if (process.env.INBRAIN_NO_ONBOARD_NUDGE === '1') return;
    ...
    process.stderr.write(
      ...
      `[onboard] Run 'inbrain onboard --check' to see the plan.` +
      (partial ? ` (${checksRan}/${checksAttempted} checks complete; run inbrain onboard --check for full recommendations)` : '') +
      `\n`,
    );
  } catch (err) {
...
export async function runUpgradeBanner(_engine: BrainEngine): Promise<void> {
  try {
    ...
    if (!process.stderr.isTTY) return;
    process.stderr.write(
      `\n[onboard] Upgrade complete. Run 'inbrain onboard --check' to see if the new version surfaces any new opportunities.\n`,
    );
  } catch {
