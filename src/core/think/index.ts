/**
 * v0.28: `inbrain think` — INTENT → GATHER → SYNTHESIZE → (optional) COMMIT.
 *
...
export async function runThink(
  ...
  if (opts.stubResponse) {
    ...
    //      anthropic_api_key from inbrain config OR env, gateway rate-leases,
    ...
    // That bypassed gateway config (inbrain config set anthropic_api_key)
...
// doesn't inherit shell env, so `inbrain config set anthropic_api_key sk-...`
// (which writes to ~/.inbrain/config.json) never reached the SDK and every
// MCP think call degraded to "no LLM available."
//
// The adapter routes through gateway.chat() — the canonical seam per
// CLAUDE.md. Gateway reads the API key from inbrain config OR env, picks
...
function buildGracefulMessage(modelStr: string): {
  ...
  return {
    ...
    role: 'assistant',
    model: modelStr,
    content: [{ type: 'text', text: '(no LLM available — set anthropic_api_key via inbrain config or ANTHROPIC_API_KEY env)' }],
