/**
 * Baseline file format for `inbrain bench publish` + `inbrain eval gate --baseline`.
 *
 ...
 *
 * The `_kind` discriminator lets `inbrain eval replay` skip the metadata line
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (metadata === null) {
      ...
      if (meta.schema_version !== BASELINE_FILE_SCHEMA_VERSION) {
        throw new BaselineParseError(
          `Unsupported schema_version ${meta.schema_version} (this inbrain build expects ${BASELINE_FILE_SCHEMA_VERSION})`,
          i + 1,
        );
      }
      if (
      ...
      if (typeof row.query_hash !== 'string') {
        throw new BaselineParseError(`Row on line ${i + 1} missing query_hash (was it written by inbrain bench publish?)`, i + 1);
      }
 ...
 * `inbrain eval export`) so the existing `eval replay` parser accepts the
