 ...
 */
export async function runRemoteDoctor(config: InbrainConfig, args: string[]): Promise<void> {
 ...
 *
 * Also honors INBRAIN_DOCTOR_SKIP_SCOPE_PROBE=1 for ops bypass; explicit
...
export async function collectRemoteDoctorReport(
  ...
  const clientSecret = process.env.INBRAIN_REMOTE_CLIENT_SECRET ?? remote.oauth_client_secret;
  const clientSecretSource: 'env' | 'config' | 'none' = process.env.INBRAIN_REMOTE_CLIENT_SECRET
    ? 'env'
  ...
  if (!clientSecret) {
    checks.push({
      ...
      status: 'fail',
      message: 'No client_secret available. Set INBRAIN_REMOTE_CLIENT_SECRET or rerun `inbrain init --mcp-only` with --oauth-client-secret.',
    });
    return {
      schema_version: 2,
      mode: 'thin-client',
  ...
  // `inbrain stats` / `inbrain history` and fail today; this check surfaces
  // the gap during `inbrain remote doctor` instead of mid-command.
  //
  // Skippable via opts.skipScopeProbe (preferred for tests) OR
  // INBRAIN_DOCTOR_SKIP_SCOPE_PROBE=1 (env-flag for ops bypass) — the MCP
  ...
  const skipProbe = opts.skipScopeProbe || process.env.INBRAIN_DOCTOR_SKIP_SCOPE_PROBE === '1';
  if (!skipProbe) {
  ...
  // self-fix hint — thin-client users can't run `inbrain extract links
  ...
  //     `inbrain upgrade` (or, if state shows a prior 'failed' attempt,
 ...
 * local `inbrain doctor` `orphan_ratio` check uses. Computes the ratio,
 ...
 * run `inbrain extract links --by-mention` themselves — they need to
 * ping whoever runs the brain server.
 *
 * Errors non-fatal — informational check.
 */
export async function runOrphanRatioCheck(config: InbrainConfig): Promise<RemoteCheck> {
  type OrphanData = {
    orphans: unknown[];
  ...
  const hint =
    `Ask the brain operator at ${url} to run: inbrain extract links --by-mention ` +
 ...
 * `inbrain doctor` so quiet/non-TTY users (who don't see the interactive
 ...
 */
export async function runUpgradeDriftCheck(config: InbrainConfig): Promise<RemoteCheck> {
  ...
  const fixHint = priorFailed
    ? `Prior \`inbrain upgrade\` did not advance the binary. See https://github.com/inbrain-ai/inbrain/releases for manual install.`
    : `Run \`inbrain upgrade\` to install v${remoteVersion}.`;
...
async function probeScopes(config: InbrainConfig): Promise<ScopeProbeResult> {
  ...
  if (adminMissing) {
    return {
      ...
      message:
        ...
        '`inbrain auth register-client <name> --grant-types client_credentials --scopes read,write,admin`',
      detail: {
        granted: grantedScope || null,
        read_ok: true,
        admin_ok: false,
...
function finalize(
  remote: NonNullable<InbrainConfig['remote_mcp']>,
  checks: RemoteCheck[],
  scope?: string,
): RemoteDoctorReport {
...
function printHumanReport(report: RemoteDoctorReport): void {
  console.log('\nInbrain Health Check (thin-client)');
  console.log('=================================');
  ...
  if (report.status === 'ok') {
    ...
    console.log('  - Confirm the host is reachable + `inbrain serve --http` is running.');
    console.log('  - Confirm OAuth credentials are valid (have the host operator re-mint via `inbrain auth register-client`).');
