 ...
 * `engine.logEvalCaptureFailure(reason)` so `inbrain doctor` can see drops
...
// public `inbrain/types` subpath. Surfaced from hybridSearch via the
 ...
 *   3. `process.env.INBRAIN_CONTRIBUTOR_MODE === '1'` → on (contributor opt-in)
 ...
 */
export function isEvalCaptureEnabled(config: InbrainConfig | null | undefined): boolean {
  if (config?.eval?.capture === true) return true;
  if (config?.eval?.capture === false) return false;
  return process.env.INBRAIN_CONTRIBUTOR_MODE === '1';
}
 ...
 */
export function isEvalScrubEnabled(config: InbrainConfig | null | undefined): boolean {
