...
interface Migration {
   ...
   * older migrations rely on `inbrain upgrade --force-schema` for recovery.
   */
...
export class MigrationRetryExhausted extends Error {
  constructor(
    ...
    const hint = lastB
      ...
      : 'No idle-in-transaction blockers detected; check pg_locks for active waiters and ~/.inbrain/audit/connection-events-*.jsonl';
    super(
...
export const MIGRATIONS: Migration[] = [
  ...
  },
  {
    ...
    // Original report: https://github.com/inbrain-ai/inbrain/issues/170 (PR #215).
    //
  ...
  },
  {
    ...
    // candidates for re-resolution via `inbrain extract
  ...
  },
  {
    ...
    //   while file_upload / `inbrain files` kept accepting writes.
    //
  ...
  },
  {
    ...
    // v0.14.1 (fix wave): fixes https://github.com/inbrain-ai/inbrain/issues/219
  ...
  },
  {
    ...
    //          WHERE id = 'inbrain-cycle' AND holder_pid = <my pid> — between phases.
    // Release: DELETE WHERE id = 'inbrain-cycle' AND holder_pid = <my pid>.
    sql: `
      CREATE TABLE IF NOT EXISTS inbrain_cycle_locks (
        id TEXT PRIMARY KEY,
      ...
      CREATE INDEX IF NOT EXISTS idx_cycle_locks_ttl ON inbrain_cycle_locks(ttl_expires_at);
    `,
  },
  {
    version: 24,
    name: 'rls_backfill_missing_tables',
    // v0.18.1 RLS hardening: 10 inbrain-managed public tables shipped
    ...
    // subagent_rate_leases, inbrain_cycle_locks, budget_ledger,
    ...
    sql: `
      ...
      BEGIN
        ...
        ALTER TABLE inbrain_cycle_locks ENABLE ROW LEVEL SECURITY;
        ...
        -- logs) or was pinned to a pre-v12 inbrain version when the table
  ...
  },
  {
    ...
    // grants access via `inbrain auth permissions <id> set-takes-holders`.
    sql: `
  ...
  },
  {
    ...
    //     subagent_id let inbrain-evals partition replay by run.
    ...
    //     the capture path records the reason here so `inbrain doctor` can
  ...
  },
  {
    ...
    // v0.26 OAuth 2.1 tables for `inbrain serve --http`. Supports client credentials,
  ...
  },
  {
    ...
    //
    // Problem: tables created outside inbrain migrations (Baku's face_detections,
    ...
    //      and whose comment does NOT match the INBRAIN:RLS_EXEMPT contract
    ...
    //     owned by that role). inbrain has BYPASSRLS so inbrain itself is unaffected.
    ...
    // RLS-off public tables that don't carry the INBRAIN:RLS_EXEMPT comment.
    ...
    sqlFor: {
      postgres: `
        ...
        -- Honors the same INBRAIN:RLS_EXEMPT regex doctor.ts uses
        -- (^INBRAIN:RLS_EXEMPT\\s+reason=\\S.{3,}) so the two surfaces stay aligned.
        ...
        BEGIN
          ...
          FOR r IN
            ...
            WHERE n.nspname = 'public'
              ...
              AND (d.description IS NULL OR d.description !~ '^INBRAIN:RLS_EXEMPT\\s+reason=\\S.{3,}')
          LOOP
      ...
      if (engine.kind === 'postgres') {
        ...
        if (rows.length === 0) {
          throw new Error(
            ...
            `then re-run \`inbrain apply-migrations --yes\`.`
          );
        }
        ...
        if (major === 0 && min < 5) {
          throw new Error(
            ...
            `Fix: ALTER EXTENSION vector UPDATE; then re-run \`inbrain apply-migrations --yes\`.\n` +
  ...
  },
  {
    ...
    // ranking before the cycle has run; users run `inbrain dream --phase
  ...
  },
  {
    ...
    //
    // Without these fields, `inbrain eval replay` cannot reproduce a captured
    ...
    // inbrain-evals consumers that don't know about them ignore them
  ...
  },
  {
    ...
    // `inbrain backfill emotional_weight` command both stamp this column
  ...
  },
  {
    ...
    // the UPDATE so other inbrain processes (workers, MCP queries) can
  ...
  },
  {
    ...
    //
    // One row per `inbrain eval suspected-contradictions` run. The headline
    // numbers (queries_evaluated, with_contradiction, total_flagged) plus
    // Wilson 95% CI bounds enable `inbrain eval suspected-contradictions
  ...
  },
  {
    ...
    //     it; existing rows inherit DEFAULT 1. `inbrain reindex --markdown`
  ...
  },
  {
    ...
    //                   at read time and pruned by `inbrain cache prune`.
  ...
  },
  {
    ...
    //
    // Powers `inbrain search stats [--days N]` and `inbrain search tune` so an
    ...
    // derives averages so concurrent ON CONFLICT writes from multiple inbrain
  ...
  },
  {
    ...
    // INBRAIN_ACCEPT_SILENT_WIDEN=1 env var is the explicit opt-in for
    ...
    sql: `
      ...
      -- INBRAIN_ACCEPT_SILENT_WIDEN env-flag stays in the runner for future
  ...
  },
  {
    ...
    sql: `
      ...
      BEGIN
        ...
        IF bad_count > 0 THEN
          RAISE EXCEPTION 'oauth_clients has % rows where source_id is not in federated_read after v62 backfill. This is a bug in v62 — re-run inbrain apply-migrations --force-retry 62.', bad_count;
        END IF;
      END $$;
    `,
  },
  ...
  },
  {
    ...
    // bulk reindex completes (via `inbrain reindex --multimodal --build-index`
  ...
  },
  {
    ...
    // by `inbrain eval trajectory` + the `find_trajectory` MCP op.
    //
  ...
  },
  {
    ...
    //   - wave_version stamps every row so `inbrain calibration --undo-wave
  ...
  },
  {
    ...
    //     `inbrain dream --phase propose_takes` run so --rollback <run_id>
  ...
  },
  {
    ...
    // v0.36.1.0 (T18 / D19) — A/B harness data for `inbrain think --ab`.
    //
    // Each row records one side-by-side comparison of think with vs.
    // without --with-calibration. After 30 days of data, `inbrain
  ...
  },
  {
    ...
    // ~/.inbrain/checkpoints/<op>-<fingerprint>.json because it's single-host
    // by construction.
    //
  ...
  },
  {
    ...
    // Lets `inbrain doctor --remediate` runs be queried by run id for audit
  ...
  },
  {
    ...
    // signal for `inbrain lsd`'s "your brain at 3am noticing what it forgot"
  ...
  },
  {
    ...
    sql: `
      ALTER TABLE subagent_tool_executions
        ...
        ADD COLUMN IF NOT EXISTS inbrain_tool_use_id UUID;
      DO $$
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM pg_constraint
  ...
  },
  {
    ...
    //
    // Adds `eval_candidates.schema_pack_per_source JSONB` so `inbrain
    ...
    // pack file still existing in ~/.inbrain/schema-packs/. ~1KB per row
  ...
  },
  {
    ...
    // eval-trajectory, inbrain think) already defensively skip NULL-metric
  ...
  },
  {
    ...
    // reserved by inbrain-ai/v0.40.2.0-trajectory-routing for
    ...
    //   explicitly via `inbrain mounts trust-frontmatter <source>`. Host
  ...
  },
  {
    ...
    // survive `inbrain jobs prune`. Each audit table denormalizes context
  ...
  },
  {
    ...
    // mentions from `inbrain extract links --by-mention`. Backlink-count
  ...
  {
    version: 98,
    name: 'inbrain_cycle_locks_last_refreshed_at',
    // v0.41.15.0 (D-V3-4 + D-V4-1) — add last_refreshed_at column for
    // `inbrain sync --break-lock --max-age <s>` to correctly identify
    ...
    // If we backfilled = acquired_at (e.g. 25 min ago), then `inbrain sync
    ...
    sql: `
      ALTER TABLE inbrain_cycle_locks ADD COLUMN IF NOT EXISTS last_refreshed_at TIMESTAMPTZ;
      UPDATE inbrain_cycle_locks SET last_refreshed_at = NOW() WHERE last_refreshed_at IS NULL;
    `,
  },
  {
    version: 99,
  ...
  },
  {
    ...
    // v0.41.18.0 (inbrain onboard wave, A10 + codex finding #12):
  ...
  },
  {
    ...
    // v0.41.18.0 (inbrain onboard wave, A11 + codex finding #11):
  ...
  },
  {
    ...
    // v0.41.18.0 (inbrain onboard wave, A6 + A25 + A13 + codex #9 + #10):
  ...
  },
  {
    ...
    // Audit JSONL at ~/.inbrain/audit/extract-rounds-YYYY-Www.jsonl remains
  ...
  },
  {
    ...
    // should surface the page titled "Mingtang". inbrain stored that mapping
  ...
  },
  {
    ...
    // surfacing it. This column lets `inbrain extract --stale` sweep the historical
    ...
    // links_extracted_at, so the first `inbrain doctor` correctly surfaces the real
    // backlog (the whole point). The doctor check is warn-only by default; it only
    // hard-fails if INBRAIN_EXTRACTION_LAG_FAIL_PCT is set — so the upgrade never
    // breaks a CI/cron pipeline that gates on `inbrain doctor` exit code.
    //
  ...
  },
  {
    ...
    // provenance without a per-deriver inbrain migration. Format: lowercase
 ...
 * for both `inbrain doctor --locks` output and the internal DDL pre-flight.
 */
 ...
 *   - `inbrain doctor --locks` (CLI diagnostic)
...
async function runMigrationSQLWithRetry(
  ...
  // INBRAIN_MIGRATE_BACKOFF_MS lets tests skip the 5s/15s/45s backoff. In
  // production the env var is unset and the default cadence applies.
  const fastBackoff = process.env.INBRAIN_MIGRATE_BACKOFF_MS;
 ...
 * `inbrain stats` / `inbrain query` / `inbrain doctor`.
 *
...
export async function runMigrations(engine: BrainEngine): Promise<{ applied: number; current: number }> {
  ...
  // `inbrain jobs submit --json | jq`) aren't polluted by migration noise.
      ...
      try {
        ...
        if (code === '57014' || err instanceof MigrationRetryExhausted) {
          ...
          if (err instanceof MigrationRetryExhausted && err.lastBlockers.length > 0) {
            ...
            console.error('');
            console.error('   Fix:');
            console.error('     1. inbrain doctor --locks    # find idle-in-transaction blockers');
            console.error('     2. Terminate blocker(s) shown by step 1 via pg_terminate_backend(<pid>)');
            console.error('     3. inbrain apply-migrations --yes  # re-run from the version that failed');
            console.error('');
          }
          console.error('   Verify:');
          console.error('     inbrain doctor              # schema_version should match latest');
          console.error('');
        }
        throw err;
      }
