...
export function readinessHint(r: CodeGraphReadiness): string | null {
  switch (r.status) {
    case 'not_built':
      return 'Symbol graph not built (no code indexed in scope). Run `inbrain sync` to index code.';
    case 'indexing':
      return 'Symbol graph still building (edges pending resolution). Re-run after the next `inbrain dream` cycle / autopilot tick.';
    case 'unknown':
