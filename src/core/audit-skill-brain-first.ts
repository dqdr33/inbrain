 ...
 * `minions/handlers/supervisor-audit.ts` so all inbrain audit channels share
...
export function loadSnapshot(): { violators: Set<string>; present: boolean } {
  ...
  try {
    ...
    if (
      ...
      warnOnce(`[inbrain] snapshot corrupt: ${file} (unexpected shape); treating as missing`);
      return { violators: new Set(), present: false };
    }
    ...
    warnOnce(`[inbrain] snapshot corrupt: ${file} (${msg}); treating as missing`);
    return { violators: new Set(), present: false };
  }
}
...
export function writeSnapshotAtomically(violators: Set<string>, now: Date = new Date()): void {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    process.stderr.write(`[inbrain] snapshot write failed (${msg}); doctor continues\n`);
...
export function logBrainFirstEvent(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    process.stderr.write(`[inbrain] brain-first audit write failed (${msg}); doctor continues\n`);
  }
}
