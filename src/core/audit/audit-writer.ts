 ...
 *
 *   3. Audit dir resolution. Honors `INBRAIN_AUDIT_DIR` env override
 *      ahead of the default `~/.inbrain/audit/`. Trim whitespace; an
...
/**
 * Resolve the audit dir. Honors `INBRAIN_AUDIT_DIR` for container /
...
export function resolveAuditDir(): string {
  const override = process.env.INBRAIN_AUDIT_DIR;
  if (override && override.trim().length > 0) return override;
  return inbrainPath('audit');
}
...
export interface AuditWriterOpts {
   ...
   * (`'shell-audit'` vs `'inbrain'` vs `'supervisor-audit'`) so the
...
export interface AuditWriter<T extends { ts: string }> {
  ...
  /** Resolve the audit directory (honors INBRAIN_AUDIT_DIR override). */
  resolveDir(): string;
}
