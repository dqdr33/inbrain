 ...
 *
 * File: `~/.inbrain/audit/batch-retry-YYYY-Www.jsonl` (ISO-week rotation,
 * honors `INBRAIN_AUDIT_DIR` via the shared `resolveAuditDir()` helper).
 *
 * Pruning (codex H-8): `pruneOldBatchRetryAuditFiles(30)` deletes files
 * older than 30 days. Called from `inbrain dream --phase purge` (the cycle's
