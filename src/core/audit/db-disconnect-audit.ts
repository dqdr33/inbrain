 ...
 * singleton mid-cycle. The reported #1570 symptom — `inbrain dream` losing
 ...
 * `~/.inbrain/audit/db-disconnect-YYYY-Www.jsonl` (honors
 * `INBRAIN_AUDIT_DIR` via the shared `resolveAuditDir()` helper).
 */
...
export function logDbDisconnect(
  ...
  // argv[2] is typically the inbrain subcommand (e.g. 'dream', 'capture').
