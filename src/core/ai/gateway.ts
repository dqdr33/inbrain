/**
 * AI Gateway — unified seam for every AI call inbrain makes.
 *
 * v0.14 exports:
...
// socket makes an `await` never settle — which hangs `inbrain capture`/`search`
...
const AI_CHAT_TIMEOUT_MS = resolveAiTimeoutMs('INBRAIN_AI_CHAT_TIMEOUT_MS', 300_000);
/** embed sub-batch (per SDK call, NOT per whole import). */
const AI_EMBED_TIMEOUT_MS = resolveAiTimeoutMs('INBRAIN_AI_EMBED_TIMEOUT_MS', 60_000);
/** multimodal per request. */
const AI_MULTIMODAL_TIMEOUT_MS = resolveAiTimeoutMs('INBRAIN_AI_MULTIMODAL_TIMEOUT_MS', 60_000);
 ...
 * for the `inbrain models doctor` probe path (where the operator may want
 ...
 * paths like `inbrain --version` that never touch a brain). Per Codex F3
...
function requireConfig(): AIGatewayConfig {
  if (!_config) {
    throw new AIConfigError(
      ...
      'This is a inbrain bug — file an issue at https://github.com/inbrain-ai/inbrain/issues',
    );
  }
  return _config;
}
    ...
    for (const embedding of result.embeddings) {
      if (Array.isArray(embedding) && embedding.length !== expectedDims) {
        throw new AIConfigError(
          ...
          `Run \`inbrain migrate --embedding-model ${getEmbeddingModel()} --embedding-dimensions ${embedding.length}\` or change models.`,
        );
      }
    }
...
/**
 * Convert inbrain's provider-neutral ChatMessage[] into AI SDK v6 ModelMessage[].
 *
 ...
 *   - tool results must be a `role: 'tool'` message (inbrain pushes them as
 ...
 *
 * The key layer uses `hasAnthropicKey` (env OR inbrain config file), which is
...
export function probeChatModel(modelStr: string): ChatModelProbe {
  ...
  if (v.parsed.providerId === 'anthropic' && !hasAnthropicKey()) {
    return {
      ok: false,
      reason: 'unavailable',
      detail: 'no Anthropic API key configured (set ANTHROPIC_API_KEY or run: inbrain config set anthropic_api_key ...)',
    };
  }
  return { ok: true };
}
 ...
 * by inbrain-owned `inbrainToolUseId` (D11), NOT provider-supplied IDs.
 * `priorMessages` is the chat history up to the assistant's last turn;
 * `priorTools` maps inbrainToolUseId → outcome. The D5 read-time shim
 * synthesizes inbrainToolUseIds for legacy v1 rows so this Map sees both
 * shapes uniformly.
 */
...
export interface ToolLoopOpts {
   ...
   * returns them so the loop can key replay by inbrainToolUseId. The provider
  ...
  ) => Promise<{ inbrainToolUseId: string }>;
  onToolCallComplete?: (inbrainToolUseId: string, output: unknown) => Promise<void>;
  onToolCallFailed?: (inbrainToolUseId: string, error: string) => Promise<void>;
 ...
 *   - inbrain-stable IDs (D11) at first observation
 *   - write-ordering invariant (persist before side effect)
 *   - crash-replay reconciliation via inbrainToolUseId
...
export async function toolLoop(opts: ToolLoopOpts): Promise<ToolLoopResult> {
  ...
  while (turnIdx < maxTurns) {
    ...
    for (let callIdx = 0; callIdx < toolCalls.length; callIdx++) {
      ...
      // Step 2: persist pending row + claim inbrainToolUseId. The caller's
      ...
      const { inbrainToolUseId } = (await opts.onToolCallStart?.(
        turnIdx,
        assistantMessageIdx,
        callIdx,
        call.toolName,
        call.input,
        call.toolCallId,
      )) ?? { inbrainToolUseId: `inline-${turnIdx}-${callIdx}` };

      // Replay short-circuit: prior outcome wins, idempotent re-execute allowed.
      const prior = opts.replayState?.priorTools.get(inbrainToolUseId);
      ...
      if (prior?.status === 'pending' && !handler.idempotent) {
        ...
        throw new Error(
          `non-idempotent tool "${call.toolName}" pending on resume; inbrainToolUseId=${inbrainToolUseId} — cannot safely re-run`,
        );
      }
      ...
      try {
        ...
        await opts.onToolCallComplete?.(inbrainToolUseId, output);
        toolResultBlocks.push({
          type: 'tool-result',
          toolCallId: call.toolCallId,
          toolName: call.toolName,
        ...
        await opts.onToolCallFailed?.(inbrainToolUseId, errMsg);
        toolResultBlocks.push({
          type: 'tool-result',
          toolCallId: call.toolCallId,
          toolName: call.toolName,
