...
export const llamaServerReranker: Recipe = {
  ...
  touchpoints: {
    reranker: {
      ...
      // by the user via `inbrain config set search.reranker.model
  ...
  setup_hint:
    ...
    'path). Then `inbrain config set search.reranker.model ' +
    'llama-server-reranker:<short-id>` and `inbrain config set ' +
