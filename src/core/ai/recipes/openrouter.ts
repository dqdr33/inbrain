 ...
 * back-compat alias per OR docs). Defaults to `https://inbrain.ai` / `inbrain`;
 ...
 * attribution on OR's leaderboard instead of polluting inbrain's.
 *
 ...
 * hard-pins inbrain's subagent infra to Anthropic-direct (stable tool_use_id
...
export const openrouter: Recipe = {
  ...
  resolveDefaultHeaders(env) {
    const referer = env.OPENROUTER_REFERER ?? 'https://inbrain.ai';
    const title = env.OPENROUTER_TITLE ?? 'inbrain';
    return {
  ...
  touchpoints: {
    embedding: {
      ...
      // a smaller dim via `inbrain config set embedding_dimensions <N>`.
