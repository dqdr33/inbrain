/**
 * buildGatewayConfig — translate a stored InbrainConfig into the gateway's
...
export function buildGatewayConfig(c: InbrainConfig): AIGatewayConfig {
  // v0.32 (#121 reworked): when ~/.inbrain/config.json declares
  ...
  // the env-mapping at this seam never picked it up. `inbrain config set
  // zeroentropy_api_key X` wrote DB plane (ignored by gateway). The file-
  // plane field now exists (InbrainConfig type) and gets mapped here, so
  // setting it via `~/.inbrain/config.json` propagates into the gateway.
