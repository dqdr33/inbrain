 ...
 * prose and then calling the inbrain MCP tools the same server already exposes.
 *
 ...
 * inbrain treats `file_list`/`file_url` as `admin + localOnly` for exactly this
 ...
 *      OFF at runtime; new installs turn it on at `inbrain init`, existing installs
 ...
 *   5. No install_path serve for remote — a hosted inbrain with no agent repo
 *      returns `storage_error`, never inbrain's own bundled dev skills.
...
export const MAX_SKILL_MD_BYTES = (() => {
  const raw = process.env.INBRAIN_MAX_SKILL_MD_BYTES;
  if (raw) {
...
/**
 * Read `mcp.publish_skills` honoring BOTH config planes. `inbrain config set`
 * writes the DB plane (engine.setConfig) — so the DB value wins; `inbrain init`
...
/**
 * Read `mcp.skills_dir` from the DB plane (what `inbrain config set` writes),
 ...
 * inbrain must never serve its own bundled dev skills); LOCAL callers
...
export function resolveSkillsDir(
  ...
  if (skillsDirOverride && skillsDirOverride.trim().length > 0) {
    if (!existsSync(skillsDirOverride)) {
      throw new OperationError(
        ...
        'Fix it with `inbrain config set mcp.skills_dir <path>` or unset it to autodetect.',
      );
    }
  ...
  if (!det.dir || !det.source) {
    ...
    throw new OperationError(
      ...
      `Set it explicitly: \`inbrain config set mcp.skills_dir <path>\` (or $INBRAIN_SKILLS_DIR). Search order:\n${hint}`,
    );
  }
...
export function getSkillDetail(
  ...
  if (size > MAX_SKILL_MD_BYTES) {
    throw new OperationError(
      ...
      'Raise INBRAIN_MAX_SKILL_MD_BYTES if this is a legitimately large skill.',
    );
  }
...
export function assertPublishEnabled(ctx: OperationContext, publishSkills: boolean): void {
  ...
  throw new OperationError(
    ...
    'The owner can enable it with `inbrain config set mcp.publish_skills true`.',
  );
}
