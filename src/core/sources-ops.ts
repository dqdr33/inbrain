/**
 * inbrain sources-ops — pure async functions for source-management operations
 ...
 *                              │
 *                              ▼  (id free)
 *                  mkdir $INBRAIN_HOME/clones/.tmp/<id>-<rand>/
 *                              │
 *                              ▼
 ...
 * is symlink-unsafe and would let $INBRAIN_HOME/clones/<id> → /etc resolve
 * out of the confine.
 */
...
export interface SourceRow {
   ...
   * mode bundle. Written only by `inbrain sources set-cr-mode <id> <mode>`
   ...
   * `inbrain mounts trust-frontmatter <id>`. Host source (id='default') is
...
export interface AddSourceOpts {
  ...
  /**
   * Override clone destination. Defaults to $INBRAIN_HOME/clones/<id>/.
 ...
 * sources-ops-tagged error so `inbrain sources add` keeps its user-facing
...
/** Default clone dir for a remote-URL source: $INBRAIN_HOME/clones/<id>/ */
export function defaultCloneDir(id: string): string {
  return inbrainPath('clones', id);
}

/** Temp clone dir under $INBRAIN_HOME/clones/.tmp/<id>-<rand>/ */
function makeTempCloneDir(id: string): string {
  const rand = randomBytes(6).toString('hex');
  return inbrainPath('clones', '.tmp', `${id}-${rand}`);
}
 ...
 * $INBRAIN_HOME/clones/<id> → /etc bypass the confine.
 *
...
/**
 * Did inbrain CREATE this clone (so re-clone/delete is safe)? Ownership, NOT
 * path-containment — a user-supplied working tree is NEVER owned, even if it
 * happens to sit under $INBRAIN_HOME. This is the #1881 guard: recloneIfMissing
 * deletes local_path, so it must only ever fire on a clone inbrain owns.
 *
 ...
 *      before the marker existed (inbrain's default location), via exact
...
export function unownedHint(
  ...
  if (state === 'healthy') {
    return (
      ...
      `clone inbrain created. inbrain syncs it read-only and will never re-clone or delete ` +
      `it. To silence this, drop config.remote_url, or re-register with --url so inbrain ` +
      `owns the clone.`
    );
  }
  return (
    `Source "${src.id}" has config.remote_url set but local_path ${path} is not a clone ` +
    `inbrain created and is not a usable git repo (state: ${state}). inbrain will NOT ` +
    `re-clone over it (it is your working tree, not a inbrain-managed mirror). Restore the ` +
    `directory yourself, or remove + re-add the source with --url to let inbrain manage the ` +
    `clone.`
  );
}
...
export async function addSource(
  ...
  if (existing.length > 0) {
    throw new SourceOpError(
      ...
      `Source id "${opts.id}" is already registered. ` +
        `Use 'inbrain sources remove ${opts.id} --confirm-destructive' first, then re-add.`,
    );
  }
  ...
  if (parsedUrl) {
    ...
    // recloneIfMissing to rm+replace this clone — inbrain created it, here or at
    ...
    // rename) is rare on a single-host brain but possible if $INBRAIN_HOME
    // and the temp dir are on different mounts. We don't fall back to
    // recursive copy because the temp dir is in $INBRAIN_HOME by design.
    try {
...
export async function resolveDefaultSource(engine: BrainEngine): Promise<string> {
  ...
  if (sources.length === 0) {
    throw new SourceResolutionError(
      'no sources registered; run `inbrain sources add` first',
      'no_sources',
      [],
    );
  }
 ...
 * seed_default), so a `.inbrain-source` pin (or any real signal) selects the
 ...
 *    confined under $INBRAIN_HOME/clones/. realpath+lstat (not startsWith)
...
export async function removeSource(
  ...
  const cloneRoot = inbrainPath('clones');
  ...
  if (
    ...
    try {
      ...
      console.error(
        `[inbrain] WARN: clone cleanup at ${src.local_path} failed: ${(e as Error).message}`,
      );
    }
  }
 ...
 * missing on disk. Used by `inbrain sources restore` after an operator
 * autopurged $INBRAIN_HOME/clones/. Idempotent: returns false (didn't clone)
...
export async function recloneIfMissing(
  ...
  // deletes local_path; inbrain may only do that to a clone it created, never a
  ...
  const rand = randomBytes(6).toString('hex');
  const tempDir = join(parent, `.inbrain-reclone-${basename(src.local_path)}-${rand}`);
  try {
  ...
  try {
    if (existsSync(src.local_path)) {
      ...
      // is a real dir inbrain created; a symlink here means tamper, so fail closed.
      // (Symlinked ANCESTORS are intentionally NOT rejected here: for an owned
      // clone inbrain created the dir at this path — cloneRepo refuses a non-empty
