 ...
 *   - `inbrain dream` (CLI alias; one-shot cron-triggered cycle)
 *   - `inbrain autopilot` (daemon; scheduled on an interval)
 ...
 * COORDINATION:
 *
 * Postgres: a row in inbrain_cycle_locks with a TTL (30 min). Refreshed
 ...
 *
 * PGLite / engine=null: a file lock at ~/.inbrain/cycle.lock holding
...
import { inbrainPath } from './config.ts';
...
export type CyclePhase =
  ...
  //    to a review queue. User accepts/rejects via `inbrain takes propose`.
  ...
  //    active pack's `phases:` declaration (inbrain-creator or inbrain-
...
export const ALL_PHASES: CyclePhase[] = [
  ...
  // declaration (inbrain-creator + inbrain-everything declare; others skip).
  'extract_atoms',
  ...
  // `inbrain config set cycle.skillopt.enabled true`. Bundled-skill safety
...
export interface CycleReport {
  ...
  totals: {
     ...
     * cycle by `INBRAIN_PHANTOM_REDIRECT_LIMIT` (default 50).
     */
     ...
     * the `~/.inbrain/audit/phantoms-YYYY-Www.jsonl` audit log.
     */
...
export interface CycleOpts {
   ...
   * - `synthInputFile`: ad-hoc transcript path (`inbrain dream --input <file>`).
   ...
   * Wired from `inbrain dream --unsafe-bypass-dream-guard`. Never auto-applied
   ...
   * `inbrain-cycle:<source_id>` instead of the legacy global `inbrain-cycle`,
 ...
 * time use this row in `inbrain_cycle_locks`.
 */
const LEGACY_CYCLE_LOCK_ID = 'inbrain-cycle';
...
// Lazy: INBRAIN_HOME may be set after module load; resolve at call time.
const getLockFilePathDefault = () => inbrainPath('cycle.lock');
 ...
 *
 * - `undefined` returns the legacy `'inbrain-cycle'` ID, preserving
 *   back-compat for every existing caller (autopilot, `inbrain dream`
 ...
 * - Valid IDs return `'inbrain-cycle:<source_id>'` so per-source cycles
 *   acquire distinct rows in `inbrain_cycle_locks` and don't serialize
...
export async function runPhaseBacklinks(brainDir: string, dryRun: boolean): Promise<PhaseResult> {
  try {
    ...
    // filesystem fixer remains available explicitly via `inbrain check-backlinks
    ...
    return {
      ...
      summary: gaps === 0
        ...
        : `${gaps} missing back-link(s) found (audit-only; run inbrain check-backlinks fix to materialize)`,
...
// its own phase participation explicitly. The inbrain-everything meta-
...
// that extends inbrain-creator may NOT want extract_atoms to run (e.g. they
  ...
  try {
    ...
    const result = await performSync(engine, {
      ...
      noExtract: willRunExtractPhase,      // dedupe ONLY when cycle's extract phase will also run.
                                           // If extract isn't scheduled (e.g. `inbrain dream --phase sync`),
    ...
    if (result.guardTriggered) {
      return {
        ...
        details: {
          ...
          hint: 'inbrain apply-migrations --yes',
...
async function runPhaseEmbed(engine: BrainEngine, dryRun: boolean, signal?: AbortSignal): Promise<PhaseResult> {
  try {
    ...
    // after the job was killed — which left inbrain_cycle_locks held and
 ...
 *
 * Mirrors the operator escape hatches: `inbrain sources purge` (no id) and
 * `inbrain pages purge-deleted` both call the same library functions, so
 * scripted purges and the autopilot phase converge on a single behavior.
 */
/**
 * v0.28 P1: sweep $INBRAIN_HOME/clones/.tmp/ for entries older than the
...
export async function runCycle(
  ...
  // this, `inbrain dream --source repo-a` on a checkout-less brain would scope
  ...
  if (needsLock) {
    if (engine) {
      ...
      try {
        ...
        // `inbrain-cycle` otherwise (autopilot still passes nothing).
  ...
  try {
    ...
    // (e.g. user is on inbrain-base or inbrain-investor), this phase is a
    // no-op with reason='not_in_active_pack'. When the pack does declare
    // it (inbrain-creator, inbrain-everything), dispatches to the
    ...
    if (phases.includes('extract_atoms')) {
      ...
      if (!engine) {
        ...
        phaseResults.push({
          ...
          status: 'skipped',
          duration_ms: 0,
          summary: 'extract_atoms: active pack does not declare this phase (run `inbrain dream --phase extract_atoms --drain` to drain a backlog)',
    ...
    // to user data — output goes to ~/.inbrain/audit/schema-events-*.jsonl
    // (T15) and the disk-derived candidate set surfaced by `inbrain schema
    // review-candidates`.
...
function deriveStatus(phases: PhaseResult[], totals: CycleReport['totals']): CycleStatus {
  ...
  const anyWork =
    ...
    // A7: a code brain runs `inbrain dream` specifically to build the call graph
