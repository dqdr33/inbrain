 ...
 * inbrain's --progress-json events. Pure local fs; no DB, no network.
 */
...
/** Default fixtures root, override via $INBRAIN_CLAW_SCENARIOS_DIR for tests. */
function defaultFixturesRoot(): string {
  if (process.env.INBRAIN_CLAW_SCENARIOS_DIR) {
    return resolve(process.env.INBRAIN_CLAW_SCENARIOS_DIR);
  }
