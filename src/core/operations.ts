...
import type { InbrainConfig } from './config.ts';
 ...
 * inbrain-evals) get autocomplete on the named codes AND remain TS-forward-
 * compatible when inbrain adds new codes in future releases. This shape is
...
export interface OperationContext {
  engine: BrainEngine;
  config: InbrainConfig;
  logger: Logger;
  dryRun: boolean;
  /**
   * OAuth auth info (v0.8+). Present when the caller authenticated via OAuth 2.1
   * through `inbrain serve --http`. Contains clientId and granted scopes for
  ...
  /**
   * Connected-inbrains brain id (v0.19+ / v0.26 mounts). Identifies which brain
   * this op is targeting. 'host' for the default brain configured in
   * ~/.inbrain/config.json; otherwise a mount id registered in ~/.inbrain/mounts.json.
   *
   ...
   * (INBRAIN_SOURCE) / `.inbrain-source` dotfile / per-token sources scope
...
export function resolvePerCallMode(ctx: OperationContext, raw: unknown): string | undefined {
  ...
  if (!isSearchMode(raw)) {
    throw new OperationError(
      ...
      `inbrain search "<query>" --mode balanced`,
    );
  }
  return raw;
}
...
const put_page: Operation = {
  ...
  handler: async (ctx, p) => {
      ...
      try {
        ...
        if (pageType && !knownTypes.has(pageType)) {
          ...
          if (process.stderr.isTTY && ctx.remote === false) {
            console.error(
              `[schema] put_page wrote type=\`${pageType}\` which isn't in active pack \`${activePack.page_types.length ? '<configured>' : 'inbrain-base'}\`. ` +
              `Run \`inbrain schema review-candidates\` to promote or ignore.`,
            );
          }
        }
      } catch {
    ...
    if (!ctx.dryRun && result.status !== 'error' && !isSandboxSubagent) {
      ...
      // Shared canonical write-through (also used by `inbrain brainstorm/lsd
    ...
    // ingest_log + ~/.inbrain/validator-lint.jsonl. Does NOT reject the
...
const query: Operation = {
  ...
  params: {
    ...
    salience: {
      ...
      description:
        ...
        "Omit and inbrain auto-detects from query text. Independent of `recency`.",
    },
    recency: {
      type: 'string',
      enum: ['off', 'on', 'strong'],
      description:
        ...
        "Omit and inbrain auto-detects. Independent of `salience` (orthogonal axes).",
    },
    since: {
      type: 'string',
      description:
    ...
    source_id: {
      type: 'string',
      description:
        "v0.34: scope search to a single source. Defaults to OperationContext.sourceId (set from CLI --source / INBRAIN_SOURCE / .inbrain-source dotfile). Pass '__all__' to span every source for trusted local callers; for remote callers '__all__' spans only your granted sources.",
    },
    cross_modal: {
      type: 'string',
  ...
  handler: async (ctx, p) => {
    ...
    // Op-layer capture (v0.25.0). Fire-and-forget. meta tells inbrain-evals
...
const think: Operation = {
  ...
  params: {
    ...
    model: { type: 'string', description: 'Model override (alias or full id). Falls through models.think → models.default → INBRAIN_MODEL → opus.' },
...
const add_link: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    const linkOpts = ctx.sourceId
      ...
      : undefined;
    await ctx.engine.addLink( // inbrain-allow-direct-insert: add_link MCP op is the explicit canonical surface for manual link creation; auto-link reconciliation runs separately via auto_link post-hook
...
const traverse_graph: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    if (requestedDepth > TRAVERSE_DEPTH_CAP) {
      ctx.logger.warn(`[inbrain] traverse_graph depth clamped from ${requestedDepth} to ${TRAVERSE_DEPTH_CAP}`);
    }
...
const add_timeline_entry: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    // v0.31.8 (D7): thread ctx.sourceId.
    const sourceOpts = ctx.sourceId ? { sourceId: ctx.sourceId } : {};
    await ctx.engine.addTimelineEntry(p.slug as string, { // inbrain-allow-direct-insert: add_timeline_entry MCP op is the explicit canonical surface for manual timeline entries
      date,
...
const get_brain_identity: Operation = {
  ...
  handler: async (ctx) => {
    ...
    // (bonus channel; the CLI stderr marker + `inbrain self-upgrade` are the
...
/**
 * v0.41.19.0 — `inbrain status` thin-client surface.
 *
 * Returns a snapshot of sync freshness + last cycle state for thin-client
 * `inbrain status` callers. Per D2/D10 in the plan:
 *
 ...
 *     callers shouldn't see at all (and the local `inbrain status` renders
 ...
 *     directly (no MCP round-trip when running against ~/.inbrain).
 */
const get_status_snapshot: Operation = {
  name: 'get_status_snapshot',
  description: 'Snapshot for `inbrain status` thin-client mode: sync freshness + last cycle. Admin-scope.',
  params: {},
  handler: async (ctx) => {
 ...
 * side renderer lives in `src/commands/remote.ts` (used by `inbrain remote
...
const file_url: Operation = {
  ...
  handler: async (_ctx, p) => {
    ...
    return { storage_path: rows[0].storage_path, url: `inbrain:files/${rows[0].storage_path}` };
  },
};
...
const submit_job: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    // INBRAIN_ALLOW_SHELL_JOBS env flag — even if that flag is on, MCP callers
...
const submit_agent: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    // binding check — `inbrain agent run` already runs through subagent.ts
    // directly without going through this op.
    if (ctx.remote === false) {
      throw new OperationError('invalid_request', 'submit_agent over the local CLI: use `inbrain agent run` instead.');
    }
...
const find_orphans: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    // source-isolation class. Local CLI callers route through `inbrain
...
// v0.33: expertise + relationship-proximity routing. CLI: inbrain whoknows.
...
const find_contradictions: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    if (rows.length === 0) {
      return { contradictions: [], note: 'No probe runs in the last 30 days; run `inbrain eval suspected-contradictions` first.' };
    }
    const latest = rows[0];
...
const get_recent_transcripts: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    if (ctx.remote === true) {
      throw new OperationError(
        'permission_denied',
        'get_recent_transcripts is local-only — call via the inbrain CLI.',
      );
    }
...
const whoami: Operation = {
  ...
  handler: async (ctx) => {
    ...
    // OAuth tokens have client_id starting with 'inbrain_cl_'; legacy
    ...
    const isOauth = ctx.auth.clientId.startsWith('inbrain_cl_');
    if (isOauth) {
      return {
        transport: 'oauth',
...
const sources_add: Operation = {
  ...
  description:
    ...
    'SSRF gate, clones into $INBRAIN_HOME/clones/<id>/ via temp-dir + rename ' +
  ...
  params: {
    ...
    url: {
      type: 'string',
      description:
        'HTTPS git URL. Cloned into $INBRAIN_HOME/clones/<id>/. SSRF-guarded.',
    },
    federated: {
      type: 'boolean',
    ...
    clone_dir: {
      type: 'string',
      description:
        'Override clone destination (only valid with url). Default: $INBRAIN_HOME/clones/<id>/.',
    },
  },
  mutating: true,
  scope: 'sources_admin',
  handler: async (ctx, p) => {
    ...
    //   "source"; later `inbrain sync --all` walks every sources.local_path,
    ...
    //   arbitrary-delete.
    //
    // Both fields are CLI-only (the operator runs `inbrain sources add --path
    // /home/me/notes`). For HTTP MCP, ignore overrides — clone_dir defaults
    // to $INBRAIN_HOME/clones/<id>/ and path is rejected. Local CLI callers
    ...
    if (!isLocal && (p.path !== undefined || p.clone_dir !== undefined)) {
      ctx.logger.warn(
        '[sources_add] ignoring path/clone_dir overrides on HTTP MCP transport ' +
          ...
          'fixed under $INBRAIN_HOME/clones/).',
      );
    }
...
const sources_remove: Operation = {
  ...
  description:
    ...
    'under $INBRAIN_HOME/clones/ (realpath+lstat — symlink-safe). For most ' +
...
const forget_fact: Operation = {
  name: 'forget_fact',
  description: 'v0.32.2: forget a fact. Rewrites the page\'s `## Facts` fence to strike through the row and set valid_until=today (the DB\'s expired_at derives via valid_until + now() on the next reconcile so the forget survives `inbrain rebuild`). Falls back to legacy DB-only expire for pre-v51 / thin-client rows. Idempotent on already-expired or unknown ids.',
  params: {
...
// the CLI_ONLY set at cli.ts:30 — agents calling inbrain via MCP couldn't reach
...
const search_by_image: Operation = {
  ...
  description:
    ...
    '(`inbrain reindex --multimodal` + `search.unified_multimodal: true`).',
  params: {
...
const list_schema_packs: Operation = {
  ...
  handler: async (_ctx) => {
    ...
    const { inbrainPath } = await import('./config.ts');
    const bundled = ['inbrain-base', 'inbrain-recommended'];
    const installedDir = inbrainPath('schema-packs');
...
const schema_lint: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    const { loadActivePack } = await import('./schema-pack/load-active.ts');
    const { loadConfig, inbrainPath } = await import('./config.ts');
    ...
    if (p.pack) {
      ...
      for (const c of candidates) {
        const candidate = join(inbrainPath('schema-packs', packName), c);
...
// brain installs to drive `inbrain onboard --auto` over MCP. Admin scope
...
const run_skillopt: Operation = {
  name: 'run_skillopt',
  description: 'Run SkillOpt against a single skill. Admin scope; mutating; rate-limited per-skill via DB lock. See inbrain skillopt CLI for the full flag surface.',
  params: {
...
export const operations: Operation[] = [
  ...
  // v0.41.19.0: thin-client `inbrain status` payload (admin-scope, sync + cycle only)
  get_status_snapshot,
  // Sync
  sync_brain,
  // Raw data
