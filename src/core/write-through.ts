 ...
 * round-trips cleanly through `inbrain sync`. The file is rendered FROM the DB
 * row, so the two sinks cannot diverge.
 *
 * Extracted from the v0.38 `put_page` write-through (operations.ts) so the
 * `put_page` op AND `inbrain brainstorm/lsd --save` share one implementation
 ...
 * into a live git tree that `inbrain sync` / autopilot actively walk, so a crash
 ...
 * best-effort and reconciled by the next `inbrain sync`).
 */
