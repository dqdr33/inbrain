 ...
 * glosses. Drives `inbrain search stats` JSON output (`_meta.metric_glossary`
 * block), `inbrain eval compare` reports, and the auto-generated
 ...
 * to machine-readable consumers (inbrain-evals repo, CI gates).
 *
 ...
 * `inbrain search stats` JSON output's _meta.metric_glossary block and in
...
export function renderMetricGlossaryMarkdown(): string {
  ...
  lines.push('');
  lines.push('Every metric `inbrain eval *` and `inbrain search stats` reports has a plain-English explanation here. Industry terms are preserved verbatim so users searching the literature find what we report.');
  lines.push('');
  ...
  lines.push('## Coverage');
  lines.push('');
  lines.push(`Every metric printed by any \`inbrain eval *\` or \`inbrain search stats\` command resolves through \`getMetricGloss()\` in \`src/core/eval/metric-glossary.ts\`. Adding a new metric to the glossary REQUIRES updating this doc; the CI guard catches drift.`);
  lines.push('');
