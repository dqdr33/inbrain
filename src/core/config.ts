 ...
 * attempt. Used by `inbrain doctor --fast` so the user gets a precise message
 * instead of the misleading "No database configured" when INBRAIN_DATABASE_URL
...
// INBRAIN_HOME is honored uniformly. Lazy: never call homedir() at module scope.
function getConfigDir() { return configDir(); }
function getConfigPath() { return configPath(); }

export interface InbrainConfig {
   ...
   * slot. `inbrain config set zeroentropy_api_key X` wrote DB plane,
   ...
   * `inbrain init --no-embedding`. When true, embed callsites and `inbrain
   * import` refuse with a `inbrain config set embedding_model <id>` hint
   ...
   * at process boot (NOT `inbrain config set` which writes the DB plane —
   * those are different stores). Edit `~/.inbrain/config.json` directly.
  ...
  autopilot?: {
     ...
     * each autopilot tick. Disable with `inbrain config set autopilot.auto_drain.enabled false`.
     */
    auto_drain?: {
   ...
   * runtime behavior. They live in the DB plane primarily — `inbrain config
   ...
   * the DB plane (`inbrain config set ...`) so users can flip without
  ...
  content_sanity?: {
     ...
     *  Env override: `INBRAIN_NO_SANITY=1` flips to true. */
    disabled?: boolean;
   ...
   * `INBRAIN_DREAM_*` env vars; do not add an env layer without first
   ...
   * have a local DB; it talks to a remote `inbrain serve --http` over MCP.
   ...
   * `INBRAIN_REMOTE_CLIENT_SECRET` env var (preferred for headless agents);
   ...
   * Default: `inbrain-base` (reproduces pre-v0.38 hardcoded behavior).
   *
   * Resolution priority (highest → lowest, per D13):
   *   1. Per-call SearchOpts.schema_pack (CLI-only; rejected for remote callers)
   *   2. INBRAIN_SCHEMA_PACK env var
   *   3. Per-source DB config `schema_pack.source.<id>`
   *   4. Brain-wide DB config `schema_pack`
   *   5. inbrain.yml `schema:` section
   *   6. THIS field (~/.inbrain/config.json)
   *   7. Default 'inbrain-base'
   *
   * `inbrain config set schema_pack <name>` writes the DB plane (tier 4);
   ...
   * `inbrain serve`. See `src/core/skill-catalog.ts` for the trust-boundary memo.
   */
  mcp?: {
    /**
     ...
     * existing read tokens host-skill read. `inbrain init` writes `true` for new
     ...
     * tier so a hosted inbrain never serves its own bundled dev skills).
     */
    skills_dir?: string;
  };
}
 ...
 * `inbrain serve --http`. Single source of truth for the "is this a
 * thin-client install?" check used by the CLI dispatch guard, doctor
 * branch, and remote subcommands.
 */
export function isThinClient(config: InbrainConfig | null): boolean {
...
function migrateLegacyEmbeddingConfig(raw: Record<string, unknown>): Record<string, unknown> {
  ...
  console.warn(
    ...
    ` Rewrite ~/.inbrain/config.json to: "embedding_model": "${rest.embedding_model}".`,
  );
  return rest;
}

/**
 * File-only config loader. Reads ~/.inbrain/config.json and applies the
 * legacy embedding-config migration shim. Does NOT merge env vars, does
 * NOT infer engine kind from DATABASE_URL.
 *
 * Used by `inbrain init`'s config-merge path (B.4) where loading
 ...
 */
export function loadConfigFileOnly(): InbrainConfig | null {
  try {
    const raw = readFileSync(getConfigPath(), 'utf-8');
    const parsed = JSON.parse(raw) as Record<string, unknown>;
    return migrateLegacyEmbeddingConfig(parsed) as unknown as InbrainConfig;
  } catch {
    return null;
  }
}

export function loadConfig(): InbrainConfig | null {
  let fileConfig: InbrainConfig | null = null;
  try {
    const raw = readFileSync(getConfigPath(), 'utf-8');
    const parsed = JSON.parse(raw) as Record<string, unknown>;
    fileConfig = migrateLegacyEmbeddingConfig(parsed) as unknown as InbrainConfig;
  } catch { /* no config file */ }

  // Try env vars
  const dbUrl = process.env.INBRAIN_DATABASE_URL || process.env.DATABASE_URL;
  ...
  const merged = {
    ...
    ...(process.env.INBRAIN_EMBEDDING_MODEL ? { embedding_model: process.env.INBRAIN_EMBEDDING_MODEL } : {}),
    ...(process.env.INBRAIN_EMBEDDING_DIMENSIONS ? { embedding_dimensions: parseInt(process.env.INBRAIN_EMBEDDING_DIMENSIONS, 10) } : {}),
    ...(process.env.INBRAIN_EXPANSION_MODEL ? { expansion_model: process.env.INBRAIN_EXPANSION_MODEL } : {}),
    ...(process.env.INBRAIN_CHAT_MODEL ? { chat_model: process.env.INBRAIN_CHAT_MODEL } : {}),
    ...(process.env.INBRAIN_CHAT_FALLBACK_CHAIN
      ? { chat_fallback_chain: process.env.INBRAIN_CHAT_FALLBACK_CHAIN.split(',').map(s => s.trim()).filter(Boolean) }
      : {}),
    ...(process.env.INBRAIN_EMBEDDING_MULTIMODAL
      ? { embedding_multimodal: process.env.INBRAIN_EMBEDDING_MULTIMODAL === 'true' }
      : {}),
    ...(process.env.INBRAIN_EMBEDDING_IMAGE_OCR
      ? { embedding_image_ocr: process.env.INBRAIN_EMBEDDING_IMAGE_OCR === 'true' }
      : {}),
    ...(process.env.INBRAIN_EMBEDDING_MULTIMODAL_MODEL
      ? { embedding_multimodal_model: process.env.INBRAIN_EMBEDDING_MULTIMODAL_MODEL }
      : {}),
    ...(process.env.INBRAIN_EMBEDDING_IMAGE_OCR_MODEL
      ? { embedding_image_ocr_model: process.env.INBRAIN_EMBEDDING_IMAGE_OCR_MODEL }
      : {}),
    ...(process.env.INBRAIN_REMOTE_CLIENT_SECRET && fileConfig?.remote_mcp
      ? { remote_mcp: { ...fileConfig.remote_mcp, oauth_client_secret: process.env.INBRAIN_REMOTE_CLIENT_SECRET } }
      : {}),
  };
  ...
  // The env vars use natural names (INBRAIN_NO_SANITY=1 is more
  // operator-friendly than INBRAIN_CONTENT_SANITY_DISABLED=true).
  const envContentSanity: InbrainConfig['content_sanity'] = {};
  if (process.env.INBRAIN_PAGE_WARN_BYTES) {
    const n = parseInt(process.env.INBRAIN_PAGE_WARN_BYTES, 10);
    if (Number.isFinite(n) && n > 0) envContentSanity.bytes_warn = n;
  }
  if (process.env.INBRAIN_PAGE_BLOCK_BYTES) {
    const n = parseInt(process.env.INBRAIN_PAGE_BLOCK_BYTES, 10);
    if (Number.isFinite(n) && n > 0) envContentSanity.bytes_block = n;
  }
  if (process.env.INBRAIN_NO_JUNK_PATTERNS === '1') {
    envContentSanity.junk_patterns_enabled = false;
  }
  if (process.env.INBRAIN_NO_SANITY === '1') {
    envContentSanity.disabled = true;
  }
  if (process.env.INBRAIN_MAX_MARKUP_RATIO) {
    const n = parseFloat(process.env.INBRAIN_MAX_MARKUP_RATIO);
  ...
  if (Object.keys(envContentSanity).length > 0) {
    (merged as InbrainConfig).content_sanity = {
      ...(fileConfig?.content_sanity ?? {}),
      ...envContentSanity,
    };
  }

  return merged as InbrainConfig;
}

/**
 * v0.27.1 — async config loader that overlays DB-plane config on top of the
 * file/env config. Used by `inbrain` CLI's connectEngine() AFTER engine.connect()
 * so flags written via `inbrain config set` actually take effect. Unlike the
...
export async function loadConfigWithEngine(
  ...
  base?: InbrainConfig | null,
): Promise<InbrainConfig | null> {
  ...
  // `inbrain config set`. Since we have a live engine here, synthesize a
  ...
  const fileConfig: InbrainConfig =
    (base !== undefined ? base : loadConfig()) ??
    ({ engine: 'postgres' } as InbrainConfig);
  ...
  const merged: InbrainConfig = { ...fileConfig };
  ...
  if (merged.embedding_columns === undefined && dbEmbeddingColumns !== undefined) {
    try {
      ...
      if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        ...
        console.warn('[inbrain] config: embedding_columns DB value is not a JSON object; ignoring');
      }
    } catch (err) {
      console.warn(`[inbrain] config: embedding_columns DB value is not valid JSON; ignoring (${(err as Error).message})`);
    }
  }
  ...
  const mergedCS: NonNullable<InbrainConfig['content_sanity']> = { ...existingCS };
  ...
  // per key (NO env layer; see InbrainConfig.dream JSDoc). Without this,
  ...
  // config set via `inbrain config set`.
  ...
  const mergedSynth: NonNullable<NonNullable<InbrainConfig['dream']>['synthesize']> = { ...existingSynth };
  const mergedPatterns: NonNullable<NonNullable<InbrainConfig['dream']>['patterns']> = { ...existingPatterns };
  ...
  if (Object.keys(mergedSynth).length > 0 || Object.keys(mergedPatterns).length > 0) {
    const mergedDream: NonNullable<InbrainConfig['dream']> = {};
...
/**
 * v0.37 (D6): canonical list of known config keys for `inbrain config set`
 * validation. Includes both the static InbrainConfig fields (file plane)
 ...
 *   1. Add it to the InbrainConfig interface (if file-plane) OR document it
 *      below (if DB-plane).
 *   2. Add the canonical name to this list so `inbrain config set` accepts it
 *      without `--force`.
 */
export const KNOWN_CONFIG_KEYS: readonly string[] = [
  // File-plane (InbrainConfig static fields)
  'engine',
  'database_url',
  'database_path',
  'openai_api_key',
 ...
 * Levenshtein suggestion in `inbrain config set`.
 */
...
export function saveConfig(config: InbrainConfig): void {
  ...
  // where `~/.inbrain/` lives inside a git worktree (Conductor + gstack
...
/**
 * Idempotently lay down `~/.inbrain/.gitignore` containing the single line `*`.
 * Honors INBRAIN_HOME via `configDir()`. Best-effort: errors are logged to
 ...
 *   - `inbrain post-upgrade` so existing users get it on next upgrade.
 *
 * What this DOES cover: a casual `git add ~/.inbrain` from inside an enclosing
 ...
 *   - `git add -f ~/.inbrain` (deliberate force-add bypasses .gitignore).
...
export function ensureGitignore(): void {
  try {
    ...
    process.stderr.write(`[inbrain] ensureGitignore failed (${msg}); continuing\n`);
  }
}

export function toEngineConfig(config: InbrainConfig): EngineConfig {
  return {
...
export function configDir(): string {
  ...
  // INBRAIN_HOME is a parent dir; we always append '.inbrain' ourselves so
  // setting INBRAIN_HOME=/tmp/x yields configDir() === '/tmp/x/.inbrain'.
  // Validates the override: must be absolute, no '..' segments.
  const override = process.env.INBRAIN_HOME;
  if (override && override.trim()) {
    const trimmed = override.trim();
    if (!isAbsolute(trimmed)) {
      throw new Error(`INBRAIN_HOME must be an absolute path; got: ${trimmed}`);
    }
    if (trimmed.split(/[\\/]/).includes('..')) {
      throw new Error(`INBRAIN_HOME must not contain '..' segments; got: ${trimmed}`);
    }
    return join(trimmed, '.inbrain');
  }
  return join(homedir(), '.inbrain');
}
...
/**
 * Sugar for joining paths under the active inbrain home. Use this anywhere you
 * would otherwise write `join(homedir(), '.inbrain', ...rest)`. Honors
 * INBRAIN_HOME, validates input, and centralizes the convention so future
 * audits stay simple.
 */
export function inbrainPath(...segments: string[]): string {
...
export function getDbUrlSource(): DbUrlSource {
  if (process.env.INBRAIN_DATABASE_URL) return 'env:INBRAIN_DATABASE_URL';
  if (process.env.DATABASE_URL) return 'env:DATABASE_URL';
  if (!existsSync(configPath())) return null;
  try {
    const raw = readFileSync(configPath(), 'utf-8');
    const parsed = JSON.parse(raw) as Partial<InbrainConfig>;
