/**
 * Self-upgrade decision + state foundation (v0.42 self-upgrading-inbrain wave).
 *
 * Mirrors gstack's invocation-riding update mechanism for inbrain: a throttled
 * check that rides every `inbrain` invocation (CLI / MCP), emits a marker, and
 ...
 * always the hardcoded `inbrain upgrade` / `inbrain self-upgrade`).
 *
 * NO DB. The hot path runs before `connectEngine()` and thin clients have no
 * local DB, so all state is file-based under `~/.inbrain/` (honors INBRAIN_HOME).
 */
...
export function updateCachePath(): string {
  return inbrainPath('last-update-check');
}

export function snoozePath(): string {
  return inbrainPath('update-snoozed');
}

export function justUpgradedPath(): string {
  return inbrainPath('just-upgraded-from');
}

/**
 * Record the version we just upgraded FROM, so the next `inbrain` invocation's
 ...
 * Takes a loosely-typed config so it doesn't pull the full InbrainConfig type
 * onto the hot path. Env (`INBRAIN_SELF_UPGRADE_MODE`) is the operator / CI
 * escape hatch.
 */
export function resolveSelfUpgradeMode(
  cfg: { self_upgrade?: { mode?: string } } | null | undefined,
): SelfUpgradeMode {
  const env = normalizeMode(process.env.INBRAIN_SELF_UPGRADE_MODE);
  if (env) return env;
