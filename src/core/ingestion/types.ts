 ...
 *     `inbrain capture`) leave it false.
 *
...
/**
 * Contract version stamped on every inbrain.plugin.json that ships an
 ...
 * deprecation window.
 */
export const INGESTION_SOURCE_API_VERSION = 'inbrain-ingestion-source-v1';
 ...
 * `inbrain doctor ingestion_health`. Optional — sources that don't implement
...
export interface IngestionSourceContext {
  ...
  /** Source-specific config resolved at daemon startup from inbrain.yml
   *  (built-in sources) or inbrain.plugin.json default_config + per-install
