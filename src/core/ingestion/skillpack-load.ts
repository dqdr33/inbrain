 ...
 * (which loads subagent definitions); shares the same INBRAIN_PLUGIN_PATH
 * discovery mechanism and inbrain.plugin.json manifest format, but reads a
 * different optional field (`ingestion_sources`) and produces a different
 * shape (factory functions, not subagent definitions).
 *
 * A skillpack that ships an ingestion source adds to its inbrain.plugin.json:
 *
 *   {
 *     "name": "granola-source",
 ...
 * the daemon. The TOFU prompt during `inbrain skillpack scaffold` is the user
...
const SUPPORTED_PLUGIN_VERSION = 'inbrain-plugin-v1';
...
export interface LoadSkillpackSourcesOpts {
  /** Override the INBRAIN_PLUGIN_PATH env (for tests). */
  envPath?: string;
...
/**
 * Discover and load every IngestionSource from INBRAIN_PLUGIN_PATH. Iteration
...
export async function loadSkillpackSources(
  ...
  const raw = opts.envPath ?? process.env.INBRAIN_PLUGIN_PATH ?? '';
  ...
  // inbrain.yml — and we want the warning to name both sides so the user
  // can pick.
  ...
  for (const p of paths) {
    ...
    const manifestPath = path.join(p, 'inbrain.plugin.json');
    ...
    if (manifest.plugin_version !== SUPPORTED_PLUGIN_VERSION) {
      result.warnings.push(
        `[ingestion-load] unsupported plugin_version '${manifest.plugin_version}' at ${manifestPath} ` +
          `(inbrain supports '${SUPPORTED_PLUGIN_VERSION}')`,
      );
      continue;
    }
      ...
      if (!COMPATIBLE_API_VERSIONS.has(decl.api_version)) {
        result.warnings.push(
          `[ingestion-load] ${manifest.name} source '${decl.kind}' declares ` +
            `api_version='${decl.api_version}' but inbrain expects ` +
            ...
            `IngestionSource contract) OR downgrade inbrain. Skillpack docs: ` +
            `https://github.com/inbrain-ai/inbrain/blob/master/docs/ingestion-source-skillpack.md`,
        );
        continue;
      }
