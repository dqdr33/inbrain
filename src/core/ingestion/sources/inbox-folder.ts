 ...
 *
 * Watches `~/.inbrain/inbox/` by default. When a file appears (anyone can
 ...
 * `~/.inbrain/inbox/.archived/YYYY-MM-DD/<filename>` so the user has a
...
export interface InboxFolderSourceOpts {
  ...
  id?: string;
  /** Inbox directory to watch. Required. Typical: ~/.inbrain/inbox/. */
  inboxDir: string;
...
export function createInboxFolderSource(opts: InboxFolderSourceOpts): IngestionSource {
  if (!opts.inboxDir || typeof opts.inboxDir !== 'string') {
    throw new Error('InboxFolderSource: inboxDir is required (typical: ~/.inbrain/inbox)');
  }
