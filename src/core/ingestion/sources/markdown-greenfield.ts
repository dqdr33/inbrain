 ...
 * your OpenClaw → inbrain epistemology migration.
 *
 ...
 * caller wires via inbrain capture --source markdown-greenfield).
 *
 * Walk:
 ...
 *   - Append a JSONL line to ~/.inbrain/audit/markdown-greenfield-failures-
 *     YYYY-Www.jsonl with {path, error, ts} per D12
 *   - Continue with remaining files (don't fail-fast on one bad row)
 *
 * CLI activation: inbrain capture --source markdown-greenfield
...
export interface MarkdownGreenfieldOpts {
  ...
  limit?: number;
  /** Audit JSONL output dir (default: ~/.inbrain/audit). */
  auditDir?: string;
...
export class MarkdownGreenfieldSource implements IngestionSource {
  ...
  constructor(opts: MarkdownGreenfieldOpts = {}) {
    ...
    this.opts = {
      ...
      limit: opts.limit,
      auditDir: opts.auditDir ?? join(homedir(), '.inbrain', 'audit'),
  ...
  /**
   * Derive the inbrain page slug from the absolute file path. Strips
