 ...
 *
 * Reads `~/.inbrain/junk-substrings.txt` (operator-maintained) and returns
...
/** Path to the operator literals file. Honors `INBRAIN_HOME` via
 *  `inbrainPath`. Resolved at load time so test fixtures can set
 *  `INBRAIN_HOME` to a tempdir per the test-isolation conventions in
 *  CLAUDE.md. */
function resolveLiteralsPath(): string {
  ...
  const { inbrainPath } = require('./config.ts');
  return inbrainPath('junk-substrings.txt');
}
