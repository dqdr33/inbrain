 ...
 * `inbrain sync --all`. Sibling of `DEFAULT_PARALLEL_WORKERS`; the two
 ...
 * in inbrain (e.g. `inbrain eval prune --older-than`):
 *
...
/**
 * Resolve `INBRAIN_MAX_CONNECTIONS` (a positive integer) or undefined when
...
export function resolveMaxConnections(): number | undefined {
  const raw = process.env.INBRAIN_MAX_CONNECTIONS;
 ...
 * doctor nudge tells the operator to lower `INBRAIN_POOL_SIZE`.
 *
 ...
 * inside a fan-out command (`inbrain sync --all` + per-source workers)
 ...
 * is the lone legacy caller that bypasses it (`INBRAIN_EMBED_CONCURRENCY
