/**
 * skillpack/harvest-lint.ts — privacy linter for `inbrain skillpack harvest`.
 *
 * Reads `~/.inbrain/harvest-private-patterns.txt` (one regex per line,
 * user-maintained) plus a small built-in default list of patterns that
 * commonly leak when harvesting from a personal fork into inbrain core:
 *
 *   - `\bWintermute\b` — the canonical private fork name (CLAUDE.md
 *     explicitly bans this from inbrain core)
 *   - common email regex
...
/** Default patterns shipped with inbrain (CLAUDE.md responsible-disclosure rule). */
...
export function runPrivacyLint(
  ...
  if (hits.length > 0) {
    throw new PrivacyLintError(
      `Privacy lint found ${hits.length} match(es) in harvested content. Harvest rolled back. Edit your skill, run the editorial genericization, or add a pattern exception to ${patternsPath ?? '~/.inbrain/harvest-private-patterns.txt'}.`,
      hits,
    );
  }
}
