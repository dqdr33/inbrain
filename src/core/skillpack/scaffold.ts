/**
 * skillpack/scaffold.ts — `inbrain skillpack scaffold <name>`.
 *
 * One-time, additive copy of a bundled skill into a host workspace. The
 * file-copy primitive is shared with harvest (host→inbrain) via
 ...
 *      user owns it. To update, run `inbrain skillpack reference <name>`
 *      and decide.
...
export function runScaffold(opts: ScaffoldOptions): ScaffoldResult {
  ...
  // symlink check or path confinement (the scaffold source is inbrain's
  // own trusted bundle).
