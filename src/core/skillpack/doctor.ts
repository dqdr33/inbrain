/**
 * skillpack/doctor.ts — `inbrain skillpack doctor` runner.
 *
...
export async function runDoctor(opts: DoctorOptions): Promise<DoctorResult> {
  ...
  // Log the run for the inbrain doctor activity surface.
  logSkillpackEvent({
    event: 'doctor_run',
    pack: manifest.name,
...
function buildResult(
  ...
  return {
    ...
    full_mode_hint:
      opts.mode === 'full'
        ? '--full mode runs the publish-gate test + LLM-judge + routing-eval suites in a sandbox. Implementation lands in a follow-up wave; for now use `inbrain skillpack test <pack-dir>` once W4 ships.'
        : null,
  };
}
      ...
      case 'bootstrap_runbook_present': {
        ...
        if (!existsSync(path)) {
          const stub = [
            ...
            '2. (edit me) agent: inbrain put_page wiki/_bootstrap-stub --frontmatter type=stub',
            '',
            '<!-- v0.36 contract: inbrain displays this post-scaffold but DOES NOT auto-execute. -->',
            '',
          ].join('\n');
