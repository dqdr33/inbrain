 ...
 * after `inbrain init` or `inbrain upgrade`.
 *
 * inbrain users typically interact through their host agent (openclaw,
 * claude-code) rather than the inbrain CLI directly. So an interactive
...
const V0_25_1_RECOMMENDED: RecommendedSkill[] = [
  ...
  },
  {
    ...
    description:
      'Universal archivist for personal file archives (Dropbox / B2 / Gmail-takeout). REFUSES to run without a inbrain.yml allow-list — safe-by-default.',
  },
  {
  ...
  return renderAdvisory({
    ...
    installCommand:
      missing.length === V0_25_1_RECOMMENDED.length
        ...
        : `inbrain skillpack install ${missing.map((s) => s.slug).join(' ')}`,
  });
}
...
function buildAdvisoryWithoutWorkspace(
  ...
  return renderAdvisory({
    ...
    installCommand: 'inbrain skillpack install --all',
...
function renderAdvisory(opts: {
  ...
  lines.push('');
  lines.push(bar);
  lines.push(`inbrain ${opts.version} — RECOMMENDED SKILLS FOR THE AGENT TO INSTALL`);
  lines.push(bar);
  lines.push('');
  lines.push(
    `The user just ${verb} inbrain ${opts.version}. This release ships ${opts.missing.length} ` +
  ...
  if (opts.workspaceNotDetected) {
    lines.push(
      '(NOTE: No OpenClaw workspace was detected at the default location. ' +
        'Set $OPENCLAW_WORKSPACE or pass --skills-dir to inbrain skillpack install.)',
    );
    lines.push('');
  }
  ...
  lines.push('');
  lines.push('         inbrain skillpack install <name>      # one skill');
  lines.push('         inbrain skillpack install --all       # all bundled');
  lines.push('         inbrain skillpack list                # see all options');
  lines.push('');
