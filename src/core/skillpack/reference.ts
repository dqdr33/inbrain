/**
 * skillpack/reference.ts — `inbrain skillpack reference <name>`.
 *
 * Read-only update lens. Compares every file in inbrain's bundle (skill
 * dir + paired sources + shared deps) against the host's local copy
 * and emits per-file status + unified diffs.
 *
 * The agent reads this output and decides what to integrate. inbrain
...
export interface ReferenceOptions {
  /** Absolute path to inbrain repo root (source-of-truth bundle). */
  inbrainRoot: string;
...
export function runReference(opts: ReferenceOptions): ReferenceResult {
  ...
  const framing = FRAMING_TEMPLATE(`${opts.inbrainRoot}/skills/${opts.skillSlug}/`);
  return {
    framing,
    files,
    summary: {
 ...
 * Used by `inbrain skillpack reference --all` for the upgrade sweep
 * workflow.
 */
export function runReferenceAll(opts: Omit<ReferenceOptions, 'skillSlug'>): ReferenceAllResult {
  ...
  return {
    framing: FRAMING_TEMPLATE(`${opts.inbrainRoot}/skills/`),
    skills,
  };
}
 ...
 * the diff between the user's local copy (target) and inbrain's bundle
 ...
 * base tracking, this command cannot distinguish "inbrain changed line
 * X" from "the user changed line X." Both look like differences from
 * inbrain's current bundle. Applied hunks therefore replace the user's
 * local content with inbrain's wherever they differ, including spots
 * the user intentionally edited. Use `--dry-run` first, or run
 * `inbrain skillpack reference <name>` to inspect the diff before
...
export function runReferenceApply(opts: ReferenceApplyOptions): ReferenceApplyResult {
  ...
  return {
    framing: FRAMING_TEMPLATE(`${opts.inbrainRoot}/skills/${opts.skillSlug}/`),
...
function applyOne(
  ...
  // apply: inbrain (a) is what we want to bring into the target. So
