/**
 * skillpack/harvest.ts — `inbrain skillpack harvest <slug> --from <host-repo-root>`.
 *
 * Inverse of scaffold: lifts a skill from a host agent repo into
 * inbrain's tree so other clients can scaffold it via the normal path.
 *
 ...
 * the mirror path inside inbrain.
 *
 ...
 * symlink could leak secrets into inbrain's source tree.
 *
 ...
 * patterns" (defaults + user-maintained `~/.inbrain/harvest-private-patterns.txt`).
...
export interface HarvestOptions {
  ...
  hostRepoRoot: string;
  /** Absolute path to inbrain repo root (destination). */
  inbrainRoot: string;
  /** Skip the privacy linter. */
  noLint?: boolean;
  /** Dry-run: preview, no writes. */
  dryRun?: boolean;
  /** Custom private-patterns file (defaults to ~/.inbrain/harvest-private-patterns.txt). */
  privatePatternsPath?: string;
  /** Allow overwriting an existing inbrain/skills/<slug>/ tree. */
...
export function runHarvest(opts: HarvestOptions): HarvestResult {
  ...
  const inbrainSkillDir = join(opts.inbrainRoot, 'skills', opts.slug);
  if (existsSync(inbrainSkillDir) && !opts.overwriteLocal) {
    throw new HarvestError(
      `Slug collision: inbrain already has skills/${opts.slug}/. Pass --overwrite-local to replace.`,
      'slug_collision',
    );
  }
  ...
  // Build items list:
  //   - skill dir → inbrain/skills/<slug>/
  //   - paired sources → inbrain/<source-path>
  const items: Array<{ source: string; target: string }> = [];
  for (const item of walkSourceDir(hostSkillDir, inbrainSkillDir)) {
    items.push(item);
  }
  for (const src of pairedSources) {
    items.push({
      source: join(opts.hostRepoRoot, src),
      target: join(opts.inbrainRoot, src),
    });
  }
  ...
  if (!opts.noLint && !dryRun) {
    try {
      ...
      if (err instanceof PrivacyLintError) {
        ...
        rollbackHarvest(inbrainSkillDir, pairedItems.map(i => i.target));
        return {
          status: 'lint_failed',
          slug: opts.slug,
          hostSkillDir,
  ...
  if (!dryRun) {
    manifestUpdated = addToBundleManifest(opts.inbrainRoot, opts.slug);
  }
 ...
 * (the bundler resolves paths against inbrainRoot, not the host). Mirrors
...
function rollbackHarvest(inbrainSkillDir: string, pairedTargets: string[]): void {
  try {
    if (existsSync(inbrainSkillDir)) {
      rmSync(inbrainSkillDir, { recursive: true, force: true });
    }
  } catch {}
 ...
 */
export function addToBundleManifest(inbrainRoot: string, slug: string): boolean {
  const manifestPath = join(inbrainRoot, PLUGIN_JSON);
