 ...
 *   → copyArtifacts → saveState (~/.inbrain/skillpack-state.json) → buildBootstrapDisplay
 *
...
export async function runScaffoldThirdParty(
  ...
  // 2. inbrain version check.
  if (!semverGte(currentGbrainVersion, manifest.inbrain_min_version)) {
    throw new ScaffoldThirdPartyError(
      `skillpack ${manifest.name} requires inbrain >= ${manifest.inbrain_min_version}; you have ${currentGbrainVersion}. Run \`inbrain upgrade\` first.`,
      'inbrain_version_too_old',
    );
  }
  ...
  try {
    entries = enumerateScaffoldEntries({
      inbrainRoot: opts.resolved.path,
