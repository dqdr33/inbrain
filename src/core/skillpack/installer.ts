 ...
 *   - Concurrency / lockfile (D-CX-11): acquire `.inbrain-skillpack.lock`
...
export function planInstall(opts: InstallOptions): InstallPlan {
  const manifest = loadBundleManifest(opts.inbrainRoot);
  const entries = enumerateBundle({
    inbrainRoot: opts.inbrainRoot,
...
function lockPath(workspace: string): string {
  return join(workspace, '.inbrain-skillpack.lock');
}
...
const MANAGED_BEGIN = '<!-- inbrain:skillpack:begin -->';
const MANAGED_END = '<!-- inbrain:skillpack:end -->';

// Receipt comment embedded inside the fence on every write. Lets the
// next install distinguish "row inbrain installed previously" from
// "row a user hand-added inside the fence." Format is intentionally
// regex-friendly.
//
//   <!-- inbrain:skillpack:manifest cumulative-slugs="a,b,c" version="0.19.0" -->
//
// Sorted, comma-separated slug list. version is the inbrain version
// that wrote this receipt.
const RECEIPT_RE =
  /<!-- inbrain:skillpack:manifest cumulative-slugs="([^"]*)" version="([^"]*)" -->/;

function buildReceipt(cumulativeSlugs: string[], version: string): string {
  const sorted = [...cumulativeSlugs].sort();
  return `<!-- inbrain:skillpack:manifest cumulative-slugs="${sorted.join(',')}" version="${version}" -->`;
}
...
export function buildManagedBlock(
  ...
  return [
    MANAGED_BEGIN,
    '',
    `<!-- Installed by inbrain ${manifest.version} — do not hand-edit between markers. -->`,
    receipt,
    '',
    '| Trigger | Skill |',
    '|---------|-------|',
...
function applyManagedBlock(
  ...
  // Step 1: figure out what inbrain previously installed into this fence.
  //   - If receipt is present, trust it as the cumulative-slug history.
  //   - If receipt is absent (pre-v0.19 fence), fall back to the rows
  //     currently in the fence — they were ALL inbrain-written before
  ...
  // intentionally-pruned set is something inbrain never wrote: a user
  ...
  // receipt yet). All existing rows are presumed inbrain-written and
  ...
  for (const slug of unknownSlugs) {
    console.error(
      `[skillpack] unknown row in managed block: "${slug}" at skills/${slug}/SKILL.md — not in inbrain's installed set. Investigate: user-added skill, hand-edited fence, or typo?`,
    );
  }
...
export function diffSkill(
  ...
  const entries = enumerateBundle({ inbrainRoot, skillSlug, manifest });
  const out: SkillDiff[] = [];
...
/**
 * `inbrain skillpack uninstall <name>` is the inverse of install. Two
 ...
 *     inbrain didn't install it; inbrain won't uninstall it either. Exit
...
export function applyUninstall(opts: UninstallOptions): UninstallResult {
  ...
  try {
    ...
    if (!receipt) {
      ...
      // inbrain-installed. Trust it, but warn.
      const existingRowSlugs = extractManagedSlugs(resolverContent);
      if (!existingRowSlugs.includes(opts.skillSlug)) {
        throw new UninstallError(
          `Skill '${opts.skillSlug}' is not in the managed block. Either it was never installed by inbrain, or the slug is mistyped. Inspect ${resolver} and the skills/${opts.skillSlug}/ directory before retrying.`,
          'unknown_skill',
        );
      }
      ...
      // user-added (not inbrain's row) or the slug doesn't exist at all.
      // Either way, refuse-and-warn.
      throw new UninstallError(
        `Skill '${opts.skillSlug}' is not in inbrain's installed set (cumulative-slugs receipt has no record of it). inbrain refuses to uninstall what it didn't install. If you hand-added this row to ${resolver}, remove it manually. If the slug is mistyped, run \`inbrain skillpack list\` to see what's installed.`,
        'user_added_slug',
      );
    }
    ...
    if (entries.length === 0) {
      throw new UninstallError(
        `Skill '${opts.skillSlug}' has no bundle entries — likely an unknown slug or stale receipt. Verify with \`inbrain skillpack list\`.`,
        'unknown_skill',
      );
    }
