 ...
 * Schema is `inbrain-skillpack-v1` plus forward-compat extensions for
...
export const SKILLPACK_API_VERSION = 'inbrain-skillpack-v1' as const;
...
export interface SkillpackManifest {
  ...
  version: string;
  /** One-line description, shown by `inbrain skillpack info`. */
  description: string;
  ...
  homepage: string;
  /** Minimum inbrain version this pack requires (semver). */
  inbrain_min_version: string;
...
export function validateSkillpackManifest(
  ...
  for (const field of ['description', 'author', 'license', 'homepage', 'inbrain_min_version']) {
  ...
  if (!SEMVER_RE.test(obj.inbrain_min_version as string)) {
    throw new SkillpackManifestError(
      `inbrain_min_version must be semver shape (e.g. "0.36.0"); got ${JSON.stringify(obj.inbrain_min_version)}`,
      'manifest_invalid_field',
      { field: 'inbrain_min_version', expected: SEMVER_RE.source, actual: obj.inbrain_min_version },
    );
  }
  ...
  if (obj.runbook_schema_version !== undefined) {
    ...
    if (v > maxRunbook) {
      throw new SkillpackManifestError(
        `runbook_schema_version ${v} exceeds maximum supported (${maxRunbook}). Run \`inbrain upgrade\``,
        'manifest_unsupported_schema_version',
  ...
  if (obj.eval_schema_version !== undefined) {
    ...
    if (v > maxEval) {
      throw new SkillpackManifestError(
        `eval_schema_version ${v} exceeds maximum supported (${maxEval}). Run \`inbrain upgrade\``,
        'manifest_unsupported_schema_version',
