/**
 * skillpack/migrate-fence.ts — `inbrain skillpack migrate-fence`.
 *
 * One-shot conversion for users running on the old (v0.19–v0.32.x)
 * managed-block model. Strips the `<!-- inbrain:skillpack:begin -->` /
...
export interface MigrateFenceOptions {
  ...
  /** Absolute path to inbrain repo root (source-of-truth bundle). When
...
/**
 * Parse a resolver file's content for the inbrain managed-block fence.
...
export function stripFence(content: string, parsed: ParsedFence): string {
  ...
  // comment line, the inbrain-installed comment line, and the end
  ...
  // Strip the "Installed by inbrain X.Y.Z — do not hand-edit" reminder
  // (matches even when version varies).
  const innerSansReminder = innerSansReceipt.replace(
    /<!-- Installed by inbrain [^>]*-->/,
    '',
  );
...
export function runMigrateFence(opts: MigrateFenceOptions): MigrateFenceResult {
  ...
  // mechanic via copyArtifacts). Optional — when inbrainRoot is unset,
  // only the fence is stripped.
  const skillsCopied: string[] = [];
  const skillsAlreadyPresent: string[] = [];
  if (opts.inbrainRoot) {
    const root = opts.inbrainRoot;
...
// Convenience: auto-discover inbrainRoot via findGbrainRoot when caller
// doesn't pass one. Used by the CLI dispatch.
export function runMigrateFenceAuto(opts: MigrateFenceOptions): MigrateFenceResult {
  if (opts.inbrainRoot) return runMigrateFence(opts);
  const root = findGbrainRoot();
  return runMigrateFence({ ...opts, inbrainRoot: root ?? undefined });
}
