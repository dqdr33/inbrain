/**
 * skillpack/init-scaffold.ts — `inbrain skillpack init <name>` scaffold.
 *
 ...
 * need; `inbrain skillpack doctor --quick` on a freshly-init'd pack
...
export function runInitScaffold(opts: InitScaffoldOptions): InitScaffoldResult {
  ...
  plan.push({
    ...
    content: [
      '# Bootstrap',
      '',
      'Post-scaffold steps. inbrain displays this but does NOT auto-execute.',
      ...
      `2. (edit me) agent: inbrain put_page wiki/_${opts.name}-config --frontmatter type=config`,
      '',
    ].join('\n'),
  });
  ...
  plan.push({
    ...
    content: [
      ...
      '',
      '```bash',
      `inbrain skillpack scaffold your-user/skillpack-${opts.name}`,
      '```',
      '',
      '## What it does',
      '',
