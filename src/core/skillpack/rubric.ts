    ...
    check: async (input) => {
      try {
        ...
        return {
          ...
          detail: msg,
          fix_hint:
            'Run `inbrain skillpack init <name>` to regenerate a valid stub manifest, or fix the field listed above.',
        };
      }
    },
  },
    ...
    check: async (input) => {
      ...
      return failures.length === 0
        ...
        : {
            ...
            fix_hint:
              'For each missing SKILL.md, run `inbrain skillpack init` (regenerates the stub) or hand-write the frontmatter with name + description + triggers (array of >=1 strings).',
          };
    },
  },
  {
    ...
    check: async (input) => {
      ...
      return failures.length === 0
        ...
        : {
            ...
            detail: failures.join('; '),
            fix_hint:
              'Add intents to skills/<slug>/routing-eval.jsonl — one JSON object per line with {intent, expected_skill, ambiguous_with?}. `inbrain skillpack doctor --fix` will scaffold stubs.',
          };
    },
  },
  {
    ...
    check: async (input) => {
      ...
      if (!existsSync(path)) {
        return {
          ...
          fix_hint:
            'Create CHANGELOG.md with at least a `## [<version>] - <YYYY-MM-DD>` entry for the current version. `inbrain skillpack doctor --fix` will scaffold a stub.',
        };
      }
    ...
    check: async (input) => {
      ...
      if (!existsSync(abs)) {
        return {
          ...
          fix_hint: `Create ${path} with at least one bootstrap step. \`inbrain skillpack doctor --fix\` will scaffold a stub.`,
        };
      }
