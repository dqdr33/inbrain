 ...
 * `inbrain-ai/inbrain-skillpack-registry` catalog files.
 *
...
export const REGISTRY_SCHEMA_VERSION = 'inbrain-registry-v1' as const;
export const ENDORSEMENTS_SCHEMA_VERSION = 'inbrain-endorsements-v1' as const;
...
export interface RegistryEntry {
  ...
  name: string;
  /** One-line description for `inbrain skillpack search` output. */
  description: string;
  /** Author display name (display only; account binding is via author_handle). */
  author: string;
  /** Account handle on the source host (e.g. "inbrain-ai" for github.com/inbrain-ai). */
  ...
  /** Minimum inbrain version the pack supports. */
   ...
   *  `inbrain skillpack endorse` command writing endorsements.json. */
...
export interface RegistryBundles {
  /** Named bundles — `inbrain skillpack scaffold starter-pack` walks the list. */
...
export function validateRegistryEntry(raw: unknown): RegistryEntry {
  ...
  for (const f of ['description', 'author', 'author_handle', 'homepage', 'tarball_sha256', 'inbrain_min_version', 'validated_at', 'validation_run_id', 'version']) {
