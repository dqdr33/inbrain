 ...
 *
 * inbrain ships a curated set of skills (plus shared rule/convention
 * files they depend on) that agents install into their OpenClaw
 * workspace via `inbrain skillpack install`. The source of truth is
 * `openclaw.plugin.json` at the inbrain repo root.
 */
...
export interface BundleManifest {
  ...
  version: string;
  description?: string;
  skills: string[]; // e.g. "skills/brain-ops" (relative to inbrain root)
 ...
 * sibling to `src/cli.ts`. That pair identifies a inbrain repo root.
 */
...
/**
 * Parse `openclaw.plugin.json` from the supplied inbrain root (absolute).
 * Throws BundleError on missing file or malformed JSON.
 */
export function loadBundleManifest(inbrainRoot: string): BundleManifest {
  const manifestPath = join(inbrainRoot, 'openclaw.plugin.json');
...
export interface BundleEntry {
  /** Absolute source path under inbrainRoot. */
  source: string;
...
export interface EnumerateOptions {
  /** Absolute path to inbrain repo root (source). */
  inbrainRoot: string;
...
export function enumerateBundle(opts: EnumerateOptions): BundleEntry[] {
  const { inbrainRoot, skillSlug, manifest } = opts;
  ...
  for (const rel of skillsToIncludePaths) {
    const abs = join(inbrainRoot, rel);
    if (!existsSync(abs)) {
      throw new BundleError(
        `Bundle lists '${rel}' but the path does not exist in ${inbrainRoot}`,
        'skill_not_found',
      );
    }
  ...
  for (const dep of manifest.shared_deps) {
    const abs = join(inbrainRoot, dep);
 ...
 * between `version` and HEAD in the inbrain source tree. Used by
 * `inbrain skillpack reference --since <version>` so an agent can sweep
 * only the skills that actually moved since the last time it looked.
 *
 * Returns `null` (not an empty array) when:
 *   - the inbrain root is not a git checkout (tarball install)
...
export function changedSlugsSinceVersion(
  ...
  if (!existsSync(join(inbrainRoot, '.git'))) return null;
  ...
  for (const ref of candidates) {
    const probe = spawnSync(
      'git',
      ['-C', inbrainRoot, 'rev-parse', '--verify', '--quiet', `${ref}^{commit}`],
      { encoding: 'utf-8' },
    );
...
/**
 * Read `<inbrainRoot>/<skillRel>/SKILL.md` and return its `sources:`
 ...
 *   - every referenced file must exist under `<inbrainRoot>`
 *
 * Skills without `sources:` declared, or with `sources: []`, return an
 * empty array (no validation work performed).
 */
export function loadSkillSources(inbrainRoot: string, skillRel: string): SkillSources {
  const slug = pathSlug(skillRel);
  const skillMd = join(inbrainRoot, skillRel, 'SKILL.md');
  ...
  for (const entry of raw) {
    ...
    const abs = join(inbrainRoot, entry);
    if (!existsSync(abs)) {
      throw new BundleError(
        `${skillMd}: frontmatter \`sources:\` declares "${entry}" but the file is missing from ${inbrainRoot}`,
        'manifest_malformed',
      );
    }
    sources.push(entry);
...
export function enumerateScaffoldEntries(opts: EnumerateOptions): ScaffoldEntry[] {
  ...
  // 1. Skill files — every file under `<inbrainRoot>/skills/<slug>/`.
  ...
  for (const rel of skillsToIncludePaths) {
    const { sources } = loadSkillSources(inbrainRoot, rel);
    for (const src of sources) {
      entries.push({
        source: join(inbrainRoot, src),
