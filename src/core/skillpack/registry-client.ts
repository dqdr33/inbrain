 ...
 * `inbrain skillpack-registry` catalog.
 *
 * The default registry lives at
 *   https://raw.githubusercontent.com/inbrain-ai/inbrain-skillpack-registry/main/registry.json
 *   https://raw.githubusercontent.com/inbrain-ai/inbrain-skillpack-registry/main/endorsements.json
 *
...
/** Default cache directory under ~/.inbrain/skillpack-cache. */
function defaultCacheDir(): string {
  return inbrainPath('skillpack-cache');
}
    ...
    try {
      ...
      if (cached) {
        ...
        if (origin === 'cache_hard_stale') {
          process.stderr.write(
            `[skillpack] cache is older than 7 days. When back online run \`inbrain skillpack registry --refresh\`.\n`,
          );
        }
        return {
          catalog: cached.catalog,
