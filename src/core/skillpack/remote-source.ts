 ...
 *
 *   ~/.inbrain/skillpack-cache/git/<host>/<owner>/<repo>/<sha>/   (git sources)
 *   ~/.inbrain/skillpack-cache/tarball/<sha256-hex>/              (tarball sources)
 *
...
export interface ResolveSourceOptions {
  /** Override the cache root (test-only; defaults to ~/.inbrain/skillpack-cache). */
  cacheRoot?: string;
...
function cacheRoot(opts: ResolveSourceOptions): string {
  return opts.cacheRoot ?? inbrainPath('skillpack-cache');
}
