 ...
 *
 * The friction CLI (`inbrain friction log/render/list/summary`) writes here.
 * The claw-test harness reads here. The agent calls `inbrain friction log`
 * directly when it hits something confusing, missing, or wrong.
 *
 * Storage shape: append-only JSONL files under `$INBRAIN_HOME/friction/`.
 *   - `<run-id>.jsonl` for each harness run (run-id from $INBRAIN_FRICTION_RUN_ID)
...
export function activeRunId(): string {
  const env = process.env.INBRAIN_FRICTION_RUN_ID?.trim();
