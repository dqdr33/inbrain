 ...
 *
 * Used by:
 *   - src/commands/extract.ts             (batch DB + FS extraction — `inbrain extract links|timeline|all`)
 ...
 * `inbrain extract --stale` sweep. Same role CHUNKER_VERSION plays for chunking.
 *
 ...
 * prune via `inbrain graph-query <slug> --type wikilink_basename`.
 *
...
// the common inbrain repo layout directories so arbitrary prose like
 ...
 *
 * mode: 'batch' (migration / inbrain extract) — pg_trgm only, NO search
 ...
 *   1. Env var `INBRAIN_LINK_RESOLUTION_GLOBAL_BASENAME=1` (operator override)
 *   2. DB plane via `engine.getConfig('link_resolution.global_basename')`
 *   3. Default false
 *
 * Closes https://github.com/inbrain-ai/inbrain/issues/972.
 */
export async function isGlobalBasenameEnabled(engine: BrainEngine): Promise<boolean> {
  const envVal = process.env.INBRAIN_LINK_RESOLUTION_GLOBAL_BASENAME;
  if (envVal != null) {
