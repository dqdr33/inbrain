 ...
 *
 * Writes info-severity rows to `~/.inbrain/audit/slug-fallback-YYYY-Www.jsonl`
 ...
 *
 * Why a separate JSONL instead of `~/.inbrain/sync-failures.jsonl`:
...
export interface SlugFallbackAuditEvent {
  ...
  severity: 'info';
  /** Stable code consumed by `inbrain doctor`'s slug_fallback_audit check. */
...
export function logSlugFallback(slug: string, sourcePath: string): void {
  ...
  process.stderr.write(`[inbrain] slug fallback: ${sourcePath} → ${slug} (frontmatter slug; path slugified empty)\n`);
  writer.log({
    slug,
 ...
 * latest week's JSONL. Used by `inbrain doctor`'s slug_fallback_audit check.
