 ...
 * ------------------------------
 * - INBRAIN_BULK_MAX_RETRIES   — int >= 0 (0 disables retries for debugging)
 * - INBRAIN_BULK_RETRY_BASE_MS — int > 0
 * - INBRAIN_BULK_RETRY_MAX_MS  — int >= base
 *
 * Bad values throw `InbrainError` with a paste-ready fix hint. Doctor's
 ...
 *
 * Throws InbrainError with paste-ready fix on bad input. Never silently
...
export function resolveBulkRetryOpts(
  ...
  const maxRetries = env.INBRAIN_BULK_MAX_RETRIES;
  if (maxRetries !== undefined && maxRetries !== '') {
    const n = Number(maxRetries);
    if (!Number.isInteger(n) || n < 0) {
      throw new Error(
        `INBRAIN_BULK_MAX_RETRIES must be an integer >= 0 (got "${maxRetries}"). ` +
        `Fix: export INBRAIN_BULK_MAX_RETRIES=3   # or 0 to disable retries`,
      );
    }
    out.maxRetries = n;
  }

  const baseMs = env.INBRAIN_BULK_RETRY_BASE_MS;
  if (baseMs !== undefined && baseMs !== '') {
    const n = Number(baseMs);
    if (!Number.isInteger(n) || n <= 0) {
      throw new Error(
        `INBRAIN_BULK_RETRY_BASE_MS must be an integer > 0 (got "${baseMs}"). ` +
        `Fix: export INBRAIN_BULK_RETRY_BASE_MS=1000`,
      );
    }
    out.delayMs = n;
  }

  const maxMs = env.INBRAIN_BULK_RETRY_MAX_MS;
  if (maxMs !== undefined && maxMs !== '') {
    const n = Number(maxMs);
    if (!Number.isInteger(n) || n < out.delayMs) {
      throw new Error(
        `INBRAIN_BULK_RETRY_MAX_MS must be an integer >= INBRAIN_BULK_RETRY_BASE_MS=${out.delayMs} ` +
        `(got "${maxMs}"). Fix: export INBRAIN_BULK_RETRY_MAX_MS=10000`,
      );
    }
    out.delayMaxMs = n;
  }
