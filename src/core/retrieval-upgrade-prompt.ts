...
export interface PromptOpts {
  ...
  /** Bypass the `prompt_shown` gate (for `inbrain ze-switch --force`). */
  force?: boolean;
}
...
export async function runRetrievalUpgradePrompt(
  ...
  if (!isTTY) {
    writeFn('[ze-switch] non-TTY environment; skipping prompt. Run `inbrain ze-switch` manually when ready.');
  ...
  if (normalized === 's') {
    ...
    if (result.status === 'applied') {
      writeFn('[ze-switch] Schema rebuilt at 1024d. Run `inbrain embed --stale` to refill embeddings (or wait for autopilot).');
  ...
  if (normalized === 'n') {
    ...
    writeFn('[ze-switch] Will not ask again. Re-enable with `inbrain ze-switch --force`.');
...
> {
  ...
  if (result.status === 'undone') {
    writeFn(`[ze-switch --undo] Restored ${snapshot.embedding_model} at ${snapshot.embedding_dimensions}d. Run \`inbrain embed --stale\` to refill embeddings.`);
    return result;
  }
  return result;
}
...
export function formatBanner(plan: RetrievalUpgradeState): string {
  ...
  lines.push('               (runs via `inbrain embed --stale` or autopilot — you can walk away)');
  ...
  lines.push('`inbrain config set search.reranker.enabled false`.');
