 ...
 *   - `inbrain remote ping`   — submits autopilot-cycle, polls get_job
 *   - `inbrain remote doctor` — calls run_doctor MCP op
 *
 ...
 * amortizes when a single `inbrain remote ping` makes multiple calls (submit_job
...
export function extractToolErrorCode(message: string): string | undefined {
  // Try to parse a JSON payload first — inbrain server-side tool errors
...
function requireRemoteMcp(config: InbrainConfig | null): NonNullable<InbrainConfig['remote_mcp']> {
  if (!config?.remote_mcp) {
    throw new RemoteMcpError(
      'config',
      'No remote_mcp config. Run `inbrain init --mcp-only` first.',
    );
  }
  return config.remote_mcp;
}

function resolveSecret(remote: NonNullable<InbrainConfig['remote_mcp']>): string {
  const secret = process.env.INBRAIN_REMOTE_CLIENT_SECRET ?? remote.oauth_client_secret;
  if (!secret) {
    throw new RemoteMcpError(
      'config',
      'No client_secret available. Set INBRAIN_REMOTE_CLIENT_SECRET or rerun `inbrain init --mcp-only`.',
    );
  }
  return secret;
}
 ...
 */
async function getAccessToken(config: InbrainConfig, force = false): Promise<string> {
...
async function buildClient(mcpUrl: string, accessToken: string, signal?: AbortSignal): Promise<Client> {
  ...
  const client = new Client(
    { name: 'inbrain-remote-cli', version: '1' },
    { capabilities: {} },
  );
    ...
    try {
      ...
      try {
        ...
        if (mintErr instanceof RemoteMcpError && mintErr.reason === 'auth') {
          throw new RemoteMcpError(
            'auth_after_refresh',
            `Auth failed after token refresh. Verify oauth_client_id and secret are still valid; the host operator may need to re-run \`inbrain auth register-client\`.`,
 ...
 * inbrain ops set the JSON-encoded result as `text` of the first content item.
