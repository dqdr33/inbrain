 ...
 * `~/.inbrain/audit/quality-probe-YYYY-Www.jsonl` (ISO-week rotation).
 * Mirrors `audit-slug-fallback.ts` for filename + best-effort write
 * semantics. Honors `INBRAIN_AUDIT_DIR` via the shared `resolveAuditDir`.
 *
 * Read by `inbrain doctor`'s `nightly_quality_probe_health` check to
...
export function logQualityProbeEvent(event: Omit<QualityProbeAuditEvent, 'ts'> & { ts?: string }): void {
  ...
  try {
    ...
    process.stderr.write(`[inbrain] quality-probe audit write failed (${msg}); probe continues\n`);
  }
}

/**
 * Read recent quality-probe events from the current + prior ISO week files.
 * Used by `inbrain doctor`'s nightly_quality_probe_health check. Missing
