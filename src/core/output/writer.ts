...
class WriteTxImpl implements WriteTx {
  ...
  async appendTimeline(slug: string, entry: TimelineInput): Promise<void> {
    await this.engine.addTimelineEntry(slug, entry); // inbrain-allow-direct-insert: BrainWriter is the canonical synthesize-phase write surface — output gets fenced into pages via putPage in the same transaction
    this.touchedSlugs.add(slug);
  }
  ...
  async addLink(from: string, to: string, context?: string, linkType?: string): Promise<void> {
    await this.engine.addLink(from, to, context, linkType); // inbrain-allow-direct-insert: BrainWriter is the canonical synthesize-phase write surface
    ...
    await this.engine.addLink(to, from, context, linkType ? `${linkType}_back` : 'backlink'); // inbrain-allow-direct-insert: BrainWriter synthesize-phase reverse back-link in the same transaction as the forward addLink above
...
export class BrainWriter {
   ...
   * (grandfathered pages opt out until `inbrain integrity` repairs them).
   */
