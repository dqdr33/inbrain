 ...
 *
 * Inbrain ingests content (markdown, code) into its retrieval layer and routes
 ...
 *
 * This module exposes the seams without binding Inbrain to any specific vendor.
 ...
 *   reorder Inbrain behavior through this interface. Enforcement, if ever added,
 ...
 * - **No verdict persistence.** Inbrain does not write guardrail results to the
 ...
 * tool name, model id, etc.). Neither field is mutated by Inbrain.
 */
 ...
 * value is intentionally `unknown` and intentionally ignored by Inbrain — the
 * type exists only so providers can return a typed verdict to *their own*
 * logging/audit code. Inbrain never reads it.
 */
 ...
 * cost is bounded by each provider's own timeout discipline; Inbrain does not
  ...
  await Promise.all(
    snapshot.map(async (provider) => {
      try {
        ...
        // Fail open. A guardrail provider MUST NOT be able to break Inbrain.
        // Provider-side logging is the provider's responsibility; Inbrain does
