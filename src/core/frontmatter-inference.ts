 ...
 * ## Why this exists
 *
 * Inbrain's sync and import pipelines work fine without frontmatter — gray-matter
 ...
 *
 *   1. Written back to the file on disk (via `inbrain frontmatter generate --fix`)
 *   2. Used at import time without modifying the file (DB-only inference)
 *   3. Shown as a dry-run preview (via `inbrain frontmatter generate --dry-run`)
 *
 * ## Design principles
 *
