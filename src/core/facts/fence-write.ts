 ...
 * `~/.inbrain/page-locks/<sha256-of-slug>.lock` with PID-liveness +
 * 5-minute TTL. Multi-process safe — two `inbrain` invocations writing
 ...
 * (`facts.write_failures.jsonl`) records the failure for `inbrain
...
const FAILURE_LOG_PATH = (): string => inbrainPath('facts.write_failures.jsonl');
...
function recordWriteFailure(slug: string, sourceId: string, warnings: string[], filePath: string): void {
  ...
  // log is the operator-visibility surface; `inbrain doctor` reads it
 ...
 * page validates as inbrain-canonical markdown and survives an
  ...
  return withPageLock(
    ...
    async () => {
      ...
      if (existsSync(filePath)) {
        ...
        // page on disk.
        //
        // Sunset target: v0.36. Once `stub_guard_24h` (the inbrain doctor
        ...
        // The audit log under `~/.inbrain/audit/stub-guard-YYYY-Www.jsonl`
      ...
      const result = await engine.insertFacts(enriched, { source_id: target.sourceId }); // inbrain-allow-direct-insert: writeFactsToFence is the markdown-first reconcile path; runs only after the atomic fence write commits
