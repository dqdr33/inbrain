...
async function runPipelineWithBody(
  ...
  if (localPath === null) {
    warnOnce(
      ...
      ' — falling through to DB-only inserts. Configure local_path via `inbrain sources update` to enable system-of-record fence writes.',
    );
  ...
  for (const { f, resolvedSlug } of legacyBucket) {
    ...
    const result = await ctx.engine.insertFact(newFact, { source_id: ctx.sourceId }); // inbrain-allow-direct-insert: legacy DB-only fallback for unparented / thin-client facts (no entity page to fence onto)
  ...
  for (const [slug, group] of byEntity) {
    ...
    if (result.stubGuardBlocked) {
      ...
      for (const { f } of group) {
        ...
        const legacyResult = await ctx.engine.insertFact(newFact, { source_id: ctx.sourceId }); // inbrain-allow-direct-insert: stub-guard fallback for unprefixed entity slugs (no fenceable page)
