...
// v0.41.22 (T21, codex F-ELIGIBLE finding): UNION of inbrain-base's hardcoded
// types AND inbrain-base-v2's canonical extractable types. Pre-rebase plan
// deferred pack-aware ELIGIBLE_TYPES to v0.43+; codex outside voice caught
// it as a blocker — changing the default taxonomy to inbrain-base-v2 while
// `eligibility.ts:49` hardcodes only inbrain-base's types means post-unify
...
// true` flag in inbrain-base.yaml as "cosmetic on the backstop path because
...
//   - inbrain-base brains: all original types still eligible (back-compat)
//   - inbrain-base-v2 brains: post-unify canonical types also eligible
//
...
const ELIGIBLE_TYPES: PageType[] = [
  // inbrain-base (legacy) types
  'note', 'meeting', 'slack', 'email', 'calendar-event', 'source', 'writing',
  // inbrain-base-v2 canonical types declared extractable in the pack
