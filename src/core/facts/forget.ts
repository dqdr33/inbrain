 ...
 *
 * Before v0.32.2 `inbrain forget` and the MCP `forget_fact` op called
 * `engine.expireFact(id)` directly, which UPDATEs `facts.expired_at`
 * in the DB. After `inbrain rebuild` (v0.32.3) that DB-only mutation
 ...
 * (matches the existing `inbrain forget` CLI which takes no reason
...
export async function forgetFactInFence(
  ...
  if (!canFence) {
    // Legacy path — DB-only forget. Doesn't survive `inbrain rebuild`.
    const ok = await engine.expireFact(factId); // inbrain-allow-direct-insert: legacy fallback path inside forgetFactInFence — fence rewrite not possible (pre-v51 row / missing local_path / file deleted / row_num drift)
  ...
  return withPageLock(slug, async () => {
    ...
    const begin = body.indexOf('<!--- inbrain:facts:begin -->');
    const end   = body.indexOf('<!--- inbrain:facts:end -->', begin + 1);
    if (begin === -1 || end === -1) {
    ...
    const newBody = body.slice(0, begin) + newFence + body.slice(end + '<!--- inbrain:facts:end -->'.length);
