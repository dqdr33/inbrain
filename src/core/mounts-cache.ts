 ...
 * host's checked-in `skills/RESOLVER.md`, inbrain publishes an aggregated
 * `~/.inbrain/mounts-cache/RESOLVER.md` + `manifest.json` whenever mounts
...
function getMountsCacheDir(): string {
  return join(homedir(), '.inbrain', 'mounts-cache');
}
...
export function renderResolverMarkdown(composed: ComposedResolver): string {
  const lines: string[] = [];
  lines.push('# Inbrain Skill Resolver (aggregated)');
  lines.push('');
  lines.push('Auto-generated by `inbrain mounts add|remove|sync`. Do not edit by hand.');
...
/**
 * Write the aggregated cache to ~/.inbrain/mounts-cache/. Safe to call
...
export function writeMountsCache(
  ...
  // Unique tmp names per call so concurrent `inbrain mounts add|remove`
...
/** Remove the aggregated cache dir. Called by `inbrain mounts remove` and tests. */
