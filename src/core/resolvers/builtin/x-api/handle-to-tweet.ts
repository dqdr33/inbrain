 ...
 *
 * Driven by `inbrain integrity --auto`: a brain page says "Garry tweeted about
 ...
 *
 * Confidence scoring (the contract `inbrain integrity` relies on):
...
export interface XHandleToTweetInput {
  /** X handle without leading @. e.g. "inbrain-ai". */
  handle: string;
...
/**
 * Confidence buckets align with `inbrain integrity --auto` three-bucket logic:
 *   >=0.8 auto-repair
