 ...
 *   wave's auto-grade ('inbrain:grade_takes'). Manual resolutions
...
export interface UndoWaveOpts {
  ...
  scrubGstack?: boolean;
  /** The resolved_by label that identifies wave-applied resolutions. Default 'inbrain:grade_takes'. */
...
export async function undoWave(
  ...
  const resolvedByLabel = opts.resolvedByLabel ?? 'inbrain:grade_takes';
  ...
  // un-resolving a take a manual `inbrain takes resolve` operation
