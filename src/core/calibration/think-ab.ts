/**
 * v0.36.1.0 (T18 / D19) — A/B harness for `inbrain think`.
 *
 ...
 * 30 days of data, `inbrain calibration ab-report` aggregates win/loss
...
export function formatAbReport(report: AbReportResult, days: number): string {
  ...
  if (report.total_trials === 0) {
    lines.push('  No data yet. Try: inbrain think --ab "<question>"');
    return lines.join('\n');
  }
