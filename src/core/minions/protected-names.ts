...
export const PROTECTED_JOB_NAMES: ReadonlySet<string> = new Set([
  ...
  // `inbrain agent run` CLI path (which sets allowProtectedSubmit) or a
  ...
  // can't auto-apply; user must run `inbrain onboard --auto-with-prompt`
  // or submit explicitly via `inbrain jobs submit unify-types --allow-protected`.
  'unify-types',
  ...
  // auto-drain branch, an explicit `inbrain jobs submit extract-atoms-drain
