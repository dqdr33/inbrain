 ...
 * side), so `inbrain agent run --follow` on the CLI side polls getJob() until
 * the job reaches a terminal state.
 *
 * On timeout, the job is NOT cancelled — the user can `inbrain jobs get <id>`
 * later to check. Explicit cancellation is the user's call via `inbrain jobs
 * cancel <id>`.
 */
