 ...
 * Before `inbrain jobs submit` queues a batch, this projection prints
...
export function projectBatch(input: ProjectBatchInput): BatchProjection {
  ...
  if (stats.lease_headroom !== undefined && stats.lease_headroom <= 0.1 && current_lease_cap < 128) {
    ...
    if (projectedFasterMs < totalDurationMs * 0.75) {
      raise_cap_hint =
        `raise INBRAIN_ANTHROPIC_MAX_INFLIGHT to ${suggestedCap} to finish in ~${Math.ceil(projectedFasterMs / 60_000)}min`;
    }
  }
...
export function shouldPromptAtThreshold(p: BatchProjection): boolean {
  const usdThreshold = Number(process.env.INBRAIN_BATCH_PROMPT_THRESHOLD_USD ?? '5');
  const minThreshold = Number(process.env.INBRAIN_BATCH_PROMPT_THRESHOLD_MIN ?? '30');
