 ...
 * `subagent_health` and `inbrain jobs stats lease_pressure` read this
 ...
 * post-NULL forensic queries (after `inbrain jobs prune` pulls the job row)
...
/**
 * Read lease-pressure events in a rolling window. Used by `inbrain doctor`'s
 * `subagent_health` check and by `inbrain jobs stats lease_pressure`.
