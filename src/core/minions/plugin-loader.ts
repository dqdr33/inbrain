/**
 * INBRAIN_PLUGIN_PATH loader for host-repo subagent definitions (v0.15).
 *
 * Your OpenClaw (and future downstream agents) ship custom subagent defs
 * from their own repos. inbrain discovers them at worker startup via
 * INBRAIN_PLUGIN_PATH = colon-separated absolute paths (like $PATH). Each
 * path must contain a inbrain.plugin.json manifest describing the plugin
 ...
 * version of inbrain understands.
 */
...
export const SUPPORTED_PLUGIN_VERSION = 'inbrain-plugin-v1';
...
/** Public entry point: load every plugin directory from INBRAIN_PLUGIN_PATH. */
...
export function loadSinglePlugin(
  ...
  const manifestPath = path.join(rootDir, 'inbrain.plugin.json');
  if (!fs.existsSync(manifestPath)) {
    return { error: 'missing inbrain.plugin.json' };
  }
  ...
  if (manifest.plugin_version !== SUPPORTED_PLUGIN_VERSION) {
    return {
      error: `unsupported plugin_version "${manifest.plugin_version}" (inbrain supports "${SUPPORTED_PLUGIN_VERSION}")`,
    };
  }
