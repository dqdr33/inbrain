 ...
 *
 * Each running `inbrain jobs work` process self-registers a small JSON file
 ...
 *
 * Location is brain-isolated via inbrainPath() (honors INBRAIN_HOME); entries are
 * additionally tagged with a brain id so a single INBRAIN_HOME hosting multiple
...
import { inbrainPath, loadConfig } from '../config.ts';
...
/** Directory holding one file per live worker. Brain-isolated via INBRAIN_HOME. */
export function workerRegistryDir(): string {
  return inbrainPath('workers');
}
 ...
 * Used to tag + filter registry entries so multiple DBs under one INBRAIN_HOME
 * don't cross-report.
 */
...
export function readWorkers(
  ...
  for (const f of files) {
    ...
    // Only this brain's workers (multi-DB under one INBRAIN_HOME).
