 ...
 *
 * Spawns `inbrain jobs work` as a child process and restarts it on crash
 ...
 * any separate worker process, so `inbrain jobs supervisor` cannot work
 ...
 *
 * Usage:
 *   inbrain jobs supervisor [--concurrency N] [--queue Q] [--pid-file PATH]
...
export interface SupervisorOpts {
  ...
  queue: string;
  /** PID file path. Default: `${HOME}/.inbrain/supervisor.pid` (parent dir auto-created). */
  pidFile: string;
  /** Max consecutive crashes before giving up. Default: 10. */
  maxCrashes: number;
  /** Health check interval in ms. Default: 60000. */
  healthInterval: number;
  /** Path to the inbrain CLI executable (MUST be a compiled binary; .ts sources cannot be spawned). */
  cliPath: string;
  /** Allow shell jobs on child worker. Default: false. When true, sets INBRAIN_ALLOW_SHELL_JOBS=1 on child env. */
  ...
  maxRssMb: number;
  /** Niceness (issue #1815) the operator requested via `--nice` / `INBRAIN_NICE`,
...
export const DEFAULT_PID_FILE: string = (() => {
  const envOverride = process.env.INBRAIN_SUPERVISOR_PID_FILE;
  ...
  return `${home}/.inbrain/supervisor-${brainId}.pid`;
})();
...
/**
 * Build the argv the supervisor uses to spawn `inbrain jobs work`. Extracted from
 ...
 * Exported so `inbrain doctor` queries the same row to surface the holder +
 * effective --max-rss.
 */
export function supervisorLockId(queue: string): string {
  return `inbrain-supervisor:${queue}`;
}
...
export class MinionSupervisor {
  ...
  async start(): Promise<void> {
    ...
    this.emit('started', {
      ...
      // #1849: record the EFFECTIVE --max-rss so `inbrain doctor` can surface
  ...
  private acquirePidLock(): 'acquired' | 'held' | 'unwritable' {
    // Ensure parent directory exists. Idempotent; creates ~/.inbrain on fresh installs.
    try {
      ...
      if (code !== 'EEXIST') {
        console.error(
          ...
          }. Set INBRAIN_SUPERVISOR_PID_FILE or pass --pid-file to a writable location.`
        );
        return 'unwritable';
      }
    }
  ...
  private async runSuperviseLoop(): Promise<void> {
    ...
    // Build child env. Explicit handling for INBRAIN_ALLOW_SHELL_JOBS:
    // inherit only when caller opts in, otherwise strip from the clone.
    const env: Record<string, string | undefined> = { ...process.env };
    if (this.opts.allowShellJobs) {
      env.INBRAIN_ALLOW_SHELL_JOBS = '1';
    } else {
      delete env.INBRAIN_ALLOW_SHELL_JOBS;
    }
