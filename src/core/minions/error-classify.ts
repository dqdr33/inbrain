 ...
 *
 *   1. **D3 error clustering** — `inbrain jobs stats --cluster-errors` and
 *      `inbrain jobs get --cluster <name>` group dead/failed jobs by bucket
...
export type ErrorCluster =
  ...
  | 'rate_lease_full'      // inbrain's internal RateLeaseUnavailableError
...
export function classifyJobError(lastError: string | null | undefined): ErrorCluster {
  ...
  // inbrain-internal errors first (most specific).
