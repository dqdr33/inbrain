/**
 * Minions — BullMQ-inspired Postgres-native job queue for Inbrain.
 *
 * Usage:
...
export interface MinionWorkerOpts {
   ...
   *  Automatically disabled when running under a supervisor (INBRAIN_SUPERVISED=1).
   *  Provides DB liveness probes and stall detection for bare `inbrain jobs work`
...
export interface SubagentHandlerData {
   ...
   * `inbrain agent run` does not yet accept a `--brain` flag that would
