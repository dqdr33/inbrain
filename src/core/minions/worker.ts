...
export class MinionWorker extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    // maxRssMb is 0 (default for bare `inbrain jobs work`; supervisor sets 2048).
    ...
    //      runs only when NOT supervised (INBRAIN_SUPERVISED=1); under a
    ...
    if (this.opts.healthCheckInterval > 0) {
      ...
      const runHealthCheck = async (): Promise<void> => {
        ...
        try {
          ...
          // supervisor SIGTERM race). Bare `inbrain jobs work` keeps it.
  ...
  private async executeJob(
    ...
    try {
      ...
      // `inbrain doctor` + `inbrain jobs stats lease_pressure`.
      ...
      if (isLeaseFull) {
        ...
        // Denormalized columns persist past `inbrain jobs prune` so post-NULL
