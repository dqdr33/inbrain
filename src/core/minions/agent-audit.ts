 ...
 * `~/.inbrain/audit/agent-jobs-YYYY-Www.jsonl` (override via
 * `INBRAIN_AUDIT_DIR`). Best-effort writes — disk-full / permission errors
 ...
 * the same way they treat ~/.inbrain (private).
 */
...
function resolveAuditDir(): string {
  const env = process.env.INBRAIN_AUDIT_DIR;
...
/** Read recent events across files (for `inbrain doctor` integrations + tests). */
