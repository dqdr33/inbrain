 ...
 * doctor scan.
 *
 * File: `~/.inbrain/audit/backpressure-YYYY-Www.jsonl` (override dir via
 * `INBRAIN_AUDIT_DIR` for container/sandbox deployments where `$HOME` is read-only).
 *
 * `inbrain jobs stats` will surface coalesce counts from this file in a v0.19.2+
...
/** Honors `INBRAIN_AUDIT_DIR` for container/sandbox deployments. */
