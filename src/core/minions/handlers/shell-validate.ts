 ...
 *   - `src/commands/jobs.ts` — `inbrain jobs submit shell` CLI handler
...
export interface ValidateShellJobOpts {
  /**
   * Loaded inbrain config used to verify every `inherit` name resolves to a
    ...
    for (const name of inherit) {
      ...
      if (value === undefined) {
        throw new UnrecoverableError(
          ...
          `Fix: \`inbrain config set ${name} <value>\` or set the value in the worker's config file. ` +
          '(see: docs/guides/minions-shell-jobs.md#secrets)',
        );
      }
    }
