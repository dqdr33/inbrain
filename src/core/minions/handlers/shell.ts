 ...
 *   2. This handler only registers when `process.env.INBRAIN_ALLOW_SHELL_JOBS === '1'`.
...
export interface ShellJobParams {
   ...
   *  Cannot contain secret env keys (INBRAIN_DATABASE_URL, DATABASE_URL, etc.) —
 ...
 *      `deriveEnvKey` (e.g. `database_url` → `INBRAIN_DATABASE_URL`).
...
export async function shellHandler(ctx: MinionJobContext): Promise<ShellJobResult> {
  if (process.env.INBRAIN_ALLOW_SHELL_JOBS !== '1') {
    const warning =
      `[shell] Job #${ctx.id} rejected: INBRAIN_ALLOW_SHELL_JOBS=1 not set on this worker.\n` +
      '        Shell jobs require the env var on the worker process.';
    console.warn(warning);
    throw new UnrecoverableError(
      'shell handler disabled on this worker (set INBRAIN_ALLOW_SHELL_JOBS=1 to execute shell jobs)',
    );
  }
