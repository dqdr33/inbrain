 ...
 *
 * Writes a JSONL line per shell-job submission to `~/.inbrain/audit/shell-jobs-YYYY-Www.jsonl`
 * (ISO week rotation, override via `INBRAIN_AUDIT_DIR`). Best-effort: write failures go
 ...
 * AND callers across `inbrain-home-isolation.test.ts`, `minions.test.ts`,
...
/** Resolve the audit dir. Honors `INBRAIN_AUDIT_DIR` for container/sandbox deployments
 *  where `$HOME` is read-only. Defaults to `~/.inbrain/audit/`.
 *
