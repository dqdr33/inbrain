 ...
 *   - `inbrain sync --all` finishes fast (pages searchable via keyword
 ...
 *
 *   - D2: per-source DB lock (`inbrain-embed-backfill:<source>`) prevents
...
function embedBackfillLockId(sourceId: string): string {
  return `inbrain-embed-backfill:${sourceId}`;
}
