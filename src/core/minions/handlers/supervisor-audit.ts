 ...
 * verbatim and let consumers (like `inbrain doctor`) grep for events of
 ...
 * operational trace for `inbrain doctor`, not forensic insurance.
 *
 * `INBRAIN_AUDIT_DIR` overrides the default `~/.inbrain/audit/` path for
 ...
 * Used by `inbrain doctor` (Lane D) to surface supervisor health.
 *
 ...
 * `inbrain doctor` and `inbrain jobs supervisor status` consume this — single
