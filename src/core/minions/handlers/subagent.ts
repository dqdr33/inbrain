...
export function resolveLeaseCap(raw: string | undefined): number {
  ...
  throw new Error(
    `INBRAIN_ANTHROPIC_MAX_INFLIGHT="${raw}" is invalid. ` +
    `Use a positive integer, "unlimited" (or "none"), or omit for default 32.`,
  );
}
const DEFAULT_MAX_CONCURRENT = resolveLeaseCap(process.env.INBRAIN_ANTHROPIC_MAX_INFLIGHT);
...
export interface SubagentDeps {
  ...
  rateLeaseKey?: string;
  /** Max concurrent inflight calls on that key. Defaults to INBRAIN_ANTHROPIC_MAX_INFLIGHT or 8. */
...
export function makeSubagentHandler(deps: SubagentDeps) {
  ...
  const config = deps.config ?? loadConfig() ?? ({ engine: 'postgres' } as InbrainConfig);
  ...
  return async function subagentHandler(ctx: MinionJobContext): Promise<SubagentResult> {
    ...
    //      `inbrain config set agent.use_gateway_loop true`
    ...
    // for queue-submitted jobs; the check here covers direct `inbrain agent run`
 ...
 * inbrain_tool_use_id from migration v81), and invokes the gateway loop.
 *
 * Replay semantics: loads prior `subagent_messages` + `subagent_tool_executions`,
 * builds a `ToolLoopReplayState` keyed by `inbrain_tool_use_id`. For pre-v81
...
async function runSubagentViaGateway(args: GatewayRunArgs): Promise<SubagentResult> {
  ...
  const result = await gatewayToolLoop({
    ...
    onToolCallStart: async (turnIdx, messageIdx, ordinal, toolName, input, providerToolCallId) => {
      // CRITICAL — read back the canonical inbrain_tool_use_id from RETURNING,
      ...
      const rows = await engine.executeRaw<{ inbrain_tool_use_id: string }>(
        `INSERT INTO subagent_tool_executions
           (job_id, message_idx, tool_use_id, tool_name, input, status, schema_version, ordinal, inbrain_tool_use_id, provider_id)
         ...
         RETURNING inbrain_tool_use_id::text AS inbrain_tool_use_id`,
        [ctx.id, messageIdx, providerToolCallId, toolName, JSON.stringify(input ?? null), ordinal, candidateId, recipeIdFromModel(model)],
      );
      const inbrainToolUseId = rows[0]?.inbrain_tool_use_id ?? candidateId;
      heartbeat('tool_called', { turn_idx: turnIdx, tool_name: toolName });
      return { inbrainToolUseId };
    },
    onToolCallComplete: async (inbrainToolUseId, output) => {
      await engine.executeRaw(
        `UPDATE subagent_tool_executions
           SET status = 'complete', output = $1::jsonb, ended_at = now()
         WHERE inbrain_tool_use_id::text = $2`,
        [JSON.stringify(output ?? null), inbrainToolUseId],
      );
    },
    onToolCallFailed: async (inbrainToolUseId, errorMsg) => {
      await engine.executeRaw(
        `UPDATE subagent_tool_executions
           SET status = 'failed', error = $1, ended_at = now()
         WHERE inbrain_tool_use_id::text = $2`,
        [errorMsg, inbrainToolUseId],
      );
    },
 ...
 *
 * v0.41 Bug 3 — pre-fix, `inbrain agent run --model anthropic:claude-sonnet-4-6`
 ...
 *
 *   - v2 rows: inbrain_tool_use_id is the stable key (set at first observation
 *     by onToolCallStart).
...
async function loadPriorToolsV2(engine: BrainEngine, jobId: number): Promise<PriorToolV2Row[]> {
  const rows = await engine.executeRaw<Record<string, unknown>>(
    `SELECT message_idx, tool_use_id, tool_name, ordinal, inbrain_tool_use_id::text AS inbrain_tool_use_id,
            status, output, error
  ...
  return rows.map(r => {
    const inbrainId = r.inbrain_tool_use_id as string | null;
    const stableKey = inbrainId
      ? inbrainId
