...
const CONN_PATTERNS = [
  ...
  // v0.41.2.1: inbrain's own InbrainError thrown by getConnection() when
...
interface PgError {
  ...
  message?: string;
  cause?: unknown;
  // v0.41.2.1: inbrain's InbrainError uses `problem` (typed) + `detail` so
...
export function isRetryableConnError(err: unknown): boolean {
  ...
  // v0.41.2.1: typed-shape match for inbrain's own InbrainError
  // (problem === 'No database connection'). Avoids brittle string match
  // when the error wrapper is inbrain-internal.
  if (
