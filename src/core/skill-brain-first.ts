...
export const BRAIN_REFERENCE_PATTERNS: ReadonlyArray<RegExp> = [
  /\binbrain[\s_]+search\b/i,
  /\binbrain[\s_]+query\b/i,
  /\binbrain[\s_]+get[_-]?page\b/i,
  /\binbrain[\s_]+find[_-]?experts\b/i,
  /\binbrain[\s_]+get[_-]?backlinks\b/i,
  /\binbrain[\s_]+get[_-]?timeline\b/i,
  /\binbrain[\s_]+traverse[_-]?graph\b/i,
 ...
 *   1. `inbrain doctor --fix` to auto-add the canonical callout, OR
...
export const FORMERLY_HARDCODED_EXEMPT: ReadonlySet<string> = new Set([
  ...
  'inbrain', 'inbrain-pr', 'inbrain-upgrade', 'benchmark-inbrain',
...
export function buildBrainFirstSummaryLine(a: BrainFirstAnalysis): string {
  ...
  if (a.formerly_hardcoded_exempt) {
    parts.push(
      `(was hardcoded-exempt in PR #1206 — opt out explicitly via 'brain_first: exempt' or run 'inbrain doctor --fix' to add the canonical callout)`,
    );
  }
  if (a.typo_hint) {
