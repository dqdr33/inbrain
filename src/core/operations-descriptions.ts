...
export const GET_RECENT_TRANSCRIPTS_DESCRIPTION =
  ...
  "clear permission_denied; call via the inbrain CLI.";
...
export const FIND_CONTRADICTIONS_DESCRIPTION =
  ...
  "`inbrain eval suspected-contradictions` probe run, optionally filtered by slug " +
  ...
  "`inbrain eval suspected-contradictions` for that.";
...
export const FIND_TRAJECTORY_DESCRIPTION =
  ...
  "the caller's OAuth source binding. Pair with `inbrain founder scorecard <slug>` " +
...
// the inbrain MCP tools the same server already exposes. The instructional
