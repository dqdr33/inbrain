 ...
 *
 * The single seam every batch-style inbrain operation routes through.
 ...
 * `INBRAIN_PROGRESSIVE_BATCH_STAGES=10,100,500`.
 *
 * Env overrides:
 *   - INBRAIN_PROGRESSIVE_BATCH_DISABLED=1 — skip ramp, go to full,
 *     stderr-warn at orchestrator entry.
 *   - INBRAIN_PROGRESSIVE_BATCH_AUTO=1 — skip interactive grace
 *     window (cron / launchd / Minion workers).
 *   - INBRAIN_PROGRESSIVE_BATCH_STAGES=10,100,500 — override default
 *     stage list.
 */
...
/**
 * Parse `INBRAIN_PROGRESSIVE_BATCH_STAGES` env override.
 ...
 *   1. env INBRAIN_PROGRESSIVE_BATCH_STAGES (if parseable)
 *   2. Policy.stages
...
export function resolveStages(policy: Policy): number[] {
  const envStages = parseEnvStages(process.env.INBRAIN_PROGRESSIVE_BATCH_STAGES);
 ...
 *
 * INBRAIN_PROGRESSIVE_BATCH_AUTO=1 forces non-interactive.
 */
function isInteractive(): boolean {
  if (process.env.INBRAIN_PROGRESSIVE_BATCH_AUTO === '1') return false;
 ...
 *
 * Defensively removes its listener on exit so other inbrain handlers
...
export async function runProgressiveBatch<T>(
  ...
  const disabled = process.env.INBRAIN_PROGRESSIVE_BATCH_DISABLED === '1';
  if (disabled) {
    process.stderr.write(
      `[progressive-batch] DISABLED via INBRAIN_PROGRESSIVE_BATCH_DISABLED=1 — skipping ramp for label=${label}\n`,
    );
  }
 ...
 *   `import { runProgressiveBatch, type Verifier } from 'inbrain/progressive-batch'`
