 ...
 *
 * Eng-review fold: reuses the v0.17 `~/.inbrain/cycle.lock` PID-liveness
 * pattern (src/core/cycle.ts:acquireFileLock) but scoped per page so two
 * parallel `inbrain takes add` calls + a `takes seed --refresh` running in
 * autopilot can't race on the same `<slug>.md` file.
 *
 * Lock file path: `~/.inbrain/page-locks/<sha256-of-slug>.lock`. SHA-256
...
function lockPathFor(slug: string, lockRoot?: string): string {
  ...
  const dir = lockRoot ?? inbrainPath('page-locks');
