 ...
 *
 * Single source of truth for `inbrain sources status` AND `inbrain doctor`'s
...
export function resolvePriorityLabel(
  ...
  if (!_warnedSources.has(sourceId)) {
    ...
    process.stderr.write(
      `[inbrain] source "${sourceId}": invalid config.priority value ${JSON.stringify(raw)}; ` +
      `falling back to 'normal'. Fix: inbrain sources config set ${sourceId} priority normal\n`,
    );
  }
  return 'normal';
}
...
export async function computeAllSourceMetrics(
  ...
  const now = Date.now();
  // v0.41.32.0: LOCAL callers (inbrain sources status/audit) opt into a live
