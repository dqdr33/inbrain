...
//   3. `inbrain sync --skip-failed` acknowledges a known-bad set.
...
// in `~/.inbrain/sync-failures.jsonl`, one JSON object per line.
//
...
import { inbrainPath as _inbrainPath } from './config.ts';
...
/**
 * Resolve the auto-skip threshold from `INBRAIN_SYNC_AUTOSKIP_AFTER`
 * (default 3). `0` disables the valve entirely (pure fail-closed).
 */
export function resolveAutoSkipThreshold(): number {
  const raw = process.env.INBRAIN_SYNC_AUTOSKIP_AFTER;
 ...
 * sync gate (e.g. `inbrain import`). Increments attempts like the gate.
 */
