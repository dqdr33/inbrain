 ...
 *
 * Writes warn-severity rows to `~/.inbrain/audit/rerank-failures-YYYY-Www.jsonl`
 ...
 * `inbrain doctor reranker_health` reads.
 *
 ...
 * `inbrain doctor`'s `reranker_health` check. Missing file / corrupt rows
...
// stderr label "inbrain" + qualifier "rerank-failure audit " preserve the
// pre-v0.40.4 message byte-for-byte:
//
//   `[inbrain] rerank-failure audit write failed (${msg}); search continues`
//
