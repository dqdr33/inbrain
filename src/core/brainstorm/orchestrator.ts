...
export interface BrainstormOptions {
   ...
   * `~/.inbrain/brainstorm/<run_id>.json`. Set by `--resume <run_id>`.
...
/**
 * Run the brainstorm or LSD pipeline. The CLI command (`inbrain brainstorm` /
 * `inbrain lsd`) calls this with the question + profile; renders the result
...
async function _runBrainstormInner(
  ...
  if (farResult.pages.length === 0) {
    throw new Error(
      `${profile.label}: brain has no usable far pages. Try \`inbrain import <dir>\` to seed cross-domain content, or check the prefix cache via \`inbrain doctor\`.`
    );
  }
  ...
  if (allRawIdeas.length === 0) {
    throw new Error(
      `${profile.label}: no ideas generated across ${crosses.length} crosses. Check API keys via \`inbrain models doctor\`.`
    );
  }
  ...
  if (liveCheckpoint.failed_crosses.length > 0) {
    stderr(
      `[${profile.label}] ${liveCheckpoint.failed_crosses.length} cross(es) failed. Resume with: inbrain ${profile.label} --resume ${runId}\n`,
    );
  } else {
