 ...
 *
 * Persists step-by-step progress through `inbrain post-upgrade` so a partial
 * failure can be resumed via `inbrain upgrade --resume` instead of
...
function checkpointPath(): string {
  return inbrainPath(CHECKPOINT_FILENAME);
}
