 ...
 * inbrain thinks it's on v29 but the actual table is missing columns.
 *
 ...
 *
 * This is a best-effort parser that handles the inbrain schema conventions:
...
export async function verifySchema(engine: BrainEngine): Promise<VerifyResult> {
  ...
  if (result.failed.length > 0) {
    ...
    throw new Error(
      ...
      'Fix: connect directly to Postgres (not through PgBouncer) and run: inbrain apply-migrations --yes'
    );
  }
