 ...
 * that the remote `inbrain serve --http` host is running a newer version (minor
 * or major drift), prompt the user interactively to run `inbrain upgrade`.
 *
 ...
 * - D5 — Re-read `inbrain --version` post-upgrade to verify the binary actually
 *   advanced. `inbrain upgrade` returns 0 even on bun/clawhub catch-and-print
...
function statePath(): string {
  return inbrainPath('upgrade-prompt-state.json');
}
...
function lockPath(): string {
  return inbrainPath(LOCK_FILENAME);
}
...
function defaultVerifyUpgradeAdvanced(remoteVersion: string): { advanced: boolean; newVersion: string | null } {
  try {
    // Spawn `inbrain --version` as a fresh subprocess so we read the NEW binary
    // the upgrade just installed (not the old VERSION constant baked into the
    // currently-running process). Output shape: "inbrain X.Y.Z" or just "X.Y.Z".
    const out = execFileSync('inbrain', ['--version'], { encoding: 'utf-8', timeout: 10_000 });
...
function defaultRunUpgrade(): void {
  execSync('inbrain upgrade', { stdio: 'inherit' });
}
...
export function _clearPromptStateForTest(): void {
  ...
  // INBRAIN_HOME tempdirs for isolation. This stub exists for symmetry with
...
export async function maybePromptForUpgrade(
  ...
  try {
    ...
    if (!result.advanced) {
      writeStateBestEffort(state, mcpUrl, identity.version, 'failed', nowIso);
      log(
        `inbrain upgrade did not actually advance the binary` +
