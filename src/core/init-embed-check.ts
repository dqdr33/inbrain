/**
 * Embedding-key validation at `inbrain init` (issue #1780 Gap 2).
 *
 * Before this, `inbrain init` persisted `--embedding-model` to config.json but
 ...
 * `INBRAIN_INIT_SKIP_EMBED_CHECK=1`. Warnings go to stderr; the caller folds the
...
export interface RunInitEmbedCheckOpts {
  ...
  skipFlag?: boolean;
  // ── test seams ──
  loadFileConfig?: () => InbrainConfig | null;
...
function formatInitEmbedWarning(d: Exclude<EmbeddingDiagnosis, { ok: true }>): string {
  ...
  switch (d.reason) {
    case 'missing_env':
      lines.push(`  Model "${d.model}" needs ${d.missingEnvVars.join(', ')} — not set in your shell or ~/.inbrain/config.json.`);
  ...
  lines.push('  Without it, `inbrain sync` imports pages but embeds 0 (search + code graph stay empty).');
  lines.push('  Fixes:');
  lines.push('    • Set the key above, then run `inbrain sync`.');
  lines.push('    • Or defer embedding entirely: re-run init with --no-embedding.');
  lines.push('    • Or skip this check: --skip-embed-check (or INBRAIN_INIT_SKIP_EMBED_CHECK=1).');
  return lines.join('\n');
}
...
function formatLiveProbeWarning(p: { reason: string; message: string }, model: string): string {
  return [
    ...
    `  Model: ${model}`,
    `  Error: ${p.message}`,
    '  `inbrain sync` may fail to embed. Verify the key/endpoint, or re-run init',
...
export async function runInitEmbedCheck(opts: RunInitEmbedCheckOpts): Promise<InitEmbedCheckResult> {
  ...
  if (process.env.INBRAIN_INIT_SKIP_EMBED_CHECK === '1') return { ok: true, skipped: 'env' };
  ...
  const fileCfg = loadFile() ?? ({} as InbrainConfig);
  const effective: InbrainConfig = {
    ...fileCfg,
