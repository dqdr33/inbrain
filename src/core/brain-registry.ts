/**
 * BrainRegistry — connected inbrains (v0.19, PR 0).
 *
 * A registry of BrainEngine handles keyed by brainId. Supports:
 *   - 'host': the brain defined by ~/.inbrain/config.json (single-brain default).
 *   - <mount-id>: brains declared in ~/.inbrain/mounts.json.
 *
...
import { InbrainError } from './types.ts';
import { loadConfig, type InbrainConfig } from './config.ts';
 ...
 *
 * v0.40.3.0: INBRAIN_MOUNTS_PATH override exists for tests (homedir() is
...
function getMountsPath(): string {
  ...
  return join(homedir(), '.inbrain', 'mounts.json');
}

/**
 * A single entry in ~/.inbrain/mounts.json.
 *
...
export interface MountEntry {
  ...
  enabled?: boolean;
  /** Managed by `inbrain mounts sync` (PR 1). */
  expected_sha?: string;
  /** Managed by `inbrain mounts sync` (PR 1). */
   ...
   * `inbrain mounts trust-frontmatter <id>`. The host source (id='default')
...
/** Top-level shape of ~/.inbrain/mounts.json. */
...
export interface BrainHandle {
  ...
  engine: BrainEngine;
  /** InbrainConfig used to create the engine. */
...
export class DuplicateMountPathError extends InbrainError {
  constructor(path: string, existingId: string, attemptedId: string) {
    super(
      ...
      'Use a different local clone path, or remove the existing mount first: ' +
        `inbrain mounts remove ${existingId}`,
    );
  }
}

/** Error thrown when a caller requests an unknown or disabled brain id. */
export class UnknownBrainError extends InbrainError {
  constructor(id: string, available: string[]) {
    ...
    super(
      ...
      `Run 'inbrain mounts list' to see registered mounts. Add a new mount with 'inbrain mounts add ${id} --path <path> --db-url <url>'.`,
    );
  }
}
...
export function loadMounts(mountsPath: string = getMountsPath()): MountEntry[] {
  ...
  try {
    ...
    throw new InbrainError(
      ...
      e instanceof Error ? e.message : String(e),
      `Fix the JSON syntax at ${mountsPath} or remove it and re-add mounts via 'inbrain mounts add'`,
    );
  }
  ...
  if (file.version !== 1) {
    throw new InbrainError(
      ...
      `This inbrain binary supports version 1`,
      `Upgrade inbrain or regenerate mounts.json`,
    );
  }
...
/** Convert a MountEntry to a InbrainConfig (for OperationContext). */
function mountToInbrainConfig(mount: MountEntry): InbrainConfig {
  return {
    engine: mount.engine,
    database_url: mount.database_url,
    database_path: mount.database_path,
...
export class BrainRegistry {
  ...
  /**
   * Return the host brain handle (from ~/.inbrain/config.json). Lazy-
  ...
  private async initHostBrain(): Promise<BrainHandle> {
    ...
    if (!config) {
      throw new InbrainError(
        ...
        '~/.inbrain/config.json is missing and INBRAIN_DATABASE_URL is unset',
        "Run 'inbrain init' to configure the host brain",
      );
    }
  ...
  private async initMountBrain(mount: MountEntry): Promise<BrainHandle> {
    ...
    return {
      id: mount.id,
      engine,
      config: mountToInbrainConfig(mount),
      path: mount.path,
    };
  }
}
