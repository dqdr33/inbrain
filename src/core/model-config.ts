 ...
 *   5. Env var (process.env[envVar] or INBRAIN_MODEL)
...
export interface ResolveModelOpts {
  ...
  /** Env var to consult after global default. Defaults to `INBRAIN_MODEL`. */
  envVar?: string;
  /**
 ...
 *
 * Users override via `inbrain config set models.tier.<tier> <model>`.
 */
...
export async function resolveModel(
  ...
  const envVar = opts.envVar ?? 'INBRAIN_MODEL';
  ...
  if (verdict === 'unusable:no_tools' || verdict === 'unknown') {
    if (!_subagentTierWarningsEmitted.has(key)) {
      ...
      process.stderr.write(
        ...
        `Fix: inbrain config set models.tier.subagent <provider>:<model-with-tools>\n`,
      );
    }
