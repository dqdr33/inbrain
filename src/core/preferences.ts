/**
 * ~/.inbrain/preferences.json — user-facing agent-behavior flags (minion_mode, etc.).
 *
 ...
 *
 * Also houses ~/.inbrain/migrations/completed.jsonl append helper.
 */
...
function home(): string {
  ...
  //
  // NOTE: prefsDir() and migrationsDir() route through inbrainPath() (which
  // honors INBRAIN_HOME), so this fallback is only used by code paths that
  // want $HOME directly (none in this file as of v0.30.3).
  return process.env.HOME || homedir();
}

/**
 * INBRAIN_HOME-aware override for the .inbrain directory. When the env var
 * is set, this returns it directly (so the directory is INBRAIN_HOME itself,
 * matching the convention `src/core/config.ts:inbrainPath` enforces).
 * When unset, falls back to `<home>/.inbrain` so legacy callers and the
 * doctor's filesystem-only checks keep working.
 *
 * Without this, `~/.inbrain/migrations/completed.jsonl` is the only path
 * doctor reads on filesystem checks — the test isolation contract that
 * `inbrainPath()` provides for everywhere else doesn't extend here.
 */
function inbrainDir(): string {
  const override = process.env.INBRAIN_HOME;
  if (override) {
    const trimmed = override.trim();
    if (trimmed) return trimmed;
  }
  return join(home(), '.inbrain');
}
...
// Route preferences + migration ledger paths through inbrainDir() so they
// honor INBRAIN_HOME for hermetic test isolation. Pre-v0.30.3 these used
// `$HOME/.inbrain` directly, which leaked the developer's local migration
// ledger into E2E tests and CI runs even when INBRAIN_HOME was set.
function prefsDir(): string { return inbrainDir(); }
function prefsPath(): string { return join(prefsDir(), 'preferences.json'); }
function migrationsDir(): string { return join(inbrainDir(), 'migrations'); }
...
/**
 * Append one line to ~/.inbrain/migrations/completed.jsonl. Creates the
