 ...
 *
 * `inbrain init --embedding-dimensions N` on an existing brain whose
 ...
 * to kill.
 *
 * Loud-failure path: `inbrain init` AND `inbrain doctor` both consult this
 ...
 * config.json. Every embed callsite (CLI: `inbrain embed`, `inbrain import`;
...
export function assertEmbeddingEnabled(cfg: { embedding_disabled?: boolean } | null): void {
  if (cfg?.embedding_disabled) {
    throw new EmbeddingDisabledError(
      ...
      '  inbrain config set embedding_model <provider>:<model>\n' +
      '  inbrain config set embedding_dimensions <N>\n' +
      '  inbrain init --force --embedding-model <provider>:<model>   # re-init to size schema\n',
    );
  }
}
 ...
 *   via `inbrain init --pglite --embedding-model X --embedding-dimensions N`.
 *   The recipe derives the active database path so users don't paste a
 *   stale literal that ignores `INBRAIN_HOME` / `--path` / their config.
 * - **Postgres** keeps the existing four-step SQL recipe.
 *
 * The old recipe pointed at `inbrain config set embedding_model X` which
...
export interface EmbeddingMismatchOpts {
   ...
   * omitted, falls back to the default `inbrainPath('brain.pglite')`.
   * Resolving at the call site is preferred because the caller knows
   * about `--path` flags and `INBRAIN_HOME` overrides.
   */
  databasePath?: string;
}
...
export function embeddingMismatchMessage(opts: EmbeddingMismatchOpts): string {
  ...
  if (engineKind === 'pglite') {
    const activePath = databasePath ?? inbrainPath('brain.pglite');
    ...
    const lines = [
      ...
      ``,
      `  inbrain reinit-pglite${modelArg} --embedding-dimensions ${requestedDims}`,
      ``,
      `Or by hand:`,
      ``,
      `  mv ${activePath} ${activePath}.bak`,
      `  inbrain init --pglite${modelArg} --embedding-dimensions ${requestedDims}`,
      `  inbrain sync   # re-imports your brain repo from disk`,
      `  inbrain embed --stale`,
      ``,
  ...
  const lines = [
    ...
    `  inbrain init --supabase${modelArg} --embedding-dimensions ${requestedDims}`,
    `  inbrain embed --stale`,
    ``,
    `Full guide: docs/embedding-migrations.md`,
  ];
    ...
    if (!tp) {
      return {
        ...
        error:
          ...
          `Pick a recipe with an embedding touchpoint (inbrain providers list).`,
      };
    }
    ...
    if (!tp.supports_multimodal) {
      return {
        ...
        error:
          ...
          `Run \`inbrain providers list\` to see touchpoint coverage.`,
      };
    }
...
//      `inbrain doctor` `embedding_dim_mismatch` check to surface drift.
...
export async function assertFactsEmbeddingDimMatchesConfig(engine: BrainEngine): Promise<void> {
  ...
  const message = [
    ...
    recipe,
    ``,
    `Or run \`inbrain doctor --json\` for the full diagnostic + fix surface.`,
  ].join('\n');
