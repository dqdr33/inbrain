...
/**
 * Storage tier configuration loaded from inbrain.yml.
 *
...
/**
 * Parse the inbrain.yml shape: a top-level `storage:` section with up to four
 ...
 * shape inbrain controls. Trades expressiveness for zero-dep parsing and
 ...
 *      once-per-process deprecation warning suggesting `inbrain doctor --fix`.
...
function normalizeStorageConfig(raw: RawStorage): StorageConfig {
  ...
  if (hasDeprecated && !_deprecationWarned) {
    ...
    if (hasCanonical) {
      console.warn(
        `Warning: ${which} in inbrain.yml is deprecated and ignored ` +
          `(canonical keys db_tracked/db_only are present). ` +
          `Remove the deprecated keys, or run \`inbrain doctor --fix\`.`,
      );
    } else {
      console.warn(
        `Warning: ${which} in inbrain.yml is deprecated. ` +
          `Rename to db_tracked / db_only — see docs/storage-tiering.md. ` +
          `Run \`inbrain doctor --fix\` for an automated rename.`,
      );
    }
  }
...
/**
 * Load inbrain.yml configuration from the brain repository root.
 *
 * Returns null when:
 *   - repoPath is null/undefined
 *   - inbrain.yml doesn't exist at the repo root
 *   - inbrain.yml exists but has no `storage:` section (with sanity warning)
 *
 * Throws when:
 *   - inbrain.yml exists but is unreadable (permission denied, etc.) — D36 lock:
...
export function loadStorageConfig(repoPath?: string | null): StorageConfig | null {
  ...
  const yamlPath = join(repoPath, 'inbrain.yml');
  ...
  try {
    ...
    console.warn(
      `Warning: Failed to parse inbrain.yml: ${error instanceof Error ? error.message : String(error)}`,
    );
    return null;
  }
    ...
    if (!_missingStorageWarned) {
      ...
      console.warn(
        `Warning: ${yamlPath} exists but has no storage configuration. ` +
          ...
          `or remove inbrain.yml to suppress this warning.`,
      );
    }
    return null;
  }
...
export function normalizeAndValidateStorageConfig(input: StorageConfig): StorageConfig {
  ...
  if (allChanged.length > 0 && !_normalizationInfoEmitted) {
    ...
    console.warn(
      `Note: normalized ${allChanged.length} storage path(s) in inbrain.yml — ` +
  ...
  for (const path of dbonly.normalized) {
    if (trackedSet.has(path)) {
      throw new StorageConfigError(
        `inbrain.yml: directory "${path}" appears in both db_tracked and db_only — ` +
          `pick one tier. Edit inbrain.yml to remove the overlap.`,
      );
    }
  }
