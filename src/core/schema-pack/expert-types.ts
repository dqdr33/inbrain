...
// `expert_routing: true`. inbrain-base preserves person + company as
...
// the hardcoded DEFAULT_TYPES = ['person', 'company'] — which inbrain-
 ...
 * applicable" rather than falling back to inbrain-base defaults (the
 * pack made an explicit choice).
 *
 * For inbrain-base, this returns ['person', 'company'] (the pre-v0.38
...
export function expertTypesFromPackOrThrow(
  ...
  if (types.length === 0) {
    throw new Error(
      ...
      `expert_routing: true. \`inbrain whoknows\` and \`find_experts\` ` +
