// v0.39 T3 — inbrain schema suggest: LLM-powered runSuggest library.
//
...
export async function runSuggest(
  ...
  if (detected.untyped_pages > 0 && suggestions.length === 0) {
    notes.push(`${detected.untyped_pages} untyped pages detected but no suggestions produced — run \`inbrain schema review-candidates --json\` to see the disk-derived candidate set.`);
  }
