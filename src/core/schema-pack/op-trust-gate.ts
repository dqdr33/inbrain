...
export function validateSchemaPackTrustGate(
  ...
  if (ctx.remote !== false) {
    throw new SchemaPackTrustGateError(
      ...
      'Pass via inbrain.yml `schema:` section, ~/.inbrain/config.json `schema_pack`, ' +
      'INBRAIN_SCHEMA_PACK env var, or `inbrain config set schema_pack <name>`. ' +
    ...
    if (scope.sourceIds.length === 1) {
      ...
      for (const sid of scope.sourceIds) {
        const res = resolveActivePackName({
          ...
          envVar: process.env.INBRAIN_SCHEMA_PACK?.trim() || undefined,
          sourceId: sid,
