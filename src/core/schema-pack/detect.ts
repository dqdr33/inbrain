// v0.39 T2 — inbrain schema detect: SQL-driven heuristic clustering.
//
...
// validated + applied via `inbrain schema use <candidate-name>` after
...
export interface DetectResult {
  ...
  total_pages: number;
  /** Page count that already has a frontmatter type matching inbrain-base. */
  typed_pages: number;
...
export function buildCandidate(opts: {
  ...
  return {
    ...
    version: '0.0.1',
    description: 'Auto-detected from brain shape via `inbrain schema detect`. Review with `inbrain schema review-candidates` before activating.',
    page_types,
