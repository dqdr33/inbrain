...
export const BUNDLED_PACK_NAMES = new Set(['inbrain-base', 'inbrain-recommended', 'inbrain-base-v2']);
...
/**
 * Locate a user-mutable pack file. Bundled packs (inbrain-base,
 * inbrain-recommended) are explicitly refused per D6 — they live inside
...
export function locateMutablePackFile(name: string): { path: string; format: PackFileFormat } {
  if (BUNDLED_PACK_NAMES.has(name)) {
    throw new SchemaPackMutationError(
      'PACK_READONLY',
      `pack '${name}' is bundled and read-only. Use 'inbrain schema fork ${name} <new-name>' to create a writable copy.`,
      { pack: name },
    );
  }
  ...
  throw new SchemaPackMutationError(
    'PACK_NOT_FOUND',
    `no pack file at ${baseDir}. Run 'inbrain schema init ${name}' or 'inbrain schema fork <source> ${name}' first.`,
    { pack: name, baseDir },
  );
}
