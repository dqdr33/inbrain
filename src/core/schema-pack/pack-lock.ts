...
// the same pack file across processes. Two `inbrain schema add-type foo`
...
//
// Lock path: `~/.inbrain/schema-packs/.locks/<packName>.lock`. Per-pack
// so two different packs never block each other. Honors `INBRAIN_HOME`
// via the shared `inbrainPath()` helper.
...
function resolveLockPath(packName: string, lockDir?: string): string {
  const dir = lockDir ?? inbrainPath('schema-packs', '.locks');
