  ...
  for (let i = 0; i < opts.rules.length; i++) {
    ...
    if (apply && would_convert > 0) {
      ...
      if (linksBuffer.length > 0) {
        await ctx.engine.addLinksBatch(linksBuffer); // inbrain-allow-direct-insert: page-to-link mapping_rules under unify-types convert edge-shaped pages to canonical link rows; PROTECTED Minion handler, source-scoped, atomic per-rule
