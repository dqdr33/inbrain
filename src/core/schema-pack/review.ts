// v0.39 T4 — inbrain schema review-candidates + T5 review-orphans.
//
...
import { loadConfig, inbrainPath, configPath } from '../config.ts';
...
export async function runReviewCandidates(
  ...
  let activePackName = 'inbrain-base';
  try {
  ...
  if (opts.applySlug) {
    ...
    // ~/.inbrain/schema-pack-deltas/<active>-<timestamp>.json so users can
    // review + merge into their pack via `inbrain schema edit`.
    const deltaDir = inbrainPath('schema-pack-deltas');
