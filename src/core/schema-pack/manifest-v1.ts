...
// here is the SOURCE OF TRUTH for what a `.inbrain-schema` tarball can carry.
//
...
export const SCHEMA_PACK_API_VERSION = 'inbrain-schema-pack-v1' as const;
...
const ExtractableSpecSchema = z.object({
   ...
   * array; specific values consumed by `inbrain extract benchmark`. */
  eval_dimensions: z.array(z.string()).default([]),
  /** Optional recall floor for `inbrain extract benchmark` CI gate.
 ...
 * can declare `pricing_judgment` or `hiring_quality` without a inbrain
 ...
 * algorithms ship via inbrain release; new domain names ship via pack
 ...
 * aggregation needs. Each addition is a versioned inbrain release.
 */
 ...
 * load with a paste-ready `inbrain models doctor`-style hint.
 */
 ...
 * `migration_from: {pack: inbrain-base, version: "1.x"}`, the
...
export const SchemaPackManifestSchema = z.object({
  ...
  /** v0.38 — minimum inbrain version required to load this pack. */
  inbrain_min_version: z.string().regex(/^\d+\.\d+\.\d+(?:\.\d+)?$/).default('0.38.0'),
  /** Parent pack (one of: 'inbrain-base', another installed pack name, or null for full override). */
  extends: z.string().nullable().default('inbrain-base'),
