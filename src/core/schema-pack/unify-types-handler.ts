...
// `inbrain onboard` owns the prompt):
...
//   3. Acquire `inbrain-unify` db-lock (mirrors inbrain-sync pattern).
...
export interface UnifyTypesInput {
  /** The pack name to upgrade TO (e.g. 'inbrain-base-v2'). */
  target_pack: string;
...
export async function runUnifyTypes(
  ...
  if (apply) {
    lockHandle = await tryAcquireDbLock(ctx.engine, 'inbrain-unify', 60);
    if (lockHandle === null) {
      throw new Error(
        `[unify-types] could not acquire inbrain-unify db-lock (held by another process). ` +
        `Wait for the other unify run to complete (lock TTL: 60min).`,
      );
    }
    onProgress(`[unify-types] inbrain-unify lock acquired`);
  }
  ...
  try {
    ...
    if (apply) {
      ...
      //     callers that read from ~/.inbrain/config.json (homeConfig tier).
      ...
      try {
        ...
        warnings.push(
          ...
          `${(e as Error).message}. Run \`inbrain schema use ${input.target_pack}\` ` +
    ...
    if (lockHandle !== null) {
      try {
        ...
        onProgress(`[unify-types] inbrain-unify lock released`);
      } catch (e) {
        onProgress(
