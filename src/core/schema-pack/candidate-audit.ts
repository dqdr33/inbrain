...
// `inbrain schema review-candidates` and decide promote/rename/ignore.
//
...
//     Opt in to full type names with `INBRAIN_SCHEMA_AUDIT_VERBOSE=1`.
...
// `~/.inbrain/audit/schema-candidates-YYYY-Www.jsonl`. Honors
// `INBRAIN_AUDIT_DIR` env override.
//
...
export interface CandidateAuditRecord {
  ...
  /** Rolling counter — useful for `inbrain schema review-candidates` ordering. */
  count: number;
...
export function isAuditVerbose(): boolean {
  return process.env.INBRAIN_SCHEMA_AUDIT_VERBOSE === '1';
}
 ...
 * `inbrain schema review-candidates`. Returns rows in disk order (chronological
