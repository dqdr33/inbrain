...
//      - Built-in 'inbrain-base' lives at src/core/schema-pack/base/inbrain-base.yaml
//      - User packs live at ~/.inbrain/schema-packs/<name>/pack.yaml
...
// writing to `~/.inbrain/schema-packs/`.
...
export interface LoadActivePackInput {
  /** Loaded Inbrain config (file + env merged). Pass null for default-only resolution. */
  cfg: InbrainConfig | null;
  ...
  sourceId?: string;
  /** Tier-3 per-source DB config map (from `inbrain config get` keyspace). */
  perSourceDb?: ReadonlyMap<string, string>;
  /** Tier-5 inbrain.yml schema.pack field (already parsed by storage-config). */
  inbrainYml?: string;
 ...
 * Production wires this to the built-in + ~/.inbrain/schema-packs lookup.
 ...
 * without writing to ~/.inbrain. Always pair with `_resetPackLocatorForTests`
 ...
 *   'inbrain-base' → bundled src/core/schema-pack/base/inbrain-base.yaml
 *   other         → ~/.inbrain/schema-packs/<name>/pack.yaml or pack.json
 *
...
function defaultPackLocator(name: string): string | null {
  // v0.39 T8 — bundled packs registry. inbrain-base + inbrain-recommended
  ...
  const BUNDLED: ReadonlyArray<string> = [
    ...
    'inbrain-engineer',
    'inbrain-everything',
    // v0.42 type-unification: 15-type canonical successor to inbrain-base.
    // Ships as install default (Lane E T17) + via inbrain onboard pack
  ...
  // User-installed pack at ~/.inbrain/schema-packs/<name>/pack.{yaml,json}
 ...
 * manifest from disk. Used by `inbrain schema active` to surface
 * provenance ("active pack: garry — source: inbrain.yml") without
 ...
 * inbrain-base@1.x; inbrain-base-v2@1.0.0 is available." Results sorted by
 * successor version descending so the highest-available successor wins.
 *
 * Walks BUNDLED_PACK_NAMES + any installed pack under `~/.inbrain/schema-packs/`
...
export async function findPackSuccessors(
  ...
  // Walk ~/.inbrain/schema-packs/* via the locator. We can't enumerate
  // directly without filesystem scan; defer to v0.43+ for installed-pack
  // enumeration. Bundled packs alone cover v0.42's inbrain-base→v2 path.
...
function buildResolutionInput(input: LoadActivePackInput): ResolutionInput {
  const envVar = process.env.INBRAIN_SCHEMA_PACK?.trim() || undefined;
  // tier-6: ~/.inbrain/config.json schema_pack field
