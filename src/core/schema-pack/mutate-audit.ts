...
// to `~/.inbrain/audit/schema-mutations-YYYY-Www.jsonl`. The audit lets:
//   - `inbrain doctor` detect anomalous mutation patterns (Phase 9)
...
//   `~/.inbrain/audit/` and a single leaked screenshot of either could
//   otherwise reveal sensitive taxonomy (mental_health_diagnosis,
//   patients/oncology/, contracts/litigation/, etc.).
//
//   Opt out of redaction with `INBRAIN_SCHEMA_AUDIT_VERBOSE=1` (same env
...
/**
 * Aggregate mutation records for cross-surface parity. Both `inbrain
 * doctor` (Phase 9) and `inbrain schema audit` (future surface) MUST
