...
//   6. `~/.inbrain/config.json schema_pack` field
...
//     STAT_TTL_MS, env override INBRAIN_PACK_STAT_TTL_MS). Inside the
...
export interface ResolutionResult {
  pack_name: string;
  source: 'per-call' | 'env' | 'per-source-db' | 'db-config' | 'inbrain-yml' | 'home-config' | 'default';
}
...
export function resolveActivePackName(input: ResolutionInput): ResolutionResult {
  ...
  if (input.inbrainYml) return { pack_name: input.inbrainYml, source: 'inbrain-yml' };
  if (input.homeConfig) return { pack_name: input.homeConfig, source: 'home-config' };
  return { pack_name: 'inbrain-base', source: 'default' };
}
 ...
 * `INBRAIN_PACK_STAT_TTL_MS` env override. Invalid values fall back to
 * the default with no warning (this is a power-user knob).
 */
function resolveStatTtlMs(): number {
  const raw = process.env.INBRAIN_PACK_STAT_TTL_MS;
 ...
 * successful pack mutation; also exposed via `inbrain schema reload`.
 */
