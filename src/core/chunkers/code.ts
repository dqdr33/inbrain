...
// after a inbrain upgrade. See A2 / C2 in the v0.19.0 plan.
//
...
function resolveChunkerTimeoutMs(): number {
  const raw = process.env.INBRAIN_CHUNKER_TIMEOUT_MS;
  if (raw) {
    const n = Number(raw);
  ...
  try {
    ...
    try {
      ...
      if (e instanceof ChunkerTimeoutError) {
        console.warn(
          `[inbrain chunker] timeout parsing ${filePath} after ${timeoutMs}ms; ` +
...
function extractSymbolName(node: any): string | null {
  ...
  // The `statement` wrapper is unique to SQL among inbrain's 37 grammars
...
// inbrain's code-def is a DDL retrieval signal, not a DML one.
