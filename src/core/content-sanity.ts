 ...
 *     so EVERY ingestion path inherits it (sync, inbrain import, put_page
 *     MCP op, inbrain capture, POST /ingest webhook via ingest_capture).
 ...
 *
 * The kill-switch (`INBRAIN_NO_SANITY=1` / `content_sanity.disabled: true`)
 ...
 *  `content_sanity.bytes_warn` config key or `INBRAIN_PAGE_WARN_BYTES`
 ...
 *  `content_sanity.bytes_block` config key or `INBRAIN_PAGE_BLOCK_BYTES`
...
export const BUILT_IN_JUNK_PATTERNS: ReadonlyArray<JunkPattern> = Object.freeze([
  ...
  // (`~/.inbrain/audit/content-sanity-YYYY-Www.jsonl`) and doctor's
...
export function assessContentSanity(opts: {
   ...
   *  `~/.inbrain/junk-substrings.txt` via `src/core/content-sanity-literals.ts`.
