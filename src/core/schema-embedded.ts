...
-- Inbrain Postgres + pgvector schema
...
-- name:       mutable display label. Rename via \`inbrain sources rename\`.
...
CREATE TABLE IF NOT EXISTS pages (
  ...
  -- v0.37.0 (migration v79): real stale-page signal for \`inbrain lsd\`. Bumped
  ...
  -- path). Powers \`inbrain extract --stale\` + the \`links_extraction_lag\` doctor
...
-- scans (\`extract --stale --source X\`, \`inbrain doctor --source X\`) are indexed;
...
-- v0.41.18.0 (codex finding #9): partial index for \`inbrain embed --stale\`
...
CREATE TABLE IF NOT EXISTS links (
  ...
  -- without a inbrain migration. Format gate only: lowercase kebab, <=64 chars.
...
-- ============================================================
-- v0.41.18.0 (inbrain onboard wave). Every completion captured by the
-- onboard remediation pipeline records before/after metric stats so
-- \`inbrain onboard --history --json\` can show "you reduced orphans 47%".
...
CREATE TABLE IF NOT EXISTS subagent_tool_executions (
  ...
  schema_version      INTEGER     NOT NULL DEFAULT 1,
  provider_id         TEXT,
  -- v0.38 D11: inbrain-owned stable IDs (ordinal assigned at first
  -- observation of a tool call; inbrain_tool_use_id is uuid v7). Reconciliation
  -- on crash-replay uses (job_id, message_idx, ordinal) as the unique key.
  -- Legacy rows (pre-v82) have ordinal=NULL + inbrain_tool_use_id=NULL and
  -- are resolved by the read-time D5 shim that recomputes the key from
  -- (job_id, message_idx, content_blocks index, tool_name).
  ordinal             INTEGER,
  inbrain_tool_use_id  UUID,
...
-- autopilot-cycle handler, inbrain dream CLI) tries to acquire this
...
CREATE TABLE IF NOT EXISTS inbrain_cycle_locks (
  ...
  -- refresh tick. Used by \`inbrain sync --break-lock --max-age <s>\` to
...
-- by \`inbrain doctor\` (in-process counters can't bridge MCP server + doctor
...
CREATE TABLE IF NOT EXISTS eval_candidates (
  ...
  -- \`inbrain eval replay\` reproduces the same column the capture ran against.
...
-- \`inbrain think --ab\`. One row per side-by-side comparison.
...
-- ============================================================
-- The postgres role (used by inbrain via pooler) has BYPASSRLS.
