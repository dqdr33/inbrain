 ...
 * `find_trajectory` MCP op and the `inbrain eval trajectory` CLI consume
 ...
 *     env var `INBRAIN_TRAJECTORY_REGRESSION_THRESHOLD` (default 0.10).
...
/**
 * Read the regression threshold from `INBRAIN_TRAJECTORY_REGRESSION_THRESHOLD`
...
export function resolveRegressionThreshold(): number {
  const raw = process.env.INBRAIN_TRAJECTORY_REGRESSION_THRESHOLD;
