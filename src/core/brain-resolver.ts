 ...
 *   2. INBRAIN_BRAIN_ID env var.
 *   3. .inbrain-mount dotfile in CWD or any ancestor directory.
 *   4. Registered mount whose `path` contains CWD (longest-prefix match).
 *   5. Brain-level default (future: ~/.inbrain/config.json `brains.default`).
...
const DOTFILE = '.inbrain-mount';
/** Same regex as brain-registry. Kept in sync. */
const BRAIN_ID_RE = /^[a-z0-9](?:[a-z0-9-]{0,30}[a-z0-9])?$/;

/**
 * Walk up from startDir looking for a .inbrain-mount dotfile. Returns the
 ...
 * @param cwd  Working directory for .inbrain-mount walk. Defaults to process.cwd().
 * @param mountsLoader  Override for testability. Returns the list of enabled
 *                      mounts. Defaults to reading ~/.inbrain/mounts.json.
...
export function resolveBrainId(
  ...
  // 2. Env var.
  const env = process.env.INBRAIN_BRAIN_ID;
  if (env && env.length > 0) {
    if (env === HOST_BRAIN_ID) return HOST_BRAIN_ID;
    validateMountId(env, 'INBRAIN_BRAIN_ID');
    return env;
  }
