/**
 * archive-crawler-config.ts — inbrain.yml `archive-crawler:` section.
 *
 ...
 * inbrain.yml. This is a deliberate safety fence against the agent
...
export function normalizeAndValidateArchiveCrawlerConfig(
  ...
  if (rawScan.length === 0) {
    throw new ArchiveCrawlerConfigError(
      'archive-crawler.scan_paths is empty. The skill refuses to run without an explicit allow-list. Add at least one path under archive-crawler.scan_paths in inbrain.yml.',
      'empty_scan_paths',
    );
  }
...
/**
 * Load inbrain.yml from the brain repo root and return the
 ...
 *
 * Throws:
 *   - ArchiveCrawlerConfigError(missing_section) when inbrain.yml
 *     exists but has no archive-crawler section (or inbrain.yml is
 *     absent entirely).
...
export function loadArchiveCrawlerConfig(
  ...
  if (!repoPath) {
    throw new ArchiveCrawlerConfigError(
      'No brain repo path provided. archive-crawler requires a brain repo with inbrain.yml. Run `inbrain init` or set sync.repo_path in inbrain config.',
      'missing_section',
    );
  }
  ...
  if (!existsSync(yamlPath)) {
    throw new ArchiveCrawlerConfigError(
      `inbrain.yml not found at ${yamlPath}. archive-crawler refuses to run without an explicit allow-list — add an archive-crawler section to inbrain.yml first.`,
      'missing_section',
    );
  }
