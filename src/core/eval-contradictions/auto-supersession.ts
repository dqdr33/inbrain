...
export function renderResolutionCommand(
  ...
  switch (kind) {
    case 'takes_supersede': {
      ...
      return `inbrain takes supersede ${takeSide.slug} --row ${takeId}`;
    }
    case 'dream_synthesize': {
      const curatedSide = isCuratedEntitySlug(pair.a.slug)
        ? pair.a
        : (isCuratedEntitySlug(pair.b.slug) ? pair.b : pair.a);
      return `inbrain dream --phase synthesize --slug ${curatedSide.slug}`;
    }
    case 'takes_mark_debate': {
      const takeSide = pair.b.take_id !== null ? pair.b : (pair.a.take_id !== null ? pair.a : pair.a);
      const takeId = takeSide.take_id ?? '<row>';
      return `inbrain takes mark-debate ${takeSide.slug} --row ${takeId}`;
    }
    case 'temporal_supersede': {
      ...
      if (aDate && bDate) {
        ...
        if (olderTakeId !== null) {
          return `inbrain takes supersede ${olderSide.slug} --row ${olderTakeId} --since ${newerDate}`;
        }
    ...
    case 'log_timeline_change': {
      ...
      return `# temporal_evolution: ${pair.a.slug} (${aDate}) → ${pair.b.slug} (${bDate}); record in timeline when the inbrain timeline writer lands`;
    }
