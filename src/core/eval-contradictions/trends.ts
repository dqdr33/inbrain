 ...
 * Render helpers produce the plain-text chart for `inbrain eval
...
export function renderTrendChart(rows: readonly TrendRow[]): string {
  if (rows.length === 0) {
    return 'No contradiction-probe runs in this window. Run `inbrain eval suspected-contradictions` to populate.';
  }
