 ...
 *     INBRAIN_PROBE_PROMPT_GRACE_SECONDS).
 *   - Non-TTY auto-proceeds with a stderr note (autopilot path).
 *   - INBRAIN_NO_PROBE_PROMPT=1 skips entirely.
 *
 ...
 *   v0.41.14.0+ once `inbrain eval suspected-contradictions` is fully
 ...
 * --yes override, INBRAIN_NO_PROBE_PROMPT, TTY detection, and the persisted
...
export async function maybePromptForCostBeforeProbe(
  ...
  if (process.env.INBRAIN_NO_PROBE_PROMPT === '1') {
  ...
  if (!isTty) {
    ...
    stderr(`${banner}\nNon-TTY: proceeding automatically. Set INBRAIN_NO_PROBE_PROMPT=1 to suppress.\n`);
    return { kind: 'proceed', reason: 'non_tty_auto' };
  }

  const graceRaw = process.env.INBRAIN_PROBE_PROMPT_GRACE_SECONDS;
