 ...
 *
 * Letting these fire as two separate prompts on the same `inbrain upgrade`
 ...
 *   [fresh brain]
 *        |
 *        |  user picks "s" or runs `inbrain ze-switch`
 *        v
 ...
 *        v
 *   applied=true  -> stable. Re-embed via `inbrain embed --stale` or autopilot.
 *
 ...
 * INBRAIN_EMBEDDING_* env vars would override the target at runtime.
 ...
 *
 * `process.env.INBRAIN_EMBEDDING_MODEL` and `INBRAIN_EMBEDDING_DIMENSIONS`
...
export function detectEnvOverride(
  ...
  const envModel = env.INBRAIN_EMBEDDING_MODEL?.trim();
  if (envModel && envModel !== targetModel) {
    vars.push({ name: 'INBRAIN_EMBEDDING_MODEL', current: envModel, target: targetModel });
  }
  const envDimRaw = env.INBRAIN_EMBEDDING_DIMENSIONS?.trim();
  if (envDimRaw) {
    const envDim = Number(envDimRaw);
    if (!Number.isFinite(envDim) || envDim !== targetDim) {
      vars.push({
        name: 'INBRAIN_EMBEDDING_DIMENSIONS',
        current: envDimRaw,
 ...
 * and suggests `inbrain ze-switch --resume`.
 */
export async function applyRetrievalUpgrade(
  ...
  // responsibility to invoke the existing `inbrain reindex --markdown` flow.
