...
export function loadBenchmark(
  ...
  try {
    ...
    throw errorFor({
      ...
      hint: `Auto-generate a starter with 'inbrain skillopt <skill> --bootstrap-from-skill' (reads SKILL.md), then review + run with --bootstrap-reviewed --split 1:1:1. Or pass --bootstrap-from-routing if a routing-eval.jsonl exists.`,
    });
  }
