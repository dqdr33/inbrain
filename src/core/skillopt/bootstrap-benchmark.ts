...
export async function runBootstrap(opts: BootstrapOpts): Promise<BootstrapResult> {
  ...
  if (!fs.existsSync(routingPath)) {
    throw errorFor({
      ...
      hint: `Create a routing-eval.jsonl file first (inbrain skillify scaffold <name> generates one).`,
    });
  }
...
export async function runBootstrapFromSkill(opts: BootstrapFromSkillOpts): Promise<BootstrapResult> {
  ...
  process.stderr.write(`[skillopt]   inbrain skillopt ${skillName} --bootstrap-reviewed --split 1:1:1\n`);
