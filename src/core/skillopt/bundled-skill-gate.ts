 ...
 *
 * A "bundled" skill is one that lives in the inbrain repo's `skills/` tree
 ...
 * human review.
 *
 * User-owned skills (under a user's `~/.inbrain/skills/` or their own
 ...
 *     (i.e. it's `<inbrain-install>/skills`). If so, this is a bundled skill.
...
export function getBundledSkillContext(skillsDir: string, skillName: string): BundledSkillContext {
  ...
  // if the resolved skillsDir is under the inbrain install tree (somewhere
  // up the chain has node_modules/inbrain or VERSION matching this binary),
