...
export async function runRollout(opts: RolloutOpts): Promise<Trajectory> {
  ...
  // We capture outputs in a Map keyed by inbrainToolUseId so we can stitch
  // them into the trajectory's tool_calls array (preserving call order).
  const toolCalls: Trajectory['tool_calls'] = [];
  const callsById = new Map<string, number>(); // inbrainToolUseId -> index in toolCalls
  let nextOrdinal = 0;
  ...
  const result = await toolLoopImpl({
    ...
    onToolCallStart: async (_turnIdx, _messageIdx, _ordinal, toolName, input, providerToolCallId) => {
      const inbrainToolUseId = `skillopt-${nextOrdinal++}-${providerToolCallId}`;
      const idx = toolCalls.length;
      callsById.set(inbrainToolUseId, idx);
    ...
    onToolCallComplete: async (inbrainToolUseId, output) => {
      const idx = callsById.get(inbrainToolUseId);
      if (idx !== undefined && toolCalls[idx]) {
        toolCalls[idx]!.output = output;
      }
    },
    onToolCallFailed: async (inbrainToolUseId, error) => {
      const idx = callsById.get(inbrainToolUseId);
      if (idx !== undefined && toolCalls[idx]) {
