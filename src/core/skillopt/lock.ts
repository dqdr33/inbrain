 ...
 * lock id is `skillopt:<skill-name>` so two concurrent `inbrain skillopt foo`
 ...
 *  - Reuses the existing primitive (same TTL semantics as inbrain sync,
 ...
 * "another run is in progress; wait or check `inbrain jobs supervisor status`".
 *
...
export async function withSkilloptLock<T>(
  ...
  if (handle === null) {
    throw errorFor({
      ...
      hint: `Wait for it to finish, or check 'inbrain jobs supervisor status'. Stale lock holders auto-expire after ${ttlMinutes} minutes.`,
    });
  }
