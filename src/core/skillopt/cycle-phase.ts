 ...
 * to `~/.inbrain/skillopt-proposed-bundled/<skill>.md` for review.
 *
 ...
 * run invoke `inbrain skillopt <name> --epochs N` directly.
 */
...
export async function runPhaseSkillopt(opts: SkilloptPhaseOpts): Promise<SkilloptPhaseResult> {
  ...
  if (!enabled) {
    return {
      ...
      summary: 'feature flag off (inbrain config set cycle.skillopt.enabled true to enable)',
