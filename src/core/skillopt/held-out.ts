...
export function capturesDir(): string {
  const home = process.env.INBRAIN_HOME ?? process.env.HOME ?? '';
  return path.join(home, '.inbrain', 'skillopt-captures');
}
