/**
 * inbrain skillopt --help text.
 */

export const SKILLOPT_HELP_TEXT = `inbrain skillopt <skill-name> [flags]
...
Examples:
  ...
  inbrain skillopt meeting-prep --bootstrap-from-routing

  # After review (sentinel deleted), run the optimizer:
  inbrain skillopt meeting-prep --bootstrap-reviewed

  # Dry-run cost preview:
  inbrain skillopt meeting-prep --dry-run

  # Optimize a bundled skill in place (requires an independent held-out set):
  inbrain skillopt brain-ops --allow-mutate-bundled --held-out skills/brain-ops/held-out.jsonl
  # ...or omit --allow-mutate-bundled to get proposed.md for manual review (no held-out needed).

  # Resume after interruption:
  inbrain skillopt meeting-prep --resume <run-id>
