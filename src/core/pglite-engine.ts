...
import { InbrainError, PAGE_SORT_SQL, ENRICH_ORDER_SQL } from './types.ts';
  ...
  try {
    ...
    if (!fs.existsSync(snapshotPath)) {
      if (!_snapshotWarnLogged) {
        ...
        console.warn(`[pglite] INBRAIN_PGLITE_SNAPSHOT set but file missing: ${snapshotPath} — using normal init.`);
  ...
  switch (verdict) {
    case 'bunfs':
      hint =
        ...
        '  does not help, run via Node: `node src/cli.ts` or install inbrain\n' +
        '  using the Node-based path. See #1340 for details.';
      break;
    case 'macos-26-3':
      hint =
        '  This is most commonly the macOS 26.3 WASM bug:\n' +
        '  https://github.com/inbrain-ai/inbrain/issues/223';
      break;
    case 'unknown':
    default:
      hint =
        ...
        '  (https://github.com/inbrain-ai/inbrain/issues/223).\n' +
        '  Run `inbrain doctor` for a full diagnosis.';
      break;
  }
...
export class PGLiteEngine implements BrainEngine {
  ...
  // Tier 3: when INBRAIN_PGLITE_SNAPSHOT loaded a post-initSchema state into
  ...
  async connect(config: EngineConfig): Promise<void> {
    ...
    if (!this._lock.acquired) {
      throw new Error('Could not acquire PGLite lock. Another inbrain process is using the database.');
    }
    ...
    if (!dataDir && process.env.INBRAIN_PGLITE_SNAPSHOT) {
      const snapshotResult = tryLoadSnapshot(process.env.INBRAIN_PGLITE_SNAPSHOT);
      if (snapshotResult) {
  ...
  async disconnect(): Promise<void> {
    ...
    // lock and the next inbrain invocation would wedge waiting for it.
  ...
  async resolveSlugs(partial: string, opts?: { sourceId?: string; sourceIds?: string[] }): Promise<string[]> {
    ...
    // behavior so internal CLI callers (`inbrain query --resolve` etc.)
  ...
  async searchKeyword(query: string, opts?: SearchOpts): Promise<SearchResult[]> {
    ...
    if (opts?.limit && opts.limit > MAX_SEARCH_LIMIT) {
      console.warn(`[inbrain] Warning: search limit clamped from ${opts.limit} to ${MAX_SEARCH_LIMIT}`);
    }
  ...
  async searchVector(embedding: Float32Array, opts?: SearchOpts): Promise<SearchResult[]> {
    ...
    // by `inbrain reindex --multimodal`. No modality filter — the column
  ...
  async countStaleChunks(opts?: { sourceId?: string; signature?: string }): Promise<number> {
    // D7: source-scoped count for `inbrain embed --stale --source X`. Always
  ...
  async listLinkSources(
    ...
    // v114 (#1941): distinct provenances + counts for `inbrain link-sources`.
  ...
  async updateTake(
    ...
    if (result.rows.length === 0) {
      throw new InbrainError(
        ...
        'list takes for this page with `inbrain takes <slug>` to see valid row numbers',
      );
    }
  }
  ...
  async supersedeTake(
    ...
    return await this.db.transaction(async (tx) => {
      ...
      if (!existing) {
        throw new InbrainError('TAKE_ROW_NOT_FOUND', `take not found at page_id=${pageId} row=${oldRow}`, 'list takes with `inbrain takes <slug>`');
      }
      if (existing.resolved_at) {
        throw new InbrainError('TAKE_RESOLVED_IMMUTABLE', `take ${pageId}#${oldRow} is resolved`, 'resolved bets are immutable; add a new take instead');
      }
  ...
  async resolveTake(pageId: number, rowNum: number, resolution: TakeResolution): Promise<void> {
    ...
    const existing = existingRes.rows[0] as { resolved_at?: unknown } | undefined;
    if (!existing) {
      throw new InbrainError('TAKE_ROW_NOT_FOUND', `take not found at page_id=${pageId} row=${rowNum}`, 'list takes with `inbrain takes <slug>`');
    }
    if (existing.resolved_at) {
      throw new InbrainError('TAKE_ALREADY_RESOLVED', `take ${pageId}#${rowNum} already resolved`, 'resolution is immutable; add a new take to record a new outcome');
    }
  ...
  async getHealth(): Promise<BrainHealth> {
    ...
    // unhealthy on `inbrain doctor`, which was a structural surprise to users
