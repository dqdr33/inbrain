...
// `.inbrain-schema` is a fan-out hazard. The structural fix is one
 ...
 *   - .inbrain-schema or .inbrain-skillpack file extension (tarball)
 *   - directory with pack.yaml + api_version 'inbrain-schema-pack-v1' (schemapack)
 *   - directory with skillpack.json + api_version 'inbrain-skillpack-v1' (skillpack)
 *
 * Returns null on unrecognized input.
 */
export function detectArtifactKind(path: string): ArtifactKind | null {
  if (path.endsWith('.inbrain-schema')) return 'schemapack';
  if (path.endsWith('.inbrain-skillpack')) return 'skillpack';
  ...
  try {
    ...
    if (existsSync(join(path, 'pack.yaml'))) {
      ...
      if (raw.includes('inbrain-schema-pack-v1')) return 'schemapack';
    }
...
/**
 * Install-target directory by kind. Both kinds land under ~/.inbrain/
 * but at distinct subdirectories so doctor + uninstall can scope
 * cleanly.
 */
export function targetDirForKind(kind: ArtifactKind, inbrainHome: string): string {
  return kind === 'schemapack'
    ? join(inbrainHome, 'schema-packs')
    : join(inbrainHome, 'skillpacks');
}

/**
 * Validate a manifest by kind. For schemapack: shape check against
 * SchemaPackManifest v1. For skillpack: shape check against
 * inbrain-skillpack-v1. Throws on validation failure with a
 * descriptive message.
 */
export function validateManifestByKind(kind: ArtifactKind, manifest: unknown): void {
  if (kind === 'schemapack') {
    ...
    if (m.api_version !== 'inbrain-schema-pack-v1') {
      throw new Error(`schemapack manifest api_version must be "inbrain-schema-pack-v1"; got ${JSON.stringify(m.api_version)}`);
    }
    return;
  }
  if (kind === 'skillpack') {
    ...
    const m = manifest as { api_version?: unknown };
    if (m.api_version !== 'inbrain-skillpack-v1') {
      throw new Error(`skillpack manifest api_version must be "inbrain-skillpack-v1"; got ${JSON.stringify(m.api_version)}`);
    }
    return;
  }
...
export function installArtifact(
  ...
  const targetParent = targetDirForKind(desc.kind, inbrainHome);
 ...
 */
export function listInstalledArtifacts(kind: ArtifactKind, inbrainHome: string): string[] {
  const dir = targetDirForKind(kind, inbrainHome);
