/**
 * inbrain remote-source git helpers (v0.28).
 *
 ...
 * reachable only via Tailscale, set INBRAIN_ALLOW_PRIVATE_REMOTES=1; loud
 ...
 *
 * INBRAIN_ALLOW_PRIVATE_REMOTES=1 lets the URL through with a stderr warning.
...
export function parseRemoteUrl(s: string): ParsedRemoteUrl {
  ...
  if (isInternalUrl(s)) {
    if (process.env.INBRAIN_ALLOW_PRIVATE_REMOTES === '1') {
      console.error(
        `[inbrain] WARN: INBRAIN_ALLOW_PRIVATE_REMOTES=1, accepting internal/private URL: ${url.hostname}`,
      );
    } else {
      throw new RemoteUrlError(
        'internal_target',
        `URL targets internal/private network: ${url.hostname} ` +
          `(set INBRAIN_ALLOW_PRIVATE_REMOTES=1 for self-hosted git over Tailscale or similar)`,
      );
    }
  }
...
const GIT_ENV = {
  // Confine to the inbrain SSRF model — no credential helpers, no SSH askpass,
