 ...
 * embedding provider can't be reached. Without this, inbrain proceeds into
 * the import phase, hits 565 per-file embed errors, writes 565 identical
 * "OpenAI embedding requires OPENAI_API_KEY." rows to `~/.inbrain/sync-failures.jsonl`,
 ...
 * consumers (`inbrain doctor --json`, future autopilot health checks) to
  ...
  switch (d.reason) {
    case 'no_gateway_config':
      return [
        ...
        'without embedding, then file an issue at https://github.com/inbrain-ai/inbrain/issues',
      ].join('\n');
    ...
    case 'no_model_configured':
      return [
        ...
        '',
        '  Set one: inbrain config set embedding_model openai:text-embedding-3-small',
    ...
    case 'unknown_provider':
      return [
        ...
        `  ${d.message}`,
        '',
        '  Pick a known provider: inbrain config set embedding_model openai:text-embedding-3-small',
      ].join('\n');
    ...
    case 'no_touchpoint':
      return [
        ...
        '',
        '  Switch providers: inbrain config set embedding_model openai:text-embedding-3-small',
    ...
    case 'user_provided_model_unset':
      return [
        ...
        '',
        `  Set one: inbrain config set embedding_model ${d.provider}:<model-name>`,
        '  Or run with --no-embed to import-only and embed later.',
      ].join('\n');
    ...
    case 'missing_env': {
      ...
      if (d.provider !== 'openai') {
        lines.push(`  • Switch providers: inbrain config set embedding_model openai:text-embedding-3-small`);
      } else {
        lines.push(`  • Switch providers: inbrain config set embedding_model voyage:voyage-3-large`);
      }
      lines.push('');
