...
export function formatReembedPrompt(est: ReembedEstimate, graceSeconds: number): string {
  ...
  // `inbrain config set search.mode tokenmax` to upgrade pages to per-chunk
 ...
 * the caller should proceed to invoke `inbrain reindex --markdown`.
 *
 * Env overrides (codex C3 + D3=B):
 *   - INBRAIN_NO_REEMBED=1     → bail out entirely (writes a doctor warning marker).
 *   - INBRAIN_REEMBED_GRACE_SECONDS=0 → skip wait (proceed immediately).
 *   - Non-TTY (CI / cron) → skip wait, proceed.
 *
 * v0.41.13.0 T13 retrofit relationship: this prompt is a pre-flight gate
 * for `inbrain reindex --markdown` (which is a separate site we retrofitted
 ...
 * estimate before that work starts. The `INBRAIN_NO_REEMBED=1` env var
 * remains the authoritative bail-out at THIS layer; the
 * `INBRAIN_PROGRESSIVE_BATCH_DISABLED=1` env var at the reindex layer
...
export async function runPostUpgradeReembedPrompt(
  ...
  if (env.INBRAIN_NO_REEMBED === '1') {
    writeFn(`[chunker-bump] INBRAIN_NO_REEMBED=1 set; skipping re-embed sweep. Pending: ${estimate.pendingCount} pages. Re-run \`inbrain reindex --markdown\` when ready.`);
  ...
  const grace = typeof opts.graceSeconds === 'number'
    ? opts.graceSeconds
    : (() => {
        const n = parseInt(env.INBRAIN_REEMBED_GRACE_SECONDS ?? '', 10);
