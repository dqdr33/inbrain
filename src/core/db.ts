...
import { InbrainError, type EngineConfig } from './types.ts';
 ...
 * entirely. So `endPoolBounded` ALSO wraps each end in a inbrain-owned
...
/**
 * #1972: end a postgres.js pool with a inbrain-owned hard bound. Resolves as soon
 ...
 * MaxClients errors when `inbrain upgrade` spawns subprocesses that each open
 * their own pool. Set `INBRAIN_POOL_SIZE=2` (or similar) before the command.
 */
 ...
 * `INBRAIN_PREPARE=true` env var (or `?prepare=true` on the URL) is the
 ...
 *
 * Precedence:
 *   1. `INBRAIN_PREPARE` env var (`true`/`1` or `false`/`0`)
...
export function resolvePrepare(url: string): boolean | undefined {
  const envPrepare = process.env.INBRAIN_PREPARE;
...
export function resolvePoolSize(explicit?: number): number {
  ...
  const raw = process.env.INBRAIN_POOL_SIZE;
  if (raw) {
 ...
 *   - INBRAIN_STATEMENT_TIMEOUT
 *   - INBRAIN_IDLE_TX_TIMEOUT
 *   - INBRAIN_CLIENT_CHECK_INTERVAL (Postgres 14+; empty default - opt-in
...
export function resolveSessionTimeouts(): Record<string, string> {
  ...
  add('INBRAIN_STATEMENT_TIMEOUT', 'statement_timeout', DEFAULT_STATEMENT_TIMEOUT);
  add('INBRAIN_IDLE_TX_TIMEOUT', 'idle_in_transaction_session_timeout', DEFAULT_IDLE_TX_TIMEOUT);
  ...
  add('INBRAIN_CLIENT_CHECK_INTERVAL', 'client_connection_check_interval', '');
  return out;
}
...
export function getConnection(): ReturnType<typeof postgres> {
  if (!sql) {
    throw new InbrainError(
      ...
      'Run inbrain init --supabase or inbrain init --url <connection_string>',
    );
  }
  return sql;
}
...
export async function connect(config: EngineConfig): Promise<boolean> {
  if (sql) {
    ...
    if (config.database_url && connectedUrl && config.database_url !== connectedUrl) {
      console.warn('[inbrain] connect() called with a different database_url but a connection already exists. Using existing connection.');
    }
  ...
  try {
    ...
    const opts: Record<string, unknown> = {
      ...
      // `inbrain jobs submit --json | ...`). Opt back in with INBRAIN_PG_NOTICES=1.
    ...
    if (typeof prepare === 'boolean') {
      ...
      if (!prepare) {
        console.warn(
          '[inbrain] Prepared statements disabled (PgBouncer transaction-mode convention on port 6543). Override with INBRAIN_PREPARE=true if your pooler runs in session mode.',
        );
      }
    }
    ...
    throw new InbrainError(
      ...
      msg,
      'Check your connection URL in ~/.inbrain/config.json',
    );
  }
}
...
export async function connectWithRetry(
  ...
  const noRetry = opts.noRetry ?? (process.env.INBRAIN_NO_RETRY_CONNECT === '1');
