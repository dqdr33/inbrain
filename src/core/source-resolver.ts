...
//     message (explicit `--source` flag tier 1, INBRAIN_SOURCE env tier 2).
 ...
 * @param cwd  The working directory to walk for .inbrain-source. Defaults
...
export async function resolveSourceId(
  ...
  // 2. Env var.
  const env = process.env.INBRAIN_SOURCE;
  if (env && env.length > 0) {
    if (!SOURCE_ID_RE.test(env)) {
      throw new Error(`Invalid INBRAIN_SOURCE value "${env}". Must match [a-z0-9-]{1,32}.`);
    }
    await assertSourceExists(engine, env);
    return env;
  }

  // 3. .inbrain-source dotfile walk-up.
  ...
  //      Vault-mounted source ran `inbrain sync` without --source and routed
 ...
 * `INBRAIN_NO_SOLE_NON_DEFAULT_NUDGE=1` (CI / scripted-pipeline ergonomics).
 *
...
export function formatSoleNonDefaultNudge(sourceId: string): string | null {
  if (process.env.INBRAIN_NO_SOLE_NON_DEFAULT_NUDGE === '1') return null;
  return `[inbrain] routing to source '${sourceId}' (sole non-default source registered; pass --source to override).`;
}
...
async function assertSourceExists(engine: BrainEngine, id: string): Promise<void> {
  ...
  if (rows.length === 0) {
    throw new Error(
      ...
      `run \`inbrain sources list\` to see registered sources, ` +
      `or \`inbrain sources add ${id}\` to create it.`,
    );
  }
}
 ...
 * operating against. Used by `inbrain storage status` and `inbrain export
...
export async function getDefaultSourcePath(
  ...
  // tiering works without forcing a `inbrain sources add . --path .` migration.
 ...
 * `inbrain sources current --json` and downstream consumers share a
 ...
 * same six steps in the same order. Used by `inbrain sources current`
...
export async function resolveSourceWithTier(
  ...
  if (env && env.length > 0) {
    ...
    await assertSourceExists(engine, env);
    return { source_id: env, tier: 'env', detail: `INBRAIN_SOURCE=${env}` };
  }
  ...
  if (dotfile) {
    await assertSourceExists(engine, dotfile);
    return { source_id: dotfile, tier: 'dotfile', detail: `.inbrain-source` };
  }
