 ...
 * logging).
 *
 * Writes failure events to `~/.inbrain/audit/synopsis-failures-YYYY-Www.jsonl`
 * (ISO-week rotation, mirrors `audit-slug-fallback.ts` shape). Surfaced
 * by `inbrain doctor`'s `synopsis_refusal_rate` check.
 *
...
export function logSynopsisFailure(args: LogSynopsisFailureArgs): void {
  ...
  try {
    ...
    process.stderr.write(
      `[inbrain] synopsis-failure audit write failed (${msg}); page ${args.pageSlug} continues\n`,
    );
  }
}
