...
// closed union was always a fiction — every inbrain user accumulated organic
...
// `inbrain-base` declares (the universal starter pack). It is NO LONGER an
...
//
// Historical type docs (kept for `inbrain-base` codegen reference):
// - email | slack | calendar-event: native Page types for inbox/chat/calendar
//   ingest (and the amara-life-v1 eval corpus in the sibling inbrain-evals repo).
...
/**
 * v0.38: Seed list of types declared by the built-in `inbrain-base` schema
 * pack. NO LONGER exhaustive — schema packs add their own types via manifest.
 * This array is referenced by `scripts/generate-inbrain-base.ts` codegen to
 * produce the `inbrain-base.yaml` pack manifest that reproduces today's
...
export interface Page {
  ...
  // surfaced through getPage / list_pages so `inbrain call get_page | jq
...
export interface PageInput {
   ...
   * post-upgrade `inbrain reindex --markdown` sweep can find pre-bump pages
  ...
  // path) but never to the DB columns — `inbrain call get_page | jq .source_kind`
...
export interface PageFilters {
   ...
   * commands (inbrain storage status, inbrain export --restore-only) to scope
 ...
 */
/**
 * v0.37.0 — domain-bank sampling for `inbrain brainstorm` / `inbrain lsd`.
...
/** v0.37.0 — corpus-sampling fallback for `inbrain brainstorm` when prefix-stratified can't fill M. */
...
/**
 * v0.41.39 (issue #1700) — `inbrain enrich --thin` candidate selection.
 *
 ...
 * timeline + frontmatter) so `inbrain extract --stale` extracts in ~1 query per
...
export interface SearchResult {
   ...
   * for agent introspection + the `inbrain search --explain` formatter.
   */
   ...
   * typed-edge traversal (not lexical/vector). Drives `inbrain search
   ...
   * `inbrain search --explain` formatter. Every boost stage stamps its
 ...
 * config registry. Users declare entries via `inbrain config set
...
export interface SearchOpts {
   ...
   * is `inbrain whoknows` (filters to ['person','company']); future
   ...
   * attachments/, .raw/) and the INBRAIN_SEARCH_EXCLUDE env var. Stacks with
   ...
   * 'python', 'ruby'). Used by `inbrain query --lang <lang>`. NULL/undefined
   * returns all languages.
   */
  language?: string;
  /**
   * v0.20.0 Cathedral II: filter by content_chunks.symbol_type (e.g., 'function',
   * 'class', 'method', 'type', 'interface'). Used by `inbrain query --symbol-kind`.
   */
  symbolKind?: string;
  /**
   * v0.20.0 Cathedral II: anchor the two-pass retrieval at a specific qualified
   * symbol name. Pairs with walkDepth. Used by `inbrain query --near-symbol`.
   */
  nearSymbol?: string;
  /**
   ...
   *    (unified column populated by `inbrain reindex --multimodal`). Custom
   ...
   * (operations.ts image branch, tests, inbrain-evals) pass strings;
...
export interface BrainHealth {
   ...
   * construction. Displayed by `inbrain doctor` when brain_score < 100.
   ...
   * brain WITHOUT shelling SSH + inbrain doctor. Two ledgers (schema +
...
// so inbrain-evals can replay them as BrainBench-Real. The companion
// eval_capture_failures table records insert failures so inbrain doctor can
...
export interface EvalCandidateInput {
   ...
   *
   * Replay (`inbrain eval replay`) re-runs the captured query against the
 ...
 * existing SearchResult[] consumers (Cathedral II, inbrain-evals, etc.)
...
export interface HybridSearchMeta {
   ...
   * this query. Surfaced for debugging — agents and the `inbrain query`
   ...
   * Omitted when the gate is off. Surfaced for `inbrain search --explain`.
   */
...
// Errors
export class InbrainError extends Error {
  constructor(
