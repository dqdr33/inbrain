 ...
 *
 * Reuses the inbrain_cycle_locks table (id PK + holder_pid + ttl_expires_at)
 * with a parameterized lock id. Both `inbrain-cycle` (the broad cycle lock)
 * and `inbrain-sync` (performSync's writer lock) live here.
 *
 ...
 * lock ids let callers nest cleanly: cycle holds inbrain-cycle for its run;
 * performSync (called from anywhere — cycle, jobs handler, CLI) takes
 * inbrain-sync just for the write window.
 *
 ...
 * `tryAcquireDbLock`) and `inbrain sync --break-lock`'s safe path so the two
 * never drift.
 *
...
export async function tryAcquireDbLock(
  ...
  if (engine.kind === 'postgres' && maybePG.sql) {
    ...
    // `inbrain sync --break-lock --max-age <s>` uses last_refreshed_at (not
    ...
    const rows: Array<{ id: string }> = await sql`
      INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at, last_refreshed_at)
      ...
      ON CONFLICT (id) DO UPDATE
        ...
        WHERE inbrain_cycle_locks.ttl_expires_at < NOW()
          AND (inbrain_cycle_locks.last_refreshed_at IS NULL
               OR inbrain_cycle_locks.last_refreshed_at < NOW() - ${stealGraceSeconds} * INTERVAL '1 second')
      RETURNING id
    `;
    if (rows.length === 0) return null;
    const deregister = registerCleanup(`db-lock:${lockId}`, async () => {
      await sql`
        DELETE FROM inbrain_cycle_locks
  ...
  if (engine.kind === 'pglite' && maybePGLite.db) {
    ...
    const { rows } = await db.query(
      `INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at, last_refreshed_at)
       ...
       ON CONFLICT (id) DO UPDATE
         ...
         WHERE inbrain_cycle_locks.ttl_expires_at < NOW()
           AND (inbrain_cycle_locks.last_refreshed_at IS NULL
                OR inbrain_cycle_locks.last_refreshed_at < NOW() - $5 * INTERVAL '1 second')
       RETURNING id`,
    ...
    const deregister = registerCleanup(`db-lock:${lockId}`, async () => {
      await db.query(
        `DELETE FROM inbrain_cycle_locks WHERE id = $1 AND holder_pid = $2`,
        [lockId, pid],
      );
    });
    return {
 ...
 *   - inbrain doctor's `stale_locks` check (queries all rows where
 *     ttl_expires_at < NOW()).
 *   - inbrain sync --break-lock to verify holder state before clearing.
 *
...
/**
 * Raw row shape returned by every `inbrain_cycle_locks` SELECT. Lives here so
...
/**
 * The single canonical reader for `inbrain_cycle_locks`. Engine-branches once
...
async function selectLockRows(engine: BrainEngine, opts: SelectLockRowsOpts = {}): Promise<LockSnapshot[]> {
  ...
  if (engine.kind === 'postgres' && maybePG.sql) {
    ...
    if (opts.lockId !== undefined) {
      rows = await sql`SELECT id, holder_pid, holder_host, acquired_at, ttl_expires_at, last_refreshed_at FROM inbrain_cycle_locks WHERE id = ${opts.lockId}`;
    } else if (opts.staleOnly) {
      rows = await sql`SELECT id, holder_pid, holder_host, acquired_at, ttl_expires_at, last_refreshed_at FROM inbrain_cycle_locks WHERE ttl_expires_at < NOW() ORDER BY acquired_at`;
    } else {
      rows = await sql`SELECT id, holder_pid, holder_host, acquired_at, ttl_expires_at, last_refreshed_at FROM inbrain_cycle_locks ORDER BY acquired_at`;
    }
  } else if (engine.kind === 'pglite' && maybePGLite.db) {
    if (opts.lockId !== undefined) {
      rows = (await maybePGLite.db.query(`SELECT ${LOCK_SELECT_COLS} FROM inbrain_cycle_locks WHERE id = $1`, [opts.lockId])).rows as RawLockRow[];
    } else if (opts.staleOnly) {
      rows = (await maybePGLite.db.query(`SELECT ${LOCK_SELECT_COLS} FROM inbrain_cycle_locks WHERE ttl_expires_at < NOW() ORDER BY acquired_at`)).rows as RawLockRow[];
    } else {
      rows = (await maybePGLite.db.query(`SELECT ${LOCK_SELECT_COLS} FROM inbrain_cycle_locks ORDER BY acquired_at`)).rows as RawLockRow[];
    }
  } else {
...
/**
 * v0.41.6.0 D3: list every lock whose TTL has expired. Used by inbrain
...
/**
 * v0.41.6.0 D3: atomic verify-and-delete for `inbrain sync --break-lock`.
 *
 ...
 * verify-and-delete for `inbrain sync --break-lock --max-age <seconds>`.
 *
 * Runs:
 *   DELETE FROM inbrain_cycle_locks
 *    WHERE id = $1
 ...
 * sweep. Intended to run at cycle start (and under `inbrain doctor --fix` for
 * no-autopilot brains).
 *
 * Scoped to the `inbrain-sync:*` and `inbrain-cycle`/`inbrain-cycle:*` namespaces
 * ONLY. `inbrain_cycle_locks` is shared by enrich, the minion supervisor,
...
function isReapableNamespace(lockId: string): boolean {
  return (
    lockId === 'inbrain-cycle' ||
    lockId.startsWith('inbrain-cycle:') ||
    lockId.startsWith('inbrain-sync:')
  );
}
 ...
 *
 * Before v0.40: SYNC_LOCK_ID was a bare 'inbrain-sync' constant, taken by
 ...
 * parallel. The cycle's broader `inbrain-cycle` lock still serializes inside
 * a single cycle invocation. Two-source layered semantics:
 *
 *   cycle              acquires `inbrain-cycle`
 *     → performSync(A) acquires `inbrain-sync:A`
 *     → performSync(B) acquires `inbrain-sync:B`  (in a different process, fine)
 *
 ...
 *
 * Lock-id naming convention: `<scope>:<dbname>` (e.g. `inbrain-migrate:postgres`)
 ...
 * existing `inbrain_cycle_locks` table (which `tryAcquireDbLock` already
 ...
 * with the database name so two inbrain installs sharing a Postgres cluster
