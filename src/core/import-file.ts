 ...
 *
 * Roughly 40% of inbrain's brain is docs/guides/architecture notes with
 ...
 * `INBRAIN_MAX_FENCES_PER_PAGE` env var if docs-heavy brains legitimately
 * exceed 100 fences on a single page.
 */
const MAX_FENCES_PER_PAGE = Number.parseInt(process.env.INBRAIN_MAX_FENCES_PER_PAGE || '100', 10);
...
async function extractFencedChunks(
  ...
  for (const tok of tokens) {
    ...
    if (fencesSeen >= MAX_FENCES_PER_PAGE) {
      console.warn(
        `[inbrain] markdown fence cap hit (${MAX_FENCES_PER_PAGE} fences/page); skipping additional fences. ` +
        `Override via INBRAIN_MAX_FENCES_PER_PAGE env var.`,
      );
      break;
    }
    fencesSeen++;
    ...
    try {
      ...
      console.warn(
        `[inbrain] fence extraction failed for lang=${code.lang}: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
  return out;
...
export async function importFromContent(
  ...
  opts: {
     ...
     * re-embed. Used by `inbrain reindex --markdown` so a chunker version
     ...
     * uses the pack's path_prefixes instead of the hardcoded inbrain-base table.
  ...
  //     `INBRAIN_NO_SANITY=1`) → assess + audit with bypass flag, emit
  ...
  let pageFlagReason: 'markup_heavy' | 'oversized' | undefined;
  {
    ...
    try {
      ...
      process.stderr.write(`[inbrain] content-sanity: DB config lift failed (${msg}); falling back to file/env\n`);
    }
    const cs = effectiveCfg?.content_sanity ?? {};
    // INBRAIN_NO_SANITY=1 fast-path: loadConfig() returns null when
    // there's no `~/.inbrain/config.json` AND no DATABASE_URL env var
    ...
    const sanityDisabled =
      cs.disabled === true || process.env.INBRAIN_NO_SANITY === '1';
    ...
    if (sanityDisabled) {
      ...
      if (sanityResult.shouldQuarantine || sanityResult.shouldFlag) {
        process.stderr.write(
          `[inbrain] content-sanity bypass (INBRAIN_NO_SANITY=1): ${slug} — ${sanityResult.reason_messages.join('; ')}\n`,
        );
      }
      ...
      // via get_page / `inbrain quarantine list`.
      ...
      process.stderr.write(
        `[inbrain] content-sanity quarantine: ${slug} — ${detail} (hidden from search, reviewable via 'inbrain quarantine list')\n`,
      );
      ...
      if (flagReason === 'oversized') {
        ...
        process.stderr.write(
          `[inbrain] content-sanity flag (oversized): ${slug} (${sanityResult.bytes} bytes) — page lands, embedding skipped, agent warned\n`,
        );
      } else {
        ...
        process.stderr.write(
          `[inbrain] content-sanity flag (markup_heavy): ${slug} (ratio ${sanityResult.markup_ratio?.toFixed(2)}) — stays searchable, agent warned\n`,
        );
      }
      ...
      process.stderr.write(
        `[inbrain] content-sanity warn: ${slug} (${sanityResult.bytes} bytes) — exceeds warn threshold, consider splitting\n`,
      );
    }
  }

  // v0.39.3.0 CV8 — DB content_hash excludes timestamp-bearing frontmatter
  // keys so identical body content from `inbrain capture` (which stamps
  ...
  // `inbrain import /vault/Subdir/` then later `inbrain import /vault/`,
  ...
  await engine.transaction(async (tx) => {
    ...
    // (inbrain embed --stale or contextual reindex Minion) will set it.
    if (!opts.noEmbed) {
    ...
    // visibly under `inbrain reindex --markdown` (#1621), which re-imports every
    ...
    // later via `inbrain reconcile-links` (Layer 8 D3, v0.21.0).
 ...
 * `people/elon` page on the next `inbrain sync` or `inbrain import`. In shared
...
export async function importFromFile(
  ...
  // is not modified. Use `inbrain frontmatter generate --fix` to write back.
 ...
 * storage; the page itself is written via separate put_page); `inbrain
 ...
 * user runs `inbrain dream` or the consolidate phase to backfill facts
...
export async function importCodeFile(
  ...
  if (!opts.noEmbed && needsEmbedIndexes.length > 0) {
    try {
      ...
      console.warn(`[inbrain] embedding failed for code file ${slug}: ${e instanceof Error ? e.message : String(e)}`);
    }
  }
  ...
  if (extractedEdges.length > 0 && chunks.length > 0) {
    try {
      ...
      console.warn(`[inbrain] edge extraction failed for ${slug}: ${edgeErr instanceof Error ? edgeErr.message : String(edgeErr)}`);
    }
  }
 ...
 * the kinds of brains inbrain serves and adding them would expand the WASM
...
async function maybeOcr(
  ...
  const opt = process.env.INBRAIN_EMBEDDING_IMAGE_OCR;
  if (opt !== 'true') return '';
  ...
  try {
    ...
    if (!isAvailable('expansion')) {
      if (!_ocrWarnedThisSession) {
        console.warn('[inbrain] OCR opt-in is true but expansion model is unavailable; skipping OCR for this session');
    ...
    if (!_ocrWarnedThisSession) {
      console.warn(`[inbrain] OCR call failed (continuing without OCR text): ${err instanceof Error ? err.message : String(err)}`);
      _ocrWarnedThisSession = true;
    }
...
export async function importImageFile(
  ...
  await withImportTransaction(engine, {
    ...
    after: async (tx) => {
      ...
      // let `inbrain reconcile-links` pick up later additions.
