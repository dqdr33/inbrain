  ...
  switch (status.status) {
    ...
    case 'stale':
      return (
        ...
        `Re-run \`inbrain eval cross-modal\` against the current skill output.`
      );
    case 'missing':
      return `no cross-modal eval receipt for ${slug} yet — run \`inbrain eval cross-modal\` to add one`;
  }
}
