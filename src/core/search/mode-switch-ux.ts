 ...
 *
 * When the user runs `inbrain config set search.mode <X>`, this module
 ...
 *      (mirrors `inbrain doctor`'s queue_health check). Returns active /
 ...
 *      per D3.
 *
 * Suppression: INBRAIN_NO_MODE_SWITCH_UX=1 skips the entire UX for
...
/**
 * Worker stale threshold: 2 minutes (matches inbrain doctor's
...
export function summarizeTransition(
  ...
  if (newMode === 'tokenmax') {
    return {
      ...
      reindex_command: 'inbrain reindex --markdown',
      ...
      callout_lines: [
        ...
        `Run \`inbrain reindex --markdown\` to backfill existing pages.`,
      ],
    };
  }
 ...
 *
 * The PGLite inline path (`inbrain agent run` with `--follow`) doesn't
...
export async function probeWorkerAvailable(engine: BrainEngine): Promise<WorkerProbeResult> {
  const startCmd = 'inbrain jobs work';
  try {
    // inbrain doesn't have a minion_workers heartbeat table yet (B7 follow-up
...
/**
 * Orchestrator. Called by `inbrain config set search.mode <X>` after the
 * config write lands. Honors INBRAIN_NO_MODE_SWITCH_UX=1 (full skip),
...
export async function runModeSwitchUx(opts: ModeSwitchOpts): Promise<void> {
  if (process.env.INBRAIN_NO_MODE_SWITCH_UX === '1') return;
  ...
  if (worker.status !== 'active') {
    ...
    // still invoke reindex inline (`inbrain reindex --markdown` runs
  ...
  try {
    ...
    console.error(`[mode-switch] Submitted as job ${jobId}. Watch with: inbrain jobs follow ${jobId}`);
  } catch (err) {
