 ...
 *   2. inbrain.yml `recency:` section
 *   3. INBRAIN_RECENCY_DECAY env var (prefix:halflifeDays:coefficient,...)
...
export const DEFAULT_RECENCY_DECAY: RecencyDecayMap = {
  ...
  // inbrain.yml, never in shipped defaults.
...
/**
 * Parse the INBRAIN_RECENCY_DECAY env var.
  ...
  for (const triple of triples) {
    ...
    if (lastIdx <= 0) {
      throw new RecencyDecayParseError(
        `Invalid INBRAIN_RECENCY_DECAY entry "${triple}": expected prefix:halflife:coefficient`,
        'env',
      );
    }
    ...
    if (!prefix) {
      throw new RecencyDecayParseError(`Empty prefix in INBRAIN_RECENCY_DECAY entry "${triple}"`, 'env');
    }
    if (!Number.isFinite(halflife) || halflife < 0) {
      throw new RecencyDecayParseError(
        `Invalid halflifeDays "${halflifeRaw}" in INBRAIN_RECENCY_DECAY (must be number >= 0; 0 = evergreen)`,
        'env',
      );
    }
    if (!Number.isFinite(coefficient) || coefficient < 0) {
      throw new RecencyDecayParseError(
        `Invalid coefficient "${coefficientRaw}" in INBRAIN_RECENCY_DECAY (must be number >= 0)`,
        'env',
      );
    }
...
/**
 * Parse a `recency:` section from a parsed inbrain.yml. The shape is:
 *   recency:
 *     daily/: { halflifeDays: 14, coefficient: 1.5 }
 *     concepts/: { halflifeDays: 0, coefficient: 0 }
 *
...
export function parseRecencyDecayYaml(parsed: unknown): RecencyDecayMap {
  ...
  if (typeof recency !== 'object' || Array.isArray(recency)) {
    throw new RecencyDecayParseError(`inbrain.yml recency: must be a map, got ${typeof recency}`, 'yaml');
  }
  const out: RecencyDecayMap = {};
  for (const [prefix, raw] of Object.entries(recency as Record<string, unknown>)) {
    if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) {
      throw new RecencyDecayParseError(
        `inbrain.yml recency."${prefix}" must be an object with halflifeDays + coefficient`,
        'yaml',
      );
    }
    ...
    if (!Number.isFinite(halflife) || halflife < 0) {
      throw new RecencyDecayParseError(
        `inbrain.yml recency."${prefix}".halflifeDays invalid (must be number >= 0)`,
        'yaml',
      );
    }
    if (!Number.isFinite(coefficient) || coefficient < 0) {
      throw new RecencyDecayParseError(
        `inbrain.yml recency."${prefix}".coefficient invalid (must be number >= 0)`,
        'yaml',
      );
    }
    out[prefix] = { halflifeDays: halflife, coefficient };
...
export function resolveRecencyDecayMap(opts: {
  ...
  const fromEnv = parseRecencyDecayEnv(opts.envValue ?? process.env.INBRAIN_RECENCY_DECAY);
  return {
