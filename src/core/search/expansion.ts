 ...
 * the gateway is provider-agnostic; sanitization is inbrain's responsibility.
 *
...
export function sanitizeQueryForPrompt(query: string): string {
  ...
  if (q !== original) {
    console.warn('[inbrain] sanitizeQueryForPrompt: stripped content from user query before LLM expansion');
  }
  return q;
}
