 ...
 * inbrain processes flushing the same (date, mode, intent) tuple accumulate
 * correctly.
 *
 ...
 * because the consumer is the operator (or an agent running `inbrain search
...
/**
 * Per-process telemetry singleton. Each inbrain process (CLI, stdio MCP,
...
class TelemetryWriter {
  ...
  private ensureExitHook(): void {
    ...
    // alive past beforeExit — short-lived CLI invocations (`inbrain query "the"`
    ...
    // hang (the harness spawns short-lived inbrain queries that should exit
    // in <1s but never did).
    //
