 ...
 *
 * Future write-path PR (`inbrain embed --column X --model Y`, deferred per
...
export function getEmbeddingColumnRegistry(
  ...
  // `~/.inbrain/config.json`. Falling straight from `cfg.embedding_model`
...
export function resolveEmbeddingColumn(
  ...
  // could pass a hand-rolled object via the public `inbrain/types`
 ...
 */
export function isCacheSafe(resolved: ResolvedColumn, cfg: InbrainConfig): boolean {
...
export function normalizeEngineColumn(
  ...
  // `inbrain reindex --multimodal`. Voyage multimodal-3, vector(1024).
