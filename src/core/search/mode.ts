...
export interface ModeBundle {
   ...
   * `inbrain reindex --multimodal` populates the unified column.
   */
...
export const MODE_BUNDLES: Readonly<Record<SearchMode, Readonly<ModeBundle>>> = Object.freeze({
  ...
  balanced: Object.freeze({
    ...
    // `inbrain config set search.reranker.enabled false`.
    ...
    // bounded. Opt out with `inbrain config set search.graph_signals false`.
    graph_signals: true,
...
/**
 * Per-knob source attribution for `inbrain search modes` dashboard.
 * Tells the user where each resolved value came from so override drift
 * is legible. Mirrors `inbrain models` (v0.31.12) attribution shape.
 */
...
/** The full list of config keys this module reads. Used by `inbrain search modes --reset`. */
