/**
 * Inbrain OAuth 2.1 Provider — implements MCP SDK's OAuthServerProvider.
 *
...
/**
 * Allow-list of RFC 7591 §2 `token_endpoint_auth_method` values inbrain
 ...
 *   1. CLI `inbrain auth register-client` (src/commands/auth.ts)
 *   2. Admin `POST /admin/api/register-client` (src/commands/serve-http.ts)
 *   3. DCR `POST /register` (this file, InbrainClientsStore.registerClient)
 *
...
interface InbrainOAuthProviderOptions {
  sql: SqlQuery;
...
class InbrainClientsStore implements OAuthRegisteredClientsStore {
  ...
  async registerClient(
    ...
    const clientId = generateToken('inbrain_cl_');
    ...
    const clientSecret = isPublicClient ? undefined : generateToken('inbrain_cs_');
...
export class InbrainOAuthProvider implements OAuthServerProvider {
  private sql: SqlQuery;
  private _clientsStore: InbrainClientsStore;
  private readonly dcrDisabled: boolean;
  private tokenTtl: number;
  private refreshTtl: number;

  constructor(options: InbrainOAuthProviderOptions) {
    this.sql = options.sql;
    this._clientsStore = new InbrainClientsStore(this.sql);
  ...
  async authorize(
    ...
    const code = generateToken('inbrain_code_');
  ...
  async exchangeRefreshToken(
    ...
    // gstack /setup-inbrain Path 4 — which mints a sources_admin-scoped
  ...
  async verifyAccessToken(token: string): Promise<AuthInfo> {
    ...
    if (legacyRows.length > 0) {
      ...
      return {
        ...
        // serve-http transport fell back to INBRAIN_SOURCE/'default' for
  ...
  private async issueTokens(
    ...
    const accessToken = generateToken('inbrain_at_');
    ...
    if (includeRefresh) {
      const refreshToken = generateToken('inbrain_rt_');
