 ...
 * `inbrain serve --http` makes PGLite network-accessible.
 *
...
-- Inbrain PGLite schema (local embedded Postgres)
...
CREATE TABLE IF NOT EXISTS pages (
  ...
  -- v0.37.0 (migration v79): real stale-page signal for inbrain lsd
  ...
  -- inbrain extract --stale + the links_extraction_lag doctor check.
...
CREATE TABLE IF NOT EXISTS subagent_tool_executions (
  ...
  schema_version      INTEGER     NOT NULL DEFAULT 1,
  provider_id         TEXT,
  -- v0.38 D11: inbrain-owned stable IDs (ordinal assigned at first observation;
  -- inbrain_tool_use_id is uuid v7). Reconciliation on crash-replay uses
...
-- row + the file lock at ~/.inbrain/cycle.lock prevent concurrent
...
CREATE TABLE IF NOT EXISTS inbrain_cycle_locks (
  ...
  -- refresh tick. Used by inbrain sync --break-lock --max-age <s> to identify
...
-- One row per 'inbrain eval suspected-contradictions' run; source for the
...
-- ============================================================
-- migration_impact_log (v0.41.18.0 — inbrain onboard wave)
-- ============================================================
