 ...
 *
 *   ┌─────────────────────────────┐
 *   │ INBRAIN_DATABASE_URL         │   INBRAIN_DIRECT_DATABASE_URL (override)
 *   │ (pooler, port 6543)         │
 *   └────────┬────────────────────┘
 *            │
 ...
 *  - Kill-switch (F1): INBRAIN_DISABLE_DIRECT_POOL=1 falls back to single-pool
 ...
 *    INBRAIN_DIRECT_DATABASE_URL override, ddl()/bulk() share the read pool.
 */
...
export interface ConnectionManagerOpts {
   ...
   * Sourced from INBRAIN_DIRECT_DATABASE_URL or explicit caller config.
   */
  ...
  readPoolSize?: number;
  /**
   * Direct pool size override (defaults to INBRAIN_DIRECT_POOL_SIZE env or 3).
   */
...
export function readKillSwitchEnv(): boolean {
  return process.env.INBRAIN_DISABLE_DIRECT_POOL === '1' ||
...
export function resolveDirectPoolSize(explicit?: number): number {
  ...
  const raw = process.env.INBRAIN_DIRECT_POOL_SIZE;
  if (raw) {
...
export class ConnectionManager {
  ...
  constructor(opts: ConnectionManagerOpts) {
    ...
    if (opts.parent) {
      ...
      const envOverride = process.env.INBRAIN_DIRECT_DATABASE_URL;
  ...
  async disconnect(): Promise<void> {
    // #1972: end both pools concurrently with a inbrain-owned hard bound, so the
