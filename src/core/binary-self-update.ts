 ...
 * (v0.42 self-upgrading-inbrain wave, eng-review Finding 2 — "make the atomic
 * swap claim true for the one method we own").
 *
 * `bun` / `bun-link` / `clawhub` delegate their swap to those package managers.
 * The compiled standalone binary is the only method inbrain itself writes, so
 ...
 * returning null. Trust model: TLS + GitHub, same as `inbrain upgrade` (no
 * signature verification this wave — D7a TODO).
 *
 * Published asset matrix mirrors `.github/workflows/release.yml`:
 *   darwin-arm64 → inbrain-darwin-arm64
 *   linux-x64    → inbrain-linux-x64
 */
...
/** The release asset basename inbrain publishes for this platform/arch, or null
...
export function expectedAssetName(platform: NodeJS.Platform, arch: NodeJS.Architecture): string | null {
  if (platform === 'darwin' && arch === 'arm64') return 'inbrain-darwin-arm64';
  if (platform === 'linux' && arch === 'x64') return 'inbrain-linux-x64';
  return null;
}
...
export interface BinarySelfUpdateDeps {
  ...
  /** Smoke-test the staged binary; returns true if `<path> --version` looks like inbrain. */
...
async function defaultFetchRelease(): Promise<{ tag: string; assets: ReleaseAsset[] } | null> {
  try {
    const res = await fetch('https://api.github.com/repos/inbrain-ai/inbrain/releases/latest', {
      headers: { 'User-Agent': 'inbrain-self-upgrade' },
