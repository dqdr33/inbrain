...
const CODE_EXTENSIONS = new Set<string>([
  ...
  // Terraform repos were invisible to `inbrain sync --strategy code`.
 ...
 * gate is on. The runtime gate flips through `process.env.INBRAIN_EMBEDDING_MULTIMODAL`
...
function isMultimodalEnabled(): boolean {
  return process.env.INBRAIN_EMBEDDING_MULTIMODAL === 'true';
}
 ...
 * them at file-emit time. Used by every walker in inbrain (sync, extract,
...
export function pruneDir(name: string, parentDir?: string): boolean {
  ...
  // `.raw` is the literal directory name; `*.raw` is the inbrain sidecar
 ...
 * that had been indexed by older inbrain versions (or via direct put_page).
...
/**
 * Filter a file path to determine if it should be synced to Inbrain.
...
/**
 * Convert a repo-relative file path to a Inbrain page slug.
 */
