 ...
 *   - `env_explicit`                 — $INBRAIN_SKILLS_DIR (operator override; v0.31.7)
 ...
 *   - `repo_root`                    — walked up from cwd, found inbrain repo
...
function resolveWorkspaceSkillsDir(
  ...
  // Preferred: workspace/skills with a resolver file inside it (inbrain-native).
 ...
 *   0. $INBRAIN_SKILLS_DIR explicit operator override (any caller)
 *   1. $OPENCLAW_WORKSPACE when explicitly set (env > repo-root walk)
 *   2. ~/.openclaw/workspace/ (user's default OpenClaw deployment)
 *   3. findRepoRoot() walk from cwd (inbrain's own repo)
 *   4. ./skills fallback (dev scratch, fixtures)
 *
 * Tier 0 ($INBRAIN_SKILLS_DIR) is safe for both read and write paths because
 ...
 * `export OPENCLAW_WORKSPACE=...; inbrain check-resolvable` run from
 * inside the inbrain repo silently shadowed the env var by walking up
 * to inbrain's own skills/. Explicit env should win. Unset env → behavior
 ...
 * a write-path install-path fallback would let `inbrain skillpack install`
 * from `~` silently target the bundled inbrain repo's skills/ instead of the
 * user's workspace.
 *
...
export function autoDetectSkillsDir(
  ...
  // 0. $INBRAIN_SKILLS_DIR explicit operator override. Safe for all callers
  ...
  if (env.INBRAIN_SKILLS_DIR) {
    const explicit = isAbsolute(env.INBRAIN_SKILLS_DIR)
      ? env.INBRAIN_SKILLS_DIR
      : resolvePath(startDir, env.INBRAIN_SKILLS_DIR);
  ...
  //     && inbrain skillpack scaffold X` finds the agent repo, not an
 ...
 * walks up from this module's install location to find a inbrain repo root,
 * gated by `isGbrainRepoRoot` to avoid false-positive on unrelated repos.
 *
 * Use this from READ-ONLY callers only: `inbrain doctor`,
 * `inbrain check-resolvable`, `inbrain routing-eval`. Never from write paths.
 *
 ...
 * `inbrain skillpack install` from `~` silently target the bundled inbrain
 ...
 * github:inbrain-ai/inbrain && cd ~ && inbrain doctor`) without expanding the
...
export function autoDetectSkillsDirReadOnly(
  ...
  // that vendored inbrain in a subdir).
  try {
...
export const AUTO_DETECT_HINT = [
  `  1. --skills-dir flag`,
  `  2. $INBRAIN_SKILLS_DIR (explicit operator override)`,
 ...
 * `autoDetectSkillsDirReadOnly` adds for `inbrain doctor` /
 * `inbrain check-resolvable` / `inbrain routing-eval`.
 */
export const AUTO_DETECT_HINT_READ_ONLY = [
  AUTO_DETECT_HINT,
  `  7. (read-only) walk up from inbrain's install path`,
].join('\n');
