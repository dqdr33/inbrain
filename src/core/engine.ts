 ...
 *
 * - `auditSite`: typed label for the JSONL audit emission (`~/.inbrain/audit/
...
// primitive's seed list in inbrain-base reproduces {fact|take|bet|hunch}
...
export interface NewFact {
   ...
   * `inbrain eval trajectory` + `find_trajectory` MCP op consume them for
...
/** Per-source operational health snapshot consumed by `inbrain doctor`. */
...
export interface BrainEngine {
   ...
   * `deleted_at` populated — used by `inbrain pages purge-deleted` listing,
   ...
   * issue #1309) created two pages per file when a user ran `inbrain import
   * /vault/Subdir/` then `inbrain import /vault/` — the slug-shape changed
   ...
   * `inbrain sync` (one path per call), test setup teardown, and the v0.41.19.0
   ...
   * failures to `failedFiles`). `inbrain sync` calls this on EVERY run that
   ...
   * in-flight batch. `inbrain sync` is idempotent (next run picks them up
   ...
   * Called by the autopilot purge phase and by the `inbrain pages purge-deleted`
   ...
   * `inbrain query --resolve` CLI path, etc.). Field names match the
  ...
  /**
   * v0.37.0 — prefix-stratified page sampling for `inbrain brainstorm` / `inbrain lsd`
  ...
  /**
   * v0.37.0 — corpus-sampling fallback for `inbrain brainstorm` when prefix-stratified
   ...
   * `inbrain embed --stale --source media-corpus` expect only that
   ...
   * sourceId scope); used by the `inbrain sync --all` cost preview to price
  ...
  // Powers `inbrain extract --stale` + the `links_extraction_lag` doctor check.
  // ============================================================
  /**
   ...
   * `defaultExtractedAt` arg. `inbrain extract --stale` passes the row's READ
   ...
   * `inbrain link-sources`. Source-scoped via `{sourceId?, sourceIds?}` (both
   ...
   * Used by `inbrain orphans` and `runCycle`'s orphan sweep phase.
   *
   * v0.41.29.0: scopes the CANDIDATE set to one source (`sourceId`, from
   * `inbrain doctor/orphans --source` + single-source MCP clients) or a
  ...
  /** Pre-flight count for `inbrain embed --stale`. WHERE active AND embedding IS NULL. */
   ...
   * newest first. Used by `inbrain eval suspected-contradictions trend` and
   ...
   * newest first. Drives `inbrain recall --supersessions`.
   */
  listSupersessions(
    source_id: string,
   ...
   * phase yet (active + unconsolidated). Drives `inbrain recall --pending`.
  ...
  /** Per-source operational metrics for `inbrain doctor` facts_health check. */
   ...
   * stale state. content_hash is included so the next `inbrain sync` sees the
   ...
   * No-op when the key doesn't exist. Used by `inbrain config unset` and by
   * `inbrain search modes --reset`. Engine-agnostic.
   */
  unsetConfig(key: string): Promise<number>;
  /**
   * v0.32.3 — list config keys matching a literal prefix (e.g. "search.").
   * Used by `inbrain config unset --pattern` and the search-modes --reset path.
  ...
  // `inbrain eval export` (NDJSON) for sibling inbrain-evals consumption.
  ...
  /** Read candidates by time window / limit / tool filter. Used by `inbrain eval export`. */
  listEvalCandidates(filter?: { since?: Date; limit?: number; tool?: 'query' | 'search' }): Promise<EvalCandidate[]>;
  /** Delete candidates created before `date`. Returns rows deleted. Used by `inbrain eval prune`. */
  deleteEvalCandidatesBefore(date: Date): Promise<number>;
  /** Log a capture failure so `inbrain doctor` can surface drops cross-process. Best-effort; symmetric with logEvalCandidate (failure-of-failure is lost). */
  logEvalCaptureFailure(reason: EvalCaptureFailureReason): Promise<void>;
  /** Read capture failures within an optional time window. Used by `inbrain doctor`. */
   ...
   * `inbrain enrich --thin`. ONE source-aware SQL query: thin-filter +
