 ...
 * phantoms per cycle (configurable via `INBRAIN_PHANTOM_REDIRECT_LIMIT`).
 *
 ...
 *
 * Lock contract: single `inbrain-sync` writer-lock acquisition at the top
 ...
 * `src/core/import-file.ts:241`'s shape exactly so `inbrain sync`'s
...
export async function tryRedirectPhantom(
  ...
  // next `inbrain sync` sees the canonical as unchanged. We re-parse the
    ...
    try {
      ...
      if (code !== 'ENOENT') {
        ...
        process.stderr.write(
          `[inbrain] phantom-redirect: unlink ${phantomPath} failed (${msg}); cycle continues\n`,
        );
      }
    }
  }
...
export async function runPhantomRedirectPass(
  ...
  const limitRaw = process.env.INBRAIN_PHANTOM_REDIRECT_LIMIT;
  const limit = (() => {
  ...
  try {
    ...
    for (let i = 0; i < Math.min(rows.length, limit); i++) {
      ...
      try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        process.stderr.write(`[inbrain] phantom-redirect: ${slug} failed (${msg}); skipping\n`);
        logPhantomEvent({
          phantom_slug: slug,
          outcome: 'drift',
          source_id: sourceId,
