 ...
 * `warn` with a hint to run `inbrain apply-migrations --yes`. Without
...
export interface ExtractFactsOpts {
   ...
   * the phantom-redirect pass is skipped (callers like `inbrain dream`
...
export async function runExtractFacts(
  ...
  if (legacyCount > 0) {
    ...
    result.warnings.push(
      ...
      `Run \`inbrain apply-migrations --yes\` to complete v0_32_2 before this phase ` +
  ...
  for (const slug of slugs) {
    ...
    const inserted = await engine.insertFacts(extracted, { source_id: sourceId }); // inbrain-allow-direct-insert: extract_facts cycle phase reconciles fence → DB
