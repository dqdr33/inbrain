 ...
 * `take_proposals` queue. User accepts/rejects via `inbrain takes propose`.
 *
 ...
 *
 * F2 fence dedup:
 *   The phase reads the page's existing `<!-- inbrain:takes:begin -->` fence
 ...
 *   the canonical takes table. Operator opt-in via `inbrain takes propose
 ...
 *   eval in the inbrain-evals repo. First live run scored 0.952 F1 on
 ...
 * via inbrain-evals cat15 (claude-sonnet-4-6 extractor, claude-haiku-4-5
 * matcher judge):
 *
...
/**
 * Detect whether a page already has a complete `<!-- inbrain:takes:begin -->`
...
export function hasCompleteFence(pageBody: string): boolean {
  return /<!---?\s*inbrain:takes:begin[\s\S]*?inbrain:takes:end\s*-->/.test(pageBody);
}
...
export function extractExistingTakesForDedup(pageBody: string): Array<{
  ...
  const fenceMatch = pageBody.match(/<!---?\s*inbrain:takes:begin\s*-->([\s\S]*?)<!---?\s*inbrain:takes:end\s*-->/);
...
class ProposeTakesPhase extends BaseCyclePhase {
  ...
  protected override mapErrorCode(err: unknown): string {
    if (err instanceof InbrainError) return err.problem;
