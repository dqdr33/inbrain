...
export interface SynthesizePhaseOpts {
   ...
   * `inbrain dream --input <file>` ad-hoc path; bypasses config reads.
   */
  inputFile?: string;
  date?: string;
  from?: string;
  ...
  try {
    ...
    for (const t of transcripts) {
      ...
      try {
        ...
        // shape of the problem in `inbrain dream --phase synthesize --dry-run`.
...
// via `inbrain config set models.dream.synthesize_verdict <provider>:<model>`.
//
