...
//
// ONE drain path, three callers: `inbrain dream --phase extract_atoms --drain`
...
export interface DrainForSourceOpts {
   ...
   * `undefined` → `cycleLockIdFor(undefined)` = the bare `inbrain-cycle` lock the
   * unscoped routine cycle holds; a real id → `inbrain-cycle:<id>`. Either way the
