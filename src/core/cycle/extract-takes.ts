 ...
 * derived index. `inbrain extract takes --rebuild` deletes all takes for
 ...
 * orchestrator persists to ~/.inbrain/sync-failures.jsonl via the existing
...
export async function extractTakesFromFs(
  ...
  for (const { path, relPath } of files) {
    ...
    if (pageId === null) {
      result.warnings.push(`TAKES_PAGE_NOT_IN_DB: slug=${slug} has takes fence but no page row; run 'inbrain sync' first`);
      continue;
    }
...
/** Single-entry dispatch for `inbrain extract takes` and the v0_28_0 orchestrator. */
