 ...
 *
 * Stored in ~/.inbrain/eval-receipts/ (best-effort disk artifact). Real
...
/** Full disk path under ~/.inbrain/eval-receipts/<filename>. */
export function buildReceiptPath(id: ReceiptIdentity): string {
  return join(inbrainPath('eval-receipts'), buildReceiptFilename(id));
}
