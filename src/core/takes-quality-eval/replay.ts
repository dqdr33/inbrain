...
export function loadReceiptFromDisk(receiptPath: string): TakesQualityReceipt {
  if (!existsSync(receiptPath)) {
    throw new Error(
      ...
      `use \`inbrain eval takes-quality trend --json\` to find the row and ` +
      `re-export with \`inbrain eval takes-quality replay --from-db <id>\` (v0.33+).`,
    );
  }
  ...
  if (r.schema_version !== 1) {
    throw new Error(
      ...
      `Receipt was likely produced by a newer inbrain; upgrade to read it.`,
    );
  }
