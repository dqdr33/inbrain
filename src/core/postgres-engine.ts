...
export class PostgresEngine implements BrainEngine {
  ...
  async connect(config: EngineConfig & { poolSize?: number; parentConnectionManager?: ConnectionManager }): Promise<void> {
    ...
    if (config.poolSize) {
      ...
      // INBRAIN_POOL_SIZE cap below the caller's requested size when set — the
      // env var is a user escape hatch, so it wins.
      const url = config.database_url;
      if (!url) throw new InbrainError('No database URL', 'database_url is missing', 'Provide --url');
      ...
      // Without this, `inbrain jobs work` against a Supabase pooler URL hits
      ...
      const opts: Record<string, unknown> = {
        ...
        // INBRAIN_PG_NOTICES=1.
        onnotice: process.env.INBRAIN_PG_NOTICES === '1' ? undefined : () => {},
      };
  ...
  async disconnect(): Promise<void> {
    ...
    if (this._sql) {
      // #1972: inbrain-owned hard bound so a PgBouncer drain that never settles
  ...
  async searchVector(embedding: Float32Array, opts?: SearchOpts): Promise<SearchResult[]> {
    ...
    // by `inbrain reindex --multimodal`. Carries BOTH text and image content
  ...
  async listStaleChunks(opts?: {
    ...
    // `inbrain embed --stale --source X` actually does what it says.
    //
  ...
  async getBacklinkCounts(slugs: string[]): Promise<Map<string, number>> {
    ...
    // mentions from `inbrain extract links --by-mention`; they're
  ...
  async updateTake(
    ...
    `;
    if (result.length === 0) {
      throw new InbrainError('TAKE_ROW_NOT_FOUND', `take not found at page_id=${pageId} row=${rowNum}`, 'list takes for this page with `inbrain takes <slug>` to see valid row numbers');
    }
  }
  ...
  async supersedeTake(
    ...
    return await conn.begin(async (tx) => {
      ...
      `;
      if (!existing) throw new InbrainError('TAKE_ROW_NOT_FOUND', `take not found at page_id=${pageId} row=${oldRow}`, 'list takes with `inbrain takes <slug>`');
  ...
  async resolveTake(pageId: number, rowNum: number, resolution: TakeResolution): Promise<void> {
    ...
    if (!existing) throw new InbrainError('TAKE_ROW_NOT_FOUND', `take not found at page_id=${pageId} row=${rowNum}`, 'list takes for this page with `inbrain takes <slug>` to see valid row numbers');
    if ((existing as { resolved_at?: unknown }).resolved_at) {
  ...
  async listEvalCandidates(filter?: { since?: Date; limit?: number; tool?: 'query' | 'search' }): Promise<EvalCandidate[]> {
    ...
    // — without this, `inbrain eval export --since` could dupe or miss rows
