/**
 * v0.32 — Per-source last-run cursor for `inbrain recall --since-last-run`.
 *
 * Two cursor variants per source (Codex round 2 #8):
 *   'briefing' → ~/.inbrain/recall-cursors/<source>.json
 *   'watch'    → ~/.inbrain/recall-cursors/<source>.watch.json
 *
...
function cursorPath(sourceId: string, variant: CursorVariant): string {
  ...
  return inbrainPath(CURSOR_DIR_SEGMENT, basename);
}
