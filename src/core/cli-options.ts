 ...
 * `inbrain --progress-json doctor` works: the global flag is stripped
...
export interface CliOptions {
  ...
  /**
   * v0.40.4 — `--explain` flag for `inbrain search/query`. Switches the
...
export function parseGlobalFlags(argv: string[]): { cliOpts: CliOptions; rest: string[] } {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    // v0.40.4 — --explain for `inbrain search/query` per-stage attribution.
    if (a === '--explain') {
...
/**
 * Build the global-flag suffix to append to child `inbrain …` subprocess
...
//
//   JOB=$(inbrain embed --stale --background | grep -oE 'job_id=[0-9]+' | cut -d= -f2)
//   inbrain jobs get $JOB
//
// `--background --follow` submits then execs `inbrain jobs follow <id>`
...
export async function maybeBackground(opts: MaybeBackgroundOpts): Promise<boolean> {
  ...
  try {
    ...
    if (follow) {
      // exec `inbrain jobs follow <id>` so the user sees live stream
