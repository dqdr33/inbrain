 ...
 * When `inbrain embed` (which can take minutes) is running and another process tries
...
const LOCK_DIR_NAME = '.inbrain-lock';
...
export async function acquireLock(dataDir: string | undefined, opts?: { timeoutMs?: number }): Promise<LockHandle> {
    ...
    try {
      ...
      if (Date.now() - startTime >= timeoutMs) {
        ...
        try {
          const lockData = JSON.parse(readFileSync(lockPath, 'utf-8'));
          throw new Error(
            `Inbrain: Timed out waiting for PGLite lock. Process ${lockData.pid} has held it since ${new Date(lockData.acquired_at).toISOString()} (command: ${lockData.command}). ` +
            `If that process is dead, remove ${lockDir} and try again.`
          );
        } catch (readErr) {
          if (readErr instanceof Error && readErr.message.startsWith('Inbrain')) throw readErr;
          throw new Error(
            `Inbrain: Timed out waiting for PGLite lock. Remove ${lockDir} and try again.`
          );
        }
      }
  ...
  throw new Error(`Inbrain: Timed out waiting for PGLite lock.`);
}
