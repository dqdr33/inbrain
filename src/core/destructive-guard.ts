 ...
 *
 * Protects against accidental data loss in inbrain by requiring explicit
...
export function checkDestructiveConfirmation(
  ...
  return (
    `\n${impact.summary}\n\n` +
    `To proceed, pass --confirm-destructive (or use soft-delete: inbrain sources archive ${impact.sourceId}).\n` +
 ...
 * TTL expires, the autopilot purge phase or manual `inbrain sources purge`
...
export function formatSoftDelete(sd: SoftDeletedSource): string {
  ...
  return [
    ...
    ``,
    `  Restore:  inbrain sources restore ${sd.id}`,
    `  Purge now: inbrain sources purge ${sd.id} --confirm-destructive`,
    ``,
  ].join('\n');
}
