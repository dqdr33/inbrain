/**
 * skillify/templates.ts — template strings for `inbrain skillify scaffold`.
 *
 ...
 * carries this marker until an implementer replaces it. `inbrain
...
export function skillMdTemplate(v: ScaffoldVars): string {
  ...
  lines.push('');
  lines.push('```bash');
  lines.push('inbrain eval cross-modal \\');
  ...
  lines.push(
    'land at `~/.inbrain/.inbrain/eval-receipts/<slug>-<sha8>.json` (sha-8 of SKILL.md',
  );
  lines.push(
    'content). `inbrain skillify check` surfaces the receipt status as informational.',
  );
  lines.push(
...
// ${v.name} — scaffolded by inbrain skillify scaffold
...
export function run(input: unknown): unknown {
  // TODO: implement. This stub is detected by \`inbrain check-resolvable
 ...
 *
 * Scaffolded by inbrain skillify scaffold. Replace these stubs with
