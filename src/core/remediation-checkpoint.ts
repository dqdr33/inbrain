 ...
 *
 * When `inbrain doctor --remediate --max-cost N` blows past the cap mid-run
 ...
 * completed so the user can continue with `inbrain doctor --remediate --resume`.
 *
 * Checkpoint file: `~/.inbrain/remediation/<plan_hash>.json`
...
function checkpointDir(): string {
  return inbrainPath('remediation');
}
