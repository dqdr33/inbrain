 ...
 * `inbrain frontmatter` CLI, the `frontmatter_integrity` doctor subcheck, and
 ...
 * rewrites). Backups live under ~/.inbrain/backups/frontmatter/... instead of
...
export function defaultFrontmatterBackupRoot(runId = makeFrontmatterBackupRunId()): string {
  return inbrainPath('backups', 'frontmatter', runId);
}
 ...
 *                          the YAML zone
 *   - SLUG_MISMATCH     — remove `slug:` line (inbrain derives slug from path)
 *
...
export function autoFixFrontmatter(
  ...
  //    inbrain derives slug from path; the field shouldn't be in frontmatter.
  if (opts?.filePath) {
 ...
 * ~/.inbrain/backups/frontmatter/... before any in-place mutation (the contract
      ...
      try {
        if (opts.deadline) {
          ...
          if (remainingMs <= 0) {
            ...
            // https://github.com/inbrain-ai/inbrain/actions/runs/77611667786
 ...
 * was the root cause of the `inbrain doctor` hang on 216K-page brains
 * reported in PR #1287.
 *
