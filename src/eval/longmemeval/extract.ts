...
export async function extractAndInsertClaims(opts: {
  ...
  try {
    const ins = await opts.engine.insertFacts(rows, { source_id: opts.sourceId }); // inbrain-allow-direct-insert: benchmark harness only — populates ephemeral in-memory PGLite per LongMemEval run; no markdown source-of-truth contract applies (chat sessions are the corpus, NOT a brain repo).
