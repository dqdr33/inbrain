/**
 * HTTP transport for `inbrain serve --http` (legacy bearer-auth path).
 *
 ...
 *   - Create/manage tokens with auth.ts (inbrain auth create/list/revoke)
 *   - No open OAuth, no client_credentials, no self-service tokens
 *
 * Hardening:
 *   - CORS default-deny: allowlist via INBRAIN_HTTP_CORS_ORIGIN (comma-separated)
 ...
 *   - Body cap: 1 MiB default (INBRAIN_HTTP_MAX_BODY_BYTES). Stream-counted, not buffered —
...
interface AuthResult {
   ...
   * Operators migrate to the full OAuth transport (inbrain serve --http)
...
/** Resolve client IP. Honors X-Forwarded-For only when INBRAIN_HTTP_TRUST_PROXY=1. */
function resolveClientIp(req: Request, server: { requestIP: (r: Request) => { address: string } | null }): string {
  if (process.env.INBRAIN_HTTP_TRUST_PROXY === '1') {
...
export async function startHttpTransport(opts: HttpTransportOptions) {
  ...
  const bodyCap = envInt('INBRAIN_HTTP_MAX_BODY_BYTES', DEFAULT_BODY_CAP);
  ...
  const server = Bun.serve({
    ...
    async fetch(req, server) {
      ...
      if (!auth.ok) {
        ...
        return Response.json(
          { error: 'invalid_token', message: 'Bearer token required. Create one: inbrain auth create <name>' },
        ...
        return Response.json(
          {
            result: {
              ...
              serverInfo: { name: 'inbrain', version: VERSION },
              capabilities: { tools: {} },
            },
            jsonrpc: '2.0',
            id,
  ...
  console.error(`Inbrain HTTP MCP server running on port ${port}`);
  console.error(`  Health: http://localhost:${port}/health`);
  console.error(`  MCP:    http://localhost:${port}/mcp`);
  console.error(`  Auth:   Bearer token required (create with: inbrain auth create <name>)`);
  if (!corsAllowlist) {
    console.error('  CORS:   default-deny. Set INBRAIN_HTTP_CORS_ORIGIN=https://your.app to allow browser clients.');
  } else {
    console.error(`  CORS:   allowlist = ${[...corsAllowlist].join(', ')}`);
  }
  console.error('');
  console.error('⚠️  Do NOT use open OAuth registration for remote MCP access.');
  console.error('   Tokens are managed via: inbrain auth create/list/revoke');
