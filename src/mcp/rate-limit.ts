/**
 * Rate limiter for `inbrain serve --http`.
 *
...
export function buildDefaultLimiters(clock: Clock = Date.now): { ip: RateLimiter; token: RateLimiter } {
  const lruCap = envInt('INBRAIN_HTTP_RATE_LIMIT_LRU', 10000);
  const windowMs = 60_000;
  return {
    ip: new RateLimiter({ limit: envInt('INBRAIN_HTTP_RATE_LIMIT_IP', 30), windowMs, lruCap }, clock),
    token: new RateLimiter({ limit: envInt('INBRAIN_HTTP_RATE_LIMIT_TOKEN', 60), windowMs, lruCap }, clock),
  };
}
