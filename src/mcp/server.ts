...
export async function startMcpServer(engine: BrainEngine) {
  ...
  // inbrain ops are synchronous, so we return the legacy `{ content, isError? }`
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request: any): Promise<any> => {
    ...
    // `inbrain call <op>` (sets remote=false in src/cli.ts).
    return dispatchToolCall(engine, name, params, {
      remote: true,
      takesHoldersAllowList: ['world'],
      // v0.31: source defaults to 'default' for stdio (no per-token scope).
      // Operators who want a different source on stdio MCP should set
      // INBRAIN_SOURCE in the env or use --source via `inbrain call`.
      sourceId: process.env.INBRAIN_SOURCE || 'default',
  ...
  const shutdown = (reason: string, code = 0) => {
    ...
    shuttingDown = true;
    process.stderr.write(`[inbrain-serve] shutdown: ${reason}\n`);
...
// Backward compat: used by `inbrain call` command (trusted local path).
// v0.31.8 (D22): accept opts.sourceId so `inbrain call --source X <op> <json>`
