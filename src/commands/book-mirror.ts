 ...
 * filed in TODOS.md.
 *
 * `inbrain book-mirror` — flagship of the v0.25.1 skills wave.
 *
...
function printHelp(): void {
  console.log(`inbrain book-mirror — personalized chapter-by-chapter book analysis

USAGE
  inbrain book-mirror --chapters-dir <path> --slug <slug> [flags]
...
EXAMPLES
  ...
  inbrain book-mirror --chapters-dir ./chapters --slug test --dry-run
`);
}
...
async function confirmInteractive(estimateUsd: number, chapters: number): Promise<boolean> {
  if (process.stdin.isTTY !== true) {
    ...
    process.stderr.write(
      `inbrain book-mirror: refusing to spend ~$${estimateUsd.toFixed(2)} on ${chapters} chapters from a non-TTY context. ` +
      `Pass --yes to confirm.\n`
    );
    return false;
  }
...
This page was generated by \`inbrain book-mirror\`. Each chapter analysis came from a separate read-only subagent that had access to the chapter text and a reader-context pack but no write tools — so the brain wasn't modified during the per-chapter analysis. This page is the only artifact written.

`;

  const failedSection = opts.chapterAnalyses
    .filter(a => a.failed)
    .map(a => `> Chapter ${a.index}: analysis failed (${a.error ?? 'unknown error'}). Re-run \`inbrain book-mirror\` to retry; idempotent on the same inputs.`)
    .join('\n\n');
...
export async function runBookMirrorCmd(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!flags.chaptersDir) {
    console.error('inbrain book-mirror: --chapters-dir is required. Run with --help.');
    process.exit(2);
  }
  if (!flags.slug) {
    console.error('inbrain book-mirror: --slug is required. Run with --help.');
    process.exit(2);
  }
  if (!/^[a-z0-9][a-z0-9-]*$/i.test(flags.slug)) {
    console.error(`inbrain book-mirror: invalid --slug "${flags.slug}". Use kebab-case (a-z, 0-9, hyphens).`);
    process.exit(2);
  }
  if (flags.contextFile && !fs.existsSync(flags.contextFile)) {
    console.error(`inbrain book-mirror: --context-file not found: ${flags.contextFile}`);
    process.exit(2);
  }
  ...
  try {
    ...
    console.error(`inbrain book-mirror: ${e instanceof Error ? e.message : String(e)}`);
    process.exit(2);
  }
  ...
  if (flags.dryRun) {
    process.stderr.write(`inbrain book-mirror: --dry-run — exiting without submission.\n`);
    return;
  }
  ...
  if (!flags.noConfirm) {
    ...
    if (!ok) {
      process.stderr.write(`inbrain book-mirror: cancelled by user.\n`);
      process.exit(0);
    }
  }
  ...
  if (!flags.follow) {
    ...
    process.stderr.write(
      `inbrain book-mirror: detached. Run \`inbrain jobs get <id>\` per child, then re-run with same args once all are complete.\n`
    );
    return;
  }
  ...
  if (completed === 0) {
    console.error(`inbrain book-mirror: every chapter failed. Not writing the brain page. Re-run after diagnosing.`);
    process.exit(1);
  }
  ...
  if (failed > 0) {
    process.stderr.write(
      `\ninbrain book-mirror: ${failed} chapter(s) failed. The page was written with the completed chapters; run again to retry the failed ones (idempotency keys dedupe successful chapters).\n`
    );
    process.exit(1);
  }
}
