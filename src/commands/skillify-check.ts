/**
 * inbrain skillify check — 11-item post-task audit.
 *
 ...
 * the CLI entry point is now `inbrain skillify check`.
 *
 ...
 *   8. check-resolvable gate (runs `inbrain check-resolvable --json`)
 ...
 *      receipt at `inbrainPath('eval-receipts')/<slug>-<sha8>.json`
...
function runCheckResolvableCached(): ResolverResult {
  ...
  try {
    const res = spawnSync('inbrain', ['check-resolvable', '--json'], {
      encoding: 'utf-8',
      maxBuffer: 10 * 1024 * 1024,
    });
    if (res.error || res.status === null) {
      const reason = res.error?.message ?? 'spawn returned null status';
      console.error(`[skillify] inbrain check-resolvable not runnable: ${reason}`);
    ...
    if (payload.ok === true) {
      ...
      _resolverCache = {
        ok: false,
        detail: `${count} issue(s)${err} — run: inbrain check-resolvable`,
      };
    }
    return _resolverCache;
  } catch (err) {
...
function runSkillifyCheckTarget(target: string, root: string): CheckResult {
  ...
  // `inbrain skillify check` with exit 1 so new skills can't be born
  // non-compliant.
 ...
 *
 * Reads the receipt from `inbrainPath('eval-receipts')` (T5 correction:
 * this resolves to <INBRAIN_HOME>/.inbrain/eval-receipts/, NOT the legacy
 * <INBRAIN_HOME>/eval-receipts/ that the original plan claimed).
 */
function lookupCrossModalReceipt(
  ...
  try {
    status = findReceiptForSkill(skillMdPath, inbrainPath('eval-receipts'));
  } catch (err) {
...
const HELP = `inbrain skillify check [path] [--recent] [--json]
...
/**
 * Entry point invoked by `inbrain skillify check`. The outer
