/**
 * `inbrain cache` \u2014 manage the semantic query cache (v0.32.x search-lite).
 *
 * Subcommands:
 *   inbrain cache stats   \u2014 print row/hit counts and freshness breakdown.
 *   inbrain cache clear   \u2014 wipe all cache rows.
 *   inbrain cache prune   \u2014 delete only stale (past-TTL) rows.
 *
...
Usage:
  inbrain cache stats        Print cache row counts, hit counts, freshness.
  inbrain cache clear        Wipe ALL cache rows. Requires --yes.
  inbrain cache prune        Delete only stale (past-TTL) rows.
...
export async function runCache(args: string[]): Promise<void> {
  ...
  if (!config) {
    // eslint-disable-next-line no-console
    console.error('inbrain cache: no brain configured. Run `inbrain init` first.');
    process.exit(1);
  }
  ...
  try {
    ...
    if (sub === 'clear') {
      ...
      if (!yes) {
        // eslint-disable-next-line no-console
        console.error('inbrain cache clear: refusing to wipe without --yes flag.');
        process.exit(1);
      }
    ...
    // eslint-disable-next-line no-console
    console.error(`inbrain cache: unknown subcommand "${sub}". See \`inbrain cache --help\`.`);
    process.exit(1);
  } finally {
