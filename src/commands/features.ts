/**
 * inbrain features — Scan brain usage and recommend unused features.
 *
 * Usage:
 *   inbrain features [--json] [--auto-fix] [--help]
 */
...
function offersPath(): string {
  return join(process.env.HOME || '', '.inbrain', 'feature-offers.json');
}
...
async function scanFeatures(engine: BrainEngine): Promise<FeatureScanResult> {
  ...
  if (health.dead_links > 0) {
    recommendations.push({
      ...
      command: 'inbrain check-backlinks fix',
      auto_fixable: false,
    });
  }
  ...
  if (stats.page_count >= 3) {
    ...
    if (stats.timeline_entry_count === 0 && stats.page_count > 5) {
      recommendations.push({
        ...
        command: 'inbrain extract timeline',
        auto_fixable: true,
      });
    }
    ...
    if (unconfigured.length > 0) {
      recommendations.push({
        ...
        command: `inbrain integrations list`,
        auto_fixable: false,
      });
    }
    ...
    try {
      ...
      if (!syncRepo) {
        recommendations.push({
          ...
          command: 'inbrain sync --repo <path>',
          auto_fixable: false,
        });
      }
    } catch { /* skip */ }
...
export async function runFeatures(engine: BrainEngine, args: string[]) {
  ...
  if (autoFix) {
    ...
    console.log(`Run 'inbrain features --auto-fix' to fix all auto-fixable issues.`);
  }
...
export async function featuresTeaserForDoctor(engine: BrainEngine): Promise<string | null> {
  try {
    ...
    return `Tip: ${parts.join(', ')}. Run 'inbrain features' to fix.`;
  } catch {
    return null;
  }
}
