/**
 * inbrain orphans — Surface pages with no inbound wikilinks.
 *
 ...
 *
 * Usage:
 *   inbrain orphans                  # list orphans grouped by domain
 *   inbrain orphans --json           # JSON output for agent consumption
 *   inbrain orphans --count          # just the number
 *   inbrain orphans --include-pseudo # include auto-generated/pseudo pages
 */
 ...
 * Two consumers sharing one definition = doctor and `inbrain orphans`
 ...
 * `inbrain orphans` CLI AND doctor's `orphan_ratio` check. Aliased to
...
export async function runOrphans(engine: BrainEngine, args: string[]) {
  ...
  // when the flag is absent and silently scope a bare `inbrain orphans`.
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: inbrain orphans [options]
