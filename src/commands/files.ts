...
export async function runFiles(engine: BrainEngine, args: string[]) {
  ...
  switch (subcommand) {
    ...
    case 'status':
      ...
      break;
    default:
      console.error(`Usage: inbrain files <command> [args]`);
...
async function uploadFile(engine: BrainEngine, args: string[]) {
  ...
  if (!filePath || !existsSync(filePath)) {
    console.error('Usage: inbrain files upload <file> --page <slug>');
    process.exit(1);
  }
...
async function uploadRaw(engine: BrainEngine, args: string[]) {
  ...
  if (!filePath || !existsSync(filePath)) {
    console.error('Usage: inbrain files upload-raw <file> --page <slug> [--type <type>] [--no-pointer]');
    process.exit(1);
  }
  ...
  const config = loadConfig();
  if (!config?.storage) {
    console.error('No storage backend configured. Run inbrain init with storage settings.');
    console.error('Or use inbrain files upload for manual uploads.');
    process.exit(1);
  }
...
async function signedUrl(args: string[]) {
  ...
  if (!storagePath) {
    console.error('Usage: inbrain files signed-url <storage-path>');
    process.exit(1);
  }
...
async function syncFiles(engine: BrainEngine, dir?: string) {
  if (!dir || !existsSync(dir)) {
    console.error('Usage: inbrain files sync <directory>');
    process.exit(1);
  }
...
async function verifyFiles(engine: BrainEngine) {
  ...
  if (mismatches === 0 && missing === 0) {
    ...
    console.error(`Run: inbrain files sync --retry-failed`);
    process.exit(1);
  }
}
...
async function mirrorFiles(args: string[]) {
  ...
  if (!dir || !existsSync(dir)) { console.error('Usage: inbrain files mirror <dir> [--dry-run]'); process.exit(1); }

  const { createStorage } = await import('../core/storage.ts');
  const { loadConfig } = await import('../core/config.ts');
  const { stringify } = await import('../core/yaml-lite.ts');
  const config = loadConfig();
  if (!config?.storage) { console.error('No storage backend configured. Run inbrain init with storage settings.'); process.exit(1); }
...
async function unmirrorFiles(args: string[]) {
  const dir = args.find(a => !a.startsWith('--'));
  if (!dir) { console.error('Usage: inbrain files unmirror <dir>'); process.exit(1); }
...
async function redirectFiles(args: string[]) {
  const dir = args.find(a => !a.startsWith('--'));
  const dryRun = args.includes('--dry-run');
  if (!dir || !existsSync(dir)) { console.error('Usage: inbrain files redirect <dir> [--dry-run]'); process.exit(1); }

  const markerPath = join(dir, '.supabase');
  if (!existsSync(markerPath)) {
    console.error('Directory must be mirrored first. Run: inbrain files mirror <dir>');
    process.exit(1);
  }
  ...
  if (skippedMissing > 0) {
    console.log(`Skipped ${skippedMissing} files (not found in remote storage — run 'inbrain files mirror' first).`);
  }
  console.log('To undo: inbrain files restore <dir>');
}

async function restoreFiles(args: string[]) {
  const dir = args.find(a => !a.startsWith('--'));
  if (!dir || !existsSync(dir)) { console.error('Usage: inbrain files restore <dir>'); process.exit(1); }
...
async function cleanFiles(args: string[]) {
  ...
  if (!dir || !existsSync(dir)) { console.error('Usage: inbrain files clean <dir> [--yes]'); process.exit(1); }
...
async function filesStatus(args: string[]) {
  ...
  if (mirrored === 0 && redirected === 0 && local > 0) {
    console.log(`\n${local} local files. Run: inbrain files mirror <dir> to start migration.`);
  } else if (redirected > 0) {
    console.log(`\n${redirected} files redirected to storage. Run: inbrain files clean <dir> --yes to remove breadcrumbs.`);
  }
}
