/**
 * `inbrain eval brainstorm` — three-axis evaluation gate for the v0.37.0
 ...
 *
 * Exit codes mirror inbrain's eval convention:
...
const HELP = `Usage: inbrain eval brainstorm <fixture.jsonl> [options]

Three-axis evaluation gate for \`inbrain brainstorm\`. Runs the fixture's
...
export async function runEvalBrainstorm(engine: BrainEngine, args: string[]): Promise<number> {
  ...
  if (parsed.error) {
    console.error(`inbrain eval brainstorm: ${parsed.error}`);
    console.error(HELP);
    return 2;
  }
  if (!parsed.fixture) {
    console.error('inbrain eval brainstorm: fixture path required');
    console.error(HELP);
    return 2;
  }
  ...
  try {
    ...
    console.error(`inbrain eval brainstorm: ${err instanceof Error ? err.message : err}`);
    return 2;
  }
