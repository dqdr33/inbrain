/**
 * inbrain brainstorm — bisociation-style idea generation grounded in your
 * own notes.
 *
...
const BRAINSTORM_HELP = `Usage: inbrain brainstorm <question> [options]
...
Examples:
  inbrain brainstorm "why are AI coding tools converging on the same UX?"
  inbrain brainstorm "what's the real bottleneck on lab automation" --json

Cost: ~$0.05-0.15 per run. Set INBRAIN_NO_BRAINSTORM_PREVIEW=1 or pass --yes
to skip the TTY grace window in scripted callers.

See also: inbrain lsd — Lateral Synaptic Drift, the inverted-judge variant
that prefers forgotten pages and rejects ideas that are "too obvious."
`;

const LSD_HELP = `Usage: inbrain lsd <question> [options]

LSD = Lateral Synaptic Drift. Same bisociation engine as \`inbrain brainstorm\`
...
Examples:
  inbrain lsd "why are AI coding tools converging on the same UX?"
  inbrain lsd "the unspoken assumption in venture pricing" --save

Cost: ~$0.20-0.40 per run.

See also: inbrain brainstorm — the sober, cite-heavy default variant.
`;
...
async function runBrainstormCli(
  ...
  if (parsed.error) {
    console.error(`inbrain ${profile.label}: ${parsed.error}`);
    console.error(help);
    process.exit(2);
    return;
  }
  ...
  if (!parsed.question || parsed.question.trim().length === 0) {
    console.error(`inbrain ${profile.label}: question required`);
    console.error(help);
    process.exit(2);
    return;
  }
  ...
  const skipPreview = parsed.yes || process.env.INBRAIN_NO_BRAINSTORM_PREVIEW === '1';
  ...
  if (shouldSave) {
    ...
    // the saved file round-trips through `inbrain sync` byte-for-byte. Include
 ...
 * means the two sinks cannot diverge, and the row matches what `inbrain sync`
 ...
 * the DB row landed stays exit 0 — the row is durable and `inbrain sync`
...
export function formatSaveOutcome(
  ...
  const stderr: string[] = [];
  if (dbError) stderr.push(`inbrain ${ctx.profileLabel}: DB save failed: ${dbError}`);
  if (writeThrough.error) {
    stderr.push(`inbrain ${ctx.profileLabel}: file write failed: ${writeThrough.error}`);
  }
  ...
  if (dbSaved) {
    ...
    return {
      stdout: `\n_Saved to DB page \`${ctx.slug}\` (file NOT written — see error above; \`inbrain sync\` will reconcile)._`,
      stderr,
      exitCode: 0,
    };
  }
  ...
  stderr.push(
    `inbrain ${ctx.profileLabel}: save FAILED — neither DB page nor file was written. The idea is NOT persisted.`,
  );
