/**
 * Inbrain HTTP MCP server with OAuth 2.1.
 *
 * Combines:
...
import { InbrainOAuthProvider, validateTokenEndpointAuthMethod } from '../core/oauth-provider.ts';
...
export function resolveBootstrapToken(
  ...
  if (!/^[A-Za-z0-9_-]{32,}$/.test(trimmed)) {
    return {
      kind: 'error',
      message:
        'INBRAIN_ADMIN_BOOTSTRAP_TOKEN must be at least 32 chars and match [A-Za-z0-9_-]+.\n' +
...
/**
 * Resolve `INBRAIN_HTTP_TRUST_PROXY` into a value Express's `app.set('trust
 ...
 *
 * SECURITY: only set INBRAIN_HTTP_TRUST_PROXY when BOTH (a) inbrain is
...
/**
 * Parse `INBRAIN_HTTP_CORS_ORIGIN` into a Set of allowed origins for OAuth
...
export function parseCorsAllowlistOAuth(): Set<string> | null {
  const v = process.env.INBRAIN_HTTP_CORS_ORIGIN;
  if (!v) return null;
...
interface ServeHttpOptions {
   ...
   *
   * Operators running inbrain on their own laptop and debugging agent behavior
   ...
   * v0.34.1+ — inbrain's primary use case is a personal-knowledge brain on a
   ...
   * startup. Combined with `INBRAIN_ADMIN_BOOTSTRAP_TOKEN`, lets long-lived
...
export function skillPublishStatus(publishSkills: boolean): { bannerValue: string; nudge: string | null } {
  ...
  return {
    ...
    nudge:
      ...
      'inbrain config set mcp.publish_skills true',
  };
}
...
export async function runServeHttp(engine: BrainEngine, options: ServeHttpOptions) {
  ...
  // inbrain's primary use case is a personal-knowledge brain on a laptop;
  ...
  // (skill-catalog.ts): the DB plane (`inbrain config set`) wins over the file
  ...
  // a postgres.js singleton, so `inbrain serve --http` works against PGLite
  ...
  const oauthProvider = new InbrainOAuthProvider({
    sql,
    tokenTtl,
  ...
  // restart is the original gripe; with `INBRAIN_ADMIN_BOOTSTRAP_TOKEN` set
  // and `--suppress-bootstrap-token`, no value reaches the log.
  const resolved = resolveBootstrapToken(process.env.INBRAIN_ADMIN_BOOTSTRAP_TOKEN);
  ...
  // Express 5 app
  const app = express();
  // v0.41.3 (T8): configurable trust-proxy via INBRAIN_HTTP_TRUST_PROXY env.
  ...
  // set INBRAIN_HTTP_TRUST_PROXY=1 (one hop) so X-Forwarded-For lands as the
  ...
  app.set('trust proxy', resolveTrustProxy(process.env.INBRAIN_HTTP_TRUST_PROXY));
  ...
  // INBRAIN_HTTP_CORS_ORIGIN the same way the legacy transport already does
  ...
  if (!corsAllowlistOAuth && bind === '0.0.0.0') {
    console.error(
      '[serve-http] WARNING: --bind 0.0.0.0 is set but INBRAIN_HTTP_CORS_ORIGIN is unset. OAuth endpoints will reject ALL cross-origin requests until you set the env var (comma-separated origins).',
    );
  }
  ...
  const authRouterOptions: any = {
    ...
    resourceName: 'Inbrain MCP Server',
  };
  ...
  app.post('/admin/login', express.json(), (req, res) => {
    ...
    res.cookie('inbrain_admin', sessionId, adminCookie(req, 24 * 60 * 60 * 1000));
...
<div class="prompt">&ldquo;Give me the Inbrain admin login link&rdquo;</div>
</div></div></body></html>`);
      return;
    }
    ...
    res.cookie('inbrain_admin', sessionId, adminCookie(req, 7 * 24 * 60 * 60 * 1000));
    res.redirect('/admin/');
  });

  // Admin auth middleware
  function requireAdmin(req: express.Request, res: express.Response, next: express.NextFunction) {
    const sessionId = (req.cookies as Record<string, string>)?.inbrain_admin;
  ...
  // TTY `inbrain jobs watch` command so the two surfaces stay 1:1.
  ...
  app.post('/admin/api/api-keys', requireAdmin, express.json(), async (req: Request, res: Response) => {
    try {
      ...
      const token = generateToken('inbrain_');
  ...
  //      globally-installed `inbrain serve --http` actually serves /admin
  ...
  // backchannel for server-initiated messages. inbrain's transport is stateless
  ...
  app.post('/mcp', requireBearerAuth({ verifier: oauthProvider }), async (req: Request, res: Response) => {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      // fell back to INBRAIN_SOURCE env / 'default' — the silent-fallback
  ...
  const ingestMaxBytes = (() => {
    const fromEnv = process.env.INBRAIN_INGEST_MAX_BYTES;
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      const declared = (req.header('x-inbrain-content-type') || req.header('content-type') || '').toLowerCase();
      ...
      const sourceUri = (req.header('x-inbrain-source-uri') || `mcp-webhook:${authInfo.clientId}:${Date.now()}`).slice(0, 1024);
      const sourceId = (req.header('x-inbrain-source-id') || `webhook-${authInfo.clientId}`).slice(0, 256);
      const callerSlug = req.header('x-inbrain-slug');
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      if (!secret || typeof secret !== 'string') {
        res.status(401).json({ error: 'webhook_not_configured', message: 'Run: inbrain sources webhook set ' + source.id });
        return;
      }
  ...
  app.listen(port, bind, () => {
    console.error(`
╔══════════════════════════════════════════════════════╗
║  Inbrain MCP Server v${VERSION.padEnd(37)}║
╠══════════════════════════════════════════════════════╣
...
${suppressBootstrapPrint
  ...
  : bootstrapFromEnv
    ? '║  Admin Token: from $INBRAIN_ADMIN_BOOTSTRAP_TOKEN     ║\n╚══════════════════════════════════════════════════════╝'
