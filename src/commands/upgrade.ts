...
export async function runUpgrade(args: string[]) {
  ...
  switch (method) {
    ...
    case 'bun': {
      ...
      try {
        execFileSync('bun', ['update', 'inbrain'], { cwd: bunGlobalRoot, stdio: 'inherit', timeout: 120_000 });
        upgraded = true;
      } catch {
        console.error('Upgrade failed. Try running manually:');
        console.error(`  cd ${bunGlobalRoot} && bun update inbrain`);
      }
      break;
    }
    ...
    case 'binary': {
      ...
      console.log('Updating inbrain binary (atomic download + replace)...');
      ...
      if (result.ok) {
        ...
        console.log('  https://github.com/inbrain-ai/inbrain/releases');
      } else {
        ...
        console.error('  https://github.com/inbrain-ai/inbrain/releases');
        recordUpgradeError({
          phase: 'binary-self-update',
          fromVersion: oldVersion,
          toVersion: '',
          error: `${result.reason}${result.error ? `: ${result.error}` : ''}`,
          hint: 'Download from https://github.com/inbrain-ai/inbrain/releases',
        });
      }
      break;
    }
    ...
    case 'clawhub':
      ...
      try {
        execSync('clawhub update inbrain', { stdio: 'inherit', timeout: 120_000 });
        upgraded = true;
      } catch {
        console.error('ClawHub upgrade failed. Try: clawhub update inbrain');
      }
      break;
    ...
    default:
      ...
      console.log('  bun update inbrain');
      console.log('  clawhub update inbrain');
      console.log('  Download from https://github.com/inbrain-ai/inbrain/releases');
  }
  ...
  if (upgraded) {
    ...
    // INBRAIN_POST_UPGRADE_TIMEOUT_MS env var.
    const postUpgradeTimeoutMs = Number(
      process.env.INBRAIN_POST_UPGRADE_TIMEOUT_MS || 1_800_000,
    );
    try {
      execSync('inbrain post-upgrade', { stdio: 'inherit', timeout: postUpgradeTimeoutMs });
    } catch (e) {
      // post-upgrade is best-effort, don't fail the upgrade. BUT leave a
      // trail so `inbrain doctor` can surface it and give the user a clear
      ...
      recordUpgradeError({
        ...
        hint: 'Run: inbrain apply-migrations --yes',
      });
    }
    // Run features scan to show what's new and what to fix
    try {
      execSync('inbrain features', { stdio: 'inherit', timeout: 30_000 });
    } catch {
...
function findBunInstallRootFromArgv(): string | null {
  try {
    ...
    for (let i = 0; i < 10; i++) {
      if (basename(dir) === 'inbrain' && basename(dirname(dir)) === 'node_modules') {
...
function verifyUpgrade(): string {
  try {
    const output = execSync('inbrain --version', { encoding: 'utf-8', timeout: 10_000 }).trim();
    console.log(`Upgrade complete. Now running: ${output}`);
    return output.replace(/^inbrain\s*/i, '').trim();
  } catch {
...
/**
 * Append a structured record to ~/.inbrain/upgrade-errors.jsonl when a
 * best-effort phase of the upgrade fails (e.g., `inbrain post-upgrade`
 * silently bombing). Without this trail, users end up with half-upgraded
 * brains and no signal. `inbrain doctor` reads this file and surfaces the
...
export function recordUpgradeError(record: {
  ...
  try {
    const dir = join(process.env.HOME || '', '.inbrain');
 ...
 *      has a from/to pair).
 *   2. Invoke `inbrain apply-migrations --yes` so the mechanical side of
...
async function applySelfUpgradeSetup(): Promise<void> {
  try {
    ...
    if (cfg) {
      ...
      if (!su.mode_prompted) {
        console.log('');
        console.log('═══════════════════════════════════════════════════════════════');
        console.log('[inbrain] Self-upgrade is ON in NOTIFY mode.');
        console.log('[inbrain] Every inbrain invocation now checks for new versions and');
        console.log('[inbrain] nudges when one is available. Apply with: inbrain self-upgrade');
        console.log('[inbrain]');
        console.log('[inbrain] Hands-off (silent quiet-hours auto-upgrade for always-on installs):');
        console.log('[inbrain]   inbrain config set self_upgrade.mode auto');
        console.log('[inbrain] Turn it off entirely: inbrain config set self_upgrade.mode off');
        console.log('═══════════════════════════════════════════════════════════════');
        console.log('');
  ...
  try {
    ...
    if (r.rewritten) {
      console.log('[inbrain] Updated autopilot systemd unit to Restart=always (self-upgrade relaunch).');
    }
  } catch {
    /* best-effort */
  }
...
export async function runPostUpgrade(args: string[] = []): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log('Usage: inbrain post-upgrade');
  ...
  // v0.35.8.0: lay down ~/.inbrain/.gitignore retroactively. Existing users
  // never re-run `inbrain init`, so init-only coverage misses them entirely
  ...
  try {
    const statePath = join(process.env.HOME || '', '.inbrain', 'upgrade-state.json');
        ...
        for (const m of migrations) {
          if (isNewerThan(m.version, from)) {
            ...
            if (m.featurePitch.recipe) {
              console.log(`Run \`inbrain integrations show ${m.featurePitch.recipe}\` to set it up.`);
            }
            console.log('');
          }
        }
  ...
  try {
    ...
    // Users can re-run `inbrain apply-migrations` manually if they want
    // to retry.
    const msg = e instanceof Error ? e.message : String(e);
    console.error(`\napply-migrations failed: ${msg}`);
    console.error('Run `inbrain apply-migrations --yes` manually to retry.');
  }
  ...
  // `inbrain upgrade` leaves wedged brains wedged — the user has to read
  // the WARN and run `inbrain init --migrate-only` themselves. We've shipped
  ...
  try {
      ...
      try {
        ...
        try {
          ...
          if (shown !== 'true' && !existingMode) {
            console.log('');
            console.log('═══════════════════════════════════════════════════════════════');
            console.log('[inbrain] v0.32.3 added named search modes (conservative/balanced/tokenmax).');
            console.log('[inbrain] Your current behavior is unchanged — the `query` op still defaults');
            console.log('[inbrain] expand=true and limit=20, same as v0.31.x.');
            console.log('═══════════════════════════════════════════════════════════════');
            console.log('');
            ...
            console.log('[AGENT] Run `inbrain config set search.mode <mode>` after they decide.');
            console.log('');
            ...
            console.log('');
            console.log('To pick:');
            console.log('  inbrain search modes              # see what is running');
            console.log('  inbrain config set search.mode <conservative|balanced|tokenmax>');
            console.log('  inbrain search tune               # data-driven recommendations');
            console.log('');
            console.log('tokenmax bumps limit to 50 (current default is 20). To preserve');
            console.log('your EXACT current shape:');
            console.log('  inbrain config set search.mode tokenmax');
            console.log('  inbrain config set search.searchLimit 20');
            console.log('');
        ...
        // `inbrain init`; EXISTING installs stay OFF (default-OFF runtime = no
        ...
        try {
          ...
          if (prompted !== 'true' && current == null) {
            ...
            const dirLine = det.dir
              ...
              : 'Skills dir: not auto-detected — set $INBRAIN_SKILLS_DIR or mcp.skills_dir before enabling.';
            console.log('');
            console.log('═══════════════════════════════════════════════════════════════');
            console.log('[inbrain] Publish your skills to MCP clients?');
            console.log('[inbrain] Codex desktop, Claude Code/Cowork, and Perplexity can then');
            console.log("[inbrain] DISCOVER and FOLLOW your agent's skills over `inbrain serve`.");
            console.log('[inbrain] This makes your MCP server dramatically more useful.');
            console.log('[inbrain]');
            console.log(`[inbrain] ${dirLine}`);
            console.log('[inbrain] Effect: the CONTENTS of your SKILL.md files become readable by');
            console.log('[inbrain] remote MCP callers you have authorized. Source code is NOT exposed.');
            console.log('═══════════════════════════════════════════════════════════════');
            ...
            if (isTty) {
              ...
              enabled = await new Promise<boolean>((resolveAns) => {
                ...
                rl.question('[inbrain] Enable skill publishing now? (recommended) [Y/n] ', (answer) => {
                  rl.close();
              ...
              console.log('[AGENT] Enable with: inbrain config set mcp.publish_skills true');
            }
            if (enabled) {
              await engine.setConfig('mcp.publish_skills', 'true');
              console.log('[inbrain] Skill publishing ENABLED. Disable anytime: inbrain config set mcp.publish_skills false');
            } else if (isTty) {
              console.log('[inbrain] Left disabled. Enable later: inbrain config set mcp.publish_skills true');
            }
        ...
        try {
          ...
          console.warn('Run `inbrain reindex --markdown` manually when ready.');
        }
      } finally {
    ...
    // hint to run `inbrain init --migrate-only`.
    const msg = e instanceof Error ? e.message : String(e);
    console.warn(`\nSchema auto-apply skipped: ${msg}`);
    console.warn('Run `inbrain init --migrate-only` manually if your brain is wedged.');
  }
  ...
  // v0.36 DX: skillpack reference sweep. After an upgrade, the inbrain bundle
  ...
  //   - INBRAIN_SKIP_REFERENCE_SWEEP=1 in env
  //   - no target workspace can be auto-detected (inbrain installed but
  //     never scaffolded anywhere)
  //   - the detected workspace IS the inbrain repo (dev-mode, would just
  //     compare inbrain against itself)
  ...
  // recommendations need engine access via `inbrain onboard --check`;
...
export async function postUpgradeReferenceSweep(
  opts: { inbrainRoot?: string; targetWorkspace?: string } = {},
): Promise<void> {
  if (process.env.INBRAIN_SKIP_REFERENCE_SWEEP) return;
  try {
    ...
    const inbrainRoot = opts.inbrainRoot ?? findGbrainRoot();
    if (!inbrainRoot) return;

    // Dev-mode guard: the detected workspace IS the inbrain repo. Sweeping
    // inbrain against itself is always identical — print nothing.
    if (path.resolve(targetWorkspace) === path.resolve(inbrainRoot)) return;

    const result = runReferenceAll({ inbrainRoot, targetWorkspace });
    ...
    console.log('');
    console.log(
      'Run `inbrain skillpack reference <slug>` to inspect per-skill diffs.\nSee `skills/_AGENT_README.md` for what your agent should do on update.\nSkip this sweep: `INBRAIN_SKIP_REFERENCE_SWEEP=1`.',
    );
  } catch {
...
export function detectInstallMethod(): 'bun' | 'bun-link' | 'binary' | 'clawhub' | 'unknown' {
  ...
  // bun link puts a symlink at ~/.bun/bin/inbrain → either the source's bin
  ...
  // (we publish under inbrain-ai/inbrain) OR the squatter (npm `inbrain@1.3.x`).
  ...
  if (execPath.endsWith('/inbrain') || execPath.endsWith('\\inbrain.exe')) {
    return 'binary';
  }
 ...
 * contains `inbrain-ai/inbrain` (case-insensitive substring).
 *
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (existsSync(gitConfigPath)) {
        try {
          ...
          if (cfg.toLowerCase().includes(INBRAIN_GITHUB_REPO.toLowerCase())) {
            return { repoRoot: dir };
          }
 ...
 *
 * When `bun add -g inbrain` (or `npm install -g inbrain`) installs from
 * npm, the package is the squatter — an unrelated `inbrain@1.3.x` that
 ...
 *   - `repository.url` contains `inbrain-ai/inbrain` (case-insensitive)
 ...
 * name like `@inbrain-ai/inbrain` (tracked v0.29 follow-up).
 */
      ...
      if (existsSync(pkgPath)) {
        try {
          ...
          const repoUrl = (typeof pkg.repository === 'string'
            ? pkg.repository
            : pkg.repository?.url) ?? '';
          if (repoUrl.toLowerCase().includes(INBRAIN_GITHUB_REPO.toLowerCase())) {
            return 'canonical';
          }
...
function printSquatterRecovery(): void {
  console.warn('');
  console.warn('  WARNING: inbrain install does not appear to be from inbrain-ai/inbrain.');
  console.warn('  This is likely the npm-name collision tracked in issue #658:');
  console.warn('    https://www.npmjs.com/package/inbrain (an unrelated package).');
  console.warn('');
  console.warn('  Recovery options:');
  console.warn('    1. Install from source:');
  console.warn('         bun remove -g inbrain');
  console.warn('         git clone https://github.com/inbrain-ai/inbrain.git');
  console.warn('         cd inbrain && bun install && bun link');
  console.warn('');
  console.warn('    2. Download a release binary:');
  console.warn('         https://github.com/inbrain-ai/inbrain/releases');
  console.warn('');
