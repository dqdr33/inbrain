/**
 * `inbrain apply-migrations` — migration runner CLI.
 *
 * Reads ~/.inbrain/migrations/completed.jsonl, diffs against the TS migration
 ...
 *
 * Invoked from:
 *   - `inbrain upgrade` → runPostUpgrade() tail (Lane A-5)
...
function printHelp(): void {
  console.log(`inbrain apply-migrations — run pending migration orchestrators.

Usage:
  inbrain apply-migrations                Run all pending migrations interactively.
  inbrain apply-migrations --yes          Non-interactive; uses default mode (pain_triggered).
  inbrain apply-migrations --dry-run      Print the plan; take no action.
  inbrain apply-migrations --list         Show applied + pending migrations.
  inbrain apply-migrations --migration vX.Y.Z
                                         Force-run a specific migration by version.
  inbrain apply-migrations --force-retry vX.Y.Z
                                         Clear a wedged migration (3+ consecutive
                                         partials). Writes a 'retry' marker so the
                                         next run treats it as fresh.
  inbrain apply-migrations --force-orchestrator
                                         Reset every wedged orchestrator migration
                                         in one shot (writes 'retry' for each).
  inbrain apply-migrations --force-schema
                                         Reset schema-version drift; re-runs
                                         runMigrations from current config.version.
  inbrain apply-migrations --force        (alias --force-all) Apply both
                                         --force-orchestrator and --force-schema.
  inbrain apply-migrations --skip-verify  Bypass post-condition verify hooks on
...
function printList(plan: Plan, installed: string): void {
  console.log(`Installed inbrain version: ${installed}\n`);
  ...
  if (needsWork === 0) {
    ...
    console.log(`${needsWork} migration(s) need action. Run \`inbrain apply-migrations --yes\` to apply.`);
  }
}

function printDryRun(plan: Plan, installed: string): void {
  console.log(`Dry run — installed inbrain version: ${installed}`);
  console.log('');
...
export async function runApplyMigrations(args: string[]): Promise<void> {
  ...
  // First-install guard (postinstall hook calls us even on `bun add inbrain`
  // before the user has run `inbrain init`). No config = no brain = nothing
  ...
  if (!loadConfig()) {
    if (cli.list) console.log('No brain configured. Run `inbrain init` to set one up.');
    else if (cli.dryRun) console.log('No brain configured (run `inbrain init` first). Nothing to migrate.');
    return;
  }

  // Bug 3 — --force-retry: write an explicit reset marker for a wedged
  // migration, then return. User re-runs `inbrain apply-migrations --yes`
  // to actually re-attempt.
  if (cli.forceRetry) {
    const target = migrations.find(m => m.version === cli.forceRetry);
    if (!target) {
      console.error(`No migration registered with version "${cli.forceRetry}". Run \`inbrain apply-migrations --list\`.`);
      process.exit(2);
    }
    appendCompletedMigration({ version: cli.forceRetry, status: 'retry' });
    console.log(`Wrote 'retry' marker for v${cli.forceRetry}. Run \`inbrain apply-migrations --yes\` to re-attempt.`);
    return;
  }
  ...
  // `inbrain apply-migrations --yes` to actually re-attempt.
  if (cli.forceOrchestrator || cli.forceAll) {
    ...
    if (resetCount === 0) {
      ...
      console.log(`\nReset ${resetCount} wedged orchestrator migration(s). Run \`inbrain apply-migrations --yes\` to re-attempt.`);
    }
  ...
  try {
    ...
    if (cfg) {
      ...
      // phase spawns `inbrain init --migrate-only` as a subprocess (the
      ...
      if (!skipPreflight) {
        ...
        if (schemaVer < LATEST_VERSION) {
          console.warn(
            ...
            `   To run them now: inbrain init --migrate-only\n`,
          );
        }
      }
    }
  ...
  if (plan.wedged.length > 0) {
    for (const m of plan.wedged) {
      console.error(
        ...
        `Check ~/.inbrain/upgrade-errors.jsonl for the last failure reasons, fix the underlying issue, then run:\n` +
        `  inbrain apply-migrations --force-retry ${m.version}\n` +
  ...
  if (cli.specificMigration && plan.applied.length + plan.partial.length + plan.pending.length + plan.skippedFuture.length === 0) {
    console.error(`No migration registered with version "${cli.specificMigration}". Run \`inbrain apply-migrations --list\` to see registered versions.`);
    process.exit(2);
  }
  ...
  for (const m of toRun) {
    ...
    try {
      ...
      if (result.status === 'partial') {
        console.log(`Migration v${m.version} finished as PARTIAL. Re-run \`inbrain apply-migrations --yes\` after resolving any pending host-work items.`);
      } else {
