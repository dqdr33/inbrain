/**
 * inbrain claw-test — end-to-end "fresh user" test harness.
 *
 * Two tiers:
 *   inbrain claw-test                              — scripted (no LLM, CI gate)
 *   inbrain claw-test --live --agent openclaw      — real agent, friction discovery
 *
 * Phases (scripted mode):
 *   setup → install_brain → import → query → extract → verify → render
 *
 * The harness sets INBRAIN_HOME=<tempdir> so the run is hermetic. Each child
 * inbrain invocation runs with --progress-json and the harness captures stderr
...
interface HarnessOpts {
  ...
  listAgents: boolean;
  help: boolean;
  /** Path to the inbrain binary used to invoke child commands. Defaults to argv[0]. */
  inbrainBin?: string;
}
...
export async function runClawTest(args: string[]): Promise<number> {
  ...
  const inbrainHome = runRoot; // configDir() appends '.inbrain' itself
  ...
  try {
    if (opts.live) {
      exitCode = await runLive(opts, scenario, { runId, runRoot, inbrainHome, transcriptPath });
    } else {
      exitCode = await runScripted(opts, scenario, { runId, runRoot, inbrainHome });
    }
  } finally {
  ...
  console.log(`render report:   inbrain friction render --run-id ${runId}`);
 ...
 * contract. The harness runs `inbrain init --pglite` then a sequence of
 ...
 */
const POSTGRES_POLLUTION_ENV_VARS = ['DATABASE_URL', 'INBRAIN_DATABASE_URL'];

async function runScripted(
  opts: HarnessOpts,
  scenario: ScenarioConfig,
  ctx: { runId: string; runRoot: string; inbrainHome: string },
): Promise<number> {
  ...
  const childEnv: Record<string, string> = { INBRAIN_HOME: ctx.inbrainHome, INBRAIN_FRICTION_RUN_ID: ctx.runId };
  for (const [k, v] of Object.entries(parentEnv)) {
    if (v === undefined) continue;
    if (POSTGRES_POLLUTION_ENV_VARS.includes(k)) continue;
    childEnv[k] = v;
  }
  // Re-apply the explicit overrides so a parent INBRAIN_HOME / INBRAIN_FRICTION_RUN_ID
  // can't accidentally win the merge.
  childEnv.INBRAIN_HOME = ctx.inbrainHome;
  childEnv.INBRAIN_FRICTION_RUN_ID = ctx.runId;
  ...
  // inbrain init --pglite doesn't register a fs source).
  ...
  if (scenario.kind === 'upgrade' && scenario.seedRelative) {
    ...
    if (existsSync(seedSql)) {
      const dbPath = join(ctx.inbrainHome, '.inbrain', 'brain.pglite');
      mkdirSync(join(ctx.inbrainHome, '.inbrain'), { recursive: true });
  ...
  for (const phase of phases) {
    const outcome = await invokeGbrain(opts.inbrainBin ?? 'inbrain', phase.argv, ctx.runRoot, childEnv);
    ...
    if (outcome.exitCode !== 0) {
      logFriction({
        runId: ctx.runId,
        phase: phase.name,
        message: `command failed (exit ${outcome.exitCode}): inbrain ${phase.argv.join(' ')}`,
        severity: 'error',
...
async function runLive(
  opts: HarnessOpts,
  scenario: ScenarioConfig,
  ctx: { runId: string; runRoot: string; inbrainHome: string; transcriptPath: string },
): Promise<number> {
  let runner;
  try {
  ...
  const env: Record<string, string> = {
    INBRAIN_HOME: ctx.inbrainHome,
    INBRAIN_FRICTION_RUN_ID: ctx.runId,
  };
...
function parseArgs(args: string[]): HarnessOpts {
  const out: HarnessOpts = {
    ...
    inbrainBin: process.env.INBRAIN_BIN_OVERRIDE || process.execPath,
  };
...
function printHelp() {
  console.log(`inbrain claw-test — end-to-end claw-setup friction harness

Usage:
  inbrain claw-test [--scenario <name>] [--live --agent <name>] [--keep-tempdir]
  inbrain claw-test --list-agents
...
Examples:
  inbrain claw-test --scenario fresh-install
  inbrain claw-test --scenario upgrade-from-v0.18 --keep-tempdir
  inbrain claw-test --live --agent openclaw`);
}
