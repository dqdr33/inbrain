/**
 * `inbrain agent` CLI: the user-facing entry point for the v0.15 subagent
 * runtime.
 *
 *   inbrain agent run <prompt> [flags]
 *   inbrain agent logs <job_id> [--follow] [--since <spec>]
 *
 ...
 * `inbrain jobs work` process. `--follow` tails status until terminal;
 * without `--follow` (or with `--detach`) the CLI prints the job id and
 * exits, leaving the user to check back with `inbrain agent logs`.
 */
...
export async function runAgent(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  switch (sub) {
    ...
    case 'logs':
      ...
      return;
    default:
      console.error(`inbrain agent: unknown subcommand "${sub}"`);
      printHelp();
      process.exit(2);
  }
}
...
function printHelp(): void {
  console.log(`inbrain agent — durable LLM agent runs (v0.15)
...
SUBMITTING
  inbrain agent run <prompt>
    --subagent-def <name>        Named plugin subagent (from INBRAIN_PLUGIN_PATH)
...
function parseRunFlags(args: string[]): { flags: RunFlags; rest: string[] } {
  ...
  while (i < args.length) {
    ...
    switch (a) {
      ...
      default:
        throw new Error(`unknown flag: ${a}. Run \`inbrain agent run --help\` for usage.`);
    }
  }
...
export async function runAgentRun(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!prompt) {
    console.error('inbrain agent run: prompt is required');
    process.exit(2);
  }
...
async function runFanout(engine: BrainEngine, queue: MinionQueue, flags: RunFlags, promptTemplate: string): Promise<void> {
  ...
  try {
    ...
    console.error(`inbrain agent run: invalid --fanout-manifest ${manifestPath}: ${msg}`);
    process.exit(2);
  }

  if (manifest.length === 0) {
    console.error('inbrain agent run: --fanout-manifest is empty; nothing to run');
    process.exit(2);
  }
...
async function followJob(engine: BrainEngine, queue: MinionQueue, jobId: number, timeoutMs?: number): Promise<void> {
  process.stderr.write(`[inbrain agent] following job ${jobId} (Ctrl-C to detach)...\n`);
  ...
  try {
    ...
    try {
      ...
      ac.abort();
      await logsP.catch(() => {});
      process.stderr.write(`[inbrain agent] job ${jobId} terminal: ${job.status}\n`);
      if (job.result != null) process.stdout.write(JSON.stringify(job.result, null, 2) + '\n');
      if (job.status !== 'completed') process.exit(1);
    } catch (e) {
      if (e instanceof TimeoutError) {
        process.stderr.write(`[inbrain agent] timeout after ${e.elapsedMs}ms — job is still running. Check with: inbrain jobs get ${jobId}\n`);
        process.exit(3);
      }
      throw e;
    }
...
async function runAgentLogsCmd(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!jobIdStr) {
    console.error('inbrain agent logs: <job_id> is required');
    process.exit(2);
  }
  const jobId = parseInt(jobIdStr, 10);
  if (!Number.isFinite(jobId) || jobId <= 0) {
    console.error(`inbrain agent logs: "${jobIdStr}" is not a valid job id`);
    process.exit(2);
  }
