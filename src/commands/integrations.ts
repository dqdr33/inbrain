/**
 * inbrain integrations — standalone CLI command for recipe discovery and health.
 *
 * NOT an operation (no database connection needed).
 * Reads embedded recipe files and heartbeat JSONL from ~/.inbrain/integrations/.
 *
 * ARCHITECTURE:
 ...
 *     └── (bare)  → dashboard view
 *
 *   ~/.inbrain/integrations/<id>/heartbeat.jsonl
 ...
 * legacy path that writes to ~/.inbrain/skills/). 'copy-into-host-repo'
 * recipes write their bundle into the operator's host agent repo via the
 * `inbrain integrations install` subcommand.
 */
...
export async function executeHealthCheck(
  ...
  switch (check.type) {
    ...
    case 'command': {
      ...
      // (from $INBRAIN_RECIPES_DIR or ./recipes) must NOT be able to spawn arbitrary binaries.
      if (!isEmbedded) {
...
//   TRUSTED (embedded=true):  package-bundled recipes shipped with inbrain
//     - source install: ../../recipes relative to this file
//     - global install: ~/.bun/install/global/node_modules/inbrain/recipes
...
export function getRecipeDirs(): Array<{ dir: string; trusted: boolean }> {
  ...
  const globalDir = join(homedir(), '.bun', 'install', 'global', 'node_modules', 'inbrain', 'recipes');
  if (existsSync(globalDir)) dirs.push({ dir: globalDir, trusted: true });
  if (process.env.INBRAIN_RECIPES_DIR && existsSync(process.env.INBRAIN_RECIPES_DIR)) {
    dirs.push({ dir: process.env.INBRAIN_RECIPES_DIR, trusted: false });
  }
...
function heartbeatDir(id: string): string {
  return inbrainPath('integrations', id);
}
...
function cmdList(args: string[]): void {
  ...
  console.log("\n  Run 'inbrain integrations show <id>' for setup details.");
  console.log('');
}
...
function cmdShow(args: string[]): void {
  ...
  if (!id) {
    console.error('Usage: inbrain integrations show <recipe-id>');
    return;
  }
...
function cmdStatus(args: string[]): void {
  ...
  const id = args.find(a => !a.startsWith('-'));
  if (!id) {
    console.error('Usage: inbrain integrations status <recipe-id>');
    return;
  }
  ...
  if (heartbeat.length > 0) {
    ...
    if (ageMs > 24 * 60 * 60 * 1000) {
      ...
      console.log('  Run: inbrain integrations doctor');
    }
  } else {
...
function cmdTest(args: string[]): void {
  ...
  if (!filePath) {
    console.error('Usage: inbrain integrations test <recipe-file.md>');
    return;
  }
...
function printHelp(): void {
  console.log(`inbrain integrations — manage integration recipes

USAGE
  inbrain integrations                  Show integration dashboard
  inbrain integrations list [--json]    List available integrations
  inbrain integrations show <id>        Show recipe details
  inbrain integrations status <id>      Check secrets + health
  inbrain integrations doctor [--json]  Run health checks
  inbrain integrations stats [--json]   Show signal statistics
  inbrain integrations test <file>      Validate a recipe file
`);
}
...
// =============================================================================
// `inbrain integrations install <recipe-id>` — copy-into-host-repo path.
//
...
// <target>/services/voice-agent/.inbrain-source.json so future --refresh calls
...
//   - Must NOT be inbrain itself OR a parent of inbrain.
 ...
 *   - Not inbrain itself; not a parent of inbrain
...
function validateTargetRepo(
  ...
  // Refuse if target is inbrain itself or contains inbrain.
  let inbrainRoot: string | null = null;
  try {
    inbrainRoot = realpathSync(pathResolve(__dirname, '..', '..'));
  } catch {
    // ignore — non-fatal
  }
  if (inbrainRoot && (resolvedTarget === inbrainRoot || inbrainRoot.startsWith(resolvedTarget + '/'))) {
    return `refusing to install into inbrain itself (or a parent dir): ${resolvedTarget}`;
  }
...
function classifyForRefresh(
  ...
  for (const entry of manifestEntries) {
    ...
    try {
      ...
      // src missing? Skip — this would mean a manifest pointing at a missing file in inbrain.
      continue;
    }
...
function appendRefreshLog(targetVoiceAgentDir: string, event: object) {
  try {
    const logPath = pathResolve(targetVoiceAgentDir, '.inbrain-source.refresh.log');
...
/**
 * Refresh mode: read `.inbrain-source.json`, classify, apply decisions.
 */
async function refreshRecipeIntoHostRepo(
  ...
  if (!existsSync(sourceFilePath)) {
    throw new Error(`.inbrain-source.json not found at ${sourceFilePath} — this target was never installed via copy-into-host-repo; run without --refresh first`);
  }
  ...
  try {
    ...
    throw new Error(`failed to parse .inbrain-source.json: ${(err as Error).message}`);
  }

  if (recorded.recipe !== recipeId) {
    throw new Error(`.inbrain-source.json recipe="${recorded.recipe}" does not match requested recipe="${recipeId}"`);
  }
  ...
  for (const c of classifications) {
    ...
    switch (c.state) {
      ...
      case 'source-deleted': {
        // inbrain reference removed this file; offer cleanup with --auto take-theirs.
  ...
  // Re-write .inbrain-source.json with the updated SHAs.
  const inbrainVersion = (() => {
    try {
  ...
  const updatedRecord: GbrainSourceJson = {
    recipe: recipeId,
    inbrain_version: inbrainVersion,
...
export async function installRecipeIntoHostRepo(
  ...
  // Write the .inbrain-source.json manifest into the target repo.
  ...
  const inbrainSource: GbrainSourceJson = {
    recipe: recipeId,
    inbrain_version: inbrainVersion,
    install_kind: 'copy-into-host-repo',
    copied_at: new Date().toISOString(),
  ...
  writeFileSync(sourceFilePath, JSON.stringify(inbrainSource, null, 2) + '\n');
  ...
  if (manifest.resolver_rows_to_append && manifest.resolver_rows_to_append.length > 0) {
    ...
    if (resolverPath) {
      const rowsBlock = `\n\n<!-- inbrain:agent-voice:resolver-rows -->\n` +
...
async function cmdInstall(args: string[]): Promise<void> {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--target' || arg === '-t') {
      ...
      console.log('Usage:');
      console.log('  inbrain integrations install <recipe-id> --target <host-repo-path> [--overwrite] [--dry-run]');
      console.log('  inbrain integrations install <recipe-id> --target <host-repo-path> --refresh [--auto keep-mine|take-theirs] [--dry-run]');
      return;
  ...
  if (!recipeId) {
    console.error('Usage: inbrain integrations install <recipe-id> --target <host-repo-path>');
    process.exit(2);
  }
  if (!opts.target) {
