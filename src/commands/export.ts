...
export async function runExport(engine: BrainEngine, args: string[]) {
  ...
  if (restoreOnly && !repoPath) {
    ...
    if (!repoPath) {
      console.error(
        `Error: inbrain export --restore-only requires --repo <path> or a configured\n` +
          `default source with a local_path. Run \`inbrain sources list\` to inspect\n` +
  ...
  if (restoreOnly && !storageConfig) {
    console.error(
      `Error: inbrain export --restore-only requires a storage tiering config\n` +
        `(inbrain.yml with a "storage:" section) at ${repoPath}/inbrain.yml.\n` +
        `Without it, there's nothing to scope the restore to.\n` +
        `Run \`inbrain storage status\` to inspect the current configuration.`,
    );
    process.exit(1);
  }
