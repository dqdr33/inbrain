/**
 * v0.21.0 Cathedral II Layer 13 (E2) — `inbrain reindex-code`.
 *
 ...
 * The nudge surfaces this from `inbrain reindex --code` on dry-run AND
...
function printCodeModelNudge(decision: Extract<NudgeDecision, { shouldNudge: true }>): void {
  process.stderr.write(
    `[reindex-code] Configured embedding model is \`${decision.currentModel}\`. For pure code retrieval, Voyage's code-tuned \`voyage-code-3\` typically outperforms general-purpose models. Switch:\n` +
      `  inbrain config set embedding_model ${decision.recommendedModel}\n` +
      `  inbrain config set embedding_dimensions 1024\n` +
      `Suppress with INBRAIN_NO_CODE_MODEL_NUDGE=1.\n`,
  );
}
  ...
  for (const flag of ['--max-cost', '--max-cost-usd']) {
    ...
    if (idx >= 0) {
      ...
      if (!Number.isFinite(n) || n <= 0) {
        console.error(`inbrain reindex --code: ${flag} requires a positive number in USD (got ${v ?? '(missing)'})`);
        process.exit(2);
      }
      maxCostUsd = n;
      break;
