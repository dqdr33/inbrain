/**
 * inbrain extract-conversation-facts — batch fact extraction for
 ...
 * claim with its own embedding and entity linkage, and `inbrain search`
...
async function processPage(
  ...
  for (const seg of segments) {
    ...
    if (!state.dryRun && extracted.length > 0) {
      ...
      try {
        const ins = await state.engine.insertFacts(rows, { source_id: state.sourceId }); // inbrain-allow-direct-insert: canonical bulk extraction path for conversation pages — fences-as-system-of-record doesn't apply because conversations don't carry `## Facts` fences (the chat-log shape is the source-of-truth)
...
async function writeTerminalAuditRow(
  ...
  await engine.insertFacts([fact], { source_id: sourceId }); // inbrain-allow-direct-insert: page-level TERMINAL audit row (Codex C7 / E16) marks extraction completion in the durable facts table — there's no fence equivalent because this is internal audit state, not user-facing knowledge
}
...
const HELP = `Usage: inbrain extract-conversation-facts [options]
...
Options:
  ...
  --background           Submit as a Minion job; print job_id; exit (use 'inbrain jobs follow').
  --yes                  Auto-confirm cost preview in non-TTY contexts.
  --help, -h             Show this help.

Multi-source: when --source-id is omitted, the command iterates ALL
sources from inbrain sources list. Per-source budget cap defaults to
...
in the facts table (source='${TERMINAL_AUDIT_SOURCE}'). inbrain doctor's
...
function buildJobParams(args: string[]): Record<string, unknown> {
  ...
  return {
    ...
    // so `inbrain extract-conversation-facts --background --workers 20`
