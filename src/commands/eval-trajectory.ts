/**
 * inbrain eval trajectory <entity-slug> — chronological typed-claim
 * trajectory + regression detection + narrative drift score.
 *
 * v0.35.4 (T6) — pure data fn + JSON formatter + human formatter +
 * thin-client routing seam. Mirrors `inbrain salience` / `inbrain anomalies`
 * shape so the four temporal-axis read CLIs feel consistent.
 *
 * Usage:
 *   inbrain eval trajectory companies/acme-example
 *   inbrain eval trajectory companies/acme-example --metric mrr
 *   inbrain eval trajectory companies/acme-example --since 2026-01-01 --until 2026-07-31
 *   inbrain eval trajectory companies/acme-example --json
 */
...
const HELP = `Usage: inbrain eval trajectory <entity-slug> [options]
