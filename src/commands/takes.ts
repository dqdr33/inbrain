...
async function resolveBrainDir(engine: BrainEngine | null, explicitDir: string | null): Promise<string> {
  ...
  console.error('No brain directory configured. Pass --dir <path> or run `inbrain init` first.');
  process.exit(1);
}
...
async function getPageId(engine: BrainEngine, slug: string): Promise<number> {
  ...
  if (!rows[0]) {
    console.error(`Page not found in brain: ${slug}. Run \`inbrain sync\` first.`);
    process.exit(1);
  }
  return rows[0].id;
}
...
async function cmdList(engine: BrainEngine, args: string[]): Promise<void> {
  const slug = args[0];
  if (!slug) {
    console.error('Usage: inbrain takes <slug> [--json]');
    process.exit(1);
  }
...
async function cmdSearch(engine: BrainEngine, args: string[]): Promise<void> {
  const query = args[0];
  if (!query) {
    console.error('Usage: inbrain takes search "<query>" [--who h] [--json]');
    process.exit(1);
  }
  const json = flagPresent(args, '--json');
...
async function cmdAdd(engine: BrainEngine, args: string[]): Promise<void> {
  const slug = args[0];
  if (!slug) {
    console.error('Usage: inbrain takes add <slug> --claim "..." --kind <k> --who <h> [--weight 0.5] [--source "..."] [--since YYYY-MM]');
    process.exit(1);
  }
...
async function cmdUpdate(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!slug || !rowNumStr) {
    console.error('Usage: inbrain takes update <slug> --row N [--weight 0.7] [--source "..."] [--since YYYY-MM]');
    process.exit(1);
  }
  ...
  await withPageLock(slug, async () => {
    ...
    if (!target) {
      console.warn(`[takes update] DB updated but row #${rowNum} not in markdown fence on disk; markdown may be out of sync. Run 'inbrain extract takes --slugs ${slug}' to reconcile.`);
      return;
    }
...
async function cmdSupersede(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  const rowNumStr = flagValue(args, '--row');
  if (!slug || !rowNumStr) {
    console.error('Usage: inbrain takes supersede <slug> --row N --claim "..." [--kind k] [--who h] [--weight 0.5] [--source "..."]');
    process.exit(1);
  }
  const rowNum = parseInt(rowNumStr, 10);
  const claim = flagValue(args, '--claim');
...
async function cmdResolve(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!slug || !rowNumStr || (!qualityStr && !outcomeStr)) {
    console.error('Usage: inbrain takes resolve <slug> --row N --quality correct|incorrect|partial|unresolvable [--evidence "..."] [--value N --unit usd|pct|count] [--by <slug>]');
    console.error('       (back-compat) inbrain takes resolve <slug> --row N --outcome true|false [...]');
    process.exit(1);
  }
  ...
  await withPageLock(slug, async () => {
    ...
    const target = parsed.takes.find(t => t.rowNum === rowNum);
    if (!target) {
      console.warn(`[takes resolve] DB updated but row #${rowNum} not in markdown fence; run 'inbrain extract takes --slugs ${slug}' to reconcile.`);
      return;
    }
...
export async function runTakes(engine: BrainEngine, args: string[]): Promise<void> {
  if (args.length === 0 || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: inbrain takes <subcommand> [options]
...
/**
 * v0.41.18.0 (A12, A24, T9) — `inbrain takes extract --from-pages` runs
...
async function cmdExtract(engine: BrainEngine, rest: string[]): Promise<void> {
  ...
  if (sub !== '--from-pages') {
    process.stderr.write(
      'Usage: inbrain takes extract --from-pages [--yes] [--dry-run] [--source-id <id>] [--max-pages N] [--holder <name>]\n',
    );
    process.exit(1);
  }
...
/**
 * v0.36.1.0 (TD4 / D30) — `inbrain takes revisit <slug>` opens $EDITOR on
...
async function cmdRevisit(_engine: BrainEngine, rest: string[]): Promise<void> {
  ...
  if (!repoPath) {
    process.stderr.write('No brain repo configured. Run `inbrain config set sync.repo_path /path/to/brain`.\n');
    process.exit(1);
  }
  ...
  const marker = '\n<!-- inbrain:revisit -->\n';
  if (!existing.includes('<!-- inbrain:revisit -->')) {
