 ...
 *
 * Source resolution: honors the full chain (incl. the `.inbrain-source` pin)
...
export async function runCodeCallees(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!sym) {
    const err = errorFor({
      ...
      hint: 'inbrain code-callees <symbol> [--source S | --all-sources] [--limit N] [--json]',
    });
  ...
  // Full source-resolution chain (honors .inbrain-source pin, env, local_path,
