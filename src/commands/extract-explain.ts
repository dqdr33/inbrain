/**
 * v0.42 Wave D2 — `inbrain extract --explain <kind>` CLI.
 *
...
export async function runExtractExplain(
  ...
  if (!kindArg || kindArg.startsWith('--')) {
    console.error('Usage: inbrain extract --explain <kind>');
    process.exit(2);
  }
  ...
  if (!pack) {
    if (json) {
      ...
      console.log('');
      console.log('Configure a pack with: inbrain schema use <pack-name>');
    }
    return;
  }
  ...
  if (!spec && !cyclePhaseKinds.has(kindArg)) {
    ...
    if (json) {
      ...
      console.log('');
      console.log('Built-in kinds (shipped by inbrain cycle phases):');
      for (const k of Array.from(cyclePhaseKinds).sort()) console.log(`  ${k}`);
      console.log('');
      console.log(`To declare it: inbrain schema scaffold-extractable ${kindArg} --pack ${pack.manifest.name}`);
    }
    return;
  }
