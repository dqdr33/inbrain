/**
 * inbrain bench publish — turn a captured-run NDJSON into a baseline file (v0.41).
 *
 ...
 * of `inbrain eval export` and writes a `*.baseline.ndjson` file with:
 ...
 *
 * Usage:
 *   inbrain bench publish --from <captured.ndjson> --to <X.baseline.ndjson>
...
function printHelp(): void {
  console.log(`inbrain bench publish — write a baseline file from captured queries

Usage:
  inbrain bench publish --from <captured.ndjson> --to <X.baseline.ndjson> [flags]

Required:
  --from FILE                    Input NDJSON from \`inbrain eval export\`
...
Examples:
  ...
  inbrain bench publish --from /tmp/captured.ndjson --to ~/.inbrain/baselines/personal.baseline.ndjson --label "personal-2026-05"
