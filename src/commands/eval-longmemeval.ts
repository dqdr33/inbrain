/**
 * v0.28.1: `inbrain eval longmemeval <dataset.jsonl>` — public LongMemEval
 ...
 * is invoked, so the user's ~/.inbrain brain is never opened. Tests stub
 ...
 * "inbrain + Haiku-preprocess pipeline" vs "inbrain alone", not directly
...
function printHelp(): void {
  process.stderr.write(
    `inbrain eval longmemeval <dataset.jsonl> [options]\n\n` +
    `Run the LongMemEval benchmark against inbrain's hybrid retrieval. Spins up an\n` +
...
export interface RunOpts {
   ...
   * one inside this call. Production callers (the inbrain CLI) leave this
...
export async function runEvalLongMemEval(args: string[], runOpts: RunOpts = {}): Promise<void> {
  ...
  const work = async (engine: PGLiteEngine): Promise<void> => {
    ...
    for (const q of questions) {
      ...
      if (process.env.INBRAIN_LME_DEBUG === '1') {
