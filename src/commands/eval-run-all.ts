/**
 * v0.32.3 — `inbrain eval run-all` orchestrator.
 *
 * Sweeps every requested mode × suite combination and writes per-run
 * results to `<repo>/.inbrain-evals/eval-results.jsonl` [CDX-23]. Personal
 * brain (~/.inbrain) is never touched; the repo's git history is the
 * audit trail.
 *
...
function printHelp(): void {
  process.stderr.write(
    `inbrain eval run-all [flags]\n\n` +
    `Sweeps every requested search-lite mode × eval suite. Writes per-run results to\n` +
    `<repo>/.inbrain-evals/eval-results.jsonl. Personal brain is never touched.\n\n` +
    `Flags:\n` +
    ...
    `  --output DIR                  Override .inbrain-evals/ location.\n` +
...
function evalResultsPath(repoRoot: string, outputDirOverride?: string): string {
  ...
  return join(repoRoot, '.inbrain-evals', 'eval-results.jsonl');
}
...
export async function runEvalRunAll(_engine: BrainEngine | null, args: string[]): Promise<void> {
  ...
  // responsibility today — `inbrain eval run-all` is the orchestrator's
  // shape + cost guard + audit trail. The per-suite per-mode calls land
  // as a follow-up: each suite's CLI is already exposed (inbrain eval
  // longmemeval --mode X, inbrain eval replay --mode X), so wiring them
