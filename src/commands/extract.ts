 ...
 *
 * inbrain extract — Extract links and timeline entries from brain content.
 *
 * Two data sources:
 ...
 *
 * Subcommands:
 *   inbrain extract links    [--source fs|db] [--dir <brain>] [--dry-run] [--json] [--type T] [--since DATE]
 *   inbrain extract timeline [--source fs|db] [--dir <brain>] [--dry-run] [--json] [--type T] [--since DATE]
 *   inbrain extract all      [--source fs|db] [--dir <brain>] [--dry-run] [--json] [--type T] [--since DATE]
 *
...
// Normal pages are KBs; raise via INBRAIN_EXTRACT_STALE_BATCH for throughput.
const STALE_BATCH_SIZE = Math.max(1, Number(process.env.INBRAIN_EXTRACT_STALE_BATCH) || 25);
...
const STALE_TIME_BUDGET_MS = Math.max(1000, Number(process.env.INBRAIN_EXTRACT_TIME_BUDGET_MS) || 30 * 60 * 1000);
...
export async function extractLinksFromFile(
  ...
  for (const { name, relTarget } of extractMarkdownLinks(scanContent)) {
    ...
    // `wikilink_basename` so users can audit via `inbrain graph-query
...
export async function runExtractCore(engine: BrainEngine, opts: ExtractOpts): Promise<ExtractResult> {
  ...
  // Full walk path: CLI `inbrain extract` or first-run.
...
export async function runExtract(engine: BrainEngine, args: string[]) {
  ...
  //
  //   inbrain extract status [--source-id ID] [--kind X] [--run-id Y] [--json]
  //   inbrain extract benchmark --pack X --kind Y [--json]
  //   inbrain extract --explain <kind>
  ...
  // v0.42.7 (#1696): `inbrain extract --stale` — incremental link+timeline sweep
  // over pages whose links_extracted_at watermark is stale. Intercepts BEFORE
  // the links|timeline|all subcommand validation so `inbrain extract --stale`
  // works with no subcommand (and `inbrain extract all --stale` too). DB-source
  ...
  // a footgun: a user who runs `inbrain extract links` from anywhere outside
  ...
  // match what `inbrain sync` already does, and keeps cwd-cwd usage available
  // via explicit `--dir .`.
  ...
  if (!subcommand || !['links', 'timeline', 'all'].includes(subcommand)) {
    console.error(`Usage: inbrain extract <subcommand> [flags]

Extraction (existing):
  inbrain extract links    [--source fs|db] [--source-id <id>] [--dir <brain-dir>] [--dry-run] [--json] [--type T] [--since DATE] [--workers N]
  inbrain extract timeline [--source fs|db] [--source-id <id>] [--dir <brain-dir>] [--dry-run] [--json] [--type T] [--since DATE] [--workers N]
  inbrain extract all      [--source fs|db] [--source-id <id>] [--dir <brain-dir>] [--dry-run] [--json] [--type T] [--since DATE] [--workers N]
  inbrain extract <links|timeline> --by-mention --source db
  inbrain extract <links|timeline|all> --ner --source db
  inbrain extract <timeline|all> --from-meetings

Incremental sweep (v0.42.7):
  inbrain extract --stale [--source-id <id>] [--catch-up] [--dry-run] [--json]
...
Status (v0.42):
  ...
  if (byMention && source === 'fs') {
    console.error(
      ...
      `  inbrain extract ${subcommand} --by-mention --source db` +
  ...
  if (byMention && subcommand === 'timeline') {
    console.error(
      ...
      `Re-run as 'inbrain extract links --by-mention' or 'inbrain extract all --by-mention'.`,
    );
    process.exit(2);
  }
  ...
  if (ner && source === 'fs') {
    console.error(
      ...
      `  inbrain extract ${subcommand} --ner --source db` +
      (sourceIdFilter ? ` --source-id ${sourceIdFilter}` : '') +
      (since ? ` --since ${since}` : '') +
      (dryRun ? ' --dry-run' : '') + '\n',
    );
  ...
  if (fromMeetings && source === 'fs') {
    console.error(
      ...
      `  inbrain extract timeline --from-meetings --source db` +
      (sourceIdFilter ? ` --source-id ${sourceIdFilter}` : '') +
      (dryRun ? ' --dry-run' : '') + '\n',
    );
    process.exit(2);
  ...
  if (fromMeetings && subcommand !== 'timeline' && subcommand !== 'all') {
    console.error(
      `--from-meetings is a timeline-pass only. Re-run as 'inbrain extract timeline --from-meetings' or 'inbrain extract all --from-meetings'.`,
    );
    process.exit(2);
  }

  // FS source needs a brain dir. When --dir wasn't passed, resolve from
  // sources(local_path) — same path `inbrain sync` uses — instead of
  ...
  if (source === 'fs' && !explicitDir) {
    ...
    if (configured) {
      ...
      console.error(
        ...
        `run: inbrain sources add default --path <brain-dir>`,
      );
      process.exit(1);
    }
  }
...
async function extractForSlugs(
  ...
  async function flushLinks() {
    ...
    try {
      ...
      linksCreated += await engine.addLinksBatch(snapshot, { auditSite: 'extract.links_inc' }); // inbrain-allow-direct-insert: inbrain extract command — canonical link reconciliation from markdown body
    } catch (e) {
...
async function extractLinksFromDir(
  ...
  async function flush() {
    ...
    batch.length = 0;
    try {
      created += await engine.addLinksBatch(snapshot, { auditSite: 'extract.links_fs' }); // inbrain-allow-direct-insert: inbrain extract command — canonical link reconciliation from markdown body
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      if (jsonMode) {
  ...
  for (const slug of slugs) {
    ...
    try {
      ...
      for (const link of await extractLinksFromFile(content, slug + '.md', allSlugs, { globalBasename })) {
        try { await engine.addLink(link.from_slug, link.to_slug, link.context, link.link_type, link.link_source, undefined, undefined, linkOpts); created++; } catch { /* skip */ } // inbrain-allow-direct-insert: inbrain extract single-row fallback when batch path declines a row
      }
    } catch { /* skip */ }
  }
  return created;
...
export async function extractTimelineForSlugs(
  ...
  for (const slug of slugs) {
    ...
    try {
      const content = readFileSync(filePath, 'utf-8');
      for (const entry of extractTimelineFromContent(content, slug)) {
        try { await engine.addTimelineEntry(entry.slug, { date: entry.date, source: entry.source, summary: entry.summary, detail: entry.detail }, entryOpts); created++; } catch { /* skip */ } // inbrain-allow-direct-insert: inbrain extract single-row fallback for timeline entries
      }
    } catch { /* skip */ }
  }
  return created;
...
async function extractLinksFromDB(
  ...
  // `inbrain extract links --source db`). Only stamp when the caller ran BOTH
  ...
  // the loop so a manual `inbrain extract links|all --source db` clears the
  ...
  async function flush() {
    ...
    batch.length = 0;
    try {
      created += await engine.addLinksBatch(snapshot, { auditSite: 'extract.links_db' }); // inbrain-allow-direct-insert: inbrain extract command — canonical link reconciliation from markdown body
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      if (jsonMode) {
        process.stderr.write(JSON.stringify({ event: 'batch_error', size: snapshot.length, error: msg }) + '\n');
  ...
  for (const { slug, source_id } of allRefs) {
    ...
    // user-invoked `inbrain extract links` stays outgoing-only.
    // Issue #972: globalBasename routes bare `[[name]]` wikilinks through
...
/**
 * v0.42.7 (#1696) — `inbrain extract --stale`: incremental link + timeline
...
async function extractStaleFromDB(
  ...
  for (;;) {
    ...
    for (let i = 0; i < linkRows.length; i += BATCH_SIZE) {
      linksCreated += await engine.addLinksBatch(linkRows.slice(i, i + BATCH_SIZE), { auditSite: 'extract.stale' }); // inbrain-allow-direct-insert: inbrain extract --stale — canonical link reconciliation from markdown body
    }
  ...
  if (!jsonMode) {
    ...
    if (budgetHit && staleRemaining > 0) {
      console.log(`Time budget reached — ${staleRemaining} page(s) still stale. Re-run 'inbrain extract --stale' (or pass --catch-up) to continue.`);
    }
  } else {
    process.stdout.write(JSON.stringify({
...
async function extractMentionsFromDb(
  ...
  async function flushBatch() {
    if (batch.length === 0) return;
    try {
      created += await engine.addLinksBatch(batch, { auditSite: 'extract.by_mention' }); // inbrain-allow-direct-insert: inbrain extract --by-mention — canonical auto-link write from body-text mention scan
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      if (jsonMode) {
