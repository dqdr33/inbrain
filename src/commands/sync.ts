...
function resolveSyncCheckpointEvery(): number {
  const raw = process.env.INBRAIN_SYNC_CHECKPOINT_EVERY;
...
function resolveSyncCheckpointSeconds(): number {
  const raw = process.env.INBRAIN_SYNC_CHECKPOINT_SECONDS;
...
function resolveSyncMaxCheckpointFailures(): number {
  const raw = process.env.INBRAIN_SYNC_MAX_CHECKPOINT_FAILURES;
...
function resolveSyncYieldEvery(): number {
  const raw = process.env.INBRAIN_SYNC_YIELD_EVERY;
...
export interface SyncOpts {
  ...
  concurrency?: number;
  /**
   * Internal: skip acquiring the inbrain-sync DB lock. Set by the cycle
   * handler (cycle.ts) which already holds inbrain-cycle and therefore
   ...
   *
   * Defaults to:
   *   - `inbrain-sync:<sourceId>` when `opts.sourceId` is set
   *     (multi-source / federated brains; the per-source invariant)
   *   - `inbrain-sync` (the legacy global lock) when `sourceId` is unset
   ...
   *   lock fixes — `inbrain sync --all` (per-source lock) running
   *   concurrently with `inbrain sync --source foo` (global lock) would
...
/**
 * v0.40 Federated Sync v2: `inbrain sync trigger --source <id> [--priority high|normal|low]`
 *
 ...
 * a manual git pull, scripted dispatch from `inbrain sources federate`.
 *
...
export async function runSyncTrigger(engine: BrainEngine, args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: inbrain sync trigger --source <id> [--priority high|normal|low]
...
Use cases:
  - GitHub webhook → 'inbrain sync trigger --source <repo>'
  - Manual nudge after 'git pull' inside a federated source
  - Programmatic triggers from CI / shell automation

See also:
  inbrain sources webhook set <id>   Set up GitHub-signed push webhook
  inbrain sources status             Per-source sync + embed coverage
`);
    return;
  }
  ...
  if (!sourceIdArg) {
    ...
    console.error("Usage: inbrain sync trigger --source <id> [--priority high|normal|low]");
    process.exit(2);
  }
  ...
  if (!source) {
    console.error(`Source "${sourceIdArg}" not found. List with: inbrain sources list`);
    process.exit(1);
  }
...
export async function performSync(engine: BrainEngine, opts: SyncOpts): Promise<SyncResult> {
  ...
  // mechanism (e.g. cycle.ts holds inbrain-cycle for the broader scope).
  if (opts.skipLock) {
  ...
  // bare `inbrain sync` path (no --source/--lockId) included. The pre-v0.42 code
...
async function formatLockBusyMessage(engine: BrainEngine, lockKey: string): Promise<string> {
  ...
  if (!snap) {
    return (
      ...
      `Wait for it to finish, or run 'inbrain doctor' if it has been more than 30 minutes.`
    );
  }

  const ageHuman = formatAgeHuman(snap.age_ms);
  const breakHint = lockKey.startsWith('inbrain-sync:')
    ? `inbrain sync --break-lock --source ${lockKey.slice('inbrain-sync:'.length)}`
    : `inbrain sync --break-lock`;
...
/**
 * v0.41.6.0 D3: `inbrain sync --break-lock` / `--force-break-lock` worker.
...
async function runBreakLock(
  ...
  if (opts.maxAgeSeconds !== undefined && !opts.force) {
    ...
    if (opts.json) {
      ...
      if (snap.last_refreshed_at === null) {
        ...
        console.error('Run `inbrain apply-migrations --yes` to land v98, OR use --force-break-lock if you know the holder is dead.');
      } else {
  ...
  if (opts.force) {
    ...
    if (opts.json) {
      ...
      console.log('WARNING: the holder may still be writing. Verify with `inbrain doctor` before re-running.');
    } else {
...
async function performSyncInner(engine: BrainEngine, opts: SyncOpts): Promise<SyncResult> {
  ...
  // impossible to triage. Mirror the existing `[inbrain phase] sync.git_pull`
  ...
  serr(`[inbrain phase] sync.resolve_repo`);
  // Resolve repo path
  const repoPath = opts.repoPath || await readSyncAnchor(engine, opts.sourceId, 'repo_path');
  if (!repoPath) {
    const hint = opts.sourceId
      ? `Source "${opts.sourceId}" has no local_path. Run: inbrain sources add ${opts.sourceId} --path <path>`
      : `No repo path specified. Use --repo or run inbrain init with --repo first.`;
    throw new Error(hint);
  }

  serr(`[inbrain phase] sync.load_active_pack`);
  ...
  if (opts.sourceId) {
    serr(`[inbrain phase] sync.validate_repo_state`);
    ...
    if (remoteUrl) {
      ...
      switch (state) {
        ...
        case 'no-git':
        case 'not-a-dir':
          // #1881: only re-clone a clone inbrain owns. An unowned local_path
          // (the user's working tree) is refused loudly, never deleted.
          if (!isOwnedClone(ownSrc)) {
            throw new Error(unownedHint(ownSrc, state));
          }
          serr(
            `[inbrain] auto-recovery: re-cloning "${opts.sourceId}" (clone state: ${state}).`,
          );
        ...
        case 'corrupted':
          throw new Error(
            `Source "${opts.sourceId}" clone at ${repoPath} is corrupted ` +
              ...
              `inbrain sources remove ${opts.sourceId} --confirm-destructive && ` +
              `inbrain sources add ${opts.sourceId} --url ${remoteUrl}`,
          );
        case 'url-drift':
          throw new Error(
            `Source "${opts.sourceId}" clone at ${repoPath} has a remote ` +
              `that differs from config.remote_url=${remoteUrl}. ` +
              `Re-clone with: inbrain sources rebase-clone ${opts.sourceId} ` +
  ...
  if (!existsSync(join(repoPath, '.git'))) {
    throw new Error(`Not a git repository: ${repoPath}. Inbrain sync requires a git-initialized repo.`);
  }

  serr(`[inbrain phase] sync.detect_head`);
  ...
  if (!opts.noPull && !detachedHead && originRemotePresent) {
    const _t0 = Date.now();
    serr(`[inbrain phase] sync.git_pull start`);
    try {
      ...
      // failure.
      pullRepo(repoPath);
      serr(`[inbrain phase] sync.git_pull done ${Date.now() - _t0}ms`);
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : String(e);
      serr(`[inbrain phase] sync.git_pull error ${Date.now() - _t0}ms (${msg.slice(0, 80)})`);
  ...
  // pages had been indexed by an older inbrain version (or via direct
  ...
  // page requires `inbrain pages purge-deleted` or a direct MCP delete.
  // Filed as v0.42+ follow-up for a `inbrain pages remove <slug>` surface.
  ...
  const partial = async (reason: 'timeout' | 'pull_timeout'): Promise<SyncResult> => {
    ...
    serr(
      `[sync] banked ${banked} file(s) this run; next 'inbrain sync' resumes from ` +
  ...
  // v0.42.x (#1794, 4A): clamp the worker fan-out under INBRAIN_MAX_CONNECTIONS
  ...
  if (maxConnections !== undefined && engine.kind !== 'pglite') {
    ...
    if (clampResult.clamped) {
      serr(
        `  [sync] INBRAIN_MAX_CONNECTIONS=${maxConnections}: clamped workers ` +
  ...
  if (importsToDo.length > 0) {
    ...
    async function importOnePath(eng: BrainEngine, path: string): Promise<void> {
      ...
      // (INBRAIN_SYNC_TRACE=1) to avoid a line per file on huge brains. serr is
      ...
      if (process.env.INBRAIN_SYNC_TRACE) serr(`[sync] begin import: ${path}`);
      try {
  ...
  if (!gate.advanced) {
    ...
    if (gate.sentinelBlocked) {
      ...
      serr(
        ...
        `${codeBreakdown}\n\n` +
        `Fix the frontmatter and re-run, or use 'inbrain sync --skip-failed' to ` +
    ...
    serr(
      `[sync] banked ${bankedFiles} file(s) this run; next 'inbrain sync' resumes ` +
  ...
  if (gate.autoSkipped.length > 0) {
    serr(
      ...
      `sync-failures.jsonl. 'inbrain doctor' will warn until they're fixed.`,
    );
  }
  ...
  if (!opts.noExtract && totalChanges > 100 && pagesAffected.length > 0) {
    slog(
      ...
      `Run 'inbrain extract --stale${opts.sourceId ? ` --source-id ${opts.sourceId}` : ''}' ` +
  ...
  if (!noEmbed && pagesAffected.length > 0 && pagesAffected.length <= 100) {
    try {
      ...
      if (e instanceof EmbeddingDimMismatchError) {
        ...
        serr(`run 'inbrain embed --stale' after fixing the schema.\n`);
      }
      // Other errors stay best-effort — rate limits, transient network.
    }
  } else if (noEmbed || totalChanges > 100) {
    slog(`Text imported. Run 'inbrain embed --stale' to generate embeddings.`);
  }
...
async function performFullSync(
  ...
  // default-markdown `isSyncable(rel)`, so `inbrain sync --strategy
  ...
  serr(`[inbrain phase] sync.fullsync.import start strategy=${opts.strategy ?? 'markdown'}`);
  ...
  serr(
    `[inbrain phase] sync.fullsync.import done ${Date.now() - _fullImportT0}ms ` +
  ...
  if (fullGate.autoSkipped.length > 0) {
    serr(
      ...
      `'inbrain doctor' will warn until they're fixed.`,
    );
  }
...
/**
 * Resolve the out-of-band hard-deadline for a `inbrain sync` invocation (#1633).
 ...
 * (`inbrain config set`) is unreadable pre-connect, so the operator knob is the
 * `INBRAIN_SYNC_MAX_RUNTIME_SECONDS` env var; bare cron is covered by the non-TTY
...
export function resolveSyncHardDeadline(
  ...
  const envRaw = env.INBRAIN_SYNC_MAX_RUNTIME_SECONDS;
  if (envRaw !== undefined && envRaw !== '') {
    const n = Number(envRaw);
    if (Number.isFinite(n)) return mk(n, 'env:INBRAIN_SYNC_MAX_RUNTIME_SECONDS'); // n<=0 disables
  }
...
export async function runSync(engine: BrainEngine, args: string[]) {
  // v0.40 Federated Sync v2: `inbrain sync trigger` subcommand
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: inbrain sync [options]
...
Options:
  --no-embed           Skip the embed step. Use this when the embed
                       ...
                       embedding (run 'inbrain embed --stale' later).
  --no-extract         Skip the link/timeline extraction step. Pages will
                       show as stale in 'inbrain doctor'; run
                       'inbrain extract --stale' later to catch up.
  ...
  --no-schema-pack     Skip loading the active schema pack (no per-file pack
                       ...
                       PGLite is single-writer: stop 'inbrain serve' before a
                       large sync (see docs/architecture/serve-sync-concurrency.md).
                       INBRAIN_SYNC_TRACE=1 names the file being imported (hang triage).
  ...
  --parallel N         (with --all) Run up to N sources concurrently.
                       ...
                       (inbrain-sync:<source_id>) so independent sources
...
See also:
  inbrain embed --stale    Re-embed all stale chunks (post --no-embed).
  inbrain doctor           Diagnose dim mismatches and other sync issues.
`);
    return;
  }
  ...
  // v0.40 lock keys are inbrain-sync:<sourceId>).
  ...
  if (breakLock || forceBreakLock) {
    if (syncAll) {
      ...
      for (const src of activeSources) {
        const lockKey = `inbrain-sync:${src.id}`;
    ...
    const lockKey = `inbrain-sync:${sourceId}`;
  ...
  //
  // Validation: --timeout requires --source OR --all. Bare `inbrain sync
  ...
  // is set. Pre-fix, `inbrain sync` without --source skipped resolution and
  ...
  // bookmarks live in the sources table (not ~/.inbrain/config.json),
  ...
  if (syncAll) {
    ...
    if (!sources || sources.length === 0) {
      console.log('No sources with local_path configured. Use `inbrain sources add <id> --path <path>` first.');
      return;
    }
    ...
    if (!noEmbed) {
      ...
      if (mode === 'deferred') {
        ...
        // is NOT swept by sync; `inbrain embed --stale` clears it. So we show
        ...
        const staleNote = staleChars > 0
          ? ` (plus ~${staleChars.toLocaleString()} stale-backlog chars pending \`inbrain embed --stale\`)`
          : '';
        const previewMsg =
  ...
  if (!watch) {
    ...
    // `inbrain sync` without passing --repo every time.
    if (
      result.status !== 'dry_run' &&
      result.status !== 'blocked_by_failures' &&
      result.status !== 'partial'
...
export async function syncOneSource(
  ...
  const repoOpts: SyncOpts = {
    ...
    // lockId defaults to `inbrain-sync:${src.id}` via the invariant in
...
/**
 * v0.40.3.0 — read-only per-source dashboard for `inbrain sources status`.
 *
 ...
 *
 * Staleness thresholds match `inbrain doctor`'s sync-freshness rule
...
export async function buildSyncStatusReport(
  ...
  // loadConfig() returns null when ~/.inbrain/config.json is missing.
 ...
 *
 * `sink` defaults to `process.stdout` so the bare `inbrain sources status`
...
export function printSyncStatusReport(
  ...
  if (severe > 0) {
    write(`WARNING: ${severe} source(s) are SEVERELY stale (>72h). Run \`inbrain sync --all\` to refresh.`);
  }
}
 ...
 *   - INBRAIN_NO_GITIGNORE=1 — D23 escape hatch for shared-repo setups
...
export function manageGitignore(
  ...
  if (process.env.INBRAIN_NO_GITIGNORE === '1') {
    return;
  }
  ...
  gitignoreContent += '\n# Auto-managed by inbrain (db_only directories)\n';
 ...
 * threshold (INBRAIN_EXTRACTION_LAG_WARN_PCT, default 20%) the doctor check uses
 * so the nudge fires iff doctor would warn — one source of truth. Always
 * stderr (never stdout — keeps `--json` clean), suppressible via
 * INBRAIN_SYNC_NO_EXTRACT_NUDGE, best-effort (never throws). Source-prefix-aware
...
async function maybeExtractionNudge(engine: BrainEngine, sourceId?: string): Promise<void> {
  if (process.env.INBRAIN_SYNC_NO_EXTRACT_NUDGE) return;
  try {
    ...
    if ((stale / total) * 100 > warnPct) {
      serr(`[sync] ${stale} page(s) have un-extracted edges — run 'inbrain extract --stale'`);
    }
...
function printSyncResult(result: SyncResult, sink: NodeJS.WriteStream = process.stdout) {
  ...
  switch (result.status) {
    ...
    case 'blocked_by_failures':
      ...
      write(`  See ~/.inbrain/sync-failures.jsonl for details, or run 'inbrain doctor'.`);
      write(`  Fix the files then re-run 'inbrain sync', or 'inbrain sync --skip-failed' to move on.`);
      break;
    case 'partial':
      ...
      write(`  Re-run 'inbrain sync' to continue (last_commit unchanged; safe to retry).`);
      break;
  }
}
