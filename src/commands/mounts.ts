/**
 * inbrain mounts — manage connected inbrains (v0.19.0, PR 0).
 *
 * A "mount" is a SEPARATE inbrain DATABASE connected to your host agent.
 ...
 *   inbrain mounts add <id> --path <path> --engine pglite|postgres [--db-url|--db-path]
 *   inbrain mounts list [--json]
 *   inbrain mounts remove <id>
 *
 * Not yet shipped (PR 1+):
 *   inbrain mounts pin <id> <sha>        — freeze at a tested version (PR 1)
 *   inbrain mounts sync [--id <id>]      — git pull + cache refresh (PR 1)
 *   inbrain mounts enable/disable <id>   — toggle without removing (PR 1)
 *   inbrain mounts add --mcp-url         — HTTP MCP transport + OAuth (PR 2)
 */
...
import { InbrainError } from '../core/types.ts';

function getMountsDir(): string { return join(homedir(), '.inbrain'); }
// v0.40.3.0: INBRAIN_MOUNTS_PATH override exists for tests (libuv caches
...
function getMountsPath(): string {
  const override = process.env.INBRAIN_MOUNTS_PATH;
 ...
 *
 *  Unique tmp filename per call (pid + random). Two concurrent `inbrain
 ...
 *  address, deferred to PR 1 under `inbrain mounts sync --lock`). */
...
function parseAddArgs(args: string[]): AddArgs {
  if (args.length === 0) {
    throw new InbrainError(
      'Missing mount id',
      'inbrain mounts add <id> --path <path> [flags]',
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    const next = (flag: string): string => {
      const v = args[++i];
      if (!v) throw new InbrainError(`Missing value for ${flag}`, '', `Pass a value: ${flag} <value>`);
      return v;
    };
    if (a === '--path') path = next('--path');
      ...
      if (v !== 'postgres' && v !== 'pglite') {
        throw new InbrainError(`Invalid engine: "${v}"`, 'Must be "postgres" or "pglite"', 'Pass --engine pglite or --engine postgres');
      }
      engine = v;
    }
    ...
    else throw new InbrainError(`Unknown flag: ${a}`, '', 'See `inbrain mounts add --help`');
  }

  if (!path) {
    throw new InbrainError('Missing --path', 'Every mount needs a local clone path (for skills + handlers)', 'Add --path /absolute/path/to/mount');
  }
  ...
  if (engine === 'postgres' && !database_url) {
    throw new InbrainError('postgres mount requires --db-url', '', 'Pass --db-url postgresql://...');
  }
  if (engine === 'pglite' && !database_path && !database_url) {
    throw new InbrainError('pglite mount requires --db-path', '', 'Pass --db-path /path/to/mount/.pglite');
  }
...
async function runAdd(args: string[]): Promise<void> {
  ...
  if (file.mounts.some(m => m.id === parsed.id)) {
    throw new InbrainError(
      ...
      `Use 'inbrain mounts list' to see registered mounts`,
      `Remove the existing mount first: inbrain mounts remove ${parsed.id}`,
    );
  }
  ...
  // Publish aggregated resolver + manifest to ~/.inbrain/mounts-cache/. This
  ...
  // isn't inside a inbrain repo, skip (a later mounts invocation from a
...
function runList(args: string[]): void {
  ...
  if (file.mounts.length === 0) {
    process.stdout.write(
      ...
      `Add a mount with:\n` +
      `  inbrain mounts add <id> --path <path> --engine pglite --db-path <path>\n`,
    );
    return;
  }
...
function runRemove(args: string[]): void {
  if (args.length === 0) {
    throw new InbrainError(
      'Missing mount id',
      'inbrain mounts remove <id>',
      `Run 'inbrain mounts list' to see registered mounts`,
    );
  }
  const id = args[0];
  if (id === HOST_BRAIN_ID) {
    throw new InbrainError(
      ...
      `"host" is not a mount — it is the default brain from ~/.inbrain/config.json`,
      `Use 'inbrain init' to reconfigure the host brain`,
    );
  }
...
/**
 * Recompute + publish ~/.inbrain/mounts-cache/{RESOLVER.md,manifest.json}.
 * Looks for the host skills dir via findRepoRoot(cwd). When not in a inbrain
 ...
 * write already succeeded; a stale cache is recoverable via `inbrain mounts
 * list` (PR 1 will add `inbrain mounts sync --cache` for explicit refresh).
 */
function refreshMountsCache(): void {
  const repoRoot = findRepoRoot(process.cwd());
  if (!repoRoot) {
    process.stderr.write(
      'NOTE: mounts-cache not refreshed (not inside a inbrain repo). ' +
      'Run `inbrain mounts add|remove` from within a repo to publish ' +
...
function printHelp(): void {
  process.stdout.write(`inbrain mounts — manage connected inbrains (PR 0: direct transport only)
...
EXAMPLES
  # Mount a team-published yc-media inbrain (PGLite)
  git clone https://github.com/yc-team/yc-media-inbrain ~/inbrains/yc-media
  inbrain mounts add yc-media --path ~/inbrains/yc-media --engine pglite \\
    --db-path ~/inbrains/yc-media/.pglite

  # List registered mounts
  inbrain mounts list

  # Remove a mount
  inbrain mounts remove yc-media

v0.40.3.0 ADDITIONS
  inbrain mounts enable <id>             — re-enable a disabled mount
  inbrain mounts disable <id>            — toggle a mount off without removing
  inbrain mounts trust-frontmatter <id>  — let this mount's per-page
                                          contextual_retrieval_mode
                                          frontmatter override the source
                                          default. Off by default for
                                          mounted brains; host is always
                                          trusted.
  inbrain mounts untrust-frontmatter <id> — clear the trust flag.

NOT YET IMPLEMENTED (coming in PR 1/2)
  inbrain mounts pin <id> <sha>          — freeze a mount at a tested version
  inbrain mounts sync [--id <id>]        — git pull + refresh attestation
  inbrain mounts add --mcp-url <url>     — HTTP MCP transport + OAuth
`);
}
