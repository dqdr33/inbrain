/**
 * v0.31.2 — `inbrain notability-eval` mining + review CLI.
 *
 * Two subcommands:
 *
 *   inbrain notability-eval mine [--target-high N] [--target-medium N]
 ...
 *
 *   inbrain notability-eval review [--in PATH] [--out PATH]
 ...
 *      to `~/.inbrain/eval/notability-real.jsonl`.
 *
 ...
 *   - Private real: ~/.inbrain/eval/notability-real.jsonl (50 mined
 *     cases from the user's actual brain, local-only, runs only when
 *     INBRAIN_NOTABILITY_EVAL_REAL=1).
 *
...
export function defaultMiningOutPath(): string {
  return join(homedir(), '.inbrain', 'eval', 'notability-mining-candidates.jsonl');
}

/** Resolve the path where review writes confirmed cases. */
export function defaultReviewOutPath(): string {
  return join(homedir(), '.inbrain', 'eval', 'notability-real.jsonl');
}
...
export async function runNotabilityEval(args: RunNotabilityEvalArgs): Promise<void> {
  switch (args.cmd) {
    case 'mine': {
      ...
      // eslint-disable-next-line no-console
      console.log(`Run \`inbrain notability-eval review --in ${out}\` to hand-confirm tiers.`);
      return;
    }
    ...
    default:
      // eslint-disable-next-line no-console
      console.log([
        'inbrain notability-eval — eval suite for the notability gate.',
        '',
        'Subcommands:',
        '  mine   Walk the brain repo, sample paragraphs, write candidates.',
        '  review Hand-confirm tiers in a TTY. Writes ~/.inbrain/eval/notability-real.jsonl.',
        '',
        'Flags:',
