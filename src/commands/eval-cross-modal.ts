/**
 * inbrain eval cross-modal — multi-model quality gate (v0.27.x).
 *
 ...
 * before `inbrain init`. Receipts are bound to (slug, SKILL.md sha-8) so
 * `inbrain skillify check` can detect stale audits (T10=A).
 *
...
import { inbrainPath, loadConfig } from '../core/config.ts';
...
const HELP = `inbrain eval cross-modal — multi-model quality gate

USAGE:
  inbrain eval cross-modal --task "<description>" --output <path-or-skill-slug> [flags]
  inbrain eval cross-modal --batch <jsonl> [--limit N] [--output <receipt-path>] [flags]   (v0.40.1.0 Track D)
...
REQUIRED (batch mode, v0.40.1.0 Track D / T3):
  --batch <jsonl>          LongMemEval-shape JSONL (output of \`inbrain eval
...
BATCH FLAGS:
  ...
  --output PATH            Where to write the SUMMARY receipt (NOT where to
                           ...
                           ~/.inbrain/eval-receipts/cross-modal-batch-<sha8>.json
...
FLAGS:
  ...
  --receipt-dir <path>     Default: inbrainPath('eval-receipts').
...
CONFIGURATION:
  Models resolve via the inbrain AI gateway. Configure with:
    inbrain providers test            # see what's configured
    inbrain config                    # set keys
  Or set env vars: OPENAI_API_KEY, ANTHROPIC_API_KEY, GOOGLE_GENERATIVE_AI_API_KEY,
  TOGETHER_API_KEY, etc. The gateway reads from \`~/.inbrain/config.json\` plus
  process.env.

EXAMPLES:
  inbrain eval cross-modal \\
...
/**
 * Configure the AI gateway from `~/.inbrain/config.json` + process.env.
 *
...
export async function runEvalCrossModal(args: string[], opts: RunCrossModalOpts = {}): Promise<number> {
  ...
  const receiptDir = parsed.receiptDir ?? inbrainPath('eval-receipts');
  ...
  if (!isAvailable('chat')) {
    process.stderr.write(
      'Error: AI gateway has no usable chat provider. ' +
        ...
        'in your shell or run `inbrain config` to set keys.\n',
    );
    return 1;
  }
...
// at end of run (per D10 — keeps ~/.inbrain/eval-receipts/ clean).
// ---------------------------------------------------------------------------
 ...
 * `inbrain eval longmemeval`. Carries `question`+`question_type` and an
...
function readBatchRows(path: string): BatchReadResult {
  ...
  for (const line of raw.split('\n')) {
    ...
    // `inbrain eval longmemeval` carry an `error` field and an empty/missing
...
async function runBatchMode(parsed: ParsedArgs, opts: RunCrossModalOpts): Promise<number> {
  ...
  // end of the batch (per D10 — keeps ~/.inbrain/eval-receipts/ clean).
  const batchTempDir = mkdtempSync(join(tmpdir(), 'inbrain-batch-receipts-'));
  const runEvalFn = opts.runEval ?? runEval;
  ...
  try {
    ...
    const summaryPath = parsed.output ??
      join(inbrainPath('eval-receipts'), `cross-modal-batch-${batchSha8(summary)}.json`);
