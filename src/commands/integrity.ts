/**
 * inbrain integrity — scan, report, and repair brain-integrity issues.
 *
 ...
 *
 * Subcommands:
 *   inbrain integrity check              Read-only report to stdout
 *   inbrain integrity auto               Three-bucket repair with confidence
 *   inbrain integrity --dry-run          Same as auto, no writes
 *
 * Three-bucket confidence (contract with x_handle_to_tweet resolver):
 *   >= 0.8 → auto-repair through BrainWriter transaction
 *   0.5–0.8 → append to ~/.inbrain/integrity-review.md for human review
 *   < 0.5 → skip, log to ~/.inbrain/integrity.log.jsonl
 *
 * Progress is durable at ~/.inbrain/integrity-progress.jsonl. Re-running
...
import { loadConfig, toEngineConfig, inbrainPath } from '../core/config.ts';
...
// Lazy: INBRAIN_HOME may be set after module load.
const getReviewFile = () => inbrainPath('integrity-review.md');
const getLogFile = () => inbrainPath('integrity.log.jsonl');
const getProgressFile = () => inbrainPath('integrity-progress.jsonl');
 ...
 * no resolver calls. Called by `inbrain integrity check` for the full report
 * and by `inbrain doctor` (non-fast) for a sampled health signal.
 *
...
export async function scanIntegrity(
  ...
  // postgres.js connection, so the gate keeps the INBRAIN_DEBUG fallback
...
async function scanIntegrityBatch(
  ...
  // — inbrain lint should reject stringly-typed validate at write time.
...
function cmdReview(): void {
  if (!existsSync(getReviewFile())) {
    console.log(`No review queue yet. Run: inbrain integrity auto --confidence 0.8`);
    return;
  }
...
function printHelp(): void {
  console.log(`Usage: inbrain integrity <subcommand> [options]
...
Subcommands:
  ...
  reset-progress                Clear ~/.inbrain/integrity-progress.jsonl

Paths:
  Review queue: ~/.inbrain/integrity-review.md
  Skip log:     ~/.inbrain/integrity.log.jsonl
  Progress:     ~/.inbrain/integrity-progress.jsonl
`);
}
