/**
 * inbrain eval — Retrieval Evaluation Command
 *
 ...
 *
 * Usage:
 *   inbrain eval --qrels <path|json>
 *   inbrain eval --qrels <path> --config-a <path|json> --config-b <path|json>
 *   inbrain eval --qrels <path> --strategy hybrid --rrf-k 30 --k 5
 */
...
export async function runEvalCommand(engine: BrainEngine, args: string[]): Promise<void> {
  // v0.25.0 — sub-subcommand dispatch. Bare `inbrain eval --qrels ...`
...
function printSingleTable(report: EvalReport): void {
  ...
  console.log(`\ninbrain eval — ${queries.length} quer${queries.length === 1 ? 'y' : 'ies'} · strategy: ${label} · k=${k}\n`);
...
function printABTable(reportA: EvalReport, reportB: EvalReport, k: number): void {
  ...
  console.log(`\ninbrain eval — ${n} quer${n === 1 ? 'y' : 'ies'} · A/B comparison · k=${k}\n`);
...
function printHelp(): void {
  console.log(`
inbrain eval — measure and compare retrieval quality

USAGE
  inbrain eval --qrels <path>
  inbrain eval --qrels <path> --config-a <path> --config-b <path>
...
EXAMPLES
  inbrain eval --qrels ./my-queries.json
  inbrain eval --qrels ./qrels.json --strategy keyword
  inbrain eval --qrels ./qrels.json --rrf-k 30
  inbrain eval --qrels ./qrels.json --config-a baseline.json --config-b experiment.json
`.trim());
}
