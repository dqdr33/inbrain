/**
 * v0.36.1.0 (T7) — `inbrain calibration` CLI.
 *
 * Reads the latest calibration profile from the DB and prints it. Mirror of
 * the v0.29 `inbrain salience` / `inbrain anomalies` shape (pure data fn + JSON
 * formatter + human formatter + thin CLI dispatch).
 *
 * Sub-commands:
 *   inbrain calibration                              — print active profile for default holder
 *   inbrain calibration --holder <id>                — print for a specific holder
 *   inbrain calibration --json                       — machine output
 *   inbrain calibration --regenerate                 — run the calibration_profile phase now
 *   inbrain calibration --undo-wave <version>        — D18 undo command (Lane D adds the impl)
 *   inbrain calibration ab-report                    — D19 A/B harness report (Lane D adds the impl)
 *
...
export function formatProfileText(profile: CalibrationProfileRow | null, holder: string): string {
  if (!profile) {
    return (
      ...
      `  inbrain dream --phase calibration_profile\n` +
...
function ctxFromCli(engine: BrainEngine, config: InbrainConfig, sourceId: string): OperationContext {
  return {
    engine,
    config,
