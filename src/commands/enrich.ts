/**
 * inbrain enrich — batch enrichment primitive (issue #1700).
 *
 ...
 *
 * Why brain-internal: inbrain's own LLM tooling can only see brain tools
...
async function enrichOneLocked(ctx: EnrichOneCtx, candidate: EnrichCandidate): Promise<void> {
  ...
  // auto-link + disk write-through fire, exactly like `inbrain capture`. The
  ...
  if (!putPageOp) throw new Error('put_page operation missing (inbrain build issue)');
...
const HELP = `Usage: inbrain enrich [options]
...
export async function runEnrich(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (args.includes('--background') && engine.kind !== 'pglite') {
    ...
    if (sourceIds.length <= 1) {
      ...
      console.log('Follow with: inbrain jobs follow <id>');
      return;
    }
  ...
  if (!parsed.dryRun && !isAvailable('chat')) {
    console.error('Chat gateway unavailable. Configure a chat model (e.g. `inbrain config set chat_model anthropic:claude-haiku-4-5`), or pass --dry-run to preview candidates.');
    process.exit(1);
  }
