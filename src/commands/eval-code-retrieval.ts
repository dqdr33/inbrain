/**
 * v0.34 pre-w0 — `inbrain eval code-retrieval` CLI.
 *
 * Subcommands:
...
function parseArgs(args: string[]): ParsedArgs {
  ...
  while (i < args.length) {
    ...
    } else if (a === '--corpus') out.corpus = args[++i] ?? 'inbrain';
...
inbrain eval code-retrieval — v0.34 retrieval baseline + gate
...
USAGE:
  inbrain eval code-retrieval --baseline [--corpus NAME] [--save PATH] [--json]
  inbrain eval code-retrieval --with-code-intel [--corpus NAME] [--save PATH] [--json]
  inbrain eval code-retrieval --compare BASELINE.json WITH_CODE_INTEL.json [--json]
...
OPTIONS:
  ...
  --corpus NAME            Brain corpus to query (default: inbrain)
...
PRE-V0.34 BASELINE WORKFLOW (the assignment):
  ...
  inbrain eval code-retrieval --baseline --save /tmp/baseline-1.json
  inbrain eval code-retrieval --baseline --save /tmp/baseline-2.json
  inbrain eval code-retrieval --baseline --save /tmp/baseline-3.json

V0.34 SHIP GATE:
  # After v0.34 ships, capture the with-code-intel run and compare
  inbrain eval code-retrieval --with-code-intel --save /tmp/v034.json
  inbrain eval code-retrieval --compare /tmp/baseline-1.json /tmp/v034.json
`);
}
