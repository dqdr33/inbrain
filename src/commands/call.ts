...
/**
 * `inbrain call <tool> <json>` — trusted local op-dispatch surface.
 *
 ...
 * 6-tier chain (--source > INBRAIN_SOURCE > .inbrain-source dotfile > path-match
...
export async function runCall(engine: BrainEngine, args: string[]) {
  ...
  // tokens to keep the existing `inbrain call <tool> <json>` shape readable,
  ...
  if (!tool) {
    console.error("Usage: inbrain call [--source <id>] <tool> '<json>'");
    process.exit(1);
  }
