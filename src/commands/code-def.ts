...
export async function findCodeDef(
  ...
  // kinds here is what makes `inbrain code-def users` work against SQL.
  const DEF_TYPES = [
...
export async function runCodeDef(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  // args[0] is the symbol when invoked as `inbrain code-def <symbol>`
  ...
  if (!sym) {
    const err = errorFor({
      ...
      hint: 'inbrain code-def <symbol> [--lang <language>] [--json]',
    });
