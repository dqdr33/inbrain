/**
 * v0.32.3 — `inbrain search` CLI surface.
 *
 * Three sub-subcommands, mirroring `inbrain models` (v0.31.12) for shape
 * consistency:
 *
 *   inbrain search modes [--json]
 ...
 *
 *   inbrain search tune [--apply] [--json]
...
async function buildModesReport(engine: BrainEngine): Promise<SearchModesReport> {
  ...
  const knobs: Array<keyof ModeBundle> = [
    ...
    'expansion',
    'searchLimit',
    // v0.35.6.0 — floor-ratio surfaced in `inbrain search modes` dashboard
...
function formatModesText(report: SearchModesReport): string {
  ...
  lines.push('');
  lines.push('Mode bundles (frozen — set via `inbrain config set search.mode <mode>`):');
...
async function runModesSubcommand(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (reset || dryRunSource) {
    ...
    for (const k of toRemove) console.log(`  - ${k}`);
    console.log(`Mode bundle is now the only voice. Verify with: inbrain search modes`);
    return;
  }
...
async function runStatsSubcommand(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (stats.total_calls === 0) {
    console.log('');
    console.log('No telemetry recorded yet. Run a few `inbrain query` calls and re-check.');
...
async function runTuneSubcommand(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (stats.total_calls < 20) {
    if (json) {
      console.log(JSON.stringify({
        ...
        recommendations: [],
        message: 'Not enough search activity in the last 7 days to tune. Run `inbrain search stats` after some real usage.',
      }, null, 2));
      return;
    }
    ...
    console.log('Run a few `inbrain query` calls, then re-run `inbrain search tune`.');
    return;
  }
  ...
  if (resolved.resolved_mode === 'conservative' && stats.total_calls > 0) {
    ...
    if (dropPctPerCall > 2) {
      recs.push({
        ...
        apply_command: 'inbrain config set search.mode balanced',
      });
    }
  }
  ...
  if (stats.cache_hit_rate > 0.85 && stats.cache_hits + stats.cache_misses > 50) {
    recs.push({
      ...
      apply_command: 'inbrain config set search.cache.similarity_threshold 0.94',
    });
  }
  ...
  if (!resolved.cache_enabled && stats.total_calls > 5) {
    recs.push({
      ...
      apply_command: 'inbrain config unset search.cache.enabled',
    });
  }
  ...
  if (apply) {
    ...
    console.log('Run `inbrain search tune --apply` to apply these changes automatically.');
  }
}
...
async function maybeApplyRecommendation(engine: BrainEngine, r: TuneRecommendation): Promise<void> {
  ...
  if (parts[0] !== 'inbrain' || parts[1] !== 'config') return;
...
function buildRevertCommand(r: TuneRecommendation): string {
  const parts = r.apply_command.split(/\s+/);
  if (parts[2] === 'set') {
    return `inbrain config set ${parts[3]} ${String(r.current)}`;
...
const USAGE = `Usage: inbrain search <modes|stats|tune> [flags]
...
Examples:
  inbrain search modes
  inbrain search modes --reset
  inbrain search stats --days 30 --json
  inbrain search tune
  inbrain search tune --apply
`;
...
/**
 * `inbrain search modes` is read-only — no DB connection strictly required
 ...
 * this export is here so future no-engine modes (e.g. `inbrain search --help`
