/**
 * inbrain eval whoknows — v0.33 two-layer eval gate (ENG-D2).
 *
 ...
 *
 * Usage:
 *   inbrain eval whoknows test/fixtures/whoknows-eval.jsonl
 *   inbrain eval whoknows test/fixtures/whoknows-eval.jsonl --json
 *   inbrain eval whoknows test/fixtures/whoknows-eval.jsonl --skip-replay
 */
...
const HELP = `Usage: inbrain eval whoknows <fixture.jsonl> [options]

Two-layer eval gate (v0.33 ENG-D2) for naive inbrain whoknows:
...
async function runRegressionGate(
  ...
  if (captured.length < MIN_REPLAY_ROWS) {
    return {
      status: 'skipped',
      reason: `only ${captured.length} replay-eligible eval_candidates rows (< ${MIN_REPLAY_ROWS} threshold); INBRAIN_CONTRIBUTOR_MODE may have been off`,
...
export async function runEvalWhoknows(
  ...
  if (!opts.fixturePath) {
    console.error('inbrain eval whoknows: fixture path required');
    console.error(HELP);
    return 2;
  }
  ...
  try {
    ...
    console.error(`inbrain eval whoknows: ${(e as Error).message}`);
    return 2;
  }
  if (fixture.length === 0) {
    console.error('inbrain eval whoknows: fixture file is empty');
    return 2;
  }
  ...
  if (!thinClient && !engine) {
    console.error('inbrain eval whoknows: local engine required (not thin-client and no engine connected)');
    return 2;
  }
