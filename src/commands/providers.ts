/**
 * `inbrain providers` CLI — list, test, env, explain.
 *
 * This command operates WITHOUT a brain connection (no engine needed) so
 * users can verify provider setup before `inbrain init`.
 */
...
/**
 * Pure formatter for the recipe matrix shown by `inbrain providers list` and
...
function printHelp(): void {
  console.log(`inbrain providers — AI provider status and testing

USAGE
  inbrain providers list                                   List all known providers + status
  inbrain providers test [--touchpoint T] [--model ID]     Smoke-test configured (or specified) providers
  inbrain providers env <id>                               Show env vars required/optional for a provider
  inbrain providers explain [--json]                       Emit a provider choice matrix (agent-friendly)
...
EXAMPLES
  inbrain providers list
  inbrain providers test --model openai:text-embedding-3-large
  inbrain providers test --touchpoint chat --model anthropic:claude-haiku-4-5
  inbrain providers test --touchpoint chat --model deepseek:deepseek-chat
  inbrain providers env ollama
  inbrain providers explain --json
`);
}
...
async function runTest(args: string[]): Promise<void> {
    ...
    try {
      ...
      if (!configuredModel) {
        console.error(
          ...
          `Set the active provider with \`inbrain config set ${tpArg}_model <id>\` after running init.`,
        );
      } else if (configuredModel !== modelArg) {
        console.error(
          `Note: tested ${modelArg} in isolation; inbrain's configured ${tpArg} is ${configuredModel}. ` +
          `\`providers test\` does NOT verify your brain's active path.`,
        );
      }
  ...
  if (!gwIsAvailable(tpArg)) {
    console.error(`${tpArg[0]?.toUpperCase()}${tpArg.slice(1)} provider not configured or not ready. Run \`inbrain providers list\` to see status.`);
    process.exit(1);
  }
  ...
  try {
    if (tpArg === 'embedding') {
      const v = await embedOne('inbrain smoke test');
...
function runEnv(args: string[]): void {
  const id = args[0];
  if (!id) {
    console.error('Usage: inbrain providers env <id>');
    process.exit(1);
  }
  const recipe = getRecipe(id);
  if (!recipe) {
    console.error(`Unknown provider: ${id}. Run \`inbrain providers list\` to see known providers.`);
    process.exit(1);
  }
...
async function runExplain(args: string[]): Promise<void> {
  ...
  console.log(`  inbrain init --embedding-model ${matrix.recommended.split(':')[0]}:${matrix.recommended.split(':').slice(1).join(':')}`);
}
