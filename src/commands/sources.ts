/**
 * inbrain sources — manage multi-source brain configuration (v0.18.0).
 *
 ...
 *
 * Subcommands:
 *   inbrain sources add <id> --path <path> [--name <display>] [--federated|--no-federated]
 *   inbrain sources list [--json]
 *   inbrain sources remove <id> [--yes] [--dry-run] [--keep-storage]
 *   inbrain sources rename <id> <new-name>
 *   inbrain sources default <id>
 *   inbrain sources attach <id>   — write .inbrain-source in CWD
 *   inbrain sources detach        — remove .inbrain-source from CWD
 *   inbrain sources federate <id>   — sources.config.federated = true
 *   inbrain sources unfederate <id> — sources.config.federated = false
 *
...
async function runAdd(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!id) {
    console.error(
      'Usage: inbrain sources add <id> [--path <path> | --url <https-url>] ' +
...
async function runRemove(engine: BrainEngine, args: string[]): Promise<void> {
  const id = args[0];
  if (!id) {
    console.error('Usage: inbrain sources remove <id> [--yes] [--confirm-destructive] [--dry-run] [--keep-storage]');
    process.exit(2);
  }
...
//
// `inbrain sources set-cr-mode <id> <none|title|per_chunk_synopsis>`
...
async function runSetCrMode(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!id || !mode) {
    console.error('Usage: inbrain sources set-cr-mode <id> <none|title|per_chunk_synopsis>');
  ...
  if (exists.length === 0) {
    ...
    console.error(`  Run 'inbrain sources list' to see registered sources.`);
    process.exit(4);
  }
...
async function runArchive(engine: BrainEngine, args: string[]): Promise<void> {
  const id = args[0];
  if (!id) {
    console.error('Usage: inbrain sources archive <id>');
    process.exit(2);
  }
...
async function runRestore(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!id) {
    console.error('Usage: inbrain sources restore <id> [--no-federate]');
    process.exit(2);
  }
  ...
  // autopurged (e.g. operator rm -rf'd $INBRAIN_HOME/clones/), re-clone
  ...
  try {
    ...
    if (e instanceof SourceOpError && e.code === 'unmanaged_path') {
      // #1881: local_path is the user's own working tree, not a clone inbrain
      // created. inbrain won't re-clone over it, and `inbrain sync` will refuse it
      ...
      console.error(`  The DB row is restored; inbrain syncs this path read-only.`);
      ...
      console.error(`  Try \`inbrain sync --source ${id}\` to recover, or remove + re-add.`);
    } else {
      throw e;
    }
  }
...
async function runListArchived(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  for (const a of archived) {
    ...
    console.log(`  ${a.id.padEnd(20)}  ${String(a.pageCount).padStart(6)} pages  expires in ${hours}h  (restore: inbrain sources restore ${a.id})`);
  }
}
...
async function runRename(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!id || !newName) {
    console.error('Usage: inbrain sources rename <id> <new-display-name>');
    process.exit(2);
  }
  const src = await fetchSource(engine, id);
  if (!src) {
...
async function runDefault(engine: BrainEngine, args: string[]): Promise<void> {
  const id = args[0];
  if (!id) {
    console.error('Usage: inbrain sources default <id>');
    process.exit(2);
  }
  const src = await fetchSource(engine, id);
  if (!src) {
...
function runAttach(args: string[]): void {
  const id = args[0];
  if (!id) {
    console.error('Usage: inbrain sources attach <id>');
    process.exit(2);
  }
  validateSourceId(id);
  const dotfile = join(process.cwd(), '.inbrain-source');
  writeFileSync(dotfile, id + '\n', 'utf8');
  console.log(`Attached ${process.cwd()} to source "${id}" via .inbrain-source.`);
...
function runDetach(): void {
  ...
  if (!existsSync(dotfile)) {
    console.log(`No .inbrain-source file in ${process.cwd()}.`);
    return;
  }
  unlinkSync(dotfile);
  console.log(`Detached ${process.cwd()} (removed .inbrain-source).`);
}
...
async function runFederate(engine: BrainEngine, args: string[], value: boolean): Promise<void> {
  const id = args[0];
  if (!id) {
    console.error(`Usage: inbrain sources ${value ? 'federate' : 'unfederate'} <id>`);
    process.exit(2);
  }
  const src = await fetchSource(engine, id);
  if (!src) {
  ...
  try {
    ...
    if (sub.status === 'submitted') {
      ...
      console.log(`  → embed-backfill skipped (cooldown). Manually trigger with: inbrain jobs submit embed-backfill --params '{"sourceId":"${id}"}'`);
...
async function runStatus(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (sources.length === 0) {
    if (json) {
      ...
      console.log('No sources registered. Use `inbrain sources add <id> --path <path>` first.');
    }
    return;
  }
  ...
  for (const m of metrics) {
    ...
    if (m.lag_seconds === null) warns.push(`never synced — run \`inbrain sync --source ${m.source_id}\``);
    if (m.embed_coverage_pct < 95 && m.total_chunks > 100) {
      warns.push(`${(100 - m.embed_coverage_pct).toFixed(1)}% un-embedded — run \`inbrain embed --stale --source ${m.source_id}\``);
    }
    if (m.failed_jobs_24h >= 3) {
      warns.push(`${m.failed_jobs_24h} failures in 24h — check \`inbrain jobs list --status failed\``);
    }
    if (warns.length > 0) {
...
async function runWebhook(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  switch (sub) {
    ...
    case '--help':
    case '-h':
      console.log(`Usage: inbrain sources webhook <subcommand> <source-id> [options]
...
async function runWebhookSet(engine: BrainEngine, args: string[]): Promise<void> {
  const id = args[0];
  if (!id) {
    console.error('Usage: inbrain sources webhook set <id> [--secret VAL] [--github-repo owner/name]');
    process.exit(2);
  }
  const src = await fetchSource(engine, id);
  if (!src) {
  ...
  console.log('  Payload URL:  <your inbrain serve --http URL>/webhooks/github');
  console.log('  Content type: application/json');
  console.log(`  Secret:       ${secret}`);
  console.log('  Events:       Just the push event');
  console.log('  Active:       checked');
  console.log('');
  console.log('⚠ This secret is shown ONCE. Save it now; subsequent `inbrain sources webhook show` will NOT display it.');
}

async function runWebhookShow(engine: BrainEngine, args: string[]): Promise<void> {
  const id = args[0];
  if (!id) {
    console.error('Usage: inbrain sources webhook show <id>');
    process.exit(2);
  }
  const src = await fetchSource(engine, id);
  if (!src) {
...
async function runWebhookRotate(engine: BrainEngine, args: string[]): Promise<void> {
  const id = args[0];
  if (!id) {
    console.error('Usage: inbrain sources webhook rotate <id>');
    process.exit(2);
  }
  const src = await fetchSource(engine, id);
  if (!src) {
...
async function runWebhookClear(engine: BrainEngine, args: string[]): Promise<void> {
  const id = args[0];
  if (!id) {
    console.error('Usage: inbrain sources webhook clear <id>');
    process.exit(2);
  }
  const src = await fetchSource(engine, id);
  if (!src) {
...
async function runTrackedBranch(engine: BrainEngine, args: string[]): Promise<void> {
  const id = args[0];
  if (!id) {
    console.error('Usage: inbrain sources tracked-branch <id> [--set <branch>] [--detect]');
    process.exit(2);
  }
  const src = await fetchSource(engine, id);
  if (!src) {
...
/**
 * v0.41 — `inbrain sources audit <id>` dry-run scan.
 *
...
async function runAudit(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!sourceId) {
    console.error('Usage: inbrain sources audit <source-id> [--json] [--include-warns]');
    process.exit(2);
  }
  ...
  if (!src) {
    console.error(`Source not found: ${sourceId} (run \`inbrain sources list\` to see registered sources)`);
    process.exit(1);
  }
  if (!src.local_path) {
  ...
  // Walk recursively. Mirror inbrain sync's descent rules so the file set
  ...
  if (factsBackfillEstimate.pages > 0) {
    console.log(
      ...
      `Run: inbrain extract-conversation-facts --source-id ${sourceId} --max-cost-usd ${Math.max(factsBackfillEstimate.est_cost_usd, 1)}`,
    );
  }
...
function printHelp(): void {
  console.log(`inbrain sources — manage multi-source brain configuration (v0.26.5)
...
Subcommands:
  ...
  attach <id>                       Write .inbrain-source in CWD (like kubectl context).
  detach                            Remove .inbrain-source from CWD.
