/**
 * inbrain capture — the single human-facing entrypoint for getting content
 ...
 *
 *   inbrain capture "thought to remember"
 *   inbrain capture --file ./notes/2026-05-20.md
 *   echo "from stdin" | inbrain capture --stdin
 *   inbrain capture "..." --slug inbox/specific
 *   inbrain capture "..." --quiet           # slug-only output for pipelines
 *
 * Behavior:
 *   - Local install: writes to ~/.inbrain/inbox/<slug>.md OR routes through
...
const HELP = `Usage: inbrain capture [content] [options]
...
Modes (mutually exclusive — first match wins):
  inbrain capture "thought"          inline content
  inbrain capture --file PATH        read content from a file
  inbrain capture --stdin            read content from stdin (piped)
...
Options:
  ...
  --source ID          Multi-source brains: write under a non-default source.
                       Resolution: --source flag > INBRAIN_SOURCE env >
                       .inbrain-source dotfile (walk-up) > local_path >
...
Examples:
  inbrain capture "remember to follow up on the X deal"
  echo "from a pipe" | inbrain capture --stdin
  inbrain capture --file ./notes/today.md --slug daily/2026-05-20
  JOB=$(inbrain capture "..." --quiet)
`;
...
export async function runCapture(engine: BrainEngine | null, args: string[]): Promise<void> {
  ...
  if (parsed.source && isThinClient(cfg)) {
    console.error(`inbrain capture: --source is not supported on thin-client installs.`);
    console.error(`Server-side OAuth client registration determines source scope.`);
    console.error(`On the server, run:`);
    console.error(`  inbrain auth register-client <name> --source ${parsed.source} --scopes "read write"`);
    process.exit(1);
  }
  ...
  if (parsed.stdin) {
    ...
    try {
      ...
      console.error(
        `inbrain capture: failed to read ${parsed.filePath}: ${e instanceof Error ? e.message : String(e)}`,
      );
      process.exit(1);
    }
    ...
    console.error('inbrain capture: provide content positionally, --file PATH, or --stdin');
    console.error('Run `inbrain capture --help` for examples.');
    process.exit(1);
  }
  ...
  if (nullByteOffset !== -1) {
    console.error(
      `inbrain capture: refusing to capture binary content from ${inputLabel}\n` +
  ...
  if (normalizedBody.length === 0) {
    console.error('inbrain capture: refusing to capture empty content');
    process.exit(1);
  }
  ...
  if (!isThinClient(cfg) && engine) {
    try {
      ...
      console.error(`inbrain capture: ${e instanceof Error ? e.message : String(e)}`);
      process.exit(1);
    }
  }
  ...
  if (isThinClient(cfg)) {
    ...
    try {
      ...
      if (hint) {
        console.error(`inbrain capture: ${hint}`);
      } else if (e instanceof RemoteMcpError) {
        console.error(`inbrain capture: remote put_page failed: ${e.message}`);
        console.error('Run `inbrain remote doctor` to diagnose the connection.');
      } else {
        console.error(
          `inbrain capture: remote put_page failed: ${e instanceof Error ? e.message : String(e)}`,
        );
        console.error('Run `inbrain remote doctor` to diagnose the connection.');
      }
      process.exit(1);
  ...
  if (!engine) {
    console.error('inbrain capture: engine not connected');
    process.exit(1);
  }
  const putPageOp = operations.find((o) => o.name === 'put_page');
  if (!putPageOp) {
    console.error('inbrain capture: put_page operation missing (inbrain build issue)');
    process.exit(1);
  }
  ...
  try {
    ...
    if (hint) {
      ...
      console.error(
        `inbrain capture: put_page failed: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
    process.exit(1);
  }
