...
export async function runConfig(engine: BrainEngine, args: string[]) {
  ...
  if (action === 'show') {
    ...
    if (!config) {
      console.error('No config found. Run: inbrain init');
      process.exit(1);
    }
    console.log('Inbrain config:');
  ...
  // v0.32.3 [CDX-7+8]: `unset` is required before `inbrain search modes
  // --reset` can implement its contract. Two shapes:
  //   inbrain config unset <key>             — single-key delete
  //   inbrain config unset --pattern <pfx>   — prefix-bulk delete
  if (action === 'unset') {
    const flagIdx = args.indexOf('--pattern');
    if (flagIdx !== -1) {
      const prefix = args[flagIdx + 1];
      if (!prefix || prefix.length === 0) {
        console.error('Usage: inbrain config unset --pattern <prefix>');
        process.exit(1);
      }
    ...
    const key = args[1];
    if (!key) {
      console.error('Usage: inbrain config unset <key> | --pattern <prefix>');
      process.exit(1);
    }
  ...
  if (action === 'get' && key) {
    ...
    // column at init time and are file-plane canonical. `inbrain config set
    ...
    if (key === 'embedding_model' || key === 'embedding_dimensions') {
      ...
      const dbPath = inbrainPath('brain.pglite');
      ...
      if (isPgliteEngine) {
        ...
        if (key === 'embedding_model') {
          console.error(`[config]   inbrain init --pglite --embedding-model ${value}`);
        } else {
          console.error(`[config]   inbrain init --pglite --embedding-dimensions ${value}`);
        }
        console.error(`[config]   inbrain sync   # re-imports your brain repo`);
      } else {
    ...
    if (!forceFlag) {
      ...
      if (!isKnown && !matchesPrefix) {
        ...
        if (suggestion) {
          ...
          console.error(`[config] No similar known key. Run \`inbrain config show\` to see currently-set keys.`);
        }
      ...
      const matchesPrefix = KNOWN_CONFIG_KEY_PREFIXES.some(p => key.startsWith(p));
      if (!isKnown && !matchesPrefix) {
        console.error(`[config] WARN: writing unknown key "${key}" with --force. Nothing in inbrain reads this.`);
      }
    }
    ...
    if (key === 'search_embedding_column') {
      ...
      // We re-read merged config so a prior `inbrain config set
      ...
      if (mergedCfg) {
        ...
        if (!Object.hasOwn(registry, value)) {
          ...
          console.error(
            `[config] Unknown embedding column "${value}". ` +
              ...
              `Add it via: inbrain config set embedding_columns '<JSON>'`,
          );
          process.exit(1);
        }
        ...
        try {
          ...
          if (total > 0 && pct < 90 && !coverageOverride) {
            ...
            console.error(
              `[config]   inbrain config set search_embedding_column ${value} --coverage-override`,
            );
            process.exit(1);
          }
        } catch (err) {
    ...
    // search.mode writes. Honors INBRAIN_NO_MODE_SWITCH_UX=1 + non-TTY.
    ...
    console.error('Usage: inbrain config [show|get|set|unset] <key> [value]');
    console.error('       inbrain config unset --pattern <prefix>');
    process.exit(1);
  }
}
