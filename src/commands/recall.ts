/**
 * v0.31 — `inbrain recall` + `inbrain forget` CLI.
 *
 ...
 *
 *   inbrain recall <entity>                  # listFactsByEntity
 *   inbrain recall --since "8 hours ago"     # listFactsSince
 *   inbrain recall --session <id>            # listFactsBySession
 *   inbrain recall --today                   # markdown render with kind icons
 *   inbrain recall --grep <text>             # text filter (case-insensitive)
 *   inbrain recall --supersessions [--since DUR]   # audit log
 *   inbrain recall --include-expired
 *   inbrain recall --as-context              # prompt-injection-ready markdown
 *   inbrain recall --json                    # structured output
 *
 *   inbrain forget <fact-id>                  # shorthand for expireFact
 *
 * v0.32 additions (this file):
 *   --since-last-run            # read+advance ~/.inbrain/recall-cursors/<src>.json
 ...
 *
 * v0.32 also adds thin-client routing: on `inbrain init --mcp-only` installs
  ...
  try {
    ...
    while (!sigintReceived) {
      if (isTty) {
        ...
        process.stdout.write(
          `inbrain recall --watch ${intervalSec}s  ` +
...
export async function runForget(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!idArg) {
    process.stderr.write('Usage: inbrain forget <fact-id> [--reason <text>]\n');
    process.exit(1);
  }
  ...
  // v0.33: thin-client routing. Without this, `inbrain forget <id>` on a
  ...
  // page's `## Facts` fence and survives `inbrain rebuild`. Legacy rows
  ...
  const suffix = result.path === 'fence' ? '' : ' (legacy DB-only — will not survive inbrain rebuild)';
...
function renderAsContext(rows: FactRow[]): string {
  if (rows.length === 0) return '<!-- inbrain hot memory: empty -->\n';
  const parts = ['<!-- inbrain hot memory (auto-injected) -->', ''];
  for (const r of rows) {
    const icon = KIND_ICON[r.kind];
    const tag = r.entity_slug ? ` [${r.entity_slug}]` : '';
    const ageStr = humanAge(r.valid_from);
