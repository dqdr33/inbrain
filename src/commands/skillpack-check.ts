/**
 * `inbrain skillpack-check` — agent-readable health report.
 *
 * Wraps `inbrain doctor --json` + `inbrain apply-migrations --list` into a
 ...
 *
 * Usage:
 *   inbrain skillpack-check              # pretty-printed JSON + exit code
 *   inbrain skillpack-check --quiet      # only exits with status; no output
 *   inbrain skillpack-check --help
 *
 * Exit codes:
...
/**
 * Resolve the inbrain binary + args for spawning subcommands from
 * within skillpack-check. Handles three install cases:
 *   - Running the compiled binary (argv[1] ends in /inbrain): re-exec it.
 *   - Running via `bun run src/cli.ts` (argv[1] is a .ts file): prefix with `bun run`.
 *   - Anything else: fall back to `which inbrain` on $PATH.
 */
function inbrainSpawn(): { cmd: string; prefix: string[] } {
...
function runMigrationsList(): SkillpackReport['migrations'] {
  ...
  try {
    ...
    //   Installed inbrain version: 0.11.1
    //
...
function buildReport(): SkillpackReport {
  ...
  if ('applied_count' in migrations) {
    if (migrations.partial_count > 0 || migrations.pending_count > 0) {
      healthy = false;
      const action = 'inbrain apply-migrations --yes';
  ...
  const summary = healthy
    ? 'inbrain skillpack healthy'
    : `inbrain skillpack needs attention: ${actions.length} action(s) — ${actions[0]}`;
...
export async function runSkillpackCheck(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`inbrain skillpack-check — agent-readable health report.

Wraps doctor + apply-migrations --list into one JSON blob.

Usage:
  inbrain skillpack-check            Pretty JSON to stdout, exit 0/1/2 (legacy).
  inbrain skillpack check            v0.33 subcommand. Default: informational
                                     (exit 0 even with drift). Pass --strict
                                     to exit non-zero on action-needed.
  inbrain skillpack-check --quiet    Exit code only, no output.
...
`);
  ...
  // v0.33: When invoked as the new `inbrain skillpack check` subcommand,
  ...
  //
  // Top-level `inbrain skillpack-check` (cron compat) keeps exit-1 on
...
/**
 * Detect whether this invocation came via `inbrain skillpack check`
 * (subcommand) vs the top-level `inbrain skillpack-check` (cron compat).
 * Subcommand → informational default. Top-level → strict default.
 */
function isSkillpackCheckSubcommand(): boolean {
  const argv = process.argv;
  // argv shape for `inbrain skillpack check` after the binary name and
