/**
 * inbrain eval prune — delete old eval_candidates rows (v0.21.0).
 *
 * Retention is unlimited by default (matches ingest_log precedent).
 * This command is the explicit cleanup; pairs with `inbrain eval export`
 * (snapshot first, then prune if you want to reset).
 *
 * Usage:
 *   inbrain eval prune --older-than 30d
 *   inbrain eval prune --older-than 1h --dry-run
 */
...
function printHelp(): void {
  console.error(`inbrain eval prune — delete old eval_candidates rows

USAGE:
  inbrain eval prune --older-than DUR [--dry-run]
...
EXAMPLES:
  inbrain eval prune --older-than 30d
  inbrain eval prune --older-than 90d --dry-run
`);
}
