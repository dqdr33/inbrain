...
import { loadConfig, inbrainPath } from '../core/config.ts';
...
export async function runImport(
  ...
  // later with `inbrain embed --stale` after configuring a provider).
  if (!noEmbed) {
    ...
    try {
      ...
      console.error('Tip: run `inbrain import <dir> --no-embed` to import without embedding now.');
      process.exit(1);
    }

    // v0.41.6.0 D1: preflight embedding credentials. Closes the bug class
    // where `inbrain import` per-file embed writes N identical
  ...
  // passing `inbrain import --source dept-x ...` silently fell back to
  ...
  if (!sourceId && process.env.INBRAIN_SOURCE) {
  ...
  if (!dirArg) {
    console.error('Usage: inbrain import <dir> [--no-embed] [--workers N] [--fresh] [--source-id <id>] [--json]');
    process.exit(1);
  }
  ...
  console.error(`[inbrain phase] import.collect_files start dir=${dir} strategy=${strategy}`);
  const allFiles = collectSyncableFiles(dir, { strategy });
  console.error(
    `[inbrain phase] import.collect_files done ${Date.now() - _walkT0}ms files=${allFiles.length}`,
  );
  ...
  const checkpointPath = inbrainPath('import-checkpoint.json');
  ...
  async function processFile(eng: BrainEngine, filePath: string) {
    ...
    try {
      ...
      // up images when INBRAIN_EMBEDDING_MULTIMODAL=true so this branch is
      // unreachable when the gate is off; defense-in-depth check anyway.
      const result = isImageFilePath(relativePath) && process.env.INBRAIN_EMBEDDING_MULTIMODAL === 'true'
      ...
      if (_fileMs > 5000) {
        console.error(`[inbrain phase] import.process_file slow ${_fileMs}ms ${relativePath}`);
      }
    ...
    try {
      ...
      if (total > 0 && untyped / total >= 0.1) {
        ...
        console.error(
          ...
          `have no \`type\` matching the active schema pack. Run \`inbrain schema detect\` ` +
          `to propose a pack matching your content shape, or \`inbrain doctor --json\` ` +
  ...
  if (gitHead && !opts.managedBookmark) {
    ...
    if (failures.length === 0) {
      ...
      console.error(
        ...
        `sync.last_commit NOT advanced — re-run 'inbrain sync' to retry, or ` +
        `'inbrain sync --skip-failed' to acknowledge and move past them.`,
      );
    }
 ...
 * via `INBRAIN_MAX_WALK_DEPTH`.
 */
function resolveMaxWalkDepth(): number {
  const raw = process.env.INBRAIN_MAX_WALK_DEPTH;
  if (raw) {
    const n = Number(raw);
 ...
 * `INBRAIN_EMBEDDING_MULTIMODAL=true`. Codex (C5) flagged the contradiction
...
export function collectSyncableFiles(dir: string, opts: CollectOpts = {}): string[] {
  const strategy: SyncStrategy = opts.strategy ?? 'markdown';
  const multimodalOn = process.env.INBRAIN_EMBEDDING_MULTIMODAL === 'true';
  ...
  function walk(d: string, depth: number): void {
    if (depth >= maxDepth) {
      console.warn(`[inbrain] walker depth limit reached at ${d}; skipping`);
      return;
    }
    let entries: string[];
    try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        console.warn(`[inbrain import] Skipping unreadable path: ${full}`);
        continue;
      }

      if (stat.isSymbolicLink()) {
        console.warn(`[inbrain import] Skipping symlink: ${full}`);
        continue;
      }
      ...
      if (stat.isDirectory()) {
        ...
        if (visitedInodes.has(inodeKey)) {
          console.warn(`[inbrain] walker cycle detected at ${full}; skipping`);
          continue;
        }
