/**
 * v0.31.12 — `inbrain models` CLI.
 *
 * Two modes:
 *
 *   `inbrain models`           — read-only routing table. Prints the four
 ...
 *
 *   `inbrain models doctor`    — opt-in probe. Fires a 1-token `gateway.chat()`
...
const PER_TASK_KEYS: Array<{ key: string; tier: ModelTier; description: string }> = [
  ...
  { key: 'models.think',                    tier: 'deep',      description: '`inbrain think` synthesis op' },
  { key: 'models.subagent',                 tier: 'subagent',  description: '`inbrain agent run` subagent loop' },
...
async function buildReport(engine: BrainEngine): Promise<ModelsReport> {
  ...
  for (const { key, tier, description } of PER_TASK_KEYS) {
    ...
    const explicit = await probeSource(engine, key, 'INBRAIN_MODEL');
...
function formatText(report: ModelsReport): string {
  ...
  lines.push('');
  lines.push('Tip: probe reachability with `inbrain models doctor` (opt-in; spends a minimal request per configured chat/embed/rerank surface).');
  return lines.join('\n');
}
    ...
    if (providerId === 'voyage' && supportsVoyageOutputDimension(modelId)) {
      if (!isValidVoyageOutputDim(dims)) {
        return {
          ...
          fix:
            `inbrain config set embedding_dimensions <${VOYAGE_VALID_OUTPUT_DIMS.join('|')}>, ` +
    ...
    if (providerId === 'zeroentropyai' && supportsZeroEntropyDimension(modelId)) {
      if (!isValidZeroEntropyDim(dims)) {
        return {
          ...
          fix:
            `inbrain config set embedding_dimensions <${ZEROENTROPY_VALID_DIMS.join('|')}>.`,
          elapsed_ms: Date.now() - start,
        };
      }
    }
 ...
 * `InbrainConfig.reranker_model` — a file-plane field nothing currently
 ...
 * while every `inbrain search` call is using a mode default. This helper
...
async function probeRerankerConfig(engine: BrainEngine): Promise<ProbeResult> {
  ...
  if (!modelStr) {
    ...
    return {
      ...
      status: 'ok',
      message: 'reranker not configured (set `inbrain config set search.reranker.model <provider:model>` and `search.reranker.enabled true`)',
      elapsed_ms: Date.now() - start,
    };
  }
  ...
  try {
    ...
    if (tp.models.length > 0 && !tp.models.includes(parsed.modelId)) {
      return {
        ...
        fix: `inbrain config set search.reranker.model ${recipe.id}:<one of ${tp.models.join('|')}>`,
        elapsed_ms: Date.now() - start,
      };
    }
    return {
...
`Usage:
  inbrain models                   Show routing table (read-only)
  inbrain models doctor [flags]    Probe each configured model (~1 token each)
  inbrain models --json            Machine-readable output
...
Configure routing:
  inbrain config set models.default <model>           # global hammer
  inbrain config set models.tier.<tier> <model>       # per-tier (utility/reasoning/deep/subagent)
  inbrain config set models.aliases.<name> <model>    # custom alias
