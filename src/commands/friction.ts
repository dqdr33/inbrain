/**
 * inbrain friction — friction reporter CLI.
 *
 * Four subcommands in v1 (analytical/clustering ones move to v1.1):
 *   inbrain friction log     Append a friction or delight entry
 *   inbrain friction render  Render a run as markdown or JSON
 *   inbrain friction list    List recent runs with counts
 *   inbrain friction summary Side-by-side friction + delight summary
 *
 * Subcommands stay thin (≤ ~30 LOC each). Core logic lives in src/core/friction.ts.
 *
 * The CLI is dispatched from src/cli.ts. See `inbrain friction --help`.
 */
...
function cmdLog(args: string[]): number {
  ...
  if (!phase || !message) {
    console.error('usage: inbrain friction log --phase <name> --message <text> [--severity ...] [--hint ...] [--kind ...] [--run-id ...]');
    return 2;
  }
...
function printHelp() {
  console.log(`inbrain friction — friction reporter
...
Examples:
  inbrain friction log --severity confused --phase install --message "init didn't say which engine"
  inbrain friction render --run-id claw-test-20260428-... --transcripts
  inbrain friction list --json
  inbrain friction summary

Run-id resolution: --run-id > $INBRAIN_FRICTION_RUN_ID > 'standalone'.`);
}
