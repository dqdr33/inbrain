/**
 * inbrain skillify <scaffold|check> — W4 CLI namespace.
 *
...
const HELP_TOP = `inbrain skillify <subcommand> [options]
...
Run \`inbrain skillify <subcommand> --help\` for per-subcommand options.
`;
...
const HELP_SCAFFOLD = `inbrain skillify scaffold <name> [options]
...
\`inbrain check-resolvable --strict\` fails if any skill still has the
...
export async function runSkillifyScaffold(args: string[]): Promise<void> {
  ...
  if (flags.json) {
    ...
    console.log(`  3. inbrain skillify check skills/${flags.name}/scripts/${flags.name}.mjs`);
    console.log(`  4. inbrain check-resolvable`);
  }
}

// ---------------------------------------------------------------------------
// `inbrain skillify check` — delegates to scripts/skillify-check.ts via same
