/**
 * inbrain resolvers — introspect the Resolver SDK registry.
 *
 * Subcommands:
 *   inbrain resolvers list              Pretty table of all registered resolvers.
 *   inbrain resolvers list --json       Machine-readable output.
 *   inbrain resolvers describe <id>     Detail view: schema + availability.
 *
 ...
 * ~/.inbrain/resolvers/) plugs in here.
 */
...
async function cmdDescribe(args: string[]): Promise<void> {
  ...
  if (!id) {
    console.error('Usage: inbrain resolvers describe <id>');
    process.exit(1);
  }
...
function printHelp(): void {
  console.log(`Usage: inbrain resolvers <subcommand> [options]
...
Examples:
  inbrain resolvers list
  inbrain resolvers list --cost paid
  inbrain resolvers describe x_handle_to_tweet
`);
}
