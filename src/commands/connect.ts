/**
 * `inbrain connect` — one-command coding-agent onboarding from a bearer token
 ...
 * `inbrain serve --http` and teaches it to self-orient via `get_brain_identity`
 * + `list_skills`. Direct HTTP MCP — no local install or thin-client config
 * needed for the connection.
 *
 *   inbrain connect <mcp-url> [--token <bearer>] [--name inbrain]
 ...
 *
 * Auth:
 *   - Bearer (default): a `inbrain auth create` token. Simple; long-lived +
 ...
 *     INBRAIN_REMOTE_TOKEN` (bearer via env var; --install runs it).
...
const OAUTH_SECRET_NOTE =
  ...
  'short-lived, scoped access tokens; revoke with `inbrain auth revoke-client`.';

const PERPLEXITY_REMOTE_NOTE = [
  'Perplexity connects remotely, so the brain must be reachable over HTTPS. On the',
  'host run: inbrain serve --http --bind 0.0.0.0 --public-url <your-https-url> (the',
  'default 127.0.0.1 bind refuses tunneled connections). See docs/mcp/PERPLEXITY.md.',
].join('\n');

const HELP = `inbrain connect — wire a coding agent to a remote inbrain over MCP

Usage:
  inbrain connect <mcp-url> [--token <bearer>] [flags]
...
'inbrain serve --http' endpoint; a bare host is rejected — pass an explicit
https:// URL.
...
Flags:
  --token <bearer>     Bearer token (else $${ENV_VAR}; from 'inbrain auth create')
  ...
  --register           With --oauth: mint a client on the host (inbrain auth register-client)
...
Examples:
  inbrain connect https://brain.example.com/mcp --token inbrain_xxx
  inbrain connect https://brain.example.com:3131 --install --yes
  inbrain connect https://brain.example.com/mcp --token inbrain_xxx --agent codex
  inbrain connect https://brain.example.com/mcp --agent perplexity --oauth --register
  inbrain connect https://brain.example.com/mcp --agent perplexity --oauth \\
    --client-id inbrain_cl_xxx --client-secret inbrain_cs_xxx
`;
 ...
 * a documented, supported topology (`inbrain serve --http --bind`).
 */
...
export function normalizeMcpUrl(input: string): UrlResult {
  ...
  if (!raw) {
    return { ok: false, error: 'Missing MCP URL. Usage: inbrain connect <https://host/mcp> --token <bearer>' };
  }
...
export function resolveToken(opts: { tokenFlag?: string | null; env?: string | null; mode: 'print' | 'install' }): TokenResolution {
  ...
  return {
    kind: 'error',
    error: `No token. Pass --token <bearer> or set ${ENV_VAR}. Create one on the host with: inbrain auth create "<name>"`,
  };
}
...
function claudeBlock(p: { name: string; url: string; token: string | null }): string {
  ...
  if (!p.token) lines.push(`Replace ${PLACEHOLDER_TOKEN} with a token from \`inbrain auth create "claude-code"\` on the host.`, '');
...
function codexBlock(p: { name: string; url: string; token: string | null }): string {
  ...
  if (!p.token) lines.push(`Replace ${PLACEHOLDER_TOKEN} with a token from \`inbrain auth create "codex"\` on the host.`, '');
  lines.push(
...
function genericBearerBlock(p: { url: string; token: string | null }): string {
  const headerToken = p.token ?? PLACEHOLDER_TOKEN;
  return [
    '# Add an HTTP MCP server pointed at your inbrain:',
    `#   URL:    ${p.url}`,
...
function genericOAuthBlock(p: { oauth: OAuthCreds }): string {
  const secret = p.oauth.clientSecret ?? PLACEHOLDER_SECRET;
  return [
    '# Add an OAuth 2.1 (client-credentials) MCP server pointed at your inbrain:',
    `#   URL:           ${p.oauth.issuer}/mcp`,
    `#   Issuer URL:    ${p.oauth.issuer}`,
    `#   Client ID:     ${p.oauth.clientId}`,
    `#   Client Secret: ${secret}`,
...
/** Mint an OAuth client by shelling to the host's `inbrain auth register-client`. */
function defaultRegisterOAuthClient(name: string, scopes: string): RegisterResult {
  const r = defaultRunBinary('inbrain', [
  ...
  if (r.code !== 0) {
    return { ok: false, message: r.stderr || r.stdout || 'inbrain auth register-client failed' };
  }
  ...
  if (f.register) {
    ...
    if (!r.ok) {
      fail(`Could not register an OAuth client (run this on the brain host where the DB lives): ${r.message}\n` +
        `Or mint one manually: inbrain auth register-client ${f.name} --grant-types client_credentials --scopes "${f.scopes}"`);
    }
    return { issuer, clientId: r.clientId, clientSecret: r.clientSecret };
  }
  return fail(
    '--oauth needs an OAuth client. Either:\n' +
    `  • --register  (mint one on the host: inbrain auth register-client ${f.name} --grant-types client_credentials --scopes "${f.scopes}")\n` +
