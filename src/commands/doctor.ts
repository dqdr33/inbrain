...
import { inbrainPath } from '../core/config.ts';
...
export interface Check {
   ...
   * `inbrain doctor --remediation-plan` / `--remediate`. Optional and
 ...
 *   - inbrain doctor --json (CLI)
 *   - run_doctor MCP op (remote callers)
 *   - inbrain remote doctor (renders this from the MCP op response)
 *
...
export interface DoctorReport {
   ...
   *
   * Preserved verbatim from pre-v0.41.19.0 for back-compat with `inbrain
   * doctor --remediate`, `inbrain remote doctor`, the MCP `run_doctor` op,
...
/**
 * Focused doctor for `run_doctor` MCP op + `inbrain remote doctor` CLI.
 *
...
export function resolveWhoknowsFixturePath(
  ...
  if (env.INBRAIN_WHOKNOWS_FIXTURE_PATH) {
    return isAbsolute(env.INBRAIN_WHOKNOWS_FIXTURE_PATH)
      ? env.INBRAIN_WHOKNOWS_FIXTURE_PATH
      : resolvePath(process.cwd(), env.INBRAIN_WHOKNOWS_FIXTURE_PATH);
  }
 ...
 * not the eval gate outcome (that runs via `inbrain eval whoknows`).
 *
 ...
 * regression detection lives in `inbrain eval whoknows --json` and is
  ...
  try {
    ...
    if (!fixturePath) {
      return {
        ...
        status: 'warn',
        message: 'whoknows eval fixture path could not be resolved. Set INBRAIN_WHOKNOWS_FIXTURE_PATH to the absolute path for test/fixtures/whoknows-eval.jsonl.',
      };
    }
    ...
    return {
      name: 'whoknows_health',
      status: 'ok',
      message: `whoknows eval fixture present (${rows.length} queries). Run \`inbrain eval whoknows test/fixtures/whoknows-eval.jsonl\` to grade.`,
    };
  } catch (e) {
  ...
  try {
    ...
    if (ratio > 0.10) {
      return {
        ...
        status: 'fail',
        message: `${offGrid}/${total} takes off the 0.05 grid (${(ratio * 100).toFixed(1)}%). Fix: inbrain apply-migrations --yes`,
      };
    }
    if (ratio > 0.01) {
      return {
...
export async function doctorReportRemote(engine: BrainEngine): Promise<DoctorReport> {
  ...
  try {
    ...
    if (version >= LATEST_VERSION) {
      ...
      checks.push({
        name: 'schema_version',
        status: 'fail',
        message: `No schema version recorded. Migrations never ran. Run \`inbrain apply-migrations --yes\` on the host.`,
      });
    } else {
      checks.push({
        name: 'schema_version',
        status: 'warn',
        message: `Version ${version}, latest is ${LATEST_VERSION}. Run \`inbrain apply-migrations --yes\` on the host.`,
      });
    }
  } catch {
  ...
  try {
    ...
    if (wedged.length > 0) {
      const cmd = wedged.map(v => `inbrain apply-migrations --force-retry ${v}`).join(' && ');
      checks.push({
      ...
      checks.push({
        name: 'minions_migration',
        status: 'fail',
        message: `MINIONS HALF-INSTALLED on brain host: ${stuck.join(', ')}. Run on the host: inbrain apply-migrations --yes`,
      });
    }
  } catch {
  ...
  try {
    ...
    const failHours = _resolveSyncFreshnessHours('INBRAIN_SYNC_FRESHNESS_FAIL_HOURS', 72);
    ...
    const msg =
      sev.unresolved === 0
        ...
        : `${sev.unresolved} unresolved sync failure(s)` +
          ...
          ` — run \`inbrain sync --skip-failed\` on the host to acknowledge`;
      ...
      if (result.walk_truncated) {
        ...
        checks.push({
          ...
          message:
            ...
            `Verify on the brain host: \`inbrain sources status\` then \`inbrain sync --source <id> --full\`.`,
        });
      } else {
        checks.push({
          name: 'multi_source_drift',
    ...
    try {
      ...
      checks.push({
        ...
        message: stalled === 0
          ...
          : `${stalled} active job(s) stalled > 1h — \`inbrain jobs cancel <id>\` or \`inbrain jobs retry <id>\` on the host`,
      });
    } catch {
  ...
  // buildChecks). Surfaces when INBRAIN_EMBEDDING_* env vars disagree
  ...
  // so the user sees it at the next `inbrain doctor` run instead of at the
  ...
  // v0.42.x (#1794, 4A): pool-budget nudge when INBRAIN_MAX_CONNECTIONS is set.
  ...
  // ~/.inbrain/audit/rerank-failures-*.jsonl. Failure-only (no success
  ...
  // thin-client parity so `inbrain remote doctor` sees the same hint.
  ...
  try {
    ...
    if (mode === 'off') {
      return {
        ...
        status: 'ok',
        message: 'Self-upgrade disabled (mode=off). Enable: inbrain config set self_upgrade.mode notify',
      };
    }
    ...
    if (entry && isCacheFresh(entry, Date.now()) && entry.marker.kind === 'upgrade_available') {
      parts.push(`update available: ${entry.marker.current} -> ${entry.marker.latest} (run: inbrain self-upgrade)`);
    }
    ...
    if (failures.length > 0) {
      ...
      return {
        ...
        message:
          ...
          `Check ~/.inbrain/upgrade-errors.jsonl; apply manually with inbrain self-upgrade.`,
      };
    }
 ...
 *
 * Reads `~/.inbrain/audit/synopsis-failures-YYYY-Www.jsonl` via
  ...
  try {
    ...
    const fixHint =
      chunkerDrift > 0 || modeNull > 0
        ? ` Run \`inbrain reindex --markdown\` to align.`
        : '';
    return {
      name: 'contextual_retrieval_coverage',
 ...
 * `INBRAIN_SEARCH_EXCLUDE` env additions). Makes the surviving exclude policy
 ...
 * `inbrain config set link_resolution.global_basename true` hint when
    ...
    if (wouldResolveCount >= 5 && ratio >= 0.20) {
      ...
      return {
        ...
        message:
          ...
          `Enable with: inbrain config set link_resolution.global_basename true`,
      };
    }
    const pct = Math.round(ratio * 100);
    return {
...
export async function checkAbandonedThreads(engine: BrainEngine): Promise<Check> {
  try {
    ...
    return {
      name: 'abandoned_threads',
      status: 'ok',
      message: `${count} high-conviction take(s) older than 12 months and never revisited — see \`inbrain calibration\` for details`,
    };
  } catch (e) {
    return {
      name: 'abandoned_threads',
  ...
  try {
    ...
    if (ageDays > staleDays) {
      return {
        name: 'calibration_freshness',
        status: 'warn',
        message: `Calibration profile is ${ageDays} days old (stale at >${staleDays}d). Run \`inbrain calibration --regenerate\``,
      };
    }
    return {
      name: 'calibration_freshness',
  ...
  try {
    ...
    if (bounces >= 1000) {
      return {
        name: 'subagent_health',
        status: 'fail',
        message: `${bounces} lease-pressure bounces in last 24h — this is blocking real work. Raise the cap: \`export INBRAIN_ANTHROPIC_MAX_INFLIGHT=64\` (or \`unlimited\` for Azure / Bedrock / self-hosted upstreams with no provider-side rate limit). After raising, restart \`inbrain jobs work\`.`,
      };
    }
    ...
    if (bounces >= 100 && completed === 0) {
      return {
        name: 'subagent_health',
        status: 'warn',
        message: `${bounces} lease-pressure bounces in last 24h with no completed subagent jobs — cap is too tight. Raise via \`export INBRAIN_ANTHROPIC_MAX_INFLIGHT=64\` (or \`unlimited\` for upstreams with no provider-side cap).`,
      };
    }
    return {
      name: 'subagent_health',
    ...
    if (process.env.INBRAIN_DEBUG === '1') {
 ...
 *   2) Walk last 7 days of `~/.inbrain/audit/rerank-failures-*.jsonl`.
  ...
  try {
    ...
    if (authFails.length > 0) {
      return {
        name: 'reranker_health',
        status: 'warn',
        message: `${authFails.length} reranker auth failure(s) in last 7 days. Fix: verify ZEROENTROPY_API_KEY and run \`inbrain models doctor\`.`,
      };
    }
 ...
 * `~/.inbrain/audit/batch-retry-YYYY-Www.jsonl`.
 *
 ...
 * startup so bad INBRAIN_BULK_* config fails at doctor time, not first-retry.
 */
/**
 ...
 * INBRAIN_WEDGED_QUEUE_WARN_MINUTES (default 15).
 */
export async function computeWedgedQueueCheck(engine: BrainEngine): Promise<Check> {
  ...
  const thresholdMin = _resolveEnvNumber('INBRAIN_WEDGED_QUEUE_WARN_MINUTES', 15);
  try {
    ...
    return {
      ...
      message:
        ...
        `\`inbrain jobs supervisor stop && inbrain jobs supervisor start\`, ` +
        `then \`inbrain jobs retry <id>\` on any dead-lettered jobs.`,
  ...
  try {
    ...
    try {
      ...
      return {
        ...
        status: 'warn',
        message: `INBRAIN_BULK_* env override invalid: ${e instanceof Error ? e.message : String(e)}`,
      };
    }
    ...
    if (result.files_unreadable > 0) {
      return {
        name: 'batch_retry_health',
        status: 'warn',
        message: `${result.files_unreadable} audit file(s) unreadable (permission / IO). Fix: check ~/.inbrain/audit/ (or $INBRAIN_AUDIT_DIR if set).`,
      };
    }
    ...
    // operator runs `inbrain doctor` and the stack trace tells them which
    ...
    if (exhausted.length >= 20) {
      return {
        name: 'batch_retry_health',
        status: 'fail',
        message: `${exhausted.length} exhausted batch retries in last 24h (worst: ${worstSite[0]} = ${worstSite[1]}). Sustained circuit-breaker incident. Fix: check pooler status; consider raising INBRAIN_BULK_MAX_RETRIES or moving to direct-connection.${disconnectNote}`,
      };
    }
    ...
    if (worstSite[1] >= 3 || exhausted.length >= 5) {
      return {
        name: 'batch_retry_health',
        status: 'warn',
        message: `${exhausted.length} exhausted batch retries in last 24h (worst: ${worstSite[0]} = ${worstSite[1]}). Tune via INBRAIN_BULK_MAX_RETRIES / INBRAIN_BULK_RETRY_MAX_MS.${disconnectNote}`,
      };
    }
 ...
 *              sparse to fire often; fix: `inbrain extract all` to
...
export async function checkGraphSignalsCoverage(engine: BrainEngine): Promise<Check> {
  try {
    ...
    if (cfgVal !== null && cfgVal !== undefined) {
      ...
      // this, `inbrain config set search.graph_signals TRUE` enables
    ...
    if (pct < 10) {
      return {
        name: 'graph_signals_coverage',
        status: 'warn',
        message: `graph_signals enabled but only ${pctStr}% of pages have inbound links (<10%). Signal will rarely fire. Fix: \`inbrain extract all\` to populate the link graph from frontmatter + markdown.`,
      };
    }
 ...
 *
 * Surfaces three readiness signals for `inbrain brainstorm` / `inbrain lsd`:
 *
 ...
 *      fallback only). Fix: `inbrain apply-migrations --yes`.
 *
 ...
 *      Fix: `inbrain config set search.track_retrieval true`.
 *
 ...
 *      Fix: `inbrain calibration --regenerate` once enough takes are resolved.
 *
  ...
  try {
    ...
    if (!columnPresent) {
      return {
        ...
        status: 'warn',
        message: `pages.last_retrieved_at column missing. LSD stale-bias degraded to corpus-sampling. Fix: \`inbrain apply-migrations --yes\``,
      };
    }
  } catch (e) {
  ...
  try {
    ...
    if (trackCfg === 'false' || trackCfg === '0' || trackCfg === 'off' || trackCfg === 'no') {
      return {
        name: 'brainstorm_health',
        status: 'warn',
        message: `search.track_retrieval is explicitly off — LSD's stale-page signal never accumulates. Fix: \`inbrain config set search.track_retrieval true\` (or accept and use brainstorm only).`,
      };
    }
  } catch {
  ...
  try {
    ...
    if (!Array.isArray(tags) || tags.length === 0) {
      return {
        name: 'brainstorm_health',
        status: 'ok',
        message: `Migration v79 applied; tracking enabled. Calibration cold-start (no active_bias_tags) — judge runs unbiased. Fix when ready: \`inbrain calibration --regenerate\`.`,
      };
    }
    return {
      name: 'brainstorm_health',
 ...
 * `inbrain models doctor` probe covers that, and we don't want every
 * `inbrain doctor` run to spend tokens. Surfaces a paste-ready fix when the
 * key is missing.
 */
export async function checkZeEmbeddingHealth(engine: BrainEngine): Promise<Check> {
  try {
    ...
    // File plane: zeroentropy_api_key on InbrainConfig (added by C.3).
    ...
    if (!envKey && !fileKey) {
      return {
        ...
        message:
          ...
          `\`export ZEROENTROPY_API_KEY=...\` or edit ~/.inbrain/config.json ` +
          `to add "zeroentropy_api_key": "...". (inbrain config set writes the DB plane, which the embed pipeline ignores.)`,
      };
    }
    return {
      name: 'ze_embedding_health',
 ...
 * crashed, or vice versa). Surfaces a paste-ready `inbrain ze-switch
 * --resume` hint.
 */
  ...
  try {
    ...
    if (!existing.exists) {
      return {
        name: 'embedding_width_consistency',
        status: 'warn',
        message: 'content_chunks.embedding column not found. Fix: run `inbrain init --migrate-only` or check schema.',
      };
    }
    ...
    if (existing.dims !== configDim) {
      ...
      // the no-op `inbrain config set` path. The recipe is paste-ready
 ...
 * the message and paste a `inbrain search modes --reset` fix command.
 */
    ...
    if (emptySources.length > 0) {
      return {
        ...
        message:
          ...
          `If you've recently run \`inbrain import --source-id <id>\` against these, the writes may have ` +
          `silently fallen to the default source pre-v0.37.7.0. Re-run with --source-id; verify via ` +
          `\`inbrain sources current --json\`.`,
      };
    }
    return {
      name: 'source_routing_health',
  ...
  try {
    ...
    for (const m of metrics) {
      ...
      if (m.lag_seconds !== null && m.lag_seconds > 24 * 3600) {
        fails.push(`${m.source_id}: stale ${Math.floor(m.lag_seconds / 3600)}h — run \`inbrain sync trigger --source ${m.source_id}\``);
        continue;
      }
      if (m.embed_coverage_pct < 50 && m.total_chunks > 1000) {
        fails.push(`${m.source_id}: ${m.embed_coverage_pct.toFixed(1)}% embed coverage (${m.total_chunks.toLocaleString()} chunks) — run \`inbrain jobs submit embed-backfill --params '{"sourceId":"${m.source_id}"}'\``);
        continue;
      }
      // Warns
      if (m.federated && m.lag_seconds !== null && m.lag_seconds > 3600) {
        warns.push(`${m.source_id}: federated source ${Math.floor(m.lag_seconds / 3600)}h+ stale — run \`inbrain sync trigger --source ${m.source_id}\``);
      }
      if (m.embed_coverage_pct < 95 && m.total_chunks > 100) {
        warns.push(`${m.source_id}: ${m.embed_coverage_pct.toFixed(1)}% embed coverage — run \`inbrain jobs submit embed-backfill --params '{"sourceId":"${m.source_id}"}'\``);
      }
      if (m.failed_jobs_24h >= 3) {
        warns.push(`${m.source_id}: ${m.failed_jobs_24h} failures in 24h — check \`inbrain jobs list --status failed\``);
      }
    }
    ...
    if (broken.length > 0) {
      return {
        ...
        message:
          ...
          `. Fix: \`inbrain auth revoke-client <id> && inbrain auth register-client …\` for each, OR \`inbrain upgrade\` if pre-v0.37.7.0.`,
      };
    }
    return {
      name: 'oauth_confidential_client_health',
 ...
 *
 * Detects stale autopilot lockfiles. When `INBRAIN_HOME` is set, the
 * canonical lock path lives under `inbrainPath('autopilot.lock')`.
 * If a hardcoded `~/.inbrain/autopilot.lock` ALSO exists outside the
 * current `INBRAIN_HOME`, that's a pre-v0.37.7.0 leftover or a
...
export function checkAutopilotLockScope(): Check {
  try {
    const canonical = inbrainPath('autopilot.lock');
    const home = process.env.HOME || '';
    const legacy = home ? `${home}/.inbrain/autopilot.lock` : '';
    ...
    // legacy lock exists outside INBRAIN_HOME. Read its PID for a safe hint.
    ...
    return {
      ...
      status: 'warn',
      message:
        `Stale lockfile outside INBRAIN_HOME: ${legacy} (owning PID: ${owningPid}). ` +
 ...
 *
 * Surfaces every row in `inbrain_cycle_locks` whose `ttl_expires_at < NOW()`.
 ...
 * `inbrain-sync:<source>` lock-key shape so users can copy-paste the
...
export async function checkStaleLocks(
  ...
  try {
    ...
    // #1972: under `inbrain doctor --fix`, reap dead-holder sync/cycle locks
    ...
    // brain that never runs `inbrain dream` never hits the cycle-start sweep, so
    ...
    const lines = stale.slice(0, 10).map(s => {
      ...
      const source = s.id.startsWith('inbrain-sync:') ? s.id.slice('inbrain-sync:'.length) : null;
      const breakHint = source ? `inbrain sync --break-lock --source ${source}` : `inbrain sync --break-lock`;
    ...
    const msg = e instanceof Error ? e.message : String(e);
    // Pre-v0.30 brains may not have the inbrain_cycle_locks table yet.
    if (/relation .* does not exist|no such table/i.test(msg)) {
      return { name: 'stale_locks', status: 'ok', message: 'inbrain_cycle_locks table not yet provisioned (skipping)' };
    }
  ...
  try {
    ...
    if (!mode) {
      return {
        name: 'search_mode',
        status: 'ok',
        message: 'search.mode is unset (using balanced fallback). Run `inbrain search modes` to see what is running and pick a mode explicitly.',
      };
    }
    ...
    return {
      name: 'search_mode',
      status: 'ok',
      message: `Mode: ${mode} with ${overrideKeys.length} per-key override(s) (${overrideKeys.join(', ')}). To consolidate to the pure mode bundle: inbrain search modes --reset`,
    };
  } catch (e) {
    return {
      name: 'search_mode',
...
export async function checkEvalDrift(engine: BrainEngine): Promise<Check> {
  try {
    ...
    return {
      name: 'eval_drift',
      status: 'ok',
      message: `${drifted.length} retrieval-affecting file(s) changed since HEAD: ${summary}. Re-run \`inbrain eval run-all\` after committing these changes.`,
    };
  } catch (e) {
    return {
      name: 'eval_drift',
 ...
 *
 * INBRAIN_EMBEDDING_MODEL / INBRAIN_EMBEDDING_DIMENSIONS win over DB+file
...
async function checkEmbeddingEnvOverride(engine: BrainEngine): Promise<Check> {
  const envModel = process.env.INBRAIN_EMBEDDING_MODEL?.trim();
  const envDim = process.env.INBRAIN_EMBEDDING_DIMENSIONS?.trim();
  if (!envModel && !envDim) {
    return {
  ...
  if (envModel && dbModel && envModel !== dbModel) {
    mismatches.push({ key: 'INBRAIN_EMBEDDING_MODEL', env: envModel, db: dbModel });
  }
  if (envDim && dbDim && envDim !== dbDim) {
    mismatches.push({ key: 'INBRAIN_EMBEDDING_DIMENSIONS', env: envDim, db: dbDim });
  }
...
async function checkSubagentCapability(engine: BrainEngine): Promise<Check> {
  try {
      ...
      if (verdict === 'unusable:no_tools') {
        return {
          ...
          message:
            ...
            `Fix: \`inbrain config set ${source} <provider>:<model-with-tools>\` (e.g. anthropic:claude-sonnet-4-6 or openai:gpt-5.2).`,
        };
      }
      if (verdict === 'unknown') {
        return {
          ...
          message:
            ...
            `Fix: \`inbrain config set ${source} anthropic:claude-sonnet-4-6\` or pick another known provider.`,
        };
      }
      if (verdict === 'degraded:no_caching') {
        return {
          ...
          message:
            ...
            `\`inbrain config set models.tier.subagent anthropic:claude-sonnet-4-6\`.`,
        };
      }
      return null;
    };
    ...
    // require Anthropic at runtime; without the key, inbrain dream / inbrain
    // agent run / inbrain autopilot will all fail at job submission. Catches
      ...
      if (chatModel && !isAnthropicProvider(chatModel) && !process.env.ANTHROPIC_API_KEY) {
        return {
          ...
          message:
            ...
            `Subagent features (inbrain dream, inbrain agent run, inbrain autopilot) will fail at job submission ` +
            `unless agent.use_gateway_loop=true. Chat alone (inbrain think) still works. ` +
            `Either set ANTHROPIC_API_KEY or enable: \`inbrain config set agent.use_gateway_loop true\`.`,
        };
      }
...
export function _resolveEnvNumber(varName: string, fallback: number, opts?: { unit?: string }): number {
  ...
  if (!Number.isFinite(n) || n <= 0) {
    if (!_envNumberWarned.has(varName)) {
      ...
      console.warn(
        `[inbrain doctor] Ignoring invalid ${varName}=${raw}; using default ${fallback}${opts?.unit ?? ''}.`,
      );
    }
    return fallback;
  }
 ...
 * been synced recently. Detects the silent failure mode where `inbrain sync`
 ...
 *     INBRAIN_DRIFT_LIMIT + INBRAIN_DRIFT_TIMEOUT_MS guards.
 *
 * Thresholds (env-overridable, default = 24h warn / 72h fail):
 *   - INBRAIN_SYNC_FRESHNESS_WARN_HOURS
 *   - INBRAIN_SYNC_FRESHNESS_FAIL_HOURS
 ...
 * `inbrain sync --source <id>` matches what the user copy-pastes.
 */
/**
...
export function computeNightlyQualityProbeHealthCheck(
  ...
  if (!probeEnabled && events.length === 0) {
    ...
    return {
      name,
      status: 'ok',
      message: `disabled (opt-in). Enable with: inbrain config set autopilot.nightly_quality_probe.enabled true`,
    };
  }
  if (events.length === 0) {
    return {
  ...
  try {
    ...
    if (!enabled) {
      return {
        ...
        status: 'ok',
        message:
          'disabled (opt-in). Enable with: inbrain config set cycle.conversation_facts_backfill.enabled true',
      };
    }
    ...
    if (backlog > 10) {
      const fixHint =
        'inbrain extract-conversation-facts --background --max-cost-usd 5';
      return {
        name,
        status: 'warn',
 ...
 * via INBRAIN_EXTRACTION_LAG_FAIL_PCT — so a just-upgraded 280K-page brain
 * (every page NULL → 100% stale) gets a loud WARN, never a non-zero exit that
 * would break a CI/cron pipeline gating on `inbrain doctor`.
 *
...
export async function checkLinksExtractionLag(
  ...
  const fix = "Run: inbrain extract --stale";
  try {
    ...
    const warnPct = _resolveEnvNumber('INBRAIN_EXTRACTION_LAG_WARN_PCT', EXTRACTION_LAG_WARN_PCT_DEFAULT, { unit: '%' });
    ...
    const failRaw = process.env.INBRAIN_EXTRACTION_LAG_FAIL_PCT;
    if (failRaw !== undefined && failRaw !== '') {
      const n = Number(failRaw);
      if (Number.isFinite(n) && n > 0) {
        failPct = n;
      } else if (!_envNumberWarned.has('INBRAIN_EXTRACTION_LAG_FAIL_PCT')) {
        _envNumberWarned.add('INBRAIN_EXTRACTION_LAG_FAIL_PCT');
        console.warn(`[inbrain doctor] Ignoring invalid INBRAIN_EXTRACTION_LAG_FAIL_PCT=${failRaw}; hard-fail stays disabled.`);
      }
    }
...
export async function computeExtractAtomsBacklogCheck(
  ...
  try {
    ...
    if (!declared && backlog > 10) {
      const fix = 'inbrain dream --phase extract_atoms --drain --window 120 (or declare extract_atoms in your active schema pack)';
      return {
        name, status: 'warn',
  ...
  try {
    ...
    if (totalRollupFailures > 0) {
      return {
        name,
        status: 'warn',
        message: `${totalRollupFailures} rollup write failure(s) in last 7d (audit JSONL is source of truth; rebuild via inbrain extract status --rebuild-rollup)`,
        details: {
          schema_version: 1,
          kinds,
          rollup_write_failures_7d: totalRollupFailures,
...
export async function checkSyncFreshness(
  ...
  try {
    ...
    // `inbrain sync`'s up-to-date predicate at sync.ts:1057+1075 checks.
    ...
    const warnHours = _resolveSyncFreshnessHours('INBRAIN_SYNC_FRESHNESS_WARN_HOURS', 24);
    const failHours = _resolveSyncFreshnessHours('INBRAIN_SYNC_FRESHNESS_FAIL_HOURS', 72);
    ...
    for (const source of sources) {
      // Embed source.id in user-visible messages so `inbrain sync --source <id>`
    ...
    if (hasFailures) {
      return {
        name: 'sync_freshness',
        status: 'fail',
        message: `${issues.join('; ')}. Run \`inbrain sync --source <id>\` for each stale source`,
        details,
      };
    }
    if (hasWarnings) {
      return {
        name: 'sync_freshness',
        status: 'warn',
        message: `${issues.join('; ')}. Run \`inbrain sync --source <id>\` to refresh`,
        details,
      };
    }
 ...
 *
 * Background: `inbrain sync --all --parallel N --workers N --skip-failed`
 ...
 * inside `inbrain doctor` so the operator sees it whenever they run a
  ...
  try {
    ...
    return {
      ...
      message:
        ...
        '`inbrain sync --all --parallel 4 --workers 4 --skip-failed`. ' +
...
/**
 * v0.42.x (#1794, 4A) — pure pool-budget check. When `INBRAIN_MAX_CONNECTIONS`
 ...
 * other inbrain process competes for the same cap — the operator should lower
 * `INBRAIN_POOL_SIZE`. Pure so it's unit-testable without env/engine.
 */
export function computePoolBudgetCheck(
  ...
  if (maxConnections === undefined) {
    return {
      name: 'pool_budget',
      status: 'ok',
      message: 'INBRAIN_MAX_CONNECTIONS not set — connection budget clamp disabled (default behavior).',
    };
  }
  if (parentPool + perWorkerPool > maxConnections) {
    return {
      ...
      status: 'warn',
      message:
        `INBRAIN_MAX_CONNECTIONS=${maxConnections} leaves no room for a parallel sync worker ` +
        ...
        '`inbrain config` / set INBRAIN_POOL_SIZE=2 (recommended for low-cap poolers like Supabase Supavisor).',
    };
  }
  ...
  return {
    ...
    status: 'ok',
    message:
      `INBRAIN_MAX_CONNECTIONS=${maxConnections}: room for up to ${maxWorkers} parallel sync ` +
 ...
 *   - INBRAIN_CYCLE_FRESHNESS_WARN_HOURS (default 6)
 *   - INBRAIN_CYCLE_FRESHNESS_FAIL_HOURS (default 24)
 */
export async function checkCycleFreshness(
  ...
  try {
    ...
    const warnHours = _resolveSyncFreshnessHours('INBRAIN_CYCLE_FRESHNESS_WARN_HOURS', 6);
    const failHours = _resolveSyncFreshnessHours('INBRAIN_CYCLE_FRESHNESS_FAIL_HOURS', 24);
    const warnMs = warnHours * 60 * 60 * 1000;
    const failMs = failHours * 60 * 60 * 1000;
    const now = opts?.nowMs ?? Date.now();
    ...
    if (hasFailures) {
      return {
        name: 'cycle_freshness',
        status: 'fail',
        message: `${issues.join('; ')}. Run \`inbrain dream --source <id>\` for each stale source, or start \`inbrain autopilot\`.`,
      };
    }
    if (hasWarnings) {
      return {
 ...
 */
/**
 * Build the full check list for `inbrain doctor` against an engine + arg vector.
 *
 ...
 *
 * One `inbrain doctor` line replaces the hours of log archaeology the #1678
 ...
 *   - BARE `inbrain jobs work`: NO supervisor event is written; the only trace is
...
export async function computeWorkerOomLoopCheck(
  ...
  const fixHint =
    'raise --max-rss (inbrain jobs work --max-rss <bigger>; auto-sizes to min(0.5×RAM,16GB))';
...
export async function buildChecks(
  ...
  // silently scope a bare `inbrain doctor` to one source; we want bare doctor to
  ...
  // github:inbrain-ai/inbrain && cd ~ && inbrain doctor`) can still find the
  ...
  if (scope === 'all' && skillsDir) {
    ...
    // to SKILL.md files; a user running `cd ~ && inbrain doctor --fix`
    // without an explicit signal would have install_path resolve to the
    // bundled inbrain repo and silently rewrite the install-tree skills.
    // Codex caught this leak in the v0.31.7 ship review (D6 lock).
    if (doFix) {
      if (detected.source === 'install_path') {
        process.stderr.write(
          'inbrain doctor --fix refused: skills dir resolved via install-path fallback (read-only).\n' +
          'The --fix flag writes to SKILL.md files; running it against the bundled install\n' +
          'tree would silently mutate inbrain itself. Set $INBRAIN_SKILLS_DIR, $OPENCLAW_WORKSPACE,\n' +
  ...
  //
  // Audit trail: snapshot+diff at ~/.inbrain/audit/skill-brain-first-
  ...
  // `inbrain apply-migrations --yes` afterward. This check fires on every
  // `inbrain doctor` invocation so your OpenClaw's health skill catches it.
  //
  ...
  try {
    ...
    // `inbrain apply-migrations --force-retry <v>` once before plain --yes
    ...
    if (wedged.length > 0) {
      ...
      checks.push({
        name: 'minions_migration',
        status: 'fail',
        message: `MINIONS HALF-INSTALLED (partial migration: ${stuck.join(', ')}). Run: inbrain apply-migrations --yes`,
      });
    }
  ...
  // 3b. Upgrade-error trail (v0.13+). `inbrain upgrade` silently swallows
  // best-effort failures in `inbrain post-upgrade`; the failure record is
  // appended to ~/.inbrain/upgrade-errors.jsonl so we can surface it here
  // with a paste-ready recovery hint. Without this, users end up with
  // half-upgraded brains and no signal.
  try {
    const home = process.env.HOME || '';
    const errPath = join(home, '.inbrain', 'upgrade-errors.jsonl');
  ...
  // Reads the default PID file (`~/.inbrain/supervisor.pid` unless the user
  // overrode with INBRAIN_SUPERVISOR_PID_FILE) and the latest audit file
  ...
  try {
    ...
    // Shared classifier — same code path runs in `inbrain jobs supervisor
    ...
    if (supervisorPid !== null || events.length > 0) {
      if (maxCrashesEvent) {
        checks.push({
          name: 'supervisor',
          status: 'fail',
          message: `Supervisor gave up at ${maxCrashesEvent.ts} (max_crashes_exceeded). Restart with: inbrain jobs supervisor start --detach`,
        });
      } else if (!running && events.length > 0) {
        checks.push({
          name: 'supervisor',
          status: 'warn',
          message: `Supervisor not running (last_start=${lastStart ?? 'unknown'}). Restart with: inbrain jobs supervisor start --detach`,
        });
        ...
        checks.push({
          name: 'supervisor',
          status: 'warn',
          message: `Worker crashed ${crashes24h}x in last 24h (${causeStr}). Check ~/.inbrain/audit/supervisor-*.jsonl for context.`,
        });
      } else {
        checks.push({
          name: 'supervisor',
    ...
    if (lastStarted && engine) {
      ...
      const lockRows = await engine.executeRaw<{ holder_pid: number; holder_host: string; live: boolean }>(
        `SELECT holder_pid, holder_host, ttl_expires_at > now() AS live
           FROM inbrain_cycle_locks WHERE id = $1`,
      ...
      if (verdict === 'mismatch') {
        checks.push({
          ...
          message:
            ...
            `and keep a single supervisor per queue: inbrain jobs supervisor stop.`,
  ...
  // ~/.inbrain/audit/stub-guard-YYYY-Www.jsonl. This check is the operator
    ...
    if (events.length > 10) {
      ...
      checks.push({
        ...
        message:
          ...
          `missing a case. Check ~/.inbrain/audit/stub-guard-*.jsonl for the slugs ` +
          `that hit it.`,
      });
  ...
  // to ~/.inbrain/sync-failures.jsonl with the commit hash + exact error.
  ...
  try {
    ...
    if (unresolved.length > 0) {
      ...
      // run `inbrain sync --skip-failed` manually.
      ...
      checks.push({
        ...
        message:
          ...
          `Fix the file(s) and re-run 'inbrain sync', or use 'inbrain sync --skip-failed' to acknowledge.`,
  ...
  if (engine) {
    try {
      ...
      // Wire a remediation step on WARN so `inbrain doctor --remediate`
  ...
  // `inbrain conversation-parser scan <slug>` so the operator can
        ...
        if (unmatchedPct > 10) {
          checks.push({
            ...
            message:
              ...
              `Breakdown: ${breakdown}. ` +
              `Investigate: inbrain conversation-parser scan <slug> | ` +
              `Enable LLM fallback (opt-in): inbrain config set conversation_parser.llm_fallback_enabled true`,
          });
        } else {
          checks.push({
            name: 'conversation_format_coverage',
  ...
  // days of `~/.inbrain/audit/progressive-batch-YYYY-Www.jsonl` and
    ...
    if (aborts.length === 0) {
      ...
      checks.push({
        ...
        message:
          ...
          `Breakdown: ${breakdown}. ` +
          `Inspect: cat ~/.inbrain/audit/progressive-batch-*.jsonl | jq 'select(.verdict != "proceed")'`,
      });
    }
  } catch (err) {
    checks.push({
  ...
  checks.push({
    ...
    message:
      ...
      '`inbrain config set autopilot.conversation_parser_probe.enabled true`)',
  });

  // 3e. home_dir_in_worktree (v0.35.8.0). Walks up from `inbrainPath()`
  // looking for a `.git` directory OR file. If found, warns: `~/.inbrain/`
  ...
  // `~/.inbrain/.gitignore` (single-line `*`) laid down by saveConfig +
  ...
  // who set INBRAIN_HOME=/tmp/something). Handles `.git` as both a directory
  // (main repo) and a file (linked worktree pointing at parent's worktrees/).
  // Honors INBRAIN_HOME via inbrainPath().
  try {
    const inbrainHome = inbrainPath();
    const home = process.env.HOME || '';
    let worktreeRoot: string | null = null;
    if (inbrainHome && home && inbrainHome.startsWith(home + '/')) {
      // Walk up from inbrainHome's parent toward $HOME, stopping at $HOME.
      // We don't check inbrainHome itself: a `.git` directly inside ~/.inbrain
      // isn't a containing-worktree, it would be a brain repo cloned there.
      let cur = dirname(inbrainHome);
    ...
    if (worktreeRoot) {
      const homeEnvHint = process.env.INBRAIN_HOME
        ? `# Or move \`~/.inbrain\` outside the worktree by setting INBRAIN_HOME elsewhere.`
        : `# Fix: \`export INBRAIN_HOME=/some/path/outside/the/worktree\` (inbrain appends \`.inbrain\`).`;
      checks.push({
        name: 'home_dir_in_worktree',
        status: 'warn',
        message:
          `~/.inbrain lives inside git worktree at ${worktreeRoot}. ` +
          `Config + brain DB could be committed by accident. ` +
          `A retroactive ~/.inbrain/.gitignore blocks casual \`git add\`, but does NOT cover ` +
      ...
      checks.push({
        name: 'home_dir_in_worktree',
        status: 'ok',
        message: 'inbrain home is outside any enclosing git worktree.',
      });
    }
  } catch {
      ...
      if (result.walk_truncated) {
        checks.push({
          ...
          message:
            ...
            `Re-run on a quieter brain or shorter walk via INBRAIN_DRIFT_LIMIT/INBRAIN_DRIFT_TIMEOUT_MS.`,
        });
      } else if (result.count > 0) {
        const sampleStr = result.sample.map(s => `${s.slug} (intended=${s.intended_source})`).join(', ');
        checks.push({
          ...
          message:
            ...
            `Verify with 'inbrain sources status', then either re-sync with ` +
            `'inbrain sync --source <id> --full' or 'inbrain delete <slug>' if the default-source ` +
            `row is the misroute. (A 'inbrain sources rehome' cleanup command is tracked for v0.32.0.)`,
        });
      } else {
        checks.push({
          name: 'multi_source_drift',
  ...
  // 3c. Orphan clone temp dirs (v0.28 P1). `inbrain sources add --url` clones
  // into $INBRAIN_HOME/clones/.tmp/<id>-<rand>/ and renames atomically; if the
  ...
  try {
    ...
    const tmpRoot = cfg.inbrainPath('clones', '.tmp');
      ...
      if (stale.length === 0) {
        ...
        checks.push({
          ...
          message:
            ...
            `. Run \`inbrain sources purge-orphan-clones\` or wait for the autopilot purge phase.`,
        });
      }
    }
  } catch {
  ...
  if (fastMode || !engine) {
    if (!engine) {
      ...
      if (fastMode && dbSource) {
        ...
        msg = 'No database configured (filesystem checks only). Set INBRAIN_DATABASE_URL or run `inbrain init`.';
      }
        ...
        if (parsed.port === '6543') {
          checks.push({
            ...
            message:
              ...
              'Fix: unset INBRAIN_PREPARE (or set =false), or add ?prepare=false to the connection URL.',
          });
        }
      } catch {
  ...
  // 5. RLS — check ALL public tables, not just inbrain's own.
  ...
  //
  //     INBRAIN:RLS_EXEMPT reason=<non-empty reason>
  //
  ...
  // `inbrain rls-exempt add` CLI on purpose. Doctor re-enumerates the
  ...
  if (engine.kind === 'pglite') {
    ...
    try {
      ...
      `;
      const EXEMPT_RE = /^INBRAIN:RLS_EXEMPT\s+reason=\S.{3,}/;
      const exempt: string[] = [];
      const gaps: string[] = [];
      ...
      if (gaps.length === 0) {
        ...
        checks.push({
          ...
          message:
            ...
            `Fix: ${fixes} ` +
            `If a table should stay readable by the anon key on purpose, see docs/guides/rls-and-you.md for the INBRAIN:RLS_EXEMPT comment escape hatch.`,
        });
      }
    } catch {
  ...
  try {
    ...
    if (schemaVersion >= LATEST_VERSION) {
      ...
      checks.push({
        name: 'schema_version',
        status: 'fail',
        message: `No schema version recorded. Migrations never ran. Fix: inbrain apply-migrations --yes. ` +
                 `If you installed via 'bun install -g github:...', see https://github.com/inbrain-ai/inbrain/issues/218.`,
      });
    } else {
      checks.push({
        name: 'schema_version',
        status: 'warn',
        message: `Version ${schemaVersion}, latest is ${LATEST_VERSION}. Fix: inbrain apply-migrations --yes`,
      });
    }
  } catch {
    checks.push({ name: 'schema_version', status: 'warn', message: 'Could not check schema version' });
  ...
  // That's a valid fresh-install state after `inbrain init` — the migration
    ...
    try {
      ...
      if (rows.length === 0) {
        checks.push({
          ...
          status: 'warn',
          message:
            'Auto-RLS event trigger missing. New tables created outside inbrain may not get RLS. ' +
            'Fix: inbrain apply-migrations --force-retry 35',
        });
  ...
  try {
    ...
    if (health.embed_coverage >= 0.9) {
      ...
      checks.push({ name: 'embeddings', status: 'warn', message: `${pct}% coverage, ${health.missing_embeddings} missing. Run: inbrain embed --stale` });
    } else {
      checks.push({ name: 'embeddings', status: 'warn', message: 'No embeddings yet. Run: inbrain embed --stale' });
    }
  } catch {
  ...
  try {
    ...
    //   - empty brain (no embedded chunks) → `inbrain init --force --embedding-model …`
    //   - non-empty brain → `inbrain retrieval-upgrade --to … --reindex`
    // The bug-reporter's `rm -rf ~/.inbrain` recovery is never the right answer.
        ...
        if (colDim.exists && colDim.dims !== null && colDim.dims !== configuredDims) {
          ...
          if (totalChunks > 0) {
            const fix = embeddedCount === 0
              ? `No embeddings yet — drop the empty schema and re-init at the right dim:\n        inbrain init --force --pglite --embedding-model ${configuredModel} --embedding-dimensions ${configuredDims}`
              : `Non-empty brain (${embeddedCount} embedded chunks). Migrate cleanly:\n        inbrain retrieval-upgrade --to ${configuredModel} --reindex`;
            ...
            checks.push({
              ...
              message:
                ...
                `Persist your provider choice with \`inbrain config set embedding_model ${configuredModel}\` AND fix the schema:\n      ${fix}`,
            });
    ...
    if (surfacedUnconfiguredDrift) {
      ...
      const vec = await embedOne('inbrain doctor embedding smoke test');
  ...
  try {
    ...
    if (alternatives.length > 0) {
      checks.push({
        ...
        status: 'ok',
        message: `Detected ${alternatives.length} alternative embedding provider${alternatives.length > 1 ? 's' : ''} ready to use: ${alternatives.join(', ')}. Run \`inbrain providers list\` to switch.`,
      });
    }
        ...
        if (actualType !== entry.type) {
          issues.push(
            `${colName}: declared type=${entry.type} but actual is ${actual}. ` +
              `Fix: inbrain config set embedding_columns '<JSON>' OR ` +
      ...
      if (activeCol && actualByName.has(activeCol)) {
        ...
        if (total > 0 && pct < 90) {
          coverageWarn =
            ...
            `Fix: inbrain embed --column ${activeCol} --stale (write-side support v2) ` +
            `OR inbrain config set search_embedding_column embedding`;
        }
      }
  ...
  try {
    ...
    if (entityCount === 0) {
      ...
      checks.push({
        name: 'graph_coverage',
        status: 'warn',
        message: `Entity link coverage ${linkPct}%, timeline ${timelinePct}% (${entityCount} entity pages). Run: inbrain extract all`,
      });
    }
  ...
  // `inbrain orphans --count` (D1), so the two surfaces cannot disagree.
  //
  ...
  // `inbrain extract links --by-mention` as the fix.
    ...
    if (entityCount < 100 && !srcId) {
      ...
      const caveat =
        entityCount < 100
          ...
          : '';
      const hint =
        'Run: inbrain extract links --by-mention   (auto-links entity mentions in body text). ' +
        'Run inbrain orphans for the list.';
      if (ratio > 0.8) {
        checks.push({
          name: 'orphan_ratio',
          status: 'fail',
  ...
  // warning. Full-brain scan: `inbrain integrity check`.
  ...
  try {
    ...
    if (total === 0) {
      ...
      // Three-bucket repair handled by `inbrain integrity auto` (the
      ...
      checks.push({
        name: 'integrity',
        status: 'warn',
        message: `Sampled ${res.pagesScanned} pages; ${res.bareHits.length} bare-tweet phrase(s), ${res.externalHits.length} external link(s). Run: inbrain integrity check (or integrity auto to repair).`,
  ...
  try {
    ...
    if (totalBad === 0) {
      ...
      checks.push({
        ...
        status: 'warn',
        message: `${totalBad} row(s) double-encoded (${breakdown.join(', ')}). Fix: inbrain repair-jsonb`,
      });
    }
  } catch {
  ...
  // gate itself runs via `inbrain eval whoknows`; this check is the
  ...
  // surface the gap so operators can run `inbrain backfill modality`.
      ...
      if (unifiedOnlyOn && lowestCoverage < 0.99) {
        checks.push({
          ...
          message:
            ...
            `Run \`inbrain reindex --multimodal\` to bring coverage to 99%+ or disable strict mode.`,
        });
        ...
        checks.push({
          ...
          message:
            ...
            `Run \`inbrain reindex --multimodal\` to fill the gap.`,
        });
      } else {
        checks.push({
          name: 'unified_multimodal_coverage',
  ...
  try {
    ...
    if (rows.length === 0) {
      ...
      checks.push({
        ...
        status: 'warn',
        message: `${rows.length} page(s) appear truncated (sample: ${sample}). Re-import with: inbrain sync --force`,
      });
    }
  } catch {
  ...
  // - content_sanity_audit_recent: reads ~/.inbrain/audit/content-sanity-*.jsonl
  //   over the last 7 days, aggregates by event type + source. Caveat
  //   (Codex r1 #14): JSONL is local-only — multi-host operators should
  //   share INBRAIN_AUDIT_DIR. Message names this so the limitation is
  ...
  try {
    ...
    if (events.length === 0) {
      checks.push({
        ...
        status: 'ok',
        message: 'No content-sanity events in last 7 days (audit JSONL is local to this host; share INBRAIN_AUDIT_DIR for multi-host visibility)',
      });
    } else {
      ...
      checks.push({
        name: 'content_sanity_audit_recent',
        status,
        message: `${events.length} events (hard=${summary.by_type.hard_block} soft=${summary.by_type.soft_block} warn=${summary.by_type.warn})${topPatterns ? ', patterns: ' + topPatterns : ''}${topSources ? ', sources: ' + topSources : ''}. (Local audit only — multi-host operators set INBRAIN_AUDIT_DIR.)`,
      });
    }
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
  ...
  try {
    ...
    checks.push({
      ...
      status: n > 0 ? 'warn' : 'ok',
      message: n > 0
        ? `${n} page(s) quarantined as junk (hidden from search). Review with 'inbrain quarantine list'; clear a false positive with 'inbrain quarantine clear <slug>'.`
  ...
  try {
    ...
    checks.push({
      ...
      status: n > 0 ? 'warn' : 'ok',
      message: n > 0
        ? `${n} page(s) flagged (markup-heavy or oversize) — still searchable, agent warned on retrieval. Review with 'inbrain quarantine list --include-flagged'.`
        : 'No flagged pages',
    });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
  ...
  // `inbrain frontmatter validate <source-path> --fix`, which writes .bak
  ...
  //    JSON consumer.
  //  - Configurable via INBRAIN_DOCTOR_FM_TIMEOUT_MS (default 30000ms).
  progress.heartbeat('frontmatter_integrity');
  const fmHb = startHeartbeat(progress, 'scanning frontmatter…');
  const fmTimeoutMs = (() => {
    const raw = process.env.INBRAIN_DOCTOR_FM_TIMEOUT_MS;
    ...
    if (report.total === 0 && !report.partial) {
      ...
      for (const src of report.per_source) {
        if (src.status === 'skipped') {
          // Codex adversarial #1: `inbrain frontmatter validate` takes a
          ...
          sourceMessages.push(
            `${src.source_id}: NOT SCANNED (timeout — run \`inbrain frontmatter validate ${src.source_path}\`)`,
          );
          continue;
        }
      ...
      const fixHint = report.partial
        ? `Raise INBRAIN_DOCTOR_FM_TIMEOUT_MS or run \`inbrain frontmatter validate <source>\` directly. Fix issues: \`inbrain frontmatter validate <source> --fix\``
        : `Fix: inbrain frontmatter validate <source-path> --fix`;
      checks.push({
        name: 'frontmatter_integrity',
        status: 'warn',
        message:
  ...
  // can see drops cross-process (the MCP server captures; `inbrain doctor`
    ...
    if (failures.length === 0) {
      ...
      checks.push({
        ...
        message: `${failures.length} capture failure(s) in the last 24h (${breakdown}). ` +
          ...
          `in ~/.inbrain/config.json to silence.`,
      });
    }
  } catch (err) {
  ...
  try {
    ...
    if (recent.length === 0) {
      checks.push({
        name: 'contradictions',
        status: 'ok',
        message: 'No probe runs in the last 7 days. Run `inbrain eval suspected-contradictions --query "..." --top-k 5` to populate.',
      });
    } else {
      ...
      if (total === 0) {
        ...
        if (highFindings.length > 3) {
          lines.push(`  …and ${highFindings.length - 3} more — see \`inbrain eval suspected-contradictions review\``);
        }
        checks.push({
          name: 'contradictions',
  ...
  try {
      ...
      checks.push({
        ...
        message: anyOverThreshold
          ? `Facts:absorb failures over the threshold (${threshold}) in the last 24h: ${summary}. ` +
            `Run \`inbrain recall --since 24h --json\` to inspect what landed; ` +
            `tune the gate via \`inbrain config set facts.absorb_warn_threshold N\`.`
    ...
    if (code === '42P01' || code === '42703') {
      ...
      checks.push({
        name: 'facts_extraction_health',
        status: 'ok',
        message: 'Skipped (ingest_log.source_id unavailable — run `inbrain apply-migrations --yes`).',
      });
    } else if (code === '42501') {
      checks.push({
        name: 'facts_extraction_health',
  ...
  try {
    ...
    if (fallbackWithFm > 0 || future > 0 || pre1990 > 0) {
      ...
      checks.push({
        ...
        status: 'warn',
        message: `${parts.join('; ')} (sample of last 1000 pages). Run \`inbrain reindex-frontmatter\` to recompute.`,
      });
    } else {
      checks.push({
        name: 'effective_date_health',
    ...
    if (code === '42703') {
      ...
      checks.push({ name: 'effective_date_health', status: 'ok', message: 'Skipped (effective_date column unavailable — run inbrain apply-migrations)' });
    } else {
  ...
  try {
    ...
    if (zeroWithTakes > 0) {
      checks.push({
        ...
        status: 'warn',
        message: `${zeroWithTakes} pages with active takes have emotional_weight=0. Run \`inbrain dream --phase recompute_emotional_weight\` to populate. Brain has ${nonzero} pages with non-zero emotional_weight.`,
      });
    } else if (nonzero === 0) {
      checks.push({
        name: 'salience_health',
  ...
  //      Surface the ID so the operator can `inbrain jobs get <id>` to inspect
  //      or `inbrain jobs cancel <id>` to force-kill.
  //
  //   2. backpressure-missed: per-name waiting depth exceeds the threshold
  //      (default 10, override via INBRAIN_QUEUE_WAITING_THRESHOLD env). Signal
  ...
  if (engine.kind === 'pglite') {
    ...
    try {
      ...
      const rawThreshold = process.env.INBRAIN_QUEUE_WAITING_THRESHOLD;
      ...
      if (stalledRows.length > 0) {
        ...
        problems.push(
          ...
          `Fix: inbrain jobs get <id> to inspect; inbrain jobs cancel <id> to force-kill.`
        );
      }
      if (depthRows.length > 0) {
        ...
        problems.push(
          ...
          `Fix: set maxWaiting on the submitter (or raise INBRAIN_QUEUE_WAITING_THRESHOLD).`
        );
      }
      if (rssKillCount > 0) {
        problems.push(
          `${rssKillCount} job(s) dead-lettered for RSS-watchdog memory-limit kills in last 24h. ` +
          `Fix: raise the limit (e.g. \`inbrain jobs work --max-rss 4096\`) or opt out (\`--max-rss 0\`). ` +
      ...
      if (promptTooLongCount > 0) {
        problems.push(
          ...
          `Fix: \`inbrain dream --phase synthesize --dry-run --json\` to identify fat transcripts; ` +
      ...
      if (rows.length === 0) {
        ...
        checks.push({
          ...
          message: `${vanished} of ${rows.length} image(s) missing from disk (e.g. ${vanishedPaths.join(', ')}). ` +
                   `Fix: restore from git, or \`inbrain sync --skip-failed\` to acknowledge.`,
        });
      }
    } catch {
  ...
  if (engine !== null) {
    ...
    // v0.41.6.0 D3 — stale_locks (inbrain_cycle_locks rows with ttl_expires_at < NOW())
...
/**
 * CLI entry point for `inbrain doctor`. Thin wrapper around buildChecks +
...
export function skillBrainFirstCheck(skillsDir: string): Check {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    process.stderr.write(`[inbrain] skill_brain_first audit step failed (${msg}); check continues\n`);
  }
  ...
  summary.push(
    ...
    `Fix via 'inbrain doctor --fix' (adds canonical Convention callout) ` +
  ...
  if (formerlyExempt.length > 0) {
    summary.push(
      ...
      `These need explicit opt-out now: run 'inbrain doctor --fix' to add the canonical callout, ` +
...
/**
 * `inbrain doctor --locks` — list idle-in-transaction backends older
...
async function runLocksCheck(engine: BrainEngine | null, jsonOutput: boolean): Promise<void> {
  if (!engine) {
    if (jsonOutput) {
      ...
      console.log('inbrain doctor --locks requires a database connection. Configure a URL and retry.');
    }
    process.exit(1);
  }
  ...
  if (engine.kind !== 'postgres') {
    if (jsonOutput) {
      ...
      console.log(`inbrain doctor --locks is Postgres-only. Current engine: ${engine.kind}. No blockers possible (no connection pool).`);
    }
    return;
  }
  ...
  console.log('After terminating, retry: inbrain apply-migrations --yes');
  process.exit(1);
}
    ...
    },
    {
      ...
      onBudgetExhausted: (planHash, snapshot) => {
        console.error(
          ...
          `  inbrain doctor --remediate --resume ${planHash}\n`,
        );
      },
    },
  );
...
async function checkSchemaPackActive(engine: BrainEngine): Promise<Check> {
  try {
    ...
    return {
      name: 'schema_pack_active',
      status: 'warn',
      message: `Active pack failed to resolve: ${(e as Error).message}. Run \`inbrain schema active\` to debug.`,
    };
  }
}
  ...
  try {
    ...
    if (worstPct >= 0.1) {
      return {
        ...
        status: 'warn',
        message: `Source \`${worstSrc}\`: ${worstUntyped} of ${worstTotal} pages (${pctStr}%) have no type matching the active pack. Run \`inbrain schema detect --source ${worstSrc}\` to propose a pack matching your content shape.`,
      };
    }
    return {
      name: 'schema_pack_consistency',
...
async function checkSchemaPackSourceDrift(engine: BrainEngine): Promise<Check> {
  try {
    ...
    return {
      ...
      status: 'warn',
      message: `Per-source pack divergence detected: ${distinctPacks.size} distinct packs across ${rows.length} sources. Run \`inbrain sources list\` then \`inbrain schema active --source <id>\` per source to audit.`,
    };
  } catch (e) {
    return {
      name: 'schema_pack_source_drift',
