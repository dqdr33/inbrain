/**
 * v0.29.1 — `inbrain reindex-frontmatter`.
 *
 ...
 *     column refreshed without a full `inbrain sync`.
...
export interface ReindexFrontmatterOpts {
   ...
   * `inbrain reindex --workers N` surfaces but currently INFORMATIONAL
...
export async function reindexFrontmatterCli(args: string[]): Promise<void> {
  ...
  if (!cfg) {
    console.error('No inbrain config; run `inbrain init` first.');
    process.exit(1);
  }
