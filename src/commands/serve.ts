...
export async function runServe(
  ...
  // that used `inbrain auth create` keep working unchanged).
  ...
  if (isHttp) {
    ...
    // (privacy-first); operators running inbrain on their own laptop can flip
    ...
    // INBRAIN_ADMIN_BOOTSTRAP_TOKEN for production deployments that don't
  ...
  console.error('Starting Inbrain MCP server (stdio)...');
...
function installStdioLifecycle(
  ...
  const beginShutdown = (reason: string): void => {
    ...
    deps.log(`Inbrain MCP server: graceful exit (${reason})`);
    ...
    const deadline = setTimeout(() => {
      deps.log(
        `Inbrain MCP server: cleanup deadline (${CLEANUP_DEADLINE_MS}ms) exceeded — forcing exit`,
      );
      deps.exit(0);
    }, CLEANUP_DEADLINE_MS);
    deadline.unref?.();
    ...
    Promise.resolve()
      ...
      .catch((err: unknown) => {
        ...
        deps.log(`Inbrain MCP server: cleanup error: ${msg}`);
      })
      .finally(() => {
  ...
  // Skip when stdin is a TTY: interactive `inbrain serve` use shouldn't
  ...
  if (initialParentPid !== 1) {
    if (!deps.probeWatchdog()) {
      deps.log(
        '[inbrain serve] watchdog disabled: ps unavailable, parent-death detection unavailable — child will rely on stdin EOF / signals only',
      );
    } else {
  ...
  if (idleTimeoutSec > 0) {
    ...
    deps.log(`Inbrain MCP server: stdio idle timeout = ${idleTimeoutSec}s`);
  }
}
