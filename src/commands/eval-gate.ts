/**
 * inbrain eval gate — fail CI on retrieval regressions OR correctness drops (v0.41).
 *
 ...
 * the inbrain-version-drift bug class for source-tree CI runs.
 *
...
function printHelp(): void {
  console.log(`inbrain eval gate — fail CI on retrieval regressions or correctness drops

Usage:
  inbrain eval gate --baseline X.baseline.ndjson [flags]      # regression-only
  inbrain eval gate --qrels Y.qrels.json [flags]              # correctness-only
  inbrain eval gate --baseline X --qrels Y [flags]            # both required

Required (at least one):
  --baseline FILE              Baseline NDJSON from \`inbrain bench publish\`
