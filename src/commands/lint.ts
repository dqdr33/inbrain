/**
 * inbrain lint — Deterministic brain page quality checker.
 *
 ...
 *
 * Usage:
 *   inbrain lint <dir>              # report issues
 *   inbrain lint <dir> --fix        # auto-fix what's fixable
 *   inbrain lint <dir> --fix --dry-run  # preview fixes
 *   inbrain lint <file.md>          # lint single file
 */
...
import { loadConfig, loadConfigWithEngine, inbrainPath } from '../core/config.ts';
...
/** Codes whose lint findings are fixable by `inbrain frontmatter validate --fix`. */
 ...
 * the file/env values. CI without `~/.inbrain/` falls through
 * immediately since `loadConfig()` returns minimal config.
 *
 * Also loads the operator literals file (`~/.inbrain/junk-substrings.txt`)
...
async function resolveLintContentSanity(
  ...
  if (sharedEngine) {
    ...
    // Standalone path (CLI `inbrain lint`, which is CLI_ONLY and shares no
...
export async function runLint(args: string[]) {
  ...
  if (!target) {
    console.error('Usage: inbrain lint <dir|file.md> [--fix] [--dry-run]');
