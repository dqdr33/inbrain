/**
 * inbrain frontmatter install-hook — Install a pre-commit hook in a brain
 * source's git repo that runs `inbrain frontmatter validate` against staged
 ...
 *   - When the inbrain binary is missing, the hook prints a one-line warning
 *     and exits 0 (don't break commits if a developer uninstalls inbrain).
...
const HOOK_BANNER = '# inbrain frontmatter pre-commit hook (v0.22.4+)';

const HOOK_SCRIPT = `#!/bin/sh
${HOOK_BANNER}
# Validates YAML frontmatter on staged .md / .mdx files. Bypass with
# 'git commit --no-verify'. Uninstall with 'inbrain frontmatter install-hook --uninstall'.

set -e

if ! command -v inbrain >/dev/null 2>&1; then
  echo "inbrain not on PATH; skipping frontmatter pre-commit (install inbrain to re-enable)." >&2
  exit 0
fi
...
for f in $staged; do
  [ -f "$f" ] || continue
  if ! inbrain frontmatter validate "$f" >/dev/null 2>&1; then
    inbrain frontmatter validate "$f" >&2
    failed=1
  fi
done
...
if [ $failed -ne 0 ]; then
  echo "" >&2
  echo "Frontmatter validation failed. Run 'inbrain frontmatter validate <file> --fix' to repair, or 'git commit --no-verify' to bypass." >&2
  exit 1
fi
`;
  ...
  try {
    ...
    if (sources.length === 0) {
      console.log(sourceId
        ...
        : 'No registered sources. Run `inbrain sources list` to inspect.');
      return;
    }
    ...
    for (const src of sources) {
      ...
      if (uninstall) {
        if (uninstallHook(src.local_path)) {
          ...
          console.log(`[${src.id}] no inbrain pre-commit hook found; nothing to uninstall`);
        }
        continue;
      }
...
function printHelp() {
  console.log(`inbrain frontmatter install-hook — install pre-commit hook in source git repos

Usage:
  inbrain frontmatter install-hook [--source <id>] [--force] [--uninstall]

The hook runs \`inbrain frontmatter validate\` against staged .md/.mdx files,
...
export function installHook(repoPath: string, force: boolean): InstallResult {
  ...
  if (existsSync(hookPath)) {
    ...
    if (existing.includes(HOOK_BANNER)) {
      // Already a inbrain hook — refresh the script content silently.
