/**
 * v0.36 Phase 3 — `inbrain reindex --multimodal` sweep.
 *
 ...
 *
 * Wired patterns:
 *   - D7: acquires `inbrain-reindex-multimodal` writer lock via
 ...
 * `inbrain reindex --markdown` walks markdown pages and re-imports via
...
const LOCK_ID = 'inbrain-reindex-multimodal';
...
/**
 * Entry point for `inbrain reindex --multimodal`.
 */
export async function runReindexMultimodal(
    ...
    if (multimodalModel) {
      ...
      if (!pre.ok) {
        ...
        throw new Error(
          ...
          `Fix with \`inbrain config set embedding_multimodal_model <provider>:<model>\`.`,
        );
      }
    }
  } catch (e) {
  ...
  // INBRAIN_NO_REEMBED bypass (CI / cron / opt-out).
  if (process.env.INBRAIN_NO_REEMBED === '1') {
    process.stderr.write(
      `[reindex-multimodal] skipping: INBRAIN_NO_REEMBED=1. ` +
  ...
  if (!lockHandle) {
    progress.finish();
    throw new Error(
      `LOCK_HELD: another inbrain-reindex-multimodal process is already running. ` +
  ...
  const checkpointPath = inbrainPath(CHECKPOINT_FILE);
    ...
    if (currentFlag !== 'true' && currentFlag !== '1') {
      if (process.stdout.isTTY) {
        process.stderr.write(
          ...
          `Run \`inbrain config set search.unified_multimodal true\` to route all queries ` +
        ...
        process.stderr.write(
          `[reindex-multimodal] Coverage now 100%. ` +
          `inbrain config set search.unified_multimodal true\n`,
        );
        unifiedFlagPrompted = true;
      }
    }
