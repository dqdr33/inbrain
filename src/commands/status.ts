/**
 * `inbrain status` — single-screen brain health dashboard.
 *
 * The command that answers "is my brain healthy and working?" without
 * making operators run five other commands (inbrain sources status, inbrain
 * stats, inbrain jobs supervisor status, inbrain jobs list, tail audit logs).
 *
 * Six sections:
 ...
 *   - Locks      — active rows in inbrain_cycle_locks
...
import { inbrainPath, loadConfig, isThinClient } from '../core/config.ts';
...
async function buildQueueCounts(engine: BrainEngine): Promise<QueueCounts> {
  ...
  try {
    ...
    // jobs are the failure mode `inbrain status` should surface, not hide.
    const rows = await engine.executeRaw<Row>(
  ...
  if (report.autopilot) {
    ...
    if ('local_only_remote' in report.autopilot) {
      ...
      if (a.running) {
        ...
        lines.push(`  stale lockfile (PID ${a.pid ?? '?'} not alive). Run \`inbrain autopilot --install\` to restart.`);
      } else {
        lines.push('  not running. Install with `inbrain autopilot --install`.');
      }
    }
    lines.push('');
  }
...
export async function runStatus(
  ...
  if (sectionFlag === 'usage_error') {
    stderr(
      `inbrain status: invalid --section. Valid: ${VALID_SECTIONS.join('|')}\n`,
    );
    return { exitCode: 2 };
  }
  ...
  try {
    if (useThinClient) {
      ...
      if (!engine) {
        stderr('inbrain status: no engine connected (DB unreachable?). Run `inbrain doctor` to diagnose.\n');
        return { exitCode: 1 };
      }
      report = await buildLocalReport(engine, { sections });
    }
  } catch (err) {
    stderr(`inbrain status: snapshot failed: ${(err as Error).message}\n`);
    return { exitCode: 1 };
  }
