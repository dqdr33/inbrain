...
import { saveConfig, loadConfig, loadConfigFileOnly, toEngineConfig, inbrainPath, configPath, isThinClient, type InbrainConfig } from '../core/config.ts';
...
export async function runInit(args: string[]) {
  ...
  // handler with --help in argv. Without this guard, `inbrain init --help`
  ...
  // with a fresh PGLite brain at ~/.inbrain/brain.pglite. Confirmed in the
  ...
  // inbrain-base-v2 (the 15-type canonical taxonomy); --schema-pack
  // inbrain-base opts back to the legacy 24-type pack for users who don't
  ...
  // friction (running setup-inbrain repeatedly on a thin-client machine).
  ...
  if (isThinClient(existing) && !isForce && !isMigrateOnly) {
    ...
    const msg = `Thin-client config already present at ${configPath()} (remote_mcp.mcp_url=${url}).\n` +
      ...
      `Use --force to overwrite, or \`inbrain init --mcp-only --force\` to refresh thin-client config.`;
    if (jsonOutput) {
  ...
  // script, and the postinstall hook. Bare `inbrain init` defaults to PGLite
  ...
  // key validation (also honored via INBRAIN_INIT_SKIP_EMBED_CHECK=1).
  ...
  if (isPGLite || (!isSupabase && !manualUrl && !isNonInteractive)) {
    ...
    if (!isPGLite && !isSupabase) {
      ...
      if (fileCount >= 1000) {
        ...
        console.log('');
        console.log('  inbrain init --supabase   Set up with Supabase (recommended for large brains)');
        console.log('  inbrain init --pglite     Use local PGLite anyway');
        console.log('');
  ...
  if (manualUrl) {
    ...
    const envUrl = process.env.INBRAIN_DATABASE_URL || process.env.DATABASE_URL;
    if (envUrl) {
      databaseUrl = envUrl;
    } else {
      console.error('--non-interactive requires --url <connection_string> or INBRAIN_DATABASE_URL env var');
      process.exit(1);
    }
  } else {
...
/**
 * Resolve AI provider options for `inbrain init`.
 *
...
async function resolveAIOptions(opts: ResolveAIOptionsArgs): Promise<ResolvedAIOptions> {
  ...
  // When `~/.inbrain/config.json` already has embedding_model set (a re-init
  ...
  // multiple tests share `~/.inbrain` and only the first init has flags.
  //
  ...
  if (verbose) {
    ...
    if (!recipe) {
      console.error(`Unknown provider: ${shorthand}. Run \`inbrain providers list\` to see known providers.`);
      process.exit(1);
    }
    ...
    if (recipe.touchpoints.embedding?.user_provided_models === true) {
      console.error(
        ...
        `  inbrain init --embedding-model ${shorthand}:<your-model-id> --embedding-dimensions <N>\n` +
...
function printNoEmbeddingProviderHint(typos: Array<{ userSet: string; suggested: string }>): void {
  ...
  console.error('Then re-run: inbrain init --pglite');
  console.error('');
  console.error('Or pick explicitly:');
  console.error('  inbrain init --pglite --embedding-model openai:text-embedding-3-large');
  console.error('');
  console.error('Or defer setup: inbrain init --pglite --no-embedding');
  console.error('  (you can configure later with `inbrain config set embedding_model <id>`)');
...
/**
 * `inbrain init --mcp-only` — thin-client setup. Writes a `remote_mcp` config
 ...
 *   --issuer-url <url>          (or INBRAIN_REMOTE_ISSUER_URL)
 *   --mcp-url <url>             (or INBRAIN_REMOTE_MCP_URL)
 *   --oauth-client-id <id>      (or INBRAIN_REMOTE_CLIENT_ID)
 *   --oauth-client-secret <s>   (or INBRAIN_REMOTE_CLIENT_SECRET; preferred)
 *
...
async function initRemoteMcp(opts: {
  ...
  const issuerUrl = (arg('--issuer-url') ?? process.env.INBRAIN_REMOTE_ISSUER_URL ?? '').trim();
  const mcpUrl = (arg('--mcp-url') ?? process.env.INBRAIN_REMOTE_MCP_URL ?? '').trim();
  const clientId = (arg('--oauth-client-id') ?? process.env.INBRAIN_REMOTE_CLIENT_ID ?? '').trim();
  const clientSecret = (arg('--oauth-client-secret') ?? process.env.INBRAIN_REMOTE_CLIENT_SECRET ?? '').trim();
  ...
  if (!issuerUrl) fail('missing_issuer_url', '--issuer-url is required (or set INBRAIN_REMOTE_ISSUER_URL). Example: --issuer-url https://brain-host.local:3001');
  if (!mcpUrl) fail('missing_mcp_url', '--mcp-url is required (or set INBRAIN_REMOTE_MCP_URL). Example: --mcp-url https://brain-host.local:3001/mcp');
  if (!clientId) fail('missing_client_id', '--oauth-client-id is required (or set INBRAIN_REMOTE_CLIENT_ID). Get it from `inbrain auth register-client` on the host.');
  if (!clientSecret) fail('missing_client_secret', '--oauth-client-secret is required (or set INBRAIN_REMOTE_CLIENT_SECRET). Get it from `inbrain auth register-client` on the host.');
  ...
  if (!disco.ok) {
    fail(
      ...
      `Hint: confirm the issuer_url, that the host is reachable, and that \`inbrain serve --http\` is running there.`,
  ...
  if (!tokenRes.ok) {
    fail(
      ...
      `Hint: the host operator can run \`inbrain auth register-client <name> --grant-types client_credentials --scopes read,write,admin\` to mint fresh credentials.`,
  ...
  const baseConfig: Partial<InbrainConfig> = existing
  ...
  const config: InbrainConfig = {
    ...(baseConfig as InbrainConfig),
    ...
    remote_mcp: {
      ...
      ...(process.env.INBRAIN_REMOTE_CLIENT_SECRET === clientSecret
        ? {}
  ...
  if (jsonOutput) {
    ...
    console.log('  2. Run `inbrain doctor` to re-verify connectivity at any time.');
    console.log('  3. Run `inbrain remote ping` after writing markdown if you want the host to re-index immediately (Tier B).');
  }
}
 ...
 * when a flag was passed. Bare `inbrain init --pglite` left the gateway
...
async function configureGatewayWithMergedPrecedence(
  ...
  const existingFile = loadConfigFileOnly() ?? ({} as InbrainConfig);
  ...
  const envOverlay = loadConfig() ?? ({} as InbrainConfig);
...
function printResolvedAIChoice(
  ...
  if (resolved.embedding_model.startsWith('zeroentropyai:')) {
    ...
    if (!process.env.ZEROENTROPY_API_KEY && !fileCfg?.zeroentropy_api_key) {
      ...
      console.warn('  Or add to ~/.inbrain/config.json:');
      console.warn('    "zeroentropy_api_key": "..."');
      console.warn('  Or pick a different provider:');
      console.warn('    inbrain init --pglite --embedding-model openai:text-embedding-3-large --embedding-dimensions 1536');
    }
  }
}
...
async function initPGLite(opts: {
  ...
  const dbPath = opts.customPath || inbrainPath('brain.pglite');
  ...
  if (opts.aiOpts?.noEmbedding) {
    ...
    console.log(`  --no-embedding: deferred setup — configure with \`inbrain config set embedding_model <id>\` before import`);
  ...
  // Skipped by --no-embedding / --skip-embed-check / INBRAIN_INIT_SKIP_EMBED_CHECK=1.
  ...
  try {
    ...
    // a 1536 brain runs `inbrain init --pglite` after upgrading to v0.36+
    ...
    if (resolvedDim) {
      ...
      if (after.exists && after.dims !== null && after.dims !== resolvedDim) {
        ...
        console.error('  This is a bug. Please file an issue with the output of `inbrain doctor`.\n');
    ...
    // inbrain invocation). mode_prompted=true so the upgrade-time banner doesn't
    // also fire on a fresh install. Hands-off: inbrain config set self_upgrade.mode auto
    ...
    if (opts.jsonOutput) {
      ...
      if (stats.page_count > 0) {
        ...
        console.log('  inbrain extract links --source db        (typed link backfill)');
        console.log('  inbrain extract timeline --source db     (structured timeline backfill)');
        console.log('  inbrain stats                            (verify links > 0)');
      } else {
        console.log('Next: inbrain import <dir>');
      }
      console.log('');
      console.log('When you outgrow local: inbrain migrate --to supabase');
      reportModStatus();
...
async function initPostgres(opts: {
  ...
  // --no-embedding / --skip-embed-check / INBRAIN_INIT_SKIP_EMBED_CHECK=1.
  const embedCheck = await runInitEmbedCheck({
    resolvedModel,
    resolvedDim,
    expansionModel: opts.aiOpts?.expansion_model,
  ...
  try {
    ...
    config.self_upgrade = { mode: 'notify', mode_prompted: true, ...(config.self_upgrade ?? {}) };
    saveConfig(config);
    console.log('Config saved to ~/.inbrain/config.json');
    if (opts.schemaPack) {
      process.stderr.write(
        `[init] Using schema pack: ${opts.schemaPack} (override with --schema-pack <name>)\n`,
      );
...
async function supabaseWizard(): Promise<string> {
  try {
    ...
    console.log('Then use: inbrain init --url <your-connection-string>');
  } catch {
...
export function installDefaultTemplates(workspaceDir: string): string[] {
  const inbrainRoot = dirname(dirname(__dirname)); // up from src/commands/ to repo root
  const templatesDir = join(inbrainRoot, 'templates');
...
export function reportModStatus(): void {
  ...
  const inbrainRoot = dirname(dirname(__dirname));
  const skillsDir = join(inbrainRoot, 'skills');
  ...
  console.log('');
  console.log('--- Inbrain Mod Status ---');
  ...
  if (!gstack.found) {
    ...
    console.log('  git clone https://github.com/inbrain-ai/gstack.git ~/.claude/skills/gstack');
    console.log('  cd ~/.claude/skills/gstack && ./setup');
  }
  console.log('Resolver: skills/RESOLVER.md');
  console.log('Soul audit: run `inbrain soul-audit` to customize agent identity');
  console.log('');
}

function printInitHelp() {
  console.log(`
inbrain init — initialize a brain (PGLite or Supabase Postgres)
...
ENGINE SELECTION (mutually exclusive)
  ...
  --mcp-only            Thin-client mode: connect to a remote inbrain MCP, no local engine
...
OPTIONS
  ...
  --skip-embed-check    Skip the init-time embedding-key validation (config +
                        live test-embed). Also via INBRAIN_INIT_SKIP_EMBED_CHECK=1

EXAMPLES
  inbrain init --pglite                      # Local-only, no API keys
  inbrain init --supabase                    # Interactive Supabase setup
  inbrain init --url postgresql://...        # Use a custom Postgres
  inbrain init --mcp-only --url https://...  # Thin-client mode

NOTES
  - Bare \`inbrain init\` in a directory with 1000+ .md files defaults to Supabase
    interactive setup. With <1000 files (or with --pglite explicitly), defaults
    to PGLite at ~/.inbrain/brain.pglite.
