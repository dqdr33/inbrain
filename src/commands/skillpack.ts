/**
 * inbrain skillpack <list|scaffold|reference|migrate-fence|scrub-legacy-fence-rows|harvest|diff|check>
 *
 ...
 *   - `harvest`                — inverse: lift host skill into inbrain
 *
...
const HELP_TOP = `inbrain skillpack <subcommand> [options]
...
Subcommands:
  ...
  scaffold --all             Scaffold every bundled skill (inbrain only).

  reference <name>           Read-only: diff inbrain's bundle vs your local copy.
...
Run \`inbrain skillpack <subcommand> --help\` for per-subcommand options.
...
export async function runSkillpack(args: string[]): Promise<void> {
  ...
  switch (sub) {
    ...
    case 'install':
      console.error(
        "Error: 'inbrain skillpack install' was removed in v0.33. Use 'inbrain skillpack scaffold <name>' instead.\n" +
          "If you're upgrading from an older release, run 'inbrain skillpack migrate-fence' once to strip the legacy managed block, then scaffold any new skills.",
      );
      process.exit(2);
      return;
    case 'uninstall':
      console.error(
        "Error: 'inbrain skillpack uninstall' was removed in v0.33. The new scaffold model lets you own scaffolded files outright — to remove a skill, delete its directory (rm -rf skills/<slug>/) and any paired source files declared in its frontmatter.",
      );
      process.exit(2);
      return;
    default:
...
function findGbrainOrDie(): string {
  ...
  if (!root) {
    console.error('Error: could not find inbrain repo root.');
    process.exit(2);
  }
  return root;
}
...
async function cmdList(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log('inbrain skillpack list [--json]\n\nPrint every skill bundled in openclaw.plugin.json.');
    process.exit(0);
  }
  const json = args.includes('--json');
  const inbrainRoot = findGbrainOrDie();
  let manifest;
  try {
    manifest = loadBundleManifest(inbrainRoot);
  } catch (err) {
  ...
  if (json) {
    const entries = slugs.map(slug => {
      const skillMd = join(inbrainRoot, 'skills', slug, 'SKILL.md');
...
async function cmdScaffold(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'inbrain skillpack scaffold <name> | <source> | --all [--workspace PATH] [--dry-run] [--trust] [--no-cache] [--json]\n\n' +
      ...
      '  --all               Scaffold every bundled skill (inbrain only)\n' +
  ...
  if (!all && name !== null && !isThirdPartyShape) {
    ...
    const inbrainRoot = findGbrainRoot();
    if (inbrainRoot) {
      try {
        const manifest = loadBundleManifest(inbrainRoot);
        const slugs = bundledSkillSlugs(manifest);
      ...
      if (!found) {
        console.error(
          `Error: no skillpack named "${cls.normalized}" in the registry (${loaded.registry_url}).\n` +
            `Run \`inbrain skillpack search ${cls.normalized}\` for matches, or pass a full source (owner/repo, https URL, ./path, ./*.tgz).`,
        );
        process.exit(2);
      }
  ...
  try {
    ...
    if (opts.json) {
      ...
      if (!opts.dryRun && summary && summary.wroteNew > 0) {
        console.log(
          `\nNext: your agent walks skills/*/SKILL.md frontmatter triggers: for routing.\n` +
            `Run \`inbrain skillpack reference ${m.name}\` later if upstream changes.`,
        );
      }
    }
...
async function cmdReference(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'inbrain skillpack reference <name> | --all [--workspace PATH] [--apply-clean-hunks] [--since <version>] [--dry-run] [--json]\n\n' +
        '  --since <version>   With --all, restrict the sweep to skills whose source\n' +
        '                      changed in inbrain between <version> and HEAD. Useful\n' +
        '                      after `inbrain upgrade` to see only what moved.',
    );
    process.exit(0);
  }
  const json = args.includes('--json');
  ...
  try {
    if (apply) {
      ...
      const twoWayWarning =
        'WARNING: --apply-clean-hunks is a two-way diff against inbrain\'s CURRENT bundle.\n' +
        '         inbrain does NOT have access to the version you originally scaffolded.\n' +
        '         Hunks where your LOCAL edits differ from inbrain WILL be aligned to inbrain.\n' +
        '         If you have intentional local edits, run `inbrain skillpack reference ' + name + '`\n' +
        '         (read-only) first to inspect, OR pass --dry-run on this command.';
      if (!dryRun && !json) console.error(twoWayWarning);

      const result = runReferenceApply({ inbrainRoot, targetWorkspace, skillSlug: name!, dryRun });
    ...
    if (all) {
      ...
      // --since filter: keep only skills whose source changed in inbrain
      ...
      if (since) {
        ...
        const slugs = changedSlugsSinceVersion(inbrainRoot, since);
        if (slugs === null) {
          console.error(
    ...
    const result = runReference({ inbrainRoot, targetWorkspace, skillSlug: name! });
    if (json) console.log(JSON.stringify(result, null, 2));
    else {
      console.log(result.framing);
      console.log(
      ...
      if (result.summary.missing > 0 || result.summary.differs > 0) {
        ...
        if (result.summary.missing > 0) {
          console.log(
            '  missing → inbrain has a file you don\'t. Usually safe to `inbrain skillpack scaffold ' + name + '` again to land it.',
          );
        }
        if (result.summary.differs > 0) {
          console.log(
            '  differs → was your local edit intentional? Keep it (inbrain is reference, not law).\n            Accidental drift? Patch by hand, or `inbrain skillpack reference ' + name + ' --apply-clean-hunks`\n            (READ the two-way merge warning in that command\'s output first).',
          );
        }
      }
    }
...
async function cmdMigrateFence(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log('inbrain skillpack migrate-fence [--workspace PATH] [--dry-run] [--json]');
    process.exit(0);
  }
  const json = args.includes('--json');
  const dryRun = args.includes('--dry-run');
  ...
  const inbrainRoot = findGbrainOrDie();
  const targetWorkspace = resolveWorkspace({ workspace });
  const result = runMigrateFence({ targetWorkspace, inbrainRoot, dryRun });
  if (json) console.log(JSON.stringify(result, null, 2));
  else {
...
async function cmdScrubLegacy(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'inbrain skillpack scrub-legacy-fence-rows [--workspace PATH] [--dry-run] [--json]',
    );
    process.exit(0);
  }
  const json = args.includes('--json');
...
async function cmdSearch(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'inbrain skillpack search [<query>] [--tier endorsed|community|experimental|dead] [--json] [--refresh] [--url URL]',
    );
    process.exit(0);
  }
  const json = args.includes('--json');
...
async function cmdInfo(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log('inbrain skillpack info <name> [--json] [--refresh] [--url URL]');
    process.exit(0);
  }
  const json = args.includes('--json');
  const refresh = args.includes('--refresh');
      ...
      JSON.stringify(
        {
          ...
          inbrain_min_version: found.entry.inbrain_min_version,
  ...
  console.log(`  inbrain min:    ${found.entry.inbrain_min_version}`);
  ...
  console.log('\nTo scaffold:');
  console.log(`  inbrain skillpack scaffold ${found.entry.name}`);
  process.exit(0);
}
...
async function cmdRegistry(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'inbrain skillpack registry [--url URL] [--refresh] [--json]\n' +
        '\n' +
        '  --url URL    Set the registry URL (writes to ~/.inbrain/config.json)\n' +
  ...
  if (setUrl) {
    // Persist to ~/.inbrain/config.json under skillpack.registry_url.
    const { inbrainPath } = await import('../core/config.ts');
    const cfgPath = inbrainPath('config.json');
...
async function cmdDoctor(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'inbrain skillpack doctor <pack-dir> [--quick|--full] [--fix] [--yes] [--json]\n\n' +
...
async function cmdInit(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'inbrain skillpack init <name> [--target PATH] [--minimal] [--author NAME] [--license SPDX] [--homepage URL] [--dry-run] [--json]\n\n' +
  ...
  try {
    ...
    if (json) {
      ...
      if (!dryRun) {
        console.log(
          `\nNext:\n  cd ${result.targetDir}\n  inbrain skillpack doctor . --quick\n  # iterate, then:\n  inbrain skillpack pack`,
        );
      }
    }
    process.exit(0);
...
async function cmdPack(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'inbrain skillpack pack [<pack-dir>] [--out PATH] [--dry-run] [--skip-doctor] [--json]\n\n' +
  ...
  try {
    ...
    if (json) {
      ...
      console.error('\nRun `inbrain skillpack doctor . --fix --yes` to auto-scaffold what you can, then re-run.');
      process.exit(2);
...
async function cmdEndorse(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'inbrain skillpack endorse <name> [--tier endorsed|community|experimental|dead] [--repo PATH] [--note TEXT] [--push] [--dry-run] [--json]\n\n' +
        ...
        'requires a clone of inbrain-ai/inbrain-skillpack-registry (or any registry-shaped repo).',
    );
    process.exit(0);
  }
  const json = args.includes('--json');
...
async function cmdHarvest(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'inbrain skillpack harvest <slug> --from <host-repo-root> [--no-lint] [--dry-run] [--overwrite-local] [--json]',
    );
    process.exit(0);
  }
  const json = args.includes('--json');
...
async function cmdDiff(args: string[]): Promise<void> {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log('inbrain skillpack diff <name> [--workspace PATH] [--json]');
    process.exit(0);
  }
  const json = args.includes('--json');
  let name: string | null = null;
  ...
  try {
    const diffs = diffSkill(inbrainRoot, name, targetSkillsDir);
    ...
    if (json) console.log(JSON.stringify({ ok: true, skillName: name, diffs }, null, 2));
      ...
      console.log(clean ? '\n✓ all files match the bundle.' : '\n(Run `inbrain skillpack reference ' + name + '` for a unified diff.)');
    }
