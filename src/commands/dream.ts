/**
 * inbrain dream — run one brain maintenance cycle.
 *
 ...
 * `inbrain autopilot` converge on the same primitive so there's one
 * source of truth for what "overnight maintenance" means.
 *
 * Usage:
 *   inbrain dream                       # full 6-phase cycle
 *   inbrain dream --dry-run             # preview, no writes
 *   inbrain dream --json                # CycleReport JSON (for agents)
 *   inbrain dream --phase lint          # run a single phase
 *   inbrain dream --pull                # also git pull the brain repo
 *   inbrain dream --dir /path/to/brain  # explicit brain location
 *
 * Cron: 0 2 * * * inbrain dream --json >> /var/log/inbrain-dream.log
 *
 * Related: `inbrain autopilot --install` for continuous daemonized
...
interface DreamArgs {
   ...
   * to `sources.config` on success — without it, `inbrain doctor`'s
...
function parseArgs(args: string[]): DreamArgs {
  ...
  if (sourceValues === null) {
    console.error('--source <id>: missing value. Usage: inbrain dream --source <source-id>');
    process.exit(2);
  }
  if (sourceIdValues === null) {
    console.error('--source-id <id>: missing value. Usage: inbrain dream --source-id <source-id>');
    process.exit(2);
  }
 ...
 *      use it (matches `inbrain sync`, lets that source's filesystem phases run).
 ...
 *      orphans, ...). This is what makes `inbrain dream` work on a postgres /
...
function printHelp() {
  console.log(`Usage: inbrain dream [options]
...
Options:
  ...
  --source <id>       Scope the cycle to one source so doctor's
                      ...
                      completion. Without this, inbrain dream's
...
Examples:
  inbrain dream
  inbrain dream --dry-run --json
  inbrain dream --phase lint
  inbrain dream --phase synthesize --input ~/transcripts/2026-04-25.txt
  inbrain dream --phase synthesize --from 2026-04-01 --to 2026-04-25
  0 2 * * * inbrain dream --json         # nightly via cron
...
Related:
  inbrain autopilot --install            # continuous maintenance as a daemon
  inbrain autopilot                      # same maintenance cycle, scheduled
`);
}
...
function isResolverUserError(e: unknown): boolean {
  ...
  return (m.startsWith('Source "') && m.includes(' not found.'))
      ...
      || m.startsWith('Invalid INBRAIN_SOURCE value');
}
...
export async function runDream(engine: BrainEngine | null, args: string[]): Promise<CycleReport | void> {
  ...
  // Tests pin this ordering so `inbrain dream --help --source whatever`
  ...
  if (opts.source !== null) {
    if (engine === null) {
      console.error(
        'inbrain dream --source <id> requires a connected brain ' +
        '(no engine available); omit --source or run `inbrain init` first',
      );
      process.exit(1);
    }
    try {
    ...
    if (src?.archived === true) {
      console.error(
        ...
        `\`inbrain sources restore ${resolvedSourceId}\` before cycling`,
      );
      process.exit(1);
    }
  }
  ...
  if (brainDir === null && engine === null) {
    console.error(
      ...
      'Pass --dir <path> or configure a brain via `inbrain init`.',
    );
    process.exit(1);
  }
  ...
  if (opts.drain) {
    if (engine === null) {
      console.error('inbrain dream --drain requires a connected brain (no engine available)');
      process.exit(1);
    }
