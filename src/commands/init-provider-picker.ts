/**
 * v0.37.x — interactive provider picker for `inbrain init`.
 *
 ...
 * picker's UI and `inbrain providers list` can't drift.
 *
 * Trust contract:
...
export function printSubagentAnthropicCaveat(write: (s: string) => void): void {
  write(
    '\n' +
    'Note: subagent features (inbrain dream, inbrain agent run, inbrain autopilot)\n' +
    '      require ANTHROPIC_API_KEY regardless of which chat model you pick.\n' +
    '      Chat alone (inbrain think, inbrain query expansion) works without it.\n' +
