 ...
 *
 * Usage:
 *   inbrain migrate --to supabase [--url <connection_string>]
 *   inbrain migrate --to pglite [--path <db_path>]
 *   inbrain migrate --to <engine> --force  (overwrite non-empty target)
 */

import { createEngine } from '../core/engine-factory.ts';
import { loadConfig, saveConfig, toEngineConfig, inbrainPath, type InbrainConfig } from '../core/config.ts';
...
function parseArgs(args: string[]): MigrateOpts {
  ...
  if (toIdx === -1 || !args[toIdx + 1]) {
    throw new Error('Usage: inbrain migrate --to <supabase|pglite> [--url <url>] [--path <path>] [--force]');
  }
...
function getManifestPath(): string {
  return inbrainPath('migrate-manifest.json');
}
...
export async function runMigrateEngine(sourceEngine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (opts.targetEngine === 'postgres') {
    targetConfig.database_url = opts.targetUrl || process.env.INBRAIN_DATABASE_URL || process.env.DATABASE_URL;
    if (!targetConfig.database_url) {
      console.error('Target is Supabase but no connection string provided. Use: --url <connection_string>');
      process.exit(1);
    }
  } else {
    targetConfig.database_path = opts.targetPath || inbrainPath('brain.pglite');
  }
  ...
  // (which lives in the file plane via `~/.inbrain/config.json`). When this
  ...
  const existingFile = (await import('../core/config.ts')).loadConfigFileOnly() ?? ({} as InbrainConfig);
  const newConfig: InbrainConfig = {
    ...existingFile,
...
async function verifyTarget(engine: BrainEngine, expectedPages: number): Promise<void> {
  ...
  try {
    ...
    if (health.embed_coverage >= 0.9) {
      ...
      console.warn(`  WARN embeddings: ${pct}% coverage, ${health.missing_embeddings} missing. Run: inbrain embed --stale`);
    }
  } catch (e) {
  ...
  try {
    ...
    if (schemaVersion >= LATEST_VERSION) {
      ...
      console.warn(`  WARN schema: version ${schemaVersion} (latest: ${LATEST_VERSION}). Run: inbrain apply-migrations --yes`);
    }
  } catch {
    console.warn('  WARN schema: version could not be read');
  }

  console.log('  Full health check: inbrain doctor');
}
