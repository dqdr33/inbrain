 ...
 *
 * Runs as a phase inside `inbrain init` AFTER `engine.initSchema()` so DB
 ...
 * `balanced` and prints the one-line hint pointing at `inbrain config set
...
The "cost" isn't inbrain itself — it's the downstream agent's input cost
reading the retrieved chunks back into its context window. inbrain's own
...
You can change this any time with: inbrain config set search.mode <mode>
Per-knob tuning + recommendation engine ships at: inbrain search tune
`;
...
export async function runModePicker(
  ...
  if (!process.stdin.isTTY) {
    ...
    console.log('');
    console.log('═══════════════════════════════════════════════════════════════');
    console.log(`[inbrain] search mode tentatively set to: ${rec.mode}`);
    console.log(`[inbrain] Reason: ${rec.reason}`);
    console.log('═══════════════════════════════════════════════════════════════');
    console.log('');
    ...
    console.log('[AGENT] v0.31.x retrieval shape. Run `inbrain config set search.mode');
    ...
    console.log('');
    console.log('To change later: inbrain config set search.mode <mode>');
    console.log('To see what is running: inbrain search modes');
    console.log('');
    return rec.mode;
  }
  ...
  try {
    ...
    console.error(`[inbrain] WARN: failed to persist search.mode (${(err as Error).message ?? 'unknown'}). Defaulting to ${picked} at search-time.`);
  }
