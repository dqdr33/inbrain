 ...
 *
 * Called by `inbrain reconcile-links` CLI surface. Respects the
  ...
  for (const mdSlug of mdSlugs) {
    ...
    for (const ref of refs) {
      ...
      try {
        ...
        await engine.addLink(mdSlug, codeSlug, ctx, 'documents', 'markdown', mdSlug, 'compiled_truth', linkOpts); // inbrain-allow-direct-insert: inbrain reconcile-links command — code-graph reconciliation from markdown references
        await engine.addLink(codeSlug, mdSlug, ref.path, 'documented_by', 'markdown', mdSlug, 'compiled_truth', linkOpts); // inbrain-allow-direct-insert: inbrain reconcile-links command — reverse documented_by edge
      } catch (e: unknown) {
...
export async function runReconcileLinksCli(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (result.status === 'auto_link_disabled') {
    console.log(
      ...
      'Set `inbrain config set auto_link true` to re-enable.',
    );
    return;
  }
