/**
 * CLI handler for `inbrain jobs` subcommands.
...
/** Parse `--nice N` (then `INBRAIN_NICE` env). Returns:
...
export function parseNiceFlag(args: string[], env: NodeJS.ProcessEnv = process.env): number | undefined {
  const raw = parseFlag(args, '--nice') ?? env.INBRAIN_NICE;
...
export function resolveWorkerConcurrency(args: string[], env: NodeJS.ProcessEnv = process.env): number {
  const raw = parseFlag(args, '--concurrency') ?? env.INBRAIN_WORKER_CONCURRENCY ?? '1';
  const parsed = parseInt(raw, 10);
  ...
  if (!Number.isFinite(parsed) || parsed < 1) {
    const source = parseFlag(args, '--concurrency') !== undefined
      ? '--concurrency flag'
      : 'INBRAIN_WORKER_CONCURRENCY env';
    process.stderr.write(
      `[inbrain jobs] invalid concurrency from ${source} (${JSON.stringify(raw)}); ` +
...
export async function runJobs(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!sub || sub === '--help' || sub === '-h') {
    console.log(`inbrain jobs — Minions job queue

USAGE
  inbrain jobs submit <name> [--params JSON] [--follow] [--priority N]
  ...
  inbrain jobs list [--status S] [--queue Q] [--limit N]
  inbrain jobs get <id>
  inbrain jobs cancel <id>
  inbrain jobs retry <id>
  inbrain jobs prune [--older-than 30d]
  inbrain jobs delete <id>
  inbrain jobs stats
  inbrain jobs smoke
  inbrain jobs work [--queue Q] [--concurrency N] [--max-rss MB]
  ...
  inbrain jobs supervisor [start] [--detach] [--json]
    ...
    --nice N   OS scheduling priority, -20 (highest) to 19 (nicest). Lowers CPU
               ...
               to spawned workers and their children. Env: INBRAIN_NICE (flag
               wins). Effective value shows in 'jobs stats' and 'inbrain doctor'.
               Negative values need root.
  inbrain jobs supervisor status [--json] [--pid-file PATH]
  inbrain jobs supervisor stop [--json] [--pid-file PATH]

    Auto-restarting wrapper around 'inbrain jobs work'. Spawns the worker
    as a child process and restarts on crash with exponential backoff
    (1s -> 60s cap). Writes a PID file to ~/.inbrain/supervisor.pid by
    default (override via --pid-file or INBRAIN_SUPERVISOR_PID_FILE env).
    Lifecycle events are appended to
      \${INBRAIN_AUDIT_DIR:-~/.inbrain/audit}/supervisor-YYYY-Www.jsonl
    ...
    EXAMPLES
      inbrain jobs supervisor --concurrency 4         # foreground (Ctrl-C stops)
      inbrain jobs supervisor start --detach --json   # agent-friendly: fork + return JSON
      inbrain jobs supervisor status --json           # machine-readable health check
      inbrain jobs supervisor stop                    # graceful stop
      inbrain jobs supervisor --json --allow-shell-jobs  # JSONL events + shell-exec on
...
HANDLER TYPES (built in)
  ...
  shell             Run a command or argv. Requires INBRAIN_ALLOW_SHELL_JOBS=1
...
`);
  ...
  switch (sub) {
    case 'submit': {
      const name = args[1];
      if (!name) {
        console.error('Error: job name required. Usage: inbrain jobs submit <name>');
        process.exit(1);
      }
      ...
      // regardless of the submitter's own `INBRAIN_ALLOW_SHELL_JOBS` — the submitter
      ...
      if (!follow && name.trim() === 'shell') {
        process.stderr.write(
          `\n⚠  Shell jobs require INBRAIN_ALLOW_SHELL_JOBS=1 on the worker process.\n` +
          ...
          `     INBRAIN_ALLOW_SHELL_JOBS=1 inbrain jobs work\n\n`,
        );
      }
    ...
    case 'get': {
      ...
      if (isNaN(id)) { console.error('Error: job ID required. Usage: inbrain jobs get <id>'); process.exit(1); }
    ...
    case 'stats': {
      ...
      // operator/agent catches a silent halt in `jobs stats`, not 15h later.
      {
        ...
        const wedgeMins = (() => {
          const raw = parseInt(process.env.INBRAIN_WEDGED_QUEUE_WARN_MINUTES ?? '', 10);
        ...
        if (wedged) {
          ...
          console.log(
            ...
            `       inbrain jobs supervisor stop && inbrain jobs supervisor start   # rebuild a fresh pool\n` +
            `       inbrain jobs retry <id>                                        # for dead-lettered jobs`,
          );
        }
      }
      ...
      try {
        ...
        if (lpCount > 0) {
          ...
          const tag = completed > 0
            ...
            : `(no subagent jobs completed — cap may be too tight; \`export INBRAIN_ANTHROPIC_MAX_INFLIGHT=64\`)`;
          ...
          if (errRows.length === 0) {
            ...
            for (const c of clusters.slice(0, 5)) {
              const sample = c.sample_ids.length > 0
                ? `  (e.g. \`inbrain jobs get ${c.sample_ids[0]}\`)` : '';
    ...
    case 'smoke': {
      ...
      if (engineLabel === 'pglite') {
        console.log('Note: the `inbrain jobs work` daemon requires Postgres. PGLite');
    ...
    case 'work': {
      ...
      if (config?.engine === 'pglite') {
        ...
        console.error('Use --follow for inline execution: inbrain jobs submit <name> --follow');
        process.exit(1);
      }
      ...
      // Automatically skipped when running under a supervisor (INBRAIN_SUPERVISED=1).
      ...
      if (niceVal !== undefined) {
        ...
        if (!niceResult.applied) {
          console.error(
            `[inbrain jobs] could not set niceness to ${niceVal}: ${niceResult.error ?? 'unknown'}. ` +
      ...
      const isSupervisedChild = process.env.INBRAIN_SUPERVISED === '1';
      let watchdogNote = '';
      if (maxRssMb > 0) {
      ...
      try {
        ...
        catch (e) { console.error('[inbrain jobs work] engine disconnect failed during shutdown:', e); }
    ...
    case 'supervisor': {
      ...
      //   inbrain jobs supervisor                    → foreground start (back-compat)
      //   inbrain jobs supervisor start [--detach]   → foreground or detached start
      //   inbrain jobs supervisor status             → JSON liveness + queue stats
      //   inbrain jobs supervisor stop               → SIGTERM + drain wait
      ...
      if (isStatusCmd) {
        ...
        // Shared classifier — same code path runs in `inbrain doctor` so the
      ...
      const allowShellJobs = hasFlag(args, '--allow-shell-jobs') ||
                             !!process.env.INBRAIN_ALLOW_SHELL_JOBS;
      ...
      if (supNice !== undefined) {
        ...
        if (!supNiceResult.applied) {
          console.error(
            `[inbrain jobs] could not set supervisor niceness to ${supNice}: ${supNiceResult.error ?? 'unknown'}. ` +
    ...
    default:
      console.error(`Unknown subcommand: ${sub}. Run 'inbrain jobs --help' for usage.`);
      process.exit(1);
  }
}
...
export async function registerBuiltinHandlers(
  ...
  worker.register('embed', async (job) => {
    ...
    // readable via `inbrain jobs get <id>`). Stderr from the worker daemon
  ...
  worker.register('extract-conversation-facts', async (job) => {
    ...
    const result = await runExtractConversationFactsCore(engine, {
      ...
      // `inbrain extract-conversation-facts --background --workers 20`
      // works end-to-end.
  ...
  // phase set and ordering as `inbrain dream` and autopilot's inline path —
  ...
  // `inbrain jobs get <id>` shows the full structured report. Does NOT
  ...
  worker.register('autopilot-cycle', async (job) => {
    ...
    // The queued cycle is the same primitive `inbrain dream` uses; a checkout-less
  ...
  // INBRAIN_ALLOW_SHELL_JOBS=1.
  {
    const { shellHandler } = await import('../core/minions/handlers/shell.ts');
    worker.register('shell', shellHandler);
    if (!quiet) {
      if (process.env.INBRAIN_ALLOW_SHELL_JOBS === '1') {
        process.stderr.write('[minion worker] shell handler enabled (INBRAIN_ALLOW_SHELL_JOBS=1)\n');
      } else {
        process.stderr.write('[minion worker] shell handler registered in guarded mode (set INBRAIN_ALLOW_SHELL_JOBS=1 to execute shell jobs)\n');
      }
    }
  }
  ...
  // remediation via `inbrain doctor --remediate` and autopilot.
  //
  ...
  const makePhaseHandler = (phase: string) => async (job: any) => {
    ...
    // is configured, matching the autopilot-cycle handler + `inbrain dream`. On a
  ...
  // v0.41.18.0 (A10, T7): extract-ner handler for the inbrain onboard
  ...
  // v0.41.18.0 (A13): embed-catch-up handler for the inbrain onboard
  ...
  // Operator path: `inbrain jobs submit unify-types --allow-protected --params
  // '{"target_pack":"inbrain-base-v2"}'`.
  ...
  // unconditionally; empty INBRAIN_PLUGIN_PATH is a no-op.
  try {
