/**
 * inbrain eval replay — replay captured eval_candidates against current brain (v0.25.0).
 *
 * The contributor-facing half of BrainBench-Real:
 *
 *   1. capture some real traffic    (default-on, lands in eval_candidates)
 *   2. snapshot it                  (inbrain eval export --since 7d > baseline.ndjson)
 *   3. make a code change           (tune RRF_K, edit hybrid.ts, swap an embed model)
 *   4. replay against the snapshot  (inbrain eval replay --against baseline.ndjson)
 *
 ...
 *
 * Usage:
 *   inbrain eval replay --against captured.ndjson [--limit N] [--json]
...
function printHelp(): void {
  console.error(`inbrain eval replay — replay captured queries against current brain

USAGE:
  inbrain eval replay --against FILE.ndjson [flags]

FLAGS:
  --against FILE        NDJSON file from \`inbrain eval export\` (required).
 ...
 * that `inbrain bench publish` writes. The header carries metadata (label,
...
function parseNdjson(content: string): CapturedRow[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (typeof row.schema_version !== 'number') {
      throw new Error(`Line ${i + 1} missing schema_version — not from \`inbrain eval export\`?`);
    }
    if (row.schema_version !== 1) {
      throw new Error(
        `Line ${i + 1} has schema_version=${row.schema_version}; this replay only supports v1. ` +
        `Upgrade inbrain or re-export.`,
      );
    }
    rows.push(row);
  }
 ...
 *
 * v0.41 (codex round-2 #7): exposed so `inbrain eval gate --baseline` can
 * call replay in-process rather than spawning a subprocess. Subprocess
 * spawning would run the INSTALLED inbrain (drift risk for source-tree runs).
 */
