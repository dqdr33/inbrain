/**
 * inbrain whoknows — "Who should I talk to about X?"
 *
 ...
 *
 * Usage:
 *   inbrain whoknows "lab automation"
 *   inbrain whoknows "fintech compliance" --explain
 *   inbrain whoknows "ai agents" --limit 10 --json
 */
...
const HELP = `Usage: inbrain whoknows <topic> [options]
...
export async function runWhoknows(
  ...
  if ('error' in parsed) {
    console.error(`inbrain whoknows: ${parsed.error}`);
    console.error(HELP);
    process.exit(2);
    return;
  }
