/**
 * inbrain autopilot — Self-maintaining brain daemon.
 *
 * v0.11.1 shape:
 *   - Default path (minion_mode != off AND engine == postgres): spawn a
 *     `inbrain jobs work` child process, submit ONE `autopilot-cycle` job
 ...
 *
 * Usage:
 *   inbrain autopilot [--repo <path>] [--interval N] [--json] [--inline]
 *   inbrain autopilot --install [--repo <path>]
 *   inbrain autopilot --uninstall
 *   inbrain autopilot --status [--json]
 */
...
import { loadConfig, saveConfig, inbrainPath as inbrainHomePath } from '../core/config.ts';
 ...
 * `INBRAIN_AUTOPILOT_MAX_RECONNECT_FAILS` (default 30).
 *
...
function logError(phase: string, e: unknown) {
  ...
  console.error(line);
  try {
    const logDir = join(process.env.HOME || '', '.inbrain');
...
/**
 * Resolve the inbrain CLI entrypoint for spawning the worker child.
 *
 ...
 * puts a shim at `/usr/local/bin/inbrain` (or wherever `which inbrain`
 * resolves to) that already wraps the right runtime+entrypoint; prefer it.
 *
 * Order of resolution:
 *   1. `which inbrain` — the shim on PATH, canonical for installed builds.
 *   2. process.execPath if it ends with /inbrain (compiled binary, no shim).
 *   3. argv[1] if it ends with /inbrain (e.g., direct invocation of compiled
 *      binary without PATH). Never .ts source paths.
 *   4. Throw with a clear install hint.
 */
export function resolveGbrainCliPath(): string {
  try {
    const which = execSync('which inbrain', { encoding: 'utf-8', stdio: ['ignore', 'pipe', 'ignore'] }).trim();
    if (which) return which;
  } catch { /* not on $PATH — fall through */ }

  const exec = process.execPath ?? '';
  if (exec.endsWith('/inbrain') || exec.endsWith('\\inbrain.exe')) {
    return exec;
  }

  const arg1 = process.argv[1] ?? '';
  if (arg1.endsWith('/inbrain') || arg1.endsWith('\\inbrain.exe')) {
    return arg1;
  }

  throw new Error('Could not resolve the inbrain CLI path. Install inbrain so it is on $PATH (e.g. /usr/local/bin/inbrain), or run autopilot from the compiled binary directly.');
}
...
async function computeAutopilotIdle(engine: BrainEngine, engineType: string): Promise<boolean> {
  try {
    const cycle = await inspectLock(engine, 'inbrain-cycle');
 ...
 * `inbrain upgrade --swap-only` (fast; defers post-upgrade to the relaunch),
...
async function attemptAutopilotSelfUpgrade(
  ...
  try {
    ...
    try {
      execSync('inbrain upgrade --swap-only', {
        stdio: 'inherit',
        timeout: 300_000,
...
export async function runAutopilot(engine: BrainEngine, args: string[]) {
  ...
  if (!repoPath) {
    console.error('No repo path. Use --repo or run inbrain sync --repo first.');
    process.exit(1);
  }

  // Lock file to prevent concurrent instances (#14).
  // v0.37.7.0 #1226: route through inbrainPath() so the lockfile lives
  // under INBRAIN_HOME when set, not the hardcoded ~/.inbrain. Pre-fix,
  // two brains sharing INBRAIN_HOME=different-paths still wrote to the
  // same global lockfile and one would silently respawn the other
  // forever.
  const lockPath = inbrainHomePath('autopilot.lock');
  try {
    mkdirSync(inbrainHomePath(), { recursive: true });
  ...
  if (spawnManagedWorker) {
    ...
    // RAM) to [4096,16384]. Bare `inbrain jobs work` resolves the same default;
  ...
  // controlled by INBRAIN_AUTOPILOT_MAX_RECONNECT_FAILS env (default 30).
  let autopilotReconnectFails = 0;
  const AUTOPILOT_MAX_RECONNECT_FAILS = Math.max(
    1,
    Number(process.env.INBRAIN_AUTOPILOT_MAX_RECONNECT_FAILS) || 30,
  );
  ...
  while (!stopping) {
    ...
    //     log + retry next tick. Up to INBRAIN_AUTOPILOT_MAX_RECONNECT_FAILS
    ...
    if (useMinionsDispatch) {
      ...
      // inbrain-cycle), so the "60-min floor double-processes queued
    ...
    try {
      ...
      if (probeEnabled) {
        ...
        await runNightlyQualityProbe({
          ...
          resolveRepoRoot: () => repoPath ?? inbrainHomePath('.'),
...
function plistPath(): string {
  return join(process.env.HOME || '', 'Library', 'LaunchAgents', 'com.inbrain.autopilot.plist');
}

function systemdUnitPath(): string {
  return join(process.env.HOME || '', '.config', 'systemd', 'user', 'inbrain-autopilot.service');
}

function ephemeralStartScriptPath(): string {
  return join(process.env.HOME || '', '.inbrain', 'start-autopilot.sh');
}
...
function writeWrapperScript(repoPath: string): string {
  const home = process.env.HOME || '';
  const inbrainDir = join(home, '.inbrain');
  mkdirSync(inbrainDir, { recursive: true });

  // Wrapper sources the user's shell profile for API keys so nothing is
  // baked into plist/crontab/systemd unit files (#2).
  const wrapperPath = join(inbrainDir, 'autopilot-run.sh');
  const inbrainPath = resolveGbrainCliPath();
  const safeRepoPath = repoPath.replace(/'/g, "'\\''");
  const safeGbrainPath = inbrainPath.replace(/'/g, "'\\''");
  const wrapper = `#!/bin/bash
# Auto-generated by inbrain autopilot --install
...
# subprocess). Source it first so secrets like INBRAIN_DATABASE_URL or any
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.inbrain.autopilot</string>
  ...
  <key>StandardOutPath</key><string>${escapeXml(home)}/.inbrain/autopilot.log</string>
  <key>StandardErrorPath</key><string>${escapeXml(home)}/.inbrain/autopilot.err</string>
</dict>
</plist>`;
}
...
function installLaunchd(wrapperPath: string, home: string, repoPath: string) {
  ...
  try {
    ...
    console.log('Installed launchd service: com.inbrain.autopilot');
    console.log(`  Repo: ${repoPath}`);
    console.log(`  Log: ~/.inbrain/autopilot.log`);
    console.log('  Uninstall: inbrain autopilot --uninstall');
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : String(e);
...
/**
 * Generate the inbrain-autopilot systemd user unit.
 *
...
Restart=always
RestartSec=30
StandardOutput=append:%h/.inbrain/autopilot.log
StandardError=append:%h/.inbrain/autopilot.err
 ...
 * matches the known inbrain-generated shape (never a hand-edited one), only
...
export function migrateSystemdUnitToRestartAlways(): { rewritten: boolean; reason: string } {
  ...
  const looksGenerated =
    content.includes('Description=Inbrain Autopilot') &&
    content.includes('StandardOutput=append:%h/.inbrain/autopilot.log') &&
    !!execMatch;
  if (!looksGenerated) {
    process.stderr.write(
      '[inbrain] autopilot systemd unit looks hand-edited; NOT rewriting Restart=on-failure. ' +
...
function installSystemd(wrapperPath: string, repoPath: string) {
  ...
  try {
    ...
    execSync('systemctl --user daemon-reload', { stdio: 'pipe', timeout: 10_000 });
    execSync('systemctl --user enable --now inbrain-autopilot.service', { stdio: 'pipe', timeout: 15_000 });
    console.log('Installed systemd user service: inbrain-autopilot.service');
    console.log(`  Repo: ${repoPath}`);
    console.log('  Log: ~/.inbrain/autopilot.log');
    console.log('  Uninstall: inbrain autopilot --uninstall');
  } catch (e: unknown) {
...
# Auto-generated by inbrain autopilot --install (ephemeral-container target)
# Ephemeral filesystems lose crontab on every deploy; source this from
# your agent's bootstrap instead.
nohup '${safeWrapperPath}' > ~/.inbrain/autopilot.log 2>&1 &
echo \$! > ~/.inbrain/autopilot.pid
`;
  ...
  if (shouldInject({ detected, injectBootstrap: opts.injectBootstrap, noInject: opts.noInject })) {
    for (const candidate of bootstrapCandidates) {
      try {
        ...
        const marker = '# inbrain:autopilot v0.11.0';
        if (existing.includes(marker)) {
          console.log(`  [skip] ${candidate} already has the inbrain marker`);
          continue;
        }
        // Backup before edit
...
function installCrontab(wrapperPath: string, home: string) {
  ...
  const safeWrapperPath = wrapperPath.replace(/'/g, "'\\''");
  const cronLine = `*/5 * * * * '${safeWrapperPath}' >> '${home.replace(/'/g, "'\\''")}/.inbrain/autopilot.log' 2>&1`;
  try {
    const existing = execSync('crontab -l 2>/dev/null || true', { encoding: 'utf-8' });
    if (existing.includes('inbrain autopilot') || existing.includes('autopilot-run.sh')) {
      console.log('Crontab entry already exists. Remove with: inbrain autopilot --uninstall');
      return;
    }
    // Use a temp file instead of echo pipe to avoid shell escaping issues (#1)
    const tmpFile = join(home, '.inbrain', 'crontab.tmp');
    writeFileSync(tmpFile, existing.trimEnd() + '\n' + cronLine + '\n');
    execSync(`crontab '${tmpFile.replace(/'/g, "'\\''")}'`, { stdio: 'pipe' });
    try { unlinkSync(tmpFile); } catch { /* best-effort */ }
    console.log('Installed crontab entry for inbrain autopilot (every 5 minutes)');
    console.log('  Uninstall: inbrain autopilot --uninstall');
  } catch (e: unknown) {
...
function uninstallDaemon() {
  const home = process.env.HOME || '';
  const wrapperPath = join(home, '.inbrain', 'autopilot-run.sh');
  ...
  if (existsSync(plistPath())) {
    try {
      ...
      console.log('Removed launchd service: com.inbrain.autopilot');
      removed++;
    } catch (e) {
  ...
  if (existsSync(systemdUnitPath())) {
    try {
      execSync('systemctl --user disable --now inbrain-autopilot.service 2>/dev/null || true', { stdio: 'pipe', timeout: 10_000 });
      unlinkSync(systemdUnitPath());
      try { execSync('systemctl --user daemon-reload', { stdio: 'pipe', timeout: 5_000 }); } catch { /* best-effort */ }
      console.log('Removed systemd user service: inbrain-autopilot.service');
      removed++;
    } catch (e) {
  ...
  if (existsSync(ephemeralStartScriptPath())) {
    try {
      ...
      console.log('Removed ephemeral start script: ~/.inbrain/start-autopilot.sh');
      removed++;
    } catch (e) {
  ...
  try {
    ...
    for (const candidate of bootstrapCandidates) {
      try {
        ...
        if (!content.includes('# inbrain:autopilot v0.11.0')) continue;
        const lines = content.split('\n');
        const cleaned: string[] = [];
        for (let i = 0; i < lines.length; i++) {
          if (lines[i].includes('# inbrain:autopilot v0.11.0')) {
  ...
  try {
    ...
    if (existing.includes('inbrain autopilot') || existing.includes('autopilot-run.sh')) {
      const filtered = existing.split('\n').filter(l =>
        !l.includes('inbrain autopilot') && !l.includes('autopilot-run.sh'),
      ).join('\n');
      const tmpFile = join(home, '.inbrain', 'crontab.tmp');
      mkdirSync(join(home, '.inbrain'), { recursive: true });
      ...
      try { unlinkSync(tmpFile); } catch { /* best-effort */ }
      console.log('Removed crontab entry for inbrain autopilot');
      removed++;
    }
  } catch (e) {
...
function showStatus(json: boolean) {
  const logFile = join(process.env.HOME || '', '.inbrain', 'autopilot.log');
  let lastLine = '';
  try {
  ...
  if (process.platform === 'darwin') {
    ...
    try {
      ...
      installed = crontab.includes('inbrain autopilot');
    } catch { /* no crontab */ }
  }
