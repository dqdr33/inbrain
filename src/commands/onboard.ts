...
//
// v0.41.18.0 (A1, T13). CLI shell for `inbrain onboard`. Thin wrapper over:
...
export async function runOnboard(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (auto && maxUsd === undefined) {
    process.stderr.write(
      `inbrain onboard --auto refuses without --max-usd N.\n` +
    ...
    },
    {
      onTargetUnreachable: (target, ceiling) => {
        process.stderr.write(
          ...
          `Configure missing prereqs (run inbrain doctor --remediation-plan) or lower --target-score.\n`,
        );
      },
...
/**
 * v0.42 (T16): per-cluster narrative renderer for `inbrain onboard --check --explain`.
 *
...
async function renderPackUpgradeExplain(
  ...
  try {
    ...
    process.stdout.write(
      ...
      `  inbrain jobs submit unify-types --allow-protected --params '${JSON.stringify({ target_pack: targetPack })}'\n`,
    );
