/**
 * inbrain frontmatter — Frontmatter validation, audit, and auto-repair.
 *
 * Subcommands:
 *   inbrain frontmatter validate <path> [--json] [--fix] [--dry-run]
 *     Validate one file or recursively a directory. --fix writes centralized
 *     backups under ~/.inbrain/backups/frontmatter/... then rewrites in place.
 *     --dry-run previews without writing.
 *
 *   inbrain frontmatter audit [--source <id>] [--json]
...
async function connectEngineForAudit(): Promise<BrainEngine> {
  ...
  if (!config) {
    throw new Error('No brain configured. Run: inbrain init');
  }
...
function printHelp() {
  console.log(`inbrain frontmatter — frontmatter validation, audit, auto-repair, and generation

Usage:
  inbrain frontmatter validate <path> [--json] [--fix] [--dry-run]
  inbrain frontmatter generate <path> [--fix] [--dry-run] [--json] [--include-catch-all]
  inbrain frontmatter audit [--source <id>] [--json]
  inbrain frontmatter install-hook [--source <id>] [--force] [--uninstall]
...
validate
  ...
  --fix      Auto-repair the fixable subset (NULL_BYTES, MISSING_CLOSE,
             ...
             ~/.inbrain/backups/frontmatter/... before any in-place rewrite.
...
generate
  ...
  Unknown/catch-all files are skipped by default so Inbrain does not stamp
  ...
  Examples:
    inbrain frontmatter generate /path/to/brain              # preview all
    inbrain frontmatter generate /path/to/brain --fix        # write all
    inbrain frontmatter generate /path/to/brain/people/ --fix # just people/
...
async function runValidate(rest: string[]): Promise<void> {
  ...
  if (!target) {
    console.error('error: inbrain frontmatter validate requires a <path> argument');
    process.exitCode = 1;
    return;
  }
  ...
  if (flags.json) {
    ...
    if (totalErrors === 0) {
      ...
      if (flags.fix && !flags.dryRun) {
        console.log(`\nWrote centralized backups for ${filesFixed} file(s) under ~/.inbrain/backups/frontmatter/.`);
      }
    }
  }
 ...
 * `inbrain frontmatter validate` (doctor's own remediation hint) hanging
...
function printAuditHumanReport(report: AuditReport): void {
  if (report.per_source.length === 0) {
    console.log('No registered sources to audit. Run `inbrain sources list` to inspect.');
    return;
  }
  ...
  if (report.total > 0) {
    console.log(`\nFix with: inbrain frontmatter validate <source-path> --fix`);
  }
}
...
async function runGenerate(args: string[]): Promise<void> {
  ...
  if (!targetPath) {
    console.error('error: inbrain frontmatter generate requires a <path> argument');
    console.error('usage: inbrain frontmatter generate <path> [--fix] [--dry-run] [--json]');
    process.exitCode = 1;
    return;
  }
  ...
  if (results.length > 0 && (!doFix || dryRun)) {
    ...
    if (!doFix) {
      console.log(`\n  To write: inbrain frontmatter generate ${targetPath} --fix`);
    }
  }
}
