 ...
 *      ~/.inbrain/migrations/pending-host-work.jsonl so the host agent knows
 *      to read skills/migrations/v0.14.0.md (shell-jobs adoption, autopilot
 *      cooperative handler wiring, INBRAIN_POOL_SIZE doc). Idempotent — the
...
// inbrainPath() honors INBRAIN_HOME at call time (not module-load) and routes
// through the centralized config dir, so the prior resolveHome()/HOME-env
// trick is no longer needed.
function pendingHostWorkDir(): string { return inbrainPath('migrations'); }
...
async function phaseASchema(opts: OrchestratorOpts): Promise<{ result: OrchestratorPhaseResult; engine: BrainEngine | null }> {
  ...
  try {
    ...
    if (!config) {
      return {
        result: { name: 'schema', status: 'skipped', detail: 'no brain configured (run inbrain init first)' },
        engine: null,
      };
    }
...
export const v0_14_0: Migration = {
  ...
  featurePitch: {
    ...
    description:
      'v0.14.0 unlocks `shell` as a Minion job type (gated by INBRAIN_ALLOW_SHELL_JOBS=1 ' +
