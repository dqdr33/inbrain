 ...
 * `inbrain init --migrate-only` has run, so an upgrade that somehow skipped
 * the schema step fails loudly instead of silently.
 *
 * Phases (all idempotent):
 *   A. Schema — inbrain init --migrate-only (creates tables via SCHEMA_SQL).
...
export const v0_16_0: Migration = {
  ...
  featurePitch: {
    ...
    description:
      ...
      'lease-based rate limit, parent-child fan-out with aggregation. Entry points: `inbrain ' +
      'agent run` and `inbrain agent logs`. See docs/guides/plugin-authors.md for shipping ' +
