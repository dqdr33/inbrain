 ...
 *                   `inbrain upgrade`/initSchema). This phase asserts post-condition.
 *   B. Backfill   — submit `inbrain extract takes` as a Minion job so any
 ...
 *   C. Re-chunk   — emit a pending-host-work TODO for `inbrain re-chunk
 ...
 *
 * No content mutation. No data loss. Operator runs `inbrain doctor` after
...
function migrationsDir(): string { return inbrainPath('migrations'); }
  ...
  try {
    ...
    if (v < 38) {
      return {
        name: 'schema',
        status: 'failed',
        detail: `expected schema version >= 38 (takes + access_tokens.permissions); got ${v}. Run \`inbrain apply-migrations --yes\` to apply.`,
      };
    }
...
async function phaseBBackfill(
  ...
  try {
    ...
    // `inbrain extract takes --rebuild` later; the migration's job is to get
...
function phaseCRechunkTodo(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    ...
    const entry = {
      ...
      command: "inbrain extract takes --rebuild  # forces re-chunk via reimport pipeline; see migration doc for the precise sweep command in your env",
      _key: key,
    };
...
export const v0_28_0: Migration = {
  ...
  featurePitch: {
    ...
    description:
      ...
      'Plus `inbrain takes` CLI (list/search/add/update/supersede/resolve), unified model config ' +
      ...
      'think). `inbrain think` op surface lands now; the synthesis pipeline lands incrementally in ' +
