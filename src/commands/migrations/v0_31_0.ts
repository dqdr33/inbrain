 ...
 *                   `inbrain upgrade`/initSchema). Asserts post-condition.
...
async function phaseASchema(
  ...
  try {
    ...
    // same `inbrain apply-migrations --yes` invocation.
    if (v < 45) {
      return {
        name: 'schema',
        status: 'failed',
        detail: `expected schema version >= 45 (facts hot memory); got ${v}. Run \`inbrain apply-migrations --yes\` to apply.`,
      };
    }
...
export const v0_31_0: Migration = {
  ...
  featurePitch: {
    ...
    description:
      ...
      'table via a cheap Haiku pass folded into signal-detector. `inbrain recall` queries ' +
