 ...
 *   A. Schema   — inbrain init --migrate-only (no schema changes in v0.12.2
 *                 but we still apply for consistency with v0.12.0).
 *   B. Repair   — inbrain repair-jsonb (the actual JSONB fix).
 *   C. Verify   — inbrain repair-jsonb --dry-run --json; assert 0 remaining.
...
function phaseBRepair(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    ...
    runGbrainSubprocess('inbrain repair-jsonb' + childGlobalFlags(), { timeoutMs: 600_000 });
...
function phaseCVerify(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    ...
    const out = execSync('inbrain repair-jsonb --dry-run --json', {
...
export const v0_12_2: Migration = {
  ...
  featurePitch: {
    ...
    description:
      'inbrain v0.12.0-and-earlier silently stored JSONB columns as quoted string literals on ' +
      ...
      'truncated by the splitBody horizontal-rule bug can be recovered with `inbrain sync --full`.',
  },
  orchestrator,
};
