 ...
 * `inbrain init --migrate-only` via `execSync`. On Windows + bun + Supabase
 ...
 * — `init.ts:initMigrateOnly` (the `inbrain init --migrate-only` CLI path) and
 ...
 * inbrain-subprocess spawns (extract/repair/stats). It captures child stderr and
 ...
 *  the old `execSync('inbrain init --migrate-only', { timeout: 600_000 })` used,
 ...
 * `inbrain init --migrate-only` did via subprocess: configureGateway →
...
export async function runMigrateOnlyCore(opts?: { timeoutMs?: number }): Promise<MigrateOnlyResult> {
  ...
  if (!config) {
    throw new MigrateOnlyError(
      'No brain configured. Run `inbrain init` (interactive) or `inbrain init --pglite` / `inbrain init --supabase` first.',
    );
  }
...
/**
 * Run a `inbrain ...` subcommand as a subprocess, capturing child stderr so a
 ...
 *
 * Note: stderr is piped (captured), so inbrain progress lines (which go to
