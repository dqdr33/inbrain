 ...
 *   A. Schema   — inbrain init --migrate-only (applies v8/v9/v10).
 *   B. Config   — verify auto_link is not explicitly disabled. If it's
 *                 set to false, leave it alone (user intent) but warn.
 *   C. Backfill — inbrain extract links --source db (idempotent; the
 *                 UNIQUE constraint on (from, to, link_type) guarantees
 *                 re-runs are no-op).
 *   D. Timeline — inbrain extract timeline --source db (idempotent via
 *                 the (page_id, date, summary) UNIQUE index).
 *   E. Verify   — inbrain stats; confirm link_count and
...
function phaseBConfigCheck(opts: OrchestratorOpts): OrchestratorPhaseResult & { autoLink: ConfigCheckResult } {
  ...
  // inbrain config get auto_link returns the raw value (or empty if unset).
  // Default behavior when unset = enabled (per isAutoLinkEnabled).
  let raw = '';
  try {
    raw = execSync('inbrain config get auto_link', { encoding: 'utf-8', timeout: 10_000, env: process.env }).trim();
  } catch {
  ...
  if (disabled) {
    ...
    console.log('  Skipping backfill phases. Re-enable with: inbrain config set auto_link true');
  }
...
function phaseCBackfillLinks(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    ...
    runGbrainSubprocess('inbrain extract links --source db' + childGlobalFlags(), { timeoutMs: 600_000 });
...
function phaseDBackfillTimeline(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    runGbrainSubprocess('inbrain extract timeline --source db' + childGlobalFlags(), { timeoutMs: 600_000 });
...
function readStats(): StatsSnapshot | null {
  try {
    const out = execSync('inbrain get_stats --json 2>/dev/null || inbrain stats', {
      encoding: 'utf-8', timeout: 30_000, env: process.env,
    });
    // The fallback `inbrain stats` prints human-readable output; parse loosely.
...
function phaseEVerify(opts: OrchestratorOpts, autoLinkDisabled: boolean): OrchestratorPhaseResult {
  ...
  if (!stats) {
    return { name: 'verify', status: 'failed', detail: 'could not read inbrain stats' };
  }
  ...
  if (stats.link_count === 0 && stats.page_count > 0) {
    ...
    console.log('  Try: inbrain extract links --source db --dry-run | head -20');
...
export const v0_12_0: Migration = {
  ...
  featurePitch: {
    ...
    description:
      'Every inbrain put_page now extracts entity references and creates typed links ' +
      ...
      '86.6% vs grep\'s 57.8% (+28.8 pts). See github.com/inbrain-ai/inbrain-evals.',
  },
  orchestrator,
};
