 ...
 *
 *   A. Schema — `inbrain init --migrate-only` triggers migrate.ts v11 which
 *               adds link_source + origin_page_id + origin_field columns,
 *               swaps the unique constraint to include them, and creates
 *               new indexes.
 *   B. Backfill — `inbrain extract links --source db --include-frontmatter`
 ...
 *   D. Record — append to ~/.inbrain/completed.jsonl.
 *
...
//
// Shell out to the canonical `inbrain` shim on PATH (`/usr/local/bin/inbrain`
...
function phaseBBackfill(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    ...
    runGbrainSubprocess('inbrain extract links --source db --include-frontmatter', { timeoutMs: 1_800_000 });
...
function phaseCVerify(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    ...
    // to `inbrain graph-query` as a smoke test: extract one random page and
    ...
    const out = execSync('inbrain call get_stats', {
...
export const v0_13_0: Migration = {
  ...
  featurePitch: {
    ...
    description:
      ...
      '`inbrain graph`. Direction semantics respect subject-of-verb (Pedro → meeting, ' +
