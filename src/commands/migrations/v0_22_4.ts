 ...
 * a doctor subcheck (frontmatter_integrity), a top-level `inbrain frontmatter`
 ...
 * writes a JSON report to ~/.inbrain/migrations/v0.22.4-audit.json, and emits
 * one entry per source-with-issues to ~/.inbrain/migrations/pending-host-work.jsonl.
 * It NEVER mutates brain content. The agent reads skills/migrations/v0.22.4.md
 * after upgrade and runs `inbrain frontmatter validate <source-path> --fix` with
...
function inbrainDir(): string {
  return join(process.env.HOME || '', '.inbrain');
}
  ...
  try {
    ...
    for (const src of sourcesWithIssues) {
      ...
      const entry: PendingHostWorkEntry = {
        ...
        command: `inbrain frontmatter validate ${src.source_path} --fix`,
      };
...
export const v0_22_4: Migration = {
  ...
  featurePitch: {
    ...
    description:
      'inbrain v0.22.4 adds end-to-end frontmatter validation: a `inbrain frontmatter` CLI ' +
      ...
      'report to ~/.inbrain/migrations/v0.22.4-audit.json, and queues a TODO with the exact fix ' +
      'command for malformed frontmatter only. Missing frontmatter is treated as optional metadata ' +
      'coverage for broad document sources. Run `inbrain frontmatter validate <source-path> --fix` ' +
      'to repair (creates centralized backups under ~/.inbrain/backups/frontmatter). Ships frontmatter-guard.',
  },
  orchestrator,
};
