/**
 * v0.11.0 migration orchestrator — Inbrain Minions adoption.
 *
 * Phases (all idempotent; resumable from a prior status:"partial" run):
 *   A. Schema  — inbrain init --migrate-only (never bare init — that
 *                defaults to PGLite and clobbers existing configs).
 *   B. Smoke   — inbrain jobs smoke. Fail loudly on non-zero.
 ...
 *                cron entries for INBRAIN-BUILTIN handler names only.
 *                For non-builtin handlers (host-specific, like
 *                ea-inbox-sweep) emit structured TODO rows to
 *                ~/.inbrain/migrations/pending-host-work.jsonl so the host
...
const AGENTS_MD_MARKER = '<!-- inbrain:subagent-routing v0.11.0 -->';
const CRON_MIGRATED_PROPERTY = '_inbrain_migrated_by';
const MAX_HOST_FILE_BYTES = 1_000_000;

function home(): string { return process.env.HOME || ''; }
function inbrainDir(): string { return join(home(), '.inbrain'); }
function pendingHostWorkPath(): string { return join(inbrainDir(), 'migrations', 'pending-host-work.jsonl'); }
...
async function phaseASchema(opts: OrchestratorOpts): Promise<OrchestratorPhaseResult> {
  ...
  try {
    ...
    // a `inbrain init --migrate-only` subprocess subprocess for Postgres (died with
...
function phaseBSmoke(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    execSync('inbrain jobs smoke', { stdio: 'inherit', timeout: 30_000, env: process.env });
...
async function phaseCMode(opts: OrchestratorOpts): Promise<{
  ...
  if (opts.yes || !process.stdin.isTTY) {
    console.log('Defaulting minion_mode=pain_triggered (non-interactive). Change with `inbrain config set minion_mode <always|off>`.');
  ...
  console.log('');
  console.log('How should your agent use Inbrain Minions?');
...
function injectAgentsMdMarker(path: string, opts: OrchestratorOpts): { injected: boolean; skipReason?: string } {
  ...
  try {
    ...
    const snippet = `\n\n${AGENTS_MD_MARKER}\n## Subagent routing (inbrain v0.11.0)\n\nSee \`skills/conventions/subagent-routing.md\` for the runtime routing convention.\n\`~/.inbrain/preferences.json\` controls \`minion_mode\` (always / pain_triggered / off).\n`;
    // Re-check mtime
...
function rewriteCronManifest(
  ...
  try {
    const cfg = JSON.parse(readFileSync(join(inbrainDir(), 'config.json'), 'utf-8'));
  ...
  for (const rawEntry of entries) {
    ...
    if (BUILTIN_HANDLERS.has(handler)) {
      // Rewrite to shell + inbrain jobs submit.
      let cmd: string;
      if (enginePglite) {
        cmd = `inbrain jobs submit ${handler} --params '{}' --follow`;
      } else {
        // slot computed via date(1). Host scheduler evaluates shell.
        cmd = `inbrain jobs submit ${handler} --params '{"slot":"$(date -u +%Y-%m-%dT%H:%M)"}' --idempotency-key ${handler}:$(date -u +%Y-%m-%dT%H:%M)`;
      }
      entry.kind = 'shell';
      entry.cmd = cmd;
      ...
      pendingEntries.push({
        ...
        recommendation: `Add a handler registration for \`${handler}\` in your host worker bootstrap per docs/guides/plugin-handlers.md. Once registered, re-run \`inbrain apply-migrations\` to auto-rewrite this entry.`,
...
function phaseFInstall(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    execSync('inbrain autopilot --install --yes', { stdio: 'inherit', timeout: 60_000, env: process.env });
    return { name: 'install', status: 'complete' };
  } catch (e) {
    // Install is best-effort — log but don't fail the whole migration. User
    // can re-run `inbrain autopilot --install` manually.
...
async function orchestrator(opts: OrchestratorOpts): Promise<OrchestratorResult> {
  ...
  if (pending_host_work > 0) {
    ...
    console.log('');
    console.log('The skill walks the host through each item using Inbrain\'s plugin contract.');
    console.log('Re-run `inbrain apply-migrations --yes` after each batch to auto-rewrite newly-');
...
export const v0_11_0: Migration = {
  version: '0.11.0',
  featurePitch: {
    headline: 'Inbrain Minions — durable background agents',
    description:
