 ...
 * content. `inbrain integrity --auto` clears the flag per-page as it writes
 * proper citations.
 *
 ...
 * ~/.inbrain/migrations/v0_13_1-rollback.jsonl with its pre-migration
 * frontmatter snapshot. Roll back by re-applying those snapshots via
 * `inbrain apply-migrations --rollback v0.13.0` (future CLI; not in scope).
 *
 ...
 *
 * Safety: does NOT call saveConfig. Prior learning [inbrain-init-default-pglite-flip]:
 * bare `inbrain init` defaults to PGLite and overwrites Postgres config.
...
const getRollbackDir = () => inbrainPath('migrations');
...
async function phaseAConnect(opts: OrchestratorOpts): Promise<{ result: OrchestratorPhaseResult; engine: BrainEngine | null }> {
  ...
  try {
    ...
    if (!config) {
      return {
        result: { name: 'connect', status: 'skipped', detail: 'no brain configured (run inbrain init first)' },
        engine: null,
      };
    }
...
export const v0_13_1: Migration = {
  ...
  featurePitch: {
    ...
    description:
      ...
      'content. Pages keep passing writes through unchanged; `inbrain integrity ' +
      '--auto` clears the flag per-page once citations are repaired. Rollback ' +
      'log at ~/.inbrain/migrations/v0_13_1-rollback.jsonl.',
  },
  orchestrator,
};
