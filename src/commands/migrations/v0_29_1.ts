 ...
 *   A. Schema  — `inbrain init --migrate-only` ensures v38 ran.
...
async function phaseBBackfill(opts: OrchestratorOpts): Promise<OrchestratorPhaseResult> {
  ...
  try {
    ...
    if (!cfg) throw new Error('No inbrain config; run `inbrain init` first.');
...
export const v0_29_1: Migration = {
  ...
  featurePitch: {
    ...
    description:
      'inbrain v0.29.1 adds two new optional ranking axes to the query op: salience ' +
      ...
      "`inbrain dream --phase recompute_emotional_weight` once after upgrading.",
  },
  orchestrator,
};
