 ...
 *     the full reindex NOW run `inbrain reindex-code --yes`.
 *
 * Phases:
 *   A. Schema — `inbrain init --migrate-only` applies v27 + v28.
 *   B. Backfill-prompt — emit a pending-host-work notice telling the user
 *      to choose between (1) `inbrain reindex-code --yes` for immediate full
...
function phaseBBackfillPrompt(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  console.log('');
  console.log('  1. AUTOMATIC (recommended): next `inbrain sync` detects the version');
  console.log('     mismatch via sources.chunker_version and forces a full re-walk.');
  console.log('     No action needed.');
  console.log('');
  console.log('  2. IMMEDIATE: `inbrain reindex-code --dry-run` to preview cost, then');
  console.log('     `inbrain reindex-code --yes` to reindex every code page now.');
  console.log('');
...
function phaseCVerify(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    // Round-trip the schema check through `inbrain doctor --json` if available,
...
export const v0_21_0: Migration = {
  ...
  featurePitch: {
    ...
    description:
      'v0.21.0 ships the biggest code-search upgrade in inbrain history. Chunk-grain FTS ' +
      ...
      '`inbrain reindex-code --yes`. File classifier widened to 35 extensions. Markdown ' +
