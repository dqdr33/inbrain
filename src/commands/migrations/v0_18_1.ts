 ...
 * It enables Row Level Security on 10 inbrain-managed public tables that
 ...
 * subagent_rate_leases, inbrain_cycle_locks, budget_ledger, budget_reservations.
 *
 * Phase structure mirrors v0.18.0:
 *   A. Schema — `inbrain init --migrate-only` runs the migration chain,
...
export const v0_18_1: Migration = {
  ...
  featurePitch: {
    ...
    description:
      'v0.18.1 fixes a latent security gap: 10 inbrain-managed public tables ' +
      ...
      '`inbrain apply-migrations` runs. `inbrain doctor` now scans every ' +
      ...
      '`INBRAIN:RLS_EXEMPT reason=<why>` comment via psql. See ' +
