/**
 * v0.32.7 CJK wave — `inbrain reindex --markdown` sweep.
 *
 ...
 *
 * Driven by:
 *   - `inbrain upgrade` post-upgrade hook (after the cost-estimate prompt).
 *   - Operators running `inbrain reindex --markdown` directly.
 *
...
interface ReindexOpts {
   ...
   * embedding and the next `inbrain embed --stale` pass fills them in.
...
export async function runReindex(engine: BrainEngine, args: string[]): Promise<ReindexResult> {
  ...
  if (!args.includes('--markdown')) {
    if (opts.json) {
      process.stdout.write(JSON.stringify({ error: 'inbrain reindex requires a target flag, e.g. --markdown' }) + '\n');
    } else {
      process.stderr.write('Usage: inbrain reindex --markdown [--limit N] [--dry-run] [--json] [--repo PATH]\n');
    }
    process.exitCode = 2;
