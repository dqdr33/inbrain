/**
 * v0.28: `inbrain think <question>` CLI.
 *
...
export async function runThinkCli(engine: BrainEngine, args: string[]): Promise<void> {
  if (args.length === 0 || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: inbrain think "<question>" [options]
...
Set ANTHROPIC_API_KEY (or run: inbrain config set anthropic_api_key ...) to run
...
`);
  ...
  if (!question) {
    console.error('Missing question. Try: inbrain think "What do we know about acme-example?"');
    process.exit(1);
  }
