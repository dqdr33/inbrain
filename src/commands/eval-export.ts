/**
 * inbrain eval export — stream captured eval_candidates rows as NDJSON (v0.21.0).
 *
 * Consumer: sibling inbrain-evals repo, which imports the NDJSON as a
 ...
 *
 * The schema_version prefix lets inbrain-evals detect format drift and
 * warn on unknown versions instead of silently misparsing.
 *
 * Usage:
 *   inbrain eval export [--since 7d] [--limit N] [--tool query|search] > rows.ndjson
 */
...
function parseDurationToMs(s: string): number | null {
  // Accepts "30d", "7d", "1h", "90m", "3600s". Same shape as inbrain eval prune + jobs prune.
...
function printHelp(): void {
  console.error(`inbrain eval export — emit captured eval_candidates as NDJSON to stdout
...
OUTPUT:
  ...
  "schema_version": 1 so downstream consumers (inbrain-evals) can
  detect format changes.

EXAMPLES:
  inbrain eval export > rows.ndjson
  inbrain eval export --since 7d --tool query | jq '.query'
  inbrain eval export --limit 100 | head
`);
}
...
export async function runEvalExport(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  for (const row of rows) {
    // Prefix every line with schema_version:1 so inbrain-evals can detect
