/**
 * `inbrain reinit-pglite` — wipe-and-reinit PGLite brain in one command.
 *
 ...
 *   inbrain init --pglite --embedding-model X --embedding-dimensions N
 *   inbrain sync
 *
...
import { loadConfig, loadConfigFileOnly, inbrainPath } from '../core/config.ts';
...
export async function runReinitPglite(args: string[]): Promise<void> {
  ...
  if (cfg?.engine !== 'pglite') {
    fail(
      opts.jsonOutput,
      'not_pglite',
      `inbrain reinit-pglite is for PGLite brains only (current engine: ${cfg?.engine || 'none'}). ` +
  ...
  const dbPath = opts.customPath
    || cfg.database_path
    || inbrainPath('brain.pglite');

  if (!existsSync(dbPath)) {
    fail(
      opts.jsonOutput,
      'no_brain',
      `No PGLite brain found at ${dbPath}. Run \`inbrain init --pglite\` to create one.`,
    );
  }
  ...
  if (!opts.jsonOutput) {
    console.log('');
    console.log('inbrain reinit-pglite — wipe and re-create the PGLite brain.');
    console.log('');
  ...
  if (!opts.noSync) {
    ...
    try {
      ...
      if (!newCfg) {
        if (!opts.jsonOutput) console.error('Warning: no config after reinit; skipping sync. Run `inbrain sync` manually.');
        return;
      }
      ...
      if (!opts.jsonOutput) {
        ...
        console.error('The brain is initialized but empty. Run \`inbrain sync\` to populate it.');
      }
    }
  }
...
function printHelp(): void {
  console.log(`Usage: inbrain reinit-pglite [options]
...
Optional:
  --path <path>                        Active brain path (default: ~/.inbrain/brain.pglite).
  --yes / -y                           Skip the TTY confirmation prompt.
  --no-sync                            Skip the post-init \`inbrain sync\`.
  --json                               Emit structured JSON output on stdout.

Examples:
  # Switch from OpenAI/1536 to ZeroEntropy/1280:
  inbrain reinit-pglite --embedding-model zeroentropyai:zembed-1 --embedding-dimensions 1280

  # Skip the sync step (do it later):
  inbrain reinit-pglite --embedding-model openai:text-embedding-3-large \\
    --embedding-dimensions 1536 --no-sync

The old brain is preserved as \`<path>.bak\`. To roll back, mv it back.

See also:
  inbrain doctor                        Diagnose dim mismatches before/after.
