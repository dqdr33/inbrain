/**
 * v0.32.3 — `inbrain eval compare` — render the per-mode comparison.
 *
 * Reads `<repo>/.inbrain-evals/eval-results.jsonl` (the audit trail from
 * `inbrain eval run-all` plus any manually-logged completions), groups by
...
function printHelp(): void {
  process.stderr.write(
    `inbrain eval compare [flags]\n\n` +
    `Render a per-mode comparison from <repo>/.inbrain-evals/eval-results.jsonl\n\n` +
    `Flags:\n` +
...
function readEvalResults(repoRoot: string, override?: string): ParsedRecord[] {
  const path = override
    ...
    : join(repoRoot, '.inbrain-evals', 'eval-results.jsonl');
...
function renderMarkdown(grouped: Record<string, Record<SearchMode, ParsedRecord | null>>, glossary: Record<string, string>): string {
  ...
  lines.push('');
  lines.push('_Auto-generated from `<repo>/.inbrain-evals/eval-results.jsonl`. Industry terms preserved verbatim so users searching the literature find what we report._');
  lines.push('');
...
export async function runEvalCompare(args: string[]): Promise<void> {
  ...
  if (records.length === 0) {
    process.stdout.write(`_No eval-results.jsonl found at <repo>/.inbrain-evals/eval-results.jsonl._\n`);
    process.stdout.write(`_Run: inbrain eval run-all --modes conservative,balanced,tokenmax --suites longmemeval,replay --seed 42_\n`);
    return;
  }
