 ...
 * `resolveScopedSourceOrThrow` (flag → env → .inbrain-source dotfile →
 * local_path → brain_default → sole_non_default), matching `inbrain sources
 * current`. A `.inbrain-source` pin selects the source; only a no-signal
...
/** A bad/invalid `.inbrain-source` pin or INBRAIN_SOURCE value surfaces from
...
export async function runCodeCallers(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!sym) {
    const err = errorFor({
      ...
      hint: 'inbrain code-callers <symbol> [--source S | --all-sources] [--limit N] [--json]',
    });
  ...
  // source-resolution chain (honors the .inbrain-source pin, env, local_path,
  ...
  if (!allSources && !sourceId) {
    try {
      ...
      // Bad/invalid pin (.inbrain-source or INBRAIN_SOURCE points at a missing
      ...
      if (isResolverUserError(e)) {
        const env = errorFor({
          ...
          hint: 'fix the .inbrain-source pin / INBRAIN_SOURCE value, or pass --source <id> / --all-sources',
        }).envelope;
        if (shouldEmitJson(args)) {
          console.log(JSON.stringify({ error: env }));
        } else {
