...
/**
 * `inbrain self-upgrade [--check-only] [--force] [--json]`
 *
 ...
 * command. The action is always the hardcoded `inbrain upgrade` — never
...
export async function runSelfUpgrade(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'Usage: inbrain self-upgrade [--check-only] [--force] [--json]\n\n' +
        'Check for and apply inbrain updates. The shared entry point used by the\n' +
        'CLI startup marker, the inbrain-upgrade agent skill, and the autopilot\n' +
        'silent channel.\n\n' +
  ...
  if (checkOnly) {
    ...
    if (json) {
      ...
      console.log(`Update available: ${VERSION} -> ${latest}. Run: inbrain self-upgrade`);
      if (changelogDiff) {
        console.log('\nWhat changed:\n');
