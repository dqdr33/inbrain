/**
 * v0.41 D2 — `inbrain jobs watch` live TTY dashboard.
 *
...
export function renderSnapshot(s: WatchSnapshot, opts: { useAnsi?: boolean } = {}): string {
  ...
  const lines: string[] = [];
  lines.push(`${c(ANSI.bold)}inbrain jobs watch${c(ANSI.reset)}    ${c(ANSI.dim)}q to quit | ${new Date(s.ts_ms).toLocaleTimeString()}${c(ANSI.reset)}`);
  lines.push('');
...
/**
 * Main entrypoint for `inbrain jobs watch`. See the file header for the
