...
import { inbrainPath, loadConfig, configPath } from '../core/config.ts';
...
export async function runSchema(args: string[]): Promise<void> {
  ...
  switch (sub) {
    ...
    default:
      ...
      console.error('Run `inbrain schema --help` for available commands.');
      process.exit(2);
  }
}

function printHelp(): void {
  console.log(`inbrain schema — active schema pack management

Inspection:
  active                  Show resolved pack + which tier provided it
  list                    List installed packs (bundled + ~/.inbrain/schema-packs/)
...
Activation:
  use <pack>              Activate pack (writes ~/.inbrain/config.json schema_pack)
  downgrade [--to <pack>] Restore the previous active pack
  reload [--pack <name>]  Flush the in-process pack cache; --pack scopes

Authoring (v0.40.6.0):
  init <name>             Scaffold a new pack (extends inbrain-base)
  ...
  scaffold-extractable <type> [--pack <name>] [--dims a,b,c] [--force]
                          ...
                          \`inbrain extract benchmark\` for the iteration loop.
...
Resolution chain (7-tier, tier 1 trust-gated):
  ...
  6. ~/.inbrain/config.json schema_pack
...
function runList(_args: string[]): void {
  ...
  if (installed.length > 0) {
    console.log('\nInstalled packs (~/.inbrain/schema-packs/):');
    for (const name of installed) console.log(`  ${name}`);
  } else {
    console.log('\nNo user-installed packs (~/.inbrain/schema-packs/ empty or missing).');
  }
}

async function runShow(args: string[]): Promise<void> {
  // v0.39 T18 — `inbrain schema show --as-filing-rules` emits the JSON
  ...
  if (packName) {
    ...
    if (!path) {
      ...
      console.error('Run `inbrain schema list` to see available packs.');
      process.exit(1);
    }
  ...
  if (asFilingRules) {
    ...
    const filingRules = {
      schema_version: 1,
      source: 'inbrain schema show --as-filing-rules',
...
function runValidate(args: string[]): void {
  ...
  if (packName) {
    ...
    path = packPathByName('inbrain-base');
    if (!path) {
...
function runUse(args: string[]): void {
  const packName = args[0];
  if (!packName) {
    console.error('Usage: inbrain schema use <pack-name>');
    process.exit(2);
  }
  const path = packPathByName(packName);
  if (!path) {
  ...
  // Write to file-plane config (~/.inbrain/config.json schema_pack field).
  ...
  console.log(`\nRun \`inbrain schema active\` to verify resolution.`);
}

function packPathByName(name: string): string | null {
  if (name === 'inbrain-base') {
    // Resolve bundled YAML — try a few locations.
    const here = dirname(new URL(import.meta.url).pathname);
    const candidates = [
      join(here, '..', 'core', 'schema-pack', 'base', 'inbrain-base.yaml'),
      join(here, '..', '..', 'src', 'core', 'schema-pack', 'base', 'inbrain-base.yaml'),
    ];
    for (const c of candidates) {
  ...
  const baseDir = inbrainPath('schema-packs', name);
...
async function runDetectCmd(args: string[]): Promise<void> {
  ...
  console.log('');
  console.log('Next: inbrain schema review-candidates  (decide promote / rename / ignore)');
  console.log('      inbrain schema suggest             (LLM refinement on this candidate)');
}
...
async function runReviewCandidatesCmd(args: string[]): Promise<void> {
  ...
  console.log(`Audit history (cross-reference): ~/.inbrain/audit/schema-candidates-*.jsonl`);
  console.log('');
  if (result.applied) {
...
async function runInitCmd(args: string[]): Promise<void> {
  ...
  if (!name) {
    console.error('Usage: inbrain schema init <pack-name>  (experimental)');
    process.exit(2);
  }
  const baseDir = inbrainPath('schema-packs', name);
  ...
  const stub = {
    api_version: 'inbrain-schema-pack-v1' as const,
    name,
    version: '0.0.1',
    inbrain_min_version: '0.39.0',
    extends: 'inbrain-base',
    description: `Stub pack scaffolded by 'inbrain schema init ${name}'. Edit ${baseDir}/pack.yaml then 'inbrain schema validate' + 'inbrain schema use ${name}'.`,
  ...
  const yaml = `# Stub pack — extends inbrain-base by default. Add your own page_types below.
api_version: ${stub.api_version}
name: ${stub.name}
version: ${stub.version}
inbrain_min_version: ${stub.inbrain_min_version}
extends: inbrain-base
...
`;
  ...
  console.log(`Next: edit pack.yaml, then run \`inbrain schema validate ${name}\` and \`inbrain schema use ${name}\`.`);
}
...
async function runForkCmd(args: string[]): Promise<void> {
  ...
  if (!from || !to) {
    console.error('Usage: inbrain schema fork <source-pack> <new-name>  (experimental)');
    process.exit(2);
  }
  ...
  const toDir = inbrainPath('schema-packs', to);
  if (existsSync(toDir)) {
...
async function runEditCmd(args: string[]): Promise<void> {
  ...
  const name = positional[0];
  if (!name) {
    console.error('Usage: inbrain schema edit <pack-name>  (experimental)');
    process.exit(2);
  }
  ...
  console.log(`Open it in your editor; then run \`inbrain schema validate ${name}\`.`);
}
...
async function runDiffCmd(args: string[]): Promise<void> {
  ...
  if (!a || !b) {
    console.error('Usage: inbrain schema diff <pack-a> <pack-b>  (experimental)');
    process.exit(2);
  }
...
async function runExplainCmd(args: string[]): Promise<void> {
  ...
  if (!typeName) {
    console.error('Usage: inbrain schema explain <type-name>  (experimental)');
    process.exit(2);
  }
  const cfg = loadConfig();
  const pack = await loadActivePack({ cfg, remote: false });
...
async function runDowngradeCmd(args: string[]): Promise<void> {
  ...
  const target = positional.includes('--to')
    ...
    : undefined;
  // Find the previous pack from ~/.inbrain/schema-pack-history.jsonl OR honor --to <pack>.
  const historyPath = inbrainPath('schema-pack-history.jsonl');
  ...
  console.log('Run `inbrain schema active` to verify. Note: this command restores CONFIG only.');
...
async function runSyncCmd(args: string[]): Promise<void> {
  ...
  await withConnectedEngine(async (engine) => {
    ...
    if (!apply && result.total_would_apply > 0) {
      console.log(`\nRun \`inbrain schema sync --apply\` to backfill page.type.`);
    }
  });
}
...
async function runAddTypeCmd(args: string[]): Promise<void> {
  ...
  const name = positional[0];
  if (!name) { console.error('Usage: inbrain schema add-type <name> --primitive <p> --prefix <dir/>'); process.exit(2); }
...
async function runRemoveTypeCmd(args: string[]): Promise<void> {
  ...
  if (!name) { console.error('Usage: inbrain schema remove-type <name>'); process.exit(2); }
...
async function runUpdateTypeCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const name = args.filter((a) => !a.startsWith('--'))[0];
  if (!name) { console.error('Usage: inbrain schema update-type <name> [--extractable BOOL] [--expert BOOL] [--primitive P]'); process.exit(2); }
...
async function runAddAliasCmd(args: string[]): Promise<void> {
  ...
  if (pos.length < 2) { console.error('Usage: inbrain schema add-alias <type> <alias>'); process.exit(2); }
...
async function runRemoveAliasCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const pos = args.filter((a) => !a.startsWith('--'));
  if (pos.length < 2) { console.error('Usage: inbrain schema remove-alias <type> <alias>'); process.exit(2); }
...
async function runAddPrefixCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const pos = args.filter((a) => !a.startsWith('--'));
  if (pos.length < 2) { console.error('Usage: inbrain schema add-prefix <type> <prefix>'); process.exit(2); }
...
async function runRemovePrefixCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const pos = args.filter((a) => !a.startsWith('--'));
  if (pos.length < 2) { console.error('Usage: inbrain schema remove-prefix <type> <prefix>'); process.exit(2); }
...
async function runAddLinkTypeCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const name = args.filter((a) => !a.startsWith('--'))[0];
  if (!name) { console.error('Usage: inbrain schema add-link-type <name> [--inverse <verb>] [--page-type <t>] [--target-type <t>]'); process.exit(2); }
...
async function runRemoveLinkTypeCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const name = args.filter((a) => !a.startsWith('--'))[0];
  if (!name) { console.error('Usage: inbrain schema remove-link-type <name>'); process.exit(2); }
...
async function runSetExtractableCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const pos = args.filter((a) => !a.startsWith('--'));
  if (pos.length < 2) { console.error('Usage: inbrain schema set-extractable <type> <true|false>'); process.exit(2); }
...
async function runSetExpertRoutingCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const pos = args.filter((a) => !a.startsWith('--'));
  if (pos.length < 2) { console.error('Usage: inbrain schema set-expert-routing <type> <true|false>'); process.exit(2); }
  const v = parseBool(pos[1]);
  if (v === null) { console.error('Second argument must be true|false'); process.exit(2); }
...
async function runScaffoldExtractableCmd(args: string[]): Promise<void> {
  ...
  const pos = args.filter((a) => !a.startsWith('--'));
  if (pos.length < 1) {
    console.error('Usage: inbrain schema scaffold-extractable <type> [--pack <name>] [--dims a,b,c] [--force]');
    process.exit(2);
  }
  ...
  try {
    ...
    if (json) {
      ...
      console.log(`  3. Run: inbrain extract benchmark --pack ${packName} --kind ${typeName}`);
    }
  } catch (e) {
