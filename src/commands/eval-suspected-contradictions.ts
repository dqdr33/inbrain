 ...
 * Filed in TODOS.md.
 *
 * `inbrain eval suspected-contradictions` — v0.32.6 contradiction probe CLI.
 *
...
function printHelp(): void {
  console.error(`Usage:
  inbrain eval suspected-contradictions [run]
  ...
  inbrain eval suspected-contradictions trend [--days N=30] [--json]

  inbrain eval suspected-contradictions review
...
Cost guardrails:
  - When PROMPT_VERSION changes, the runner prints a cost estimate and waits
    10s in TTY for Ctrl-C (auto-proceeds non-TTY). Skip with INBRAIN_NO_PROBE_PROMPT=1.
  ...
  if (!rows || rows.length === 0) {
    console.error(
      `--from-capture: no rows in eval_candidates. Captures are off by default in v0.25.0+.\n` +
        ...
        `or set 'eval.capture: true' in your inbrain config. Re-run queries to populate, then try again.`,
    );
    process.exit(2);
  }
...
async function runRun(engine: BrainEngine, f: ParsedFlags): Promise<void> {
  ...
  const judgeModel = await resolveModel(engine, {
    ...
    tier: 'utility',
    envVar: 'INBRAIN_CONTRADICTIONS_JUDGE_MODEL',
