#!/usr/bin/env bun
/**
 * Inbrain token management.
 *
 * Wired into the CLI as of v0.22.5:
 *   inbrain auth create "claude-desktop"
 *   inbrain auth list
 *   inbrain auth revoke "claude-desktop"
 *   inbrain auth test <url> --token <token>
 *
 ...
 * configured for. The env-var DATABASE_URL / INBRAIN_DATABASE_URL still
...
function generateToken(): string {
  return 'inbrain_' + randomBytes(32).toString('hex');
}
...
async function withConfiguredSql<T>(
  ...
  if (!config) {
    console.error('No Inbrain config found. Run `inbrain init` first, or set DATABASE_URL / INBRAIN_DATABASE_URL.');
    process.exit(1);
  }
...
async function create(name: string, opts: { takesHolders?: string[] } = {}) {
  ...
  try {
    await withConfiguredSql(async (_sql, engine) => {
      ...
      console.log(`Revoke with: inbrain auth revoke "${name}"`);
      console.log(`Update visibility: inbrain auth permissions "${name}" set-takes-holders world,garry`);
    });
  } catch (e: any) {
    if (e.code === '23505') {
...
async function list() {
  await withConfiguredSql(async (sql) => {
    ...
    `;
    if (rows.length === 0) {
      console.log('No tokens found. Create one: inbrain auth create "my-client"');
      return;
    }
    ...
    const initRes = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        params: {
          ...
          capabilities: {},
          clientInfo: { name: 'inbrain-smoke-test', version: '1.0' },
        },
        id: 1,
      }),
    });
...
/**
 * Parse `inbrain auth register-client` argv. Walks the array once instead of
...
async function registerClient(name: string, args: string[]) {
  ...
  try {
    await withConfiguredSql(async (sql) => {
      const { InbrainOAuthProvider } = await import('../core/oauth-provider.ts');
      const provider = new InbrainOAuthProvider({ sql });
      ...
      console.log(`Revoke with: inbrain auth revoke-client "${clientId}"`);
    });
  } catch (e: any) {
    console.error('Error:', e.message);
    process.exit(1);
...
/**
 * Entry point for the `inbrain auth` CLI subcommand. Also reused by the
 ...
 * the bare `inbrain auth create <name>` form.
 */
...
export async function runAuth(args: string[]): Promise<void> {
  ...
  switch (cmd) {
    ...
    case 'permissions': {
      // inbrain auth permissions <name> set-takes-holders world,garry
    ...
    default:
      console.log(`Inbrain Token Management

Usage:
  inbrain auth create <name> [--takes-holders world,garry,brain]
  ...
  inbrain auth revoke <name>                                Revoke a legacy token
  inbrain auth permissions <name> set-takes-holders <h1,h2,h3>
                                                          Update visibility for an existing token
  inbrain auth register-client <name> [options]             Register an OAuth 2.1 client (v0.26+)
  ...
  inbrain auth revoke-client <client_id>                   Hard-delete an OAuth 2.1 client (cascades to tokens + codes)
  inbrain auth test <url> --token <token>                  Smoke-test a remote MCP server
`);
  }
}
