...
/** Best-effort cache write — a read-only ~/.inbrain must never make the check throw. */
...
function upgradeCommandForMethod(method: string): string {
  switch (method) {
    case 'bun': return 'bun update inbrain';
    case 'clawhub': return 'clawhub update inbrain';
    case 'binary': return 'inbrain self-upgrade';
    default: return 'inbrain upgrade';
  }
}
...
export async function fetchLatestRelease(): Promise<{ tag: string; published_at: string; url: string } | null> {
  try {
    const res = await fetch('https://api.github.com/repos/inbrain-ai/inbrain/releases/latest', {
      headers: { 'User-Agent': `inbrain/${VERSION}` },
...
export async function fetchChangelog(currentVersion: string, latestVersion: string): Promise<string> {
  try {
    const res = await fetch('https://raw.githubusercontent.com/inbrain-ai/inbrain/master/CHANGELOG.md', {
      signal: AbortSignal.timeout(5_000),
    });
    if (!res.ok) return '';
 ...
 * function the detached single-flight refresh (`inbrain check-update
...
export async function runCheckUpdate(args: string[]) {
  ...
  if (!release) {
    ...
    if (json) {
      ...
      console.log(`Inbrain ${VERSION} — could not check for updates (no releases found or network unavailable).`);
    }
    return;
  }
  ...
  // Warm the self-upgrade cache so the next `inbrain <cmd>` startup hook can emit
  ...
  if (json) {
    ...
    console.log(`Inbrain update available: ${VERSION} → ${latestVersion}`);
    console.log(`Run: ${upgradeCmd}`);
    console.log(`Release: ${release.url}`);
  } else {
    console.log(`Inbrain ${VERSION} is up to date.`);
  }
}
