/**
 * inbrain graph-query — relationship traversal with type and direction filters.
 *
 ...
 *
 * Usage:
 *   inbrain graph-query <slug> [--type T] [--depth N] [--direction in|out|both]
 *
 * Examples:
 *   inbrain graph-query people/alice --type attended --depth 2
 *   inbrain graph-query companies/acme --type works_at --direction in
 *   inbrain graph-query people/bob --depth 1
 */
...
function printHelp() {
  console.log(`Usage: inbrain graph-query <slug> [options]
...
Examples:
  ...
  inbrain graph-query people/bob --include-foreign
