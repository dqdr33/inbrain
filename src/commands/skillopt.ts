/**
 * `inbrain skillopt <skill> [flags]` CLI dispatcher.
 *
 * Top-level command (not under `inbrain eval`) because it MUTATES files.
...
export async function runSkillOptCommand(engine: BrainEngine | null, args: string[]): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(`inbrain skillopt: ${err instanceof Error ? err.message : String(err)}\n\n`);
  ...
  if (!engine) {
    process.stderr.write('inbrain skillopt: requires a configured brain (engine connection failed)\n');
    process.exit(2);
  }
  ...
  if (!skillsDir) {
    process.stderr.write(`inbrain skillopt: cannot find skills directory. Pass --skills-dir <path> or run from a workspace with a skills/ directory.\n`);
    process.exit(2);
  }
  ...
  // `job_id=N` to stdout, exits 0; `--follow` execs `inbrain jobs follow`.
