...
export interface EmbedOpts {
   ...
   * in the DB where `inbrain jobs get` can read it.
   */
   ...
   * until countStaleChunks() returns 0. Used by `inbrain embed --stale
   ...
   * finally can release `inbrain_cycle_locks` instead of running for the
...
export async function runEmbed(engine: BrainEngine, args: string[]): Promise<EmbedResult | undefined> {
  ...
  if (slugsIdx >= 0) {
    ...
    if (!slug) {
      serr('Usage: inbrain embed [<slug>|--all|--stale|--slugs s1 s2 ...] [--dry-run] [--batch-size N] [--priority recent] [--catch-up]');
      process.exit(1);
    }
...
async function embedPage(
  ...
  // stale. embedPage is the per-slug path used by `inbrain embed <slug>` AND
  // by `inbrain sync`'s post-import embed step (runEmbedCore({slugs})).
...
async function embedAll(
  ...
  // ─────────────────────────────────────────────────────────────
  if (staleOnly) {
    // D7: thread sourceId so `inbrain embed --stale --source X` actually scopes.
  ...
  // way the `--stale` path does. Without this filter, `inbrain embed --all`
  ...
  // INBRAIN_EMBED_CONCURRENCY env var based on their tier/infra.
  const CONCURRENCY = parseInt(process.env.INBRAIN_EMBED_CONCURRENCY || '20', 10);
...
async function embedAllStale(
  ...
  const BUDGET_MS: number | null = staleOpts?.catchUp
    ? null
    : parseInt(process.env.INBRAIN_EMBED_TIME_BUDGET_MS || `${30 * 60 * 1000}`, 10);
