/**
 * inbrain check-backlinks — Check and fix missing back-links across brain pages.
 *
 * Deterministic: zero LLM calls. Scans pages for entity mentions,
 * checks if back-links exist, and optionally creates them.
 *
 * Usage:
 *   inbrain check-backlinks check [--dir <brain-dir>]     # report missing back-links
 *   inbrain check-backlinks fix [--dir <brain-dir>]        # create missing back-links
 *   inbrain check-backlinks fix --dry-run                  # preview fixes
 */
...
export async function runBacklinks(args: string[]) {
  ...
  if (!subcommand || !['check', 'fix'].includes(subcommand)) {
    console.error('Usage: inbrain check-backlinks <check|fix> [--dir <brain-dir>] [--dry-run]');
  ...
  if (result.action === 'check') {
    ...
    console.log(`\nRun 'inbrain check-backlinks fix --dir ${brainDir}' to create them.`);
  } else {
