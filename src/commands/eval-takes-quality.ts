/**
 * inbrain eval takes-quality — reproducible cross-modal eval CLI for
...
const HELP = `inbrain eval takes-quality — reproducible cross-modal quality eval
...
export async function runReplayNoBrain(argv: string[]): Promise<number> {
  if (argv.length === 0 || argv[0].startsWith('-')) {
    process.stderr.write('Usage: inbrain eval takes-quality replay <receipt-path> [--json]\n');
    return 2;
  }
