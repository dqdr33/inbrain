/**
 * inbrain report — Save a structured report to brain/reports/.
 *
 ...
 *
 * Usage:
 *   inbrain report --type enrichment-sweep --title "Enrichment Sweep" --content "..."
 *   echo "report body" | inbrain report --type meeting-sync --title "Meeting Sync"
 *   inbrain report --type enrichment-sweep --dir /path/to/brain
 */
...
export async function runReport(args: string[]) {
  ...
  if (!reportType) {
    console.error('Usage: inbrain report --type <name> --title "..." --content "..." [--dir <brain>]');
    console.error('  Or pipe content via stdin:');
    console.error('    echo "report body" | inbrain report --type meeting-sync --title "Daily Sync"');
    console.error('');
