/**
 * `inbrain remote` subcommands (multi-topology v1, Tier B).
 *
 * Two thin-client convenience commands that round-trip through the host's
 * HTTP MCP endpoint:
 *
 *   - `inbrain remote ping`  : submit_job(autopilot-cycle) → poll get_job →
 *                             exit when terminal. The "I just wrote markdown,
 *                             tell the host to re-index" affordance.
 *   - `inbrain remote doctor`: run_doctor MCP op → render the host's
 *                             DoctorReport → exit 0/1 based on status.
 *
 * Both require a thin-client install (~/.inbrain/config.json with remote_mcp).
...
export async function runRemote(args: string[]): Promise<void> {
  ...
  if (!isThinClient(config)) {
    console.error(
      '`inbrain remote` requires thin-client mode. This install has no remote_mcp config.\n' +
      'Run `inbrain init --mcp-only` to set up thin-client mode, or use the local CLI directly.',
    );
    process.exit(1);
  }
  ...
  console.error(`Unknown subcommand: inbrain remote ${sub}\n`);
  printHelp();
  process.exit(1);
}

function printHelp(): void {
  console.log('Usage: inbrain remote <subcommand>');
  console.log('');
...
async function runRemotePing(config: NonNullable<ReturnType<typeof loadConfig>>, args: string[]): Promise<void> {
  ...
  if (json) {
    ...
    console.error(`Run \`inbrain jobs get ${submitted.id}\` on the host to inspect.`);
  }
  process.exit(1);
}
...
function renderDoctorReport(report: DoctorReport): void {
  console.log('\nInbrain Health Check (remote host)');
  console.log('=================================');
