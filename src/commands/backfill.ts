/**
 * inbrain backfill — first-class bulk operations (v0.30.1 Fix 3).
 *
 ...
 *
 * Usage:
 *   inbrain backfill <kind> [--batch-size N] [--concurrency N] [--resume]
 *                          [--dry-run] [--keep-index] [--max-errors N]
 *   inbrain backfill list
 *
 * X5: --concurrency clamps to INBRAIN_DIRECT_POOL_SIZE - 1 with a warning,
...
function printHelp(): void {
  console.log(`inbrain backfill — first-class bulk operations.

Usage:
  inbrain backfill <kind> [flags]      Run a registered backfill.
  inbrain backfill list                 Show registered backfills + checkpoints.
...
Flags:
  ...
  --concurrency N    Parallel batches; clamped to INBRAIN_DIRECT_POOL_SIZE - 1
...
function clampConcurrency(requested: number | undefined): { effective: number; warning?: string } {
  ...
  if (requested > ceiling) {
    return {
      effective: ceiling,
      warning: `[backfill] --concurrency ${requested} clamped to ${ceiling} (pool size ${poolSize}, reserved 1 for HNSW/heartbeat). Bump INBRAIN_DIRECT_POOL_SIZE if you need more concurrency.`,
    };
  }
...
export async function runBackfillCommand(args: string[]): Promise<void> {
  ...
  if (!cli.kind) {
    console.error('Usage: inbrain backfill <kind> [flags]   |   inbrain backfill list');
    process.exit(2);
  }

  const reg = getBackfill(cli.kind);
  if (!reg) {
    console.error(`No backfill registered with name "${cli.kind}". Run \`inbrain backfill list\`.`);
    process.exit(2);
  }
