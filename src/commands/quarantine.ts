/**
 * inbrain quarantine — operator surface for the content-quality gate (issue #1699).
 *
 *   inbrain quarantine list [--json] [--include-flagged]
 *   inbrain quarantine clear <slug> [--force] [--no-embed] [--json]
 *   inbrain quarantine scan [--limit N] [--apply] [--no-embed] [--json]
 *
...
async function runClear(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!slug) {
    console.error('Usage: inbrain quarantine clear <slug> [--force] [--no-embed]');
    process.exit(2);
  }
  ...
  const prevNoSanity = process.env.INBRAIN_NO_SANITY;
  if (force) process.env.INBRAIN_NO_SANITY = '1';
  let result;
  try {
    ...
    if (force) {
      if (prevNoSanity === undefined) delete process.env.INBRAIN_NO_SANITY;
      else process.env.INBRAIN_NO_SANITY = prevNoSanity;
    }
  }
  ...
  console.log(
    ...
    (noEmbed ? ' Embedding skipped (--no-embed); run `inbrain embed --stale` to make it searchable.' : ''),
  );
}
...
async function runScan(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  let effCs: NonNullable<import('../core/config.ts').InbrainConfig['content_sanity']> = {};
  try {
...
export async function runQuarantine(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  switch (sub) {
    ...
    default:
      console.error('Usage: inbrain quarantine <list|clear|scan> [...]');
