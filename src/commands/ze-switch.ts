/**
 * v0.36.0.0 — `inbrain ze-switch` CLI lever for the ZeroEntropy default switch.
 *
 * Subcommands / flags:
 *   inbrain ze-switch                        Run the interactive prompt
 *   inbrain ze-switch --dry-run              Plan only; change nothing
 *   inbrain ze-switch --json                 Machine-readable envelope
 *   inbrain ze-switch --non-interactive      Switch without prompting
 *                                           (errors if ZEROENTROPY_API_KEY missing
 *                                            unless --ignore-missing-key is also set)
 *   inbrain ze-switch --resume               Finish a half-applied switch (recovery)
 *   inbrain ze-switch --force                Bypass the `prompt_shown` gate
 *                                           (use after `n` / never-ask-again)
 *   inbrain ze-switch --undo                 Reverse: restore prior model + dim
 *                                           + reranker state. Cost-warning prompt
 *                                           appears before any change.
 *   inbrain ze-switch --undo --non-interactive --confirm-reembed
...
function parseFlags(args: string[]): Flags {
  return {
    ...
    // with INBRAIN_EMBEDDING_MODEL set. Loud stderr line when used.
    ignoreEnvOverride: args.includes('--ignore-env-override'),
  };
}

function printHelp() {
  process.stdout.write(`Usage: inbrain ze-switch [flags]
...
Flags:
  ...
  --ignore-env-override  Apply even when INBRAIN_EMBEDDING_* env vars would
