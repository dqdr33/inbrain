/**
 * inbrain check-resolvable — Standalone CLI gate for skill-tree integrity.
 *
...
const HELP_TEXT = `inbrain check-resolvable [options]
  ...
  if (!detected.dir) {
    return {
      ...
      message:
        ...
        '\nFix: export INBRAIN_SKILLS_DIR=<path>, OPENCLAW_WORKSPACE=<path>, or pass --skills-dir <path>.',
      source: null,
    };
  }

  const sourceLabel = {
    env_explicit: '$INBRAIN_SKILLS_DIR (explicit operator override)',
    ...
    cwd_skills: './skills',
    install_path: 'inbrain install path (read-only fallback)',
  }[detected.source!]!;
...
export async function runCheckResolvable(args: string[]): Promise<void> {
  ...
  if (flags.fix) {
    ...
    // the cwd tree would resolve to the bundled inbrain repo via the
    ...
    if (source === 'install_path') {
      process.stderr.write(
        'inbrain check-resolvable --fix refused: skills dir resolved via install-path fallback (read-only).\n' +
