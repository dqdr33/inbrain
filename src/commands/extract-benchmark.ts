/**
 * v0.42 Wave C2 — `inbrain extract benchmark` CLI.
 *
 *   inbrain extract benchmark --pack <pack> --kind <type> [--json]
 *
 ...
 * `inbrain schema scaffold-extractable` hint.
 *
 * Exit codes:
 ...
 * (`inbrain eval conversation-parser`), and the facts.prose generic
...
/**
 * CLI entry: `inbrain extract benchmark`.
 */
export async function runExtractBenchmark(
  ...
  if (!packName || !kindName) {
    console.error('Usage: inbrain extract benchmark --pack <pack> --kind <type> [--json]');
    process.exit(2);
  }
  ...
  if (!existsSync(fixturePath)) {
    console.error(
      ...
      `  inbrain schema scaffold-extractable ${kindName} --pack ${packName}`,
    );
    process.exit(2);
  }
