...
import type { InbrainConfig } from './core/config.ts';
...
const CLI_ONLY_SELF_HELP = new Set([
  ...
  // Adding `sync` here routes `inbrain sync --help` into runSync.
  'sync',
  ...
  // reaches the user.
  'reinit-pglite',
  // v0.40.6.0 Schema Cathedral v3 — `inbrain schema --help` should hit
  ...
  'enrich',
  // `inbrain connect --help` prints its own usage (flags + examples) from
...
// INBRAIN_SKIP_STARTUP_HOOKS for any children they spawn.
 ...
 * `inbrain check-update --refresh-cache` and emits nothing this run. NEVER
...
function maybeEmitUpdateMarker(command: string): void {
  try {
    if (process.env.INBRAIN_SKIP_STARTUP_HOOKS) return;
    // Never run during the test suite: tests spawn the CLI hundreds of times,
    // each with a fresh (stale-cache) INBRAIN_HOME, which would otherwise fire a
    // detached `inbrain check-update --refresh-cache` per invocation and saturate
    ...
    if (STARTUP_HOOK_SKIP_COMMANDS.has(command)) {
      ...
      // `inbrain post-upgrade` / `inbrain features` they spawn don't re-enter.
      process.env.INBRAIN_SKIP_STARTUP_HOOKS = '1';
      return;
    }
    ...
    if (entry && isCacheFresh(entry, now)) {
      if (entry.marker.kind === 'upgrade_available' && entry.marker.latest) {
        ...
        process.stderr.write(
          `inbrain ${entry.marker.current} -> ${entry.marker.latest} available. Run: inbrain self-upgrade\n`,
        );
      }
      return;
    }
    ...
    try {
      const child = spawn('inbrain', ['check-update', '--refresh-cache'], {
        detached: true,
        stdio: 'ignore',
        env: { ...process.env, INBRAIN_SKIP_STARTUP_HOOKS: '1' },
      });
      // ChildProcess is an EventEmitter — an unhandled 'error' (e.g. ENOENT when
      // inbrain isn't on PATH) would throw uncaught. Swallow it; the refresh is
      // best-effort.
      child.on('error', () => {});
      child.unref();
    } catch {
      /* inbrain not on PATH / spawn failed — fail-open, no refresh this run */
    }
  } catch {
...
async function main() {
  ...
  // BEFORE command dispatch, so `inbrain --progress-json doctor` works.
  ...
  // INBRAIN_SKIP_STARTUP_HOOKS for their children. Runs for every real command.
  ...
  // T5 — `inbrain search modes|stats|tune` is the read-only config dashboard,
  // NOT a free-text search for the literal word "modes". Free-text
  // `inbrain search "<query>"` falls through to the cheap-hybrid `search` op
  ...
  if (!op) {
    ...
    console.error('Run inbrain --help for available commands.');
    process.exit(1);
  }
  ...
  // v0.27.1 (`inbrain query --image <path>`): swap the `image` param from
  ...
  for (const [key, def] of Object.entries(op.params)) {
    if (def.required && params[key] === undefined) {
      ...
      console.error(`Usage: inbrain ${cliName} ${usage}`);
      process.exit(1);
    }
  }
...
function printCliOnlyHelp(command: string) {
  console.log(`Usage: inbrain ${command}`);
  console.log('');
  console.log(`inbrain ${command} - run inbrain --help for the full command list.`);
}
    ...
    if (e instanceof RemoteMcpError) {
      ...
      switch (e.reason) {
        ...
        case 'discovery':
          ...
          console.error('Run `inbrain remote doctor` for details.');
          break;
        case 'auth':
          console.error('OAuth auth failed.');
          console.error('On the host, re-register your client:');
          console.error('  inbrain auth register-client <name> --grant-types client_credentials --scopes read,write,admin');
          break;
        case 'auth_after_refresh':
          console.error('OAuth auth failed after token refresh. Credentials may have been revoked.');
          console.error('Run `inbrain remote doctor` to confirm.');
          break;
        case 'network':
          if (e.detail?.kind === 'timeout') {
            ...
            console.error(`Cannot reach ${url}. Run \`inbrain remote doctor\` for details.`);
          }
          break;
        case 'tool_error':
          if (e.detail?.code === 'missing_scope') {
            ...
            console.error(e.message);
            console.error('Run `inbrain remote doctor` if this persists.');
          }
          break;
        case 'parse':
          console.error('Server response was malformed. Run `inbrain remote doctor`.');
          break;
        default: {
...
// Suppression: --quiet, non-TTY, INBRAIN_NO_BANNER=1.
...
export function bannerSuppressed(cliOpts: CliOptions): boolean {
  ...
  if (process.env.INBRAIN_NO_BANNER === '1') return true;
  // Non-TTY default is suppressed (clean pipes); explicit env-flag overrides.
  if (!process.stderr.isTTY && process.env.INBRAIN_BANNER !== '1') return true;
  return false;
}
...
async function printIdentityBannerBestEffort(
  ...
  // Cache lookup keyed by mcp_url so switching hosts via `inbrain init`
...
/**
 * v0.27.1: shared transform for `inbrain query --image <path>` (and any future
...
async function makeContext(engine: BrainEngine, params: Record<string, unknown>): Promise<OperationContext> {
  ...
  // --source / INBRAIN_SOURCE / .inbrain-source / path-match / brain default /
 ...
 * engine; if `~/.inbrain/config.json` has `remote_mcp` set, the dispatch guard
 ...
 *
 * `serve` is in this set because `inbrain serve` (stdio or http) requires a
...
const THIN_CLIENT_REFUSE_HINTS: Record<string, string> = {
  sync: 'sync runs on the host. Trigger a remote cycle with `inbrain remote ping` (queues an autopilot-cycle job).',
  embed: 'embed runs on the host as part of the autopilot cycle. `inbrain remote ping` triggers a full cycle including embed.',
  extract: 'extract runs on the host. Use `inbrain remote ping` to trigger a cycle including extract.',
  ...
  dream: 'dream runs the autopilot cycle on the host. `inbrain remote ping` queues one. (Native `inbrain dream` thin-client routing planned for v0.31.2.)',
  ...
  files: '`files list` and `files url` MCP ops are localOnly (paths live on the host filesystem). Use `inbrain files` on the host machine.',
  eval: '`eval` export/prune/replay touch the local engine and have no MCP equivalents. Run `inbrain eval` on the host.',
...
function refuseThinClient(command: string, mcpUrl: string): never {
  ...
  if (hint) {
    console.error(`\`inbrain ${command}\` is not routable. ${hint}`);
    console.error(`(thin-client of ${mcpUrl})`);
  } else {
    console.error(
      `\`inbrain ${command}\` requires a local engine. This install is a thin client of ${mcpUrl}.\n` +
...
async function handleCliOnly(command: string, args: string[]) {
  ...
  if (command === 'connect') {
    ...
    // REMOTE inbrain over HTTP from a bearer token. Print mode touches nothing;
  ...
  if (command === 'apply-migrations') {
    ...
    // `inbrain init --migrate-only` and `inbrain jobs smoke`.
  ...
  if (command === 'doctor') {
    // Multi-topology v1: thin-client doctor. When `~/.inbrain/config.json`
  ...
  // connectEngine entirely so first-run users (no `inbrain init` yet) can
  ...
  // connectEngine here keeps `inbrain eval longmemeval --help` and benchmark
  // runs working on machines that have no `~/.inbrain/config.json` configured.
  //
  ...
  // ~/.inbrain/config.json when present; falls back to env vars otherwise
  // (INBRAIN_EMBEDDING_MODEL / INBRAIN_EMBEDDING_DIMENSIONS).
  if (command === 'eval' && args[0] === 'longmemeval') {
    const { runEvalLongMemEval } = await import('./commands/eval-longmemeval.ts');
    if (!(args.length > 1 && (args[1] === '--help' || args[1] === '-h'))) {
      const config = loadConfig() ?? ({
        embedding_model: process.env.INBRAIN_EMBEDDING_MODEL,
        embedding_dimensions: process.env.INBRAIN_EMBEDDING_DIMENSIONS
          ? Number(process.env.INBRAIN_EMBEDDING_DIMENSIONS) : undefined,
      } as InbrainConfig);
  ...
  // v0.41.13.0: `inbrain conversation-parser list-builtins | validate
  ...
  // v0.33.1.3: `inbrain eval whoknows` on thin-client installs bypasses
  ...
  // v0.41.19.0: `inbrain status` on thin-client installs bypasses connectEngine
  ...
  // v0.37 fix wave (Lane D.4 + CDX2-12): short-circuit `inbrain sync --help`
  ...
  // (the production "10-day zombie inbrain search ping" bug class). The wrap
  ...
  // #1633: out-of-band hard-deadline watchdog for `inbrain sync`. Installed
  ...
  try {
    switch (command) {
      case 'import': {
        ...
        // so wrappers (sync, CI scripts, `&& inbrain doctor`) propagate.
      ...
      // v0.36 Phase 3 wave — `inbrain reindex --multimodal` re-embeds content_chunks
      ...
      // v0.41.19.0 — `inbrain status`: single-screen brain health dashboard.
      ...
      // readSupervisorEvents, inbrain_cycle_locks, minion_jobs.
      case 'status': {
        const { runStatus } = await import('./commands/status.ts');
      ...
      case 'search': {
        // v0.32.3 search-lite — `inbrain search modes/stats/tune`.
        const { runSearch } = await import('./commands/search.ts');
      ...
      case 'onboard': {
        // v0.41.18.0 (T13) — inbrain onboard. Thin shell over T2 library
      ...
      case 'founder': {
        // v0.35.4 (T7) — founder scorecard. `inbrain founder scorecard <slug>`
      ...
      case 'recall': {
        // v0.31: hot memory recall surface — `inbrain recall <entity>`,
      ...
      case 'forget': {
        // v0.31: shorthand for expireFact. `inbrain forget <fact-id>`.
      ...
      case 'notability-eval': {
        ...
        //   inbrain notability-eval mine    — sample paragraphs, write candidates
        //   inbrain notability-eval review  — TTY hand-confirm tiers
      ...
      case 'reindex-frontmatter': {
        ...
        //
        // v0.30.1: still works; canonical entrypoint is now `inbrain backfill
      ...
      case 'repos': {
        // v0.19.0: `inbrain repos ...` is an alias into the v0.18.0 sources
        ...
        // keep the alias so scripts like `inbrain repos add .` keep
        // working, with a nudge toward the canonical command.
        console.error('[inbrain] Note: "repos" is an alias for "sources" as of v0.19.0. Prefer `inbrain sources <subcommand>`.');
        const { runSources } = await import('./commands/sources.ts');
        await runSources(engine, args);
        break;
      }
    ...
    // v0.42.20.0 (#1762) — the CLI_ONLY path (which owns `inbrain capture`)
 ...
 * default wallclock timeout (`inbrain search`, `inbrain sources list`).
...
// Build the AIGatewayConfig payload from a InbrainConfig. Both configureGateway
...
async function connectEngine(opts?: { probeOnly?: boolean }): Promise<BrainEngine> {
  const config = loadConfig();
  if (!config) {
    console.error('No brain configured. Run: inbrain init');
    process.exit(1);
  }
  ...
  const noRetry = process.argv.includes('--no-retry-connect') ||
                  process.env.INBRAIN_NO_RETRY_CONNECT === '1';
  ...
  // AND initSchema(). Used by `get_health` MCP op + `inbrain upgrade --status`
  ...
  try {
    ...
    if (result.status === 'persistent') {
      ...
      console.warn('    1. Check `inbrain doctor` for stale locks or stuck advisory locks.');
      console.warn('    2. Check `inbrain jobs supervisor status` for crashed migration workers.');
      console.warn('    3. Re-run: `inbrain apply-migrations --yes`');
      ...
      console.warn('  Re-run: `inbrain apply-migrations --yes`');
    }
  ...
  // Flags like `embedding_multimodal` are user-mutable via `inbrain config set`
  ...
  try {
    ...
    if (merged) {
      ...
      // dispatches on INBRAIN_EMBEDDING_MULTIMODAL, OCR consumer reads
      // INBRAIN_EMBEDDING_IMAGE_OCR_*). The gateway itself doesn't read these
      // flags; this preserves the contract without changing the gateway shape.
      if (merged.embedding_multimodal !== undefined) {
        process.env.INBRAIN_EMBEDDING_MULTIMODAL = String(merged.embedding_multimodal);
      }
      if (merged.embedding_image_ocr !== undefined) {
        process.env.INBRAIN_EMBEDDING_IMAGE_OCR = String(merged.embedding_image_ocr);
      }
      if (merged.embedding_image_ocr_model !== undefined) {
        process.env.INBRAIN_EMBEDDING_IMAGE_OCR_MODEL = merged.embedding_image_ocr_model;
      }
...
export function printOpHelp(op: Operation, invokedName?: string) {
  ...
  // v114 (#1941): when invoked via an alias (e.g. `inbrain link-add --help`),
  // show the alias the user typed, not the primary op name.
  const name = invokedName || op.cliHints?.name || op.name;
  console.log(`Usage: inbrain ${name} ${positional} [options]\n`);
...
function printHelp() {
  ...
  console.log(`inbrain ${VERSION} -- personal knowledge brain

USAGE
  inbrain <command> [options]
...
ADMIN
  ...
  connect <mcp-url> --token <t>      Wire Claude Code to a remote inbrain (bearer token)
        [--install] [--json]         Print the paste-ready command, or --install to run it
  call <tool> '<json>'               Raw tool invocation
  version                            Version info
  --tools-json                       Tool discovery (JSON)

Run inbrain <command> --help for command-specific help.
`);
}
