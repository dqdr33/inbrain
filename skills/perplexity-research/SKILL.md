...
| Brain-only lookup | `inbrain query` / `inbrain search` |
...
inbrain get <slug>                    # or
inbrain query "<topic keywords>"

# 2. Compose the Perplexity query with brain context inline:
#    """
#    Topic: <topic>
#    Brain context (what we already know): <embedded inbrain content>
...
inbrain put_page research/<slug>      # via the put_page operation
...
```bash
BRAIN=$(inbrain get people/<slug> 2>/dev/null)
