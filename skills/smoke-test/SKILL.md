...
name: smoke-test
description: |
  Post-restart smoke tests + auto-fix for inbrain and OpenClaw environments.
  Tests critical services, auto-fixes known issues, extensible via user-defined
  test scripts in ~/.inbrain/smoke-tests.d/*.sh.
triggers:
  - "smoke test"
  - "run smoke tests"
...
> Run `inbrain smoke-test` or `bash scripts/smoke-test.sh` after any container restart.

## Contract

This skill guarantees:
- 8 core tests verify inbrain + OpenClaw health after restart
- Known failures are auto-fixed before reporting
- User-extensible via `~/.inbrain/smoke-tests.d/*.sh` drop-in scripts
- Results logged to `/tmp/inbrain-smoke-test.log`
...
| 2 | Inbrain CLI loads | Reinstall deps |
| 3 | Inbrain database (doctor) | — |
| 4 | Inbrain worker process | Start worker |
...
```bash
bash /path/to/inbrain/scripts/smoke-test.sh >> /tmp/bootstrap.log 2>&1
```

### From an agent
```
exec: bash /data/inbrain/scripts/smoke-test.sh
```

## Adding Custom Tests

Create executable scripts in `~/.inbrain/smoke-tests.d/`:

```bash
# ~/.inbrain/smoke-tests.d/check-redis.sh
#!/bin/bash
...
|-----|---------|-------------|
| `INBRAIN_SMOKE_LOG` | `/tmp/inbrain-smoke-test.log` | Log file path |
| `INBRAIN_DIR_OVERRIDE` | (auto-detect) | Force inbrain install path |
| `INBRAIN_DATABASE_URL` | (from .env) | Database connection URL |
| `OPENCLAW_GATEWAY_PORT` | `18789` | Gateway port to test |
| `INBRAIN_BRAIN_PATH` | `/data/brain` | Brain repo path |
...
### Inbrain Worker Auth Failure
- **Symptom:** Worker can't connect to DB
- **Cause:** `INBRAIN_DATABASE_URL` not propagated to worker subprocess
- **Auto-fix:** Script explicitly passes both `DATABASE_URL` and `INBRAIN_DATABASE_URL`
...
- ❌ Hardcoding paths in a user drop-in. Read env vars
  (`INBRAIN_DATABASE_URL`, `HOME`, etc.) so the script travels across
  container rebuilds.
...
structured timestamped log appends to `$INBRAIN_SMOKE_LOG`
(default `/tmp/inbrain-smoke-test.log`) for post-run forensics. Exit code
