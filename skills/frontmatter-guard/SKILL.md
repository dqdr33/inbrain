...
description: |
  ...
  `inbrain frontmatter` CLI for agent-driven workflows.
triggers:
...
- Validation logic is shared with `inbrain doctor`'s `frontmatter_integrity` subcheck — single source of truth
- Reports per source (inbrain is multi-source since v0.18.0); never silently audits the wrong root
...
Without a guard, these accumulate silently until `inbrain sync` chokes or search returns garbage. The guard makes the failure visible at audit time and trivially fixable.
...
```bash
inbrain frontmatter audit --json
```
...
```bash
inbrain frontmatter validate <path> --fix
```
...
```bash
inbrain frontmatter install-hook [--source <id>]
```

The hook runs `inbrain frontmatter validate` against staged `.md`/`.mdx` files. Bypass with `git commit --no-verify`.
...
- Always run `inbrain frontmatter audit --json` first; never assume a brain is clean.
...
- `SLUG_MISMATCH` fixes remove the frontmatter `slug:` field — inbrain derives slug from path. Mention this when the user's title is intentionally renamed.
...
- `inbrain doctor` — the `frontmatter_integrity` subcheck reports the same counts as `audit`.
- `skills/maintain/SKILL.md` — broader brain health audit; chain after this skill if other classes of issue are suspected.
- `skills/lint/SKILL.md` (via `inbrain lint`) — overlapping rules for skill-file lint; the `frontmatter-*` rule names in lint output come from this skill's validation surface.
...
Fix with: inbrain frontmatter validate /Users/me/brain --fix
```
...
`inbrain frontmatter validate <path> --json` returns a similar envelope keyed on per-file results instead of per-source.
...
```yaml
# Correct: single-quoted YAML flow (canonical form inbrain emits)
tags: ['yc', 'w2025', 'ai']
...
**Don't use `--fix` to "make doctor green" without reading the audit first.** SLUG_MISMATCH cases are surfaced for manual review specifically because inbrain derives the slug from path. A mismatch usually means the user renamed a file intentionally; auto-removing the slug field is the right outcome only when you've confirmed the rename was deliberate.
...
**Don't run `audit` on a brain where sources aren't registered.** The CLI returns "no registered sources to audit" gracefully, but the migration emits a `skipped: no_sources` phase result. Don't paper over this with a manual path-walk; the right fix is to register the source via `inbrain sources add`.
