...
> **Relationship to `inbrain eval cross-modal`:** This skill is the manual
> mid-flow gate (one model reviews work product before commit, with refusal
> routing). The `inbrain eval cross-modal` command (v0.27.x) is a sibling
...
> for ad-hoc second opinions; use `inbrain eval cross-modal` for the
