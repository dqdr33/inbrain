...
- **Bundled skills require explicit opt-in AND an independent held-out set.**
  Skills shipping with inbrain cannot be auto-mutated. To rewrite one in place
...
```
inbrain skillopt <skill-name> [flags]
  │
  ├── Pre-flight gates
...
1. **Generate the starter.** Run:
   ```
   inbrain skillopt X --bootstrap-from-skill
   ```
...
4. **Run the optimizer with `--split 1:1:1`:**
   ```
   inbrain skillopt X --bootstrap-reviewed --split 1:1:1
   ```
...
| Situation | Action |
|---|---|
| Skill has no benchmark | `inbrain skillopt foo --bootstrap-from-skill` → review + strengthen the judges → delete sentinel → `inbrain skillopt foo --bootstrap-reviewed --split 1:1:1` (see section above) |
| Skill has a `routing-eval.jsonl` and you want a head start | `inbrain skillopt foo --bootstrap-from-routing` → review the generated tasks → `--bootstrap-reviewed` (routing tasks test dispatch; tighten them into quality tasks before trusting) |
| Iterating on an existing skill | `inbrain skillopt foo --benchmark skills/foo/skillopt-benchmark.jsonl` |
| Costly run, want preview | Add `--dry-run` |
| Bundled skill (skills/ in inbrain repo) | Default writes proposed.md; to commit in place add `--allow-mutate-bundled` AND `--held-out <path>` (>=5 benchmark-disjoint tasks) — else it hard-refuses |
...
| Mid-run crash | `inbrain skillopt foo --resume <run-id>` |
...
- `~/.inbrain/audit/skillopt-YYYY-Www.jsonl` — ISO-week-rotated audit trail
...
- **Don't optimize bundled skills without `--allow-mutate-bundled` AND
  `--held-out`.** They ship with inbrain and are load-bearing for downstream
