...
You (the agent) are running on a host that scaffolded inbrain skills here. This
...
host, not by inbrain. Don't treat them as inbrain artifacts.
...
That pattern was retired in inbrain v0.36. Routing lives in frontmatter now.
...
They are reference code that the inbrain CLI calls. You do not run them
directly unless the SKILL.md tells you to.

## Updates — when inbrain ships a new version

The user runs `inbrain upgrade`. Skill files DO NOT change automatically.
inbrain becomes a reference library you compare against.

On every cold start, or any time the user mentions an upgrade, run:

```bash
inbrain skillpack reference --all
```
...
```bash
inbrain skillpack reference <slug>
```

This prints a unified diff between inbrain's bundle and the local file. Read
it, then decide per file:

- **Local edit was intentional.** Keep your version. inbrain is reference, not
  law.
- **Local edit was accidental drift** (e.g. you wrote stale content into the
  skill body). Either patch by hand, or run
  `inbrain skillpack reference <slug> --apply-clean-hunks` (read the WARNING
  about two-way merge below first).
- **Genuinely new inbrain change in a section you don't care about.** Skip or
  apply per your judgment.

For `missing` files (inbrain added a new bundled skill since you scaffolded),
run `inbrain skillpack scaffold <new-slug>` to bring it in.

### `reference --apply-clean-hunks` — two-way merge warning

This command does a two-way diff against inbrain's current bundle. It does
NOT have access to the version you originally scaffolded. Consequence: if
the user's local file differs from inbrain in ANY section (including
intentional user edits), those sections WILL be aligned to inbrain.

Always run plain `inbrain skillpack reference <slug>` first to inspect.
Use `--apply-clean-hunks` only when you're confident the local edits were
accidental or you want to fully reset to inbrain's current bundle.
...
`docs/guides/skillpacks-as-scaffolding.md` in the inbrain repo. The skill
