...
When the brain has 2+ active sources (anything `inbrain sources list` shows
...
```cron
*/5 * * * * inbrain sync --all --parallel 4 --workers 4 --skip-failed
```
...
**Avoid (legacy)**: separate `inbrain sync --source default` and
`inbrain sync --source zion-brain` entries staggered by 5 minutes. They
require manual deconfliction every time a new source is added, and a
slow source can race a fast source on the legacy global `inbrain-sync`
lock (v0.40.3.0+ uses per-source `inbrain-sync:<sourceId>` locks but the
per-source cron pattern doesn't benefit from the parallelism that
`--all --parallel` actually delivers).

`inbrain doctor` surfaces the recommended line as a `sync_consolidation`
...
- Separate per-source `inbrain sync --source <id>` cron entries when
  `inbrain sync --all --parallel N --workers N` would replace them with
