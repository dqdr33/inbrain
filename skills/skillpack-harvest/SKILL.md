...
description: |
  ...
  inbrain's bundle so other clients can scaffold it. Editorial workflow:
...
triggers:
  - "harvest this skill"
  - "harvest my skill"
  - "publish this skill to inbrain"
  - "lift this skill"
  - "share this skill"
  - "promote this skill"
  - "promote my skill"
  - "skill upstream"
  - "into the inbrain core"
  - "inbrain bundle"
mutating: true
writes_pages: false
writes_to:
...
# skillpack-harvest — Editorial workflow for lifting host skills into inbrain

> **Convention:** see [_brain-filing-rules.md](../_brain-filing-rules.md) for
> file placement rules. This skill writes into inbrain's own tree, not the
> brain repo's notes.

This skill is the inverse of `inbrain skillpack scaffold`. Scaffold ships
skills downstream (inbrain → host). Harvest lifts proven patterns
upstream (host → inbrain) so they become references every other client
can scaffold.
...
3. `inbrain skillpack harvest --dry-run` previewed the file set.
4. The real `inbrain skillpack harvest <slug> --from <host>` succeeded
   with `status: harvested` (no privacy-lint hits).
5. `bun test test/skills-conformance.test.ts` passes on the new
   `skills/<slug>/SKILL.md`.
6. The user has reviewed the diff in inbrain and explicitly approved
   the commit.

If any of these is incomplete, the skill is NOT yet harvested — the
files may sit in inbrain's working tree, but they're not landed.

## Output Format

This skill produces three artifacts in inbrain's working tree:
...
- **Skipping the dry-run.** Always preview first. Files land in
  inbrain's working tree; cleanup is a `git checkout` away, but you
  shouldn't need to.
...
- **Harvesting a skill that's still in flux.** Wait until the host
  ...
  then re-harvest, and that churns inbrain's bundle for no benefit.
...
- The user developed a skill in their host fork (Wintermute, Neuromancer,
  Zion, etc.) and wants other inbrain clients to be able to use it
...
3. **The user owns the inbrain checkout.** The harvest writes into
   inbrain's working tree. They'll review and commit. Don't harvest
...
- What slug should the harvested skill have? (Slugs must be kebab-case,
  globally unique in the inbrain bundle.)
...
```bash
inbrain skillpack harvest <slug> --from <host-repo-root> --dry-run
```

The output shows:
- Which files would land in inbrain's tree
...
3. **Fork-specific conventions → references**
   - Mentions of `<host-repo>/docs/...` files → either lift the doc
     into inbrain OR replace with a generic placeholder explanation
...
```bash
inbrain skillpack harvest <slug> --from <host-repo-root>
```

Default behavior:
- Path-confinement + symlink rejection at file copy
- Privacy linter runs against `~/.inbrain/harvest-private-patterns.txt`
...
Outcomes:
- `harvested` — success, manifest updated, files in inbrain's tree
- `lint_failed` — privacy linter caught something. Go back to Phase 3,
  scrub the host file, retry.
- `slug_collision` — inbrain already has a skill at that slug. Either
  use a different slug, or pass `--overwrite-local` if you really
  mean to replace.

### Phase 5 — Verify in inbrain
...
2. `inbrain skillpack check --strict` — confirms no drift between
   bundle and inbrain's own checkout.
3. `inbrain skillpack list` — confirms the slug shows up in the bundle.
4. Review the diff: `cd <inbrainRoot> && git diff -- skills/<slug>/`
5. Commit the additions in inbrain (do NOT commit any leftover files
   in the host repo — harvest is a copy, not a move).

### Phase 6 — Downstream announcement (optional)

If other inbrain clients should pick up the new skill:
...
```bash
inbrain skillpack harvest <slug> --from <host-repo-root> --no-lint
```
...
- It does NOT publish to npm or a remote bundle. It writes to
  inbrain's working tree; the user commits + ships via the normal
  inbrain release process.
- It does NOT support `--all` (no batch harvest). One skill at a
  time keeps the editorial review tractable.

## Files this skill touches

- inbrain's `skills/<slug>/` — every file in the host skill dir
  (copy)
- inbrain's mirror path for declared paired sources
  (e.g. `src/commands/<slug>.ts` if the host SKILL.md declares it
  in frontmatter)
- inbrain's `openclaw.plugin.json` — adds the slug to `skills:`
