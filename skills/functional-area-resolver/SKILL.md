...
Inbrain workspaces often have TWO routing files merged at runtime (per
...
your specific compression. Requires a inbrain repo checkout because the
...
# In your inbrain repo checkout:
cd /path/to/inbrain/evals/functional-area-resolver
...
The harness uses inbrain's bundled fixture set, so this verifies "did the LLM
land in the right sub-skill for routing intents the inbrain-bundled fixtures
...
- ASCII `->` vs Unicode `→` mismatch — the harness now accepts both, but
  earlier versions only matched Unicode. Pin inbrain to v0.32.3.0+.

Common false negatives on the harness eval (NOT bugs in your compression):
- The inbrain-bundled fixtures target skill names like `enrich`, `query`,
...
- The mandatory verification gate in Step 6 fires on the user's edited file, not on sample variants. The user runs `inbrain routing-eval --json` AND the inbrain-repo harness (`node harness.mjs --variants-dir <tmp> --variants my-edit`) before committing the compressed file.
...
### v1.0.0 — 2026-05-11
- Initial version. Pattern shipped in inbrain v0.32.3.0 with a held-out A/B
