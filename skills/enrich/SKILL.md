...
- `inbrain search "name"` and `inbrain query "what do we know about name"`
...
Store raw API responses via `put_raw_data` in inbrain:
```json
{
  "source": "crustdata",
...
- Store raw API data in inbrain (put_raw_data)
- Retrieve raw data from inbrain (get_raw_data)
