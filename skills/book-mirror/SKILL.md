...
book-mirror runs as a CLI command (`inbrain book-mirror`), NOT as a pure
...
4. ANALYZE   → `inbrain book-mirror` fans out N read-only subagents.
...
Resolve `$BRAIN_DIR` from the inbrain config (`inbrain config get sync.repo_path`)
...
1. **Templates: USER.md and SOUL.md** if the user maintains them
   (inbrain ships templates at `templates/USER.md` and `templates/SOUL.md`;
...
3. **Topic-relevant brain searches** tuned to the book's themes:
   - `inbrain query "marriage"`, `inbrain query "couples therapy"` for a
     marriage book.
   - `inbrain query "founders"`, `inbrain query "fundraising"` for a
     business book.
   - `inbrain query "shame"`, `inbrain query "anger"` for a psychology book.
4. **Brain pages for relevant entities** — `inbrain query "<name>"` for
...
CONTEXT="$WORK/context.md"
{
  ...
  # inbrain query the book's key themes, embed top results
  # ...
  echo
  echo "## Themes & cruxes"
...
## 4. Analysis: invoke `inbrain book-mirror`
...
```bash
inbrain put_page  # already done by the CLI; nothing to add here
...
- [ ] `inbrain book-mirror --chapters-dir … --context-file … --slug … --title …` returned exit 0.
