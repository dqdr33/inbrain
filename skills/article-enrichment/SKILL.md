...
3. CONTEXT   → inbrain query the article's key entities to ground "Why It Matters".
...
deterministic CLI command in v0.25.1. The agent uses inbrain's existing
operations:

```bash
# 1. Find candidate pages
inbrain query "needs_enrichment: true type:article" --limit 50

# 2. For each candidate, read the page
inbrain get media/articles/<slug>
