...
# Schema Unification (inbrain-base → inbrain-base-v2)

v0.41.22 ships **inbrain-base-v2** — a 15-type DRY/MECE taxonomy (14 canonical + `note` catch-all) — as the install default for new brains. Existing brains on `inbrain-base` can opt in via the `pack_upgrade_available` onboard finding + the `unify-types` PROTECTED Minion handler.

This skill is the playbook for that migration.

## brain_first: exempt

This skill is ABOUT the brain's shape — it can't depend on the brain it's reshaping. No `inbrain search` lookup first; jump straight to onboard.

## When this skill fires

- Agent runs `inbrain onboard --check` and sees `pack_upgrade_available` or `type_proliferation` warnings
...
A production inbrain brain accreted **94 distinct `pages.type` values** over years of ingestion: tweet / tweet-thread / tweet-bundle / tweet-single / media/x-tweet/bundle / tweet-stub all coexisting; 5.5K concept-redirect pages; atom-partner-link pages that should be links; civic / framework / insight / memo / anecdote one-offs. The cure: collapse to **15 canonical types** (person, company, media, tweet, social-digest, analysis, atom, concept, source, deal, email, slack, writing, project, note) with subtypes/format/origin pushed to frontmatter, alias-rows for redirects, real link-rows for edge-shaped pages, and a catch-all that bins long-tail unknowns to `note` with `frontmatter.legacy_type = <original>` for rollback.

## Workflow

### Phase 1: Discovery

Confirm the brain is actually on `inbrain-base` (not already on v2).

```bash
inbrain schema active --json | jq -r '.identity'
```

Expected: `inbrain-base@1.0.0+<sha>`. If you see `inbrain-base-v2@...`, the brain is already on v2 — skip the migration.
...
inbrain jobs submit unify-types \
  --allow-protected \
  --params '{"target_pack":"inbrain-base-v2"}'
```

Watch progress per phase:

```bash
inbrain jobs follow <job_id>
```
...
3. Acquire `inbrain-unify` db-lock (60min TTL)
4. Apply phases:
...
6. **Flip active pack** to inbrain-base-v2 (D13)
...
- `inbrain schema stats` shows ≤16 distinct types
...
Search queries get a small ranking signal: pages reached via `slug_aliases` (canonicals of one or more aliases) get a 1.05x boost. Visible via `inbrain search --explain`.
...
```bash
inbrain pages restore <slug>
```

Revert the active pack flip:

```bash
inbrain schema use inbrain-base
```
...
- **Don't run two unify jobs concurrently.** The `inbrain-unify` db-lock serializes them; the second submission rejects with "already in progress."

## Decision tree

```
Active pack already inbrain-base-v2?
  → Skip migration.
...
Brain has many custom types not covered by inbrain-base-v2 mapping_rules?
...
Worried about a specific cluster's mapping?
  → Fork inbrain-base-v2 (`inbrain schema fork inbrain-base-v2 my-pack`),
...
Inputs:
- A brain on `inbrain-base` (or any pack with `migration_from: inbrain-base-v2`).
...
- Active pack flipped to `inbrain-base-v2` atomically at end of successful run.

Side effects:
- Source pages soft-deleted with 72h restore TTL (`inbrain pages restore <slug>`).
...
Failure modes:
- Concurrent submission rejected by the `inbrain-unify` db-lock; second call exits gracefully.
...
- Edit `mapping_rules` in `inbrain-base-v2.yaml` to skip clusters you don't trust. Fork the pack instead (`inbrain schema fork`) so the source-of-truth migration stays consistent across brains.
...
- Hard-delete soft-deleted source pages before the 72h restore window. Use `inbrain pages restore <slug>` first if rollback is needed.
...
[unify-types] active_pack flipped from inbrain-base to inbrain-base-v2
```

Final celebration summary to stderr:
```
═══════════════════════════════════════════════════════════
  inbrain-base-v2 migration complete
═══════════════════════════════════════════════════════════
...
JSON output (`inbrain jobs follow <id> --json`) returns the structured `UnifyTypesResult` shape with `per_phase`, `pack_identity_after`, `active_pack_flipped`.
...
- Issue: https://github.com/inbrain-ai/inbrain/issues/1479
