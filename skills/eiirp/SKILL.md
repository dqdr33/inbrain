...
# pack via `inbrain schema show --json`. List the inbrain-recommended set
...
inbrain schema review-candidates --json
inbrain schema review-candidates --apply <prefix-or-type-name>
```

**Confidence floor (codex finding #9):** when `inbrain schema suggest`
...
- The schema-pack engine writes the delta to
  `~/.inbrain/schema-pack-deltas/` — review and merge into the active
  pack via `inbrain schema edit` (or hand-edit the YAML).
...
- Check if a brain page exists (`inbrain search "<name>"` or `inbrain get_page people/<slug>`).
- If exists: update State, append Timeline entry citing this research.
- If not: create with enrichment.

### 4c. Commit and verify
After ALL pages are written, run `inbrain sync` (or commit + push in the
...
```bash
inbrain check-resolvable                         # routing-table reachability
inbrain doctor --json                            # health surface
inbrain search "<topic keywords>"                # brain pages findable
inbrain orphans                                  # any pages without inbound links?
```
...
- [ ] `inbrain doctor` reports `schema_pack_consistency: ok`
...
- **Hardcoding directory tables in EIIRP's logic.** Every filing decision
  reads `inbrain schema show --json`. Users on `inbrain-recommended` AND
...
- **Auto-applying low-confidence schema suggestions.** Confidence < 0.6
  from `inbrain schema suggest` is "manual review required" per codex
...
- **EIIRP consumes the active schema pack as data.** Never hard-code directory tables in EIIRP's logic — read from `inbrain schema show --json` so users who picked `inbrain-recommended` OR custom packs get the right behavior automatically.

## Changelog

### v1.0.0 — inbrain v0.39.0.0
...
- Phase 3 SCHEMA CHECK rewritten to consume the v0.39 cathedral CLI
  ...
  `brain/schema.md`.
- Phase 5 SKILL GRAPH AUDIT calls `inbrain check-resolvable` instead of
  upstream `scripts/skill-dry-check.mjs`.
- Phase 6 verification uses `inbrain doctor`'s schema_pack_consistency
