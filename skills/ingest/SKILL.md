...
- Raw sources are preserved for provenance via `inbrain files upload-raw` with automatic size routing.
...
2. **For each entity mentioned:**
   - Read the entity's page from inbrain to check if it exists
   - If exists: update compiled_truth (rewrite State section with new info, don't append)
   - If new: check notability gate, then store the page in inbrain with the appropriate type and slug
3. **Append to timeline.** Add a timeline entry in inbrain for each event, with date, summary, and source citation.
4. **Create cross-reference links.** Link entities in inbrain for every entity pair mentioned together, using the appropriate relationship type.
...
2. **For each entity detected:**
   - `inbrain search "name"` -- does a page already exist?
   - **If yes:** load context with `inbrain get <slug>`. Use the compiled truth to
   ...
   - **If no:** assess notability (see `skills/_brain-filing-rules.md`). If the entity
     ...
     with what you know.
3. **After creating or updating pages:** sync to inbrain:
   ```bash
...
- `inbrain files signed-url <storage-path>` -- generate 1-hour signed URL for viewing/sharing
- `inbrain files restore <dir>` -- download back to local from cloud storage

Use `put_raw_data` in inbrain to store raw API responses and metadata (JSON, not binary).
...
- Store/update a page in inbrain (put_page)
- Add a timeline entry in inbrain (add_timeline_entry)
- Link entities in inbrain (add_link)
- List tags for a page (get_tags)
- Tag a page in inbrain (add_tag)
- Store raw data in inbrain (put_raw_data)
