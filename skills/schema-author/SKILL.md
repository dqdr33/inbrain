...
tools:
  ...
  - inbrain schema review-orphans
  - inbrain schema detect
  - inbrain schema suggest
  - inbrain schema lint
  - inbrain schema graph
  - inbrain schema explain
  - inbrain schema fork
  - inbrain schema use
  - inbrain schema add-type
  - inbrain schema remove-type
  - inbrain schema update-type
  - inbrain schema add-alias
  - inbrain schema remove-alias
  - inbrain schema add-prefix
  - inbrain schema remove-prefix
  - inbrain schema add-link-type
  - inbrain schema remove-link-type
  - inbrain schema set-extractable
  - inbrain schema set-expert-routing
  - inbrain schema sync
  - inbrain schema reload
...
- **Just looking up a type's settings** → `inbrain schema explain <type>`
...
- **Querying who knows about X** → `skills/expert-routing/SKILL.md` (or
  `inbrain whoknows` directly). schema-author makes a type expert-routable;
...
"who knows about X" (use expert-routing / `inbrain whoknows`).
...
- **5-minute walkthrough:** [`docs/schema-author-tutorial.md`](../../docs/schema-author-tutorial.md) — fork the bundled pack, add a researcher type, sync, prove the T1.5 wiring via `inbrain whoknows`. Use placeholder pages so it runs against any brain without affecting real content.
...
If `source_tier === "default"`, the user is on bundled `inbrain-base` and any
mutation will need a fork first (Phase 4).

### Phase 2 — Assess (what does the current pack cover?)

```
inbrain schema stats --json
```
...
```
inbrain schema review-orphans --limit 50 --json
```
...
If the active pack is bundled (`inbrain-base` or `inbrain-recommended`),
fork it first:
...
```
cd ~/.inbrain/schema-packs/mine
git add pack.json
git commit -m "schema: add researcher + paper types + authored link"
git push
```

If the brain daemon is running (`inbrain serve --http`), other processes
...
- Mutated pack file at `~/.inbrain/schema-packs/<name>/pack.{json,yaml}`.
- Audit row in `~/.inbrain/audit/schema-mutations-YYYY-Www.jsonl` per mutation.
...
- **Inputs:** a natural-language request that names a type / prefix / link verb / flag change, OR the result of `inbrain schema review-orphans` showing untyped pages that need a new type.
- **Outputs:** mutated pack file at `~/.inbrain/schema-packs/<name>/pack.{json,yaml}` + an audit row in `~/.inbrain/audit/schema-mutations-YYYY-Www.jsonl` + (if `sync --apply` ran) backfilled `pages.type` on matching rows.
...
- **Don't mutate `inbrain-base` or `inbrain-recommended`.** Fork first (`inbrain schema fork inbrain-base mine`). These are bundled packs; edits would be lost on upgrade. The mutation primitives refuse with `PACK_READONLY`.
...
- `PACK_READONLY` → you tried to mutate `inbrain-base` or `inbrain-recommended`. Fork first.
...
- `permission_denied` (MCP only) → your OAuth client doesn't have `admin`
  scope. Re-register with `inbrain auth register-client --scopes admin`.
