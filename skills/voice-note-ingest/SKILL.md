...
the transcript text. If not, transcribe via `inbrain transcription` (Groq
Whisper by default; OpenAI fallback for audio > 25MB segmented via ffmpeg).

## The pipeline

```
1. STORE       → Upload original audio to inbrain storage backend
                 (S3 / Supabase Storage / local — pluggable per
                 src/core/storage.ts).
2. TRANSCRIBE  → Use the agent-provided transcript verbatim, OR call
                 inbrain transcription if no transcript was supplied.
...
sources:
  voice-note:
    type: voice_note
    storage_path: "[inbrain storage URL or relative path]"
    acquired: YYYY-MM-DD
...
🔊 [Audio]([inbrain storage URL or relative path])
