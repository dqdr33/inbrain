...
1. **Skill conformance validation** — inbrain's own conformance bar
...
- Disk / memory / CPU
- inbrain: `inbrain doctor --fast --json`
...
4. Check if a dependency was updated (inbrain, bun, etc.).
...
Track results in `~/.inbrain/test-state.json` for trend tracking:
...
```json
{
  ...
  "system": { "doctor": "ok", "inbrain": "0.25.1" },
  "failureHistory": [
