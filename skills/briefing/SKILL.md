...
0. **Hot memory pulse (v0.32).** Before composing anything else, run:
   ...
   4. **Pending consolidation footer** — when `pending_consolidation_count > 0`,
      ...
      whether to run `inbrain dream` before reading further.

   The `--since-last-run` flag advances `~/.inbrain/recall-cursors/<source>.json`
...
1. **Today's meetings.** For each meeting on the calendar:
   - Search inbrain for each participant by name
   - Read their pages from inbrain for compiled_truth context
   - Summarize: who they are, recent timeline, relationship to you
2. **Active deals.** List deal pages in inbrain filtered to active status:
...
4. **Recent changes.** Pages updated in the last 24 hours:
   - What changed and why (read timeline entries from inbrain)
5. **People in play.** List person pages in inbrain sorted by recency:
   - Updated in last 7 days
   - Have high activity (many recent timeline entries)
6. **Stale alerts.** From inbrain health check:
   - Pages flagged as stale that are relevant to today's meetings

## Inbrain-Native Context Loading
...
- `inbrain search "<attendee name>"` -- find their brain page
- `inbrain get <slug>` -- load compiled truth, recent timeline, relationship context
...
- `inbrain search "<sender name>"` -- load sender context
...
- `inbrain query "active deals status"` -- deal pipeline snapshot
- `inbrain query "meetings this week"` -- recent meeting pages with insights
- `inbrain query "pending commitments follow-ups"` -- open threads and action items
- `inbrain search --type person --sort updated --limit 10` -- people in play
...
- **Briefing without brain queries.** Never generate a briefing from memory alone; always query inbrain for current data.
...
- Search inbrain by name (query)
- Read a page from inbrain (get_page)
- List pages in inbrain by type (list_pages)
