...
> skill's Phase 3 below uses `inbrain eval cross-modal` instead — three
...
receipt under `~/.inbrain/.inbrain/eval-receipts/<slug>-<sha8>.json` (the
...
Models resolve through the inbrain AI gateway. Configure once with:
...
from `~/.inbrain/config.json` plus `process.env`.
...
inbrain skillify check skills/<slug>/scripts/<slug>.mjs --json | \
  jq '.[] | .items[] | select(.name | contains("Cross-modal"))'
ls ~/.inbrain/.inbrain/eval-receipts/              # receipt landed
inbrain check-resolvable --json | jq .ok          # resolver clean
```
...
1. **The skill tree on disk.** `skills/<slug>/SKILL.md`, `scripts/<slug>.mjs`,
   ...
   `inbrain skillify scaffold <name>` and refined by the human/agent into a
   real implementation.
2. **A cross-modal eval receipt** at
   `~/.inbrain/.inbrain/eval-receipts/<slug>-<sha8>.json`. The sha-8 binds the
   receipt to the current `SKILL.md` content. `inbrain skillify check`
   surfaces the status (`found` / `stale` / `missing`) as informational.
3. **An audit verdict** from `inbrain skillify check`: `properly skilled` |
...
JSON output (`inbrain skillify check --json`) includes the same fields plus
