...
description: |
  ...
  Use when a user has just finished inbrain setup and asks "now what?"
triggers:
  - "cold start"
  - "fill my brain"
  - "bootstrap brain"
...
- Each phase is independently valuable — the user can stop after any phase and still
  have a useful brain.
- Progress is tracked in `~/.inbrain/cold-start-state.json` so interrupted sessions
  can resume.
- Entity detection and cross-linking run on every import, not as a separate pass.

## Prerequisites

- Inbrain installed and initialized (`inbrain doctor --json` all green)
- Brain repo cloned and synced
- Agent has terminal access and can run `inbrain` CLI commands
...
3. Set environment variables:
   ```bash
   inbrain config set clawvisor_url "https://app.clawvisor.com"
   inbrain config set clawvisor_agent_token "<token>"
   ```
...
6. Save the standing task ID:
   ```bash
   inbrain config set clawvisor_task_id "<task_id>"
   ```
...
inbrain migrate --from obsidian --path /path/to/vault

# For plain markdown directories
inbrain import /path/to/dir --no-embed --workers 4
...
- Run link extraction: `inbrain extract links --source db`
- Run timeline extraction: `inbrain extract timeline --source db`
- Start embeddings: `inbrain embed --stale` (runs in background)

> **Track progress:**
> ```bash
> echo '{"phase_1_complete": true, "pages_imported": N}' > ~/.inbrain/cold-start-state.json
> ```
...
2. **Check brain first** — `inbrain search "name"` to avoid duplicates
...
> ```bash
> inbrain archive-crawler --scan-only --path /path/to/archive
> ```
...
2. **Test retrieval:**
   ```bash
   inbrain query "who do I meet with most often?"
   inbrain query "what am I working on?"
   inbrain search "<person from contacts>"
   ```
...
3. **Set up live sync** (if not already):
   ...
   - X: daily ingest
   - Brain repo: `inbrain sync --repo <path>` every 5-30 minutes

4. **Track state:**
   ```json
   // ~/.inbrain/cold-start-state.json
   {
...
1. Read `~/.inbrain/cold-start-state.json`
