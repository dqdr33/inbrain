...
version: 1.0.0
description: |
  Run `inbrain skillpack-check` to produce an agent-readable JSON health report
  for the inbrain install. Wraps `inbrain doctor` + `inbrain apply-migrations
  ...
  Use when the user asks "is inbrain healthy?", when a cron fires a morning
...
Running `inbrain skillpack-check` returns a JSON report with:
...
- **`doctor`**: full `inbrain doctor --fast --json` output (filesystem checks).
...
- **Daily cron** (e.g. your OpenClaw's `morning-briefing`): `inbrain skillpack-check --quiet`.
...
- **On demand**: `inbrain skillpack-check` for the full JSON when debugging.
...
- `inbrain apply-migrations --yes` — A migration is pending or half-finished.
...
- `inbrain embed --stale` — Embeddings are stale.
- `inbrain check-backlinks --fix` — Dead links or missing back-links.
...
Treat as urgent. Probably means the inbrain binary is missing from `$PATH` or
a required subcommand crashed. Check:

1. `which inbrain` returns a path
2. `inbrain --version` exits 0
3. `~/.inbrain/` is accessible
...
```json
{
  ...
  "healthy": false,
  "summary": "inbrain skillpack needs attention: 1 action(s) — inbrain apply-migrations --yes",
  "actions": ["inbrain apply-migrations --yes"],
  "doctor": {
    "exit_code": 1,
    "checks": [
      { "name": "minions_migration", "status": "fail", "message": "MINIONS HALF-INSTALLED (partial migration: 0.11.0). Run: inbrain apply-migrations --yes" }
    ]
  },
  "migrations": {
    "applied_count": 0,
...
- `inbrain doctor` — the underlying filesystem + DB check. skillpack-check
  composes this.
- `inbrain apply-migrations --list` — the migration status view.
