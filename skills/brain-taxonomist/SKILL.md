...
description: |
  ...
  via `inbrain schema show --json` — no hardcoded directory table. Also
  runs periodic taxonomy drift detection via `inbrain schema review-orphans`.
triggers:
...
driven by `inbrain schema show --json`. This means:
- A user who runs `inbrain schema use inbrain-recommended` gets the full
  ...
  plus all inbrain-base types).
- A user who authored a custom pack via `inbrain schema init` + edit gets
  filing recommendations based on THEIR taxonomy, not inbrain's defaults.
...
```bash
inbrain schema show --json | jq '.page_types[] | select(.primitive == "entity")'
```
...
The `inbrain-recommended` pack treats books as `media/books/<category>/<slug>.md`
...
walk it from `inbrain schema show --json` instead of hardcoding here.
...
inbrain schema review-orphans --json

# What's the overall health?
inbrain doctor --json | jq '.checks[] | select(.name == "schema_pack_consistency")'
```
...
**Suggested next step:** `inbrain schema detect --source <source_id>` then
`inbrain schema review-candidates`.
```

## Anti-Patterns

- **Hardcoded directory table in this skill.** Every decision goes through
  `inbrain schema show --json`. v0.39+ broke the old hardcoded table on
  purpose so users on `inbrain-recommended` or custom packs get the right
...
- **Auto-applying a `inbrain schema review-candidates --apply` decision.**
...
- **Never hardcode a directory table in this skill.** Every decision goes
  through `inbrain schema show --json`. The active pack is canonical.
...
- Hardcoded directory table REMOVED. Every decision now reads the active
  schema pack via `inbrain schema show --json`. Single source of truth.
- Book taxonomy moved from skill-text to the `inbrain-recommended` pack's
  media/books/ branch (see `src/core/schema-pack/base/inbrain-recommended.yaml`).
