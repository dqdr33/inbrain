...
When you encounter friction running inbrain — anything confusing, missing, surprising, or wrong — log it via `inbrain friction log` so maintainers can see it without you writing a bug report. Friction reports drive the claw-test feedback loop (the harness collects, renders, and re-runs).
...
The CLI auto-fills `ts`, `cwd`, `inbrain_version`, and resolves `run_id` from `$INBRAIN_FRICTION_RUN_ID` (set by the harness) or falls back to `standalone.jsonl`. So you can call this anywhere — inside a harness run, manually during normal use, or from a scripted test.
...
```
inbrain friction list                      # recent runs with counts
inbrain friction render --run-id <id>      # markdown report (default)
inbrain friction render --run-id <id> --json
inbrain friction summary --run-id <id>     # friction + delight side-by-side
```
