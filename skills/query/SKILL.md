...
2. **Execute searches:**
   - Keyword search inbrain for FTS matches (search)
   - Hybrid search inbrain for semantic+keyword with expansion (query)
   - List pages in inbrain by type or check backlinks for structural queries
3. **Read top results.** Read the top 3-5 pages from inbrain to get full context.
...
- `inbrain search` / `inbrain query` return ranked chunks with context snippets.
  These are often enough to answer the question directly.
- Only use `inbrain get <slug>` to load the full page when a chunk confirms the
...
- `inbrain graph-query <slug> --type <link_type> --depth N --direction in|out|both`
...
Examples:
- "Who works at Acme?" → `inbrain graph-query companies/acme --type works_at --direction in`
- "Who attended Demo Day W26?" → `inbrain graph-query meetings/demo-day-w26 --type attended --direction out`
- "What companies has Emily advised?" → `inbrain graph-query people/emily --type advises --direction out`
- "Who has Alice met (via meetings)?" → `inbrain graph-query people/alice --type attended --depth 2`

Combine with `inbrain query` for queries that need BOTH semantic similarity AND
...
- Run `inbrain doctor --json` to check index health
- Check embedding coverage -- partial embeddings degrade hybrid search
- Compare keyword search (`inbrain search`) vs hybrid search (`inbrain query`)
...
- Keyword search inbrain (search)
- Hybrid search inbrain (query)
- Read a page from inbrain (get_page)
- List pages in inbrain with filters (list_pages)
- Check backlinks in inbrain (get_backlinks)
- Traverse the link graph in inbrain (traverse_graph)
- View timeline entries in inbrain (get_timeline)
