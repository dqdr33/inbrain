...
version: 0.1.0
description: Universal archivist for personal file archives (Dropbox/B2/Gmail-takeout/local-mount/hard-drive-dump). Filters for high-value content (the user's own writing, ideas, relationships) and surfaces it interactively. REFUSES TO RUN without an explicit inbrain.yml `archive-crawler.scan_paths:` allow-list.
triggers:
  - "crawl my archive"
...
explicitly set in `inbrain.yml`. This is a deliberate safety fence against
the agent over-scoping a scan and ingesting sensitive content (tax PDFs,
medical records, credentials).

```yaml
# inbrain.yml — the allow-list is mandatory
archive-crawler:
  scan_paths:
    - ~/Documents/writing/
    - ~/Dropbox/Archive/
...
in inbrain.yml. Add explicit paths the agent is permitted to scan, then re-run.
...
scan_paths: ["paths from inbrain.yml"]
---
...
## Source
- **Type:** [local|dropbox|...]
- **Allow-listed paths:** [from inbrain.yml]
- **Total files:** [N]
- **Total size:** [X GB]
