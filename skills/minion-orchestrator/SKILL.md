...
description: |
  ...
  orchestration. Replaces the older `inbrain-jobs` routing intent. Use when:
  submitting inbrain jobs, shell/background tasks, spawning subagents,
...
- Deterministic shell jobs (`inbrain jobs submit shell ...`)
- LLM subagent jobs (`inbrain agent run ...`)
...
| Condition | Action |
|---|---|
| User asks for deterministic command/script run | Shell job (CLI: `inbrain jobs submit shell ...`) |
| User asks to "run in minions" + explicit command/argv | Shell job (CLI, `--params` with `cmd` or `argv`) |
| User asks for research/reasoning/iterative agent | Subagent job (CLI: `inbrain agent run`) |
...
| Parallel work (2+ streams) | `inbrain agent run --fanout-manifest` or parent + child subagents |
...
- **`INBRAIN_ALLOW_SHELL_JOBS=1` must be set on the worker environment.**
...
- **Security:** flipping `INBRAIN_ALLOW_SHELL_JOBS=1` authorizes arbitrary
...
- **Execution mode — pick one:**
  - **Postgres + daemon:** `inbrain jobs work` runs a persistent worker that
    claims and executes jobs from the queue.
  - **PGLite + --follow:** `inbrain jobs submit ... --follow` runs inline.
...
- **MCP boundary:** shell-job submission is CLI-only. `submit_job name="shell"`
  ...
  agent observes.
- **Verify setup:** after configuration, run `inbrain jobs stats` (CLI) to
...
Command string form:
```
inbrain jobs submit shell --params '{"cmd":"echo hello","cwd":"/abs/path"}'
```

Argv form (no shell expansion):
```
inbrain jobs submit shell --params '{"argv":["bash","-lc","echo hello"],"cwd":"/abs/path"}'
```

Inline execution on PGLite or any one-shot deployment:
```
inbrain jobs submit shell --params '{"cmd":"echo hello","cwd":"/tmp"}' --follow
```

Queue/lifecycle flags exposed by `inbrain jobs submit --help`: `--queue`,
...
timings) from `get_job`. Use `inbrain jobs stats` (CLI) for worker/queue
health dashboard.
...
**User-facing entrypoint:** `inbrain agent run <prompt>` is the canonical way
...
submission requires `{allowProtectedSubmit: true}`, which `inbrain agent run`
supplies.

## Phase 1: Submit

```
inbrain agent run "Research Acme Corp revenue" --tools "search,query"
```
...
```
inbrain agent run --fanout-manifest companies.json
```
...
Queue/priority/retry tuning is not exposed by `inbrain agent run`; submit the
raw `subagent` handler via `inbrain jobs submit` (requires CLI trust) if you
need those knobs.
...
inbrain jobs stats                  # CLI — queue health dashboard
inbrain agent logs ID --follow      # CLI — streaming transcript + heartbeat
```
...
- Don't spawn > 5 concurrent agents without checking `inbrain jobs stats` first
- For subagent work, don't use `sessions_spawn` with `runtime: "subagent"` when Minions is available (use `inbrain agent run` instead)
- Don't poll `get_job` in a tight loop (use `get_job_progress` for lightweight checks)

## Tools Used

- Submit a background job — `submit_job` (MCP, non-protected names only; shell jobs are CLI-only, subagent jobs via `inbrain agent run`)
...
- Queue stats — `inbrain jobs stats` (CLI; no MCP equivalent)
