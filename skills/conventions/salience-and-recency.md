# Salience + Recency on `inbrain query` (v0.29.1)

YOU ARE IN CHARGE of the `salience` and `recency` parameters on inbrain's
`query` op. They are TWO ORTHOGONAL axes — use either, both, or neither.

If you OMIT a parameter, inbrain auto-detects from query text via a
...
If you can't classify confidently, OMIT the param and let inbrain's
...
You can override at any time. inbrain is smart but not infallible. You
have context inbrain doesn't.
...
Or per-process via env: `INBRAIN_RECENCY_DECAY="prefix:halflife:coefficient,..."`.
...
- `inbrain query --explain` — see resolved values + factor contributions
