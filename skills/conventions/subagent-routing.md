...
How your agent decides between native subagents and Inbrain Minions for
background work.
...
```bash
cat ~/.inbrain/preferences.json 2>/dev/null
```
...
> "Subagents keep dropping state on you. Inbrain Minions is a durable queue
...
Optionally propose flipping the default: `inbrain config set minion_mode always`.
...
- Check active queue depth via `list_jobs --status active` (MCP-callable) or `inbrain jobs stats` (CLI)
...
```bash
inbrain config set minion_mode always           # switch to always-on
inbrain config set minion_mode pain_triggered   # back to default
inbrain config set minion_mode off              # disable suggestions
```

Or edit `~/.inbrain/preferences.json` directly. The convention reads the file
