...
Any agent doing search-adjacent work in a inbrain brain consults this convention:
...
- Skills that recommend tuning ("the cache hit rate is high — raise threshold?"):
  route operators to `inbrain search tune` rather than rolling their own logic.
- New skills that add per-call retrieval overrides: name them explicitly so
  the resolved-knob attribution dashboard (`inbrain search modes`) reads cleanly.
...
Agents tuning a brain's retrieval should call these directly:
    ...
    inbrain search modes              # dashboard + per-knob source attribution
    inbrain search modes --reset      # clear search.* overrides (mode is canonical)
    inbrain search stats [--days N]   # hit rate, intent mix, budget drops
    inbrain search tune [--apply]     # data-driven recommendations

`inbrain search tune` reads the `search_telemetry` rollup (sums + counts of
...
If an operator or agent asks any of these, route to `inbrain search …`:

- "what search mode is active?" → `inbrain search modes`
- "is my cache hot?" → `inbrain search stats`
- "tune my retrieval" → `inbrain search tune`
- "clear search overrides" → `inbrain search modes --reset`
- "compare modes" → `inbrain eval compare`
...
- Don't add per-call `tokenBudget` overrides on the production `query` op
  without naming them in `inbrain search modes` output.
