...
Sub-agents and fresh sessions inherit inbrain tools but not the knowledge of
when and how to use them. This file is that knowledge.

## Available Inbrain Tools

Your tool inventory includes these (prefixed `inbrain__` in OpenClaw):

| Tool | Use for |
|------|---------|
| `inbrain__search` / `search` | Keyword search — fast, always works |
| `inbrain__query` / `query` | Hybrid search (keyword + semantic) — best quality |
| `inbrain__get_page` / `get_page` | Direct page read when you know the slug |
| `inbrain__get_links` / `get_links` | Outgoing links from a page |
| `inbrain__get_backlinks` / `get_backlinks` | Who references this entity |
| `inbrain__get_timeline` / `get_timeline` | Dated events for an entity |
| `inbrain__resolve_slugs` / `resolve_slugs` | Fuzzy slug resolution |
| `inbrain__traverse_graph` / `traverse_graph` | Walk the relationship graph |
| `inbrain__put_page` / `put_page` | Create or update a brain page |
| `inbrain__add_timeline_entry` | Add a dated event |
| `inbrain__add_link` | Add a relationship edge |

Tool names vary by transport (MCP uses short names, OpenClaw plugin uses
`inbrain__` prefix). Both work. Use whichever your environment provides.
...
- **After any brain page write:** trigger a sync so new pages are searchable.
  In OpenClaw: `inbrain__sync_brain`. From CLI: `inbrain sync --no-pull`.
...
| `projects/` | project | `projects/inbrain.md` |
...
out; the `inbrain doctor --fix` auto-add of the canonical Convention callout
