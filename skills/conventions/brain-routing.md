...
- **Brain** = which DATABASE. `--brain`, `INBRAIN_BRAIN_ID`, `.inbrain-mount`.
- **Source** = which REPO INSIDE the database. `--source`, `INBRAIN_SOURCE`,
  `.inbrain-source`.
...
1. Run `inbrain mounts list` if you haven't seen the user's mounts yet.
2. Trust the resolver. If the user is in `~/team-brains/media/`, their
   `.inbrain-mount` pins brain=media-team. Don't override that silently.
...
Bare `inbrain query "X"` routes to the default brain's default source. That
...
`inbrain` resolves the active source via `resolveSourceId()` in
`src/core/source-resolver.ts`. Seven tiers, highest priority first:

| # | Tier | Signal |
|---|---|---|
| 1 | `flag` | Explicit `--source <id>` CLI flag (or `--source-id <id>` on `inbrain extract` / `inbrain import`) |
| 2 | `env` | `INBRAIN_SOURCE` environment variable |
| 3 | `dotfile` | `.inbrain-source` file in CWD or any ancestor directory |
...
| 5.5 | `sole_non_default` | When tiers 1–5 missed AND exactly one registered source has a `local_path` AND isn't `'default'`, auto-route to it. Fires a one-time stderr nudge per CLI invocation. Suppress with `INBRAIN_NO_SOLE_NON_DEFAULT_NUDGE=1`. |
...
Pre-fix, `inbrain sync` from `/tmp` against a brain registering only
...
`sources.default` via `inbrain sources default <id>` has stated intent that
wins over the auto-route. Archived sources are excluded from the count.

**v0.37.7.0 tooling:**

- `inbrain sources current [--json]` echoes the resolved source AND
  which tier won. Run this before any destructive op to verify what
  you're about to target.
- `inbrain sources current --source X` shows what an explicit flag
  WOULD resolve to (validates X exists in the sources table).

CLI commands honoring this chain: `inbrain sync`, `inbrain import`,
`inbrain search`, `inbrain extract` (via `--source-id <id>` since
`--source` is the fs|db data-source axis), `inbrain graph-query`
(via `--include-foreign` for cross-source traversal).

**Trust boundary (v0.34.1.0):** the resolver is CLI-layer only.
Operations.ts handlers do NOT read `.inbrain-source` or
`INBRAIN_SOURCE`. MCP/remote callers go through
...
- The user is working in a specific repo (the `.inbrain-source` dotfile
...
2. Check `inbrain mounts list` for relevant brain ids.
...
- Ignoring `.inbrain-mount` / `.inbrain-source` dotfiles. They're load-bearing
