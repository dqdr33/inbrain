...
## 1. inbrain's internal tier system (v0.31.12+)

This is how inbrain itself picks which Claude/OpenAI/Google model runs each
...
| `deep` | slow, expensive reasoning | `claude-opus-4-7` | `inbrain think`, auto-think, cross-modal eval slot B |
| `subagent` | Anthropic-only multi-turn tool loop | `claude-sonnet-4-6` | `inbrain agent run` |

Override priority (highest first):

1. CLI flag (`--model opus`)
2. Per-task config (`inbrain config set models.dream.synthesize opus`)
3. Deprecated per-task config (stderr-warns once, then honored)
4. **Global default** (`inbrain config set models.default opus`) — single hammer
5. **Tier override** (`inbrain config set models.tier.reasoning opus`)
6. Env var (`INBRAIN_MODEL=opus`)
...
inbrain config set models.default opus

# Use opus only for reasoning + deep, keep haiku for utility
inbrain config set models.tier.reasoning opus
inbrain config set models.tier.deep opus

# Custom alias, then use it everywhere
inbrain config set models.aliases.frontier anthropic:claude-opus-4-7
inbrain config set models.default frontier
```

Visibility:

```bash
inbrain models                    # print current routing table
inbrain models doctor             # 1-token probe to each configured model
```
...
work, not what inbrain calls internally.
