...
Add the type:
```
inbrain schema add-type meeting \
  --primitive temporal \
  --prefix meetings/ \
  --extractable
...
- **Don't mutate `inbrain-base` or `inbrain-recommended`.** Fork first.
...
If your pack lives in source control (`~/.inbrain/schema-packs/<name>/`
...
v0.41.22 ships **inbrain-base-v2** as the declared successor to
inbrain-base@1.x — collapses 94 noisy types to 15 canonical via
declarative mapping_rules. Run via `inbrain onboard --check --explain`
(preview) → `inbrain jobs submit unify-types --allow-protected --params
'{"target_pack":"inbrain-base-v2"}'` (apply). See
...
- Custom types not covered by the successor's mapping_rules → fork the
  successor first (`inbrain schema fork inbrain-base-v2 my-pack`), edit
  rules, then target your fork.
- Mid-ingest or autopilot maintenance → wait. Unify holds the
  `inbrain-unify` db-lock for ~10 min on big brains.
