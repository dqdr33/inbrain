...
How cron-scheduled agent work is dispatched in a Inbrain-backed install.
...
# cycle slot. The queue dedupes long-running overlaps at the DB layer.
{
  "schedule": "*/30 * * * *",
  "kind": "shell",
  "cmd": "inbrain jobs submit ea-inbox-sweep --params '{\"slot\":\"$(date -u +%Y-%m-%dT%H:%M)\"}' --idempotency-key ea-inbox-sweep:$(date -u +%Y-%m-%dT%H:%M)"
}
...
# lock blocks a separate worker daemon, so the cron runs the job directly.
{
  "schedule": "*/30 * * * *",
  "kind": "shell",
  "cmd": "inbrain jobs submit ea-inbox-sweep --params '{}' --follow"
}
```
...
- **Durability.** Gateway restart mid-task? Worker picks the job up on
  boot. No lost state.
- **Observability.** `inbrain jobs list` + `inbrain jobs get <id>` show
...
**Inbrain only rewrites cron entries whose handler name matches a
inbrain builtin** (`sync`, `embed`, `lint`, `import`, `extract`,
...
Users who set `minion_mode: off` in `~/.inbrain/preferences.json` keep
using `agentTurn`. Respect that. No auto-rewrite.

## Forward note (v0.12.0)

Inbrain v0.12.0 ships `inbrain cron`: a scheduler loop inside
`inbrain jobs work` that owns cron expressions natively — no more
...
- `skills/migrations/v0.11.0.md` — how Inbrain migrates an existing host
