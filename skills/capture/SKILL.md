...
fragment, or any text into their brain, run `inbrain capture`. Don't reach
for `inbrain put` or commit-then-sync — `capture` is the front door and it
...
- **Side effect:** the page becomes immediately queryable via `inbrain query`,
  `inbrain search`, or any MCP-bound agent.
...
`inbrain capture` resolves to a `put_page` call (local) or a remote MCP call
...
```bash
inbrain capture "the thought I want to remember"
inbrain capture --file ./notes/today.md
echo "from a pipe" | inbrain capture --stdin
inbrain capture "..." --slug daily/2026-05-21
inbrain capture "..." --type idea --source voice-whisper
inbrain capture "..." --quiet          # script-friendly: prints just the slug
inbrain capture "..." --json           # structured output for agents
```
...
`--quiet` prints only the slug (use for `SLUG=$(inbrain capture "..." --quiet)`).
`--json` prints structured output for downstream tools.

## Anti-Patterns

- **Don't reach for `inbrain put`.** That's the old per-page primitive that
...
- **Don't try to bulk-import dozens of files by looping over `inbrain capture`.**
  That's what `inbrain sync` (or `inbrain import`) is for. Capture is for
...
- Bulk ingestion of many files → `skills/media-ingest/SKILL.md` or `inbrain sync` instead
