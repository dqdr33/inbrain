...
```bash
ls ~/.inbrain/integrations/twilio-voice-brain/heartbeat.jsonl 2>/dev/null
```
...
- Is there a public URL configured in `~/.inbrain/config.json`?
...
with `inbrain integrations show twilio-voice-brain`." Skip to Step 6.
...
`inbrain integrations show twilio-voice-brain`."
...
echo '{"version":"0.8.0","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","voice_setup":"RESULT"}' >> ~/.inbrain/migrations/completed.jsonl
```

Tell the user: "v0.8.0 migration complete. Run `inbrain integrations doctor` to
