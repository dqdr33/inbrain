...
merged on top of v0.11.1 Minions). The migration is **automatic** — `inbrain
post-upgrade` calls `inbrain apply-migrations --yes` which invokes the v0.12.0
...
- **`inbrain extract --source db`** for batch backfill on existing brains.
- **`inbrain graph-query <slug>`** for typed-edge relationship traversal with
  cycle prevention.
- **Backlink-boosted hybrid search**: well-connected entities rank higher
  (`score *= 1 + 0.05 * log(1 + n)`).
- **Graph health metrics in `inbrain health`**: `link_coverage`,
...
### Phase F — Record
Appends to `~/.inbrain/migrations/completed.jsonl` so future `inbrain
...
```bash
inbrain init --migrate-only        # Phase A
inbrain extract links --source db  # Phase C
inbrain extract timeline --source db  # Phase D
inbrain stats                      # Phase E (look for link_count > 0)
inbrain graph-query <some-person-slug> --depth 2  # smoke test
```
...
```bash
inbrain graph-query people/<slug> --type attended --depth 2
inbrain graph-query companies/<slug> --type works_at --direction in
```
...
```bash
inbrain config set auto_link false
```

Re-enable with `inbrain config set auto_link true`.

## Branch-install recovery (very rare)

If you ran the `inbrain-ai/link-timeline-extract` branch BEFORE this merge (so
