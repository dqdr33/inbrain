...
- `inbrain frontmatter` CLI (validate / audit / install-hook)
- `frontmatter_integrity` subcheck under `inbrain doctor`
...
- Audit-only migration that scans every registered source, writes
  `~/.inbrain/migrations/v0.22.4-audit.json`, and queues per-source TODO
  entries to `~/.inbrain/migrations/pending-host-work.jsonl`
- 0 warnings on `inbrain check-resolvable` (down from 7 on master)
...
- `audit` — `scanBrainSources()` over every registered source; writes
  `~/.inbrain/migrations/v0.22.4-audit.json`
- `emit-todo` — appends one entry per source-with-issues to
  `~/.inbrain/migrations/pending-host-work.jsonl`. Each entry contains the
  exact `inbrain frontmatter validate <source-path> --fix` command.
...
```bash
cat ~/.inbrain/migrations/v0.22.4-audit.json
```
...
```bash
inbrain frontmatter validate <source_path> --fix
```

`--fix` writes a `.bak` backup for every modified file. SLUG_MISMATCH errors
are surfaced for manual review (not auto-fixed) — inbrain derives slugs from
...
```bash
inbrain doctor --json | jq '.checks[] | select(.name == "frontmatter_integrity")'
inbrain frontmatter audit --json | jq '.total'
```
...
- contents of `~/.inbrain/migrations/v0.22.4-audit.json`
