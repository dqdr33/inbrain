...
version: 0.23.0
feature_pitch:
  headline: "inbrain dream now actually dreams: conversation transcripts → reflections, originals, and 25-year patterns."
  description: |
...
**Audience: host agents reading this after `inbrain apply-migrations` has
...
`inbrain upgrade` chains to `inbrain apply-migrations --yes`, which runs:
...
`inbrain dream` (and `inbrain autopilot`) now run an 8-phase cycle:
...
inbrain dream --phase synthesize --dry-run --json
```
...
inbrain config set dream.synthesize.min_chars 2000
...
inbrain config set dream.synthesize.exclude_patterns '["medical","therapy"]'
...
inbrain config set dream.synthesize.cooldown_hours 12

# Patterns lookback window in days (default 30).
inbrain config set dream.patterns.lookback_days 30

# Minimum distinct reflections needed to name a pattern (default 3).
inbrain config set dream.patterns.min_evidence 3
```
...
- **Auto git commit + push.** v1 writes markdown files to `brain_dir`
  ...
  or let `inbrain autopilot` handle it. v1.1 will add explicit
...
inbrain dream --help    # shows the 8-phase pipeline

# Dry-run against a single transcript (cheap Haiku call only):
inbrain dream --phase synthesize --input /tmp/some-transcript.txt --dry-run --json
```

If any step fails, file an issue with `inbrain doctor` output and the
