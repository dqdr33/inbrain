...
1. Schema-pack additions: `conversation` (temporal, extractable) and `atom` (annotation, NOT extractable) promoted into `inbrain-base`.
2. New CLI command `inbrain extract-conversation-facts` for bulk fact extraction over long-form conversation pages.
...
`inbrain upgrade` + `inbrain apply-migrations --yes` handles the binary + schema. The cycle phase and doctor check require no agent action.
...
1. **Estimate cost first.** For each source the user wants to backfill:
   ```bash
   inbrain sources audit <source-id> --json | jq '.facts_backfill_estimate'
   ```
...
2. **Run a dry-run** to preview segmentation:
   ```bash
   inbrain extract-conversation-facts --dry-run --slug <slug-of-one-conversation-page>
   ```
...
3. **Run with explicit cost cap.** Default cap is $5. Recommend matching the audit estimate (rounded up):
   ```bash
   inbrain extract-conversation-facts --background --max-cost-usd <cap>
   inbrain jobs follow <printed-job-id>
   ```
...
4. **Verify with doctor:**
   ```bash
   inbrain doctor --json | jq '.checks[] | select(.name == "conversation_facts_backlog")'
   ```
...
- Extracted facts use `source = 'cli:extract-conversation-facts'`. To remove them in bulk (rare), the only path today is raw SQL: `DELETE FROM facts WHERE source LIKE 'cli:extract-conversation-facts%'`. A `inbrain forget --where` bulk flag is a v0.42+ TODO.
