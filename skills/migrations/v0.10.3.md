...
Every `inbrain put` (and MCP `put_page`) now extracts entity references from the
...
### Extended `inbrain extract` + new `inbrain graph-query`

- `inbrain extract links --source db` — backfill structured links by walking pages from
  the engine (works on live brains with no local checkout). FS-source still works:
  `inbrain extract links --source fs --dir <path>` walks markdown files (v0.10.1 behavior preserved).
  Includes typed link inference, within-page dedup, content-hash-based `--since` filter.
- `inbrain extract timeline --source db` — backfill structured timeline entries from page
...
### Schema migrations (auto-applied on `inbrain init`)
...
### Graph health metrics in `inbrain health`
...
```bash
inbrain extract links --source db
```
...
If you have a local markdown checkout you'd rather walk: `inbrain extract links
...
```bash
inbrain graph-query <any-person-slug> --depth 2
```
...
Use `inbrain graph-query <slug> --type <link_type>` to filter.

### Step 7: Auto-link is on by default
No action needed. Every future `inbrain put` automatically updates the graph.
To disable: `inbrain config set auto_link false`.

### Step 8: Record completion
```bash
mkdir -p ~/.inbrain/migrations
echo '{"version":"0.10.3","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","status":"complete"}' >> ~/.inbrain/migrations/completed.jsonl
```
