...
- **`inbrain takes`** CLI — list / search / add / update / supersede / resolve.
...
- **`inbrain think` op surface** — registered now; pipeline lands in v0.28.x.

## What the migration did automatically

`inbrain apply-migrations --yes` runs three orchestrator phases:
...
2. **Backfill takes** — walked every page in the brain, parsed any fenced
   `<!--- inbrain:takes:begin/end -->` table found, populated the takes
   index in Postgres. Idempotent; safe to re-run.
3. **Re-chunk TODO queued** — `~/.inbrain/migrations/pending-host-work.jsonl`
...
inbrain doctor --check takes_fence_chunk_leak
...
inbrain extract takes --rebuild
inbrain sync   # picks up the markdown delta if any
```
...
```bash
# Old (deprecated):
inbrain config set dream.synthesize.model claude-sonnet-4-6
inbrain config set dream.patterns.model claude-sonnet-4-6

# New (one key controls everything):
inbrain config set models.default sonnet

# Per-op override (optional):
inbrain config set models.dream.synthesize opus

# Cleanup deprecated keys:
inbrain config unset dream.synthesize.model
inbrain config unset dream.patterns.model
```
...
inbrain auth permissions <token-name> set-takes-holders world,garry,brain

# Or create a new token with the wider set up-front
inbrain auth create my-claude-desktop --takes-holders world,garry,brain
```
...
```bash
# Add a take by hand
inbrain takes add people/alice-example \
  --claim "Strong technical founder I have ever met" \
  --kind take --who garry --weight 0.85 \
  --source "OH 2026-05-01"

# List
inbrain takes people/alice-example

# Search
inbrain takes search "technical founder"

# Resolve a bet
inbrain takes resolve people/alice-example --row 3 --outcome true \
...
```bash
inbrain doctor         # all checks green; schema_version >= 32
inbrain stats          # numbers stable

# If any step failed, file an issue:
# https://github.com/inbrain-ai/inbrain/issues
# Include: inbrain doctor output, ~/.inbrain/upgrade-errors.jsonl if it exists,
...
- `inbrain think` synthesis pipeline (gather → RRF → cite → synthesize)
- `inbrain takes seed <slug>` — LLM extracts claims from page prose
