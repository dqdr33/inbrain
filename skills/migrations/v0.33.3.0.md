...
- **CLI source-scoping default flipped** (Codex finding #7): `inbrain
...
unless you watch `inbrain doctor`'s autopilot stats.
...
The previously available CLI commands (`inbrain code-callers <symbol>`)
...
```bash
# 1. Confirm upgrade
inbrain --version    # 0.33.3.0

# 2. Confirm migration v51 ran
inbrain apply-migrations --status | grep edges_backfilled_at_v0_34

# 3. Confirm the new MCP ops are exposed
inbrain --tools-json | jq '.[] | select(.name | startswith("code_")) | .name'
...
inbrain code-callers parseMarkdown --json | jq '.count'

# 5. (Optional) inspect the resolver's per-source progress
inbrain doctor --json | jq '.checks | to_entries[] | select(.key | contains("code"))'
```

## Rollback

If you need to roll back: `git checkout` an older binary and run
`inbrain doctor`. The v51 column + indexes stay in your DB (no-op for
older binaries; idempotent re-add on upgrade). No data is destroyed.

## What v0.34 will add on top

Per the v0.34 design doc (`/Users/inbrain-ai/.gstack/projects/inbrain-ai-inbrain/inbrain-ai-inbrain-ai-miami-design-20260510-194145.md`):
...
- `inbrain wiki` zero-LLM aggregator CLI
