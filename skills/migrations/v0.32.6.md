...
feature_pitch:
  ...
  one_liner: "inbrain eval suspected-contradictions samples retrieval pairs and flags conflicts, with doctor + MCP + dream-cycle integration."
---

# v0.32.6 Migration: Contradiction Probe

The agent ran this version migration when the user upgraded inbrain. After
upgrade, this file is read by the auto-update agent which performs the
following steps automatically.

## Mechanical (handled by `inbrain apply-migrations`)
...
- `eval_contradictions_runs` — one row per `inbrain eval
...
inbrain eval suspected-contradictions --query "what is acme's MRR" --top-k 5 --json

# Run against a queries file:
inbrain eval suspected-contradictions --queries-file ~/.inbrain/queries.jsonl --top-k 5

# Inspect findings without re-running:
inbrain eval suspected-contradictions review --severity high
...
After the first probe run, `inbrain doctor` shows a `contradictions` check
...
```bash
inbrain doctor --json | jq '.checks[] | select(.name == "contradictions")'
```
...
didn't apply — run `inbrain apply-migrations --yes` and re-check.
