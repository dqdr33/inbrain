...
- `tokenmax` mode now defaults `search.reranker.enabled = true`. If the
  ...
  observable failure in `inbrain doctor`.
...
re-embed. Edit `~/.inbrain/config.json`:
...
inbrain models doctor                  # verify config
inbrain embed --stale --limit 50       # smoke a small re-embed
...
inbrain models doctor                  # verify reranker_config + reachability
inbrain query "some query that previously misranked"
```
...
- `inbrain models doctor --json` — probes `embedding_config`, `reranker_config`,
  and reranker reachability. Surfaces config issues with paste-ready fix hints.
- `inbrain doctor` — runs `reranker_health` against the JSONL audit at
  `~/.inbrain/audit/rerank-failures-YYYY-Www.jsonl`. Auth failures warn
  immediately; transient failures warn at >=5 in 7 days.
- `tail -100 ~/.inbrain/audit/rerank-failures-*.jsonl` — raw failure log
