...
- **Shell-job pre-enqueue validator** (`src/core/minions/handlers/shell-validate.ts`,
  ...
  `inbrain jobs submit shell` (CLI) and `submit_job` MCP op for `name='shell'`.
...
- **Env-key derivation**: name uppercased by default. One override:
  `database_url` → `INBRAIN_DATABASE_URL` (plain `DATABASE_URL` is ambiguous
...
- **Defense-in-depth re-validation** in the shell handler at job-pickup
  ...
  pre-enqueue call.
- **`inbrain doctor home_dir_in_worktree` check** warns when `~/.inbrain/`
  lives inside a git worktree. Handles `.git` as directory or file.
- **`~/.inbrain/.gitignore` retroactive** via `ensureGitignore()` called from
  `saveConfig()` AND `inbrain post-upgrade`. Idempotent, never clobbers.
- **Output-side redaction (opt-in)**: set `redact_secrets: true` on the job
  ...
  `echo "$INBRAIN_DATABASE_URL"`, not adversarial encode-then-print. Default
  false (back-compat).
- New canonical guide: `docs/guides/agent-to-inbrain.md`. Two-domain framing
...
under the derived key (`database_url` → `INBRAIN_DATABASE_URL`,
...
with a paste-ready `inbrain config set <name> <value>` hint.
...
inbrain jobs submit shell --params \
  '{"cmd":"inbrain stats","cwd":"/tmp","inherit":["database_url"]}' --follow
# Expect: page count, exit 0.

# 2. The doctor surfaces worktree risk if it exists:
inbrain doctor --json | grep -A1 home_dir_in_worktree

# 3. The retroactive gitignore landed:
test -f ~/.inbrain/.gitignore && cat ~/.inbrain/.gitignore
# Expect: file exists, contents = "*\n"

# 4. The audit-log records names not values:
tail -1 ~/.inbrain/audit/shell-jobs-*.jsonl | grep -o '"inherit":\[[^]]*\]'
...
jobs: write `database_url` plaintext to `~/.inbrain/config.json`, or pass
`env: { INBRAIN_DATABASE_URL: ... }` per-job. Both work. Both leave plaintext
