...
hosts) reading this AFTER `inbrain apply-migrations` has run its
...
## Check what inbrain left you

```bash
cat ~/.inbrain/migrations/pending-host-work.jsonl
```
...
Gbrain rewrites cron entries whose handler name matches a inbrain
builtin (`sync`, `embed`, `lint`, `import`, `extract`, `backlinks`,
`autopilot-cycle`). For host-specific handlers (e.g. `ea-inbox-sweep`,
`frameio-scan`, `x-dm-triage`, `calendar-sync` on your OpenClaw), inbrain
...
Autopilot already spawns `inbrain jobs work` as a child. Configure it to
...
One test that exercises the full pipeline: scheduler → `inbrain jobs
...
Run `inbrain check-resolvable`. It validates reachability (is the skill
...
cron entry from `agentTurn ea-inbox-sweep` to `inbrain jobs submit ...`
...
`inbrain apply-migrations`, and let inbrain re-detect + update
...
- `skills/conventions/cron-via-minions.md` — the rewrite pattern
  inbrain applies for builtins + the one your host ships for custom
  handlers.
