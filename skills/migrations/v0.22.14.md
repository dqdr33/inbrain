...
feature_pitch:
  ...
  body: |
    Bare `inbrain jobs work` now ships with the same health protection the
    ...
    If you're using `inbrain jobs supervisor`, you're already protected — the
    supervisor handles spawn-on-crash itself. The fail-stop concern only
    applies to direct `inbrain jobs work` invocations.
---
...
If you run `inbrain jobs work` directly (NOT under `inbrain jobs supervisor`),
...
| cron watchdog | Cron entry that re-spawns when `pgrep -f "inbrain jobs work"` is empty |
...
restart policy OR switch to `inbrain jobs supervisor` (which spawns its own
child + restarts on crash internally).

## What ships

- DB liveness probes inside `inbrain jobs work` (60s interval, 3 strikes → exit)
...
- `INBRAIN_SUPERVISED=1` env var (set by supervisor on its child) causes the
...
**`inbrain jobs work` now defaults `--max-rss 2048` (MB).** Pre-v0.22.14, bare
...
- You run `inbrain jobs work` directly under a process manager (systemd, Docker,
...
- You run `inbrain jobs supervisor` (the supervisor already defaulted to
...
If the user runs OpenClaw, Hermes, or any agent fork that uses `inbrain jobs
work` directly (NOT under `inbrain jobs supervisor`), they need to know. Check:

```bash
# Look for bare worker invocations in cron, systemd, Docker, launchctl, etc.
grep -r "inbrain jobs work" ~/.config/systemd/ /etc/systemd/ 2>/dev/null | head
crontab -l 2>/dev/null | grep "inbrain jobs work" | head
launchctl list 2>/dev/null | grep inbrain | head
ps -ef | grep "inbrain jobs work" | grep -v grep | grep -v supervisor
```
...
> "v0.22.14 changes `inbrain jobs work` to default `--max-rss 2048` (MB).
...
**systemd (~/.config/systemd/user/inbrain-worker.service):**

```ini
ExecStart=/usr/local/bin/inbrain jobs work --queue default --concurrency 3 --max-rss 4096
# Or to opt out: --max-rss 0
```

Then `systemctl --user daemon-reload && systemctl --user restart inbrain-worker`.

**cron (`crontab -e`):**

```cron
@reboot /usr/local/bin/inbrain jobs work --queue default --concurrency 3 --max-rss 4096
```

**Docker compose:**

```yaml
command: ["inbrain", "jobs", "work", "--queue", "default", "--concurrency", "3", "--max-rss", "4096"]
```

**launchctl (~/Library/LaunchAgents/com.user.inbrain-worker.plist):**

```xml
<key>ProgramArguments</key>
<array>
  <string>/usr/local/bin/inbrain</string>
  <string>jobs</string>
  <string>work</string>
...
```bash
inbrain jobs work --health-interval 0 --max-rss 0
```

### 6. Verify

```bash
inbrain jobs stats              # queue should be flowing normally
inbrain doctor --json | jq '.'  # no critical warnings
ps -o rss= -p $(pgrep -f "inbrain jobs work") | awk '{print $1/1024 " MB"}'
```
...
60s` segment (because `INBRAIN_SUPERVISED=1` skips the child's self-monitor).

### 7. If anything fails

Open an issue at https://github.com/inbrain-ai/inbrain/issues with:
