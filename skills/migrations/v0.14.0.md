...
feature_pitch:
  ...
  description: |
    ...
    scale. Retry, backoff, DLQ, unified `inbrain jobs list` visibility, all
...
hosts) reading this AFTER `inbrain apply-migrations` has run. There is no
...
**Before** (LLM gateway):
```
OpenClaw cron: x-inbrain-ai-unified, 3 13,16,19,22,1,4,7,10 * * *
  → runs agentTurn x-inbrain-ai-unified
...
3 13,16,19,22,1,4,7,10 * * * \
  inbrain jobs submit shell \
    --params '{"cmd":"node /data/.openclaw/workspace/scripts/x-inbrain-ai-daily.mjs","cwd":"/data/.openclaw/workspace"}' \
...
1. Wait for the next scheduled fire (or trigger manually: `inbrain jobs
   submit shell --params '...' --follow`).
2. Check `inbrain jobs list --status completed --name shell --limit 5`
   for the result.
3. `inbrain jobs get <id>` shows `exit_code`, `stdout_tail`, `stderr_tail`,
   `duration_ms`.
...
`INBRAIN_ALLOW_SHELL_JOBS=1` on the worker, jobs sit in `waiting`
...
- **Automatic crontab rewrites.** Deferred to a future `inbrain
...
- **Orphaned-shell-job stats.** `inbrain jobs stats --orphaned` would
...
1. The worker runs with `INBRAIN_ALLOW_SHELL_JOBS=1` (Postgres) or every
...
4. `inbrain jobs stats` shows shell jobs completing at expected rates
