...
users** — re-run `inbrain init --migrate-only` and re-run `inbrain extract all` if
...
1. **`inbrain extract` no longer hangs on large brains.** The N+1 dedup pre-load
...
the v0.12.0 upgrade attempt. Run `inbrain init --migrate-only` once to confirm
your schema is at version 10, then run `inbrain extract all --dir <brain>` if
...
Re-running `inbrain init --migrate-only` is a no-op for v9 (uses
...
### Re-running `inbrain extract` on a previously-stuck brain

This is the most common case. Run:

```bash
inbrain extract all --dir <brain-dir>
# or for live brains with no local checkout:
inbrain extract all --source db
```
...
If you hit any issue, file at https://github.com/inbrain-ai/inbrain/issues with the
output of `inbrain init --migrate-only` and `inbrain config get version`.
