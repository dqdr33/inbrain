...
feature_pitch:
  ...
  one_liner: "inbrain code-refs BrainEngine --json returns every usage site in <100ms."
  user_action_required: true
---

# v0.19.0 — Code Indexing

This release makes code a first-class citizen in the brain. Tree-sitter parses 29 languages into semantic chunks. `inbrain code-def` and `inbrain code-refs` let agents find symbol definitions and references without grep. Incremental chunking drops daily autopilot embedding cost by ~95%. The chunker is a strict superset of Chonkie's CodeChunker plus a structured header Chonkie lacks.
...
These run as part of `inbrain upgrade` → `inbrain apply-migrations`. No manual DDL needed.
...
1. **Confirm migrations landed:**
   ...
   inbrain doctor
   ```
   Look for `schema_version: 26`. If lower, run `inbrain apply-migrations --yes`.

2. **Register a code source** if the user wants their code indexed:
   ```bash
   inbrain sources add <id> --path <path-to-repo>
   ```
   Pick a short `<id>` (e.g. `wiki`, `inbrain`, `yc-media`). Shorter is better — it's used in citation keys.

3. **Sync the code source:**
   ```bash
   inbrain sync --source <id>
   ```
...
4. **Verify code-def and code-refs work:**
   ```bash
   inbrain code-def BrainEngine       # prints the file + line of the definition
   inbrain code-refs BrainEngine --json  # JSON array of every usage site
   ```
...
v0.19.0 deletes `~/.inbrain/config.json`'s `repos` array in favor of the `sources` table. The CLI surface is preserved as a deprecated alias: `inbrain repos add` still works, but routes into `runSources` with a one-line deprecation notice on stderr. Existing scripts keep working; prefer `inbrain sources` going forward.

If you had repos configured in `~/.inbrain/config.json`, re-register them:
```bash
inbrain sources add <name> --path <path>
```
Per-repo sync bookmarks live in the `sources` table now (not config.json).

## Flag in `pending-host-work.jsonl`

Per the v0.11.0 convention, the migration orchestrator writes an entry to `~/.inbrain/migrations/pending-host-work.jsonl` flagging the new CLI surfaces so headless agents can walk the TODOs:

```json
{"version": "0.19.0", "action": "register_code_source", "status": "pending"}
```

Agents that handle pending-host-work should offer the user a `inbrain sources add ...` prompt.
