...
feature_pitch:
  ...
  description: |
    ...
    inbrain backend can now hold multiple knowledge repos (wiki, gstack,
    ...
    Per-directory default via `.inbrain-source` dotfile walk-up +
    `INBRAIN_SOURCE` env var. Matches how kubectl / terraform / git
    scope context. `cd ~/yc-media && inbrain query "X"` just works.
...
**Audience: host agents reading this after `inbrain apply-migrations`
...
`inbrain upgrade` chains to `inbrain apply-migrations --yes`, which
runs:
...
inbrain sources rename <id> <new-display-name>
inbrain sources default <id>
inbrain sources attach <id>    # write .inbrain-source in CWD
inbrain sources detach         # remove .inbrain-source
...
Running `inbrain sources attach gstack` inside `~/.gstack/` writes a
`.inbrain-source` file containing the single word `gstack`. Any
inbrain command run from that directory (or any subdirectory) auto-
selects `gstack` as the default source. `inbrain sources detach`
removes the dotfile.
...
`inbrain sources rename`, existing citations stay valid.
...
- `inbrain sources import-from-github <url>` — deferred to a patch
...
standard `inbrain sync` flow, it continues to target the default
source and sees no behavioral change. To start using multi-source:

```
# Register a new source
inbrain sources add gstack --path ~/.gstack --no-federated

# Pin that directory to it so no --source flag is needed
cd ~/.gstack
inbrain sources attach gstack
