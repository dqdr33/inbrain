...
Run `inbrain sync` and check for "Skipped ... does not match path-derived slug"
...
`inbrain embed --all` now runs 30x faster (sliding worker pool, 20 concurrent
...
for most brains. Set `INBRAIN_EMBED_CONCURRENCY=10` if you hit API rate limits.

### Step 3: Verify search works

```bash
inbrain search "test query" --limit 5
```
...
echo '{"version":"0.9.1","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","status":"complete"}' >> ~/.inbrain/migrations/completed.jsonl
```
