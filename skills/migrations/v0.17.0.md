...
version: 0.17.0
feature_pitch:
  headline: "One brain maintenance cycle, two CLIs. `inbrain dream` delivers the README promise."
  description: |
...
# v0.17.0 Migration: `inbrain dream` + unified maintenance cycle
...
inbrain dream --dry-run --json    # preview, agent-readable
inbrain dream --phase lint        # single-phase (fast, targeted)
inbrain dream --pull              # git pull before syncing
0 2 * * * inbrain dream --json    # nightly cron
```

See `inbrain dream --help` for the full flag reference.

### Autopilot now runs lint + orphan sweep

`inbrain autopilot --install` users: on upgrade, your daemon's cycle
...
- **orphan sweep** — reports (read-only) pages with no inbound
  wikilinks. Visible in `inbrain jobs list` output for each
  `autopilot-cycle` job.
...
`inbrain_cycle_locks` (new table, migration v16) replaces
...
inbrain jobs list --status complete | head -5
# Each `autopilot-cycle` entry now has 6 phases in its report,
# not 4. Check a recent one with `inbrain jobs get <id>`.

# 4. Schema migration landed:
inbrain doctor  # should show no pending migrations
```

Expected `inbrain dream --dry-run` output on a healthy brain:
...
0 2 * * * inbrain dream --phase orphans
```
...
go wrong, `inbrain dream --dry-run` always tells you what WOULD
...
**`inbrain dream --dry-run` reports changes when you expected none:**
Check `inbrain doctor` for drift: lint issues, stale embeddings,
...
Open a GitHub issue with the output of `inbrain jobs get <id>` for a
...
- `inbrain autopilot --install` machinery (launchd / systemd /
  crontab generators). Existing installs keep working.
- `~/.inbrain/autopilot.lock` daemon-singleton lockfile. Separate
  concern from the new per-cycle lock.
- `inbrain jobs` interface. `inbrain jobs get <id>` now shows a
...
required — all changes apply automatically on `inbrain upgrade`.*
