...
`inbrain upgrade` runs:
1. `bun install` (refreshes deps; no new external deps in v0.34)
2. `inbrain post-upgrade` → `inbrain apply-migrations` → migration v56
...
```bash
inbrain edges-backfill --all-sources
```
...
```bash
inbrain doctor --json | jq '.checks.code_intel_coverage'
inbrain call code_blast --symbol performSync
inbrain call code_flow --entry_point runCycle
```
...
- **`inbrain wiki`** — depends on clusters.
- **`inbrain blast` / `inbrain flow` CLI thin-shims.** Use `inbrain call
