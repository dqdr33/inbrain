...
**TL;DR:** this release teaches the knowledge graph to read your YAML frontmatter. Every `company:`, `investors:`, `attendees:`, `key_people:`, `partner:`, `lead:`, and `related:` field you already wrote now surfaces as a typed graph edge. `inbrain graph <hub-entity> --depth 2` goes from returning ~7 nodes to 50+ on a real brain without you changing a word of content.

For most users: run `inbrain upgrade` and you're done. The orchestrator handles schema + backfill in 2-5 minutes on a 46K-page brain. You immediately see richer results from `inbrain graph` queries.
...
The migration is resumable. If it dies mid-backfill (OOM, network blip), re-run `inbrain upgrade` and it picks up where it left off via `ON CONFLICT DO NOTHING` on the new unique constraint.
...
inbrain graph <hub-entity-slug> --depth 2

# Filter to specific edge types (new in v0.13).
inbrain graph <hub-entity-slug> --depth 2 --type yc_partner,invested_in

# Count edges by provenance.
inbrain call get_stats --json
```

## Troubleshooting

**Migration failed mid-backfill.** Re-run `inbrain upgrade`. Resumable via ON CONFLICT DO NOTHING + origin_page_id scoping.
...
- Create the missing pages (then run `inbrain extract links --source db --include-frontmatter` to backfill).
