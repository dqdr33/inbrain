...
date: 2026-05-05
feature_pitch: |
  inbrain remembers what you SAW, not just what you typed. Drop a screenshot,
...
`inbrain upgrade` runs `inbrain apply-migrations --yes` automatically, which
...
- **Off by default**: nothing changes at runtime until you flip
  `embedding_multimodal: true` (DB plane via `inbrain config set` or env via
  `INBRAIN_EMBEDDING_MULTIMODAL=true`).

## Verification

```bash
inbrain apply-migrations --yes
inbrain doctor                # schema_version should be 36+
```

If `inbrain doctor` reports a `schema_version` warning, run apply-migrations
manually and check `~/.inbrain/upgrade-errors.jsonl` for details.
...
inbrain config set embedding_model voyage:voyage-3-large
inbrain config set embedding_dimensions 1024

# Required: flip the multimodal gate.
inbrain config set embedding_multimodal true

# Optional: enable OCR via gpt-4o-mini (~$0.0003 per image).
inbrain config set embedding_image_ocr true
```

Drop image files (PNG, JPG, JPEG, GIF, WEBP, HEIC, AVIF) into your brain
repo and run `inbrain sync`. The image lands as a `type: image` page with:
...
Then re-run `inbrain apply-migrations --yes`. If your provider doesn't ship
...
- `inbrain query --image <path>` flag for image-similarity search.
