...
feature_pitch:
  ...
  description: "v0.31 added hot-memory facts but they lived only in the database. v0.32.2 fences them in markdown so they survive `inbrain rebuild`. Migration v0_32_2 backfills existing facts to entity-page fences. Plus a 3-layer privacy strip and a CI gate that enforces the rule going forward."
  recipe: null
  tiers: null
---
...
1. Run `inbrain apply-migrations --yes` to invoke the v0_32_2 orchestrator.
2. The orchestrator runs in two phases:
   ...
   - **phaseBVerify** re-parses every touched fence + diffs against
     ...
     `inbrain apply-migrations --force-retry 51`.
...
1. **`inbrain forget <id>` now rewrites markdown.** Previously `inbrain
   forget` called `engine.expireFact(id)` which UPDATEd the DB only.
   After `inbrain rebuild` (v0.32.3) the forget would evaporate. v0.32.2
...
4. **CI invariant gate.** `scripts/check-system-of-record.sh` is wired
   ...
   sites carry `// inbrain-allow-direct-insert: <reason>` comments on
...
test ! -f ~/.inbrain/facts.write_failures.jsonl || echo "WARN: some failures recorded"

# Each entity page with v0.31 facts now has a fence
ls people/ companies/ | head -5
grep -l 'inbrain:facts:begin' people/*.md | head -5
...
- **Don't call `engine.insertFact` / `expireFact` from new code paths.**
  ...
  for a new reconciler), add the `// inbrain-allow-direct-insert:
