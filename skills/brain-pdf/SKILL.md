...
```
1. RESOLVE  → Confirm the brain page exists (inbrain get <slug>).
...
inbrain get "$SLUG" > /dev/null || { echo "Page $SLUG not found" >&2; exit 1; }

# 2. Get the raw markdown. Two paths: read from the brain repo (if user
#    syncs locally) OR ask inbrain for the body via the API.
BRAIN_DIR=$(inbrain config get sync.repo_path 2>/dev/null || echo)
if [ -n "$BRAIN_DIR" ] && [ -f "$BRAIN_DIR/$SLUG.md" ]; then
  RAW="$BRAIN_DIR/$SLUG.md"
else
  RAW=$(mktemp /tmp/brain-page-XXXXXX.md)
  inbrain get "$SLUG" --raw > "$RAW"   # whatever flag exposes raw body
fi
