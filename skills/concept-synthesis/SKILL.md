...
The skill is markdown agent instructions. The agent uses inbrain's
existing operations + LLM passes:

```bash
# 1. List all concept pages
inbrain query "type:concept" --limit 10000 --json
...
- Run `inbrain query "type:concept"` and spot-check the count reduction.
