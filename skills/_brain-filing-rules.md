...
**Size routing (automatic via `inbrain files upload-raw`):**
...
```bash
inbrain files signed-url <storage-path>    # Generate 1-hour signed URL
inbrain files restore <dir>                # Download back to local
```
...
The `synthesize` and `patterns` phases of `inbrain dream` write to a
...
When writing a `<!--- inbrain:takes:begin -->` fence, the **holder** column says
