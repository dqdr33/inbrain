...
| "what search mode", "is my cache hot", "tune my retrieval", "compare search modes", "clear search overrides" | `inbrain search modes/stats/tune` directly. See `skills/conventions/search-modes.md` |
| "eval results", "search benchmark", "haters-immune methodology", "regression check on retrieval" | `inbrain eval run-all` / `inbrain eval compare`. See `docs/eval/SEARCH_MODE_METHODOLOGY.md` |
...
| "Is inbrain healthy?", morning health check, skillpack-check | `skills/skillpack-check/SKILL.md` |
| "harvest this skill into inbrain", "publish this skill to inbrain", "lift this skill upstream", "share this skill with other inbrain clients", "promote my skill to inbrain" | `skills/skillpack-harvest/SKILL.md` |
...
| "Spawn agent", "background task", "parallel tasks", "steer agent", "pause/resume agent", "inbrain jobs submit", "submit a inbrain job", "submit a shell job", "shell job" | `skills/minion-orchestrator/SKILL.md` |
...
| Trigger | Skill |
|---------|-------|
| "Set up Inbrain", first boot | `skills/setup/SKILL.md` |
...
| "Brain health", "what features am I missing", "brain score" | Run `inbrain features --json` |
| "Set up autopilot", "run brain maintenance", "keep brain updated" | Run `inbrain autopilot --install --repo ~/brain` |
| "Upgrade inbrain", "update inbrain", "inbrain update available", `UPGRADE_AVAILABLE`, "is inbrain up to date" | `skills/inbrain-upgrade/SKILL.md` |
...
| "add a page type", "add a type to my schema", "schema author", "schema mutate", "schema pack add", "my brain has untyped pages", "propose new types from my corpus", "backfill page types", "evolve my schema", "researcher type", "make X an expert type" (dispatcher for: inbrain schema active/list/show/validate/graph/lint/stats/explain/use/downgrade/reload/init/fork/edit/diff/add-type/remove-type/update-type/add-alias/remove-alias/add-prefix/remove-prefix/add-link-type/remove-link-type/set-extractable/set-expert-routing/detect/suggest/review-candidates/review-orphans/sync) | `skills/schema-author/SKILL.md` |
| "unify my types", "migrate to inbrain-base-v2", "94 types to 14", "apply canonical taxonomy", "clean up my page types", "pack upgrade", "shrink type proliferation", "consolidate page types", "retype pages to canonical" (dispatcher for: inbrain onboard --check, inbrain onboard --check --explain, inbrain jobs submit unify-types, inbrain pages restore) | `skills/schema-unify/SKILL.md` |
