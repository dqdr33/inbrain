...
```bash
inbrain doctor --remediation-plan --json              # preview what would run
...
1. **Run health check.** Check inbrain health to get the dashboard.
...
```bash
inbrain extract timeline --dir ~/brain
```

### Dream cycle (v0.23): synthesize + patterns

`inbrain dream` runs the full 8-phase maintenance cycle:
...
```bash
inbrain config set dream.synthesize.session_corpus_dir /path/to/transcripts
inbrain config set dream.synthesize.enabled true
inbrain dream --phase synthesize --dry-run --json   # preview
inbrain dream                                       # full 8-phase cycle
```

**Invocation patterns:**
```bash
inbrain dream                                          # full cycle
inbrain dream --phase synthesize                       # just synthesize
inbrain dream --phase patterns                         # just patterns
inbrain dream --input ~/transcripts/2026-04-25.txt     # ad-hoc one transcript
inbrain dream --from 2026-04-01 --to 2026-04-25        # backfill range
inbrain dream --json                                   # CycleReport JSON
```

**Auto-commit deferred to v1.1:** v1 writes files to `brain_dir` but does NOT
`git add` / `commit` / `push`. Either commit yourself or let `inbrain autopilot`
handle it.
Parses `- **YYYY-MM-DD** | Source — Summary` and `### YYYY-MM-DD — Title` formats.
Note: extracted entries improve structured queries (`inbrain timeline`), not vector search.
...
```bash
inbrain autopilot --install --repo ~/brain
```
...
partially set up: schema is applied, but `~/.inbrain/preferences.json`
doesn't exist, autopilot runs inline, host manifests still reference
`agentTurn`. Repair:

```bash
# Check migration status
inbrain apply-migrations --list

# Apply pending migrations (idempotent; safe on healthy installs)
inbrain apply-migrations --yes

# If host-specific handlers are flagged in ~/.inbrain/migrations/pending-host-work.jsonl:
...
- Use inbrain search to find pages in `sources/` that reference specific
...
- Standardize to the most common variant using inbrain tag operations
...
- `inbrain extract links --source db` — backfill structured links by walking pages
...
- `inbrain extract timeline --source db` — backfill structured timeline entries.
  Parses `- **YYYY-MM-DD** | summary` lines from page content. Idempotent (DB
  UNIQUE constraint).
- `inbrain extract all --source db` — both in one run.
- `inbrain graph-query <slug> --depth 2` — verify connectivity (use any well-known
  entity slug as a probe).
- `inbrain stats` — verify `link_count > 0` and `timeline_entry_count > 0` after extraction.
- `inbrain health` — review `link_coverage` and `timeline_coverage` percentages
  on entity pages (person/company). Below 50% means more extraction is needed.

Available link types (use with `inbrain graph-query --type`):
`attended`, `works_at`, `invested_in`, `founded`, `advises`, `mentions`, `source`.

Going forward, every `inbrain put` call auto-creates and reconciles links via the
auto-link post-hook (default on; disable: `inbrain config set auto_link false`).
...
- For large embedding refreshes (>1000 chunks), use nohup:
  `nohup inbrain embed refresh > /tmp/inbrain-embed.log 2>&1 &`
- Then check progress: `tail -1 /tmp/inbrain-embed.log`

### Security (RLS verification)
Run `inbrain doctor --json` and check the RLS status.
All tables should show RLS enabled. If not, run `inbrain init` again.

### Schema health
Check that the schema version is up to date. `inbrain doctor --json` reports
the current version vs expected. If behind, `inbrain init` runs migrations
automatically.

### File storage health
Check the integrity of stored files and redirect pointers:
- Run `inbrain files verify` to check all DB records have valid data
- Run `inbrain files status` to see migration state (local, mirrored, redirected)
...
Compare results from `inbrain search` (keyword) vs `inbrain query` (hybrid).
Quality matters more than speed (2.5s right > 200ms wrong).

When to run benchmarks:
- After major brain imports or re-imports
- After inbrain version upgrades
- After embedding regeneration
- Monthly to track quality drift

## Heartbeat Integration

For production agents running on a schedule, integrate inbrain health checks into
your operational heartbeat.

### On every heartbeat (hourly or per-session)

Run `inbrain doctor --json` and check for degradation. Report any failing checks
to the user. Key signals: connection health, schema version, RLS status, embedding
staleness.

### Weekly maintenance

Run `inbrain embed --stale` to refresh embeddings for pages that have changed since
their last embedding. For large brains (>5000 pages), run this with nohup:
```bash
nohup inbrain embed --stale > /tmp/inbrain-embed.log 2>&1 &
```

### Daily verification

Verify sync is running: check `inbrain stats` and confirm `last_sync` is within
...
- Check inbrain health before and after to show improvement
...
- Check inbrain health (get_health)
- List pages in inbrain with filters (list_pages)
- Read a page from inbrain (get_page)
- Check backlinks in inbrain (get_backlinks)
- Link entities in inbrain (add_link)
- Remove links in inbrain (remove_link)
- Tag a page in inbrain (add_tag)
- Remove a tag in inbrain (remove_tag)
- View timeline in inbrain (get_timeline)
