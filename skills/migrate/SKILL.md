...
Universal migration from any wiki, note tool, or brain system into Inbrain.

## Contract

- Source data is never modified or deleted; migration is additive only.
- Every migrated page is verified round-trip: written to inbrain, read back, spot-checked.
- Cross-references from the source system (wikilinks, block refs, tags) are converted to inbrain equivalents.
...
|--------|--------|----------|
| Obsidian | Markdown + `[[wikilinks]]` | Direct import, convert wikilinks to inbrain links |
...
| Plain markdown | Any .md directory | Import directory into inbrain directly |
...
2. **Plan the mapping.** How do source fields map to inbrain fields (type, title, tags, compiled_truth, timeline)?
3. **Test with a sample.** Import 5-10 files, verify by reading them back from inbrain and exporting.
4. **Bulk import.** Import the full directory into inbrain.
5. **Verify.** Check inbrain health and statistics, spot-check pages.
6. **Build links.** Extract cross-references from content and create typed links in inbrain.

## Obsidian Migration

1. Import the vault directory into inbrain (Obsidian vaults are markdown directories)
...
Obsidian-specific:
- Tags (`#tag`) become inbrain tags
- Frontmatter properties map to inbrain frontmatter
...
5. Import the cleaned directory into inbrain
...
4. Store each page in inbrain

## Verification

After any migration:
1. Check inbrain statistics to verify page count matches source
2. Check inbrain health for orphans and missing embeddings
3. Export pages from inbrain for round-trip verification
4. Spot-check 5-10 pages by reading them from inbrain
5. Test search: search inbrain for "someone you know is in the data"
...
- **Ignoring cross-references.** Wikilinks, block refs, and tags from the source system must be converted to inbrain equivalents. Dropping them loses the knowledge graph.
...
```
MIGRATION REPORT -- [source] -> Inbrain
=======================================
...
- Store/update pages in inbrain (put_page)
- Read pages from inbrain (get_page)
- Link entities in inbrain (add_link)
- Tag pages in inbrain (add_tag)
- Get inbrain statistics (get_stats)
- Check inbrain health (get_health)
- Search inbrain (query)
