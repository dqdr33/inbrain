# Running real-world eval benchmarks against your inbrain changes

Audience: inbrain maintainers and contributors. If you're touching retrieval
(search, ranking, embeddings, intent classification, query expansion, source
boost, hybrid fusion), this is the doc.

For the **NDJSON wire format** consumed by inbrain-evals, see
...
stitched them into a gate. `inbrain bench publish` + `inbrain eval gate`
...
inbrain eval export --limit 200 --tool query > /tmp/captured.ndjson

# 2. Publish a baseline
mkdir -p ~/.inbrain/baselines
inbrain bench publish --from /tmp/captured.ndjson --to ~/.inbrain/baselines/personal.baseline.ndjson --label "personal-$(date +%Y%m%d)"

# 3. Gate against it
inbrain eval gate --baseline ~/.inbrain/baselines/personal.baseline.ndjson
```

### Privacy posture (D9)

**Public baselines in `inbrain-evals` are hermetic-synthetic ONLY.** Real
user captures stay local in `~/.inbrain/baselines/`. The boundary is
enforced at the file source, not by post-hoc scrubbing. If you publish a
baseline to `inbrain-evals`, generate it from a fixture-seeded test brain
...
`inbrain eval gate --qrels` uses bare `hybridSearch` (not the production
  ...
  gate:
    ...
    steps:
      ...
      - run: |
          ...
          inbrain eval gate \
            --baseline inbrain-evals/baselines/v0.41-launch.baseline.ndjson \
            --qrels inbrain-evals/qrels/v0.41-launch.qrels.json \
...
```bash
# In ~/.zshrc or ~/.bashrc:
export INBRAIN_CONTRIBUTOR_MODE=1
```

Verify:

```bash
inbrain query "anything" >/dev/null
psql $DATABASE_URL -c 'SELECT count(*) FROM eval_candidates'   # should be > 0
```

To override (force on/off regardless of env var), edit `~/.inbrain/config.json`:
...
inbrain eval replay --against baseline.ndjson
```
...
`inbrain eval replay` reads your NDJSON snapshot and, for each row:
...
`inbrain eval --qrels <path>` (the legacy IR-eval path, still supported). The
...
question-answer pipeline (not just retrieval) — `inbrain eval longmemeval
<dataset.jsonl>` (v0.28.8) runs the LongMemEval benchmark against inbrain's
...
shape LongMemEval's `evaluate_qa.py` consumes. Your `~/.inbrain` brain is
...
the vector half of `hybridSearch`. Cost is identical to a normal `inbrain
...
inbrain eval replay --against baseline.ndjson --limit 50

# Pre-merge — full snapshot
inbrain eval replay --against baseline.ndjson --top-regressions 20
```

## CI integration

```bash
inbrain eval replay --against baseline.ndjson --json > replay.json
...
Then run `inbrain eval replay --against handcrafted.ndjson` to confirm the
...
pipeline (`inbrain eval --qrels` with the sibling
[inbrain-evals](https://github.com/inbrain-ai/inbrain-evals) corpus).
...
```bash
unset INBRAIN_CONTRIBUTOR_MODE             # easy: just unset the env var
```

Or force off regardless of the env var via `~/.inbrain/config.json`:

```json
{"eval": {"capture": false}}
```

Existing `eval_candidates` rows stay until you `inbrain eval prune
...
`inbrain eval longmemeval` runs the public [LongMemEval](https://huggingface.co/datasets/xiaowu0162/longmemeval)
benchmark directly against inbrain's hybrid retrieval. Different evaluation
...
inbrain eval longmemeval ./longmemeval_oracle.json --limit 50 --retrieval-only \
  > /tmp/hypothesis.jsonl

# Full pipeline (Anthropic key required for answer-gen):
inbrain eval longmemeval ./longmemeval_oracle.json --limit 50 \
  > /tmp/hypothesis.jsonl
...
- One in-memory PGLite per benchmark run via `createBenchmarkBrain` +
  `withBenchmarkBrain`. Your `~/.inbrain` is never opened.
- Between questions: `TRUNCATE` over runtime-enumerated `pg_tables`, NOT a
  ...
  `inbrain_cycle_locks`, `subagent_rate_leases`) are preserved across resets.
...
`inbrain eval suspected-contradictions` is a complementary measurement
...
inbrain eval suspected-contradictions \
  --queries-file ~/.inbrain/queries.jsonl \
  --top-k 5 \
  --budget-usd 5 \
  --output ~/.inbrain/probe-runs/$(date +%Y-%m-%d).json
```
...
```bash
inbrain eval suspected-contradictions trend --days 30
```

The ASCII bar chart shows total flagged per day. Headline % surfaces in
`inbrain doctor`'s `contradictions` check with paste-ready resolution
...
### `inbrain eval longmemeval --by-type` — per-question-type R@k breakdown
...
1. Every per-question JSONL row now includes a `question: string` field so the
   `inbrain eval cross-modal --batch` consumer (below) can read it without
...
inbrain eval longmemeval ~/datasets/longmemeval_s.jsonl \
  --by-type --output /tmp/run.jsonl
tail -1 /tmp/run.jsonl | jq .   # summary line

# Strict gate in a CI script.
inbrain eval longmemeval test/fixtures/longmemeval-mini.jsonl \
...
```bash
INBRAIN_REPLAY_GATE_TOP1_FLOOR=0.85 \
INBRAIN_REPLAY_GATE_RECALL_FLOOR=0.90 \
  bun test test/eval-replay-gate.test.ts
```

Use to tighten or loosen the gate as the qrels fixture matures.

### `inbrain eval cross-modal --batch` — batch quality scoring
...
inbrain eval cross-modal --batch /tmp/run.jsonl \
...
```bash
inbrain config set autopilot.nightly_quality_probe.enabled true
inbrain config set autopilot.nightly_quality_probe.max_usd 5.00   # optional override
```
...
Observability:
- `~/.inbrain/audit/quality-probe-YYYY-Www.jsonl` — one event per run with
  ...
  audit).
- `inbrain doctor` surfaces `nightly_quality_probe_health`:
