...
Inbrain exposes **vendor-neutral guardrail seams** at the boundaries where
...
- **Observe-only.** `runGuardrails()` returns `void`. Callers never branch on a
  ...
  block, rewrite, drop, retry, or reorder Inbrain behavior. Enforcement, if ever
...
- **No verdict persistence.** Inbrain writes no guardrail rows. Providers own
...
through `importFromContent` / `importCodeFile`: `inbrain import`, sync, capture,
...
```ts
import { registerGuardrailProvider, type GuardrailInput } from 'inbrain/core/guardrails';
...
registerGuardrailProvider({
  ...
  async classify(input: GuardrailInput) {
    ...
    // Inbrain — return a typed verdict only if your own audit code consumes it.
...
Inbrain deliberately keeps the seam minimal. The provider owns:

- **Timeout discipline.** Inbrain does not impose a timeout in `runGuardrails`
...
Because the verdict is ignored by Inbrain, "shadow mode" requires *no* special
Inbrain flag — it is the only mode this interface supports. Enforcement would be
