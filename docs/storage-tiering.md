...
Inbrain supports storage tiering to separate version-controlled content from bulk machine-generated data. This prevents git repositories from becoming bloated with large amounts of automatically generated content while still preserving it in the database.

> Note on naming: prior to v0.22.11 the keys were `git_tracked` / `supabase_only`. The canonical names are now `db_tracked` / `db_only` (engine-agnostic — works on both PGLite and Postgres). The deprecated keys still load with a once-per-process warning. Run `inbrain doctor --fix` for an automated rename when that path lands.

## Configuration

Add a `storage` section to your `inbrain.yml` file in the brain repository root:
...
storage:
  ...
  # `inbrain sync` auto-manages .gitignore for these paths. `inbrain export
...
- A directory cannot appear in both tiers — that's a tier-overlap error and `loadStorageConfig` throws `StorageConfigError`. Edit `inbrain.yml` to remove the overlap and try again.

## Behavior Changes

### 1. `inbrain sync` — automatic .gitignore management

When storage configuration is present, `inbrain sync` automatically manages `.gitignore` entries on every successful sync:
...
- Skipped entirely when `INBRAIN_NO_GITIGNORE=1` is set (escape hatch for shared-repo setups where a maintainer wants inbrain to leave .gitignore alone).
...
### 2. `inbrain export --restore-only` — repopulate missing db_only files

```bash
# Restore only missing db_only files from the database.
inbrain export --restore-only --repo /path/to/brain

# Filter by page type.
inbrain export --restore-only --type media --repo /path/to/brain

# Filter by slug prefix.
inbrain export --restore-only --slug-prefix media/x/ --repo /path/to/brain

# Combine filters.
inbrain export --restore-only --type media --slug-prefix media/x/ --repo /path/to/brain
```
...
### 3. `inbrain storage status` — storage-tier health dashboard

```bash
# Human-readable status.
inbrain storage status --repo /path/to/brain

# JSON output for scripts and orchestrators.
inbrain storage status --repo /path/to/brain --json
```
...
Use: inbrain export --restore-only --repo "/data/brain"
...
- `inbrain export --restore-only` quickly restores bulk files when needed.
...
1. **Assess current repository**: use `inbrain storage status` to understand current distribution.
2. **Plan directory structure**: identify which directories should be db_tracked vs db_only.
3. **Create `inbrain.yml`**: add storage configuration to the repository root.
4. **Test with dry-run**: `inbrain sync --dry-run` to verify behavior; `.gitignore` is NOT touched on dry-run.
5. **Run a real sync**: `inbrain sync` updates `.gitignore` automatically on success.
6. **Verify restore**: test `inbrain export --restore-only --repo .` against a small db_only directory.
...
- **Document decisions**: comment your `inbrain.yml` to explain tier choices.

## PGLite engine note

On the PGLite engine (inbrain's local-only embedded Postgres), the "DB" your db_only pages live in IS the local file inbrain uses for everything else. The `.gitignore` housekeeping still helps (keeps bulk content out of git history), but the offload-to-DB promise is technically vacuous. A once-per-process soft-warn explains when the engine is detected. To get full tiering, migrate to Postgres with `inbrain migrate --to supabase`.

## Compatibility

- **Backward compatible**: systems without `inbrain.yml` work unchanged.
