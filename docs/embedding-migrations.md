...
Inbrain stores embeddings in a fixed-dimension `vector(N)` column on
...
`inbrain init`, `inbrain doctor`, and `inbrain embed --stale` all detect
...
is needed. As of v0.41.31.0, inbrain stamps an embedding-provenance
...
signatures differ from the current one, and `inbrain embed --stale`
re-embeds exactly those pages:

```bash
# After switching to the new same-dim model in your config:
inbrain embed --stale          # re-embeds signature-drifted pages
inbrain embed --stale --dry-run # preview the count without re-embedding
```

Under federated_v2, the same drift is picked up by the per-source
`embed-backfill` jobs that `inbrain sync --all` enqueues (capped
...
```bash
inbrain reinit-pglite \
  --embedding-model zeroentropyai:zembed-1 \
  --embedding-dimensions 1280
```

This backs up the existing brain to `<path>.bak`, runs `inbrain init`
with the new flags (preserving every other field in
`~/.inbrain/config.json`), and re-syncs the brain repo. Add `--no-sync`
...
mv ~/.inbrain/brain.pglite ~/.inbrain/brain.pglite.bak

# 2. Re-init with the new model + dimensions. `inbrain init` writes
#    the schema sized to the new dim, and (as of v0.37) preserves
#    every other field in ~/.inbrain/config.json (chat model,
#    expansion model, API keys).
inbrain init --pglite \
  --embedding-model zeroentropyai:zembed-1 \
  --embedding-dimensions 1280

# 3. Re-import your brain repo. `inbrain sync` reads the brain repo
...
`INBRAIN_HOME` users: substitute the active database path (or use
`inbrain config get database_path` to find it).
...
--    indexless and rely on exact scans (inbrain searchVector handles this
...
## A note on `inbrain config set`

Pre-v0.37 docs recommended `inbrain config set embedding_model X` to
...
(`~/.inbrain/config.json`). The pre-v0.37 recipe shipped the lie because
the contract wasn't surfaced.

As of v0.37, `inbrain config set embedding_model` and `inbrain config set
embedding_dimensions` REFUSE and print the wipe-and-reinit recipe.

To change schema-sizing fields, use `inbrain init` (PGLite) or the SQL
recipe (Postgres). Both update the file plane AND the schema together.

## Verify

After the recipe lands, `inbrain doctor --fast` should report green and
`inbrain doctor` should pass the `embedding_width_consistency` check:
