...
The canonical one-page reference for what a third-party inbrain skillpack
...
`inbrain skillpack init <name>` scaffolds this exact tree, pre-filled
with stubs that score 10/10 on `inbrain skillpack doctor . --quick`
immediately. Replace the stubs with real content, run the doctor
between edits, and `inbrain skillpack pack` produces a deterministic
`<name>-<version>.tgz` ready to publish to the registry.

## How the agent uses a scaffolded pack

After `inbrain skillpack scaffold <source>` lands the files:
...
3. inbrain DISPLAYS `runbooks/bootstrap.md` once after the scaffold
...
```bash
# Publisher side
inbrain skillpack init my-pack         # scaffold the tree
inbrain skillpack doctor my-pack       # see the score + fix hints
inbrain skillpack doctor my-pack --fix --yes  # auto-scaffold missing pieces
inbrain skillpack pack my-pack         # deterministic tarball + SHA-256

# Consumer side
inbrain skillpack search <query>       # browse the registry
inbrain skillpack info <name>          # show full pack metadata
inbrain skillpack scaffold <source>    # owner/repo, https, ./dir, ./*.tgz
inbrain skillpack registry --url X     # point at a custom registry
```
