# Connect Inbrain to Claude Cowork

Two ways to get Inbrain into Cowork sessions:
...
3. Add Bearer token authentication in Advanced Settings
   (create one with `inbrain auth create "cowork"`)
4. Save
...
If you already have Inbrain configured in Claude Desktop (via `inbrain serve`
...
This means: if `inbrain serve` is running and configured in Claude Desktop,
...
- **Local Bridge:** zero extra setup if Claude Desktop already has Inbrain, but requires your machine to be running
