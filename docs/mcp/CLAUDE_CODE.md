# Connect Inbrain to Claude Code
...
That's it. Claude Code spawns `inbrain serve` as a stdio subprocess. No server, no
...
If Inbrain is running somewhere as an HTTP server (`inbrain serve --http`, see the
[ngrok-tunnel recipe](../../recipes/ngrok-tunnel.md)) and you have a bearer token,
let `inbrain connect` generate the wire-up for you.

On the host (or anywhere `inbrain` is installed), mint a token and print the block:

```bash
inbrain auth create "claude-code"
inbrain connect https://YOUR-DOMAIN.ngrok.app/mcp --token inbrain_xxx
```

`inbrain connect` prints a short, copy-paste block. Paste it into Claude Code — it
...
```bash
inbrain connect https://YOUR-DOMAIN.ngrok.app --token inbrain_xxx --install
```
...
```bash
inbrain connect https://YOUR-DOMAIN.ngrok.app/mcp --token inbrain_xxx --json
```

## Option 3: Remote, manual `claude mcp add`

Equivalent to what `inbrain connect` generates, if you'd rather run it yourself:

```bash
claude mcp add inbrain -t http \
...
> A `inbrain auth create` token is a long-lived, full-access secret. Keep it
...
You should see results from your Inbrain knowledge base.

> **`list_skills` returns nothing?** Skill discovery is gated by `mcp.publish_skills`
> on the host. New brains from `inbrain init` default it ON; brains upgraded from an
> older release stay OFF until you opt in. Enable it on the host with
> `inbrain config set mcp.publish_skills true`. The core tools (search, query,
