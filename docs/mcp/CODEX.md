...
## Fastest path: `inbrain connect`

Run anywhere `inbrain` is installed (mint a token on the brain host first):

```bash
inbrain auth create "codex"
inbrain connect https://YOUR-DOMAIN.ngrok.app/mcp --token inbrain_xxx --agent codex
```

This prints a copy-paste block. Or wire it up directly and smoke-test the token:

```bash
inbrain connect https://YOUR-DOMAIN.ngrok.app/mcp --token inbrain_xxx --agent codex --install
```
...
var at runtime, keep `INBRAIN_REMOTE_TOKEN` exported in your shell profile.

## Manual setup

```bash
export INBRAIN_REMOTE_TOKEN=inbrain_xxx
codex mcp add inbrain --url https://YOUR-DOMAIN.ngrok.app/mcp \
  --bearer-token-env-var INBRAIN_REMOTE_TOKEN
```

Codex stores the env-var *name* (`INBRAIN_REMOTE_TOKEN`), not the token itself, and
...
> ON for `inbrain init` brains, OFF for brains upgraded from older releases). Enable
> it on the host: `inbrain config set mcp.publish_skills true`. The core tools
...
```bash
codex mcp remove inbrain
```

## Notes

- The token is a long-lived, full-access secret. Keep `INBRAIN_REMOTE_TOKEN` out of
