# Deploy Inbrain Remote MCP Server

> **v0.26.0+:** `inbrain serve --http` ships full OAuth 2.1 (client credentials,
...
Access your brain from any device, any AI client. Inbrain ships two transports:
`inbrain serve` (stdio) for local agents, and `inbrain serve --http` (v0.26.0+)
...
```bash
inbrain serve --http --port 3131
ngrok http 3131 --url your-brain.ngrok.app
inbrain serve --http --port 3131 --public-url https://your-brain.ngrok.app
```
...
Your AI client (Claude Desktop, Perplexity, etc.)
  ...
  → inbrain serve --http  (built-in transport with bearer auth)
  → Postgres (pooler connection or self-hosted)
```

This requires:
1. A Postgres-backed brain (the `access_tokens` table only exists on Postgres;
   running `inbrain serve --http` against a PGLite install fails fast at startup)
2. A machine running `inbrain serve --http`
3. A public tunnel (ngrok, Tailscale, or cloud host)
4. A bearer token created via `inbrain auth create <name>`
...
> want raw payloads back can pass `inbrain serve --http --log-full-params` (loud
...
```bash
inbrain auth register-client perplexity \
...
```bash
inbrain auth register-client dept-x-agent \
  --grant-types client_credentials \
  --scopes "read write" \
  --source dept-x \
...
`source_id='default'` on `inbrain upgrade`.
...
**v0.34 — bind explicitly.** `inbrain serve --http` defaults to `127.0.0.1`.
To accept connections from the ngrok tunnel (or any non-loopback source),
restart with `--bind`:

```bash
inbrain serve --http --port 3131 --bind 0.0.0.0 --public-url https://your-brain.ngrok.app
```
...
- **ChatGPT:** [setup guide](CHATGPT.md) (OAuth 2.1 + PKCE, requires `inbrain serve --http`)
...
All 30 Inbrain operations are available remotely, including `sync_brain` and
...
directory where `inbrain serve` was launched. Symlinks, `..` traversal, and absolute
...
CLI callers (`inbrain file upload ...`) keep unrestricted filesystem access since
...
**"invalid_token" error**
Run `inbrain auth list` to see active tokens.
...
**Note:** `inbrain serve --http` shipped in v0.26.0 with OAuth 2.1 + admin
