...
Inbrain's MCP server runs via `inbrain serve` (stdio transport). To make it
accessible from other devices and AI clients, run `inbrain serve --http`
...
**Note:** `inbrain serve --http` is the built-in HTTP transport (v0.22.7+). Bearer auth
