# Connect Inbrain to Perplexity Computer

Perplexity Computer connects as a **remote** MCP client, so Inbrain must be served
over HTTP and reachable at a public HTTPS URL. Perplexity does not run
`inbrain serve` (stdio) the way Claude Code does — it needs a reachable endpoint:

```
Perplexity Computer
  → ngrok tunnel (https://YOUR-DOMAIN.ngrok.app/mcp)
  → inbrain serve --http   (built-in OAuth 2.1 transport)
  → Postgres / PGLite
```

## 1. Serve Inbrain over HTTP (host side)
...
```bash
inbrain connect https://YOUR-DOMAIN.ngrok.app/mcp --agent perplexity --oauth --register
```

Or register separately and pass the creds (works anywhere, no DB needed):

```bash
inbrain auth register-client perplexity --grant-types client_credentials --scopes "read write"
inbrain connect https://YOUR-DOMAIN.ngrok.app/mcp --agent perplexity --oauth \
...
```bash
inbrain auth create "perplexity"
inbrain connect https://YOUR-DOMAIN.ngrok.app/mcp --token inbrain_xxx --agent perplexity
```
...
```
Use my Inbrain to search for [topic]
```
...
- The Mac app can also use a local MCP server (`inbrain serve` stdio) if you'd
