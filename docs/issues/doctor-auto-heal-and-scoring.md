...
The `inbrain doctor` health score system has several false-positive patterns and missing auto-heal capabilities. After the crash classification fix (shipped in this PR), these are the remaining improvements ranked by impact.
...
Store in `.inbrain/doctor-baselines.json` so it works without config too:
...
- Stored in `.inbrain/doctor-baselines.json`
...
- Write each run to `.inbrain/doctor-history.jsonl`:
  ```json
