...
From Voyage docs: voyage-multimodal-3 encodes text, images, and interleaved text+image into the same 1024-dimensional vector space. A text query embedded through this model can find relevant images, and vice versa. inbrain already uses it for the image column but never for query embedding.
...
- Voyage multimodal-3 is already configured as `embedding_multimodal_model` in inbrain config
