...
on user_shares_media(url_or_file):
    ...
    if media.type == "youtube":
        ...
        for person in transcript.mentioned_people:
            inbrain add_link <slug> <person_slug>
            inbrain add_link <person_slug> <slug>
    ...
    # PATTERN 2: Social Media Bundles
        ...
        for entity in bundle.mentioned_entities:
            inbrain add_link <slug> <entity_slug>
            inbrain add_link <entity_slug> <slug>
...
1. Ingest a YouTube video. Run `inbrain get media/youtube/{slug}`. Confirm the page has: the agent's analysis (not just a summary), key quotes with speaker attribution, and the full diarized transcript.
2. Run `inbrain get_links media/youtube/{slug}`. Confirm back-links exist to brain pages for every person and company mentioned in the video.
3. Pick a person mentioned in the video. Run `inbrain get <person_slug>`. Confirm their timeline has a new entry referencing the video with specific context.
...
5. Run `inbrain search "{topic_from_video}"`. Confirm the media page appears in search results (verifies the content is indexed and searchable).
