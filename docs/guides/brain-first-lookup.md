...
lookup(name_or_topic):
  ...
  results = inbrain search "{name_or_topic}"
  if results.length > 0:
    page = inbrain get {results[0].slug}
    return page  // done, brain had it

  // STEP 2: Hybrid search (needs embeddings, finds semantic matches)
  results = inbrain query "what do we know about {name_or_topic}"
  if results.length > 0:
    page = inbrain get {results[0].slug}
    return page

  // STEP 3: Direct slug (if you know or can guess the slug)
  page = inbrain get "people/{slugify(name_or_topic)}"
  if page: return page
...
2. **Fuzzy slug matching.** `inbrain get` supports fuzzy matching. If the exact
...
*Part of the [Inbrain Skillpack](../INBRAIN_SKILLPACK.md). See also: [Brain-Agent Loop](brain-agent-loop.md), [Search Modes](search-modes.md)*
