...
Inbrain ships with a [bundled `skills/RESOLVER.md`](../../skills/RESOLVER.md)
...
Inbrain's resolver parser used to require markdown tables:
...
Before v0.41.7.0, `inbrain doctor` only spoke the table dialect. On a
...
`../AGENTS.md`) folding everything into one unified view. Run `inbrain doctor`
...
inherits a table-format `RESOLVER.md` from inbrain and a list-format
...
`inbrain doctor` walks every skill on disk and verifies it's reachable,
...
```bash
inbrain check-resolvable --json
inbrain check-resolvable --strict  # warnings fail too
```
...
### 4. Run `inbrain doctor` and fix any unreachable skills
...
Inbrain: [github.com/inbrain-ai/inbrain](https://github.com/inbrain-ai/inbrain).
