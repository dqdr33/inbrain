...
`inbrain jobs list` visibility, without each one booting a full LLM session.

**PGLite users:** `inbrain jobs work` does not run on PGLite (exclusive file
...
2. **Env flag.** The worker only registers the shell handler when
   `INBRAIN_ALLOW_SHELL_JOBS=1` is set on the worker process. Default: off. Your
...
values resolved at child-spawn from `inbrain config set`). This stops accidental
...
to `~/.inbrain/audit/shell-jobs-YYYY-Www.jsonl` (ISO-week rotation; override
with `INBRAIN_AUDIT_DIR`). Failures log to stderr and don't block submission, so
...
```cron
# Before (LLM gateway):
#   OpenClaw cron: x-inbrain-ai-unified
# After (Minions worker):
3 13,16,19,22,1,4,7,10 * * * \
  inbrain jobs submit shell \
    --params '{"cmd":"node scripts/x-inbrain-ai-daily.mjs","cwd":"/data/.openclaw/workspace"}' \
...
### Calling `inbrain` itself from a shell job — use `inherit:` for DATABASE_URL {#secrets}

A common pattern is submitting shell jobs that run `inbrain` CLI commands:

```bash
inbrain jobs submit shell --params '{
...
env as `INBRAIN_DATABASE_URL`. The job row in `minion_jobs.data` stores
...
`inbrain config set database_url <value>` hint.
...
// ❌ Deprecated as of v0.36.5.0 — REJECTED at submit time.
{
  "cmd": "inbrain stats",
  "cwd": "/data/inbrain",
  "env": { "INBRAIN_DATABASE_URL": "postgresql://..." }
}
```
...
- `inherit: ["database_url"]` → child env `INBRAIN_DATABASE_URL`
...
override is `database_url` → `INBRAIN_DATABASE_URL` (plain `DATABASE_URL` is
...
script prints the secret to stdout or stderr (`echo "$INBRAIN_DATABASE_URL"`,
`psql "$INBRAIN_DATABASE_URL"` echoing the URL on error), the value lands
...
# Or, equivalently:
inbrain jobs submit shell \
  --params '{"cmd":"inbrain sync --skip-failed","cwd":"/data/inbrain","inherit":["database_url"]}' \
  --redact-secrets
```
...
- **Inspect with `inbrain jobs get <id>` after a failure** to verify what
  actually persisted.
...
cat ~/.inbrain/audit/shell-jobs-*.jsonl | jq '.'

# First-time failure mode: submitted without env flag on the worker
inbrain jobs list --status waiting --name shell
# If rows pile up here, no worker with INBRAIN_ALLOW_SHELL_JOBS=1 is running.
```
...
| `shell: inherit requested "<X>" but worker has no <X> configured` | Worker can't resolve the requested name from `loadConfig()`. | Run `inbrain config set <X> <value>` on the worker host, OR check the config file at `~/.inbrain/config.json`. |
...
| `aborted: timeout` / `aborted: cancel` / `aborted: shutdown` / `aborted: lock-lost` | The worker's abort signal fired mid-execution. Child got SIGTERM, 5s grace, then SIGKILL. | Expected: timeout / user cancel / deploy restart / stall. Inspect `inbrain jobs get` to see which. |
| `exit N: <stderr_tail_500>` | Script exited non-zero. | Read `stderr_tail` in `inbrain jobs get`. |
