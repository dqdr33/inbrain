...
Short version: every table in your inbrain's `public` schema needs Row Level
Security enabled. If one doesn't, `inbrain doctor` now fails, not warns, and the
process exits 1.
...
inbrain's service-role connection holds `BYPASSRLS`, so enabling RLS without
policies does NOT break inbrain itself. It just blocks the anon key's default
...
docs/guides/rls-and-you.md for the INBRAIN:RLS_EXEMPT comment escape hatch.
```

99% of the time, you want the fix. Run the SQL. Re-run `inbrain doctor`. Done.

## v0.26.7 — auto-RLS event trigger and one-time backfill

Starting in v0.26.7 (migration v35), inbrain ships two changes that close the
...
reports as a table-creation command. Tables created by inbrain itself, by
...
doesn't carry the `INBRAIN:RLS_EXEMPT` exemption (see below) and enables RLS
on each. After the upgrade, `inbrain doctor`'s `rls` check should be a no-op
on every brain.

### Breaking change: read this before upgrading

If you have public tables that are intentionally RLS-off and you want them
to stay that way, you MUST add the `INBRAIN:RLS_EXEMPT` comment **before**
running `inbrain upgrade` to v0.26.7. The backfill flips RLS on for any public
...
If a non-inbrain app (Baku, Hermes, a script you wrote, anything) creates
...
tables. The fix is at the app side, not inbrain's: either grant BYPASSRLS or
ship a policy.

### What if the trigger gets dropped?

`inbrain doctor` includes a new `rls_event_trigger` check that verifies the
...
```
inbrain apply-migrations --force-retry 35
```
...
trigger function inherits the caller's role, not the inbrain role) — which
defeats the cross-app coexistence story above. If you want defense-in-depth
`FORCE` on a specific inbrain-owned table, add it explicitly in your own
migration; inbrain's auto-RLS does not opt you in by default.
...
inbrain has an escape hatch for these. It is deliberately painful to set up.
That is the feature.
...
COMMENT ON TABLE public.your_table IS
  'INBRAIN:RLS_EXEMPT reason=<why this is anon-readable on purpose>';
```

Rules:

- The comment value MUST start with `INBRAIN:RLS_EXEMPT` (case-sensitive).
...
COMMENT ON TABLE public.expenses_ramp IS
  'INBRAIN:RLS_EXEMPT reason=analytics-only, anon-readable ok, owner=garry, 2026-04-22';
```

After that, `inbrain doctor` reports:
...
reminder. If you ever want to know which tables are open, run `inbrain doctor`.

## Why SQL and not a CLI subcommand

inbrain does NOT ship a `inbrain rls-exempt add <table>` command. A CLI command
...
- Visible in `inbrain doctor` output on every run.
...
WHERE n.nspname = 'public'
  AND c.relkind = 'r'
  AND obj_description(c.oid, 'pg_class') LIKE 'INBRAIN:RLS_EXEMPT%';
```
...
`inbrain doctor` stops listing the table as exempt and goes back to checking
it like any other.
...
doesn't apply. But inbrain still fails the check on missing RLS, because:

- The framing is "RLS on all public tables" is a inbrain security invariant,
  not a Supabase-specific workaround.
- The `ALTER TABLE ... ENABLE RLS` fix is harmless on any Postgres: it only
  constrains non-bypass roles, which inbrain doesn't use.
