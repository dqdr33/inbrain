...
`inbrain` discovers subagent definitions from outside this repo via
`INBRAIN_PLUGIN_PATH`. If you maintain a downstream agent (your OpenClaw
...
```bash
export INBRAIN_PLUGIN_PATH="/path/to/my-plugin"
inbrain jobs work           # worker startup prints the plugin load line
inbrain agent run "summarize meetings/2026-04-20" --subagent-def my-summarizer
```

Multiple plugins: colon-separated, just like `$PATH`.

```bash
export INBRAIN_PLUGIN_PATH="/path/to/plugin-a:/path/to/plugin-b"
```
...
You control where your plugin lives on disk; `inbrain` doesn't guess.

**Collision policy.** If two plugins ship a subagent with the same `name`,
the one listed FIRST in `INBRAIN_PLUGIN_PATH` wins. The other is dropped
...
| `plugin_version` | string | yes      | Contract lock. Must equal `"inbrain-plugin-v1"` for v0.15.          |
| `subagents`      | string | no       | Subdir name (default `subagents`). Escape-attempts are rejected.   |
| `description`    | string | no       | Shown in future `inbrain plugin list`.                              |
...
2. **`~/.inbrain/audit/subagent-jobs-*.jsonl` is local only.** If your
   worker runs on a different host than the `inbrain agent logs` caller,
...
`~/your-openclaw/inbrain-plugin/inbrain.plugin.json`:
...
```bash
export INBRAIN_PLUGIN_PATH="$HOME/your-openclaw/inbrain-plugin"
```

Then your OpenClaw calls `inbrain agent run --subagent-def meeting-ingestion
