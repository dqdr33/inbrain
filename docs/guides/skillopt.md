# `inbrain skillopt` — Self-evolving skills
...
> [Auto-improve a skill with `inbrain skillopt`](../tutorials/improving-skills-with-skillopt.md).
...
inbrain skillopt my-skill --bootstrap-from-skill
...
inbrain skillopt my-skill --bootstrap-reviewed --split 1:1:1
```
...
The audit trail lives at `~/.inbrain/audit/skillopt-YYYY-Www.jsonl`
(ISO-week rotated; honors `INBRAIN_AUDIT_DIR`).
...
| `--allow-mutate-bundled` | off | Required to mutate inbrain-bundled skills in place — ALSO requires `--held-out` (>=5 rows) or the run hard-refuses |
...
| Bundled-skill gate | D16 | Auto-mutating skills shipped with inbrain (in-place mutation requires `--allow-mutate-bundled` + a `--held-out` set of >=5 benchmark-disjoint tasks; else hard-refuse + proposed.md) |
...
- `inbrain skillify scaffold <name>` — create a new skill (use BEFORE skillopt)
- `inbrain skillpack-check <name>` — audit conformance + skillopt status
- `inbrain check-resolvable` — routing MECE validation (NOT mutated by skillopt)
