...
keeps serving the old text because nobody ran `inbrain sync`. Stale search
...
Inbrain is tuned for the Supabase **Transaction pooler** (port 6543): it
...
`INBRAIN_DIRECT_DATABASE_URL` to the **Session pooler** string (port 5432 on the
`pooler.supabase.com` host, IPv4), or enable Supabase's IPv4 add-on. Verify by
running `inbrain sync` and checking that the page count in `inbrain stats` matches
...
```bash
inbrain sync --repo /path/to/brain && inbrain embed --stale
```

- `inbrain sync --repo <path>` -- one-shot incremental sync. Detects changes via
  `git diff`, imports only what changed. For small changesets (<= 100 files),
  embeddings are generated inline during import.
- `inbrain embed --stale` -- backfill embeddings for any chunks that don't have
  them. Safety net for large syncs (>100 files) or prior `--no-embed` runs.
- `inbrain sync --watch --repo <path>` -- foreground polling loop, every 60s
...
**OpenClaw:**
```
Name: inbrain-auto-sync
Schedule: */15 * * * *
Prompt: "Run: inbrain sync --repo /data/brain && inbrain embed --stale
  Log the result. If sync errors mention an unreachable host or timeout,
  the direct connection isn't reachable over IPv4 (set
  INBRAIN_DIRECT_DATABASE_URL to the Session pooler, or enable the IPv4 add-on)."
```

**Hermes:**
```
/cron add "*/15 * * * *" "Run inbrain sync --repo /data/brain &&
  inbrain embed --stale. Log the result." --name "inbrain-auto-sync"
```
...
```bash
inbrain sync --watch --repo /data/brain
```
...
1. **Always chain sync + embed.** Running `inbrain sync` without
   `inbrain embed --stale` leaves new chunks without embeddings. They exist
...
4. **A single un-parseable file can't wedge all indexing.** When a file fails
   ...
   way `INBRAIN_SYNC_AUTOSKIP_AFTER` consecutive syncs (default 3, set `0` to
   disable) is auto-skipped so the rest of the brain keeps indexing past it.
   Skipped files don't disappear: `inbrain doctor` keeps warning until you fix
   ...
   `inbrain sync --skip-failed` to acknowledge a known-bad set yourself.
...
1. **Edit a file and search for the change.** Edit a brain markdown file,
   ...
   poll). Run `inbrain search "<text from the edit>"`. The updated content
   should appear in results. If it returns old content, sync failed.

2. **Compare page count to file count.** Run `inbrain stats` and count the
...
3. **Check embedded chunk count.** In `inbrain stats`, the embedded chunk
   count should be close to the total chunk count. A large gap means
   `inbrain embed --stale` isn't running after sync, leaving chunks invisible
   to vector search.
