...
| Weekly | Brain maintenance | `inbrain doctor`, embed stale, orphan detection | [maintain skill](../../skills/maintain/SKILL.md) |
...
0 6 * * 1 inbrain doctor --json >> /tmp/inbrain-health.log 2>&1 && inbrain embed --stale
...
get_user_timezone():
  calendar = inbrain search "flight" --type calendar --recent 7d
  if recent_flight:
...
dream_cycle():
  ...
  for message in conversations:
    ...
    for entity in entities:
      page = inbrain search "{entity.name}"
      if not page:
  ...
  pages = inbrain list --type person --limit 100
  for page in pages:
...
/cron add "0 2 * * *" "Dream cycle: search today's sessions for
  ...
  if a brain page exists (inbrain search), create or update it if
...
inbrain doctor --json | jq '.checks[] | select(.status=="warn")'
...
5. **Health check:** Run `inbrain doctor --json`. All checks should pass.

---

*Part of the [Inbrain Skillpack](../INBRAIN_SKILLPACK.md). See also: [Quiet Hours](quiet-hours.md), [Operational Disciplines](operational-disciplines.md)*
