...
This is the repo Inbrain indexes.
...
losing your knowledge. You can switch knowledge tools (Inbrain -> something else)
...
**Inbrain indexes the brain repo.** Run `inbrain sync --repo ~/brain/` to keep
the search index current. The agent repo is never indexed by Inbrain.
...
3. **Don't index the agent repo.** Inbrain indexes the brain repo only.
   Running `inbrain sync` against the agent repo pollutes search results
...
3. **Verify Inbrain only indexes brain.** Run `inbrain stats` and check the
