...
Inbrain v0.33 reshapes `inbrain skillpack` from a package manager into a
...
- `inbrain skillpack install <name>` copied bundled skills into your
...
- `inbrain skillpack uninstall` had its own data-loss safeguards (D8
...
inbrain ships a new version, you ask "what changed?" — the agent reads
the diff and decides what (if anything) to integrate.

## The five commands

### `inbrain skillpack scaffold <name> [--workspace PATH]`
...
frontmatter `triggers:` array — inbrain does NOT touch your `RESOLVER.md`
or `AGENTS.md` (see "How agents discover scaffolded skills" below).

```bash
cd ~/git/your-agent-repo
inbrain skillpack scaffold book-mirror
...
### `inbrain skillpack reference <name> [--workspace PATH] [--apply-clean-hunks] [--json]`

Read-only update lens. Diffs inbrain's bundle against your local copy
and emits per-file status (`identical` / `differs` / `missing`) plus
unified diffs for any `differs` entries.

```bash
inbrain skillpack reference book-mirror
# These files live at <inbrain-path> as reference. Read them and
...
parses the diff between inbrain's bundle and your local copy, applies
...
out-of-scope for v0.33), this cannot distinguish "inbrain changed X"
from "you changed X." Applied hunks align everything to inbrain. Use
`--dry-run` first to preview, or run plain `reference` to inspect the
diff before letting auto-apply touch anything.

### `inbrain skillpack migrate-fence [--workspace PATH] [--dry-run]`

One-shot conversion for workspaces on the pre-v0.33 managed-block
model. Strips the `<!-- inbrain:skillpack:begin -->` / `end -->`
...
```bash
cd ~/git/your-agent-repo
inbrain skillpack migrate-fence
...
### `inbrain skillpack scrub-legacy-fence-rows [--workspace PATH] [--dry-run]`
...
### `inbrain skillpack harvest <slug> --from <host-repo-root> [--no-lint] [--dry-run]`

Inverse of scaffold: lifts a proven skill from your host repo back
into inbrain so other clients can scaffold it. Default behavior:

- Symlinks in the host skill dir are rejected (canonical-path
  confinement).
- Privacy linter scans the harvested files against
  `~/.inbrain/harvest-private-patterns.txt` plus built-in defaults
...
This replaces the v0.32 model where `inbrain skillpack install` wrote
...
There's no `inbrain skillpack uninstall` command in v0.33. The files
...
- **New host repo, want a inbrain skill** → `scaffold`
- **inbrain shipped a new version, want to see what's changed**
...
- **Lift your fork's skill back into inbrain** → `harvest` + the
