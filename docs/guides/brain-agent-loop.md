...
READ: check brain FIRST (before responding)
  │  → inbrain search "{entity name}"
  │  → inbrain get {slug} (if you know it)
  │  → inbrain query "what do we know about {topic}"
  │
  ▼
...
on_message(text):
  ...
  for name in entities:
    results = inbrain_search(name)
    if results:
      page = inbrain_get(results[0].slug)
  ...
  if response_contains_new_info(response):
    for entity in mentioned_entities:
      inbrain_add_timeline_entry(entity.slug, {
        date: today,
...
4. **External APIs are fallback, not primary.** `inbrain search` before
   Brave Search. `inbrain get` before Crustdata. The brain has relationship
...
4. **Check the sync.** After a conversation, run `inbrain search "{topic}"`
   from the CLI. The new information should be searchable.

---

*Part of the [Inbrain Skillpack](../INBRAIN_SKILLPACK.md). See also: [Entity Detection](entity-detection.md), [Brain-First Lookup](brain-first-lookup.md)*
