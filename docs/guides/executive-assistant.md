...
on email_batch(emails):
    for email in emails:
        ...
        sender_page = inbrain search "{email.sender_name}"
        if sender_page:
            context = inbrain get <sender_slug>
...
on upcoming_meeting(meeting):
    ...
    for attendee in meeting.attendees:
        ...
        results = inbrain search "{attendee.name}"
        if results:
            page = inbrain get <attendee_slug>
            briefing[attendee] = {
                ...
                "relevant_deals": inbrain get_links <attendee_slug>,
            }
        else:
...
on inbox_cleared():
    for email in processed_emails:
        if email.contained_new_information:
            ...
            inbrain add_timeline_entry <sender_slug> \
...
2. **Unknown senders with no brain page are almost always noise.** If `inbrain search` returns nothing for a sender, they're probably not important. Classify as low priority unless the email content signals otherwise.
...
1. Run meeting prep for tomorrow's calendar. For each attendee, confirm the agent ran `inbrain search` and loaded their brain page before generating the briefing.
2. Triage 5 emails. Confirm the agent searched for each sender in the brain before classifying the email.
3. After clearing an inbox, check 2 sender brain pages with `inbrain get <slug>`. Confirm new timeline entries were added with information from the emails.
