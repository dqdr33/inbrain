...
**A single inbrain database can hold multiple knowledge repos.** Each one
...
inbrain sources add gstack --path ~/.gstack --federated

# Pin the directory so `inbrain sync` knows which source it's walking
cd ~/.gstack && inbrain sources attach gstack

# Initial sync
inbrain sync --source gstack

# Now `inbrain search "retry budgets"` returns hits from BOTH wiki and
...
inbrain sources add yc-media --path ~/yc-media --no-federated
inbrain sources add garrys-list --path ~/writing --no-federated

# Pin each checkout directory
(cd ~/yc-media && inbrain sources attach yc-media)
(cd ~/writing && inbrain sources attach garrys-list)

# Sync each independently
inbrain sync --source yc-media
inbrain sync --source garrys-list
```
...
```bash
inbrain search "tech layoffs" --source yc-media,garrys-list
```
...
inbrain sources add sessions --path ~/.claude/sessions --no-federated
```

## Resolution priority

When any command needs to pick a source, inbrain walks this list (highest
first):

1. Explicit `--source <id>` flag.
2. `INBRAIN_SOURCE` environment variable.
3. `.inbrain-source` dotfile in CWD or any ancestor directory.
4. A registered source whose `local_path` contains the CWD (longest
   prefix wins for nested checkouts).
5. The brain-level default set via `inbrain sources default <id>`.
6. The seeded `default` source.

So inside `~/.gstack/plans/` on a brain that pinned `gstack` to
`~/.gstack` via `.inbrain-source`, `inbrain put-page` implicitly writes to
...
| Value | Meaning |
|-------|---------|
| `true` | Source participates in unqualified `inbrain search "X"` results. |
...
Flip later with `inbrain sources federate <id>` / `unfederate <id>`.
...
```
inbrain sources add <id> --path <p> [--name <n>] [--federated|--no-federated]
                               Register a source. id: [a-z0-9](?:[a-z0-9-]{0,30}[a-z0-9])?
inbrain sources list [--json]   List all sources with page counts + federation state.
...
inbrain sources default <id>    Set the brain-level default.
inbrain sources attach <id>     Write .inbrain-source in CWD (like kubectl context).
inbrain sources detach          Remove .inbrain-source from CWD.
inbrain sources federate <id>
inbrain sources unfederate <id>
```
...
`inbrain sources rename` changes the display name only; existing
...
inbrain put-page topics/ai ... --source wiki

# Or rely on the dotfile / env / CWD match
cd ~/.gstack && inbrain put-page plans/multi-repo ...
...
`inbrain upgrade` runs the v16 + v17 migrations automatically. Your
...
```bash
inbrain sources add gstack --path ~/.gstack --federated
cd ~/.gstack && inbrain sources attach gstack && inbrain sync
```
...
- Session transcript ingest (`.jsonl`, raised size cap, session
  PageType) — v0.18.
- Per-source retention/TTL (`inbrain sources prune`) — v0.18.
- ACL enforcement via caller-identity — v0.17.1.
- `inbrain sources import-from-github <url>` one-shot bootstrap — patch
