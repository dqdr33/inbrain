...
```bash
inbrain doctor --json | jq '.checks[] | select(.name == "queue_health")'
```
...
- **waiting-depth**: any per-name queue deeper than 10 (override via
  `INBRAIN_QUEUE_WAITING_THRESHOLD`). Signals a missing `maxWaiting`.
...
- **The supervisor restarts a worker that stops making progress.** If a queue
  ...
  `--wedge-restart-checks` on `inbrain jobs supervisor` (0 disables).

The signal is loud now — check either:

```bash
inbrain jobs stats --queue default          # prints a WEDGED QUEUE line
inbrain doctor --json | jq '.checks[] | select(.name == "wedged_queue")'
```
...
```bash
inbrain jobs supervisor stop && inbrain jobs supervisor start   # fresh pool
inbrain jobs retry <id>                                        # dead-lettered jobs
```
...
inbrain jobs list --status active

# Who's waiting, biggest pile first?
inbrain jobs list --status waiting --limit 50
...
inbrain jobs smoke --wedge-rescue
```
...
- **waiting-depth** — Submitters are piling up jobs faster than workers
  ...
  `INBRAIN_QUEUE_WAITING_THRESHOLD=50 inbrain doctor`.
...
inbrain jobs list --status active | head -5
```
...
```bash
INBRAIN_ALLOW_SHELL_JOBS=1 inbrain jobs work --concurrency 4
```
...
- B3 — `inbrain doctor --fix` learns to rescue queue wedges.
