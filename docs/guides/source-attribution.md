...
on brain_write(page, fact):
    ...
    inbrain put <slug> --content "...fact [Source: ...]..."
...
1. Open any brain page with `inbrain get <slug>`. Read the compiled truth section above the bar. Every factual claim should have an inline `[Source: ...]` citation.
2. Search for tweet references: `inbrain search "X/@"`. Every result should have a full URL, not just an @handle.
