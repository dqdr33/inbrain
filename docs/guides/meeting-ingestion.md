...
on new_meeting_transcript(meeting):
    ...
    inbrain put <slug> --content "<compiled_truth>\n---\n<timeline>"
    ...
    for person in meeting.attendees + meeting.mentioned_people:
        inbrain add_timeline_entry <person_slug> \
    ...
    for company in meeting.mentioned_companies:
        inbrain add_timeline_entry <company_slug> \
    ...
    for entity in all_entities_mentioned:
        inbrain add_link <slug> <entity_slug>   # meeting -> entity
        inbrain add_link <entity_slug> <slug>    # entity -> meeting
...
1. After ingesting a meeting, run `inbrain get meetings/{date}-{slug}`. Confirm the page has the agent's analysis above the bar and the full diarized transcript below it.
2. For each attendee, run `inbrain get <attendee_slug>`. Check that their timeline has a new entry referencing the meeting with specific insights (not just "attended meeting").
3. Pick a company mentioned in the meeting. Run `inbrain get <company_slug>`. Confirm a timeline entry exists referencing what was discussed about the company.
4. Run `inbrain get_links meetings/{date}-{slug}`. Verify back-links exist to all attendee and entity pages.
5. Run `inbrain search "{meeting_topic}"`. Confirm the meeting page appears in search results (verifies sync ran).
