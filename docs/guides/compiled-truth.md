...
update_brain_page(slug, new_info, source):
  ...
  inbrain add_timeline_entry {slug} {
    date: today,
...
3. **Inbrain search weights compiled truth higher.** `inbrain query` returns compiled
...
4. **The --- separator matters.** Inbrain uses the first standalone `---` after
...
2. **Search for the person.** `inbrain query "Sarah Chen"`. The compiled truth
...
*Part of the [Inbrain Skillpack](../INBRAIN_SKILLPACK.md). See also: [Source Attribution](source-attribution.md), [Entity Detection](entity-detection.md)*
