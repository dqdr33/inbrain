# How a downstream agent should talk to inbrain

This guide is for authors of downstream agents (hermes, openclaw, future
forks) that need to call inbrain operations from their own runtime. Reading
this first will save you a debugging cycle: inbrain has **two distinct
...
The host runs inbrain as a long-lived HTTP server:

```bash
INBRAIN_ALLOW_SHELL_JOBS=1 inbrain serve --http --port 3131
```

The agent registers as an OAuth client (one-time):

```bash
inbrain auth register-client hermes \
...
grant. Secrets stay in the inbrain serve process; the agent never sees
DATABASE_URL or API keys.

Thin-client mode (`inbrain init --mcp-only`) gives the agent the same
client-credentials wiring, plus the `inbrain` CLI itself routes MCP-eligible
commands through the configured remote MCP. The agent can call
`inbrain search` / `inbrain query` directly and the CLI does the OAuth dance.
...
- `init` (creates `~/.inbrain/`)
- `secrets` (config management)

For these, the agent cannot route through HTTP MCP. The only path is to run
`inbrain` as a CLI subprocess. The recommended pattern is to submit the
subprocess as a shell job to the inbrain Minions worker so retry / backoff /
...
env as `INBRAIN_DATABASE_URL`. The DB row in `minion_jobs.data` carries the
...
- Pre-v0.36.5.0 callers passed `env: { INBRAIN_DATABASE_URL: "postgresql://..." }`
...
inbrain jobs submit shell --params '{...}' --follow

# Persistent worker (Postgres only — PGLite uses --follow inline):
INBRAIN_ALLOW_SHELL_JOBS=1 inbrain jobs work
```

`INBRAIN_ALLOW_SHELL_JOBS=1` is the worker-side opt-in. Without it, shell jobs
...
- **Free-form names.** `inherit:` accepts any snake_case config-key on your
  ...
  field you stuff into `~/.inbrain/config.json`. The agent picks what it
  needs.
...
`inbrain config set database_url <value>` or via `INBRAIN_DATABASE_URL` /
