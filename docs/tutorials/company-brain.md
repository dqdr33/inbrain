...
I'm Inbrain Team. I built Inbrain to run my own AI agents at Y Combinator. After a couple of months of multi-user features landing (parallel sync across team sources, per-user OAuth scoping, leak-free isolation across every read path), it's finally usable as a company brain too. This is the recipe I'd run if I were standing it up for a 10-50 person company today.
...
It is **not** a different install. The agent runtime, Supabase backend, Inbrain CLI, and AlphaClaw harness from the personal brain stay exactly as you set them up. We're adding to that stack, not replacing it.
...
inbrain sources add shared --path /srv/brain-repos/shared --name "Shared company wiki"

# A scoped source for sales/customer notes
inbrain sources add customers --path /srv/brain-repos/customers --name "Customer notes"

# A scoped source for internal-only docs (legal, HR, performance, board)
inbrain sources add internal --path /srv/brain-repos/internal --name "Internal-only"
```
...
```bash
inbrain serve --http --port 3131 --bind 0.0.0.0
```
...
```bash
inbrain serve --http --port 3131 --bind 0.0.0.0 --public-url https://brain.acme-co.com
```
...
inbrain auth register-client alice-example \
...
inbrain auth register-client bob-example \
  --grant-types client_credentials \
  --scopes read,write \
  --source internal \
...
inbrain auth register-client carol-example \
  --grant-types client_credentials \
  --scopes read,write \
  --source shared \
...
- `--scopes read,write` lets the client query the brain and write new pages. You can omit `write` for read-only clients (executive summaries, dashboards). The `admin` scope is needed for operational commands like `inbrain remote doctor` and is usually reserved for your own admin client.
...
```bash
# Terminal 1, as Alice
export INBRAIN_REMOTE_CLIENT_ID=<Alice's client_id>
export INBRAIN_REMOTE_CLIENT_SECRET=<Alice's client_secret>
export INBRAIN_REMOTE_MCP_URL=https://brain.acme-co.com/mcp

inbrain search "performance review" --remote
```
...
Each cron is just a scheduled `inbrain agent run` call scoped to the teammate's client credentials. The schedule lives in the workspace repo (the one AlphaClaw deployed in the personal-brain tutorial), in a `crons/` directory. A typical layout:
...
To install the cron schedule, commit the file to the workspace repo and let AlphaClaw pick it up on next deploy. The cron-scheduler skill (one of the 60 that Inbrain installed) handles the dispatch.

---

## Part 7: Add per-person skills

The 60+ skills Inbrain installs are generic. Your team probably wants a few that are specific to them. Examples:
...
Each `SKILL.md` declares the trigger (verbs in plain English the agent listens for) and the procedure. Use the `inbrain skillify scaffold <name>` command to generate the boilerplate:

```bash
inbrain skillify scaffold onboarding-new-hire
```
...
The actual skills that implement this in production are named `slack`, `slack-scan`, `slack-archive`. Scaffold equivalents in your workspace with `inbrain skillify scaffold slack-scan`, then edit the generated SKILL.md to declare your channel mapping and triggers.
...
When Claude Desktop launches, it talks to the local `inbrain serve` stdio bridge, which forwards every request to your remote brain over HTTPS with their OAuth token attached. From Claude Desktop's perspective it's just one MCP server.

For Claude Code, Cursor, OpenClaw, Hermes, and other clients, per-client setup steps live in [`docs/mcp/`](../mcp/). They all follow the same shape: point the agent at the local `inbrain serve` bridge, which knows about the remote.

---

## Part 11: First real query as a teammate

Have Alice run a real query from her machine. The interesting verb is `inbrain think`, which gives back a synthesized answer instead of raw pages.

```bash
inbrain think "What's the latest update from acme-co? When did we last talk to them?"
```
...
### Background daemon: `inbrain autopilot`
...
### Self-healing: `inbrain doctor --remediate`

```bash
inbrain doctor --remediate --yes --target-score 90 --max-usd 5
```
...
### Monitoring: `inbrain sources status` and the admin dashboard
...
Real numbers from the published benchmark, running the default stack (Inbrain with ZeroEntropy for embedding + reranker):

- **Embedding cost:** $0.05 per million tokens. For comparison, Inbrain configured with OpenAI is $0.13 (2.6× more expensive), Voyage is $0.18 (3.6× more).
...
- **Query latency:** about 122 ms median for a `inbrain search`. For comparison, the same query through Inbrain with OpenAI takes about 282 ms.
- **Synthesized-answer latency:** a few seconds, dominated by the Anthropic API.
- **Retrieval quality:** on the public LongMemEval benchmark, Inbrain hits 97.60% recall at the top 5 retrieved sessions, beating the previous published state of the art at 96.6%. On the in-house BrainBench corpus of relational queries, Inbrain beats commodity vector retrieval by 38 percentage points, because the graph layer surfaces relationships that vector similarity alone misses.

Full methodology and per-run receipt JSONs live in [the inbrain-evals repo](https://github.com/inbrain-ai/inbrain-evals/blob/main/docs/benchmarks/2026-05-23-v0.40.6.0-snapshot.md).
...
Check `inbrain auth list` on the host and confirm their client has `--source` set to a source that actually exists. Empty or null `--source` means the client falls through to the `default` source, which probably has no content if you set up three named sources.

### "Sync is slow and feels stuck"

The first sync embeds every page, which takes time. Check `inbrain sources status` for the live page count. If it's climbing you're not stuck, you're just embedding. If you've got a 10K-page corpus and ZeroEntropy is being throttled, the per-source parallel sync looks like progress on three sources at once rather than one source moving fast.

### "I see a page I shouldn't see"

This shouldn't happen, but if you suspect it, run `inbrain search <query> --remote --json` as the constrained client and inspect the `source_id` field on every returned result. Every row should be in the client's `--federated-read` set. If one isn't, file an issue with the exact slug and source IDs.

### "The synthesized answer is wrong"

The brain layer is grounded in the retrieved pages. If the retrieved pages contain bad information, the answer will too. The gap-analysis note often catches this: if the answer says "based on retrieved pages from date X" and date X is six months ago, the brain is telling you the information is stale. Run `inbrain sync --all` to refresh and try again.

### "OAuth `/token` endpoint returns 401 for my client"

Verify the client secret matches what was printed at register-client time. The server stores only a SHA-256 hash; if you lost the original, you have to revoke the client and re-register. Use `inbrain auth revoke-client <client_id>` and re-run `register-client`.

### "Postgres connection is exhausting"

Each parallel sync worker opens its own pool. With three sources and the default four workers per source, you can hit your Postgres connection limit if it's set low. Either reduce the worker count with `inbrain sync --all --parallel 2 --workers 2`, or raise your Postgres `max_connections` to at least 100. Supabase's free tier defaults to 60, which is tight.

### "I want to add a fourth teammate but they need access to all three sources"

```bash
inbrain auth register-client diana-example \
  --grant-types client_credentials \
  --scopes read,write \
  --source shared \
  --federated-read shared,customers,internal
...
- **Explore the rest of the brain layer.** `inbrain whoknows` (find the expert on a topic), `inbrain find_trajectory` (how a metric changed over time), `inbrain founder scorecard` (especially useful for VC and ops teams), the contradiction-detection cycle that surfaces conflicts between different people's notes.
...
Questions, gotchas, or wins worth sharing? Open an issue at [github.com/inbrain-ai/inbrain](https://github.com/inbrain-ai/inbrain/issues).
