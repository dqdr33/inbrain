# Give your coding agent a memory: Inbrain + Claude Code / Codex
...
down somewhere. Inbrain is the retrieval layer that fixes that: search, synthesis,
...
- **Path A — I already run a brain** (OpenClaw, Hermes, or any `inbrain serve`
...
Prerequisite for either path: `bun install -g github:inbrain-ai/inbrain`.
...
```bash
inbrain serve --http --bind 0.0.0.0 --public-url https://your-host.example.com
```
...
- **`--bind 0.0.0.0`** — the default bind is `127.0.0.1` (loopback only), which
  ...
  brain" and you didn't pass this, that's why. `inbrain serve --http` warns you at
...
(New brains from `inbrain init` default this ON. Brains upgraded from before
...
```bash
inbrain auth create "laptop-agents"
```

Copy the `inbrain_…` token it prints. It's a long-lived, full-access secret. Treat
...
```bash
# Claude Code
inbrain connect https://your-host.example.com/mcp --token inbrain_xxx --install

# Codex
inbrain connect https://your-host.example.com/mcp --token inbrain_xxx --agent codex --install
```
...
```
Added MCP server 'inbrain' -> https://your-host.example.com/mcp.
...
reads the bearer from `$INBRAIN_REMOTE_TOKEN` at runtime, so the token never lands
...
```bash
inbrain init --pglite      # 2 seconds; embedded Postgres via WASM, no Docker
```
...
inbrain import ~/notes/

# Or capture as you go (one thought at a time):
inbrain capture "Decided to use PGLite as the default engine: zero-config beats Postgres for <1000 files."
```
...
```bash
# Claude Code
claude mcp add inbrain -- inbrain serve

# Codex
codex mcp add inbrain -- inbrain serve
```

That's the whole wire-up. No token, no URL, no tunnel. The agent spawns
`inbrain serve` as a stdio subprocess and talks to your local brain directly.
...
(`inbrain query "..."`) and from your agent.
...
any coding agent with Inbrain connected:
...
| Symptom | Cause | Fix |
|---|---|---|
| Agent "can't reach the brain" (Path A) | `inbrain serve --http` bound to loopback | Restart with `--bind 0.0.0.0` |
| `list_skills` returns nothing / errors | Skill publishing OFF on the host | `inbrain config set mcp.publish_skills true` |
| Token rejected on first call | Wrong/expired token | Re-mint with `inbrain auth create`; `--install` smoke-tests it for you |
| `unknown tool: capture` | `capture` is CLI-only, not an MCP tool | Use `put_page` over MCP; `capture` only on the CLI |
| Empty results (Path B) | Brain has nothing in it yet | `inbrain import ~/notes/` or `inbrain capture "..."` |

## Next steps

- Go full autonomous: the overnight enrichment daemon ([dream cycle](../../CHANGELOG.md)) fixes citations, dedupes people, builds scorecards while you sleep. See `inbrain autopilot --install`.
