# Auto-improve a skill with `inbrain skillopt`
...
```bash
inbrain skillopt meeting-prep --bootstrap-from-skill
```
...
```bash
inbrain skillopt meeting-prep --bootstrap-reviewed --split 1:1:1
```

If you run an agent over this brain (OpenClaw, Claude Code, Cursor, any MCP client
with the inbrain skills installed), it does this for you: just say "improve my
...
- `inbrain` installed and a brain initialized (`inbrain --version` works).
- One embedding/chat provider configured. SkillOpt makes real LLM calls.
  `inbrain models doctor` should show at least one reachable chat model.
...
```bash
inbrain skillify scaffold meeting-prep
```
...
`inbrain skillopt meeting-prep --bootstrap-from-skill` writes the file for you, then
...
inbrain skillopt meeting-prep --split 1:1:1
```
...
```bash
inbrain skillopt meeting-prep --split 1:1:1 --dry-run
```
...
log at `~/.inbrain/audit/skillopt-YYYY-Www.jsonl`.
...
**For a skill that ships with inbrain** (anything under the inbrain repo's own
...
inbrain skillopt meeting-prep --split 1:1:1 --no-mutate
...
inbrain skillopt brain-ops --split 1:1:1 --allow-mutate-bundled \
...
`inbrain skillopt --all` runs it across every skill that has a benchmark, under a
brain-wide cost cap.
...
- **Every flag inline:** `inbrain skillopt --help`
...
- **Generate a starter benchmark from the SKILL.md** (the recommended way to start):
  `inbrain skillopt <name> --bootstrap-from-skill` → review + strengthen the judges →
  delete the sentinel → `--bootstrap-reviewed --split 1:1:1`. Tune the count with
  `--bootstrap-tasks N` (max 50).
- **Bootstrap from existing routing fixtures** instead: `inbrain skillopt <name>
