...
Inbrain ships skills in `skills/`. Downstream agents (custom OpenClaw deployments,
...
language. Once that happens, inbrain can't push updates to those forks. The agent has
...
`inbrain upgrade` ships the new binary. `inbrain post-upgrade [--execute --yes]` runs
...
says `# Based on inbrain v0.10.0` at the top, it doesn't know about v0.12.0 features.

The agent will keep manually calling `inbrain link` after every `put_page` (now redundant —
auto-link does it), miss out on `inbrain graph-query` for relationship questions, and
...
4. Update the version banner at the top of your fork (`# Based on inbrain v0.12.0`).
...
agent's mental model says it must call `inbrain link` after every `put_page`, which
...
- To disable: `inbrain config set auto_link false`. Default is on.
- Timeline entries with specific dates still need explicit `inbrain timeline-add`
  (or batch via `inbrain extract timeline --source db`).
```
...
**Why:** Eliminates redundant `inbrain link` calls per attendee (auto-link handles them
when the meeting page references attendees as `[Name](people/slug)`).

```markdown
**Note (v0.12.0):** Once the meeting page is written via `inbrain put`, the
...
need to call `inbrain link` for attendees. You DO still need `inbrain timeline-add`
...
**Why:** Same logic as brain-ops — eliminates manual `inbrain link` after writing
...
need to call `inbrain link` manually. Timeline entries still need explicit calls.
```
...
- Add back-links manually via `inbrain link` for any new entity references
```
...
Timeline entries still need explicit `inbrain timeline-add` calls.
```
...
1. **Bump the version banner** at the top of each forked file:
   ```
   # Based on inbrain v0.12.0 skills/<skill-name>, extended with <your-agent>-specific config
   ```

2. **Run the v0.12.0 backfill** (this populates the graph for your existing brain):
   ```bash
   inbrain post-upgrade
   ```
...
4. **Verify graph traversal works:**
   ```bash
   inbrain graph-query people/some-well-connected-person --depth 2
   ```
...
The `v0_12_2` orchestrator runs `inbrain repair-jsonb` automatically. It rewrites
...
```bash
inbrain repair-jsonb --dry-run --json    # expect totalRepaired: 0
```
...
- Option 3 (accept the gap): the attendee edge will not be created until a page exists.
  Re-running `inbrain extract links --source db --include-frontmatter` after creating
...
`inbrain upgrade` takes 2-5 min on a 46K-page brain (one-time). Runs out-of-process via `inbrain post-upgrade`. If your agent holds a DB connection during the upgrade, reconnect after; otherwise keep serving.

### Type normalization NOT in v0.13

Legacy rows with `link_type='attendee'` or `link_type='mention'` coexist with new `'attended'` / `'mentions'` rows. Your queries filtering on old type names keep working. A separate opt-in `inbrain normalize-types` command in v0.14 handles the rename.
...
1. Set `INBRAIN_ALLOW_SHELL_JOBS=1` on the worker process, then `inbrain jobs work`
...
2. Classify each of your host's cron entries: LLM-requiring (keep on gateway) vs
   ...
   - **Deterministic → shell:** `ycli-token-refresh`, `x-oauth2-refresh`,
     `x-inbrain-ai-unified`, `calendar-sync-to-brain`, `github-pulse`,
...
upcoming `inbrain crontab-to-minions <file>` helper is P1 in TODOS.

---

## v0.16.0: durable agent runtime

v0.15 ships `inbrain agent run` / `inbrain agent logs`, a new `subagent` handler
...
MCP callers get `permission_denied`; only `inbrain agent run` can insert
these rows).

```bash
ANTHROPIC_API_KEY=sk-ant-... inbrain jobs work
```
...
Move your custom subagent definitions out of your inbrain fork and into your own
repo as a plugin. Concretely:

```
~/<your-agent>/inbrain-plugin/
├── inbrain.plugin.json
└── subagents/
...
```json
{
  "name": "your-openclaw",
  "version": "2026.4.20",
  "plugin_version": "inbrain-plugin-v1"
}
```
...
```bash
export INBRAIN_PLUGIN_PATH="$HOME/<your-agent>/inbrain-plugin"
```
...
worker restarts, migrate those to `inbrain agent run`. The durability is free:

```bash
inbrain agent run "analyze my last 50 journal pages for recurring themes" \
...
ledger, and `inbrain agent logs <job>` shows where it died + what the last
...
frontmatter, replace them with `inbrain frontmatter validate` calls. The CLI
...
+ inbrain frontmatter validate <path> --json
```

For consumers that need the validator inside another script, import from
inbrain's `markdown` export instead of duplicating logic:

```ts
import { parseMarkdown } from 'inbrain/markdown';
...
landed), replace those references with the inbrain CLI. The `frontmatter-guard`
skill lives at `skills/frontmatter-guard/SKILL.md` and points at
`inbrain frontmatter validate` / `audit` / `install-hook`.

### 3. Wire the doctor subcheck into your health pipeline

`inbrain doctor` now reports `frontmatter_integrity` automatically. If your
fork has a custom health pipeline (e.g. a daily Slack post about brain
health), pull from `inbrain doctor --json` and surface the
...
```bash
inbrain frontmatter install-hook
```
...
After `inbrain apply-migrations --yes` runs the v0.22.4 audit, your agent
should read `~/.inbrain/migrations/pending-host-work.jsonl` (filter to
`migration === "0.22.4"`) and walk each entry's `command` field. Each entry
points to a per-source `inbrain frontmatter validate <source_path> --fix`
...
When inbrain ships a new version, this doc will be updated with the diffs for that
version. Each new version appends a section; old sections stay so you can catch up
multiple versions at once.

To check what your fork is missing:
```bash
diff <(grep -A3 "Based on inbrain" ~/<your-fork>/skills/brain-ops/SKILL.md) \
     <(grep "v[0-9]" ~/inbrain/skills/migrations/ | tail -3)
```


## v0.36.5.0 — Free-form secret inheritance for shell jobs calling `inbrain` CLI
...
**Why.** Pre-v0.36.5.0, agents that wanted to call `inbrain` from shell jobs
had to either write `database_url` to `~/.inbrain/config.json` plaintext or
pass `env: { INBRAIN_DATABASE_URL: "..." }` per-job. Both left plaintext
...
```jsonc
{
  "cmd": "inbrain sync --skip-failed && inbrain embed --stale",
  "cwd": "/data/inbrain",
...
`database_url` → `INBRAIN_DATABASE_URL`, `anthropic_api_key` →
...
- `inbrain config set database_url postgresql://...` (or any other key you
  want available for inherit)
- OR put the key in `~/.inbrain/config.json` directly
- OR set `INBRAIN_DATABASE_URL` / `DATABASE_URL` / per-provider env on the
  worker process

If the worker can't resolve a requested name, the validator fail-fasts at
submit time with `inbrain config set <X>` hint. No more silent "No database
URL" failures in child stderr minutes after submission.

**Also new.** A `inbrain doctor` check `home_dir_in_worktree` warns if
`~/.inbrain/` lives inside a git worktree. A retroactive `~/.inbrain/.gitignore`
(single line `*`) is now laid down by every `saveConfig()` call AND by
`inbrain post-upgrade`, so existing users get coverage without re-running
`inbrain init`. Honest scope: the `.gitignore` covers casual `git add` but does
NOT cover already-tracked files, screenshots, backups, or `git add -f`.

**Strategy framing.** For agent-to-inbrain calls, the new canonical guide is
`docs/guides/agent-to-inbrain.md`. Two distinct surfaces: HTTP MCP via OAuth
...
| `shell: inherit requested "<X>" but worker has no <X> configured` | Worker can't resolve the name from its `loadConfig()`. | Run `inbrain config set <X> <value>` on the worker host. |
