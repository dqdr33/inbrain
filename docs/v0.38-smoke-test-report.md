...
> PR #1299 (`inbrain-ai-agents`). Two findings were re-diagnosed during the
...
>
> - **WARN-5 root cause:** `inbrain capture --help` is minimal because
...
|----------|------|------|------|
| `inbrain capture` — basic | 7 | 0 | 0 |
| `inbrain capture` — edge cases | 3 | 3 | 1 |
...
inbrain capture --file /tmp/test.md
inbrain get inbox/2026-05-22-XXXXXX
```
...
```
inbrain capture: put_page failed: insert or update on table "pages" violates
...
Error: source 'my-source' is not registered. Register it first:
  inbrain sources add my-source --path /path/to/source
```
...
connection: connect() has not been called. Fix: Run inbrain init --supabase
```
...
```
$ inbrain capture --help
Usage: inbrain capture

inbrain capture - run inbrain --help for the full command list.
```
...
### WARN-6: `capture`, `brainstorm`, `lsd` missing from `inbrain --help`
...
`inbrain --help` lists every other command (get, put, search, etc.) but
...
inbrain capture --file /tmp/binary.bin
```
...
- **`inbrain capture "text"`** — works perfectly for the simple case.
  Frontmatter stamps, JSON output, disk write, immediate searchability.
- **`inbrain capture --file`** — works for plain text and simple markdown
  (frontmatter doubling bug only affects files with existing frontmatter).
- **`inbrain capture ""`** and no-args — clean error messages:
  `"provide content positionally, --file PATH, or --stdin"`.
- **`inbrain capture --file /nonexistent`** — clean ENOENT error.
- **Unicode/emoji** — `inbrain capture "测试 🧠🔥 émojis"` works perfectly.
...
- **Soft delete + recovery** — cleanup via `inbrain delete` works with
  72h recovery window.
...
2. **Main help completeness** — add `capture`, `brainstorm`, and `lsd` to
   the `inbrain --help` command listing. These are user-facing features that
   can't be discovered.
