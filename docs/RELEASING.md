# Releasing & contributing (inbrain)
...
- `bun run ci:local` runs the entire stack inside Docker: gitleaks (host), unit
  ...
  `INBRAIN_CI_PG_PORT=5435 bun run ci:local` if 5434 collides.
...
- Upgrade instructions: `inbrain upgrade` + any manual step if needed.
...
`inbrain brainstorm` does and how to use it. If a fact only exists because
...
**Smell test:** read the entry as a stranger who has never touched inbrain.
...
must be readable by someone who does NOT know inbrain's internals. No file paths,
...
**The smell test:** if someone who has never opened inbrain reads the first 150
...
- Latest `inbrain-evals/docs/benchmarks/[latest].md` for headline numbers (sibling repo)
...
Why: `inbrain upgrade` runs `inbrain post-upgrade` which runs `inbrain apply-migrations`.
...
recovery path; the v0.13+ `~/.inbrain/upgrade-errors.jsonl` trail + `inbrain doctor`
...
`inbrain upgrade` should do this automatically. If it didn't, or if `inbrain doctor`
...
3. **Verify the outcome:**
   ```bash
   [release-specific verify commands, e.g. `inbrain graph ... --depth 2`]
   inbrain stats
   ```
4. **If any step fails or the numbers look wrong,** please file an issue:
   https://github.com/inbrain-ai/inbrain/issues with:
   - output of `inbrain doctor`
   - contents of `~/.inbrain/upgrade-errors.jsonl` if it exists
   - which step broke

   This feedback loop is how the inbrain maintainers find fragile upgrade paths. Thank you.
```
...
- Bad: "Added INBRAIN_VERIFY.md installation verification runbook"
- Good: "Your agent now verifies the entire Inbrain installation end-to-end, catching
...
- Schema changes that require `inbrain init` or manual SQL
...
Inbrain's job is to deliver a canonical, working setup to every user on upgrade.
Anything that looks like a "host-repo change" — AGENTS.md, cron manifests,
launchctl units, config files outside `~/.inbrain/` — is a Inbrain migration
...
`~/.inbrain/migrations/pending-host-work.jsonl` + the host agent walks the
...
`~/.inbrain/update-state.json` tracks which recommended schema directories the user
...
3. **Collector branch** — create a feature branch (e.g. `inbrain-ai/fix-wave-N`), cherry-pick
...
## Checking out PRs from inbrain-ai-agents

`inbrain-ai-agents` is the AI-authored PR account and is NOT a collaborator on
...
When the user says "check out <PR link>" and the PR is from `inbrain-ai-agents`
...
2. `git push origin HEAD:<branch-name>` — push the same branch to the base
   repo (origin points at `inbrain-ai/inbrain`, not the fork). This is the move
...
Why this over alternatives: adding `inbrain-ai-agents` as a collaborator, or
