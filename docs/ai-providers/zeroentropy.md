...
Both land in inbrain v0.35.0.0 behind the openai-compatible recipe path,
...
**Important:** `inbrain config set embedding_model …` is NOT a live
...
from the **file plane** (`~/.inbrain/config.json`) and the **env plane**
(`INBRAIN_EMBEDDING_MODEL` / `INBRAIN_EMBEDDING_DIMENSIONS`). The DB plane
is intentionally ignored for these two keys (same posture as today's
Voyage setup).

### Option A — file plane (recommended for stable installs)

Edit `~/.inbrain/config.json`:
...
```bash
export INBRAIN_EMBEDDING_MODEL=zeroentropyai:zembed-1
export INBRAIN_EMBEDDING_DIMENSIONS=2560
```
...
```bash
inbrain embed --stale --limit 50    # smoke a small batch
inbrain embed --stale               # full re-embed
```

### Verify

```bash
inbrain models doctor --json | jq '.probes[] | select(.touchpoint=="embedding_config")'
```

Expected: `status: "ok"`. Invalid dims (e.g. `1024`, `1536`, `3072`)
surface as `status: "config"` with a paste-ready
`inbrain config set embedding_dimensions <one of 2560|1280|640|320|160|80|40>` fix hint.

## Reranker switch — zerank-2

The reranker is the bigger story: inbrain had no cross-encoder reranker
...
before, just with an observable failure surfaced via `inbrain doctor`.

### Opt-in on `conservative` or `balanced` mode

```bash
inbrain config set search.reranker.enabled true
```
...
```bash
inbrain models doctor --json | jq '.probes[] | select(.touchpoint=="reranker_config")'
```
...
`~/.inbrain/audit/rerank-failures-YYYY-Www.jsonl` (ISO-week rotation).

`inbrain doctor` reads the audit and surfaces:
...
| `reranker_health` doctor warns about auth | `ZEROENTROPY_API_KEY` not set or invalid | Re-export the env var; `inbrain models doctor` to verify |
