...
`{results: [{index, relevance_score}]}` — exactly the wire shape inbrain
...
path, which makes the inbrain config string ugly and brittle. With
...
(typically 8080 for embeddings, 8081 for reranking — inbrain's defaults
match that convention).

### 4. Wire inbrain at your server

```bash
# Point inbrain at the llama.cpp host (skip if running locally on default port)
inbrain config set provider_base_urls.llama-server-reranker http://your-host:8081/v1

# Tell search to use this reranker
inbrain config set search.reranker.model llama-server-reranker:qwen3-reranker-4b
...
inbrain search "some query" --json | jq '.[].rerank_score'
# Expect: rerank_score on every row
```

If `inbrain models doctor` reports the reachability probe as `network`
...
inbrain config set search.reranker.timeout_ms 60000
```
...
```bash
INBRAIN_MAX_USD=0.01 inbrain search "..." --reranker llama-server-reranker:qwen3-reranker-4b
...
response) logs to `~/.inbrain/audit/rerank-failures-*.jsonl` and
