...
`inbrain init --mcp-only` (v0.29.2) sets up a thin-client install: no local
brain content, just an OAuth client pointing at a remote `inbrain serve --http`.
...
- `src/cli.ts` — Routing seam INSIDE the existing op-dispatch path (CDX-1: no
  ...
  before each routed call (suppressed by `--quiet`, `INBRAIN_NO_BANNER=1`,
...
- `src/core/doctor-remote.ts` — `inbrain remote doctor` adds the
  ...
  `INBRAIN_DOCTOR_SKIP_SCOPE_PROBE=1` for fixtures that mock /mcp at JSON-RPC
