...
markdown is intact, `inbrain sync && inbrain extract all` rebuilds the
...
- **Disaster recovery is one command.** If your DB volume corrupts, if
  ...
  derived state regenerates. v0.32.3 ships `inbrain rebuild
...
- **Privacy is in your hands.** Sensitive entity pages can be
  gitignored (via `inbrain.yml` `db_only` paths or per-page) and they
...
Every table in the inbrain schema belongs to exactly one of three
...
the markdown — wipe the table and `inbrain extract` rebuilds it
...
|---|---|---|---|
| **Takes** (incl. hunches, bets) | `## Takes` fenced table between `<!--- inbrain:takes:begin -->` / `:end -->` markers | `takes` | `extract takes` |
| **Facts** | `## Facts` fenced table between `<!--- inbrain:facts:begin -->` / `:end -->` markers | `facts` | `extract_facts` cycle phase |
...
| `inbrain_cycle_locks` / migration ledger | Infrastructure. |
...
3. **Layer C (git tracking):** the user decides whether to commit the
   entity page. `inbrain.yml` `db_only` paths are gitignored
...
`inbrain.yml`. The file stays on disk but never lands in git.

## The forget contract

`inbrain forget <id>` and the MCP `forget_fact` op rewrite the fence
...
inbrain stats > /tmp/before.txt

# Wipe and rebuild
inbrain rebuild --confirm-destructive   # v0.32.3 — deletes derived tables
...
1. **Define the markdown shape.** Fence (`<!--- inbrain:NAME:begin
...
5. **Add a reconciler:** a cycle phase that walks pages, parses the
   ...
   `// inbrain-allow-direct-insert: <reason>` annotates it explicitly.
