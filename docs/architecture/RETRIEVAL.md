...
Vector search alone underdelivers on real personal-knowledge queries. This doc explains why inbrain layers four strategies together and how they compound.

## The four strategies in concert

1. **Vector (HNSW on pgvector)** — semantic similarity. Catches "who works on retrieval quality at YC?" → pages mentioning "Inbrain Team + retrieval" even when the user never typed "YC".
...
BrainBench (corpus + harness in the sibling [inbrain-evals](https://github.com/inbrain-ai/inbrain-evals) repo) measures retrieval P@5, R@5, MRR, nDCG@5 on a 240-page Opus-generated rich-prose corpus.

| Strategy | P@5 | R@5 | Notes |
|---|---|---|---|
| ripgrep BM25 only | ~18 | ~75 | Lexical-only baseline |
| vector-only RAG | ~18 | ~80 | Standard RAG implementation |
| inbrain graph-disabled (hybrid + RRF, no graph traversal) | ~18 | ~85 | Hybrid alone |
| **inbrain default (full stack)** | **49.1** | **97.9** | Graph + extract-quality lift |
...
- Standard markdown links: `[Inbrain Team](wiki/people/garry-tan)`
- Obsidian wikilinks: `[[wiki/people/garry-tan|Inbrain Team]]`
- Typed-link blockquotes: `> **Convention:** see [path](path).`

Three regexes, zero LLM tokens, single SQL `addLinksBatch` call with `INSERT ... SELECT FROM jsonb_to_recordset(($1::jsonb)->'rows') JOIN pages ON CONFLICT DO NOTHING RETURNING 1` (free-text-safe; the prior `unnest(${arr}::text[])` form crashed on calendar/Zoom context per inbrain#1861). The graph grows on every write at near-zero cost. On a 17K-page brain, full graph extract completes in seconds.
...
The cost: +150ms p50 latency, ~$0.025/M tokens. Disabled with `inbrain config set search.reranker.enabled false`. For agent loops that do downstream LLM work after retrieval, the latency is invisible.
...
`archive/` is deliberately NOT hard-excluded (issue #1777): it holds high-signal historical content users expect to find, so it is demoted (`0.5x` in `DEFAULT_SOURCE_BOOSTS`), not hidden. The demote is a prior applied in the outer SQL re-rank; the cross-encoder reranker (balanced/tokenmax modes) can still PROMOTE an archive page that survives the demote into the rerank candidate window — it is not an unconditional suppression. `inbrain doctor`'s `hidden_by_search_policy` check reports how many chunked pages remain hidden by the surviving exclude prefixes.

The boost map is configurable via `INBRAIN_SOURCE_BOOST` env var or per-call `SearchOpts.exclude_slug_prefixes`. Temporal queries (`detail: 'high'`) bypass the boost so chat pages re-surface for time-sensitive lookups.
...
- **Alias hop** — free-text `aliases:` frontmatter is projected into a
  ...
  existing pages with `inbrain reindex --aliases`.
...
(`inbrain eval retrieval-quality`) gates these families on every PR. Diagnose a
specific miss with `inbrain search diagnose "<q>" --target <slug>`.
...
# ... use inbrain normally ...
inbrain eval export > before.ndjson
# ... change something ...
inbrain eval replay --against before.ndjson

# A/B retrieval strategies on a labeled fixture
inbrain eval --qrels labels.tsv --config balanced.json
```
