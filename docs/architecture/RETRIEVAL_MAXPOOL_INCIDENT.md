...
**Original author:** Inbrain Team's OpenClaw. **Severity at the time:** High.
...
1. **`inbrain search` was keyword-only**, not hybrid — so the RFC's cosine scores
   (0.64/0.98) came from the hybrid `query`/MCP path the agent actually hit, not
   `inbrain search`. The repro command in the RFC was mislabeled.
...
Plus: `inbrain search "<text>"` is now cheap-hybrid (the obvious verb gives the
...
inbrain search diagnose "Greek amphitheater" --target projects/new-greek-theater/concept_v0

# Backfill aliases for existing pages whose frontmatter predates the alias layer:
inbrain reindex --aliases

# Watch retrieval quality over time (a downward avg rank-1 score = regressing):
inbrain search stats --days 30

# The gate that prevents silent reintroduction:
inbrain eval retrieval-quality test/fixtures/retrieval-quality/namedthing.jsonl
```
