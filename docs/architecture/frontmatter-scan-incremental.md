...
v0.38.2.0 fixed the load-bearing bug class that caused `inbrain doctor` to
...
2. **Incremental scan** (`inbrain frontmatter scan --incremental`). Walks
...
1. **First-ever scan.** A fresh upgrade has no rows in
   ...
   - Lazy: doctor reports "no scan state yet; run `inbrain frontmatter scan
...
3. **Path renames inside a source.** Sync would `DELETE` the old row by
   ...
   - Or: doctor reports "N stale rows in frontmatter_scan_state" as a
     freshness signal, with `inbrain frontmatter scan --reconcile` as the
     remediation.
