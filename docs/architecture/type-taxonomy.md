# Type Taxonomy (v0.41.22: inbrain-base-v2)

> The 14-canonical-type DRY/MECE taxonomy shipped in v0.41.22. Predecessor
> `inbrain-base` (24 types) stays bundled for back-compat; v0.42+ installs
> default to `inbrain-base-v2`.

## Why

A production inbrain brain (186K pages) had accreted **94 distinct
...
```
inbrain onboard --check                         # surfaces pack_upgrade_available
        ↓
inbrain onboard --check --explain               # per-cluster narrative dry-run
        ↓
inbrain jobs submit unify-types \               # PROTECTED + manual_only
  --allow-protected \
  --params '{"target_pack":"inbrain-base-v2"}'
        ↓
Handler runs 4 phases:
  ┌─────────────────────────────────────┐
  │ Phase 1: Preflight + lock           │ → inbrain-unify db-lock (60min TTL)
  ├─────────────────────────────────────┤
  ...
  └─────────────────────────────────────┘
        ↓
inbrain onboard --check                         # pack_upgrade_available cleared
...
| Page-to-link | Source page soft-deleted with 72h TTL. `inbrain pages restore <slug>` within 72h. Link row stays harmless if source restored. |
| Page-to-alias | Source page soft-deleted with 72h TTL. `inbrain pages restore <slug>` within 72h. Alias row stays harmless (or `DELETE FROM slug_aliases WHERE alias_slug = <slug>` to clean up). |
| Active-pack flip | `inbrain schema use inbrain-base` reverses the flip. |
...
1. Fork inbrain-base-v2: `inbrain schema fork inbrain-base-v2 my-pack`
2. Edit your fork to add page_types + mapping_rules covering your
   custom domain.
3. Target your fork: `inbrain jobs submit unify-types --allow-protected
   --params '{"target_pack":"my-pack"}'`

Your fork can also declare `migration_from: {pack: inbrain-base-v2,
