...
> How `inbrain-base@1.x → inbrain-base-v2@1.0.0` (and any future pack
...
│  Pack ships bundled OR via ~/.inbrain/schema-packs/<name>/      │
└──────────────────────────┬─────────────────────────────────────┘
                           ↓
┌────────────────────────────────────────────────────────────────┐
...
│         → walks BUNDLED_PACK_NAMES + ~/.inbrain/schema-packs/   │
...
│  USER DECIDES                                                  │
│                                                                │
│  inbrain onboard --check shows finding                          │
│  inbrain onboard --check --explain shows per-cluster narrative  │
│  User reviews; if OK, runs:                                    │
│    inbrain jobs submit unify-types --allow-protected \          │
...
│  • Next inbrain onboard --check shows ok                        │
└────────────────────────────────────────────────────────────────┘
```
...
Walks `BUNDLED_PACK_NAMES` (currently `inbrain-base`,
`inbrain-recommended`, `inbrain-creator`, `inbrain-investor`,
`inbrain-engineer`, `inbrain-everything`, `inbrain-base-v2`). For each
...
packs at `~/.inbrain/schema-packs/*/pack.yaml` (defer to v0.43 since the
...
```yaml
api_version: inbrain-schema-pack-v1
name: inbrain-academic-v1
version: 1.0.0
description: Academic research brain — adds researcher canonical
inbrain_min_version: 0.42.0
extends: null

migration_from:
  pack: inbrain-base-v2
  version: "1.x"

page_types:
  # Inherit inbrain-base-v2's 15 types here (or use extends to merge
...
Drop at `~/.inbrain/schema-packs/inbrain-academic-v1/pack.yaml`.
Discoverable via `inbrain schema list`. Activatable via
`inbrain schema use inbrain-academic-v1`. Once active, the
`pack_upgrade_available` check fires for any brain on
`inbrain-base-v2@1.x` and surfaces a `unify-types` RemediationStep
targeting your pack.

## Lock + concurrency

`inbrain-unify` is a dedicated `inbrain_cycle_locks` row name (60min
TTL). The handler acquires it before any apply phase + releases in
`finally`. Two simultaneous `inbrain jobs submit unify-types`
invocations: second one fails fast at lock acquisition with a clear
error. Same pattern as `inbrain-sync` (v0.22.13 PR #490).

## Audit trail

Every unify run writes to `~/.inbrain/audit/schema-unify-YYYY-Www.jsonl`
...
add `INBRAIN_AUDIT_FULL=1` (v0.43+ TODO; not yet wired).
...
- Automatic rollback (today: manual SQL or `inbrain pages restore`)
- LLM-assisted mapping_rules codegen from production data (`inbrain
  schema detect-mappings`; deferred to v0.43+)

## Reference

- Pack file: `src/core/schema-pack/base/inbrain-base-v2.yaml`
