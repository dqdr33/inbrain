...
Inbrain uses Reciprocal Rank Fusion (RRF) to merge vector and keyword search:
...
Inbrain is the deterministic layer. Skills and recipes are the latent space layer.

See [Thin Harness, Fat Skills](../ethos/THIN_HARNESS_FAT_SKILLS.md) for the full
architecture philosophy.

- **Inbrain CLI** = thin harness (same input → same output)
...
The agent reads the skill/recipe and uses Inbrain's deterministic tools to do the work.
