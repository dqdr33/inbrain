# Inbrain v0: Postgres-Native Personal Knowledge Brain
...
Inbrain is a compiled intelligence system. Not a note-taking app. Not "chat with your notes."
...
There's already a production-grade RAG system (Ruby on Rails, Postgres + pgvector) with 3-tier chunking, hybrid search with RRF, multi-query expansion, and 4-layer dedup. Inbrain ports these proven patterns to a standalone Bun + TypeScript tool.
...
**Library-first distribution.** inbrain is an npm package. OpenClaw installs it as a dependency (`bun add inbrain`), imports the engine directly. Zero-overhead function calls, shared connection pool, TypeScript types. The CLI and MCP server are thin wrappers over the same engine.
...
**Auto-embed during import.** No separate embed step. `inbrain import` chunks and embeds in one pass. Progress bar shows status. `--no-embed` flag for users who want to defer. `embedded_at` column enables `inbrain embed --stale` for backfill.
...
+-------------------+     +-------------------+     +-------------------+
|                   |     |                   |     |                   |
| bun add inbrain    |     | GitHub Releases   |     | inbrain serve      |
| import { Postgres |     | npx inbrain        |     | in mcp.json       |
|   Engine }        |     |                   |     |                   |
|                   |     |                   |     |                   |
...
OpenClaw is the AI orchestrator that uses inbrain as its knowledge backend. This is the most common install path.

```bash
# 1. Install inbrain as a ClawHub skill
clawhub install inbrain
...
Behind the scenes, `clawhub install inbrain`:
1. Installs the `inbrain` npm package
2. Ships SKILL.md files (ingest, query, maintain, enrich, briefing, migrate)
3. Registers brain tools with the orchestrator
4. Runs `inbrain init --supabase` on first use (guided wizard)

### Path 2: CLI user (standalone)

```bash
# 1. Install
npm install -g inbrain
# or: download binary from GitHub Releases

# 2. Initialize with Supabase
inbrain init --supabase
# Guided wizard:
...
#   Output: "Brain ready. Run: inbrain import <your-repo>"

# 3. Import your data
inbrain import /path/to/markdown/wiki/
# Progress bar: 7,471 files, auto-chunk, auto-embed
# ~30s for text import, ~10-15 min for embedding

# 4. Query
inbrain query "what does PG say about doing things that don't scale?"
```
...
`inbrain init --supabase` runs through these steps:
...
Step 3: Config
  ├── Write ~/.inbrain/config.json (0600 permissions)
...
Step 5: First Query
  └── "Try: inbrain query 'what does PG say about doing things that don't scale?'"
```
...
```
inbrain init [--supabase|--url <conn>]     # create brain
inbrain get <slug>                          # read a page
inbrain put <slug> [< file.md]             # write/update a page
inbrain search <query>                      # keyword search (tsvector)
inbrain query <question>                    # hybrid search (RRF + expansion)
inbrain ingest <file> [--type ...]         # ingest a source document
inbrain link <from> <to> [--type <type>]   # create typed link
inbrain unlink <from> <to>                 # remove link
inbrain graph <slug> [--depth 5]           # traverse link graph (recursive CTE)
inbrain backlinks <slug>                    # incoming links
inbrain tags <slug>                         # list tags
inbrain tag <slug> <tag>                    # add tag
inbrain untag <slug> <tag>                  # remove tag
inbrain timeline [<slug>]                   # view timeline
inbrain timeline-add <slug> <date> <text>  # add timeline entry
inbrain list [--type] [--tag] [--limit]    # list with filters
inbrain stats                               # brain statistics
inbrain health                              # brain health dashboard
inbrain import <dir> [--no-embed]          # import from markdown directory
inbrain export [--dir ./export/]           # export to markdown (round-trip)
inbrain embed [<slug>|--all|--stale]       # generate/refresh embeddings
inbrain serve                               # MCP server (stdio)
inbrain call <tool> '<json>'               # raw tool invocation
inbrain upgrade                             # self-update (npm, binary, ClawHub)
inbrain version                             # version info
inbrain config [get|set] <key> [value]     # brain config
```
...
2. **Smart slug resolution.** Fuzzy matching via pg_trgm for reads. Writes require exact slugs. `inbrain get "dont scale"` resolves to `concepts/do-things-that-dont-scale`.
3. **Brain health dashboard.** `inbrain health` shows page count, embed coverage, stale pages, orphans, dead links.
4. **Normalized timeline.** `timeline_entries` table only (no TEXT column). `detail` field supports markdown.
5. **Page version control.** `page_versions` table stores full snapshots (compiled_truth + frontmatter + links + tags). `inbrain history`, `inbrain diff`, `inbrain revert` commands. Revert re-chunks and re-embeds.
6. **Typed links + graph traversal.** `link_type` column (knows, invested_in, works_at, etc.). `inbrain graph` uses recursive CTE with max depth (default 5, configurable via `--depth`).
...
- Supabase service role key in `~/.inbrain/config.json` (0600 permissions)
- MCP stdio transport is inherently local (client spawns `inbrain serve` as subprocess)
- No multi-user, no RLS, no OAuth in v0
- Multi-user path (future): Supabase RLS + per-user API keys

## Upgrade mechanism

`inbrain upgrade` detects the installation method and updates accordingly:

| Path | How |
|------|-----|
| npm | `bun update inbrain` (or npm equivalent) |
| Compiled binary | Download new binary to temp dir, atomic rename swap, exec new process |
| ClawHub | `clawhub update inbrain` |
...
Budget alternative: `inbrain import --chunker recursive` skips sentence-level embeddings, then `inbrain embed --rechunk --chunker semantic` upgrades later.
...
1. `inbrain import /data/brain/` migrates all 7,471 files losslessly
2. `inbrain export` round-trips to semantically identical markdown
3. `inbrain query "what does PG say about doing things that don't scale?"` returns relevant hybrid search results
4. `inbrain serve` starts MCP server connectable by Claude Code
5. All 3 chunkers produce correct output with test fixtures
6. `inbrain init --supabase` works end-to-end
7. `bun test` passes all tests
8. `clawhub install inbrain` installs the skill and runs guided setup
9. `bun add inbrain` + `import { PostgresEngine } from 'inbrain'` works in external project
10. Drift tests pass: CLI and MCP produce identical results
11. `inbrain health` outputs accurate brain health metrics
...
- **`inbrain ask` natural language CLI alias.** Trivial to add. P1 TODO.
