# Migrating your OpenClaw brain to inbrain v0.41.2.0 (greenfield)
...
through the new ingestion cathedral. Pages land in inbrain with an
...
1. **Upgrade inbrain to v0.41.2.0+:**
   ```bash
   inbrain upgrade
   inbrain --version  # should be 0.41.2.0 or later
   ```

2. **Activate the creator pack** (or inbrain-everything if you also
   want investor + engineer lenses on the same brain):
   ```bash
   inbrain config set schema_pack inbrain-creator
   # OR
   inbrain config set schema_pack inbrain-everything
   ```
...
```bash
inbrain capture --source markdown-greenfield \
  --repo ~/git/brain \
  --dry-run \
  --limit 100
```
...
- `skipped_invalid` — files that failed validation (these append to
  `~/.inbrain/audit/markdown-greenfield-failures-YYYY-Www.jsonl`)

Inspect the audit JSONL:
```bash
ls ~/.inbrain/audit/markdown-greenfield-failures-*.jsonl
cat ~/.inbrain/audit/markdown-greenfield-failures-*.jsonl | jq .
```
...
```bash
inbrain capture --source markdown-greenfield --repo ~/git/brain
```
...
The next `inbrain dream` cycle will:
...
Replace with nothing — inbrain's autopilot already runs extract_atoms +
synthesize_concepts inside every dream cycle when inbrain-creator (or
inbrain-everything) is the active pack.

The OpenClaw skills themselves shrink to thin wrappers:

- `content-atom-extractor` → calls `inbrain dream --phase extract_atoms`
- `concept-synthesis` → calls `inbrain dream --phase synthesize_concepts`
...
inbrain query "imported_from:markdown-greenfield" --type atom --json | \
  jq -r '.[].slug' | xargs -I{} inbrain pages delete {}
# OR hard-delete past the soft-delete window
inbrain pages purge-deleted --older-than 0h
```
