...
v0.12.2 adds detection in `inbrain doctor` and a standalone `inbrain repair-jsonb`
...
migration runs this automatically on `inbrain upgrade`.
...
inbrain sync --force
# or per-page
inbrain import <slug> --force
```

v0.12.2 cannot recover content that was already lost if you no longer have
the source markdown file. `inbrain doctor` tells you which pages look short;
