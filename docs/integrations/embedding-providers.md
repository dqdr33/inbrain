...
Inbrain ships with 16 embedding-provider recipes covering OpenAI, ZeroEntropy, Voyage, OpenRouter (single key, many hosted models), the major hosted alternatives, three local options, and a universal escape hatch (LiteLLM proxy). Run `inbrain providers list` to see the live registry; `inbrain providers explain --json` emits a machine-readable matrix for agents.
...
```
inbrain providers list                          # see all providers
inbrain providers env <provider-id>             # see required env vars
inbrain providers test --model openai:text-embedding-3-large   # smoke-test
inbrain init --pglite --model voyage            # use a non-default provider
```

## Init resolves your provider from env keys

As of v0.37, `inbrain init --pglite` auto-detects which provider to use from your env vars. With `OPENAI_API_KEY` set, you get OpenAI. With `ZEROENTROPY_API_KEY` set, you get ZeroEntropy. If multiple provider keys are set, init fires an interactive picker. If no provider keys are set in a non-TTY context (CI, Docker build), init exits 1 with a paste-ready setup hint. Explicit flags (`--embedding-model`, `--no-embedding`) always win over env detection.

The resolved provider + dimensions get persisted to `~/.inbrain/config.json` atomically, so subsequent runs are deterministic across releases.
...
If `inbrain import` fails with `expected N dimensions, not M`, run `inbrain doctor`. The output will print the exact `inbrain config set ...` or `inbrain retrieval-upgrade` command to repair the mismatch. **You should not need to delete `~/.inbrain`.** The bug-class that historically forced `rm -rf` recoveries is closed as of v0.37.

The doctor distinguishes two repair paths:

- **Empty brain** (no embedded chunks yet) — drop and re-init at the right dim:
  ```
  inbrain init --force --pglite --embedding-model <provider>:<model> --embedding-dimensions <N>
  ```

- **Non-empty brain** — migrate cleanly with the supported reindex path:
  ```
  inbrain retrieval-upgrade --to <provider>:<model> --reindex
  ```
...
- **Anything else**: LiteLLM proxy. Run LiteLLM in front of any provider (Bedrock, Vertex, Cohere, Jina, Fireworks, etc.) and point inbrain at it via `LITELLM_BASE_URL`.

## Per-provider details

### OpenAI

Default. Set `OPENAI_API_KEY`. Models: `text-embedding-3-large` (3072 max, 1536 default), `text-embedding-3-small` (1536). Matryoshka via the `dimensions` field — inbrain pins it from `embedding_dimensions` config so existing 1536-dim brains stay aligned across SDK upgrades.
...
To switch an existing brain, use `inbrain reinit-pglite --embedding-model voyage:voyage-code-3 --embedding-dimensions 1024` (PGLite) or follow `docs/embedding-migrations.md` (Postgres). `inbrain config set embedding_model` is refused — the schema column has to resize.

`inbrain reindex --code` will print a recommendation when run against a brain whose configured embedding model isn't code-tuned; suppress with `INBRAIN_NO_CODE_MODEL_NUDGE=1` if you've intentionally chosen another model (single-vendor procurement, compliance, etc.).
...
- `OPENROUTER_REFERER` (default `https://inbrain.ai`) and `OPENROUTER_TITLE` (default `inbrain`) — attribution headers for OR's leaderboard. Forks running inbrain inside a different agent stack (OpenClaw deployments etc.) should set these so their traffic gets attributed to them, not inbrain.

**Subagent loops**: inbrain's subagent infrastructure hard-pins to Anthropic-direct (stable `tool_use_id` across crashes/replays). OR-routed Anthropic is rejected at submit time regardless of the recipe flag. If you want the price/availability story OR offers for tool-calling, use it for chat only and keep an Anthropic key for subagent work.
...
Unlike vanilla OpenAI, Azure uses `api-key:` header (not `Authorization: Bearer`) and a templated URL with `?api-version=` query param — inbrain handles both via the recipe's resolveAuth + resolveOpenAICompatConfig overrides.
...
CJK-dominant content tokenizes denser than OpenAI tiktoken; inbrain declares `chars_per_token: 2` so the batch pre-split leaves headroom.
...
Recipe ships with `nomic-embed-text` (768d, recommended), `mxbai-embed-large` (1024d), `all-minilm` (384d). `inbrain providers test --model ollama:nomic-embed-text` smoke-tests the local install.
...
User-driven models: launch llama-server with `--model <gguf-path> --embeddings`, then run `inbrain init --embedding-model llama-server:<your-id> --embedding-dimensions <N>`. The recipe refuses the implicit shorthand `--model llama-server` because there's no canonical first model.

### LiteLLM proxy (universal escape hatch)

Run [LiteLLM](https://docs.litellm.ai/docs/proxy/quick_start) in front of any provider — Bedrock, Vertex, Cohere, Jina, Fireworks, OctoAI, etc. The proxy normalizes everything to the OpenAI-compatible API; inbrain points at the proxy via `LITELLM_BASE_URL` and proxies the call.

This is the catch-all for "my provider isn't in the list above." Set up LiteLLM, then `inbrain init --embedding-model litellm:<your-model-id> --embedding-dimensions <N>`.
...
3. **HNSW cap**: pgvector's HNSW index supports up to 2000 dims. Brains above that fall back to exact vector scans (slower but correct; inbrain handles the SQL automatically via `chunkEmbeddingIndexSql` in `src/core/vector-index.ts`).
...
3. **Open a feature request** at [github.com/inbrain-ai/inbrain/issues](https://github.com/inbrain-ai/inbrain/issues) with the provider's API docs URL and a setup snippet. Recipes are ~30-40 lines of TypeScript.
...
Embedding dimensions are baked into the schema at `inbrain init` time. As of v0.37.11.0, `inbrain config set embedding_model` and `inbrain config set embedding_dimensions` are refused — the schema column has to resize alongside the config, and `config set` only touches the config row.

The supported paths:

- **PGLite (default install):** `inbrain reinit-pglite --embedding-model <provider>:<model> --embedding-dimensions <N>` — one-command wipe-and-reinit that preserves every other config field (chat model, expansion model, API keys), backs up the prior brain to `<path>.bak`, runs `inbrain init` with the new flags, and re-syncs your brain repo. Add `--no-sync` to skip the resync, `--yes` to skip the TTY confirmation, `--json` for scripts.
- **Postgres (Supabase / self-hosted):** follow the SQL recipe in `docs/embedding-migrations.md` (drop the HNSW index, ALTER COLUMN TYPE, clear stale embeddings, recreate the index conditionally, then `inbrain init --supabase --embedding-model X --embedding-dimensions N` to update the file plane and re-embed).

`inbrain doctor` 8c "alternative_providers" surfaces unconfigured providers whose env is already set — useful when you've configured OpenAI but also have e.g. `VOYAGE_API_KEY` exported and want to know you can switch without extra setup.
