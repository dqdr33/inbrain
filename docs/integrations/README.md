...
Inbrain is the retrieval layer. But retrieval is only as good as what you put in.
...
Brain pages created/updated (compiled truth + timeline)
  ↓
Inbrain indexes it (chunking, embedding, search-ready)
  ↓
...
`inbrain integrations` to see what's available and their status.
...
Only recipes shipped inside the inbrain package itself (the `recipes/` directory in
a source install, or the global install copy) are trusted. Recipes discovered at
runtime from `$INBRAIN_RECIPES_DIR` or a cwd-local `./recipes/` are marked untrusted:
