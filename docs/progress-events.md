...
Canonical reference for the JSONL progress stream that `inbrain` writes to
...
- `inbrain doctor` (DB checks, JSONB integrity, markdown body completeness,
  integrity sample)
- `inbrain orphans`
- `inbrain embed`
- `inbrain files sync`
- `inbrain export`
- `inbrain extract [links|timeline|all]` (fs or db source)
- `inbrain import`
- `inbrain sync`
- `inbrain migrate --to …`
- `inbrain repair-jsonb`
- `inbrain check-backlinks`
- `inbrain lint`
- `inbrain integrity auto`
- `inbrain eval`
- `inbrain apply-migrations` (the orchestrator + every child command)
...
so `inbrain --progress-json doctor` works the same as
`inbrain doctor --progress-json` (the latter also works — per-command
...
When a parent CLI spawns `inbrain …` child processes (mostly in
...
`inbrain jobs work` (the Minion worker daemon) keeps progress in the DB,
...
`get_job_progress` MCP operation or `inbrain jobs get <id>`.
