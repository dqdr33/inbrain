...
If you want to know how a mode behaves on YOUR brain, run `inbrain search stats --days 30` after a real usage window, then run `inbrain search tune` for actionable recommendations.
...
- **Replay captures** — NDJSON from the sibling `inbrain-evals` repo, `n=200` queries. Each query carries a `retrieved_slugs` baseline + a `latency_ms` measurement from the original production run.
- **BrainBench v1** — `n=1240` documents / `n=350` qrels (binary relevance judgments). Lives in the sibling [`inbrain-evals`](https://github.com/inbrain-ai/inbrain-evals) repo, SHA-pinned at every run.

No private brain content is used in any reported result. The committed NDJSON dumps under `<repo>/.inbrain-evals/` contain only the LongMemEval question IDs + the rank-ordered retrieved session IDs.

## 3. Sample selection

- **Random seed:** `42` throughout. Set via `--seed N` on `inbrain eval run-all`; recorded in every per-run record.
...
This document + the eval results are regenerated on every release that touches retrieval-affecting code. The `inbrain doctor eval_drift` check surfaces changes to the curated watch-list in `src/core/eval/drift-watch.ts`:
...
When `inbrain eval compare --md` reports a Δ between two modes, it computes:
...
The mode-picker prompt at `inbrain init` and the CLAUDE.md `## Search Mode` table both surface these rough cost anchors. Working through the math so they're auditable:
...
- Your agent's system prompt + reasoning tokens add input that inbrain doesn't see.
...
A "turn" here is one agent loop iteration: read user message, plan, execute tool calls (including inbrain searches), generate response. Each turn typically includes 2-4 inbrain searches.

**Per-mode scaling from the tokenmax anchor:**

The cost difference between modes is concentrated in the search-attributable fraction of per-turn cost. System prompt, tool definitions, conversation history, and reasoning tokens don't change with mode — only the chunks inbrain delivers do. Assume 3 searches per turn at the mode's `searchLimit`:
...
+ history + reasoning + search), not just inbrain's chunks. Mode choice
...
This anchor + the per-query math both live in this doc on purpose. The per-query framing is what an isolated benchmark would measure (and what `inbrain eval run-all` will produce). The realistic-scale anchor is what an operator actually pays. Both are honest; neither is the whole truth.
