...
You'll fork the bundled `inbrain-base` pack, add a custom `researcher` page type, import a handful of placeholder researcher pages, backfill their `page.type` column with one command, then prove the wiring works by running `inbrain whoknows` and seeing your new type surface in results. End state: a forked-and-active pack on disk, ~5 pages typed as `researcher`, and a query that proves the pack-aware routing fires end-to-end.
...
- inbrain v0.40.7.0 or later (`inbrain --version` to check)
- A brain that's been initialized (`inbrain init` already run; either PGLite or Postgres is fine)
...
Output: `Forked 'inbrain-base' → 'mine' at ~/.inbrain/schema-packs/mine/pack.json`.

The fork is a byte-for-byte copy of `inbrain-base` living at `~/.inbrain/schema-packs/mine/pack.json`. Now you have a writable pack you can mutate.
...
Run `inbrain schema active --json` again to confirm `pack_name` is now `mine` and `source_tier` is `home-config` (read from `~/.inbrain/config.json`).
...
```bash
inbrain schema add-type researcher \
  --primitive entity \
...
- An audit row landed in `~/.inbrain/audit/schema-mutations-YYYY-Www.jsonl` with your type name SHA-8-redacted and the prefix's first segment only (`people`) for privacy.

Verify the type is in the pack:

```bash
inbrain schema explain researcher
```
...
```bash
inbrain schema stats --json | jq '.aggregate, .dead_prefixes'
```
...
```bash
inbrain whoknows "machine learning"
```
...
You now have:
- A fork of `inbrain-base` named `mine` at `~/.inbrain/schema-packs/mine/pack.json`, active in your brain via `~/.inbrain/config.json`.
...
- 3 pages typed as `researcher` (backfilled from disk via `inbrain schema sync --apply`).
- A query path that routes through the new type: `inbrain whoknows` reads the pack and includes `researcher` in its type filter.
...
inbrain schema graph
```

The graph now shows `researcher --(authored)--> paper`.

**Add aliases for query closure.** If you want `inbrain query researcher` to also surface `person` rows (because researchers ARE people):

```bash
inbrain schema add-alias researcher person
```
...
```bash
inbrain schema lint --with-db
```

**Commit your pack to source control.** If `~/.inbrain/schema-packs/mine/` is a git repo, commit `pack.json` and push. Your pack survives across machines, and the `mutation_count_anomaly` lint rule will nudge you when you hit >50 mutations in a week (the "you should be committing this" signal).
...
**Undo a mistake.** Every mutation primitive has an inverse (`remove-type`, `remove-alias`, `remove-prefix`, `remove-link-type`, `set-extractable false`, etc.). If you fork twice and want to revert, `inbrain schema downgrade` restores the previous active pack from `~/.inbrain/schema-pack-history.jsonl`.

## Related docs

- **Reference:** `inbrain schema --help` for the full 22-verb CLI surface; CLAUDE.md's "Schema Cathedral v3 (v0.40.7.0)" section for the module-by-module architecture.
