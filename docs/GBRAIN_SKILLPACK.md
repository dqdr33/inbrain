...
<!-- source: https://raw.githubusercontent.com/inbrain-ai/inbrain/master/docs/INBRAIN_SKILLPACK.md -->
# Inbrain Skillpack: Reference Architecture for AI Agents

This is a reference architecture for how a production AI agent uses inbrain as its
...
stores everything, mechanized so it may be consulted with exceeding speed. Inbrain is
...
`skills/conventions/subagent-routing.md` — it reads `~/.inbrain/preferences.json#minion_mode`
...
auto-rewrites entries whose handler is a inbrain builtin; host-specific handlers (e.g.
...
| [Brain vs Agent Memory](guides/brain-vs-memory.md) | 3 layers: Inbrain (world knowledge), agent memory, session |
...
## Appendix: Inbrain CLI Quick Reference

| Command | Purpose |
|---------|---------|
| `inbrain search "term"` | Keyword search across all brain pages |
| `inbrain query "question"` | Hybrid search (vector + keyword + RRF) |
| `inbrain get <slug>` | Read a specific brain page by slug |
| `inbrain sync` | Sync local markdown repo to inbrain index |
| `inbrain import <path>` | Import files into the brain |
| `inbrain embed --stale` | Re-embed pages with stale or missing embeddings |
| `inbrain integrations` | Manage integration recipes (senses + reflexes) |
| `inbrain stats` | Show brain statistics (page count, last sync, etc.) |
| `inbrain doctor` | Diagnose brain health issues |
| `inbrain check-update` | Check for new versions and integration recipes |

Run `inbrain --help` for the full command reference.
...
- [Recommended Schema](INBRAIN_RECOMMENDED_SCHEMA.md) — Directory structure for your brain repo
- [Verification Runbook](INBRAIN_VERIFY.md) — End-to-end installation verification
