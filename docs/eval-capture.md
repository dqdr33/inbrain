...
[inbrain-evals](https://github.com/inbrain-ai/inbrain-evals) repo) that
...
## `inbrain eval export` — the consumer contract

```sh
inbrain eval export [--since DUR] [--limit N] [--tool query|search]
```
...
Typical usage from inbrain-evals:

```sh
# Snapshot the last week of real traffic for replay
inbrain eval export --since 7d > brainbench-real.ndjson
```

```sh
# Stream through jq for ad-hoc analysis
inbrain eval export --tool query | jq -c 'select(.latency_ms > 500)'
```
...
- **Additive changes** increment inbrain minor version (v0.25.0, v0.23.0
...
Not exported by `inbrain eval export`. Surfaced via `inbrain doctor`:

```sh
inbrain doctor   # warns when failures in last 24h > 0
```

Reason enum (stable): `db_down` | `rls_reject` | `check_violation` |
`scrubber_exception` | `other`. Cross-process visibility is the whole
point — `inbrain doctor` runs in its own process and reads the table
...
```bash
export INBRAIN_CONTRIBUTOR_MODE=1     # in ~/.zshrc or ~/.bashrc
```

**Path B — explicit config (`~/.inbrain/config.json`, file-plane only):**
...
`inbrain config set eval.capture false` does **not** work — that
