...
Branch: inbrain-ai/houston-v1 | Mode: EXPANSION
Repo: inbrain-ai/inbrain
...
- **Pack resolution chain (7 tiers)** — extends model-config's
  ...
  `INBRAIN_SCHEMA_PACK` env, (3) per-source `--source <id>` override
  via DB config key `schema_pack:source:<id>`, (4) brain-wide DB
  config key `schema_pack`, (5) `inbrain.yml schema:` section,
  (6) `~/.inbrain/config.json schema_pack`, (7) default `inbrain-base`.
...
- Schema packs distribute as `.inbrain-schema` tarballs through the
  v0.37 skillpack pipeline; skillpacks rename to `.inbrain-skillpack`
  for symmetry. Community schema packs propagate the same way
  community skillpacks do.

### Platonic Ideal
A new user clones inbrain and types `inbrain init`. Within 30 seconds
inbrain has read their existing markdown anywhere on disk, proposed a
...
want to. They can publish their pack as a `.inbrain-schema` tarball
for anyone to install and fork.

The 12-month state: `inbrain init` runs `schema detect` automatically,
...
| D3 | `inbrain schema suggest` (LLM-powered) | ~3-5 days | ACCEPTED | Closes the gap from "what exists" to "what your brain implies"; bounded cost via sampling |
...
- **Engine layer:** inbrain-base universal starter pack; 5 composable
...
- **Detect layer:** `inbrain schema detect` SQL-driven heuristic
  clustering proposing a pack manifest matching brain shape.
- **Suggest layer:** `inbrain schema suggest` LLM-powered refinement
  via gateway.chat() over a bounded sample.
- **Inspect layer:** `inbrain schema graph` (ASCII viz),
  `inbrain schema lint` (consistency check), `inbrain schema explain
  <type>` (plain English).
- **Author layer:** `inbrain schema init/use/fork/edit/validate/
...
- **Auto-prompt layer:** TTY-gated interrupt on first unknown-type
  ...
  escape hatch.
- **Distribution layer:** `.inbrain-schema` tarball format; rename
  skillpacks to `.inbrain-skillpack`; v0.37 skillpack pipeline
...
- **inbrain-base:** byte-for-byte reproduces today's hardcoded
...
- **Engine refactor coverage:** the v0.38 plan parameterizes EVERY
  ...
  inbrain-base reproduces today's values for each.
- **Cache + rollback story:**
  ...
  - `eval_candidates` rows (v0.25.0) gain a `schema_pack` column
    so `inbrain eval replay` reproduces the same retrieval space.
  ...
  - Rollback: every `inbrain schema use` operation writes the
    previous pack name to `~/.inbrain/schema-pack-history.jsonl`
    so `inbrain schema use --previous` is one keystroke. Strict-
...
- **Test budget:** ~400-600 cases across unit + e2e per the
  ...
  inbrain-base equivalence regression. **inbrain-base byte-for-byte
  parity is a CI gate**, not a hope — pinned by
  `test/regressions/inbrain-base-equivalence.test.ts` asserting the
...
2. v0.37 skillpack registry currently has zero published packs.
   The `.inbrain-schema` rename and tarball reuse doubles down on a
...
4. `schema suggest` introduces ongoing LLM cost per invocation.
   Bounded by sampling, but sets a precedent for "inbrain commands
   that cost money."
