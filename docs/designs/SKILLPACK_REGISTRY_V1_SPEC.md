...
> |---|---|---|
> | `inbrain skillpack install <name>` | `inbrain skillpack scaffold <source>` | One-time additive copy, no managed block, refuses to overwrite. |
> | `inbrain skillpack uninstall <name>` | (gone) | User owns files; deletes via `rm` or git. |
...
> | Multi-source resolver receipt | Per-scaffold state in `~/.inbrain/skillpack-state.json` | Codex G1 was the right call; v0.36 retired the resolver-block anyway. |
> | Auto-rename collision | Refuses-to-overwrite (v0.36's contract) | Codex was right; v0.36 already enforces it. |
> | Update path | `inbrain skillpack reference <name> [--apply-clean-hunks]` | Diff-lens with optional auto-merge of clean hunks. |
>
> What stays verbatim: registry at `inbrain-ai/inbrain-skillpack-registry`, rubric,
...
After v0.36.0.0 (Hindsight Calibration), inbrain has a mature skillpack system —
but it only knows how to install the **one** bundle declared in
`openclaw.plugin.json` at inbrain's own repo root. Garry wants to ship a
"hackathon-evaluation" skillpack as a standalone artifact, any inbrain user
...
3. **Install** — extend `inbrain skillpack install` to accept a third-party
...
4. **Discover + curate** — a canonical catalog at
   `github.com/inbrain-ai/inbrain-skillpack-registry` (the Printing Press
...
5. **Cross-distribute** — list in `mvanhorn/printing-press-library` as a
   ...
   wrapper around inbrain's HTTP MCP so non-inbrain agents can hit a inbrain
   instance remotely. Doubles the discovery surface.

The intended outcome is: `inbrain skillpack install hackathon-evaluation`
...
- inbrain skillpacks have a **runtime contract** (they assume `inbrain query`,
  `put_page`, sources, takes, hindsight calibration). They are not portable
  to a generic agent harness without inbrain installed. That's what justifies
  a dedicated registry instead of mirroring agentskills.io — the validation
  surface is inbrain-shaped.
...
- `installer.ts:259-293` — cumulative-slugs receipt embedded in the managed
  block (`<!-- inbrain:skillpack:manifest cumulative-slugs="..." version="..." -->`).
  Single source of truth for "what's installed from inbrain."
...
- `inbrain skillpack install <owner>/<repo>` — clones via existing
  `git-remote.ts` SSRF-hardened path.
- `inbrain skillpack install <url.git>` — verbatim https URL.
- `inbrain skillpack install <path/to/pack.tgz>` — extract to cache dir,
...
- `inbrain skillpack install <path/to/repo>` — local path (skill authors
  testing).
- `inbrain skillpack pack [--out <path>]` — publisher-side command that
...
### Registry: github.com/inbrain-ai/inbrain-skillpack-registry
...
- `registry.json` — the live catalog. Schema:
  ...
  ```json
  {
    "schema_version": "inbrain-registry-v1",
    ...
    "skillpacks": [
      {
        ...
        "author": "Inbrain Team",
        "author_handle": "inbrain-ai",
        "homepage": "https://github.com/inbrain-ai/skillpack-hackathon-evaluation",
        "source": {
          "kind": "git",
          "url": "https://github.com/inbrain-ai/skillpack-hackathon-evaluation.git",
          "pinned_commit": "abc1234567890..."
        },
        "tarball_sha256": "deadbeef...",
        "inbrain_min_version": "0.36.0",
        "tier": "endorsed",
...
- `endorsed` — Garry has used it, it works, it's in his pinned set.
  Listed first in `inbrain skillpack search` output. Manual promotion.
...
```
inbrain skillpack search <query> [--tier endorsed|community|experimental] [--json]
inbrain skillpack info <name>             # from registry, not from local install
inbrain skillpack install <name>          # resolves through registry by short name
inbrain skillpack install <url|tarball>   # still works for direct installs
inbrain skillpack install starter-pack    # special bundle entry in registry.json
inbrain skillpack registry [--url <url>]  # show/set the configured registry
```

`--url` defaults to `https://raw.githubusercontent.com/inbrain-ai/inbrain-skillpack-registry/main/registry.json`.
Operators inside corp networks can point at a fork. The registry URL is
recorded in `~/.inbrain/config.json` under `skillpack.registry_url`.

`inbrain skillpack install starter-pack` resolves a special `bundles` array
...
`inbrain skillpack install <name>` on first install of a given source
...
[skillpack] About to install:
  Name:          hackathon-evaluation
  Author:        inbrain-ai
  Source:        https://github.com/inbrain-ai/skillpack-hackathon-evaluation
...
Retry the failed pack with: inbrain skillpack install resume-roaster
```
...
```
inbrain skillpack endorse <name> [--tier endorsed|community|experimental]
                                [--push] [--dry-run]
```

Run from a clone of `inbrain-ai/inbrain-skillpack-registry`. Steps:
...
1. **Local validation** (`inbrain skillpack pack --dry-run`):
   ...
   - `inbrain check-resolvable` clean
   - `inbrain routing-eval` clean (structural layer)
2. **Security gates** (publish-gate v1, see decision Q3):
   ...
   - Trial install: extract pack into a tempdir, run `inbrain skillpack
     install <tempdir>` against an ephemeral PGLite-backed inbrain (mirrors
     the `test/e2e/longmemeval` ephemeral-PGLite pattern at
     `src/eval/longmemeval/harness.ts`), assert `inbrain check-resolvable`
     stays clean and the skill rows appear in the managed block.
   - Trial install runs with `INBRAIN_SKILLPACK_SANDBOX=1`, which disables
...
3. **Test + eval suite execution** (DX-review decision: run everything
   ...
   - **Routing evals**: structural matching via the existing
     `inbrain routing-eval` command. Asserts every `intent` in
   ...
   - **Failed eval / test surfaces actionable line**: each failure
     ...
     `inbrain routing-eval skills/<name>/routing-eval.jsonl`).
3. **Tarball + hash**:
   - `inbrain skillpack pack --out skillpack-<name>-<version>.tgz`
   - SHA-256 recorded for registry pin
4. **Registry PR** (Printing Press pattern verbatim):
   - Fork `inbrain-ai/inbrain-skillpack-registry` if not already forked
   ...
   - Open PR against `inbrain-ai/inbrain-skillpack-registry:main` with the
...
The skill file itself ships in the inbrain skillpack at
`skills/inbrain-skillpack-publish/SKILL.md`. Invokable from any agent
harness that loads inbrain skills.
...
- **Cross-list**: open a PR against `mvanhorn/printing-press-library`
  registering `inbrain-ai/inbrain-skillpack-registry` as a sister-registry
  in their catalog (their library has a `sister_registries:` section per
  their AGENTS.md). Their 1.4k-star audience discovers inbrain through
  the same search surface they already use.
- **Generate**: run `printing-press print` against `inbrain serve --http`'s
  OpenAPI spec (inbrain's HTTP MCP exposes a JSON-RPC surface with stable
  tool definitions). The output is a `inbrain-cli` agent-native binary
  with SQLite mirror that any agent — not just inbrain users — can use to
  hit a remote inbrain. Submitted back to `mvanhorn/printing-press-library`
  as a published CLI, credited to Garry. Doubles distribution surface and
  turns inbrain into something Printing Press users can route through.

The cross-list is ~1 day. The generated CLI is ~1 week and produces a
genuinely new artifact that didn't exist before: inbrain-as-a-service
viewed from outside the inbrain runtime.

### Manifest schema: `skillpack.json` (cathedral artifact)

A inbrain skillpack is a **full software package**, not just markdown.
...
```json
{
  "api_version": "inbrain-skillpack-v1",
  "name": "hackathon-evaluation",
  "version": "0.1.0",
  "description": "Score hackathon submissions with the YC rubric.",
  "author": "Inbrain Team <garry@ycombinator.com>",
  "license": "MIT",
  "homepage": "https://github.com/inbrain-ai/skillpack-hackathon-evaluation",
  "inbrain_min_version": "0.36.0",
...
- `api_version` — forward-compat key; installer refuses unknown.
  Schema is `inbrain-skillpack-v1`. Codex outside-voice gap: single
  ...
  than the local inbrain supports with a paste-ready `inbrain upgrade`
  hint. Refuses silent downgrade.
- `inbrain_min_version` — fail-fast version gate (existing semver helper).
...
- `llm_evals[]` — cross-modal eval configs in the inbrain v0.27.x format
...
The DX review's first cut had `inbrain skillpack install <name>` auto-
walk `runbooks/install.md` after dropping files. Codex pointed out
that this runs trusted-path (`remote=false`) inbrain CLI calls against
...
- `inbrain skillpack install <name>` ALWAYS drops files + updates the
...
Mirrors inbrain's own `skills/migrations/v0.21.0.md` pattern — markdown
that an agent reads top-to-bottom and executes step-by-step.

```markdown
---
runbook_kind: install
inbrain_version_range: ">=0.36.0 <0.37.0"
...
1. **agent:** `inbrain put_page wiki/_skillpack-hackathon-evaluation
...
3. **ask user:** "Want a starter list of evaluation criteria added to
   your brain?"
   - On yes: `inbrain put_page wiki/concepts/yc-rubric < seeds/rubric.md`
   - On no: skip.
```
...
export const SKILLPACK_RUBRIC_V1: RubricDimension[] = [
  {
    ...
    fix_hint: 'Run: inbrain skillpack init <name> to regenerate a valid stub',
    weight: 1,
  },
  {
    id: 2,
    ...
    fix_hint: 'Run: inbrain skillify scaffold <skill-name>',
    weight: 1,
  },
  {
    id: 3,
    ...
    fix_hint: 'inbrain skillify scaffold drops 5 example intents per skill',
    weight: 1,
  },
  {
    id: 4,
    name: 'routing_evals_clean',
    description: 'inbrain routing-eval passes structurally for every routing-eval.jsonl',
  ...
  },
  {
    ...
    description: 'inbrain check-resolvable passes for this pack\'s resolver entries (MECE, no DRY violations, all triggers reach skills). Runs against a PACK-LOCAL fixture, not the ambient workspace.',
  ...
  // `src/core/skillpack/check-resolvable-isolated.ts`.
  {
    ...
    fix_hint: 'inbrain skillify scaffold drops a passing example.test.ts you can extend',
    weight: 1,
  },
  {
    id: 7,
    ...
    fix_hint: 'inbrain skillify scaffold-eval <skill-name>',
    weight: 1,
  },
  {
    id: 8,
    ...
    fix_hint: 'inbrain skillpack init regenerates the stub; edit to taste',
    weight: 1,
  },
  {
    id: 9,
    ...
    fix_hint: 'inbrain skillpack init regenerates the stub',
    weight: 1,
  },
  {
    id: 10,
    ...
    fix_hint: 'Add a `## [<version>] - <YYYY-MM-DD>` entry. Use inbrain skillpack doctor --fix to auto-generate from VERSION + git log.',
    weight: 1,
  },
];
```
...
- `<5` → doctor refuses to score, prints "this isn't a skillpack yet — run `inbrain skillpack init` and try again"

**2. Layered doctor — `inbrain skillpack doctor`**

Two modes; the agent picks which one based on workflow phase:

```
inbrain skillpack doctor <pack-dir|tgz> [--quick|--full] [--fix] [--json]
```
...
- `--full`: equivalent to `inbrain skillpack pack --dry-run` — runs the
...
- `--fix`: auto-scaffold missing pieces. Calls `inbrain skillify
...
```json
{
  ...
  "next_action": "Run: inbrain skillpack doctor --fix to scaffold the 3 missing pieces, then re-run."
}
```
...
- After every meaningful edit during pack development: `inbrain
  skillpack doctor --quick --json`. Target a 10/10 before ever invoking
  `pack --dry-run`.
- Before publishing: `inbrain skillpack doctor --full` to catch what the
  structural pass can't.
- If doctor flags `auto_fixable: true` dimensions, the agent runs
  `inbrain skillpack doctor --fix --yes` and re-runs `--quick`.
...
Cathedral scaffold from `inbrain skillpack init` still drops all 10
...
- `examples/skillpack-reference/` — a real, working **10/10 pack**
  living in the inbrain repo. Doubles as an integration-test fixture for
  ...
  `cd examples/skillpack-reference && inbrain skillpack doctor --quick`
...
**4. Invariant: every inbrain-shipped skillpack scores 10/10**

The bundled inbrain skillpack (today's `openclaw.plugin.json` set, plus
any future bundles like `starter-pack`, `founder-pack`) MUST score
10/10 on `inbrain skillpack doctor --quick`. This is a regression
guard, not a target:

- `scripts/check-bundled-skillpacks-rubric.sh` runs in CI and
  `bun run verify`. Iterates every pack the inbrain repo ships and runs
  `inbrain skillpack doctor --quick --json`, asserts every score is 10.
- New inbrain releases that drop a bundled-pack score below 10 fail
  CI loud. The cost of fixing is a few `inbrain skillify scaffold`
  calls; the cost of skipping is that inbrain ships skillpacks that
  fail the bar inbrain demands of third parties — credibility-poison.
...
### Scaffold: `inbrain skillpack init <name>` (cathedral defaults)

Lands the complete tree out of the box. `inbrain skillpack pack --dry-run`
...
Boil-the-lake default. Same pattern as `inbrain init` which seeds
...
### Install state: `~/.inbrain/skillpack-state.json` (codex G1)
...
- `~/.inbrain/skillpack-state.json` (machine-owned, agent-readable):
...
- Resolver-block sub-headers (in RESOLVER.md / AGENTS.md) carry only
  ...
  `<!-- inbrain:skillpack:source name="..." version="..." tier="..." cumulative-slugs="..." -->`.
- Mismatch between state.json and resolver-block (e.g., resolver lists
  ...
  further mutations until reconciled by `inbrain skillpack reconcile`.
- Schema: `skillpack-state.json` has `schema_version:
  "inbrain-skillpack-state-v1"` for forward-compat; mirrors the
...
```markdown
<!-- inbrain:skillpack:begin -->
<!-- inbrain:skillpack:source name="inbrain" version="0.36.0.0" cumulative-slugs="ingest,query,..." -->
| ingest | ... |
| query  | ... |
<!-- inbrain:skillpack:source name="hackathon-evaluation" version="0.1.0" cumulative-slugs="judge-submission-2,score-rubric" pinned-commit="abc1234" rename-map="judge-submission:judge-submission-2" tofu-sha256="deadbeef..." -->
| judge-submission-2 | Judge a hackathon submission against the YC rubric. |
| score-rubric       | ... |
<!-- inbrain:skillpack:end -->
```
...
- **`--allow-private-remotes`** flag pipes through to `git-remote.ts`'s
  `INBRAIN_ALLOW_PRIVATE_REMOTES` for internal/Tailscale skillpacks.
- Signing (minisign / cosign) is a v2 escape hatch; not in scope for v1.

### CLI surface

```
inbrain skillpack install <source> [--update] [--trust] [--allow-private-remotes] [--dry-run] [--json]
inbrain skillpack list [--source <name>] [--json]
inbrain skillpack uninstall <name> [--overwrite-local] [--dry-run]
inbrain skillpack info <name>                 # NEW: show pinned commit, author, license, renames
inbrain skillpack update [<name>] [--check]   # NEW: check for upstream commits
inbrain skillpack init <name>                 # NEW: scaffold publisher repo
inbrain skillpack pack [--out <path>]         # NEW: validate + emit .tgz tarball
```

`<source>` accepts:
- `inbrain-ai/repo` → `https://github.com/inbrain-ai/repo.git`
...
1. `inbrain skillpack init <name>` — scaffolds `skillpack.json` +
   `skills/` + `RESOLVER.md` (skillpack-internal) + `.gitignore`.
2. Author writes skills using existing `inbrain skillify scaffold` patterns.
3. `inbrain skillpack pack --dry-run` — runs the full validate pipeline:
   - `skillpack.json` schema check
...
- `src/core/skillpack/remote-source.ts` — wraps `git-remote.ts` for the
  ...
  `~/.inbrain/skillpack-cache/<host>/<owner>/<repo>/<sha>/`, resolves
...
- `src/core/skillpack/registry-client.ts` — fetch + cache the live
  ...
  `~/.inbrain/skillpack-cache/registry-<sha256-of-url>.json` with a
  1-hour soft TTL.
  **Offline-safe**: on fetch failure (network down, GitHub 5xx, DNS
  ...
  warning escalates to `cache is stale, run 'inbrain skillpack registry
...
- `src/core/skillpack/registry-schema.ts` — runtime validators for
  ...
  used by both `inbrain skillpack search` and the publish-gate skill.
- `src/core/skillpack/sandbox.ts` — **subprocess-isolated** trial-install
  ...
  - Inside the sandbox, an ephemeral in-memory PGLite inbrain runs the
  ...
  - **HOME override**: `HOME=<tempdir>/sandbox-home` (empty dir).
    Side effects: no `~/.inbrain` access, no `~/.gitconfig` (credential
...
- `src/core/skillpack/sandbox-probe.ts` — pre-flight: detects which
  ...
  re-probing on every trial). `inbrain doctor` surfaces the chosen
  backend as info.
...
- `src/commands/skillpack-init.ts` — `inbrain skillpack init <name>`
  scaffold command.
- `src/commands/skillpack-pack.ts` — `inbrain skillpack pack` validator
  AND tarball emitter. Single command, `--dry-run` skips the tarball.
- `src/commands/skillpack-info.ts` + `skillpack-update.ts`.
- `src/commands/skillpack-search.ts` — `inbrain skillpack search <query>
  [--tier ...] [--json]` reads the registry, ranks by tier then tag
  match, prints a table.
- `src/commands/skillpack-registry.ts` — `inbrain skillpack registry
  [--url X] [--refresh]` show/set the configured registry URL,
  optionally force a fresh fetch.
- `src/commands/skillpack-endorse.ts` — `inbrain skillpack endorse <name>
...
- `src/core/skillpack/runbook-parser.ts` — parses
  ...
  Validates frontmatter (`runbook_kind`, `inbrain_version_range`,
...
- `src/core/skillpack/runbook-walker.ts` — executes a parsed runbook
  ...
  the inbrain CLI subcommand (only inbrain CLI; not arbitrary shell);
...
- `src/commands/skillpack-test.ts` — `inbrain skillpack test [pack-dir]`
...
- `src/core/skillpack/doctor.ts` — `runDoctor(pack, opts:
  ...
  path dispatches per-dimension auto-scaffold (calls `inbrain skillify
...
- `scripts/check-bundled-skillpacks-rubric.sh` — CI guard. Walks every
  ...
  `inbrain skillpack doctor --quick --json` against each, asserts
  score=10. Fails the build loud on regression. Wired into
  `package.json`'s `verify` script.

### New (in github.com/inbrain-ai/inbrain repo, examples + docs)

- `examples/skillpack-reference/` — a real, working **10/10 reference
  skillpack** that lives in the inbrain repo. Two skills, 2
  ...
  publish-gate full-suite E2E test, AND it's what `inbrain skillpack
...
- `src/core/skillpack/audit.ts` — JSONL audit at
  `~/.inbrain/audit/skillpack-YYYY-Www.jsonl` (ISO-week rotated, mirrors
  ...
  `readRecentSkillpackEvents(days)` is the readback path for `inbrain
...
- `skills/inbrain-skillpack-publish/SKILL.md` — the publish-gate skill
  itself. Lives in the bundled skillpack so every inbrain install ships
  with it. Walks the contributor through:
  1. Validate locally (`inbrain skillpack pack --dry-run`)
  2. Run security gates
  3. Pack tarball + compute SHA
  4. Fork `inbrain-ai/inbrain-skillpack-registry` if needed (`gh repo fork`)
...
### New (in github.com/inbrain-ai/inbrain-skillpack-registry, separate repo)
...
- `test/skillpack-manifest-v1.test.ts`,
  ...
  `inbrain skillpack doctor --quick --json examples/skillpack-reference`
...
1. **Publisher path**: `inbrain skillpack init hackathon-evaluation` in a
   tempdir → add a synthetic skill → `inbrain skillpack pack --dry-run`
   → expect pass.
2. **Install from local path**: `inbrain skillpack install <tempdir>` in
...
4. **Multi-pack coexistence**: install the bundled inbrain set AND the
...
6. **Uninstall safety**: edit one skill file in the third-party pack →
   `inbrain skillpack uninstall hackathon-evaluation` → refuses without
...
8. **Registry resolution**: `inbrain skillpack install hackathon-evaluation`
...
9. **Search**: `inbrain skillpack search yc --json` returns the entry,
   `endorsed` tier sorts before `community` sorts before `experimental`.
10. **Bundle install**: `inbrain skillpack install starter-pack` walks
...
12. **Trial install sandbox isolation**: the ephemeral PGLite the
    publish-gate spins up does NOT touch `~/.inbrain`. Tear-down is
    clean — no file artifacts, no DB connections left behind.
13. **Runbook execution end-to-end**: `inbrain skillpack install
...
16. **`inbrain skillpack test`** runs against a freshly-scaffolded
...
- Versioning constraints richer than `inbrain_min_version` (no semver
  range matching, no `^0.36`).
- Auto-update / background pulls. `inbrain skillpack update` is manual.
- A central web UI (inbrain.dev/skillpacks). The registry repo's GitHub
  page IS the web UI in v1.
- Payment / monetization. Skillpacks are free / open source by default.
- Print-press CLI generation against inbrain HTTP MCP — explicitly IN
...
2. **W2: Registry catalog** — `inbrain-ai/inbrain-skillpack-registry`
   created, `registry.json` schema + endorsements.json, registry-client
   with stale-cache fallback, `inbrain skillpack search` + `install
   <short-name>` + `info`. Initial catalog seeded with bundled inbrain
   skills + hackathon-evaluation + maybe one community pack.
3. **W3: Publish-gate skill** — `/inbrain-skillpack-publish` skill,
...
4. **W4: Audit + doctor integration** — `~/.inbrain/audit/skillpack-*`
   JSONL, `inbrain doctor` check, `inbrain skillpack history` reader.
5. **W5: Printing Press cross-list** — open the PR against
   `mvanhorn/printing-press-library` listing
   `inbrain-ai/inbrain-skillpack-registry` as a sister registry. ~1 day.
6. **W6: Generated inbrain-cli (Printing Press)** — run printing-press
   against inbrain's HTTP MCP, ship the resulting agent-native CLI to
   their library. Independent week of work; doesn't block W1-W5.

**W4.5 — Bring bundled inbrain skillpacks to 10/10** (drops between W4
...
5. **Endorsement workflow**: `inbrain skillpack endorse <name> [--tier ...] [--push]` CLI command with schema validation; hand-editing remains valid.
...
- Lane F (W6: generated inbrain-cli): independent week of work; spawns its own branch.
...
3. **Runbook format: agent-readable markdown** with three step kinds (`agent:`, `show user:`, `ask user:`). Separate `install.md`, `uninstall.md`, `upgrade-<from>-to-<to>.md` per version. Mirrors inbrain's own `skills/migrations/v0.21.0.md` pattern.
4. **`inbrain skillpack init` scaffolds the cathedral by default.** Full tree (skills, tests, e2e, evals, runbooks, CHANGELOG, README, LICENSE) lands out of the box; `inbrain skillpack pack --dry-run` passes immediately. `--minimal` flag for power users opting out.

*Round 2 — rubric + doctor + reference + invariant:*
5. **Layered doctor:** `inbrain skillpack doctor --quick` (~5s structural sweep, walks the rubric, no sandbox/LLM/DB) for rapid iteration; `--full` (runs the full publish-gate suite) for ship-readiness. Two-tool design; agent picks the mode per workflow phase. The user noted: agents do the operating, so the cognitive cost of two flags is irrelevant as long as the docs teach the agent when to use which.
...
7. **`doctor --fix` auto-scaffold:** Calls `inbrain skillify scaffold` for missing skills, drops runbook stubs, generates CHANGELOG entries from VERSION + git log. Confirm prompt on TTY; `--yes` skips; refuses to overwrite files whose mtime is newer than `skillpack.json`'s.
8. **Reference pack + anatomy doc + 10/10 invariant for EVERY bundled inbrain skillpack:** ship `examples/skillpack-reference/` (real working 10/10 pack) AND `docs/skillpack-anatomy.md` (one-page reference, auto-generated rubric section from `rubric.ts`). NEW INVARIANT (the user's strongest line): every inbrain-shipped skillpack must score 10/10 on `--quick`. `scripts/check-bundled-skillpacks-rubric.sh` is wired into `bun run verify` + CI. Bringing today's `openclaw.plugin.json` set to 10/10 is wave W4.5 — blocking on W3 (doctor) but required before v1.0 ship. Credibility-poison if inbrain ships skillpacks below the bar inbrain demands of third parties.
...
**Magical moment** (locked from DX 0D): `inbrain skillpack install <name>` lands the pack AND walks `runbooks/install.md` AND the agent immediately knows what triggers fire, what tools the skill exposes, and how to upgrade later. Zero "where are the docs?" moment.

**Second magical moment** (Round 2): `inbrain skillpack doctor --quick --json` prints a 10/10 score with paste-ready fixes for the misses. The agent reads the JSON, runs `--fix --yes` to auto-scaffold, re-runs `--quick`, and the score climbs. The first time an agent gets from 6/10 to 10/10 in 30 seconds via three `inbrain` commands is the moment the "skillpacks are real software packages" claim becomes felt rather than asserted.
...
- T2 (SCOPE) — held: cathedral is the moat; minimal v1 forfeits curation+evals+runbooks differentiation; without those inbrain skillpacks are just another agentskills.io mirror.
- T3 (10/10 BUNDLED) — held: shipping inbrain's own packs below the bar inbrain demands is credibility-poison; W4.5 retrofit costs ~3d with --fix autoscaffold, slips v1 by a week.
...
- G1 (TRUST STORE) — adopted: `~/.inbrain/skillpack-state.json` machine-owned (TOFU pins, hashes, rename maps); resolver markdown stays render-only (rows + cumulative-slugs). Mismatch fails loud.
...
- Versioning beyond `inbrain_min_version`: manifest also carries `runbook_schema_version` + `eval_schema_version`; installer rejects newer-than-supported with paste-ready upgrade hint.
