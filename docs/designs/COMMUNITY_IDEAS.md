...
> good thinking survives even when the PR that carried it is closed. inbrain moves
...
> master and (b) genuinely valuable to inbrain users. **Graduating an idea to
...
- **Configurable FTS language** (#580/#581/#582, @rafaelreis-r) — **OPEN, high.**
  ...
  stemmer. A coherent 3-PR set: `INBRAIN_FTS_LANGUAGE` config → migration recreating
  triggers with the chosen language → `inbrain reindex-search-vector` to change it
...
- **`DATABASE_URL` hijack** (#1884, @awilkinson) — **OPEN, high.** A co-located app's
  ...
  auto-migrates it). Fix precedence: `INBRAIN_DATABASE_URL` > config.json > `DATABASE_URL`.
...
- **HOME-isolation in tests** (#205/#517/#534 @orendi84, #434 @lloydarmbrust) — **OPEN,
  high.** The E2E suite spawns `inbrain init/import` against the developer's real
  `~/.inbrain/config.json`, clobbering their live DB URL+keys. Isolate HOME to a tmpdir.
...
- **Keyword search ignores page titles** (#1646, @jeades) — **OPEN, high.** `searchKeyword`
  ...
  exact-title `inbrain search` returns nothing while `query` finds it. High-impact, tiny.
...
- **Local-first chat parity** (#1854/#1855/#1858 @starm2010, #1423 @pabloglzg,
  ...
  `json_schema`. The "run inbrain fully local" cluster.
...
- **Dotfile / skills-dir confinement** (#418/#419, @garagon) — **OPEN, high, SECURITY.**
  `.inbrain-source` walk-up trusts any ancestor dotfile (source hijack on shared hosts);
...
- **Destructive reclone gate** (#1705, @mvanhorn) — **OPEN, high, SECURITY.**
  ...
  inbrain-managed, so a re-pointed source can wipe a user's working tree. Gate behind
...
- **Windows / CRLF portability** (#1294 @xwang4-svg, #1149 @samporter-31, #1554 @Sanjays2402,
  ...
  the full story.)*
- **`.inbrainignore` / per-repo exclusion** (#1483 @eepaul; repo-local code filters
  ...
  trees), bloating DB + embedding cost. gitignore-parity `.inbrainignore` + per-source
...
- **MCP tool filtering** (#747, @joelwp) — **OPEN, high.** MCP advertises all ~51 ops to
  every consumer (~10K tokens of schemas, tool confusion); `INBRAIN_EXPOSED_TOOLS` filters
  the advertised surface.
- **Install-method detection for upgrade** (#538, @brucek) — **OPEN, high.** The README's
  own recommended git-clone+bun-link install detects as `unknown`, so `inbrain upgrade`
...
- **MCP put_page parity + DB→markdown reconciliation** (#438, @rayzhux) — **HELD, high.**
  A frontmatter-only safe auto-link mode for remote callers + `INBRAIN_BRAIN_ROOT` to render
...
- **Recipe discovery convention** (#1279, @ialmeida-jera) — **OPEN, med.** `~/.inbrain/recipes/`
...
- **Honest stale-lock hint** (#1553, @Sanjays2402) — **OPEN, med.** doctor always says
  `inbrain sync --break-lock`, which silently no-ops on `inbrain-cycle` locks.
