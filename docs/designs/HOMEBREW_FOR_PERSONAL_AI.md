...
The 10-star vision for Inbrain's integration system. Ship Approach B (v0.7.0),
build toward this over subsequent releases.

## The Vision

Inbrain becomes a personal infrastructure operating system where every signal in
...
name: Voice-to-Brain
version: 0.7.0
description: Phone calls create brain pages via Twilio + OpenAI Realtime + Inbrain MCP
category: sense
...
`inbrain integrations doctor` runs health_checks from every configured recipe:
```
$ inbrain integrations doctor
...
`inbrain integrations stats` aggregates heartbeat data:
```
$ inbrain integrations stats
...
1. **Inbrain is deterministic infrastructure.** Cross-sense correlation, pattern
   detection, and intelligent responses are the agent's job (OpenClaw/Hermes).
   Inbrain provides the plumbing.
