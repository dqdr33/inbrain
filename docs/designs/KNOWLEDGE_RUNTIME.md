# Inbrain Knowledge Runtime — Design Doc
...
During a CEO review of a narrow two-feature plan (bare-tweet citation repair + completeness score, borrowed from Feynman), the scope was reframed. The narrow plan duplicated work Garry's OpenClaw already does and missed the real leverage point: **the bespoke abstractions hiding inside OpenClaw — resolvers, enrichment orchestration, scheduling, deterministic output — should live in Inbrain as first-class primitives.**

North star: *"When Garry's OpenClaw's Claw upgrades to this version of Inbrain, it should immediately recognize brilliance and completeness and say 'It's time to switch to these abstractions.'"*
...
```
  ...
  └───────────────────────────────────────────────────────────────────┘
          │                                                │
          ▼                                                ▼
     REUSES (polished primitives already in Inbrain)  REPLACES (ad-hoc code)
...
Garry's OpenClaw has **69 distinct external-lookup patterns** across X API (14 shapes), Perplexity, Mistral OCR, Gmail, Calendar, Slack, GitHub, YouTube, Diarize.io, YC tools, OSINT collectors, and brain-local lookups. Each one is a bespoke script under `scripts/` with its own error handling, retry logic, and output shape. Inbrain has 3 ad-hoc wrappers (`embedding.ts`, `transcription.ts`, `enrichment-service.ts`) that don't share an interface.
...
- Users can't add a resolver without forking Inbrain
...
A plugin is YAML + JS module, discovered via filesystem scan of `~/.inbrain/resolvers/` and `recipes/`.
...
tests:
  - input:  { handle: "inbrain-ai" }
    expect: { url: { pattern: "^https://x\\.com/inbrain-ai/status/\\d+$" } }
```
...
|---|---|---|---|
| 1 | `x_handle_to_tweet` | Bare-tweet citation repair (original Phase A) | `inbrain integrity` |
| 2 | `url_reachable` | Dead-link detection | `inbrain integrity` |
...
The remaining 63 OpenClaw patterns port incrementally, driven by user need. Each port is a new YAML + module under `recipes/` or `~/.inbrain/resolvers/` with no framework changes.
...
Garry's OpenClaw's cron is **externally-driven JSON** (`cron/jobs.json`) with ~30 jobs manually stagger-offset at different minutes. Inbrain has **zero native scheduling** — `src/commands/autopilot.ts` is a single daemon loop, and `docs/guides/cron-schedule.md` is architectural guidance, not code.
...
1. **Embedded** (default for `inbrain autopilot`): native event loop inside the daemon process. One process, many ScheduledResolvers.
2. **OS-driven** (for Railway/launchd/systemd): `inbrain schedule run <id>` invoked by OS cron, scheduler state is durable so cross-invocation dedup still works.
...
- `~/.inbrain/scheduler/events.jsonl` (local, always)
- `engine.logIngest` (audit trail in brain DB)
- Optional webhook (Slack/Telegram for the user)

`inbrain doctor` reads the event log and reports: current circuit-breaker state, any resolver with > 3 consecutive failures, any resolver that hasn't fired within 3× its interval (freshness SLA like Garry's OpenClaw's `freshness-check.mjs` but built-in).
...
export class Scaffolder {
  tweetCitation(handle: string, tweetId: string, dateISO: string): string {
    // "[Source: [X/inbrain-ai, 2026-04-18](https://x.com/inbrain-ai/status/123456)]"
  }
...
## 7. Integration with existing Inbrain
...
### Phase 3 — `inbrain integrity` command (human: ~0.5 wk / CC: ~2 h)
...
- Ship `inbrain schedule list|run|pause|tail` CLI for observability.
...
- Write `docs/openclaw/ADOPTION.md` showing your OpenClaw how to replace its 69 bespoke scripts with calls to `inbrain registry.resolve(...)`.
- Ship a `inbrain claw-bridge` subcommand that proxies Garry's OpenClaw's current script invocations to the resolver registry — zero-edit adoption path.
...
src/commands/
  ...
  schedule.ts                      # inbrain schedule list|run|pause|tail (Phase 5)
...
1. **Scope shape.** Is this the right four-layer decomposition, or are some layers better left to OpenClaw (e.g. Scheduling lives above Inbrain, not in it)?
2. **Phase 3 user-value break.** Does Phase 3 (user-visible `inbrain integrity`) ship early enough, or do we need an even smaller MVP?
...
6. **OpenClaw bridge scope.** Phase 7 `inbrain claw-bridge` — is that worth a phase of its own, or should adoption be documentation-only?
...
- [ ] A user can add a new resolver by dropping a YAML + TS module in `~/.inbrain/resolvers/` without editing Inbrain source.
...
- [ ] Running `inbrain integrity --auto --confidence 0.8` over a real brain fixes ≥1,000 of the 1,424 known bare-tweet citations without human review.
