...
Branch: inbrain-ai/minions-jobs | Mode: SCOPE EXPANSION
Repo: inbrain-ai/inbrain

## Vision

### 10x Check
Instead of "Inbrain has a queue, OpenClaw uses it," make Minions a universal agent
...
protocol. Inbrain IS the agent control plane.

### Platonic Ideal (aspirational North Star, NOT in v1 scope)
Open a terminal, type `inbrain jobs dashboard`. See every agent across every platform.
...
The agent handler does NOT live in Inbrain. Inbrain provides the queue infrastructure
...
Inbrain ships a test/echo handler for unit testing only.
```

**Handler contract (Inbrain side):**
```typescript
...
**Why this is right:** Inbrain is orchestration, not execution. OpenClaw has the
PI agent core. Hermes has AIAgent. Claude Code has its own loop. Each platform
brings its own engine and registers a handler. Inbrain manages lifecycle, progress,
...
stored as a brain file via `inbrain files upload-raw`.
...
- **Phase 2: Dashboard CLI** — `inbrain jobs dashboard` live TUI showing all agents.
...
1. Inbrain is intentionally evolving from knowledge brain to agent infrastructure (user confirmed)
2. Coupling between OpenClaw and Inbrain's Postgres is acceptable (OpenClaw already depends on Inbrain)
