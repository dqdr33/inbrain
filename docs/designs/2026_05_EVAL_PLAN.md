...
- **Synthetic-only** — LongMemEval (public) + BrainBench (in-house). No `~/.inbrain` data.
- **Answer-gen mode** — `inbrain eval longmemeval` runs the default answer-gen path
...
- `inbrain/ai/gateway` is NOT exported in v0.35.0.0. PR α exposes it.
...
**PR α — inbrain repo: v0.35.1.0 infra.** All inbrain changes bundled. Lands first.
Bisect-friendly commits inside, ship at the very end.

**PR β — inbrain-evals repo: adapter + smoke + curation + eval receipts + writeup.** The
...
**PR γ (optional) — inbrain repo: v0.35.2.0 release** that cross-links the inbrain-evals
...
## Session 1 — PR α: inbrain infra (v0.35.1.0)

**Repo:** `/Users/inbrain-ai/conductor/workspaces/inbrain/<NEW-WORKSPACE>` (fresh from `master`)
**Branch:** `inbrain-ai/v0.35.1.0-infra`
**Wallclock:** ~2h
**API spend:** $0

### What this session ships
Three changes in one PR, bundled so the embedder shootout in inbrain-evals (PR β) has a
clean prereq baseline:

1. Add `voyage:voyage-4-large` ($0.18/M) and `zeroentropyai:zembed-1` ($0.05/M) to the
   embedding pricing table. Patch the `inbrain models doctor` cost estimator + test.
2. Expose `inbrain/ai/gateway` in `package.json` exports map so the inbrain-evals
   adapters can call `configureGateway({embedding_model, embedding_dimensions, reranker_model})`
   from outside the inbrain process.
3. Add `--resume-from <jsonl>` to `inbrain eval longmemeval` so a mid-run abort
...
- On inbrain master at v0.35.0.0 baseline. `cat VERSION` shows `0.35.0.0`.
...
2. feat(exports): expose inbrain/ai/gateway with canary test
...
### Deliverable
- `master` of inbrain at v0.35.1.0
- `inbrain/ai/gateway` reachable from external consumers (verified by canary test)
- `git tag eval-run-v0.35.1.0-baseline` (annotated, names this exact commit)
- `inbrain --version` prints `0.35.1.0`

### Hand-off to Session 2
- inbrain-evals can now `bun update inbrain` to v0.35.1.0
- The tag preserves the exact commit for any future reproducibility need

---

## Session 2 — PR β setup: inbrain-evals adapter + smoke + subset flag

**Repo:** `/Users/inbrain-ai/git/inbrain-evals` (or a fresh Conductor workspace cloned from it)
**Branch:** `inbrain-ai/embedder-shootout`
**Wallclock:** ~3-4h
**API spend:** ~$0.10 (smoke verification calls only)

### What this session ships into PR β (does NOT merge yet)
Wire the harness to drive 3 embedding providers via the newly-exposed inbrain gateway:
...
2. Rewrite `vector.ts` + `hybrid-rrf.ts` to call `configureGateway()` from
   `inbrain/ai/gateway` instead of the hardcoded `inbrain/embedding` import.
...
### Prereqs
- Session 1 done. inbrain master at v0.35.1.0.
...
```
1. chore(deps): bump inbrain pin to v0.35.1.0
...
2. feat(adapter): typed EvalAdapterConfig + gateway swap
   ...
   - Drop hardcoded inbrain/embedding import
...
### Deliverable
- PR β open against inbrain-evals `main`, green CI
...
- Branch `inbrain-ai/embedder-shootout` exists on origin
...
**Repo:** `/Users/inbrain-ai/git/inbrain-evals`, branch `inbrain-ai/embedder-shootout` (same as Session 2)
...
**Repo:** Same inbrain-evals branch
...
### Wrapper script
Claude writes `scripts/run-shootout-phase1.sh` in the inbrain-evals branch. Single
entry point that loops the 7 cells serially with smoke gating + cost-cap aborts.

```
NEW: scripts/run-shootout-phase1.sh
- Per cell: inbrain config set (embedder, dim, reranker, search.reranker.enabled, search.mode=tokenmax)
- Per cell: bun run eval:smoke (abort cell on non-zero)
- Per cell: inbrain eval longmemeval ... --output results/longmemeval-{cell}.jsonl
...
3. **Paired-bootstrap p-values** per headline pair (methodology in
   `inbrain/docs/eval/SEARCH_MODE_METHODOLOGY.md`)
...
6. **Recommendation:** explicit NON-recommendation to change `inbrain init` default;
...
### Ship
```bash
# Merge PR β to inbrain-evals main
...
### Deliverable
- PR β merged to inbrain-evals `main`
- Comparison report public at
  `inbrain-evals/docs/benchmarks/2026-05-22-embedder-shootout.md`

### Hand-off to Session 6 (optional)
- inbrain-evals master has the full data + writeup
- Ready for a v0.35.2.0 inbrain release that cross-links it

---

## Session 6 (optional) — PR γ: inbrain v0.35.2.0 release

**Repo:** `/Users/inbrain-ai/conductor/workspaces/inbrain/<NEW-WORKSPACE>` (fresh from master)
**Branch:** `inbrain-ai/v0.35.2.0-benchmark-release`
**Wallclock:** ~30min
**API spend:** $0

### What this session ships
A release-notes-only PR that bumps inbrain to v0.35.2.0 with a CHANGELOG entry
cross-linking the embedder shootout benchmark. Optional — could be folded into the
next routine release if no rush.

### Prereqs
- Session 5 done. inbrain-evals merged with the comparison writeup.
...
1. docs(benchmark): mirror embedder shootout summary
   ...
   - Cross-link to inbrain-evals canonical version
...
| Mid-cell OS interrupt / cost-cap abort | Re-run with `inbrain eval longmemeval --resume-from results/longmemeval-{cell}.jsonl`. Picks up where it left off. |
...
- **Real `~/.inbrain` replay** — adds 6-12h wallclock + $40-80 embed. Filed as v0.36.x.
...
- **Matched-dim cross-vendor row** — no shared dim exists across all 3 vendors.
  Permanently out.
- **`inbrain eval whoknows` / `cross-modal` / `takes-quality`** — embedding-invariant;
  rerunning across embedders produces noise.
- **`inbrain eval code-retrieval`** — code corpus, separate concern.
- **`inbrain eval suspected-contradictions`** — wants a real brain.
- **`inbrain init --recommended` default change** — codex correctly flagged the evidence
  base as insufficient. Defer to v0.36.x with real-brain replay data.

## What already exists (reused, not rebuilt)

- `inbrain eval longmemeval` CLI (in-tree, answer-gen mode default)
- inbrain-evals BrainBench runner (`eval:run`) — needs adapter parameterization but
