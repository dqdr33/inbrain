...
Schemas are how inbrain stops being a pile of notes and becomes something with structure. A schema declares what KINDS of things live in your brain (`person`, `company`, `meeting`, `researcher`, `case`, `lab-result`), what they link to (`attended`, `authored`, `prescribed-by`), what facts the system should extract automatically (`mrr=50000`, `damages=5000000`), and which types route through expert search vs general search.

The default schema (`inbrain-base`) ships with 22 page types covering the universal shapes — people, companies, meetings, notes, daily, calendar events. That's enough to start. But your brain is yours, and your brain's shape is not the default shape. A research brain needs `researcher` and `paper` as first-class types. A founder brain needs `lead`, `investor`, `portco`, `deal-stage`. A lawyer brain needs `case`, `motion`, `deposition`, `precedent`. Same engine, totally different shape.

v0.40.7.0 made it possible for AGENTS to author that shape for you. Not just "the user manually edits YAML in `~/.inbrain/schema-packs/mine/pack.yaml`" but "your agent sees the corpus, proposes a type, asks for approval, applies it atomically with a full audit trail, then backfills 4000 existing pages with one chunked SQL command." That's the new thing.
...
```bash
inbrain whoknows "Q3 roadmap discussion"
```
...
```bash
inbrain schema add-type meeting --primitive temporal --prefix meetings/ --extractable
inbrain schema sync --apply
```

The sync backfills `page.type = 'meeting'` on all 4000 pages in 1000-row batches. Now:

- `inbrain whoknows "Q3 roadmap discussion"` routes through the meeting type, ranking by `expert_routing` signal (attendees, recency, salience) instead of raw text.
- `inbrain extract-facts` runs on every meeting page automatically (because `extractable: true`), pulling typed facts like `attended_by=alice-example`, `date=2026-05-23`.
...
```bash
inbrain schema fork inbrain-base mine
inbrain schema use mine

# Types
inbrain schema add-type lead       --primitive entity --prefix people/leads/         --expert
inbrain schema add-type investor   --primitive entity --prefix people/investors/     --expert --extractable
inbrain schema add-type portco     --primitive entity --prefix companies/portco/     --expert --extractable
inbrain schema add-type deal       --primitive entity --prefix companies/deals/      --extractable

# Link verbs
inbrain schema add-link-type invested-in --page-type investor --target-type portco
inbrain schema add-link-type intro-from  --page-type lead     --target-type lead
inbrain schema add-link-type passed-on   --page-type investor --target-type deal
inbrain schema add-link-type led-by      --page-type deal     --target-type investor

inbrain schema sync --apply
```

Now `inbrain whoknows "Series A SaaS"` routes through `investor` and `portco` types specifically, not the noisy general type set. `inbrain graph-query alice-example --type intro-from --depth 2` walks two hops of intros to surface "Alice introduced you to Bob who introduced you to Charlie." `inbrain extract-facts` starts producing typed claims from the fence in your deal pages: `(deals/acme-seed, raise=2000000, valuation=15000000, lead=widget-vc, closed_at=2026-05-23)`.
...
```bash
inbrain schema add-type paper --primitive annotation --prefix research/papers/ --extractable
inbrain schema add-link-type authored   --page-type researcher --target-type paper
inbrain schema add-link-type cites      --page-type paper      --target-type paper
inbrain schema add-link-type uses       --page-type paper      --target-type dataset
```

Suddenly "show me papers that cite this work AND use the same dataset" is a `inbrain graph-query` traversal, not 30 minutes in Google Scholar. The fact extraction picks up `arxiv_id=2402.04253`, `cited_by_count=140`, `published_date=2026-02-15` automatically. Your reading-list-as-markdown turns into a queryable research graph that knows who works on what and what's connected to what.
...
```bash
inbrain schema add-type case --primitive entity --prefix legal/cases/ --extractable --expert
inbrain schema add-type motion --primitive annotation --prefix legal/motions/ --extractable
inbrain schema add-type deposition --primitive annotation --prefix legal/depositions/ --extractable
inbrain schema add-link-type filed-in --page-type motion --target-type case
inbrain schema add-link-type cites    --page-type motion --target-type precedent
```

Now `## Facts` fences in your case notes can carry typed claims (`damages=5000000`, `filed_date=2026-05-23`, `judge=jane-doe`) that inbrain stores as first-class columns. `inbrain eval trajectory legal/cases/acme-v-widget` prints the case history with regressions flagged. `inbrain founder scorecard` (renamed for legal: roll up plaintiff success rate, average damages, settlement-vs-trial ratio) gives you a structured view of how your practice is performing.

This isn't possible without typed page kinds. You can write the same prose in any note-taking app. Only inbrain treats the numbers as comparable across pages of the same type.

### 5. The team brain

`inbrain mounts add` lets you stack additional brains alongside your personal one. Each mounted brain has its OWN schema pack. The eng team's brain has `incident`, `runbook`, `service`, `oncall-rotation`. The design team's brain has `component`, `experiment`, `ab-test`, `figma-link`. The legal team's brain has cases and depositions.
...
Your OpenClaw (or any agent connected to your brain over HTTPS MCP with admin scope) watches your ingestion stream. After a week of you dumping notes under `inbrain-ai/companies/yc-w24/`, the agent runs `inbrain schema detect` periodically, sees that prefix accumulating, and proposes:
...
Pick a real corpus you have. Run `inbrain whoknows` on a topic that should match. Note the top-3 results.

Then run `inbrain schema review-orphans --limit 50 --json` and look at the untyped pages. If 10+ of them share an obvious prefix that should be a real type, add the type + sync.
...
Three things inbrain does that generic note systems can't:

**1. The brain knows the difference between a person and an idea.** Page-type matters at query time. `inbrain whoknows` only considers `expert_routing: true` types. `inbrain extract-facts` only runs on `extractable: true` types. `inbrain graph-query` walks declared link verbs. None of that works on a flat tag system because tags don't have semantics — they're labels. Types are first-class citizens with rules attached.
...
**3. The schema is queryable AND mutable AND auditable.** You can ask the brain what its schema looks like (`inbrain schema graph`), evolve it through 14 atomic CLI verbs + 9 MCP ops with full lock + audit semantics, and recover from any mistake (every primitive has an inverse, plus `inbrain schema downgrade` restores the previous active pack). This isn't "vibes-based knowledge management." It's a production system with structural integrity guarantees.
...
- **Privacy-redacted audit log** at `~/.inbrain/audit/schema-mutations-YYYY-Www.jsonl`. Type names sha8-hashed, prefixes truncated to first segment only. A leaked screenshot of the audit can't reveal sensitive taxonomy like `personal/oncology/` or `legal/depositions/`.
...
- **Cross-process invalidation** via stat-mtime TTL gate inside `loadActivePack`. Operator runs `inbrain schema add-type` from a terminal; the autopilot daemon picks up the new type within 1 second without a restart.
...
- **Want to set up an agent that co-curates your brain?** Run `inbrain auth register-client my-agent --scopes admin` to mint an OAuth client your remote agent can use to call `schema_apply_mutations` over MCP. The agent then runs detect → suggest → apply on its own cadence and asks you to approve substantive changes.
