# `inbrain eval takes-quality` — reproducible cross-modal quality eval
...
This doc is the consumer contract. The sibling [inbrain-evals](https://github.com/inbrain-ai/inbrain-evals)
...
|---|---|---|
| `inbrain eval takes-quality run [flags]` | yes (samples takes) | 0 PASS, 1 FAIL, 2 INCONCLUSIVE |
| `inbrain eval takes-quality replay <receipt>` | **no** (disk-only) | 0 PASS, 1 FAIL, 2 INCONCLUSIVE |
| `inbrain eval takes-quality trend [flags]` | yes (reads runs table) | 0 |
| `inbrain eval takes-quality regress --against <receipt>` | yes | 0 OK, 1 regression |
...
codex review #6) AND to disk at `~/.inbrain/eval-receipts/takes-quality-<corpus>-<prompt>-<models>-<rubric>.json`
...
```bash
# Capture a baseline.
inbrain eval takes-quality run --limit 100 --json \
  > .ci/takes-quality-baseline.json

# Later, after changing the extraction prompt:
inbrain eval takes-quality regress --against .ci/takes-quality-baseline.json \
  --threshold 0.5
