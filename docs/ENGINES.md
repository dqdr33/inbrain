...
Every Inbrain operation goes through `BrainEngine`. The engine is the contract between "what the brain can do" and "how it's stored." Swap the engine, keep everything else.
...
The engine interface means we don't have to choose. PGLite is the zero-friction default. Supabase is the production scale path. `inbrain migrate --to supabase/pglite` moves between them.
...
- Data stored at `~/.inbrain/brain.db` (configurable)
...
|--------|--------|--------------------------|
| Setup | `inbrain init` (zero-config) | Account + connection string |
...
**Migration:** `inbrain migrate --to supabase` exports everything (pages, chunks, embeddings, links, tags, timeline) and imports into Supabase. `inbrain migrate --to pglite` goes the other direction. Bidirectional, lossless.
...
3. Store engine type in `~/.inbrain/config.json`: `{ "engine": "myengine", ... }`
