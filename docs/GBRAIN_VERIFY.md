# Inbrain Installation Verification Runbook

Run these checks after install to confirm every part of Inbrain is working.
...
**Expected:** The agent references INBRAIN_SKILLPACK.md Section 2 and describes
...
install paste (read `docs/INBRAIN_SKILLPACK.md`).
...
```bash
inbrain check-update --json
```

**Expected:** Returns JSON with `current_version`, `latest_version`,
`update_available` (boolean). The cron `inbrain-update-check` is registered.

**If it fails:** Run step 7 from the install paste. See INBRAIN_SKILLPACK.md
Section 17.
...
**Expected:** Page count in `inbrain stats` should be close to the file count.
...
connection on an IPv4-only host. Inbrain uses the Transaction pooler (port 6543)
...
- On an IPv4-only host, reads work but sync transactions fail and silently skip
  pages.
- Fix: set `INBRAIN_DIRECT_DATABASE_URL` to the **Session pooler** string (port
  5432 on the `pooler.supabase.com` host, IPv4), or enable Supabase's IPv4
  add-on. Then run `inbrain sync --full` to reimport everything.
...
```bash
inbrain search "<text from the correction>"
```
...
- Is `inbrain sync --watch` still alive (if using watch mode)?
- Run `inbrain config get sync.last_run` to see when sync last ran.
- Run `inbrain sync --repo /data/brain` manually and check for errors.
...
**Expected:** The agent uses `inbrain search` or `inbrain query` FIRST, not grep
...
```bash
inbrain stats | grep -E 'links|timeline'
```
...
```bash
inbrain extract links --source db --dry-run | head -5    # preview
inbrain extract links --source db                         # commit
inbrain extract timeline --source db
inbrain stats                                             # confirm > 0
```
...
inbrain graph-query people/<some-person-slug> --depth 2
```
...
(`frontmatter->>'key'` returned NULL, GIN indexes were inert). `inbrain upgrade`
runs `inbrain repair-jsonb` automatically via the `v0_12_2` orchestrator.
Verify the repair succeeded.

**Command:**

```bash
inbrain repair-jsonb --dry-run --json
```
...
```bash
inbrain call list_pages '{"frontmatterKey": "type", "frontmatterValue": "person"}'
```
...
# 2. Sync recency
inbrain config get sync.last_run

# 3. Page count + embed coverage
inbrain stats

# 4. Search works
inbrain search "test query from your brain content"
