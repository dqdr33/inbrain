#!/bin/bash
# smoke-test.sh — Inbrain post-restart smoke tests + auto-fix
#
# Ships with inbrain. Tests inbrain core services + OpenClaw plugin health.
# Users extend via ~/.inbrain/smoke-tests.d/*.sh (user-defined tests).
#
# Usage:
#   inbrain smoke-test          # run all tests
...
LOG="${INBRAIN_SMOKE_LOG:-/tmp/inbrain-smoke-test.log}"
FAILURES=0
FIXES=0
TOTAL=0
SKIPPED=0
...
echo "$(timestamp) === Inbrain Smoke Tests ===" >> "$LOG"
echo "🧪 Running inbrain smoke tests..."
echo ""

# ── Resolve paths ───────────────────────────────────────────
# Find inbrain — could be global install, workspace dep, or /data/inbrain
INBRAIN_DIR=""
for candidate in \
  "${INBRAIN_DIR_OVERRIDE:-}" \
  "/data/inbrain" \
  "$(dirname "$0")/.." \
  "${OPENCLAW_WORKSPACE:-/data/.openclaw/workspace}/node_modules/inbrain" \
  "./node_modules/inbrain"; do
  [ -n "$candidate" ] && [ -f "$candidate/src/cli.ts" ] && INBRAIN_DIR="$candidate" && break
done
...
DB_URL="${INBRAIN_DATABASE_URL:-${DATABASE_URL:-}}"
# Fallback: grep from .env (handles files with parse-breaking lines)
[ -z "$DB_URL" ] && DB_URL=$(grep '^INBRAIN_DATABASE_URL=' /data/.env 2>/dev/null | head -1 | cut -d= -f2-)
...
# ── 2. Inbrain CLI loads ────────────────────────────────────
if [ -n "$INBRAIN_DIR" ] && [ -n "$BUN_PATH" ]; then
  if timeout 15 "$BUN_PATH" run "$INBRAIN_DIR/src/cli.ts" --help >/dev/null 2>&1; then
    pass "Inbrain CLI ($INBRAIN_DIR)"
  else
    # Auto-fix: reinstall deps
    cd "$INBRAIN_DIR" && "$BUN_PATH" install --frozen-lockfile 2>/dev/null
    if timeout 15 "$BUN_PATH" run "$INBRAIN_DIR/src/cli.ts" --help >/dev/null 2>&1; then
      fixed "Inbrain deps reinstalled"
      pass "Inbrain CLI (after dep fix)"
    else
      fail "Inbrain CLI — won't start"
    fi
  fi
else
  [ -z "$INBRAIN_DIR" ] && fail "Inbrain CLI — not found"
  [ -z "$BUN_PATH" ] && skip "Inbrain CLI — bun not available"
fi

# ── 3. Inbrain database ────────────────────────────────────
if [ -n "$DB_URL" ] && [ -n "$INBRAIN_DIR" ] && [ -n "$BUN_PATH" ]; then
  DOCTOR_OUT=$(DATABASE_URL="$DB_URL" INBRAIN_DATABASE_URL="$DB_URL" timeout 20 "$BUN_PATH" run "$INBRAIN_DIR/src/cli.ts" doctor 2>&1)
  if echo "$DOCTOR_OUT" | grep -q "Health score\|brain_score\|Health Check"; then
    ...
    pass "Inbrain database (health score: $SCORE/100)"
  else
    fail "Inbrain database — doctor returned no health data"
  fi
else
  [ -z "$DB_URL" ] && fail "Inbrain database — no DATABASE_URL or INBRAIN_DATABASE_URL"
  [ -z "$INBRAIN_DIR" ] && skip "Inbrain database — inbrain not found"
fi

# ── 4. Inbrain worker process ──────────────────────────────
if [ -n "$INBRAIN_DIR" ] && [ -n "$BUN_PATH" ] && [ -n "$DB_URL" ]; then
  if [ -f /tmp/inbrain-worker.pid ] && kill -0 "$(cat /tmp/inbrain-worker.pid)" 2>/dev/null; then
    pass "Inbrain worker (PID: $(cat /tmp/inbrain-worker.pid))"
  else
    # Auto-fix: start worker
    DATABASE_URL="$DB_URL" INBRAIN_DATABASE_URL="$DB_URL" INBRAIN_ALLOW_SHELL_JOBS=1 \
      nohup "$BUN_PATH" run "$INBRAIN_DIR/src/cli.ts" jobs work --concurrency 2 > /tmp/inbrain-worker.log 2>&1 &
    echo $! > /tmp/inbrain-worker.pid
    sleep 2
    if kill -0 "$(cat /tmp/inbrain-worker.pid)" 2>/dev/null; then
      fixed "Inbrain worker started"
      pass "Inbrain worker (PID: $(cat /tmp/inbrain-worker.pid))"
    else
      fail "Inbrain worker — failed to start (check /tmp/inbrain-worker.log)"
    fi
  fi
else
  skip "Inbrain worker — prerequisites missing"
fi
...
BRAIN_PATH="${INBRAIN_BRAIN_PATH:-/data/brain}"
...
# ── User-defined tests (~/.inbrain/smoke-tests.d/*.sh) ────
USER_TESTS_DIR="${HOME}/.inbrain/smoke-tests.d"
