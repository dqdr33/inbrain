...
#
# HOME isolation: E2E tests call paths that resolve to inbrain init / saveConfig
# (e.g. setupDB writing config for the test container) and would otherwise
# write the user's real ~/.inbrain/config.json. The wrapper redirects HOME and
# INBRAIN_HOME to a tmpdir before bun starts so config writes land in the
...
# configPath/getDbUrlSource honor INBRAIN_HOME; setting only one leaves the
...
REAL_HOME="${HOME:-/tmp}"
USER_CONFIG="$REAL_HOME/.inbrain/config.json"
...
E2E_TMP_HOME=$(mktemp -d "${TMPDIR:-/tmp}/inbrain-e2e.XXXXXX")
trap 'rm -rf "$E2E_TMP_HOME"' EXIT

export HOME="$E2E_TMP_HOME"
export INBRAIN_HOME="$E2E_TMP_HOME"
mkdir -p "$E2E_TMP_HOME/.inbrain"
