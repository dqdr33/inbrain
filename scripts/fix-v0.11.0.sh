...
#
# What it does:
#   1. inbrain init --migrate-only — applies schema v7 without touching config.
#   2. inbrain jobs smoke — fails loudly if Minions isn't healthy.
#   3. Prompts for minion_mode (or defaults to pain_triggered on non-TTY).
#   4. Atomically writes ~/.inbrain/preferences.json (0o600).
#   5. Appends ~/.inbrain/migrations/completed.jsonl with status:"partial" and
...
command -v inbrain >/dev/null 2>&1 || die "inbrain not found on \$PATH. Install it first (\`bun add -g inbrain\` or download a binary)."

INBRAIN_DIR="${HOME}/.inbrain"
PREFS_PATH="${INBRAIN_DIR}/preferences.json"
COMPLETED_PATH="${INBRAIN_DIR}/migrations/completed.jsonl"

mkdir -p "${INBRAIN_DIR}/migrations"

# ------------------------------------------------------------
# Step 1: schema
# ------------------------------------------------------------
info "[1/8] Applying schema (inbrain init --migrate-only)..."
if ! inbrain init --migrate-only; then
  die "Schema migration failed. Check ~/.inbrain/config.json has a valid database_url (or database_path for PGLite), then re-run."
fi
ok "      schema ok"
...
# Step 2: smoke
# ------------------------------------------------------------
info "[2/8] Running Minions smoke test (inbrain jobs smoke)..."
if ! inbrain jobs smoke; then
...
if [ -t 0 ] && [ -t 1 ]; then
  ...
  warn "      non-interactive shell → defaulting to pain_triggered (change later: \`inbrain config set minion_mode <mode>\`)"
fi
ok "      mode=${MODE}"
...
# ------------------------------------------------------------
info "[4/8] Writing ~/.inbrain/preferences.json..."
...
if [ ${#AGENTS_FOUND[@]} -eq 0 ]; then
  ...
  for F in "${AGENTS_FOUND[@]}"; do
    ...
    echo "            2. The v0.11.1 binary's \`inbrain apply-migrations --yes\` will inject"
...
if [ ${#CRON_FOUND[@]} -eq 0 ]; then
  ...
  for F in "${CRON_FOUND[@]}"; do
    ...
    echo "              extract/backlinks/autopilot-cycle) to inbrain jobs submit"
...
echo ""
echo "      ${GREEN}inbrain autopilot --install${NC}"
echo ""
...
echo "        ${GREEN}inbrain upgrade && inbrain apply-migrations${NC}"
