...
#
# Env overrides:
#   INBRAIN_VERIFY_TIMEOUT       per-check wallclock cap, seconds (default 120)
#   INBRAIN_VERIFY_LOG_DIR       where to write per-check logs (default tempdir)
#
# Exit codes:
#   0   all checks passed
...
TIMEOUT="${INBRAIN_VERIFY_TIMEOUT:-120}"
...
if [ -n "${INBRAIN_VERIFY_LOG_DIR:-}" ]; then
  LOG_DIR="$INBRAIN_VERIFY_LOG_DIR"
  mkdir -p "$LOG_DIR" || { echo "ERROR: cannot create $LOG_DIR" >&2; exit 2; }
else
  LOG_DIR="$(mktemp -d /tmp/inbrain-verify-XXXXXX)"
