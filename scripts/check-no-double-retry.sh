...
# arrow-fn form and any direct invocation. (inbrain-allow-direct-insert: doc comment)
...
if grep -rEn "$PATTERN" src/ --include='*.ts' 2>/dev/null; then
  ...
  echo "       ~/.inbrain/audit/batch-retry-YYYY-Www.jsonl; check"
  echo "       \`inbrain doctor\` for the batch_retry_health surface."
  exit 1
fi
