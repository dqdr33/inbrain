...
#
# Runs `inbrain doctor --json` against this repo's own skills/ and parses
...
# exits 1 when no ~/.inbrain/config.json exists (the CI runner's case — no
...
# parse.
TMPOUT="$(mktemp -t inbrain-doctor-XXXXXXXX)"
# shellcheck disable=SC2064
trap "rm -f \"$TMPOUT\"" EXIT

INBRAIN_SKILLS_DIR="$ROOT/skills" bun run src/cli.ts doctor --fast --json >"$TMPOUT" 2>/dev/null || true
...
case "$STATUS" in
  ...
  warn)
    ...
    echo "  INBRAIN_SKILLS_DIR=\"\$(pwd)/skills\" bun run src/cli.ts doctor"
    echo
    ...
    echo "  3. Run 'inbrain doctor --fix' to auto-add the canonical callout"
    exit 1
    ;;
  fail)
