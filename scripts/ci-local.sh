...
# 5432 manual inbrain-test-pg, 5433 commonly held by sibling projects).
# INBRAIN_CI_PG_PORT defines BASE; shards take BASE..BASE+3.
PG_PORT_BASE="${INBRAIN_CI_PG_PORT:-5434}"
for shard in 1 2 3 4; do
  ...
  if [ -n "$PORT_OWNER" ]; then
    ...
    echo "[ci-local] Either stop that container or run with: INBRAIN_CI_PG_PORT=NNNN bun run ci:local" >&2
    exit 1
  fi
  if lsof -iTCP:"$port" -sTCP:LISTEN -P -n >/dev/null 2>&1; then
    echo "[ci-local] ERROR: host port $port (shard $shard) is held by a non-docker process." >&2
    echo "[ci-local] Run with: INBRAIN_CI_PG_PORT=NNNN bun run ci:local" >&2
    exit 1
  fi
done
export INBRAIN_CI_PG_PORT="$PG_PORT_BASE"
export INBRAIN_CI_PG_PORT_2=$((PG_PORT_BASE + 1))
export INBRAIN_CI_PG_PORT_3=$((PG_PORT_BASE + 2))
export INBRAIN_CI_PG_PORT_4=$((PG_PORT_BASE + 3))
...
if [ -z "$SELECTED" ]; then
  ...
  DATABASE_URL=postgresql://postgres:postgres@postgres-1:5432/inbrain_test echo "$SELECTED" | xargs bash scripts/run-e2e.sh
fi'
  else
    RUN_PHASES_CMD='echo "[runner] guards + typecheck"
bash scripts/check-jsonb-pattern.sh
...
DATABASE_URL=postgresql://postgres:postgres@postgres-1:5432/inbrain_test bash scripts/run-e2e.sh'
  fi
else
...
export INBRAIN_PGLITE_SNAPSHOT=test/fixtures/pglite-snapshot.tar
...
printf '%s\\n' 1 2 3 4 | xargs -P4 -I{} sh -c '
  ...
  if [ -s /tmp/e2e-selected.txt ]; then
    SHARD=\${shard}/4 \\
    DATABASE_URL=postgresql://postgres:postgres@postgres-\${shard}:5432/inbrain_test \\
