 ...
 *
 * Use this manually during dev / QA / after `inbrain doctor --fix` runs
...
function main(): number {
  ...
  if (!process.env.OPENCLAW_WORKSPACE && !process.env.INBRAIN_SKILLS_DIR) {
    process.stderr.write(
      '[live-brain-first] No skills dir source set. Set OPENCLAW_WORKSPACE or INBRAIN_SKILLS_DIR.\n' +
      '  $OPENCLAW_WORKSPACE=~/.openclaw/workspace bun run scripts/live-brain-first-check.ts\n',
    );
  ...
  if (!detected || !detected.dir || !existsSync(detected.dir)) {
    process.stderr.write(
      `[live-brain-first] Could not resolve skills dir from OPENCLAW_WORKSPACE / INBRAIN_SKILLS_DIR / cwd walk-up.\n`,
    );
    return 2;
  }
  ...
  if (jsonMode) {
    ...
    if (violators.length === 0) {
      ...
      console.log('  inbrain doctor --fix       # auto-add canonical Convention callout (writes files)');
      console.log('  inbrain doctor --fix --dry-run   # preview without writing');
