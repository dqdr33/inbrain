...
# `inbrain rebuild` (v0.32.3) would lose the data because the fence
# wasn't updated.
#
...
# `// inbrain-allow-direct-insert: <reason>` on the SAME LINE as the
...
#   1. $INBRAIN_SCAN_ROOT explicit override — tests pass this so they
...
if [ -n "${INBRAIN_SCAN_ROOT:-}" ]; then
  ROOT="$INBRAIN_SCAN_ROOT"
else
...
#   2. Does NOT contain the `inbrain-allow-direct-insert:` comment
...
violations=$(
  for dir in "${SCOPE_DIRS[@]}"; do
    ...
    | grep -vE 'inbrain-allow-direct-insert:' \
    | grep -vE ':[[:space:]]*\*[[:space:]]+' \
    | grep -vE ':[[:space:]]*//' \
    | grep -vE '`[^`]*\\.\w+\(' \
    || true
...
if [ -n "$violations" ]; then
  ...
  echo "       carry an explicit \`// inbrain-allow-direct-insert: <reason>\`"
