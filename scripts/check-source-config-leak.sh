...
# NOT every variable named "config" (which would catch global inbrain config).
#
# The risk pattern is `JSON.stringify(<srcVar>.config)` where srcVar holds
# a row from the sources table. Variables that hold the GLOBAL inbrain
...
# handle the inbrain global config, which is a different object).
