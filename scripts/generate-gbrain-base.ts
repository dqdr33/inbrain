#!/usr/bin/env bun
// v0.38 codegen — emit `inbrain-base.yaml` from source-of-truth constants.
//
// Purpose: inbrain-base IS the v0.38 reproduction of pre-v0.38 hardcoded
...
//
// And emits the canonical inbrain-base.yaml. Determinism contract (T25 +
...
//
// Usage:
//   bun scripts/generate-inbrain-base.ts --check     # CI validation
//   bun scripts/generate-inbrain-base.ts --diagnose  # show drift report
//
// Exit codes:
//   0 = inbrain-base.yaml is consistent with source constants
//   1 = drift detected; inbrain-base.yaml needs hand-update
//   2 = script error
...
const BASE_PATH = join(REPO_ROOT, 'src/core/schema-pack/base/inbrain-base.yaml');
...
function fail(msg: string): never {
  console.error('[inbrain-base-codegen] FAIL:', msg);
  process.exit(1);
}

function ok(msg: string): void {
  console.log('[inbrain-base-codegen] OK:', msg);
}

console.log('[inbrain-base-codegen] checking', BASE_PATH);
...
if (manifest.name !== 'inbrain-base') {
  fail(`expected name=inbrain-base, got ${manifest.name}`);
}
if (manifest.extends !== null) {
  fail(`inbrain-base must have extends: null (got ${JSON.stringify(manifest.extends)})`);
}

ok(`loaded inbrain-base v${manifest.version}; ${manifest.page_types.length} page types, ${manifest.link_types.length} link verbs`);
...
if (missing.length > 0) {
  if (diagnoseMode) console.error('[inbrain-base-codegen] missing seed types:', missing);
  fail(`inbrain-base.yaml is missing ${missing.length} page_types from ALL_PAGE_TYPES: ${missing.join(', ')}`);
}
if (extra.length > 0) {
  ...
  console.warn('[inbrain-base-codegen] WARN: extra types in inbrain-base.yaml not in ALL_PAGE_TYPES:', extra);
}

ok(`all ${ALL_PAGE_TYPES.length} seed types present in inbrain-base.yaml`);
...
console.log('[inbrain-base-codegen] PASS — inbrain-base.yaml is consistent');
process.exit(0);
