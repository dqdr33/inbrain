...
#   INBRAIN_TEST_SHARD_TIMEOUT    per-shard wallclock cap, seconds (default 600)
#   INBRAIN_TEST_MAX_CONCURRENCY  passed through to bun test (default 4)
#
...
INTRA_CONC="${MAX_CONCURRENCY_OVERRIDE:-${INBRAIN_TEST_MAX_CONCURRENCY:-4}}"
...
# INBRAIN_TEST_SHARD_TIMEOUT=N.
SHARD_TIMEOUT="${INBRAIN_TEST_SHARD_TIMEOUT:-1500}"
...
if mkdir -p .context/test-shards 2>/dev/null; then
  ...
  LOG_DIR="/tmp/inbrain-test-shards-$$"
  FAILURES_LOG="/tmp/inbrain-test-failures.log"
  SUMMARY_FILE="/tmp/inbrain-test-summary.txt"
...
# Seen on the inbrain-ai/port-pr-1406 PR, 2 CI runs in a row, 6 orphans
