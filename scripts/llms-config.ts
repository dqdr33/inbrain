 ...
 *   LLMS_REPO_BASE=https://raw.githubusercontent.com/fork-org/inbrain/main bun run build:llms
 */
...
export const PROJECT = {
  name: "Inbrain",
  summary:
    "Inbrain is a personal knowledge brain and GStack mod for agent platforms. Pluggable engines (PGLite default, Postgres+pgvector for scale), contract-first operations, 26 fat-markdown skills. Teaches agents brain ops, ingestion, enrichment, scheduling, identity, and access control.",
  repoUrl: "https://github.com/inbrain-ai/inbrain",
  rawBaseUrl:
    process.env.LLMS_REPO_BASE ??
    "https://raw.githubusercontent.com/inbrain-ai/inbrain/master",
};
...
export const SECTIONS: DocSection[] = [
  {
    ...
    entries: [
      ...
      },
      {
        ...
        description:
          "Per-file index for the inbrain repo: what each src/ file does + its load-bearing invariants. The on-demand detail CLAUDE.md's reference map routes to.",
  ...
  },
  {
    ...
    entries: [
      ...
      {
        title: "docs/INBRAIN_RECOMMENDED_SCHEMA.md",
        description:
          "MECE directory structure (people/, companies/, concepts/).",
        path: "docs/INBRAIN_RECOMMENDED_SCHEMA.md",
      ...
      },
      {
        ...
        description:
          "5-minute walkthrough: fork the bundled pack, add a custom `researcher` type, backfill existing pages via `inbrain schema sync --apply`, prove the T1.5 wiring via `inbrain whoknows` (v0.40.7.0).",
      ...
      },
      {
        ...
        description:
          "Deploying the inbrain jobs worker: crontab + watchdog, inline --follow, systemd/Procfile/fly.toml, upgrade checklist.",
      ...
      },
      {
        ...
        description:
          "Three-tier architecture for agents with 300+ skills: always-loaded, resolver-routed, and dormant. Per-turn token math, the v0.41.7.0 compact list-format resolver, and the `inbrain doctor` safety net. 306 skills, ~21K tokens freed per turn, zero capability loss.",
  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        description:
          "Local reranker via llama.cpp --reranking: Qwen3-Reranker or self-hosted ZE weights, --alias setup, inbrain config keys, cold-start timeout, budget-cap interaction.",
...
export const INLINE_TIPS = [
  "`inbrain doctor [--json] [--fast] [--fix]` - built-in health checks.",
  "`inbrain orphans [--json]` - pages with zero inbound wikilinks.",
  "`inbrain repair-jsonb [--dry-run]` - repair v0.12.0 double-encoded JSONB rows.",
  "`inbrain upgrade` runs post-upgrade + apply-migrations.",
];
