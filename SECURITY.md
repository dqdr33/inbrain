...
If you discover a security issue in Inbrain, please report it privately by opening
a [private security advisory](https://github.com/inbrain-ai/inbrain/security/advisories/new)
on GitHub.
...
If you deploy Inbrain's MCP server behind an HTTP wrapper with OAuth 2.1
...
### Recommended: `inbrain serve --http`

As of v0.22.7, Inbrain ships a built-in HTTP transport that uses the
existing `access_tokens` table for authentication:

```bash
# Create a token
inbrain auth create "my-client"

# Start the HTTP server
inbrain serve --http --port 8787

# Connect via ngrok, Tailscale, or any tunnel
ngrok http 8787 --url your-brain.ngrok.app
```

This is the recommended way to expose Inbrain remotely. No OAuth, no
registration endpoint, no self-service tokens. Tokens are managed
exclusively via `inbrain auth create/list/revoke`.
...
The recommended hardening posture above is: ship `inbrain serve --http`
**without** `--enable-dcr` and pre-register every client manually. As of
v0.41.3, `inbrain auth register-client` accepts the OAuth fields
browser-based clients need:

```bash
# Pre-register claude.ai (confidential client; two redirect URIs)
inbrain auth register-client claude-ai \
  --scopes "read write" \
...
inbrain auth register-client chatgpt \
  --scopes "read write" \
...
```bash
inbrain auth create "claude-desktop"   # Create a new token
inbrain auth list                       # List all tokens
inbrain auth revoke "claude-desktop"    # Revoke a token
inbrain auth test <url> --token <tok>   # Smoke-test a remote server
```
...
## `inbrain serve --http` hardening (v0.22.7+)
...
`inbrain serve --http` listens on `127.0.0.1` by default. Personal-laptop
...
`inbrain serve --http` requires a Postgres engine. PGLite is local-only by
design and the `access_tokens` / `mcp_request_log` tables don't exist in
the PGLite schema. Local agents continue to use stdio (`inbrain serve`).
...
```bash
INBRAIN_HTTP_CORS_ORIGIN=https://claude.ai inbrain serve --http --port 8787
# Multiple origins: comma-separated
INBRAIN_HTTP_CORS_ORIGIN=https://claude.ai,https://your.app inbrain serve --http
```
...
`INBRAIN_HTTP_CORS_ORIGIN`, surfacing the default-deny posture before the
first request.
...
|---|---|---|---|
| Pre-auth IP | Before the DB lookup, on every `/mcp` request | 30 req / 60s | `INBRAIN_HTTP_RATE_LIMIT_IP` |
| Post-auth token | After a valid token is resolved | 60 req / 60s | `INBRAIN_HTTP_RATE_LIMIT_TOKEN` |
| LRU cap | Maximum distinct keys across both buckets | 10000 | `INBRAIN_HTTP_RATE_LIMIT_LRU` |
...
INBRAIN_HTTP_TRUST_PROXY=1 inbrain serve --http --port 8787

# Trust N hops — Cloudflare → nginx → inbrain
INBRAIN_HTTP_TRUST_PROXY=2 inbrain serve --http --port 8787

# Disable entirely — direct-exposure deployment with no proxy
INBRAIN_HTTP_TRUST_PROXY=0 inbrain serve --http --port 8787

# Named Express modes (uniquelocal, linklocal) or CIDR lists pass through
INBRAIN_HTTP_TRUST_PROXY=uniquelocal inbrain serve --http --port 8787
INBRAIN_HTTP_TRUST_PROXY="10.0.0.0/8,192.168.1.0/24" inbrain serve --http --port 8787
```
...
1. inbrain is reachable only via a trusted reverse proxy (not directly
   exposed to the internet on the configured port). As of v0.34
   `inbrain serve --http` binds `127.0.0.1` by default, so the
   reverse-proxy-only posture is the out-of-the-box shape; only
   override with `--bind 0.0.0.0` (or a specific interface IP) when
   inbrain itself needs to accept remote connections directly.
...
If inbrain is reachable directly AND `INBRAIN_HTTP_TRUST_PROXY=1` (or any
...
```bash
INBRAIN_HTTP_MAX_BODY_BYTES=2097152 inbrain serve --http   # 2 MiB
```
...
raw payloads back can pass `inbrain serve --http --log-full-params` (loud
