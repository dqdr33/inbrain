# Inbrain Installation Guide for AI Agents
...
- `https://raw.githubusercontent.com/inbrain-ai/inbrain/master/AGENTS.md` — start here
- `https://raw.githubusercontent.com/inbrain-ai/inbrain/master/llms.txt` — full doc map
- `https://raw.githubusercontent.com/inbrain-ai/inbrain/master/llms-full.txt` — same map, inlined

## Step 1: Install Inbrain

Default path (Bun is required — inbrain is a Bun + TypeScript runtime):
...
Verify: `inbrain --version` should print a version number. If `inbrain` is not found,
restart the shell or add the PATH export to the shell profile.

> **If `bun install -g` aborts or `inbrain doctor` reports `schema_version: 0`** (Bun
...
> [#218](https://github.com/inbrain-ai/inbrain/issues/218). Run `inbrain apply-migrations --yes`
> to recover. If that doesn't work, fall back to the deterministic install path:
>
> ```bash
> git clone https://github.com/inbrain-ai/inbrain.git ~/inbrain && cd ~/inbrain
> bun install && bun link
> ```

## Step 2: API Keys

Ask the user for these. inbrain defaults to the ZeroEntropy embedding + reranker stack
(as of v0.36.2.0); OpenAI/Voyage are still supported as fallbacks via `inbrain config
...
Save to shell profile or `.env`. Keys are picked up by `inbrain config set` automatically
or can be stored in `~/.inbrain/config.json` (file plane). Without any embedding provider,
keyword search still works. Without Anthropic, search works but skips query expansion.

## Step 3: Create the Brain

```bash
inbrain init                           # PGLite, no server needed
inbrain doctor --json                  # verify all checks pass
```
...
Read `~/inbrain/docs/INBRAIN_RECOMMENDED_SCHEMA.md` and set up the MECE directory
...
`inbrain init` auto-applied a default search mode (`tokenmax` unless your subagent
...
> Your inbrain just installed with search mode `<auto-applied default>`. This is
...
```bash
inbrain config set search.mode <mode>
```

If they pick tokenmax AND want to preserve the literal v0.31.x default
(limit=20 instead of tokenmax's 50), also run:
```bash
inbrain config set search.searchLimit 20
```

Verify the choice with `inbrain search modes` before continuing.
...
```bash
inbrain import ~/brain/ --no-embed     # import markdown files
inbrain embed --stale                  # generate vector embeddings
inbrain query "key themes across these documents?"
```
...
After this step:
- `inbrain graph-query <slug> --depth 2` works (relationship traversal)
...
at `projects/struktura.md` in another — Inbrain does NOT connect those by default.
...
```bash
inbrain config set link_resolution.global_basename true
inbrain extract links --source db          # re-run so the new edges land
```

`inbrain doctor` surfaces a `link_resolution_opportunity` hint with the exact count
...
both `projects/struktura` and `archive/struktura`), Inbrain emits one edge to each
rather than guessing a winner — review and prune the duplicates with
`inbrain graph-query <slug>`. The mode is also honored on the filesystem-walk path
(`inbrain extract links` with no `--source db`) and by auto-link on every future
`put_page`.
...
inbrain skillpack scaffold --all       # copy 43 curated skills + RESOLVER.md
```

Scaffolded skills are first-class files in your repo. Edit freely; re-running scaffold
refuses to overwrite anything that exists. Use `inbrain skillpack reference <name>` to
diff against inbrain's bundle when you want upstream improvements. (The legacy
`inbrain skillpack install` managed-block model was retired in v0.36.0.0 — run
`inbrain skillpack migrate-fence` once if upgrading from an older release.)

Whether you scaffolded or not, read `skills/RESOLVER.md` (in your workspace, or the
bundled copy at `~/inbrain/skills/RESOLVER.md` when running from the cloned repo). It's
...
platform glue entirely with `inbrain autopilot --install` (built-in self-maintaining daemon):

- **Live sync** (every 15 min): `inbrain sync --repo ~/brain && inbrain embed --stale`
  — or `inbrain sync --watch` for a continuous loop.
- **Auto-update** (daily): `inbrain check-update --json` (tell user, never auto-install).
- **Dream cycle** (nightly): `inbrain dream` runs the 8-phase overnight maintenance cycle.
  ...
  full protocol.
- **Weekly**: `inbrain doctor --json && inbrain embed --stale`

## Step 8: Integrations

Run `inbrain integrations list`. Each recipe in `~/inbrain/recipes/` is a self-contained
...
Verify: `inbrain integrations doctor` (after at least one is configured)

## Step 9: Verify

Read `docs/INBRAIN_VERIFY.md` and run all 7 verification checks. Check #4 (live sync
...
```bash
inbrain upgrade                        # self-updates the binary, runs schema migrations,
...
```bash
cd ~/inbrain && git pull origin master && bun install
inbrain apply-migrations --yes         # apply schema migrations (idempotent)
inbrain post-upgrade                   # show migration notes for the version range
```

Then read `~/inbrain/skills/migrations/v<NEW_VERSION>.md` (and any intermediate
...
created before v0.32.3, `inbrain post-upgrade` prints a banner including the
...
v0.31.x retrieval shape), then run `inbrain config set search.mode <mode>`. See
...
`inbrain extract links --source db && inbrain extract timeline --source db` to
backfill the new graph layer (see Step 4.5 above).

For v0.12.2+ specifically: if your brain is Postgres- or Supabase-backed and
predates v0.12.2, the `v0_12_2` migration runs `inbrain repair-jsonb`
automatically during `inbrain post-upgrade` to fix the double-encoded JSONB
columns. PGLite brains no-op. If wiki-style imports were truncated by the old
`splitBody` bug, run `inbrain sync --full` after upgrading to rebuild
`compiled_truth` from source markdown.

## v0.42.0+ onboard surface (NEW)

`inbrain onboard` is the activation surface inbrain did not have before.
Once your brain has any content, run `inbrain onboard --check --json` to
...
**On first connect (after `inbrain init`):**
```bash
inbrain onboard --check --json
```
...
```bash
inbrain onboard --auto --max-usd 5
```
...
- `inbrain takes extract --from-pages` sends concept/atom/lore/briefing/
...
```bash
export INBRAIN_NO_ONBOARD_NUDGE=1
```
