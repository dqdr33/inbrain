...
#
# Spawns N concurrent `inbrain sync` processes against one DB; asserts:
#   1. Exactly one wins the writer lock (`inbrain-sync` row in `inbrain_cycle_locks`).
...
#   3. After all processes exit, zero leaked `inbrain_cycle_locks` rows remain.
#
...
# and snapshot the wrong table (`pg_locks` instead of `inbrain_cycle_locks`).
...
if [ -z "${DATABASE_URL:-}" ]; then
  ...
  echo "  Local: docker run -d --name inbrain-test-pg -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=inbrain_test -p 5434:5432 pgvector/pgvector:pg16" >&2
  echo "  Then: export DATABASE_URL=postgresql://postgres:postgres@localhost:5434/inbrain_test" >&2
  exit 0
fi
...
# Isolate from the developer's real ~/.inbrain so writing sync.repo_path doesn't
# clobber their config. Restored on exit.
TMP_INBRAIN_HOME=$(mktemp -d -t inbrain-sync-lock-home-XXXXXX)
export INBRAIN_HOME="$TMP_INBRAIN_HOME"
LOG_DIR="$INBRAIN_HOME/audit"
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/heavy-sync_lock_regression-$TS.log"
# Surface the log path so it survives the EXIT trap that nukes INBRAIN_HOME.
SURFACE_LOG="${TMPDIR:-/tmp}/heavy-sync_lock_regression-$TS.log"
trap 'rm -rf "$TMP_INBRAIN_HOME"; cp -f "$LOG" "$SURFACE_LOG" 2>/dev/null || true' EXIT
...
echo "[sync_lock_regression] init schema via inbrain doctor..." | tee -a "$LOG"
...
BRAIN_DIR=$(mktemp -d -t inbrain-sync-lock-XXXXXX)
# Compose with the earlier INBRAIN_HOME-cleanup trap (NOT overwrite it).
trap 'rm -rf "$BRAIN_DIR" "$TMP_INBRAIN_HOME"; cp -f "$LOG" "$SURFACE_LOG" 2>/dev/null || true' EXIT
...
# Tell inbrain to use this brain dir. v0.41 introduced the source registry
...
# Step 5: assert no leaked inbrain_cycle_locks rows. The pkey column is `id`,
# not `lock_id` (column name confirmed via \d inbrain_cycle_locks).
LEAKED=$(psql "$DATABASE_URL" -t -A -c "SELECT COUNT(*) FROM inbrain_cycle_locks WHERE id = 'inbrain-sync';" 2>>"$LOG" | tr -d ' ')
echo "[sync_lock_regression] post-run inbrain_cycle_locks(inbrain-sync) row count: $LEAKED" | tee -a "$LOG"
...
if [ "$LEAKED" != "0" ]; then
  echo "[sync_lock_regression] FAIL: $LEAKED leaked inbrain_cycle_locks(inbrain-sync) row(s) after all syncs exited" >&2
  FAIL=1
fi
