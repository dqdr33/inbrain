...
#   1. Bringing a fresh DB to LATEST shape (via `inbrain doctor` which triggers initSchema)
...
# Step 2: bring to LATEST via inbrain. The CLI's `doctor --json` triggers
...
echo "[build_legacy_fixtures] initializing to LATEST via inbrain doctor..."
