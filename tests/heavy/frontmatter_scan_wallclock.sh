...
#   1. `inbrain doctor` exits 0 OR 1 (some warns OK), NOT killed by timeout.
...
# Isolate from the developer's real ~/.inbrain. Each run uses a fresh tmpdir.
TMP_INBRAIN_HOME=$(mktemp -d -t inbrain-fm-wallclock-home-XXXXXX)
export INBRAIN_HOME="$TMP_INBRAIN_HOME"
BRAIN_DIR=$(mktemp -d -t inbrain-fm-wallclock-brain-XXXXXX)
...
trap 'cp -f "$LOG" "$SURFACE_LOG" 2>/dev/null || true; rm -rf "$TMP_INBRAIN_HOME" "$BRAIN_DIR"' EXIT
...
timeout 120s bun run src/cli.ts init --pglite --yes --no-embedding >> "$LOG" 2>&1 || {
  echo "[fm_wallclock] FAIL: inbrain init exited non-zero" >&2
  echo "Log tail:" >&2
  tail -30 "$LOG" >&2
  exit 1
}

# Register the brain dir as a source. Use raw SQL since `inbrain sources add`
...
# Step 3: run inbrain doctor; capture wall-clock + exit + frontmatter_integrity status.
echo "[fm_wallclock] running inbrain doctor (budget ${WALLCLOCK_BUDGET_S}s)..." | tee -a "$LOG"
