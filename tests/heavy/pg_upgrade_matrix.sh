...
# fixture, then run `inbrain doctor` and assert it reaches LATEST cleanly.
#
...
# wedges, not single-layer bootstrap regressions. inbrain has a multi-layer
...
LOG_DIR="${INBRAIN_HOME:-$HOME/.inbrain}/audit"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/heavy-pg_upgrade_matrix-${TS}.log"

# Each entry runs build_legacy_fixtures.sh <shape> + inbrain doctor.
...
for SHAPE in "${SHAPES[@]}"; do
  ...
  # Step 2: walk forward. `inbrain doctor` triggers engine.connect() which
  ...
  # 'fail' or 'failures' (inbrain doctor's terminal-failure shape).
