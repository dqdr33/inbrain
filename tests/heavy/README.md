...
Heavy ops-shape tests. Shell scripts that exercise inbrain end-to-end against
...
Each script writes a per-run log to `~/.inbrain/audit/heavy-<script>-<ts>.log`
...
- Honor `$INBRAIN_HOME` / `$TMP_ROOT` env overrides where relevant
