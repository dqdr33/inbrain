...
DATABASE_URL=postgresql://postgres:postgres@localhost:5434/inbrain_test bun run test:e2e
...
host port with `INBRAIN_CI_PG_PORT=5435 bun run ci:local` if 5434 collides.
...
Inbrain uses a contract-first architecture. Add your operation to one file and it
...
inbrain captures retrieval traffic so you can replay real queries against
...
1. `eval.capture: true` in `~/.inbrain/config.json` → on
2. `eval.capture: false` in `~/.inbrain/config.json` → off
3. `INBRAIN_CONTRIBUTOR_MODE=1` → on
4. otherwise → off
...
# (or `inbrain doctor` — surfaces silent capture failures cross-process)
```

To disable capture even with the env var set, write
`{"eval": {"capture": false}}` to `~/.inbrain/config.json` — explicit config
...
`search` op handlers — run `inbrain eval replay` against a snapshot of
...
```bash
inbrain eval export --since 7d > baseline.ndjson    # snapshot before your change
# ... make your change ...
inbrain eval replay --against baseline.ndjson       # diff retrieval, get Jaccard@k
```
...
For public benchmark coverage on top of replay, `inbrain eval longmemeval
<dataset.jsonl>` (v0.28.1) runs LongMemEval against inbrain's hybrid
