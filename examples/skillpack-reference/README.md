...
The canonical 10/10 reference for a third-party inbrain skillpack. This
pack ships inside inbrain's own repo at `examples/skillpack-reference/`
and doubles as an integration-test fixture (`inbrain skillpack doctor
...
then run `inbrain skillpack init <your-name>` to scaffold the same
...
```bash
inbrain skillpack scaffold your-user/skillpack-<name>
```

`scaffold` lands the files additively (refuses to overwrite), records
the install in `~/.inbrain/skillpack-state.json` with the pinned commit
...
`inbrain skillpack doctor examples/skillpack-reference --quick` should
always print:
...
the build. That's the invariant — inbrain ships its own bar.
