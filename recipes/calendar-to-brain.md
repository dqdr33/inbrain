...
1. **Inbrain installed and configured** (`inbrain doctor` passes)
...
3. If prompted, configure the OAuth consent screen first:
   ...
   - App name: anything (e.g., 'Inbrain Calendar')
...
# 4. Stores tokens in ~/.inbrain/google-tokens.json
...
### Step 5: Import Calendar Data to Inbrain

```bash
inbrain import brain/daily/calendar/ --no-embed
inbrain embed --stale
```

Verify:
```bash
inbrain search "meeting" --limit 3
```
...
1. **Check brain**: `inbrain search "attendee name"` — do they have a page?
...
```bash
inbrain sync --no-pull --no-embed && inbrain embed --stale
```

### Step 8: Log Setup Completion

```bash
mkdir -p ~/.inbrain/integrations/calendar-to-brain
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","event":"setup_complete","source_version":"0.7.0","status":"ok","details":{"accounts":"ACCOUNT_COUNT","start_year":"YYYY"}}' >> ~/.inbrain/integrations/calendar-to-brain/heartbeat.jsonl
```
