...
6. Sends one alert per cycle to Telegram (or stdout if no Telegram
   ...
   `~/.inbrain/integrations/restart-sweep/alerted.json`.
...
class MessageSweepDetector {
    ...
    constructor(deps = {}) {
        ...
        const inbrainHome = process.env.INBRAIN_HOME ?? path.join(os.homedir(), '.inbrain');
        this.STATE_DIR = path.join(inbrainHome, 'integrations', 'restart-sweep');
...
`~/.inbrain/integrations/restart-sweep/alerted.json` — the sessionKey
...
*/5 * * * * /bin/bash ~/openclaw/scripts/restart-sweep-wrapper.sh >> ~/.inbrain/integrations/restart-sweep/cron.log 2>&1
```
...
```bash
tail -20 ~/.inbrain/integrations/restart-sweep/cron.log
```

## Step 6: Verification

1. `inbrain integrations doctor restart-sweep` — should pass all three
   health checks
2. `~/.inbrain/integrations/restart-sweep/sweep.log.jsonl` exists and
   gets a new entry every 5 minutes
3. `~/.inbrain/integrations/restart-sweep/cron.log` shows successful
...
Check `~/.inbrain/integrations/restart-sweep/alerted.json`. If the
...
- The state file may not be writable. Check `ls -ld
  ~/.inbrain/integrations/restart-sweep/`.
- `INBRAIN_HOME` may be set to a different path under cron than under
...
the OpenClaw repo against `inbrain/minions` (see
`docs/guides/plugin-handlers.md`). Plugin-handler advantages:

- Built-in queue idempotency (no cooldown layer needed)
- Submit via `inbrain jobs submit restart-sweep` from any cron / agent /
  manual trigger
