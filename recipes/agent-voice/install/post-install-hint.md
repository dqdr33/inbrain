...
When `inbrain integrations install agent-voice --target <repo>` completes, print this to stdout (or to the install agent's conversation surface) so the operator knows what to do next.
...
inbrain claw-test --scenario voice-agent-install --live --agent openclaw
...
When inbrain ships a new agent-voice reference, refresh your local copy:

```bash
inbrain integrations install agent-voice --target <target-repo> --refresh
```

The refresh classifies each file (identical / stale / locally-modified / source-deleted / host-deleted) and lets you decide per-file. See `<target>/services/voice-agent/code/install/refresh-algorithm.md` (copied from inbrain) for the contract.
