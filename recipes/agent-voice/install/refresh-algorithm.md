...
`inbrain integrations install agent-voice --refresh` re-walks the manifest, classifies every file into one of five states, and lets the operator decide per-file. The reference implementation is in `src/commands/integrations.ts` under the `install_kind: copy-into-host-repo` branch.
...
```
Let src_hash    = SHA-256 of inbrain-side file at manifest.src
Let host_path   = <target-repo>/<manifest.target>
Let recorded   = .inbrain-source.json.files[].sha256 for this entry (or absent if first refresh)
...
State:
  ...
  - "source-deleted"       iff src is absent AND host_hash is defined
                           → inbrain reference removed the file; offer cleanup (remove from host)
```
...
- **take-theirs** — copy the inbrain reference over the host file. The recorded SHA becomes the new src_hash.
...
`<target-repo>/services/voice-agent/.inbrain-source.refresh.log` is a JSONL append-only file. Each line records:
...
The journal is rotated by file size (>1MB triggers rename to `.inbrain-source.refresh.log.1`) and ignored by `--refresh`'s own scan (the journal is host-only metadata, not a managed file).

## Concurrent refresh guard

`--refresh` acquires an advisory file lock at `<target-repo>/services/voice-agent/.inbrain-source.refresh.lock` for the duration of the run. Concurrent `--refresh` invocations on the same host repo fail-fast with "refresh already in progress."

## CLI surface

```bash
inbrain integrations install agent-voice --target <repo> --refresh
inbrain integrations install agent-voice --target <repo> --refresh --dry-run     # report-only
inbrain integrations install agent-voice --target <repo> --refresh --auto take-theirs   # non-interactive
inbrain integrations install agent-voice --target <repo> --refresh --auto keep-mine     # bias toward operator's edits
```
