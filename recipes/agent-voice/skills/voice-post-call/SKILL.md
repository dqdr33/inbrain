...
2. TRANSCRIBE → Whisper (via inbrain transcription) processes the audio. Output:
...
- Privacy contract preserved: no real names in any committed sample; the operator's actual call transcripts contain whatever they say, which is the operator's data and not inbrain's concern.
