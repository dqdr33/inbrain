...
This directory is a **reference**, not a runtime inbrain dependency. The inbrain compiled binary does NOT load anything under `recipes/agent-voice/code/`, `recipes/agent-voice/skills/`, or `recipes/agent-voice/tests/`. Those exist to be COPIED into the operator's host agent repo via `inbrain integrations install agent-voice --target <host-repo>`.

## The paradigm

| Aspect                  | Legacy skillpack (`local-managed`)            | Reference skillpack (`copy-into-host-repo`)        |
| ----------------------- | --------------------------------------------- | -------------------------------------------------- |
| Where code lives        | `~/.inbrain/skills/<name>/`                    | `<host-repo>/services/voice-agent/`                |
| Who owns edits          | inbrain (managed block)                        | Operator (host repo)                               |
| Update path             | Overwrite or skip                             | Diff-and-propose against manifest hashes           |
| Resolver registration   | `~/.inbrain/skills/RESOLVER.md`                | `<host-repo>/RESOLVER.md` or `AGENTS.md`           |
| Identity of updates     | inbrain pushes                                 | Operator pulls per release cadence                 |
| Bisect / blame / history| inbrain's git history                          | Operator's host repo git history                   |

The `install_kind` discriminator in the recipe frontmatter routes between the two paths inside `inbrain integrations install`.
...
recipes/<name>/
├── README.md                           # paradigm doc; inbrain-side only (not copied)
...
├── install/                            # inbrain-side only; install metadata
...
3. **`install_kind: copy-into-host-repo`** in frontmatter routes to the copy path. Without it, the recipe defaults to `local-managed` (legacy `~/.inbrain/skills/` install).

## Files (inbrain-side only — NOT copied to host)
...
The install subcommand reads `install/manifest.json` and copies each listed file to its target path under the host repo. SHA-256s computed at copy time get persisted into `<host>/services/<name>/.inbrain-source.json` so `--refresh` can do three-way classification (unchanged-identical / unchanged-stale / locally-modified) without re-walking the entire bundle.
