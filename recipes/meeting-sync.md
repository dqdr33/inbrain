...
### Step 4: Import to Inbrain

```bash
inbrain import brain/meetings/ --no-embed
inbrain embed --stale
```
...
2. **For each attendee**, check brain: `inbrain search "attendee name"`
...
6. **Sync**: `inbrain sync --no-pull --no-embed`
...
```bash
mkdir -p ~/.inbrain/integrations/meeting-sync
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","event":"setup_complete","source_version":"0.7.0","status":"ok"}' >> ~/.inbrain/integrations/meeting-sync/heartbeat.jsonl
```
...
The sync script commits and pushes automatically. This triggers Inbrain's
