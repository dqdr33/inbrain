...
3. If prompted, configure the OAuth consent screen:
   ...
   - App name: 'Inbrain Email' (anything works)
...
2. **Check the brain**: `inbrain search "sender name"` — do we have a page?
...
6. **Sync**: run `inbrain sync --no-pull --no-embed` to index changes
...
```bash
mkdir -p ~/.inbrain/integrations/email-to-brain
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","event":"setup_complete","source_version":"0.7.0","status":"ok"}' >> ~/.inbrain/integrations/email-to-brain/heartbeat.jsonl
```
