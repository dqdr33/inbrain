...
3. Create a new Project (name it anything, e.g., 'Inbrain')
...
2. **Check the brain**: `inbrain search "person name"` — do we have a page?
...
```bash
mkdir -p ~/.inbrain/integrations/x-to-brain
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","event":"setup_complete","source_version":"0.8.1","status":"ok","details":{"user_id":"X_USER_ID"}}' >> ~/.inbrain/integrations/x-to-brain/heartbeat.jsonl
```
