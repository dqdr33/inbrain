...
version: 0.8.2
description: "DEPRECATED in v0.40.0.0. New installs use `inbrain integrations install agent-voice` — the copy-into-host-repo paradigm with WebRTC-first browser client + Mars/Venus personas + read-only tool router. This recipe stays for one release as redirect; will be removed in v0.41."
category: sense
...
>
> **Migration:** `inbrain integrations install agent-voice --target <your-repo>` copies a
...
- Step 1 validates prerequisites. If Inbrain isn't configured, nothing else works.
...
Inbrain MCP (semantic search, page reads, page writes)
  ↓ Post-call
...
```bash
# 1. Verify Inbrain is configured
inbrain doctor --json
```
If this fails: "Inbrain isn't set up yet. Let's run `inbrain init --supabase` first."
...
3. Name it something like 'inbrain-voice'
...
- **Slack:** "Create an Incoming Webhook at https://api.slack.com/apps → your app →
  ...
  Validate: `curl -sf -X POST -d '{"text":"Inbrain voice test"}' $WEBHOOK_URL`
- **Discord:** "Go to your server → channel settings → Integrations → Webhooks →
  New Webhook. Copy the webhook URL."
  Validate: `curl -sf -X POST -H "Content-Type: application/json" -d '{"content":"Inbrain voice test"}' $WEBHOOK_URL`
...
The same ngrok account can also serve your Inbrain MCP server (see
...
4. **Tool executor** that:
   - Spawns Inbrain MCP client (`inbrain serve` as stdio child process)
   - Routes function calls: `search_brain` → `inbrain query`, `lookup_person` → `inbrain search` + `inbrain get`
...
5. **Post-call handler** that:
   ...
   - Runs `inbrain sync --no-pull --no-embed` to index the new page
...
- Voice works but no brain page: check post-call handler logs, run `inbrain sync` manually
...
```bash
mkdir -p ~/.inbrain/integrations/twilio-voice-brain
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","event":"setup_complete","source_version":"0.8.1","status":"ok","details":{"phone":"TWILIO_NUMBER","deployment":"local+ngrok"}}' >> ~/.inbrain/integrations/twilio-voice-brain/heartbeat.jsonl
```
...
- Run `inbrain doctor` — if it fails, the database connection is broken
- Check if the post-call handler ran (look in server logs for "transcript saved")
- Run `inbrain sync` manually to force indexing
...
- Upload via `inbrain files upload-raw <audio-file> --page meetings/call-slug --type call-recording`
- Inbrain auto-routes: small files stay in git, large files go to cloud storage
  with `.redirect.yaml` pointer. Files >= 100 MB use TUS resumable upload.
- Generate signed URLs for playback: `inbrain files signed-url <storage-path>`
