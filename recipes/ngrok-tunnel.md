...
Your Inbrain MCP server and voice agent need public URLs so Claude Desktop,
...
Local services (your machine)
  ├── Inbrain MCP server (port 3000)    inbrain serve
...
```bash
mkdir -p ~/.inbrain/integrations/ngrok-tunnel
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","event":"setup_complete","source_version":"0.7.0","status":"ok","details":{"domain":"NGROK_DOMAIN","tier":"hobby"}}' >> ~/.inbrain/integrations/ngrok-tunnel/heartbeat.jsonl
```
...
**Claude Code:**
```bash
claude mcp add inbrain -t http https://your-brain.ngrok.app/mcp \
  -H "Authorization: Bearer YOUR_INBRAIN_TOKEN"
```
...
2. From Claude Desktop, run `inbrain search "test"`. Results should come back.
...
*Part of the [Inbrain Skillpack](../docs/INBRAIN_SKILLPACK.md). See also: [Voice-to-Brain](twilio-voice-brain.md), [Remote MCP Deployment](../docs/mcp/DEPLOY.md)*
