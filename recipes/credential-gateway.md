...
3. If prompted to configure the consent screen:
   ...
   - App name: 'Inbrain' (anything works)
...
```bash
mkdir -p ~/.inbrain/integrations/credential-gateway
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","event":"setup_complete","source_version":"0.7.0","status":"ok","details":{"type":"CLAWVISOR_OR_GOOGLE"}}' >> ~/.inbrain/integrations/credential-gateway/heartbeat.jsonl
```
...
2. **Google OAuth tokens expire.** Access tokens last ~1 hour. The refresh token
   is long-lived but can be revoked. Store both in `~/.inbrain/google-tokens.json`
...
2. **Google OAuth:** Tokens exist at `~/.inbrain/google-tokens.json`.
...
*Part of the [Inbrain Skillpack](../docs/INBRAIN_SKILLPACK.md). See also: [Email-to-Brain](email-to-brain.md), [Calendar-to-Brain](calendar-to-brain.md)*
