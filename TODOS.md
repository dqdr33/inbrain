...
## inbrain#1972 job-layer follow-up (v0.43+)
...
## inbrain#1881 sync reclone ownership follow-ups (v0.43+)

Filed from the #1881 fix (`inbrain sync --strategy code` deleted a user's working
tree; `recloneIfMissing` now only re-clones a clone inbrain OWNS — `config.managed_clone`
...
- [ ] **P2 — `inbrain doctor` misconfigured-source check.** Flag every source row
  ...
  or remove + re-add with `--url` so inbrain owns the clone. **Why:** the core guard
...
- [ ] **P3 — Decide the `--clone-dir`-outside-root policy.** `inbrain sources add --url
  --clone-dir <path>` lets local callers place a inbrain-owned clone anywhere. The
  ownership marker (this PR) makes those safe to reclone, but the dormant
  `clone_dir_outside_inbrain` code in `SourceOpErrorCode` (`sources-ops.ts`) is unused —
  it hints at a previously-intended confinement rule. Decide: either wire it up (forbid
  `--clone-dir` outside `$INBRAIN_HOME/clones/`) or delete the dead code. Don't leave it
...
- [ ] **P2 — Harden the `managed_clone` ownership marker against forgery.** Ownership
  ...
  system symlinks like macOS /var, and an owned clone inbrain created is legitimately
  ...
  `managed_clone` key; (b) bind ownership to an unforgeable on-disk stamp (a `.inbrain-clone`
...
- [ ] **P3 — Sweep orphaned `.inbrain-reclone-*` temp dirs.** The EXDEV-safe reclone clones
  into a sibling temp of `local_path` (`.inbrain-reclone-<leaf>-<rand>`). Every error path
  `rmSync`s it, but a hard crash (SIGKILL/power loss) between clone and swap leaves a full
  clone orphaned next to the user's `--clone-dir` parent — outside inbrain's swept
  `clones/.tmp`. Add a startup/doctor sweep for `.inbrain-reclone-*` / `*.old-*` older than N
  minutes. Codex Medium / Claude Finding 4 from the #1881 ship review.

- [ ] **P3 — CLI `inbrain sources remove` leaks the managed clone dir.** `runRemove`
...
## inbrain#1861 JSONB batch-insert follow-ups (v0.42+)
...
- [ ] **P2 — Ownership state can desync from the shared singleton under
  ...
  connect join a dying/unverified pool. NOT REACHABLE in current inbrain — cycle
...
(PR #1816, now folded into `inbrain-ai/minion-locks-session-pool`). Deliberately
...
- [ ] **P2 — `sync.ts:2491` emits a JSON cost-refusal even without `--json`.** The
  `inbrain sync --all` cost gate has the byte-identical pattern that
...
- [ ] **P3 — `inbrain jobs --help` has no subcommand list.** jobs.ts dispatches
...
Filed from the self-upgrading-inbrain wave. All deliberately scoped OUT (D7a/D7b
...
- [ ] **P2 — Signature/checksum verification before applying an auto-upgrade
  (D7a).** Auto-upgrade currently trusts TLS + GitHub, same as `inbrain upgrade`.
...
- [ ] **P2 — `inbrain serve` host graceful request-drain on auto-upgrade (D7b).**
...
- [ ] **P3 — True binary rollback.** Today a bad release is caught by the
  post-swap `inbrain doctor` gate + recorded in `self_upgrade.failed_versions`
...
- [ ] **P2 — Harden bundled-skill detection.** `getBundledSkillContext`
  ...
  `cwd_walk_up` / `repo_root` / `$INBRAIN_SKILLS_DIR`, `isBundled=false` and the D16 ENFORCE
...
- [ ] **P3 — Add-only extraction never deletes obsolete edges; the watermark now
  asserts a currency it can't fully deliver.** All inbrain extraction is add-only
  ...
  so `inbrain doctor` reports OK while the graph carries a stale edge. Pre-existing
...
- [ ] **P3 — `dream --drain` PGLite lock-path parity.** The drain takes the DB
  ...
  PGLite's exclusive single-process file lock means a separate `inbrain dream
  --drain` process can't even open the brain while autopilot's `inbrain dream`
...
## v0.42.2.0 inbrain connect follow-ups (v0.42+)

- [ ] **T6 (P3): `inbrain connect --env-token` form.** Ship the env-var-indirection
  token form (`-H 'Authorization: Bearer ${INBRAIN_REMOTE_TOKEN}'`, single-quoted so
...
- [ ] **T7 (P3): Tier 2 — local thin-client over a bearer token.** `inbrain connect`
  today only wires the MCP *connection* (Claude Code talks straight to the remote /mcp).
  The local `inbrain` CLI (`inbrain search`, `inbrain remote ping/doctor`, routed ops) still
  ...
  config. Then `inbrain connect --install` can also `bun install -g` inbrain + write the config.
...
- [ ] **P2 — `.inbrain-source` with invalid SYNTAX still falls through silently.**
...
- [ ] **P3 — `inbrain autopilot` CLI daemon pre-guard.** `autopilot.ts:~152`
...
- [ ] **#1605-followup: convert migration backfill-phase spawns to in-process.** v0.41.37.0 made the 9 schema phases (`inbrain init --migrate-only`) run in-process via `runMigrateOnlyCore`, which unblocks `schema_version` advancement on Windows+bun+Supabase. The remaining non-schema spawns (`extract links/timeline`, `repair-jsonb`) still shell out via `runGbrainSubprocess` — they now surface child stderr (so a Windows failure is diagnosable) but still fail on Windows. Convert them to in-process calls (the extract/repair command functions are callable with an engine) so Windows brains complete data backfill, not just schema. Sites: `src/commands/migrations/v0_12_0.ts` (extract), `v0_12_2.ts` (repair), `v0_13_0.ts` (extract). Priority: P2.

- [ ] **#1569-followup: root-cause the 56K-file sync wedge with the reporter's repro.** v0.41.37.0 shipped ReDoS hardening (input-length cap + star-height lint + `--no-schema-pack` escape) + diagnostics (`INBRAIN_SYNC_TRACE=1` begin-heartbeat + PGLite serve/sync concurrency doc), but did NOT root-cause the deterministic wedge at ~3100 files — the reporter's redos-guard hypothesis didn't hold (it's not on the sync path). Get the reporter's sample files (`/tmp/inbrain-hang-sample.txt`, `/tmp/inbrain-prewedge-sample.txt`), reproduce, and pin the resume-mode deep-recursion pre-import phase (prime suspect: the walk/diff/checkpoint path). Priority: P1 once a repro exists; tracked on the #1569 thread.
...
- [ ] **v0.41.37+: routing-eval for the `list_skills` instructional envelope + per-skill `tools:` version-skew validation.** The envelope is load-bearing prose with no eval gate yet; and a skill's declared `tools:` aren't validated against the serving inbrain's actual op set for version drift. Priority: P3.
- [ ] **v0.41.37+: fix malformed `~/.agents/skills/inbrain/.../install/SKILL.md` (missing frontmatter).** Surfaced by codex's own startup error during the v0.41.36.0 plan review — an unrelated stray skill in the agents tree has no `---` frontmatter fence. Not inbrain-repo code; flag/clean separately. Priority: P3.
...
the PrecisionMemBench integration in inbrain-evals). The feature shipped
default-off; these are the gates and extensions before any default flip.

- [ ] **v0.42+: cross-surface ablation before flipping `search.adaptive_return` default.** The gate ships default-off. Before turning it on in any `MODE_BUNDLES` tier, run the recall ablation (adaptive off vs on, recall-preserving caps) across `inbrain eval longmemeval`, `inbrain eval whoknows`, `inbrain eval suspected-contradictions`, and the BrainBench-Real replay (sibling inbrain-evals repo). Confirm recall@k / answer quality does not regress; pick the safe caps; probably flip `tokenmax` first (broadest searchLimit, most noise). On-surface evidence (the PrecisionMemBench precision/recall frontier: off 0.076/0.99, e1/o2 0.40/0.91, e1/o1 0.58/0.82) is recorded in `inbrain-evals/docs/benchmarks/2026-05-29-precisionmembench.md`. Priority: P2.
...
- [ ] **v0.42+: gentle adaptive gate on `think`'s gather stage (A3).** The plan's A3 decision was a gentler return-gate on `runThink`'s gather candidates (cleaner context, fewer tokens per reasoning call). Deferred because the benefit is unvalidated without a longmemeval answer-quality run, and trimming the answer path (even default-off) carries regression risk. gather fuses 4 streams (page / takes-keyword / takes-vector / graph); the gate must operate on the fused output with a higher min-keep than search, validated on `inbrain eval longmemeval` answer quality (not retrieval precision). Also: `RunThinkOpts` has no `sourceId` today, so think's gather runs unscoped (codex finding) — scope-isolated think needs that plumbing first. Priority: P2.
- [ ] **v0.42+: `--explain` human header for adaptive_return.** The decision is in `HybridSearchMeta.adaptive_return` and surfaces in `--json` today. The per-result `explain-formatter.ts` is result-scoped and can't render a per-query meta line; the human `inbrain search --explain` header needs the meta threaded through `cli.ts:formatResult` (it currently only receives `results`). Add a one-line gate-decision header (intent / cap / kept of total). Priority: P3.
- [ ] **v0.42+: structured-alias / facts-mode fidelity for the PrecisionMemBench eval.** The inbrain-evals benchmark seeds beliefs as pages with aliases in the body (real FTS). A second fidelity that exercises inbrain's structured alias/entity-resolution layer (facts with `valid_until` + entity resolution) would measure inbrain's structured-belief path on the 23 alias cases. Lives in inbrain-evals (`eval/precisionmembench/seed.ts` throws on `fidelity:'structured'` today). Priority: P3.
...
- [ ] **v0.42+: lightweight local content-probe phase to keep `newest_content_at` fresh between syncs.**
  ...
  - **Cons:** a new background phase + its own tests + a cadence knob; only
    matters for operators who rely on `inbrain remote doctor` instead of the local
    cron doctor.
...
- [ ] **v0.42+: thin-client `inbrain doctor --source` orphan_ratio scoping.** v0.41.29.0 scopes `orphan_ratio` to `--source` on the LOCAL doctor path (`buildChecks` in `src/commands/doctor.ts`) and closes the `find_orphans` MCP read leak via `sourceScopeOpts(ctx)`. The thin-client / remote doctor path (`src/core/doctor-remote.ts` `runRemoteDoctor`) is a separate code path that does not thread `--source`, so `inbrain doctor --source x` against a remote `inbrain serve --http` brain still reports brain-wide orphan_ratio. Thread the explicit `--source` into the remote doctor request + have the server-side check honor it. Priority: P3 (most users run doctor locally).

- [ ] **v0.42+: widen `check-test-real-names.sh` BANNED_NAMES to catch real-name reintroduction in tests + src.** v0.41.29.0 scrubbed pre-existing real names (`Inbrain Team`, `Alex Graveley`) from `bold-paren-time`'s `test_positive` (and the new `bold-name-no-time` samples), but no automated guard caught them: `check-test-real-names.sh` only scans `test/**` and its BANNED_NAMES list doesn't include `garry tan`; `check-fixture-privacy.sh` only scans `test/fixtures/conversation-formats/`. Add `garry tan` / `inbrain-ai` (and consider extending the scan to `src/core/conversation-parser/builtins.ts` test samples) so future reintroductions fail CI. Priority: P3 (hardening).
...
- **TODO-LR-1 (P2): PR #1567 surrogate-pair fix for synthesize.ts.**
  ...
  - **Source:** community PR #1567, contributor `@inbrain-ai-agents`.

- **TODO-LR-2 (P2): doctor check `lock_renewal_health`.**
  v0.41.26.1 ships the audit JSONL primitive without a doctor read
  surface. For now, `tail -F ~/.inbrain/audit/lock-renewal-*.jsonl` is
  the operator UX.
  ...
  - **Why:** operators on production Supabase want a single `inbrain
...
- **TODO-V13-A (P2): `inbrain dream --max-pages <n>` plumbing.**
...
- **TODO-V13-C (P2): `inbrain pages audit-junk-titles` legacy cleanup.**
  ...
  - **What:** new operator command for soft-deleting pre-existing
    ...
    - Signature: `inbrain pages audit-junk-titles [--source <id>]
...
- **TODO-OPS-1 (P2)**: `inbrain sync print-cron` subcommand. Print the canonical
  ...
  ~80 LOC. Mirrors `inbrain sync --break-lock` argv shape.
...
- **TODO-V19-A (P3)**: Persistent `cycle_runs` table. v0.41.19.0 infers
  ...
  table written from `runCycle` directly would let `inbrain status`
  surface manual `inbrain dream` invocations + per-source partial cycles
...
- **TODO-V19-B (P2)**: Surface `extract_atoms` + `synthesize_concepts`
  ...
  each phase's `details` field; the v0.41.19.0 `inbrain status` cycle
...
- **TODO-V19-C (P3)**: Check-registry refactor for `inbrain doctor`. The
...
- **TODO-V19-E (P2)**: Apply category-aware exit codes to
  `inbrain doctor`. Today doctor exits 0 on all-ok, 1 on any fail.
...
- **TODO-A (P2)**: Pack-aware `linkable: boolean` per-type field on schema-pack
  manifests. Both `inbrain extract links --by-mention` and `--ner` would consult
...
- **TODO-C (P3)**: `inbrain onboard --explain <recommendation_id>` drill-down.
...
- **TODO-F (P3)**: Web UI surface for `inbrain onboard` recommendations in the
...
These were filed during the ship of `inbrain-ai/dar-es-salaam-v1`
...
- [ ] **v0.41.18+: dream execution-concurrency knob via queue-layer
  ...
  subagents run concurrently is `inbrain jobs work --concurrency N` —
  a process-wide setting, not per-invocation. A user running
  `inbrain dream` who wants 5 concurrent synthesize subagents has no
  ...
  `inbrain dream --execution-concurrency N` flag that uses it.
...
- [ ] **v0.41.18+: extract DB-source loops (`extractLinksFromDB`,
  ...
  Wire when an operator hits perf issues running `inbrain extract
...
- [ ] **v0.41.14.0: 9-site progressive-batch retrofit (one commit per site
  ...
  - `src/commands/reindex.ts` (markdown chunker bump) — existing 10s
    Ctrl-C grace + `INBRAIN_NO_REEMBED=1` env map to
    `interactiveAbortMs` + `INBRAIN_PROGRESSIVE_BATCH_DISABLED`.
  ...
  - `src/core/post-upgrade-reembed.ts` — TTY auto-proceed maps directly
    to `INBRAIN_PROGRESSIVE_BATCH_AUTO`.
  ...
  - `src/core/minions/handlers/contextual-reindex-per-chunk.ts` —
    `INBRAIN_PROGRESSIVE_BATCH_AUTO` defaults true for workers.
...
- [ ] **v0.42+: subprocess fan-out for `sync --all` (`--independent` mode
  ...
  isolation with zero new inbrain code. Revisit if shell `timeout` proves
  ...
  revisit, pivot to subprocess-per-source (inbrain CLI spawning inbrain
...
- [ ] **v0.42+: `runFactsBackstop(mode:'queue')` in-process microtask
  ...
  enrichment for synced pages. After `inbrain sync` returns, the CLI
...
- [ ] **v0.42+: atom-slug consolidation migration.** The v0.41.10.1 fix
  ...
  `inbrain atoms consolidate [--dry-run] [--yes]` that groups atoms by
...
- [ ] **TODO-4 (P1) — Measure actual orphan-ratio reduction on representative brain post-merge.** v0.41.10.0 CHANGELOG softens the design-doc claim from "88% → <30%" to "material reduction, exact figure TBD" per codex CK13 (strict-exact + min-length≥4 + no-aliases + no-fuzzy will under-deliver on 3-char real entities like "YC", first-name mentions like "Bob", and abbreviations). After v0.41.10.0 lands, run `inbrain extract links --by-mention` against the production OpenClaw deployment (~165K pages) and capture before/after orphan_ratio from `inbrain doctor --json`. Update `docs/designs/INBRAIN_ONBOARD.md` (in PR #1409 if still open, or as follow-up edit if merged) with the measured number. Update CHANGELOG retroactively only if the measurement is material to user expectations.
...
## v0.41.3 security/MCP fix wave follow-ups (filed during ship of `inbrain-ai/security-mcp-fix-wave`)
...
v0.26.7 auto-RLS event trigger that `inbrain doctor`'s
...
- [ ] **v0.41+: per-key rate-lease caps (`openai:responses`, `google:gemini`, etc.).**
  ...
  + `src/core/minions/handlers/subagent.ts:INBRAIN_ANTHROPIC_MAX_INFLIGHT`.
...
- [ ] **v0.41+: `minion_lease_pressure_log` + budget/self-fix audit retention sweep.**
  ...
  survive `inbrain jobs prune`. Codex pass-3 #5 caught the corollary: without
...
## v0.41.7.0 resolver-parser follow-ups (filed during ship of `inbrain-ai/pr1370-production-ready`)
...
## v0.41 content-sanity follow-ups (filed during ship of `inbrain-ai/lint-page-size-gate`)
...
- [ ] **v0.42 P1 — Source-repo remediation surface.** Codex r1 #7
  ...
  `inbrain sources prune-junk <id>` that walks `local_path`, finds files
  ...
  flagged by inbrain content-sanity`). Operator pushes the commit.
...
- [ ] **v0.41 + 30 days — Threshold default validation post-deploy.**
  ...
  before measuring real corpus distribution. Move: run `inbrain sources
...
- [ ] **v0.42 P2 — Pages soft-delete CLI (`inbrain pages soft-delete
...
- [ ] **v0.41+ — Bytes parity assertion across lint + doctor.** D2
  ...
  fixture page with frontmatter + body, runs `inbrain lint` AND
  `inbrain doctor --content-audit`, asserts both surfaces report
...
- [ ] **v0.41+ — `inbrain sources audit` E2E pin test.** The CLI
...
original 3-item slice: items 1 + 3 (autopilot wiring + `inbrain eval gate`)
shipped as planned + EXPANDED with a correctness gate (qrels-based recall@K
+ first-relevant-hit-rate) and a `inbrain bench publish` verb that closes the
...
- [ ] **P0 — `inbrain eval gate <baseline.ndjson>` for CI.** The single most
...
- [ ] **P0 — Contributor-mode eval capture ON by default with airtight
  ...
  set `INBRAIN_CONTRIBUTOR_MODE=1` produce `eval_candidates` rows. Without
  ...
  banner on first `inbrain init` post-upgrade. Cross-reference the
...
Decision: inbrain commits to being a code-brain peer of dedicated tools,
...
- [ ] **P1 — Cross-file edge resolution (Layer 5 precision upgrade).**
  ...
  TypeScript-first.
- [ ] **P1 — inbrain code-signature retrieval (C6 from v0.21).** "Find every
...
Decision: inbrain commits to first-class non-Latin support. The five
...
- [ ] **P2 — `inbrain costs`** — surfaces per-command, per-source, per-week
...
- [ ] **P2 — `inbrain jobs explain <id>`** — full job-graph trace (parent
  → children → tools called → tokens spent → outcome). Today
  `inbrain agent logs <id>` covers subagents but not the broader job
  graph. (Worker cluster.)
- [ ] **P2 — `docs/security/threat-model.md`** — catalog every untrusted
  boundary in inbrain (MCP, OAuth, capture, sync remote URLs, file_upload,
  ...
  (Safety cluster.)
- [ ] **P3 — `inbrain doctor --thin-client` parity probe** — compares
...
- [ ] **P3 — `inbrain models migrate --from openai:text-embedding-3-large
...
- [ ] **Investigate #1342 — `inbrain sync` hangs after schema v89→v92
  ...
  **Reporter's config**: PGLite, `~/.inbrain/brain.pglite`,
...
- [ ] **v0.42 P1: capture-default flip + scrubber hardening.** Flip
  ...
  `inbrain eval capture off` opt-out hint and persistent
  `eval.capture_acknowledged` config flag (banner fires once per acked-false).
  Two new CLI verbs: `inbrain eval capture on|off|status` + `acknowledge`.
...
- [ ] **v0.42-v0.43 P2: `inbrain bench publish --suggest-thresholds`.**
  Reads the last 30 days of `eval gate` JSON outputs (from inbrain-evals
  CI artifacts or `~/.inbrain/audit/bench-publish-*.jsonl`), computes p10
...
- [ ] **v0.42+ P3: `inbrain bench diff` + `inbrain bench list`.**
  ...
  defaults to `~/.inbrain/baselines/` + `inbrain-evals/baselines/` if both
  exist. Trivial; ship when there's >1 baseline to look at. Filed during
  v0.41 CEO review per D13.

- [ ] **v0.42+: ship the coordinated `inbrain-evals/baselines/v0.41-launch.baseline.ndjson`
  + `inbrain-evals/qrels/v0.41-launch.qrels.json` (hermetic-synthetic per D9).**
  Generate locally via `inbrain bench publish --from <hermetic-test-corpus>` then
  commit to the sibling inbrain-evals repo. Gives `inbrain eval gate` a canonical
...
- [ ] **v0.41+ (optional): extend `checkSyncFreshness` to include `embedding_coverage_pct`
  ...
  paste-ready `inbrain sync --source <id>` hints, warns at 24h, fails at 72h.
...
- [ ] **P3: Debug-log on malformed `search.reranker.model`.** `src/core/search/mode.ts:lookupRerankerRecipeDefaultTimeout`
  ...
  resolve, gated by `INBRAIN_DEBUG=1`.
...
- [ ] **P3: Narrow `resolveLiveRerankerModel` catch.** `src/commands/models.ts:resolveLiveRerankerModel`
  ...
  emit `INBRAIN_DEBUG=1` stderr warning before falling back.
...
- [ ] **v0.41+: contributor-mode CI capture for BrainBench-Real replay gate.**
  ...
  (b) `inbrain eval export` reads `eval_candidates` rows which only populate when
  ops fire through the operation layer with `INBRAIN_CONTRIBUTOR_MODE=1` capture —
  PGLite tests seeded via direct `engine.put*()` produce zero captured rows;
  (c) `inbrain eval replay` re-embeds query text via `gateway.embedQuery()` which
  ...
  `INBRAIN_CONTRIBUTOR_MODE=1 inbrain eval export --tool query` against a seeded
  ...
  a new gate test that calls `inbrain eval replay --against <fixture>` and asserts
...
- [ ] **Codex L1**: `inbrain search stats --days N` underreports for N > 7. audit-writer.ts reads only current + previous ISO week (~14 days). `--days 30` silently shows ~2 weeks of failure events. Fix shape: extend readRecent to walk N/7 weeks dynamically OR cap user input with a clear message.
...
- [ ] **T-todo-3: move fail-open audit events to a DB table for cross-deploy observability.** Codex outside-voice #15 caught the split-brain observability: graph-signals failures land in `~/.inbrain/audit/graph-signals-failures-*.jsonl`, but `inbrain serve --http` deploys can't read the host JSONL. `inbrain search stats` shows error counts on local but not on remote-server brains. Right shape: add a small `event_log` table (or extend an existing one) that the shared `createAuditWriter` writes to alongside the JSONL when an engine is available. Doctor + search-stats read from DB on remote, fall back to JSONL on local. Affects all 6 audit modules (rerank, shell, supervisor, slug-fallback, phantom, graph-signals), so this is a v0.41 audit-infra wave, not a one-off.

- [ ] **T-todo-4: sync-topology-aware cross-source signal.** Codex outside-voice #11 + #15 caught: `cross_source_hits` today counts ANY page in another source as cross-team corroboration, but mirrored imports from another source look identical to genuine cross-team links. Distinguishing them likely needs a `link_source_type` enum extension (e.g. `'mirror'` flag on links created during a `inbrain sources sync`) so the SQL can filter `cross_source_hits` to genuine team-authored edges only.

- [ ] **T-todo-5: replace doctor's 30% global density threshold with actual fire-rate measurement.** `checkGraphSignalsCoverage` in doctor uses % pages with ≥1 inbound link as a proxy for "graph signals fire often enough to matter." Codex outside-voice #14 caught: this is global density, not top-K subgraph density. After 30 days of `inbrain search stats` data accumulates per-query fire rates (T-todo-2 wires this), swap the doctor check to read actual fire-rate-over-window. The 30% threshold becomes "fired in ≥10% of queries in last 7 days" or similar — measured, not inferred.
...
- [ ] **T18 follow-through — DELETE `skills/_brain-filing-rules.{md,json}`.** v0.39.0.0 shipped step (a) of the 4-step deprecation sequence: `inbrain schema show --as-filing-rules` emits the JSON shape the legacy file held. v0.39.1 ships steps (b) + (c) + (d): migrate `filing-audit.ts:79`, `synthesize.ts:619`, `patterns.ts:305`, `check-resolvable.ts:196+:226` to consume `inbrain schema show --as-filing-rules` output; update 5 test files (filing-audit.test.ts, check-resolvable.test.ts, dry-fix.test.ts, resolver.test.ts, cycle-patterns.test.ts); then DELETE the two files. Codex finding #3 from /plan-eng-review made this load-bearing — premature deletion makes protected synthesize/patterns phases fail with NO_ALLOWLIST. Sequencing matters.
...
- [ ] **v0.36.x: Audit trend doctor check `skill_brain_first_trend`.** The
  ...
  `~/.inbrain/audit/skill-brain-first-YYYY-Www.jsonl` records detected /
...
- [ ] **v0.36.x: Run inbrain-side floor-ratio ablation before flipping any mode-bundle default.** v0.35.6.0 ships the gate default-off (`MODE_BUNDLES[*].floor_ratio = undefined`) because the SkyTwin labeled-retrieval ablation that surfaced the regression isn't reproducible on inbrain's own eval surfaces from outside. Before any mode-bundle default flip, run the gate at `floor_ratio: undefined`, 0.85, 0.90, 0.95 across `inbrain eval longmemeval`, `inbrain eval whoknows`, `inbrain eval suspected-contradictions`, and the BrainBench-Real replay (sibling inbrain-evals repo). Quantify per-mode P@k / R@k / nDCG@k / top-1 stability deltas. Look for: regression on queries that genuinely need the long-tail boost (specific entity lookups, low-frequency topics) vs improvement on queries where weak-overlap pages were leapfrogging. The corpus-level finding determines whether tokenmax (most exposure to the failure mode) should flip first, or whether the gate stays a per-call opt-in indefinitely. Filed during v0.35.6.0 codex outside-voice review.
...
- [ ] **v0.36.x: `PREFIX_EXPANSION_DIRS` is hardcoded to `['people', 'companies']` in `src/core/entities/resolve.ts:97`.** New entity directories (funds, advisors, deals, etc.) require a code change to opt in. Consider a config-driven list (`entities.prefix_expansion_dirs: [...]` in `inbrain.yml`) so operators can extend without forking. Filed during v0.35.4.0 plan-eng-review.
...
- [ ] **v0.34.x: Migration v60 idempotency guard against `--force-retry` race with v64.** `inbrain apply-migrations --force-retry 58` after v64 has already run will re-install the FK with `ON DELETE SET NULL`, silently downgrading the v64 RESTRICT posture. Probability low (operator has to explicitly force-retry 58) but failure mode is invisible. Fix: v60 should probe `pg_constraint.confdeltype` before re-adding and refuse to clobber `'r'` (RESTRICT) with `'n'` (SET NULL).
...
- [ ] **v0.34.x: Doctor check `oauth_orphan_source_id`** — surfaces OAuth clients whose source_id was nulled by the v60 D10 silent-widen path (`INBRAIN_ACCEPT_SILENT_WIDEN=1`). Closes the observability gap from v0.34.1's D4 decision. Sibling to the `rls_event_trigger` check pattern in `src/commands/doctor.ts`.

- [ ] **v0.34.x: `inbrain sources purge` FK error UX.** Post-v0.34, deleting a source is refused if any oauth_client references it (v64 ON DELETE RESTRICT). The CLI currently surfaces the raw Postgres FK violation. Fix: pre-check via `SELECT client_id, client_name FROM oauth_clients WHERE source_id = $1`, print "N OAuth clients reference this source: ... Revoke first via `inbrain auth revoke-client <id>`." Mirrors `assessDestructiveImpact` in destructive-guard.ts (v0.26.5).
...
- [ ] **v0.33.x: Dogfood `functional-area-resolver` on inbrain's own `skills/RESOLVER.md`** when it crosses ~12KB (currently 8KB). Apply the pattern to the Operational section first (largest). Filed during v0.32.3.0 CEO review.

- [ ] **v0.33.x: Promote `evals/functional-area-resolver/harness.mjs` to a first-class CLI command** `inbrain routing-eval --ab-compare <variant-dir>`. Removes the one-off harness as maintenance debt; gives every pattern-skill a way to ship its eval. Replaces the placeholder `--llm` flag in `src/core/routing-eval.ts:17-20`. Filed during v0.32.3.0 CEO review.
...
- [ ] **v0.32.x: GitHub Copilot embeddings (#691 originally).** tonyxu-io
  ...
  own design pass: where does the token live? `~/.inbrain/oauth/copilot.json`
  mode 0600 was the v0.32 plan; revisit + write `inbrain auth login copilot`.
...
- [ ] **v0.32.x: OpenAI Codex OAuth chat provider (#698 originally).** perlantir
  ...
  infra: `~/.inbrain/oauth/<provider>.json` + `inbrain auth login <provider>`.
...
- [ ] **v0.33+: Postgres CJK FTS via pgroonga / zhparser / ngram trigrams.**
  ...
  install permission, multi-tenant rollout), so inbrain can't default it.
...
- [ ] **v0.33+: embedding pricing refresh mechanism.** v0.32.7 added
  ...
  a `inbrain prices refresh` skill that diffs against a published canonical
...
- [x] ~~**v0.32.x: interactive provider chooser in `inbrain init`.**~~
...
- [ ] **v0.37+: dedicated migration script for v0.36 broken installs.** v0.37
  ...
  default brains in `inbrain doctor` with a paste-ready repair command. What's
  ...
  `config.embedding_model` + 0 embedded chunks) on `inbrain upgrade` and runs
...
- [ ] **v0.37+: namespaced extension fields for `inbrain config set`.** v0.37
  ...
  argued for a `inbrain.ext.<key>` namespace pattern instead of `--force`
  ...
  keys, polluting `inbrain config show`).
...
- [ ] **v0.38+: env-key typo detection at `inbrain config set` time too.**
  ...
  `inbrain config set` for value-level provider strings (e.g.
  `inbrain config set embedding_model openai:text-embedign-3-large` —
...
- [ ] **v0.38+: extend init env-detection to multimodal explicitly via picker.**
  ...
  `inbrain reindex --multimodal`. The picker doesn't yet have a 'multimodal'
  touchpoint mode — multimodal model selection happens via
  `inbrain config set embedding_multimodal_model` or env detection of
...
### Investigate: `inbrain query <common-keyword>` infinite loop
**Priority:** P1
**Filed:** 2026-05-08 from v0.31.2 bug report (separate from the sync hang).

**Evidence:** Two `bun /Users/inbrain-ai/.bun/bin/inbrain query the` processes
...
`inbrain query the` and watch CPU + RSS. If it pegs and grows, capture
...
- [ ] **v0.31.x: routed-call timing telemetry.** `INBRAIN_TIMING=1` prints
  `token_mint=Xms http=Yms server=Zms total=Wms` per routed MCP call.
  Audit log at `~/.inbrain/audit/routed-calls-YYYY-Www.jsonl`. Cherry-pick
  C from #734 plan; deferred from v0.31.1 to keep scope tight.

- [ ] **v0.31.2: job-submission routing for `inbrain dream` etc.** Route
  long-running ops (`dream`, `embed --stale`, `extract`) via `submit_job`
  + poll, mirroring the existing `inbrain remote ping` autopilot-cycle
...
- [ ] **v0.32.0: flip `inbrain auth register-client` default scope from
...
- [ ] **v0.31.x: cross-process OAuth token cache at
  `~/.inbrain/oauth-token-cache.json`.** Cuts ~200ms cold-start cost for
  shell-loop usage on thin-client installs. Today the in-memory cache is
  per-process; every `inbrain` invocation pays a fresh token mint.
...
[inbrain-evals#main:ced01f0](https://github.com/inbrain-ai/inbrain-evals/blob/main/docs/benchmarks/2026-05-07-longmemeval-s.md).
inbrain-hybrid: 97.60% R@5, beating MemPal raw 96.6% by 1.0pt on the same
...
**What:** inbrain's `links` table + `inbrain extract timeline` already build a
...
beats inbrain-hybrid (96.2% vs 94.7%, -1.5pt). Embeddings carry topic
...
from the question). Maybe ~200 lines + a benchmark line on the inbrain-evals
report once it ships.

**Context:** Per-type breakdown in
`inbrain-evals/docs/benchmarks/2026-05-07-longmemeval-s.md` shows we tie
...
inbrain core. Touches `eval/runner/longmemeval.ts:run()` per-question loop.
...
different inbrain version that re-keys the cache.
...
**Why:** Pushes retrieval into the regime where inbrain's pipeline either
...
(source-boost / hard-exclude) layer of inbrain harder than `_s` does.
...
**Pros:** Useful tradeoff line for users picking inbrain in a cost-sensitive
...
### `inbrain doctor`: warn on misconfigured multimodal model
**Priority:** P2
...
### `inbrain config unset <key>` subcommand (Codex F6 from PR #719 review)
**Priority:** P3
...
**Why:** Once a user runs `inbrain config set X val`, there's no normal CLI path to clear it. Empty string is rejected by the current `set` validator (`action === 'set' && key && value` where value is truthy). PR #719 added another DB-merge key (`embedding_multimodal_model`) and surfaces this UX gap.
...
**What:** Wire `inbrain eval cross-modal` to be invokable as a `inbrain agent run` child job. Today the CLI runs synchronously and bypasses `src/core/minions/rate-leases.ts` because the lease helper requires a `minion_jobs.id` that the CLI path doesn't have (T4=A in plans/radiant-napping-lerdorf.md).

**Why:** Cross-process concurrency cap. A user running `inbrain eval cross-modal` in one terminal alongside `inbrain agent run` in another can hit Anthropic 429s due to combined load. As a minion job, the eval gets the rate-lease behavior for free, plus stagger / quiet-hours / retry surface from the existing Minions queue.
...
**Pros:** Data-driven decision instead of guessing. Lightweight: count receipt files in `inbrainPath('eval-receipts')` against the count of skills under `skills/*/SKILL.md`.
**Cons:** "Adoption stalled" is a judgment call without a baseline. Could become a debate.
**Context:** New check in `inbrain doctor` would surface the count. v0.28.x candidate.
**Depends on:** None.
...
### Token rotation: `inbrain auth rotate <name>` + `rotate_token` MCP op
**Priority:** P2
...
**Context:** Item 4 from the gstack /setup-inbrain v1.28.1.0 enhancement request. v0.28.x candidate.
**Depends on:** Nothing.

### Migration introspection in `get_health`
**Priority:** P3

**What:** Extend `BrainEngine.getHealth()` return shape with `migrations: { pending: [...], wedged: [...] }`. `inbrain doctor` already shows this; expose it via the MCP op so remote agents can detect partial-migration state without invoking `doctor` separately.

**Why:** Closes a remote-diagnostic gap. gstack /setup-inbrain Path 4 hit a wedged-migration brain mid-session; the only readback was SSH + `inbrain doctor`. With this, the same diagnostic flows through MCP.

**Pros:** Pure additive change to the `get_health` op shape. No new op surface. Consumers ignore the new field if they don't care.
**Cons:** Wedged detection logic lives in `inbrain doctor`'s code today; need to extract or duplicate. Care needed not to leak migration internals to non-admin scopes (current op is admin-only — fine).
**Context:** Item 5 from the gstack /setup-inbrain v1.28.1.0 enhancement request.
**Depends on:** Nothing.
...
**Context:** Item 6 from the gstack /setup-inbrain v1.28.1.0 enhancement request.
**Depends on:** Nothing.

### `inbrain sources rebase-clone <id>`
**Priority:** P3
...
**What:** v0.28.2 defaults `inbrain sources add --url` to `--depth=1` (no history). For users who want commit-aware features later (page-state-at-commit-X, blame, who-edited-what), expose `--filter=blob:none` as an opt-in: keeps full graph metadata, lazy-fetches blobs.
...
**What:** `inbrain auth register-client` always passes `[]` for redirect URIs; there is no CLI flag to set them. Operators who want to register an `authorization_code` client without DCR have to hand-edit the database.
...
### F13 — `inbrain serve --http` argv positive-int validator
**Priority:** P3
...
### Adjacent 3 — sources remove + sources purge race against inbrain sync
**Priority:** P3

**What:** `inbrain sources remove <id>` and the new `inbrain sources purge <id>` paths don't acquire `SYNC_LOCK_ID` (the `inbrain-sync` writer lock from PR #490). If `inbrain sync` is mid-import for the same source, the parent row can DELETE while sync is INSERTing children, surfacing as a loud FK violation.
...
**What:** `inbrain auth revoke-client <client_id>` (v0.26.2) lands without an impact preview or `--confirm-destructive` gate. CASCADE-purges every active token + auth code in one transaction; one stray client_id wipes a production integration.
...
**Pros:** Posture consistency — every destructive verb in the inbrain CLI follows one pattern. Operators get the impact preview before nuking a production OAuth client.
...
**What:** E2E tests (`bun run test:e2e`) currently run sequentially in one shared Postgres container, each test file calling `initSchema()` from scratch (~5-20s each on cold init). Speed-up: build the schema ONCE into a template DB (`inbrain_template`), then have each test file `CREATE DATABASE foo TEMPLATE inbrain_template` (~50ms per clone). With per-shard `DATABASE_URL` overrides, E2E can fan out to N parallel shards too.
...
**Sketch:**
1. Build template DB once via `initSchema()` against `inbrain_template`.
2. Per-test-file: `CREATE DATABASE inbrain_test_clone_<n> TEMPLATE inbrain_template` (50ms vs 5-20s).
...
- `test/e2e/doctor.test.ts` (inbrain doctor exits 0 on healthy DB) — possibly related to v0.26.2 schema changes since CHANGELOG mentions extension of doctor checks.
...
**Why:** Cross-agent diff (`inbrain friction diff --base openclaw --compare hermes`) is the highest-leverage next signal. Friction unique to one agent vs common-to-both separates "agent contract bug" from "inbrain bug" automatically.
...
**What:** Three new `inbrain friction` subcommands deferred from v1:
- `inbrain friction diff --base <run-or-agent> --compare <run-or-agent>` (cross-agent comparison; ~80 LOC)
- `inbrain friction trend [--since <version-or-date>] [--phase <name>]` (time-series across runs; ~60 LOC)
- `inbrain friction migration-stub [--threshold N]` (clusters friction by phase + tokens, emits `skills/migrations/v[N+1].md` stub; ~150 LOC)
...
- `supabase-migration` — `inbrain init --pglite` then `inbrain migrate --to supabase`; verifies the cross-engine migration path
...
**Effort:** S (CC ~30m once a v0.18 checkout is handy). Depends on: ability to run a v0.18 inbrain build.

---

### Public scoreboard — `inbrain-evals.io/friction`
**Priority:** P3

**What:** Sibling-repo PR in `inbrain-ai/inbrain-evals` that renders friction JSONL into a public dashboard. Friction count per version per agent, line charts over time. v1's JSONL already includes `inbrain_version` + `agent` tags so the scoreboard is a thin layer on top.
...
### Read-side host-isolation (`$INBRAIN_HOST_HOME`)
**Priority:** P3

**What:** v0.22.16 confined every `~/.inbrain` write site to honor `$INBRAIN_HOME`. But `src/commands/init.ts:299-313` still reads real `~/.claude` / `~/.openclaw` / `~/.codex` / `~/.factory` / `~/.kiro` for module fingerprinting (host detection). Even with write-isolation, a claw-test running on a developer's box discovers their real installed mods. v1.1: add a separate `$INBRAIN_HOST_HOME` override for the read-side detection so the claw-test can run truly hermetic.
...
**Why:** Right now agents only call `inbrain friction log` if they find the protocol skill on their own. The callouts route them there proactively from any harness-exercised skill.
...
unreachable → `inbrain_cycle_locks` row stays held indefinitely. Every
...
3. End-to-end: cycle handler with embed phase + signal aborted mid-flight →
   finally runs → `inbrain_cycle_locks` row deleted
...
If you import this directly from `inbrain/sync`, replace `n` with `result.count`
...
the package exports map. External TS consumers (inbrain-evals, host agent forks)
...
### Concurrent-safe ack of `~/.inbrain/sync-failures.jsonl`
**Priority:** P3

**What:** Two concurrent `inbrain sync` runs hitting `acknowledgeSyncFailures()`
...
### C6 — inbrain code-signature "(A, B) => C"
**Priority:** P3 (stretch)
...
- **Check 5 (trigger routing eval):** `src/core/routing-eval.ts` + `inbrain routing-eval` CLI. Structural layer runs in `check-resolvable` by default; `--llm` opts into LLM tie-break. Fixtures live at `skills/<name>/routing-eval.jsonl`.
...
in the sibling repo [github.com/inbrain-ai/inbrain-evals](https://github.com/inbrain-ai/inbrain-evals):
...
the entire eval harness so inbrain users don't download the ~5MB corpus at install
time. BrainBench is now a public sibling benchmark; inbrain ships clean.
...
### v0.10.4: inbrain alias resolution feature (driven by Cat 3)
**What:** Add an alias table to inbrain so "Sarah Chen" / "S. Chen" / "@schen" / "sarah.chen@example.com" resolve to one canonical entity. Schema: `aliases (id, slug, alias_text)` with a unique index. Search blends alias matches into hybrid scoring.
...
**Why:** v0.13.0 moves shell scripts to Minions but still leaves scheduling in the host crontab. Your OpenClaw's `scripts/service-manager.sh` + crontab is the only piece left on the host side. A DB-driven scheduler would mean a single `inbrain autopilot --install` replaces the host crontab entirely, scheduling is visible via `inbrain jobs list --scheduled`, and downtime-on-one-machine tolerance improves (schedule is shared DB state, not per-host crontab).
...
### `inbrain crontab-to-minions <file>` migration helper (deferred from v0.13.0)

**What:** Parse an existing crontab file, emit a proposed rewrite using `inbrain jobs submit shell ...` for each deterministic entry, keep LLM-requiring entries as-is.
...
**Why:** PGLite's WASM files (~3MB) can't be embedded in the compiled binary. Users who install via `bun install -g inbrain` are fine (WASM resolves from node_modules), but the compiled binary can't use PGLite. Jarred Sumner (Bun founder, YC W22) would likely be receptive.
...
1. `inbrain eval prune --dry-run`: replace the `listEvalCandidates(limit:100k) + filter` count with a real `engine.countEvalCandidatesBefore(date)` method. Today the warning at `eval-prune.ts:107-109` honestly tells the user the count may be undercounted, but a brain with > 100k rows + old data could still confuse a careful operator. New `BrainEngine` method on both engines, ~30 LOC, lifts the floor count to a true count.
...
5. Public-exports canaries: 6 of 17 subpaths (`inbrain` root, `/minions`, `/engine-factory`, `/transcription`, `/backoff`, `/extract`) have `canary: []` — the test only checks the import resolves, so a barrel module accidentally losing its named exports would still pass. Pin one stable canary symbol per subpath.
...
**Why:** Idempotency keys (shipped in v0.11.0) are the foundation. Consumers currently use launchd/cron to fire `inbrain jobs submit`, but a native scheduler inside the worker would be cleaner and portable across deployments.
...
### Minions: `inbrain jobs stats --orphaned` (deferred from v0.13.0)

**What:** New CLI flag / output column surfacing jobs that are waiting with no registered handler on any live worker.

**Why:** v0.13.0 adds shell jobs that require `INBRAIN_ALLOW_SHELL_JOBS=1` on the worker. If an operator submits a shell job but no worker with the flag is running, the row sits in `waiting` silently. The CLI's starvation warning + docs help at submit time; this TODO surfaces the problem at operational-check time.
...
### Community recipe submission (`inbrain integrations submit`)
**What:** Package a user's custom integration recipe as a PR to the Inbrain repo. Validates frontmatter, checks constrained DSL health_checks, creates PR with template.

**Why:** Turns Inbrain from a single-author integration set into a community ecosystem. The recipe format IS the contribution format.
...
### `inbrain serve --http` + Fly.io/Railway deployment
**What:** Add `inbrain serve --http` as a thin HTTP wrapper around the stdio MCP server. Include a Dockerfile/fly.toml for cloud deployment.

**Why:** The Edge Function deployment was removed in v0.8.0. Remote MCP now requires a custom HTTP wrapper around `inbrain serve`. A built-in `--http` flag would make this zero-effort. Bun runs natively, no bundling seam, no 60s timeout, no cold start.

**Pros:** Simpler remote MCP setup. Users run `inbrain serve --http` behind ngrok instead of building a custom server. Supports all 30 operations remotely (including sync_brain and file_upload).

**Cons:** Users need ngrok ($8/mo) or a cloud host (Fly.io $5/mo, Railway $5/mo). Not zero-infra.

**Context:** Production deployments use a custom Hono server wrapping `inbrain serve`. This TODO would formalize that pattern into the CLI. ChatGPT OAuth 2.1 support depends on this.
...
**What:** When `inbrain put` is called with identical content (status=skipped), runAutoLink still does a full getLinks + per-candidate addLink loop. On N identical writes of a 50-entity page that's 50N round trips.
...
**Completed:** v0.26.0 (2026-04-25) — `inbrain serve --http` ships full OAuth 2.1 via MCP SDK's `mcpAuthRouter` + `OAuthServerProvider`. Authorization code flow with PKCE unblocks ChatGPT. Client credentials flow unblocks Perplexity/Claude. Dynamic Client Registration available behind `--enable-dcr` flag (off by default). See `docs/mcp/CHATGPT.md` for connector setup. Closed the P0 that had been blocking the "every AI client" promise since v0.6.
...
**Why:** String matching against error messages breaks on library upgrades (postgres.js could change its error message phrasing without bumping major). Code matching is durable. The Layer 1 cleanup follows: inbrain itself doesn't define connection-error codes; it should defer to postgres.js's classification.
...
**What:** v0.22.7 dropped SSE entirely from `inbrain serve --http` because no
...
`inbrain auth create` doesn't accept a `--scopes` flag, and `dispatchToolCall`
...
### `@inbrain-ai/inbrain` scoped-name npm publishing
**What:** Publish inbrain to npm under the scoped name `@inbrain-ai/inbrain`
instead of the bare `inbrain` name. Provides structural defense against the
unrelated `inbrain@1.x` squatter package on npm.
...
`bun add -g @inbrain-ai/inbrain` cannot collide with any non-`@inbrain-ai` package.
...
on the bare-name install (`bun add -g inbrain` → recovery message pointing
...
## v0.32.6 follow-ups from PR #880 (inbrain-context post-Codex recalibration)
...
(inbrain-evals harness, etc) actually need direct engine import.
