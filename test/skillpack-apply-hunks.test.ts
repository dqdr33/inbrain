...
describe('round-trip: unifiedDiff produces output parseUnifiedDiff + applyHunks consume', () => {
  ...
  it('user-edited file: identical diff applies clean to unrelated section', () => {
    // inbrain bundle and user file both have lines 1-20. Bundle changed
    ...
    const inbrainNew = inbrainOld.replace('line5\n', 'line5 INBRAIN\n');
    const userFile = inbrainOld.replace('line15\n', 'line15 USER\n');

    const diff = unifiedDiff(inbrainOld, inbrainNew);
    const result = applyHunks(userFile, parseUnifiedDiff(diff));

    expect(result.applied).toBe(1);
    expect(result.text).toContain('line5 INBRAIN');
