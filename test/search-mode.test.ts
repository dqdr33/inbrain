...
describe('SEARCH_MODE_CONFIG_KEYS is the full reset surface', () => {
  test('every key starts with search. prefix (inbrain config unset --pattern search.* compatibility)', () => {
  ...
  test('every ModeBundle field has a config key (consistency check)', () => {
    ...
    // intentional regression guard: `inbrain search modes --reset` must clear
    // every knob.
