/**
 * inbrain claw-test CLI dispatch tests.
 *
 ...
 * real inbrain commands (no built binary in CI yet); the canonical scripted
 * E2E that walks `inbrain init → import → query → extract → verify` lives
...
describe('shipped scenarios are loadable', () => {
  test('default fixtures root contains both v1 scenarios', () => {
    delete process.env.INBRAIN_CLAW_SCENARIOS_DIR;
...
describe('friction CLI integrates with harness run-id env', () => {
  test('INBRAIN_FRICTION_RUN_ID populates harness-style run-ids', () => {
    process.env.INBRAIN_FRICTION_RUN_ID = 'claw-test-20260428-fake-abcd1234';
    try {
      const code = runFriction(['log', '--phase', 'install', '--message', 'simulated harness write']);
      expect(code).toBe(0);
      const expectedFile = join(tmp, '.inbrain', 'friction', 'claw-test-20260428-fake-abcd1234.jsonl');
