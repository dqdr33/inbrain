/**
 * v0.36.1.0 (T7) — inbrain calibration CLI + get_calibration_profile MCP op tests.
 *
...
describe('formatProfileText', () => {
  test('null profile prints helpful cold-brain message', () => {
    ...
    expect(out).toContain('inbrain dream --phase calibration_profile');
  });
...
describe('getCalibrationProfileOp (MCP)', () => {
  ...
  test('throws on empty/non-string holder', async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(InbrainError);
      expect((err as InbrainError).problem).toBe('INVALID_HOLDER');
    }
  });
});
