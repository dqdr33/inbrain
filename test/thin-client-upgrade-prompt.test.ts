...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-upgrade-prompt-'));
...
describe('safeCompare', () => {
  ...
  test('4-segment versions parse (4th segment ignored by underlying comparator)', () => {
    ...
    // matches inbrain's actual 3-segment release practice (VERSION file).
...
describe('promptState IO', () => {
  ...
  test('atomic write: tmp file written then renamed', async () => {
    await withEnv({ INBRAIN_HOME: tmpHome }, async () => {
      ...
      const finalPath = join(tmpHome, '.inbrain', 'upgrade-prompt-state.json');
  ...
  test('corrupt JSON falls through to empty state', async () => {
    await withEnv({ INBRAIN_HOME: tmpHome }, async () => {
      // Pre-create the inbrain dir + corrupt file
      const path = join(tmpHome, '.inbrain', 'upgrade-prompt-state.json');
      const dir = join(tmpHome, '.inbrain');
...
describe('acquirePromptLock', () => {
  test('acquire then release', async () => {
    await withEnv({ INBRAIN_HOME: tmpHome }, async () => {
      ...
      const lockPath = join(tmpHome, '.inbrain', 'upgrade-prompt.lock');
...
describe('maybePromptForUpgrade orchestrator', () => {
  ...
  test('no remote_mcp → returns immediately', async () => {
    ...
    await maybePromptForUpgrade({} as InbrainConfig, identity, DEFAULT_CLI_OPTS, false, {
      localVersion: '0.31.4',
