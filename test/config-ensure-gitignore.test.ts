 ...
 * post-upgrade) lays down `~/.inbrain/.gitignore` containing the single line
...
beforeEach(() => {
  testHome = join(tmpdir(), `inbrain-eg-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('ensureGitignore', () => {
  test('creates ~/.inbrain/.gitignore with single * when missing', async () => {
    await withEnv({ INBRAIN_HOME: testHome }, async () => {
      ensureGitignore();
  ...
  test('honors INBRAIN_HOME: writes under $INBRAIN_HOME/.inbrain/, not $HOME', async () => {
    await withEnv({ INBRAIN_HOME: testHome }, async () => {
      ensureGitignore();
      const expected = join(testHome, '.inbrain', '.gitignore');
