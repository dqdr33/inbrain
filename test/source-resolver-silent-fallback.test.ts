...
beforeEach(async () => {
  ...
  cwd = mkdtempSync(join(tmpdir(), 'inbrain-resolver-'));
});

describe('source-resolver silent-fallback tiers (codex P1-F)', () => {
  describe('tier 3 — .inbrain-source dotfile', () => {
    test('valid dotfile content (registered source) is honored', async () => {
      ...
      writeFileSync(join(cwd, '.inbrain-source'), 'alpha\n');
    ...
    test('underscore in dotfile content silently falls through (strict regex rejects)', async () => {
      ...
      writeFileSync(join(cwd, '.inbrain-source'), 'has_underscore\n');
    ...
    test('whitespace-only dotfile content silently falls through', async () => {
      writeFileSync(join(cwd, '.inbrain-source'), '   \n');
      const resolved = await resolveSourceId(engine, null, cwd);
      expect(resolved).toBe('default');
    });

    test('uppercase in dotfile content silently falls through', async () => {
      writeFileSync(join(cwd, '.inbrain-source'), 'DEFAULT\n');
      const resolved = await resolveSourceId(engine, null, cwd);
      expect(resolved).toBe('default');
    });
  });
  ...
  describe('tier 2 — INBRAIN_SOURCE env (throw-on-invalid contract)', () => {
    test('valid env value is honored', async () => {
      await engine.executeRaw(
        `INSERT INTO sources (id, name, config) VALUES ('delta', 'delta', '{}'::jsonb)
         ON CONFLICT (id) DO NOTHING`,
      );
      await withEnv({ INBRAIN_SOURCE: 'delta' }, async () => {
        const resolved = await resolveSourceId(engine, null, cwd);
        expect(resolved).toBe('delta');
      });
    });

    test('underscore in INBRAIN_SOURCE THROWS (tier-2 contract)', async () => {
      await withEnv({ INBRAIN_SOURCE: 'has_underscore' }, async () => {
        await expect(resolveSourceId(engine, null, cwd)).rejects.toThrow(/INBRAIN_SOURCE/);
      });
    });
  });
});
