...
// `inbrain init --migrate-only` subprocess that died with getaddrinfo ENOTFOUND
...
describe('#1605 runMigrateOnlyCore (in-process schema)', () => {
  test('brings a fresh PGLite brain to head without spawning', async () => {
    ...
    const result = await withEnv(
      { INBRAIN_HOME: home, DATABASE_URL: undefined, INBRAIN_DATABASE_URL: undefined },
      () => runMigrateOnlyCore(),
    );
...
describe('#1605 structural guard: schema phases are in-process', () => {
  test('no schema phase still execSyncs `inbrain init --migrate-only`', () => {
  ...
  test('NO migration orchestrator anywhere spawns `inbrain init --migrate-only`', () => {
