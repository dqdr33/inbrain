...
async function startTest(cfg: FakeEngineConfig & { lruCap?: number; ipLimit?: number; tokenLimit?: number; corsOrigin?: string; bodyCap?: number; trustProxy?: boolean } = {}): Promise<TestServer> {
  if (cfg.corsOrigin) process.env.INBRAIN_HTTP_CORS_ORIGIN = cfg.corsOrigin;
  else delete process.env.INBRAIN_HTTP_CORS_ORIGIN;
  if (cfg.bodyCap) process.env.INBRAIN_HTTP_MAX_BODY_BYTES = String(cfg.bodyCap);
  else delete process.env.INBRAIN_HTTP_MAX_BODY_BYTES;
  if (cfg.trustProxy) process.env.INBRAIN_HTTP_TRUST_PROXY = '1';
  else delete process.env.INBRAIN_HTTP_TRUST_PROXY;
...
describe('http-transport: CORS', () => {
  test('10. no INBRAIN_HTTP_CORS_ORIGIN + browser request → no ACAO header', async () => {
