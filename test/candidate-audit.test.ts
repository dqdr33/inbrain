...
// audit trail used by `inbrain schema review-candidates` (v0.39).
//
...
beforeEach(() => {
  auditDir = mkdtempSync(join(tmpdir(), 'inbrain-candidate-audit-'));
});
...
describe('isAuditVerbose', () => {
  test('false when env unset', async () => {
    await withEnv({ INBRAIN_SCHEMA_AUDIT_VERBOSE: undefined }, () => {
      expect(isAuditVerbose()).toBe(false);
    });
  });
  test('true when env=1', async () => {
    await withEnv({ INBRAIN_SCHEMA_AUDIT_VERBOSE: '1' }, () => {
      expect(isAuditVerbose()).toBe(true);
    });
  });
  test('false for any other value', async () => {
    await withEnv({ INBRAIN_SCHEMA_AUDIT_VERBOSE: 'true' }, () => {
      expect(isAuditVerbose()).toBe(false);
    });
    await withEnv({ INBRAIN_SCHEMA_AUDIT_VERBOSE: 'yes' }, () => {
      expect(isAuditVerbose()).toBe(false);
    });
  });
});
...
describe('computeCandidateAuditPath', () => {
  test('honors INBRAIN_AUDIT_DIR override', async () => {
    await withEnv({ INBRAIN_AUDIT_DIR: auditDir }, () => {
...
describe('logCandidate (redacted by default)', () => {
  test('writes sha8 hash, not raw type, when verbose is unset', async () => {
    await withEnv({ INBRAIN_AUDIT_DIR: auditDir, INBRAIN_SCHEMA_AUDIT_VERBOSE: undefined }, async () => {
      await logCandidate({
        type: 'therapy-session',
        slug: 'personal/therapy/2025-03-15-session-12.md',
        frontmatterKeys: ['date', 'therapist', 'mood'],
        packIdentity: 'inbrain-base@1.0.0+aaaaaaaa',
      });
      ...
      expect(record.pack_identity).toBe('inbrain-base@1.0.0+aaaaaaaa');
      expect(record.count).toBe(1);
      expect(record.ts).toMatch(/^\d{4}-\d{2}-\d{2}T/);
    });
  });

  test('writes raw type when INBRAIN_SCHEMA_AUDIT_VERBOSE=1', async () => {
    await withEnv({ INBRAIN_AUDIT_DIR: auditDir, INBRAIN_SCHEMA_AUDIT_VERBOSE: '1' }, async () => {
      await logCandidate({
        type: 'therapy-session',
        slug: 'personal/therapy/foo.md',
        frontmatterKeys: ['date'],
        packIdentity: 'inbrain-base@1.0.0+bbbbbbbb',
      });
  ...
  test('honors custom count', async () => {
    await withEnv({ INBRAIN_AUDIT_DIR: auditDir }, async () => {
      await logCandidate({
        type: 'foo',
        slug: 'bar/baz.md',
        frontmatterKeys: [],
  ...
  test('best-effort: unwritable audit dir warns but does not throw', async () => {
    // Point INBRAIN_AUDIT_DIR at a path that mkdirSync(recursive:true) can't create.
    ...
    await withEnv({ INBRAIN_AUDIT_DIR: join(blockerFile, 'subdir') }, async () => {
      // Should not throw.
...
describe('readRecentCandidates', () => {
  test('returns [] when audit dir does not exist', async () => {
    await withEnv({ INBRAIN_AUDIT_DIR: join(auditDir, 'nope') }, () => {
