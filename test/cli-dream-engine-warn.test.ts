/**
 * v0.41.13 (#1422) — `inbrain dream` surfaces engine-connect failures on stderr.
 *
...
describe('#1422 — dream surfaces connectEngine failures', () => {
  test('connect failure prints WARNING line on stderr (does not swallow silently)', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-dream-warn-'));
    const tmpBrain = mkdtempSync(join(tmpdir(), 'inbrain-dream-brain-'));
    try {
      // Point INBRAIN_HOME at an empty tempdir so loadConfig sees no config,
      ...
      // INBRAIN_HOME=/tmp/x yields configDir() === '/tmp/x/.inbrain' (config.ts:687-690).
      mkdirSync(join(tmpHome, '.inbrain'), { recursive: true });
      writeFileSync(join(tmpHome, '.inbrain', 'config.json'), JSON.stringify({
        engine: 'postgres',
  ...
  test('successful connect emits NO WARNING line', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-dream-ok-'));
    const tmpBrain = mkdtempSync(join(tmpdir(), 'inbrain-dream-brain-'));
    try {
