/**
 * v0.36.1.x #1100: PGLite + `inbrain apply-migrations` chain spawn test.
 *
 * Spawns `inbrain init --migrate-only` followed by `inbrain apply-migrations
 ...
 *
 * Pre-fix, this exact sequence hit `Inbrain: Timed out waiting for PGLite
 * lock` because:
 *   1. apply-migrations pre-flight schema-version probe held the
 *      single-writer lock briefly and raced the v0.11.0 subprocess.
 *   2. v0.11.0 phase A spawned `inbrain init --migrate-only` as a child;
...
/**
 * Make a shim `inbrain` binary that routes to `bun run <repo>/src/cli.ts`.
 *
 * The v0.11.0 orchestrator chain spawns subprocesses via `execSync('inbrain
 * jobs smoke')` and `execSync('inbrain init --migrate-only')` (the Postgres
 * path; PGLite now routes in-process, but phase B's smoke still shells out).
 * On a developer machine `inbrain` resolves via `bun link`; on CI it
...
function makeGbrainShim(): { binDir: string; cleanup: () => void } {
  const binDir = mkdtempSync(join(tmpdir(), 'inbrain-shim-'));
  const shimPath = join(binDir, 'inbrain');
  writeFileSync(
    shimPath,
...
describe('apply-migrations on fresh PGLite (v0.36.1.x #1100)', () => {
  ...
  test('init --migrate-only → apply-migrations --yes → re-run → --list (all exit 0)', async () => {
    const home = mkdtempSync(join(tmpdir(), 'inbrain-pglite-spawn-'));
    const shim = makeGbrainShim();
    try {
      mkdirSync(join(home, '.inbrain'), { recursive: true });
      writeFileSync(
        join(home, '.inbrain', 'config.json'),
        JSON.stringify({
          engine: 'pglite',
          database_path: join(home, '.inbrain', 'brain.pglite'),
          embedding_dimensions: 1536,
        }) + '\n',
      );
      // PATH shim so orchestrator phase-B execSync('inbrain jobs smoke')
      // and similar resolve to our shim instead of requiring a global
      // install. This matches the contract users hit in production
      // (inbrain on PATH) without depending on `bun link` having run.
      const env = {
        HOME: home,
        INBRAIN_HOME: home,
      ...
      expect(existsSync(join(home, '.inbrain', 'brain.pglite'))).toBe(true);
