 ...
 * they spawn git subprocesses, mutate `process.env.INBRAIN_SYNC_CHECKPOINT_EVERY`,
...
describe('#1794 — resumable incremental sync (pinned target)', () => {
  ...
  beforeEach(async () => {
    ...
    repoPath = mkdtempSync(join(tmpdir(), 'inbrain-1794-'));
    gitInit(repoPath);
  ...
  afterEach(() => {
    delete process.env.INBRAIN_SYNC_CHECKPOINT_EVERY;
  ...
  test('[CRITICAL] killed mid-import banks progress; next run converges', async () => {
    const { performSync } = await import('../src/commands/sync.ts');
    process.env.INBRAIN_SYNC_CHECKPOINT_EVERY = '1'; // flush after every file
...
describe('#1794 pure helpers', () => {
  ...
  test('resolveMaxConnections parses env, ignores garbage', () => {
    const prev = process.env.INBRAIN_MAX_CONNECTIONS;
    try {
      delete process.env.INBRAIN_MAX_CONNECTIONS;
      expect(resolveMaxConnections()).toBeUndefined();
      process.env.INBRAIN_MAX_CONNECTIONS = '20';
      expect(resolveMaxConnections()).toBe(20);
      process.env.INBRAIN_MAX_CONNECTIONS = 'nope';
      expect(resolveMaxConnections()).toBeUndefined();
      process.env.INBRAIN_MAX_CONNECTIONS = '0';
      expect(resolveMaxConnections()).toBeUndefined();
    } finally {
      if (prev === undefined) delete process.env.INBRAIN_MAX_CONNECTIONS;
      else process.env.INBRAIN_MAX_CONNECTIONS = prev;
    }
  });
