 ...
 * package name ("inbrain/<subpath>") so it actually exercises the
 ...
 * downstream consumer (inbrain-evals) doesn't silently break first.
 *
 * To add a new public export: extend `EXPECTED_EXPORTS` below. To
 * REMOVE one: bump inbrain's minor (breaking-interface per CLAUDE.md
...
const EXPECTED_EXPORTS: ExpectedExport[] = [
  { subpath: 'inbrain', canary: [] }, // root "." export; no single canary — just require import success
  { subpath: 'inbrain/engine', canary: ['clampSearchLimit', 'MAX_SEARCH_LIMIT'] },
  { subpath: 'inbrain/types', canary: ['InbrainError'] },
  { subpath: 'inbrain/operations', canary: ['operations', 'OperationError'] },
  { subpath: 'inbrain/minions', canary: [] }, // barrel module; re-exports many names
  { subpath: 'inbrain/engine-factory', canary: [] }, // factory exports a default creator
  { subpath: 'inbrain/pglite-engine', canary: ['PGLiteEngine'] },
  { subpath: 'inbrain/link-extraction', canary: ['extractEntityRefs', 'extractPageLinks'] },
  { subpath: 'inbrain/import-file', canary: ['importFromContent'] },
  { subpath: 'inbrain/transcription', canary: [] },
  { subpath: 'inbrain/embedding', canary: ['embed'] },
  { subpath: 'inbrain/config', canary: ['loadConfig'] },
  { subpath: 'inbrain/markdown', canary: ['splitBody', 'parseMarkdown', 'serializeMarkdown'] },
  { subpath: 'inbrain/backoff', canary: [] },
  { subpath: 'inbrain/search/hybrid', canary: ['hybridSearch', 'rrfFusion'] },
  { subpath: 'inbrain/search/expansion', canary: ['expandQuery'] },
  { subpath: 'inbrain/ai/gateway', canary: ['configureGateway', 'embed'] },
  { subpath: 'inbrain/extract', canary: [] },
  { subpath: 'inbrain/ingestion', canary: ['INGESTION_SOURCE_API_VERSION', 'validateIngestionEvent', 'computeContentHash'] },
  { subpath: 'inbrain/ingestion/test-harness', canary: ['IngestionTestHarness', 'expectEvent'] },
];
...
describe('public exports — package.json exports map', () => {
  ...
  test('EXPECTED_EXPORTS list matches the exports map exactly (no drift)', () => {
    const exports = readPackageExports();
    const exportedSubpaths = Object.keys(exports).map(k => (k === '.' ? 'inbrain' : `inbrain${k.slice(1)}`)).sort();
