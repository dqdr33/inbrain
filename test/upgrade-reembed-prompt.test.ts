 ...
 * skip-wait + INBRAIN_NO_REEMBED env overrides, and falls back to an
...
describe('runPostUpgradeReembedPrompt (v0.32.7)', () => {
  ...
  test('INBRAIN_NO_REEMBED=1 bails out with doctor-warning marker', async () => {
    await seedPage('a', 'body');
    const writes: string[] = [];
    const result = await runPostUpgradeReembedPrompt(engine, 'openai:text-embedding-3-large', {
      isTTY: true,
      env: { INBRAIN_NO_REEMBED: '1' },
      write: (l) => writes.push(l),
      graceSeconds: 99,
    });
    expect(result.proceeded).toBe(false);
    expect(result.reason).toBe('bypassed_no_reembed');
    expect(writes.some(w => w.includes('INBRAIN_NO_REEMBED=1'))).toBe(true);
  });

  test('INBRAIN_REEMBED_GRACE_SECONDS=0 skips wait on TTY', async () => {
    await seedPage('a', 'body');
    const writes: string[] = [];
    const t0 = Date.now();
    const result = await runPostUpgradeReembedPrompt(engine, 'openai:text-embedding-3-large', {
      isTTY: true,
      env: { INBRAIN_REEMBED_GRACE_SECONDS: '0' },
      write: (l) => writes.push(l),
    });
    expect(result.proceeded).toBe(true);
