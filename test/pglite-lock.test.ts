...
const TEST_DIR = join(tmpdir(), 'inbrain-lock-test-' + process.pid);
...
describe('pglite-lock', () => {
  ...
  test('acquires and releases lock', async () => {
    ...
    expect(existsSync(join(TEST_DIR, '.inbrain-lock'))).toBe(true);

    await releaseLock(lock);
    expect(existsSync(join(TEST_DIR, '.inbrain-lock'))).toBe(false);
  });
  ...
  test('creates missing data directory before acquiring lock', async () => {
    ...
    expect(existsSync(join(missingDataDir, '.inbrain-lock'))).toBe(true);

    await releaseLock(lock);
    expect(existsSync(join(missingDataDir, '.inbrain-lock'))).toBe(false);
  });
  ...
  test('detects and cleans stale lock from dead process', async () => {
    ...
    const lockDir = join(TEST_DIR, '.inbrain-lock');
    mkdirSync(lockDir);
  ...
  test('lock file contains PID and command', async () => {
    const lock = await acquireLock(TEST_DIR);
    const lockData = JSON.parse(readFileSync(join(TEST_DIR, '.inbrain-lock', 'lock'), 'utf-8'));
