...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'inbrain-bs-cp-'));
  homeBackup = process.env.INBRAIN_HOME;
  process.env.INBRAIN_HOME = tmp;
});

afterEach(() => {
  if (homeBackup === undefined) delete process.env.INBRAIN_HOME;
  else process.env.INBRAIN_HOME = homeBackup;
...
describe('save + load round-trip (TX3 load-bearing — full ideas preserved)', () => {
  ...
  test('atomic write: no .tmp left behind on success', () => {
    ...
    saveCheckpoint(cp);
    const dir = join(tmp, '.inbrain', 'brainstorm');
  ...
  test('loadCheckpoint returns null + stderr WARN on schema mismatch', () => {
    ...
    const cp = fixtureCheckpoint(runId);
    saveCheckpoint(cp);
    const path = join(tmp, '.inbrain', 'brainstorm', `${runId}.json`);
  ...
  test('loadCheckpoint returns null on corrupt JSON', () => {
    ...
    writeFileSync(join(tmp, '.inbrain', 'brainstorm', `${runId}.json`), '{not json}');
    expect(loadCheckpoint(runId)).toBeNull();
  });
});
...
describe('gcStaleCheckpoints (A5 7-day window)', () => {
  test('removes files older than the threshold; returns count', () => {
    ...
    const stalePath = join(tmp, '.inbrain', 'brainstorm', `${stale}.json`);
    ...
    expect(existsSync(join(tmp, '.inbrain', 'brainstorm', `${fresh}.json`))).toBe(true);
  });
...
describe('clearCheckpoint', () => {
  test('removes file when present', () => {
    ...
    const path = join(tmp, '.inbrain', 'brainstorm', `cleartest0000000.json`);
...
describe('isCheckpointFresh', () => {
  ...
  test('false for >7 day old checkpoint', () => {
    ...
    const path = join(tmp, '.inbrain', 'brainstorm', 'oldtest000000000.json');
