...
function makeGitRepo(): string {
  const dir = mkdtempSync(join(tmpdir(), 'inbrain-dream-repo-'));
...
//   - back-compat regression: bare `inbrain dream` writes no per-source stamp
...
describe('runDream — --source / --source-id (v0.41.13)', () => {
  ...
  test('--help --source whatever prints help and exits 0 (no engine-null error)', async () => {
    ...
    expect(logSpy.mock.calls.flat().join(' ')).toMatch(/Usage: inbrain dream/);
    exitSpy.mockRestore();
    logSpy.mockRestore();
  });
  ...
  test('--source <unknown> exits 1 with assertSourceExists hint', async () => {
    ...
    expect(errOut).toMatch(/inbrain sources list/);
    exitSpy.mockRestore();
    errSpy.mockRestore();
  });
  ...
  test('--source <archived> exits 1 and leaves last_full_cycle_at untouched', async () => {
    ...
    expect(errOut).toMatch(/inbrain sources restore archived-thing/);
  ...
  // ─── Back-compat: bare `inbrain dream` does NOT write per-source stamp ─

  test('inbrain dream (no --source) leaves all sources untouched (back-compat regression)', async () => {
    await seedSource('alpha');
