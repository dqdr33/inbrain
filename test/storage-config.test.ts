...
describe('loadStorageConfig — real-disk loader', () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'inbrain-storage-test-'));
    __resetMissingStorageWarning();
    warnings = [];
  ...
  test('returns null when inbrain.yml does not exist', () => {
    try {
  ...
  test('loads canonical inbrain.yml — the test that would have caught the original gray-matter P0', () => {
    try {
...
storage:
  ...
  db_only:
    ...
    - media/articles/
`;
      writeFileSync(join(tmp, 'inbrain.yml'), yaml);
...
`;
  ...
  test('warns once when inbrain.yml exists but storage section is missing', () => {
    try {
      writeFileSync(join(tmp, 'inbrain.yml'), 'something_else: foo\n');
      const config = loadStorageConfig(tmp);
...
`;
  ...
  test('throws on unreadable inbrain.yml (permission denied) — does not silently disable feature', () => {
    try {
      const yamlPath = join(tmp, 'inbrain.yml');
