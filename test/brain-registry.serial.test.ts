...
import { InbrainError } from '../src/core/types.ts';
...
describe('validateMountId', () => {
  ...
  test('rejects empty / non-string', () => {
    expect(() => validateMountId('')).toThrow(InbrainError);
    expect(() => validateMountId(null as unknown as string)).toThrow(InbrainError);
    expect(() => validateMountId(undefined as unknown as string)).toThrow(InbrainError);
    expect(() => validateMountId(42 as unknown as string)).toThrow(InbrainError);
  });
...
describe('BrainRegistry — lazy init', () => {
  ...
  test('empty/null/undefined id routes to host', async () => {
    ...
    //
    // Hermeticity: dev machines often have a real ~/.inbrain/config.json
    // (the maintainer's own brain). Without INBRAIN_HOME isolation, the
    ...
    // INBRAIN_HOME to a guaranteed-empty tempdir so host-init has nothing
    ...
    process.env.INBRAIN_HOME = isolatedHome;
    try {
      const reg = new BrainRegistry([]);
      // Expect the host-init path to be attempted (it'll fail on missing
      // <isolated>/.inbrain/config.json, but the error will come from
      ...
      if (savedHome !== undefined) process.env.INBRAIN_HOME = savedHome;
