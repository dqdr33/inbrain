 ...
 *   INBRAIN_REPLAY_GATE_TOP1_FLOOR   (default 0.80)
 *   INBRAIN_REPLAY_GATE_RECALL_FLOOR (default 0.85)
 */
...
function resolveFloors(): { top1: number; recall: number } {
  const t = process.env.INBRAIN_REPLAY_GATE_TOP1_FLOOR;
  const r = process.env.INBRAIN_REPLAY_GATE_RECALL_FLOOR;
  return {
...
describe('eval replay gate — hermetic retrieval qrels (v0.40.1.0 Track D / T5)', () => {
  ...
  test('env-overridable floors via INBRAIN_REPLAY_GATE_TOP1_FLOOR / INBRAIN_REPLAY_GATE_RECALL_FLOOR', async () => {
    const fix = loadFixture();
    await seedCorpus(fix);
    ...
    // env vars.
    await withEnv({ INBRAIN_REPLAY_GATE_TOP1_FLOOR: '0.999', INBRAIN_REPLAY_GATE_RECALL_FLOOR: '0.999' }, async () => {
      const { top1, recall } = resolveFloors();
  ...
  test('no real names in qrels fixture (CLAUDE.md privacy rule)', () => {
    ...
    const blockList = ['Pedro Franceschi', 'Brex', 'Wintermute', 'Inbrain Team', 'Y Combinator', 'YC'];
