 ...
 *
 * Tests cover: happy path, idempotency, INBRAIN_NO_GITIGNORE escape hatch,
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'inbrain-mgi-test-'));
  ...
  originalEnv = process.env.INBRAIN_NO_GITIGNORE;
  delete process.env.INBRAIN_NO_GITIGNORE;
});

afterEach(() => {
  console.warn = originalWarn;
  if (originalEnv === undefined) delete process.env.INBRAIN_NO_GITIGNORE;
  else process.env.INBRAIN_NO_GITIGNORE = originalEnv;
...
describe('manageGitignore', () => {
  test('no-op when inbrain.yml is absent', () => {
    manageGitignore(tmp);
...
`,
  ...
  test('appends db_only directories to .gitignore — happy path', () => {
    ...
    expect(content).toContain('# Auto-managed by inbrain');
  ...
  test('INBRAIN_NO_GITIGNORE=1 skips entirely', () => {
    writeStorageConfig();
    process.env.INBRAIN_NO_GITIGNORE = '1';
    manageGitignore(tmp);
    expect(existsSync(join(tmp, '.gitignore'))).toBe(false);
  });
