...
function scratchGbrain(): string {
  const root = mkdtempSync(join(tmpdir(), 'sp-mf-inbrain-'));
  created.push(root);
...
const FENCE_WITH_RECEIPT = `<!-- inbrain:skillpack:begin -->

<!-- Installed by inbrain 0.32.0 — do not hand-edit between markers. -->
<!-- inbrain:skillpack:manifest cumulative-slugs="alpha,beta" version="0.32.0" -->
...
describe('resolveFenceSlugs', () => {
  ...
  it('F-CDX-8: falls back to row parsing when receipt is missing', () => {
    const fence = `<!-- inbrain:skillpack:begin -->
...
<!-- inbrain:skillpack:end -->`;
  ...
  it('F-CDX-8: uses union when receipt and rows drift', () => {
    const fence = `<!-- inbrain:skillpack:begin -->
<!-- inbrain:skillpack:manifest cumulative-slugs="alpha,old-removed" version="0.20.0" -->
...
describe('stripFence', () => {
  it('removes begin/end markers and the receipt comment', () => {
    ...
    expect(out).not.toContain('inbrain:skillpack:begin');
    expect(out).not.toContain('inbrain:skillpack:end');
    expect(out).not.toContain('cumulative-slugs');
    expect(out).not.toContain('Installed by inbrain');
  });
...
describe('runMigrateFence', () => {
  ...
  it('copies missing skill dirs additively when inbrainRoot is set', () => {
    const ws = scratchWorkspace();
    ...
    const result = runMigrateFence({ targetWorkspace: ws, inbrainRoot });
  ...
  it('preserves user-added rows whose slug is not in the bundle (no copy attempt)', () => {
    const ws = scratchWorkspace();
    const inbrainRoot = scratchGbrain();
    const fenceWithUserRow = `<!-- inbrain:skillpack:begin -->
<!-- inbrain:skillpack:manifest cumulative-slugs="alpha,user-extra" version="0.20.0" -->
| "alpha trigger" | \`skills/alpha/SKILL.md\` |
