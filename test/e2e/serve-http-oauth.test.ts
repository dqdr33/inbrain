 ...
 *
 * Spins up a real `inbrain serve --http` against real Postgres, registers an
 ...
 *
 * Run: INBRAIN_DATABASE_URL=... bun test test/e2e/serve-http-oauth.test.ts
 */
...
describeE2E('serve-http OAuth 2.1 E2E (v0.26.1 + v0.26.2 + v0.26.3)', () => {
  ...
  beforeAll(async () => {
    ...
    const idMatch = regOutput.match(/Client ID:\s+(inbrain_cl_\S+)/);
    const secretMatch = regOutput.match(/Client Secret:\s+(inbrain_cs_\S+)/);
  ...
  test('mint token via client_credentials grant', async () => {
    ...
    expect(data.access_token).toMatch(/^inbrain_at_/);
  ...
  test('minted token works for tools/call — search executes', async () => {
    ...
    const res = await mcpCall(access_token, 'tools/call', {
      name: 'search',
      arguments: { query: 'inbrain', limit: 1 },
    });
  ...
  test('expired/invalid token is rejected at /mcp', async () => {
    const res = await mcpCall('inbrain_at_totally_fake_token', 'tools/list');
  ...
  test('admin dashboard serves SPA index.html (not Express error)', async () => {
    ...
    expect(html).toContain('Inbrain Admin');
  ...
  test('v0.28.10: /admin/api/full-stats with valid admin cookie returns getStats() body', async () => {
    ...
    const cookieMatch = setCookie.match(/inbrain_admin=([^;]+)/);
    expect(cookieMatch).toBeTruthy();
    const cookieValue = cookieMatch![1];

    const statsRes = await fetch(`${BASE}/admin/api/full-stats`, {
      headers: { Cookie: `inbrain_admin=${cookieValue}` },
    });
  ...
  test('wrong client_secret is rejected at token endpoint', async () => {
    const res = await fetch(`${BASE}/token`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: `grant_type=client_credentials&client_id=${clientId}&client_secret=inbrain_cs_wrong_secret&scope=read`,
    });
  ...
  test('v0.26.3: /mcp request persists agent_name + params + error_message', async () => {
    ...
    const sql = postgres(process.env.INBRAIN_DATABASE_URL || process.env.DATABASE_URL || '', { prepare: false });
    try {
  ...
  // and execute arbitrary commands on the inbrain host.
  //
