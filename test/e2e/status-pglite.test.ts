/**
 * E2E `inbrain status` against a real PGLite brain.
 *
 * Seeds:
 *   - 1 source row
 ...
 *   - 1 active inbrain_cycle_locks row
...
describe('inbrain status E2E (PGLite)', () => {
  ...
  test('active lock surfaces in the locks section', async () => {
    ...
    await engine.executeRaw(
      `INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at, last_refreshed_at)
       VALUES ('inbrain-cycle', 1234, 'test-host', NOW(), NOW() + INTERVAL '30 minutes', NOW())`,
    );
    let jsonOut = '';
    await runStatus(engine, ['--json'], {
      stdout: (s) => {
    ...
    expect(parsed.locks[0].id).toBe('inbrain-cycle');
