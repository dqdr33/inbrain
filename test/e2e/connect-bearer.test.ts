/**
 * E2E for `inbrain connect`'s D4 raw-bearer smoke probe (connect-probe.ts).
 *
 * Spins up a real `inbrain serve --http` against a hermetic PGLite brain (no
 * Postgres / Docker), mints a legacy bearer token via `inbrain auth create`,
...
describe('connect bearer probe E2E (PGLite + real serve --http)', () => {
  ...
  beforeAll(async () => {
    home = mkdtempSync(join(tmpdir(), 'inbrain-connect-e2e-'));
    ...
    const env: Record<string, string | undefined> = { ...process.env, INBRAIN_HOME: home };
    ...
    token = (authOut.match(/inbrain_[a-f0-9]{64}/) ?? [''])[0];
  ...
  test('wrong token classifies as auth', async () => {
    expect(serverReady).toBe(true);
    const r = await probeBrainIdentity(MCP_URL, 'inbrain_deadbeef', { timeoutMs: 15_000 });
  ...
  // `inbrain connect --install` against the live server. Sandboxed via HOME /
  ...
  // Run `inbrain connect <args>` as a subprocess with extra env (HOME/CODEX_HOME
  // sandbox + INBRAIN_REMOTE_TOKEN). spawnSync captures stderr too — connect's
  ...
  const runConnectCli = (args: string[], extraEnv: Record<string, string>): { code: number; out: string } => {
    const r = spawnSync('bun', ['run', 'src/cli.ts', 'connect', ...args], {
      cwd: process.cwd(),
      encoding: 'utf8',
      env: { ...process.env, INBRAIN_HOME: home, ...extraEnv },
    });
  ...
  (HAS_CLAUDE ? test : test.skip)('claude-code --install registers + connects against the live server', () => {
    ...
    try {
      ...
      const got = spawnSync('claude', ['mcp', 'get', 'inbrain'], { encoding: 'utf8', env: { ...process.env, HOME: claudeHome } });
      expect(got.status).toBe(0);
      expect(`${got.stdout ?? ''}${got.stderr ?? ''}`).toContain(`:${PORT}/mcp`);
    } finally {
      try { spawnSync('claude', ['mcp', 'remove', 'inbrain'], { env: { ...process.env, HOME: claudeHome } }); } catch { /* best-effort */ }
  ...
  (HAS_CODEX ? test : test.skip)('codex --install registers the env-var bearer against the live server', () => {
    ...
    try {
      const r = runConnectCli([MCP_URL, '--token', token, '--agent', 'codex', '--install', '--yes'], { CODEX_HOME: codexHome, INBRAIN_REMOTE_TOKEN: token });
      expect(r.code).toBe(0);
      expect(r.out).toMatch(/Verified/);
      ...
      const got = spawnSync('codex', ['mcp', 'get', 'inbrain'], { encoding: 'utf8', env: { ...process.env, CODEX_HOME: codexHome } });
      expect(got.status).toBe(0);
      const text = `${got.stdout ?? ''}${got.stderr ?? ''}`;
      expect(text).toContain('INBRAIN_REMOTE_TOKEN');
