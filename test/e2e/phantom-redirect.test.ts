...
beforeEach(async () => {
  ...
  // v0.40: phantom redirect now uses per-source lock id (inbrain-sync:<source>).
  await engine.executeRaw(`DELETE FROM inbrain_cycle_locks WHERE id='inbrain-sync:default'`);
});
...
describeMaybe('phantom-redirect E2E (Postgres)', () => {
  test('bulk-pile: 60 phantoms with cap=50 → 50 redirected, more_pending=true', async () => {
    ...
    try {
      ...
      const engine = getEngine();
      await withEnv({ INBRAIN_PHANTOM_REDIRECT_LIMIT: '50' }, async () => {
  ...
  test('concurrent-sync race: external lock holder → pass skipped, audit entry', async () => {
    ...
    try {
      ...
      const engine = getEngine();
      // Take the inbrain-sync lock manually (simulates concurrent inbrain sync).
      ...
      // v0.40 D16: phantom acquires per-source lock; default source = inbrain-sync:default.
      await engine.executeRaw(
        `INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at)
         VALUES ('inbrain-sync:default', 9999, 'simulated-other-host', now(), now() + interval '1 hour')
