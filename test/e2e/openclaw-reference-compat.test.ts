 ...
 * `inbrain check-resolvable` against an OpenClaw-reference workspace
...
describe('OpenClaw reference workspace compat (W1 + W2 + W3)', () => {
  ...
  it('auto-detects skills dir via $OPENCLAW_WORKSPACE (D-CX-4 priority)', () => {
    ...
    // env var, we'd get inbrain's own repo. With it set, we should
  ...
  it('CLI subprocess: inbrain check-resolvable --json --skills-dir FIXTURE clean', () => {
    const r = spawnSync(
      'bun',
  ...
  it('skillpack install against OpenClaw-reference layout writes managed block to AGENTS.md', () => {
    ...
    expect(inbrainRoot).not.toBeNull();

    const opts = {
      inbrainRoot: inbrainRoot!,
    ...
    expect(agents).toContain('inbrain:skillpack:begin');
