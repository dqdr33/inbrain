...
function makeCtx(opts: { remote?: boolean } = {}): OperationContext {
  return {
    ...
    dryRun: false,
    // Default: trusted local invocation (matches `inbrain call` semantics).
...
describeE2E('E2E: Files', () => {
  ...
  test('file_upload stores metadata + file_list shows it', async () => {
    // Create a temp file
    const tmpDir = mkdtempSync(join(tmpdir(), 'inbrain-e2e-'));
    ...
    try {
      ...
      expect(url.url).toContain('inbrain:files/');
    } finally {
  ...
  test('file_upload rejects outside-cwd paths for remote (MCP) callers', async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'inbrain-e2e-ssrf-'));
...
describeE2E('E2E: Setup Journey', () => {
  ...
  test('inbrain init --non-interactive connects and initializes', () => {
  ...
  test('inbrain import imports fixtures via CLI', () => {
    const result = Bun.spawnSync({
  ...
  test('inbrain search returns results via CLI', () => {
    const result = Bun.spawnSync({
  ...
  test('inbrain stats shows page count via CLI', () => {
    const result = Bun.spawnSync({
  ...
  test('inbrain health runs via CLI', () => {
    const result = Bun.spawnSync({
...
describeE2E('E2E: Init Edge Cases', () => {
  ...
  test('init --non-interactive without URL fails gracefully', () => {
    ...
    delete env.INBRAIN_DATABASE_URL;
    const result = Bun.spawnSync({
...
describeE2E('E2E: RLS Verification', () => {
  ...
  const cliCwd = join(import.meta.dir, '../..');
  const cliEnv = () => ({ ...process.env, DATABASE_URL: process.env.DATABASE_URL!, INBRAIN_DATABASE_URL: process.env.DATABASE_URL! });

  // Seed a unique suffix per run so concurrent test DBs / crashed prior
  // runs don't collide. All helper tables follow `inbrain_rls_regression_<suffix>`.
  ...
  test('inbrain doctor fails with exit 1 when a public table is missing RLS', async () => {
    const conn = getConn();
    const tbl = `inbrain_rls_regression_${suffix}`;
    try {
  ...
  test('INBRAIN:RLS_EXEMPT comment with valid reason exempts a non-RLS public table', async () => {
    const conn = getConn();
    const tbl = `inbrain_rls_exempt_ok_${suffix}`;
    try {
      await conn.unsafe(`CREATE TABLE public.${tbl} (id int)`);
      await conn.unsafe(`ALTER TABLE public.${tbl} DISABLE ROW LEVEL SECURITY`);
      await conn.unsafe(`COMMENT ON TABLE public.${tbl} IS 'INBRAIN:RLS_EXEMPT reason=e2e test fixture, anon-readable ok'`);
  ...
  test('INBRAIN:RLS_EXEMPT comment WITHOUT reason= still fails doctor', async () => {
    const conn = getConn();
    const tbl = `inbrain_rls_exempt_bad_${suffix}`;
    try {
      await conn.unsafe(`CREATE TABLE public.${tbl} (id int)`);
      await conn.unsafe(`ALTER TABLE public.${tbl} DISABLE ROW LEVEL SECURITY`);
      // Missing the `reason=<...>` segment — prefix alone is not enough.
      await conn.unsafe(`COMMENT ON TABLE public.${tbl} IS 'INBRAIN:RLS_EXEMPT'`);
  ...
  test('Non-exempt unrelated COMMENT on a no-RLS table still fails doctor', async () => {
    const conn = getConn();
    const tbl = `inbrain_rls_comment_${suffix}`;
    try {
      await conn.unsafe(`CREATE TABLE public.${tbl} (id int)`);
      await conn.unsafe(`ALTER TABLE public.${tbl} DISABLE ROW LEVEL SECURITY`);
  ...
  test('v24 self-heals when budget_ledger + budget_reservations are missing', async () => {
    ...
    try {
      ...
      // (cleanup, migration from an older inbrain, etc).
...
describeE2E('E2E: Doctor Command', () => {
  // Scope INBRAIN_HOME to a hermetic tmpdir so `inbrain doctor` doesn't read
  // the developer's local ~/.inbrain/migrations/completed.jsonl. Stale partial
  ...
  let inbrainHome: string;

  beforeAll(async () => {
    await setupDB();
    await importFixtures();
    // Isolate INBRAIN_HOME to a per-block tempdir so the developer's
    // ~/.inbrain/migrations/completed.jsonl ledger doesn't leak in. Without
    ...
    inbrainHome = mkdtempSync(join(tmpdir(), 'inbrain-doctor-e2e-'));
  ...
  afterAll(async () => {
    await teardownDB();
    if (inbrainHome) rmSync(inbrainHome, { recursive: true, force: true });
  });
  ...
  const cliEnv = () => ({
    ...
    INBRAIN_DATABASE_URL: process.env.DATABASE_URL!,
    INBRAIN_HOME: inbrainHome,
  });

  test('inbrain doctor exits 0 on healthy DB', () => {
  ...
  test('inbrain doctor --json produces valid JSON', () => {
    const result = Bun.spawnSync({
      cmd: ['bun', 'run', 'src/cli.ts', 'doctor', '--json'],
      cwd: cliCwd,
      env: cliEnv(),
