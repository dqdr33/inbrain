/**
 * E2E: gstack /setup-inbrain Path 4 unblock — register a remote source over
 * HTTP MCP, sync it, recover from clone deletion.
 *
 * Spawns a real `inbrain serve --http` against real Postgres with a fake-git
 ...
 *
 * Run: INBRAIN_DATABASE_URL=... bun test test/e2e/sources-remote-mcp.test.ts
 */
...
const FIXTURE_DIR = join(tmpdir(), `inbrain-e2e-sources-${process.pid}`);
const INBRAIN_HOME = join(FIXTURE_DIR, 'inbrain-home');
...
describeE2E('sources-remote-mcp E2E (gstack /setup-inbrain Path 4)', () => {
  ...
  beforeAll(async () => {
    ...
    //   - INBRAIN_HOME: scope the clone dir to FIXTURE_DIR
    const subprocessEnv = {
      ...process.env,
    ...
    clientId = reg1.match(/Client ID:\s+(inbrain_cl_\S+)/)?.[1];
    const clientSecret = reg1.match(/Client Secret:\s+(inbrain_cs_\S+)/)?.[1];
    ...
    readOnlyClientId = reg2.match(/Client ID:\s+(inbrain_cl_\S+)/)?.[1];
    const readOnlySecret = reg2.match(/Client Secret:\s+(inbrain_cs_\S+)/)?.[1];
    if (!readOnlyClientId || !readOnlySecret) throw new Error('Failed to register read-only client');

    // Start the HTTP server with the fake-git PATH and our INBRAIN_HOME.
    serverProcess = spawn(
      'bun',
    ...
    for (const id of [clientId, readOnlyClientId].filter(Boolean) as string[]) {
      try {
        execSync(`bun run src/cli.ts auth revoke-client ${id}`, {
          cwd: process.cwd(), encoding: 'utf8', env: { ...process.env, INBRAIN_HOME },
        });
      } catch (e) {
  ...
  // -------------------------------------------------------------------------
  // Headline flow: gstack /setup-inbrain Path 4 unblock
  // -------------------------------------------------------------------------
  ...
  test('sources_add via MCP: clones, INSERTs, returns row with remote_url', async () => {
    ...
    if (process.env.INBRAIN_E2E_DEBUG) {
    ...
    expect(existsSync(join(INBRAIN_HOME, '.inbrain', 'clones', 'e2e-yc-artifacts', '.git'))).toBe(true);
  });
  ...
  test('recovery: rm clone dir → sources_status reports missing', async () => {
    const clonePath = join(INBRAIN_HOME, '.inbrain', 'clones', 'e2e-yc-artifacts');
  ...
  test('sources_remove deletes row + cleans up the clone (managed path)', async () => {
    ...
    const clonePath = join(INBRAIN_HOME, '.inbrain', 'clones', 'e2e-removable');
