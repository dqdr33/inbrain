/**
 * E2E: fresh `inbrain init --pglite` produces a brain that can embed end-to-end.
 *
 ...
 *
 * Hermetic: in-process (NOT a CLI subprocess), INBRAIN_HOME pinned to a
...
describe('E2E: fresh inbrain init --pglite → import → embed works end-to-end', () => {
  let tmpHome: string;
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-e2e-fresh-'));
  ...
  test('bare `init --pglite`: schema sized to gateway defaults (ZE/1280)', async () => {
    ...
    // fresh install: bare `inbrain init --pglite` with no env or file
    // config.
    resetGateway();
