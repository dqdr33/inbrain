 ...
 * `createInbrainContextEngine` directly — that's the ENGINE, not the PLUGIN.
...
describe('openclaw-context-engine plugin entry', () => {
  it('default export has the expected plugin-entry shape', () => {
    ...
    expect(entry.id).toBe('inbrain-context-engine');
    expect(entry.name).toBe('Inbrain Context Engine');
  ...
  it('factory returns a working ContextEngine bound to the workspace', async () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), 'inbrain-plugin-e2e-'));
    try {
