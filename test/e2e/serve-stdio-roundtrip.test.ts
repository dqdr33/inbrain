/**
 * E2E for the "standalone from nothing" funnel: `inbrain init --pglite` →
 * `inbrain serve` (stdio) wired into a coding agent as an MCP subprocess.
 *
 ...
 * StdioClientTransport, so the MCP SDK spawns `inbrain serve` for us, runs the
...
describe('serve stdio round-trip E2E (local PGLite → real MCP tool calls)', () => {
  ...
  beforeAll(async () => {
    home = mkdtempSync(join(tmpdir(), 'inbrain-stdio-e2e-'));
    ...
    // 3. Let the MCP SDK spawn `inbrain serve` (stdio) and run the initialize
    //    handshake — exactly what `claude mcp add inbrain -- inbrain serve` does.
    transport = new StdioClientTransport({
      command: 'bun',
      args: ['run', 'src/cli.ts', 'serve'],
      cwd: process.cwd(),
      env, // includes PATH (to find `bun`) + INBRAIN_HOME
    });
    client = new Client({ name: 'inbrain-stdio-e2e', version: '1.0.0' }, { capabilities: {} });
