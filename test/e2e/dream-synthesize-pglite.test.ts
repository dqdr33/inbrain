...
async function setupRig(): Promise<TestRig> {
  ...
  const brainDir = mkdtempSync(join(tmpdir(), 'inbrain-synth-brain-'));
  const corpusDir = mkdtempSync(join(tmpdir(), 'inbrain-synth-corpus-'));
  return {
    engine,
    brainDir,
    corpusDir,
...
/**
 * Run `body` with ANTHROPIC_API_KEY temporarily cleared AND INBRAIN_HOME
 * pointed at a fresh tmpdir, restoring both on return — even on throw — so
 * the developer's real ~/.inbrain/config.json never leaks the anthropic_api_key
...
async function withoutAnthropicKey<T>(body: () => Promise<T>): Promise<T> {
  ...
  const tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-synth-isol-'));
...
describe('E2E synthesize — cooldown', () => {
  ...
  test('explicit --input bypasses cooldown', async () => {
    ...
    try {
      ...
      await rig.engine.setConfig('dream.synthesize.session_corpus_dir', rig.corpusDir);
      await rig.engine.setConfig('dream.synthesize.last_completion_ts', new Date().toISOString());
      const adHoc = join(tmpdir(), `inbrain-synth-ad-hoc-${Date.now()}-${Math.random().toString(36).slice(2)}.txt`);
...
describe('E2E synthesize — round-trip self-consumption guard (v0.23.2)', () => {
  ...
  test('round-trip: bypassDreamGuard=true re-enables ingestion of marked output', async () => {
    // Power-user escape hatch (`inbrain dream --unsafe-bypass-dream-guard`).
