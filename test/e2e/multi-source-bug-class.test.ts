...
describe('multi-source bug class', () => {
  ...
  test('extract-takes processes both alice pages independently', async () => {
    ...
    // Fence markers come from src/core/takes-fence.ts (`<!--- inbrain:takes:begin -->`).
    const TAKES_BODY = `<!--- inbrain:takes:begin -->
...
<!--- inbrain:takes:end -->`;
  ...
  test('reverse-write disk layout uses .sources/<id>/<slug>.md for non-default (F6)', () => {
    ...
    const tmpDir = mkdtempSync(join(tmpdir(), 'inbrain-multi-source-disk-'));
    try {
