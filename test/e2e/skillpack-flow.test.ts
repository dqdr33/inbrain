/**
 * E2E flow tests for `inbrain skillpack` — the new v0.33 scaffold+
 * reference+migrate+harvest contract.
 *
 * Real inbrain subprocess against tempdir workspaces. No DATABASE_URL
...
const INBRAIN_CMD = 'bun';
const INBRAIN_ARGS = ['run', join(REPO_ROOT, 'src', 'cli.ts')];
...
function runGbrain(args: string[], opts: { cwd?: string } = {}): RunResult {
  const result = spawnSync(INBRAIN_CMD, [...INBRAIN_ARGS, ...args], {
...
describe('skillpack flow (E2E)', () => {
  ...
  test('4. reference --apply-clean-hunks applies upstream change to local workspace', () => {
    // Use a private bundle (scratch inbrain root) so we can mutate the
    // bundle source without polluting the real inbrain repo.
    const inbrainRoot = mkdtempSync(join(tmpdir(), 'sp-e2e-gb-'));
    tempdirs.push(inbrainRoot);
    mkdirSync(join(inbrainRoot, 'src'), { recursive: true });
    writeFileSync(join(inbrainRoot, 'src', 'cli.ts'), '// stub');
    mkdirSync(join(inbrainRoot, 'skills', 'apply-demo'), { recursive: true });
    const initial = Array.from({ length: 20 }, (_, i) => `Line ${i + 1}`).join('\n') + '\n';
    writeFileSync(join(inbrainRoot, 'skills', 'apply-demo', 'SKILL.md'), initial);
    writeFileSync(
      join(inbrainRoot, 'openclaw.plugin.json'),
      JSON.stringify({
        name: 'inbrain-test',
        version: '0.33.0-test',
    ...
    const ws = scratchWorkspace();
    // Run scaffold from inbrainRoot so it picks up our scratch bundle.
    const r0 = spawnSync(INBRAIN_CMD, [...INBRAIN_ARGS, 'skillpack', 'scaffold', 'apply-demo', '--workspace', ws], {
      cwd: inbrainRoot,
      encoding: 'utf-8',
    ...
    writeFileSync(
      join(inbrainRoot, 'skills', 'apply-demo', 'SKILL.md'),
      initial.replace('Line 10\n', 'Line 10 UPSTREAM\n'),
    );

    // Apply clean hunks.
    const r = spawnSync(INBRAIN_CMD, [...INBRAIN_ARGS, 'skillpack', 'reference', 'apply-demo', '--workspace', ws, '--apply-clean-hunks'], {
      cwd: inbrainRoot,
      encoding: 'utf-8',
      env: { ...process.env, OPENCLAW_WORKSPACE: '' },
    });
...
<!-- inbrain:skillpack:manifest cumulative-slugs="legacy-skill" version="0.32.0" -->
...
`,
    ...
    expect(rewritten).not.toContain('inbrain:skillpack:begin');
  ...
  test('7. harvest privacy-lint catches Wintermute → exit 1', () => {
    ...
    mkdirSync(join(inbrainRoot, 'src'), { recursive: true });
    writeFileSync(join(inbrainRoot, 'src', 'cli.ts'), '// stub');
    mkdirSync(join(inbrainRoot, 'skills'), { recursive: true });
    writeFileSync(
      join(inbrainRoot, 'openclaw.plugin.json'),
    ...
    const r = spawnSync(
      INBRAIN_CMD,
      [...INBRAIN_ARGS, 'skillpack', 'harvest', 'contaminated', '--from', hostRoot],
      { cwd: inbrainRoot, encoding: 'utf-8', env: { ...process.env, OPENCLAW_WORKSPACE: '' } },
    );
    expect(r.status).toBe(1); // lint_failed
    expect((r.stdout ?? '') + (r.stderr ?? '')).toContain('Wintermute');
    expect(existsSync(join(inbrainRoot, 'skills', 'contaminated'))).toBe(false); // rolled back
  });
  ...
  test('8. harvest --no-lint bypasses the privacy linter', () => {
    ...
    expect(r.status).toBe(0);
    expect(existsSync(join(inbrainRoot, 'skills', 'bypass-test', 'SKILL.md'))).toBe(true);
  });
  ...
  test('14. --apply-clean-hunks prints two-way WARNING to STDERR, not stdout', () => {
    ...
    const inbrainRoot = mkdtempSync(join(tmpdir(), 'sp-e2e-gb-warn-'));
    tempdirs.push(inbrainRoot);
    mkdirSync(join(inbrainRoot, 'src'), { recursive: true });
    writeFileSync(join(inbrainRoot, 'src', 'cli.ts'), '// stub');
    mkdirSync(join(inbrainRoot, 'skills', 'warn-demo'), { recursive: true });
    const initial = Array.from({ length: 15 }, (_, i) => `L${i + 1}`).join('\n') + '\n';
    writeFileSync(join(inbrainRoot, 'skills', 'warn-demo', 'SKILL.md'), initial);
    writeFileSync(
      join(inbrainRoot, 'openclaw.plugin.json'),
      JSON.stringify({
    ...
    const ws = scratchWorkspace();
    spawnSync(INBRAIN_CMD, [...INBRAIN_ARGS, 'skillpack', 'scaffold', 'warn-demo', '--workspace', ws], {
      cwd: inbrainRoot, encoding: 'utf-8', env: { ...process.env, OPENCLAW_WORKSPACE: '' },
    });
    // Cause drift to make apply do something.
    writeFileSync(
      join(inbrainRoot, 'skills', 'warn-demo', 'SKILL.md'),
      initial.replace('L8\n', 'L8 NEW\n'),
    );

    const r = spawnSync(
      INBRAIN_CMD,
      [...INBRAIN_ARGS, 'skillpack', 'reference', 'warn-demo', '--workspace', ws, '--apply-clean-hunks'],
      { cwd: inbrainRoot, encoding: 'utf-8', env: { ...process.env, OPENCLAW_WORKSPACE: '' } },
    );
    expect(r.status).toBe(0);
    ...
    expect(r.stderr).toContain('aligned to inbrain');
  ...
  test('15. --apply-clean-hunks --json does NOT print the WARNING (machine mode)', () => {
    const inbrainRoot = mkdtempSync(join(tmpdir(), 'sp-e2e-gb-json-'));
    tempdirs.push(inbrainRoot);
    mkdirSync(join(inbrainRoot, 'src'), { recursive: true });
    writeFileSync(join(inbrainRoot, 'src', 'cli.ts'), '// stub');
    mkdirSync(join(inbrainRoot, 'skills', 'json-demo'), { recursive: true });
    writeFileSync(join(inbrainRoot, 'skills', 'json-demo', 'SKILL.md'), 'a\nb\nc\n');
    writeFileSync(
      join(inbrainRoot, 'openclaw.plugin.json'),
      JSON.stringify({
        name: 'gb', version: '0.36-test',
    ...
    const ws = scratchWorkspace();
    spawnSync(INBRAIN_CMD, [...INBRAIN_ARGS, 'skillpack', 'scaffold', 'json-demo', '--workspace', ws], {
      cwd: inbrainRoot, encoding: 'utf-8', env: { ...process.env, OPENCLAW_WORKSPACE: '' },
    });

    const r = spawnSync(
      INBRAIN_CMD,
      [...INBRAIN_ARGS, 'skillpack', 'reference', 'json-demo', '--workspace', ws, '--apply-clean-hunks', '--json'],
      { cwd: inbrainRoot, encoding: 'utf-8', env: { ...process.env, OPENCLAW_WORKSPACE: '' } },
    );
    expect(r.status).toBe(0);
...
<!-- inbrain:skillpack:begin -->
<!-- inbrain:skillpack:manifest cumulative-slugs="lx" version="0.32.0" -->
