...
beforeAll(() => {
  server = Bun.serve({
    ...
    fetch(req) {
      ...
      if (path === '/bad-smoke-asset') return new Response(NON_INBRAIN, { status: 200 });
...
function makeTargetBinary(): { dir: string; target: string } {
  const dir = mkdtempSync(join(tmpdir(), 'inbrain-swap-'));
  const target = join(dir, 'inbrain');
...
function assets(url: string): ReleaseAsset[] {
  return [{ name: 'inbrain-linux-x64', url }];
}
...
describe('binary self-update — real swap E2E', () => {
  test('happy path: downloads, smokes, atomically replaces the running binary', async () => {
    ...
    try {
      expect(versionOf(target)).toBe('inbrain 0.42.0');
      ...
      expect(result.asset).toBe('inbrain-linux-x64');
      // The running "binary" was atomically replaced; a fresh exec sees the new version.
      expect(versionOf(target)).toBe('inbrain 0.43.0');
  ...
  test('smoke failure leaves the old binary untouched (no brick)', async () => {
    ...
    try {
      ...
      expect(versionOf(target)).toBe('inbrain 0.42.0'); // old binary intact
  ...
  test('no matching asset for platform → no_asset, no download attempted', async () => {
    ...
    try {
      const result = await runBinarySelfUpdate(target, {
        fetchRelease: async () => ({ tag: 'v0.43.0', assets: [{ name: 'inbrain-darwin-arm64', url: `${base}/good-asset` }] }),
        platform: 'linux',
        arch: 'x64',
      });
      expect(result.ok).toBe(false);
