...
function makeGitRepo(): string {
  const dir = mkdtempSync(join(tmpdir(), 'inbrain-e2e-dream-'));
...
describeE2E('E2E: inbrain dream CLI against real Postgres', () => {
  let repo: string;
  ...
  test('dream --phase orphans only reports orphans + no cycle-lock footprint', async () => {
    ...
    const before = await conn.unsafe(
      `SELECT COUNT(*)::int AS n FROM inbrain_cycle_locks`,
    );
