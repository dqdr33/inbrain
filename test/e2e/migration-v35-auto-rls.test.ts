 ...
 * backfill of existing public.* tables without RLS (modulo the INBRAIN:RLS_EXEMPT
...
describeE2E('migration v35: auto_rls_event_trigger', () => {
  ...
  test('backfill respects INBRAIN:RLS_EXEMPT comment (matches doctor regex)', async () => {
    const conn = getConn();
    await conn`DROP EVENT TRIGGER IF EXISTS auto_rls_on_create_table`;
    try {
      ...
      // Comment must match doctor.ts EXEMPT_RE: /^INBRAIN:RLS_EXEMPT\s+reason=\S.{3,}/
      // — "test exempt" is 11 chars after `reason=`, well over the .{3,} floor.
      await conn`COMMENT ON TABLE _test_backfill_exempt IS 'INBRAIN:RLS_EXEMPT reason=test exempt'`;
