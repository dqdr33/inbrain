 ...
 * brain page, commit to git, and sync to inbrain. It's deterministic —
 ...
 * inbrain jobs submit. No LLM involved.
 *
 ...
 *   - DATABASE_URL (Postgres with inbrain schema)
...
function pagePath(month: string): string {
  return join(BRAIN_PATH, 'media', 'x', 'inbrain-ai', `${month}.md`);
}

function rawPath(month: string): string {
  return join(BRAIN_PATH, 'media', 'x', 'inbrain-ai', '.raw', `${month}-bench.json`);
}
...
async function pullTweets(month: string): Promise<{ count: number; rawJson: string }> {
  ...
  const url = `https://api.x.com/2/tweets/search/all?query=from%3Ainbrain-ai&max_results=100&start_time=${month}-01T00:00:00Z&end_time=${nextMonth}-01T00:00:00Z&tweet.fields=created_at,public_metrics`;
...
function writeBrainPage(month: string, rawJson: string): number {
  ...
  const dir = join(BRAIN_PATH, 'media', 'x', 'inbrain-ai');
  ...
  // Write brain page
  let page = `---\ntitle: "@inbrain-ai — ${month}"\ntype: media/x-account/monthly\ntags: [x-archive, inbrain-ai, benchmark]\n---\n\n# @inbrain-ai — ${month}\n\n> ${unique.length} tweets (benchmark run).\n\n`;
  
  for (const t of unique) {
    const date = (t.created_at || '').slice(0, 10);
    const text = (t.text || '').replace(/\n/g, ' ').slice(0, 200);
    const likes = t.public_metrics?.like_count || 0;
    page += `- **${date}** [${text}](https://x.com/inbrain-ai/status/${t.id})\n`;
...
function gitCommit(month: string): void {
  try {
    execSync(`git add media/x/inbrain-ai/${month}.md media/x/inbrain-ai/.raw/${month}-bench.json`, {
...
function cleanup(month: string): void {
  ...
  try {
    execSync(`git checkout -- media/x/inbrain-ai/${month}.md 2>/dev/null; git clean -f media/x/inbrain-ai/${month}.md 2>/dev/null`, {
      cwd: BRAIN_PATH, stdio: 'pipe',
    });
  } catch {}
}
...
async function openclawPipeline(month: string): Promise<CallResult> {
  ...
  const task = `Pull @inbrain-ai tweets for ${month} and save as a brain page.
1. Run: curl -s -H "Authorization: Bearer $X_BEARER_TOKEN" "https://api.x.com/2/tweets/search/all?query=from%3Ainbrain-ai&max_results=100&start_time=${month}-01T00:00:00Z&end_time=${nextMonth}-01T00:00:00Z&tweet.fields=created_at,public_metrics" > /tmp/bench-${month}.json
2. Parse the JSON, write a brain page to ${BRAIN_PATH}/media/x/inbrain-ai/${month}.md with frontmatter + tweet list
3. Git commit
