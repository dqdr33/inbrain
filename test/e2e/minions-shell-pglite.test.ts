 ...
 * skill: `inbrain jobs submit shell ... --follow` runs inline because
 * `inbrain jobs work` (daemon) is not available on PGLite (exclusive file lock).
 *
...
beforeAll(async () => {
  // registerBuiltinHandlers gates shell handler on INBRAIN_ALLOW_SHELL_JOBS=1.
  // Mirror the real --follow path by setting the env var; restore on cleanup
  // so other tests see their original environment.
  originalAllowShellJobs = process.env.INBRAIN_ALLOW_SHELL_JOBS;
  process.env.INBRAIN_ALLOW_SHELL_JOBS = '1';
...
afterAll(async () => {
  ...
  if (originalAllowShellJobs === undefined) {
    delete process.env.INBRAIN_ALLOW_SHELL_JOBS;
  } else {
    process.env.INBRAIN_ALLOW_SHELL_JOBS = originalAllowShellJobs;
  }
});
...
describe('E2E: Minions shell handler on PGLite (--follow inline path)', () => {
  ...
  test('submit → worker registered via registerBuiltinHandlers → shell runs → completes', async () => {
    ...
    // Gates shell on INBRAIN_ALLOW_SHELL_JOBS=1 (set in beforeAll above).
  ...
  test('v0.35.8.0: inherit:["database_url"] resolves DATABASE_URL into child env, names-only in row + audit', async () => {
    ...
    //   1. The child env carries INBRAIN_DATABASE_URL with the resolved value.
    ...
    const tmpHome = join(tmpdir(), `inbrain-inh-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    mkdirSync(join(tmpHome, '.inbrain'), { recursive: true });
    const testDbUrl = 'postgresql://test:T0P_5ECR3T@localhost:5432/inherit_e2e_db';
    writeFileSync(
      join(tmpHome, '.inbrain', 'config.json'),
      JSON.stringify({ engine: 'postgres', database_url: testDbUrl }) + '\n',
    );

    const savedHome = process.env.INBRAIN_HOME;
    const savedGbrainUrl = process.env.INBRAIN_DATABASE_URL;
    const savedDbUrl = process.env.DATABASE_URL;
    process.env.INBRAIN_HOME = tmpHome;
    ...
    try {
      ...
      const job = await queue.add(
        ...
        { cmd: 'printenv INBRAIN_DATABASE_URL', cwd: '/tmp', inherit: ['database_url'] },
        {},
        { allowProtectedSubmit: true },
      );
      ...
      try {
        ...
        expect(status).toBe('completed');
        const final = await queue.getJob(job.id);
        // The child saw INBRAIN_DATABASE_URL = the configured URL.
      ...
      if (savedHome === undefined) delete process.env.INBRAIN_HOME;
      else process.env.INBRAIN_HOME = savedHome;
      if (savedGbrainUrl === undefined) delete process.env.INBRAIN_DATABASE_URL;
      else process.env.INBRAIN_DATABASE_URL = savedGbrainUrl;
  ...
  test('v0.36.5.0: inherit:["anthropic_api_key"] also resolves (free-form, any config key)', async () => {
    ...
    const tmpHome = join(tmpdir(), `inbrain-anth-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    mkdirSync(join(tmpHome, '.inbrain'), { recursive: true });
  ...
  test('v0.36.5.0: redact_secrets:true scrubs inherit values from stdout_tail', async () => {
    ...
    const tmpHome = join(tmpdir(), `inbrain-redact-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    mkdirSync(join(tmpHome, '.inbrain'), { recursive: true });
    ...
    const savedHome = process.env.INBRAIN_HOME;
    const savedDbUrl = process.env.INBRAIN_DATABASE_URL;
    const savedAnth = process.env.ANTHROPIC_API_KEY;
    process.env.INBRAIN_HOME = tmpHome;
    delete process.env.INBRAIN_DATABASE_URL;
    delete process.env.DATABASE_URL;
    ...
    try {
      ...
      const job = await queue.add(
        'shell',
        {
          cmd: 'printenv INBRAIN_DATABASE_URL && printenv ANTHROPIC_API_KEY',
          cwd: '/tmp',
      ...
      if (savedDbUrl === undefined) delete process.env.INBRAIN_DATABASE_URL;
      else process.env.INBRAIN_DATABASE_URL = savedDbUrl;
      if (savedAnth === undefined) delete process.env.ANTHROPIC_API_KEY;
      else process.env.ANTHROPIC_API_KEY = savedAnth;
      if (existsSync(tmpHome)) rmSync(tmpHome, { recursive: true, force: true });
    }
  ...
  test('v0.36.5.0: redact_secrets:false (default) does NOT scrub — back-compat', async () => {
    ...
    const tmpHome = join(tmpdir(), `inbrain-rd-off-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    mkdirSync(join(tmpHome, '.inbrain'), { recursive: true });
    ...
    try {
      ...
      const job = await queue.add(
        'shell',
        {
          cmd: 'printenv INBRAIN_DATABASE_URL',
          cwd: '/tmp',
  ...
  test('INBRAIN_ALLOW_SHELL_JOBS unset → shellHandler rejects at execution time', async () => {
    ...
    const saved = process.env.INBRAIN_ALLOW_SHELL_JOBS;
    delete process.env.INBRAIN_ALLOW_SHELL_JOBS;
    try {
      const ctx: any = {
        id: 1,
      ...
      await expect(shellHandler(ctx)).rejects.toThrow(/INBRAIN_ALLOW_SHELL_JOBS=1/);
    } finally {
      process.env.INBRAIN_ALLOW_SHELL_JOBS = saved;
    }
  });
});
