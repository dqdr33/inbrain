/**
 * v0.41.6.0 D1 E2E — `inbrain import` preflight rejects missing creds.
 *
 * Sibling of sync-credential-preflight.test.ts. Closes outside-voice F4:
 * pre-v0.41.6.0, `inbrain import <dir>` per-file embed wrote N identical
...
beforeAll(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-import-preflight-e2e-'));
...
beforeEach(() => {
  ...
  repoDir = mkdtempSync(join(tmpdir(), 'inbrain-import-preflight-repo-'));
...
describe('v0.41.6.0 D1 E2E — inbrain import preflight rejects missing OPENAI_API_KEY', () => {
