 ...
 *
 * Tests inbrain skills via OpenClaw agent CLI invocations.
...
Action: Sarah to draft VP Sales job description by April 7.
    ...
    const { text, exitCode, durationMs } = runOpenClaw(
      `Ingest this meeting transcript into inbrain. Create or update pages for each person mentioned. Add timeline entries for today's date. Here is the transcript:\n\n${transcript}`,
      180_000,
    );

    console.log(`  Ingest skill completed in ${durationMs}ms`);

    // The agent runs against its own configured inbrain DB, not our test DB.
...
describeT2('E2E Tier 2: Query Skill', () => {
  ...
  test('query skill returns results for known topic', async () => {
    const { text, exitCode, durationMs } = runOpenClaw(
      'Search inbrain for "NovaMind" and tell me what you found.',
      180_000,
    );
...
describeT2('E2E Tier 2: Health Skill', () => {
  ...
  test('health skill reports brain status', async () => {
    const { text, exitCode, durationMs } = runOpenClaw(
      'Run inbrain doctor --json and tell me the results.',
      180_000,
    );
