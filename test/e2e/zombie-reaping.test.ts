 ...
 * compiled-or-interpreted inbrain CLI as `jobs work --concurrency 1` against
 ...
 *
 * Why not inbrain serve --http: that path doesn't start a worker, and
...
// Skip-gate honored when INBRAIN_E2E_SKIP_ZOMBIE_REAPING=1 (opt-in for CI).
const skipReason: string | null = !hasDatabase()
  ? 'DATABASE_URL not set'
  : process.platform === 'win32'
    ? 'POSIX-only (tini/SIGCHLD)'
    : process.env.INBRAIN_E2E_SKIP_ZOMBIE_REAPING === '1'
      ? 'opt-out via INBRAIN_E2E_SKIP_ZOMBIE_REAPING=1 (v0.41 migration-bump fragility)'
      : null;
...
describeE2E('SIGCHLD handler reaps shell-job children (real binary)', () => {
  ...
  beforeAll(async () => {
    ...
    // then runs the jobs work daemon. INBRAIN_ALLOW_SHELL_JOBS=1 is required
    // for the shell handler to register.
    // Forward DATABASE_URL explicitly so the subprocess can't fall through to
    // any config-file-derived default (PGLite at $HOME/.inbrain/...) under
    ...
    try {
      submitOut = execSync(
        `bun run src/cli.ts jobs submit shell --params '${params}'`,
        {
          cwd: process.cwd(),
          encoding: 'utf8',
          // INBRAIN_ALLOW_SHELL_JOBS=1 also gates the CLI submit path, not
