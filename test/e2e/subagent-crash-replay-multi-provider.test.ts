 ...
 *        — the inbrain-owned stable key (ordinal + inbrain_tool_use_id from
 *        migration v81) is the canonical key, not the provider tool_use_id.
 *     3. Legacy v1 rows (pre-v0.38, ordinal=NULL, inbrain_tool_use_id=NULL)
 ...
 * blocks, ordinal=NULL, inbrain_tool_use_id=NULL) or v2 (post-v0.38:
 * ChatBlock content, ordinal+inbrain_tool_use_id populated).
 */
async function seedCrashedState(
  prompt: string,
  shape: 'v1' | 'v2',
): Promise<{ jobId: number; toolUseId: string; inbrainId: string | null }> {
  ...
  let inbrainId: string | null = null;
  if (shape === 'v2') {
    inbrainId = '01987654-3210-7000-8000-aaaaaaaaaaaa';
    await engine.executeRaw(
      `INSERT INTO subagent_tool_executions
         (job_id, message_idx, tool_use_id, tool_name, input, status,
          schema_version, ordinal, inbrain_tool_use_id, output)
       VALUES ($1, 1, $2, 'search', '{}'::jsonb, 'complete',
               2, 0, $3::uuid, $4::jsonb)`,
      [jobId, toolUseId, inbrainId, priorOutput],
    );
  } else {
    // v1 row: no ordinal, no inbrain_tool_use_id.
    await engine.executeRaw(
      `INSERT INTO subagent_tool_executions
         (job_id, message_idx, tool_use_id, tool_name, input, status,
  ...
  return { jobId, toolUseId, inbrainId };
}
...
describe('SIGKILL crash-replay reconciliation across provider matrix (v0.38 LOAD-BEARING)', () => {
  describe.each(PROVIDER_MATRIX)('provider $providerId', (provider) => {
    it('replay short-circuits the prior complete tool (v2 shape, inbrain_tool_use_id key)', async () => {
    ...
    it('replay short-circuits the prior complete tool (v1 legacy shape, D5 synthesized key)', async () => {
      ...
      // the inbrain_tool_use_id column.
      expect(executions.length).toBe(0);
  ...
  describe('non-idempotent tool with pending status (unrecoverable error)', () => {
    it('throws unrecoverable when a non-idempotent tool is pending on resume', async () => {
      ...
      const jobId = jobRows[0].id;
      const inbrainId = '01987654-3210-7000-8000-bbbbbbbbbbbb';
      ...
      await engine.executeRaw(
        `INSERT INTO subagent_tool_executions
           (job_id, message_idx, tool_use_id, tool_name, input, status,
            schema_version, ordinal, inbrain_tool_use_id)
  ...
  describe('failed tool on prior turn — replay surfaces the error to the LLM', () => {
    it('prior failed tool replays as is_error result, loop completes', async () => {
      ...
      const jobId = jobRows[0].id;
      const inbrainId = '01987654-3210-7000-8000-cccccccccccc';
