 ...
 * reporter hit. Each test gets its own throw-away `INBRAIN_HOME` so test runs
 * are hermetic.
 *
 * Scope covered:
 ...
 *  - `--no-embedding` D9 opt-in: init succeeds with sentinel; inbrain import refuses
...
/** Run a CLI invocation with a clean INBRAIN_HOME + chosen env. Returns { stdout, stderr, exitCode }. */
async function runCli(args: string[], opts: { env?: NodeJS.ProcessEnv; inbrainHome: string; cwd?: string; stdinIsTTY?: boolean } = { inbrainHome: '' }): Promise<{ stdout: string; stderr: string; exitCode: number }> {
  ...
  return new Promise((resolve) => {
    const child = spawn('bun', ['run', `${REPO_ROOT}/src/cli.ts`, ...args], {
      env: {
        ...
        // INBRAIN_HOME isolates state per test.
        INBRAIN_HOME: opts.inbrainHome,
        ...opts.env,
      },
...
function makeTempHome(): string {
  return mkdtempSync(join(tmpdir(), 'inbrain-e2e-init-'));
}
...
describe('v0.37 T12 — fresh init env-detection (D1, D2, D3) + persistence (D5)', () => {
  ...
  test('OPENAI_API_KEY auto-picks OpenAI, persists embedding_model + embedding_dimensions', async () => {
    ...
    const cfgPath = join(tmpHome, '.inbrain', 'config.json');
...
describe('v0.37 T12 — D6 regression: bug-reporter no-op keys exit 1 with Levenshtein', () => {
  ...
  beforeAll(async () => {
    ...
    // Bootstrap a brain so `inbrain config set` has somewhere to write.
  ...
  test('inbrain config set embedding.provider openai → exit 1 with suggestion', async () => {
  ...
  test('inbrain config set embedding.model openai:text-embedding-3-large → suggests embedding_model', async () => {
  ...
  test('inbrain config set embedding.dimensions 1536 → suggests embedding_dimensions', async () => {
  ...
  test('inbrain config set --force foo.bar baz → accepts with WARN', async () => {
  ...
  test('inbrain config set search.mode conservative → accepts (known key)', async () => {
...
describe('v0.37 T12 — D9 --no-embedding deferred-setup mode', () => {
  ...
  test('inbrain import refuses with config-set hint after --no-embedding init', async () => {
    ...
    expect(r.exitCode).toBe(1);
    expect(r.stderr).toMatch(/deferred setup|no-embedding|inbrain config set embedding_model/);
  }, 120000);

  test('inbrain import --no-embed flag bypasses the refusal (chunks land without vectors)', async () => {
...
describe('v0.37 T12 — D11 preflight refuses BEFORE disk writes', () => {
  ...
  test('--embedding-dimensions 9999 (invalid for OpenAI text-3-large) refuses early', async () => {
    ...
    expect(existsSync(join(tmpHome, '.inbrain', 'config.json'))).toBe(false);
  }, 60000);
...
describe('v0.37 T12 — happy path with picker-bypassing explicit flag', () => {
  ...
  test('explicit --embedding-model wins over env detection', async () => {
    ...
    const cfg = JSON.parse(readFileSync(join(tmpHome, '.inbrain', 'config.json'), 'utf-8'));
