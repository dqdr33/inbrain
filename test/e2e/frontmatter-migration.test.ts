 ...
 * disk. Asserts:
 *   - audit phase writes ~/.inbrain/migrations/v0.22.4-audit.json with the
 *     expected per-source counts.
 *   - emit-todo phase appends one entry per source-with-issues to
 *     ~/.inbrain/migrations/pending-host-work.jsonl, each pointing at
 *     skills/migrations/v0.22.4.md (dotted convention) with the exact
 *     inbrain frontmatter validate <source-path> --fix command.
...
beforeAll(async () => {
  ...
  mkdirSync(join(tmpHome, '.inbrain', 'migrations'), { recursive: true });
  ...
  // Redirect ~/.inbrain/migrations/ output. The orchestrator's inbrainDir()
...
describe('E2E: v0.22.4 frontmatter-guard migration', () => {
  ...
  test('audit JSON report exists and has per-source counts', () => {
    const reportPath = join(tmpHome, '.inbrain', 'migrations', 'v0.22.4-audit.json');
  ...
  test('pending-host-work.jsonl carries one entry per source-with-issues', () => {
    const jsonlPath = join(tmpHome, '.inbrain', 'migrations', 'pending-host-work.jsonl');
    ...
    for (const e of entries) {
      ...
      expect(e.command).toContain('inbrain frontmatter validate');
