 ...
 * `inbrain doctor --schema-audit` mechanism deferred to v0.26.4.
 *
...
// against fresh Postgres. If INBRAIN_PGLITE_SNAPSHOT is set (ci:local sets it
...
delete process.env.INBRAIN_PGLITE_SNAPSHOT;
...
describe.skipIf(skip)('schema drift: PGLite ↔ Postgres post-initSchema parity (E2E)', () => {
  ...
  beforeAll(async () => {
    ...
    // DATABASE_URL, but `inbrain doctor` (used by the CLAUDE.md E2E bootstrap
    ...
    // bypasses that floor. INBRAIN_TEST_DB=1 only relaxes the localhost requirement
    ...
    // db-name pattern is the floor: inbrain_test, *_test, test_*, *_e2e.
    // Required REGARDLESS of any override — a production db named "production_data"
    // cannot be reset even with INBRAIN_TEST_DB=1.
    const looksLikeTestDb = /^(inbrain_test|.*_test|test_.*|.*_e2e)$/i.test(dbName);
    const ciOptIn = process.env.INBRAIN_TEST_DB === '1';
    ...
    if (resetAllowed) {
      ...
      const reason = !looksLikeTestDb
        ? `db name "${dbName}" doesn't match the test pattern (inbrain_test, *_test, test_*, *_e2e). ` +
          `INBRAIN_TEST_DB=1 does NOT override this — db name is the hard floor.`
        : `host="${host}" is non-local AND INBRAIN_TEST_DB=1 is not set. ` +
          `Set INBRAIN_TEST_DB=1 to allow non-local hosts (e.g. CI service names) — ` +
