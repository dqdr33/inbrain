...
deliver results that are both semantically relevant and keyword-precise. Inbrain uses
...
4. **Multi-query expansion** — Inbrain generates multiple search queries from a single
...
## Implementation in Inbrain

Inbrain implements hybrid search in `src/core/search/` using Postgres as the single
...
to ship hybrid search with RRF in Inbrain v0.3 as the highest priority feature.

### 2025-04-01 — Shipped in v0.3

Hybrid search shipped as part of Inbrain v0.3. Initial results show significant
