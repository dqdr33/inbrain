 ...
 * Covers what the unit tests can't: the inbrain_cycle_locks table's
...
function makeGitRepo(): string {
  const dir = mkdtempSync(join(tmpdir(), 'inbrain-e2e-cycle-'));
...
describeE2E('E2E: runCycle against real Postgres', () => {
  ...
  test('v0.17 migration v16 created inbrain_cycle_locks table', async () => {
    const conn = getConn();
    const rows = await conn.unsafe(
      `SELECT tablename FROM pg_tables WHERE tablename = 'inbrain_cycle_locks'`,
    );
  ...
  test('dry-run full cycle: zero DB writes + zero filesystem changes', async () => {
    ...
    const locks = await conn.unsafe(`SELECT COUNT(*)::int AS n FROM inbrain_cycle_locks`);
  ...
  test('concurrent cycle is blocked by the lock (status:skipped)', async () => {
    ...
    await conn.unsafe(
      `INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at)
       VALUES ('inbrain-cycle', 99999, 'other-host', NOW(), NOW() + INTERVAL '1 hour')`,
    );
    ...
    try {
      ...
      await conn.unsafe(`DELETE FROM inbrain_cycle_locks WHERE id = 'inbrain-cycle'`);
    }
  });
  ...
  test('TTL-expired lock is auto-claimed (crashed holder recovery)', async () => {
    ...
    await conn.unsafe(
      `INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at)
       VALUES ('inbrain-cycle', 99999, 'crashed-host', NOW() - INTERVAL '2 hours', NOW() - INTERVAL '1 hour')`,
    );
