/**
 * v0.41.6.0 D1 E2E — `inbrain sync` preflight rejects missing creds
 * cleanly without writing N failures to sync-failures.jsonl.
 *
 * Repro from the production bug report:
 *   unset OPENAI_API_KEY
 *   inbrain sync --repo /tmp/test --full --yes
 *
 * Pre-v0.41.6.0: 565 identical "OpenAI embedding requires
 * OPENAI_API_KEY" entries in ~/.inbrain/sync-failures.jsonl, bookmark
 * blocked.
 *
 * Post-v0.41.6.0: single clean stderr line, exit 1, zero
 * sync-failures.jsonl entries.
 *
 * Hermetic: INBRAIN_HOME points at a tmpdir; OPENAI_API_KEY explicitly
 * unset; runs against PGLite via `inbrain init --pglite` so no real
 * Postgres needed.
 */
...
beforeAll(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-preflight-e2e-'));
  // inbrainPath() appends `.inbrain` to INBRAIN_HOME — pre-create the dir.
  const inbrainDir = join(tmpHome, '.inbrain');
  mkdirSync(inbrainDir, { recursive: true });
  // Pre-populate config.json so we can exercise the preflight without
  // running `inbrain init` (which refuses when multiple provider env keys
  // are already in the parent shell — out of scope for this test).
  // INBRAIN_HOME is hermetic; this config is private to this test run.
  writeFileSync(join(inbrainDir, 'config.json'), JSON.stringify({
    database: 'pglite',
    pglite_dir: join(inbrainDir, 'pglite'),
...
beforeEach(() => {
  ...
  repoDir = mkdtempSync(join(tmpdir(), 'inbrain-preflight-repo-'));
...
function runCli(args: string[], env: Record<string, string | undefined>): { code: number; stdout: string; stderr: string } {
  const fullEnv: Record<string, string | undefined> = { ...(process.env as Record<string, string | undefined>), INBRAIN_HOME: tmpHome, ...env };
...
describe('v0.41.6.0 D1 E2E — inbrain sync preflight rejects missing OPENAI_API_KEY', () => {
  ...
  test('does NOT write 565 identical entries to sync-failures.jsonl', () => {
    ...
    const failuresPath = join(tmpHome, '.inbrain', 'sync-failures.jsonl');
