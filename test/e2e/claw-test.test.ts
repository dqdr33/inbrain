/**
 * inbrain claw-test scripted-mode E2E.
 *
 ...
 * pglite.data, so a compiled inbrain can't init a fresh PGLite brain).
...
const BIN_PATH = join(BIN_CACHE, 'inbrain.sh');
...
describe('inbrain claw-test --scenario fresh-install (scripted)', () => {
    ...
    try {
      const result = spawnSync(BIN_PATH, ['claw-test', '--scenario', 'fresh-install', '--keep-tempdir'], {
        ...
        env: {
          ...process.env,
          INBRAIN_HOME: tmp,
          INBRAIN_BIN_OVERRIDE: BIN_PATH,
          INBRAIN_CLAW_SCENARIOS_DIR: join(REPO_ROOT, 'test', 'fixtures', 'claw-test-scenarios'),
        },
        encoding: 'utf-8',
        timeout: 120_000,
      });
      ...
      const frictionDir = join(tmp, '.inbrain', 'friction');
  ...
  test('break path: an invented command produces an error friction entry and exits non-zero', () => {
    // We do this by setting INBRAIN_BIN_OVERRIDE to a script that pretends to be inbrain
    // and rejects the `import` subcommand specifically.
    const tmp = mkdtempSync(join(tmpdir(), 'claw-test-e2e-break-'));
    const fakeBin = join(tmp, 'fake-inbrain');
    try {
      // Write a shim that delegates to real inbrain but rejects 'import' to simulate breakage.
...
describe('inbrain friction render integration', () => {
  test('render produces a markdown report with the redact placeholder', () => {
    ...
    try {
      ...
      const env = { ...process.env, INBRAIN_HOME: tmp, INBRAIN_FRICTION_RUN_ID: 'render-e2e' };
      execFileSync(BIN_PATH, ['friction', 'log', '--phase', 'p', '--message', `error at ${home}/.inbrain/x`], { env, encoding: 'utf-8' });
      ...
      expect(out).not.toContain(home + '/.inbrain');
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  }, 30_000);
