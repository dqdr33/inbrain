 ...
 *   4. `openclaw config set plugins.slots.contextEngine inbrain-context` →
...
const PLUGIN_ID = 'inbrain-context-engine';
const ENGINE_ID = 'inbrain-context';
...
const PROFILE = `inbrain-ctx-e2e-${process.pid}`;
...
describe('openclaw-plugin-load-real (Tier 2 e2e)', () => {
  beforeAll(async () => {
    ...
    fixtureDir = mkdtempSync(join(tmpdir(), 'inbrain-ctx-plugin-real-'));
    ...
    // dangerous-code scanner flags the surrounding inbrain repo (tests use
  ...
  it.skipIf(SKIP)(
    ...
    () => {
      ...
      expect(inspect.plugin.name).toBe('Inbrain Context Engine');
  ...
  it.skipIf(SKIP)(
    'plugins.slots.contextEngine binding to inbrain-context validates cleanly',
    () => {
  ...
  it.skipIf(SKIP)(
    ...
    async () => {
      ...
      const { createInbrainContextEngine } = await import('../../src/core/context-engine.ts');

      const tmp = mkdtempSync(join(tmpdir(), 'inbrain-ctx-sdk-rt-'));
      try {
        ...
        registerContextEngine!(
          dynamicId,
          () => createInbrainContextEngine({ workspaceDir: tmp }),
        );
        ...
        const engine = createInbrainContextEngine({ workspaceDir: tmp });
