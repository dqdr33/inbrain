 ...
 *
 * Spins up `inbrain serve --http` against a real Postgres, registers a
 * client with `read,write,admin` scope, runs `inbrain init --mcp-only`
 * against it from a second tempdir HOME, and exercises the canonical
 * thin-client flows:
 *
 *   - `inbrain init --mcp-only` succeeds and writes remote_mcp config
 *   - `inbrain doctor` reports `mode: thin-client` with all checks green
 *   - `inbrain sync` is refused with the canonical thin-client error
 *   - re-running `inbrain init` refuses without --force
 *
 * Tier B flows (`inbrain remote ping` / `remote doctor`) are stubbed for now
...
async function spawn(args: string[], home: string, extraEnv: Record<string, string | undefined> = {}): Promise<RunResult> {
  ...
  env.INBRAIN_HOME = home;
  delete env.INBRAIN_REMOTE_CLIENT_SECRET;
...
describeWhen('thin-client end-to-end (requires DATABASE_URL)', () => {
  let hostHome: string;          // INBRAIN_HOME for the host (with local engine)
  let clientHome: string;        // INBRAIN_HOME for the thin client (no engine)
  ...
  beforeAll(async () => {
    hostHome = mkdtempSync(join(tmpdir(), 'inbrain-thin-host-'));
    clientHome = mkdtempSync(join(tmpdir(), 'inbrain-thin-client-'));
  ...
  function parseRegisterClientOutput(out: string): { clientId: string; clientSecret: string } {
    // `inbrain auth register-client` doesn't have --json; parse human output:
    //   Client ID:     <id>
  ...
  test('init --mcp-only succeeds against the live host', async () => {
    ...
    const cfgPath = join(clientHome, '.inbrain', 'config.json');
    expect(existsSync(cfgPath)).toBe(true);
    const cfg = JSON.parse(readFileSync(cfgPath, 'utf-8'));
    expect(cfg.remote_mcp.oauth_client_id).toBe(clientId);
    // No PGLite file
    expect(existsSync(join(clientHome, '.inbrain', 'brain.pglite'))).toBe(false);
  });
  ...
  // ─── Tier B: inbrain remote ping + remote doctor ───

  test('inbrain remote doctor returns the host DoctorReport', async () => {
  ...
  // assertion needs. `inbrain serve --http` does NOT start a job worker
  // (workers run via the separate `inbrain jobs work` process). So a
  ...
  // fall back to the self-imposed `--timeout` firing, but `inbrain remote
  ...
  //      the loop overhead).
  //   2. OR start a `inbrain jobs work` subprocess in this test's beforeAll
  ...
  testRaw.skip('inbrain remote ping triggers autopilot-cycle and returns terminal state', async () => {
  ...
  test('client without admin scope cannot call run_doctor', async () => {
    ...
    // that inbrain remote doctor surfaces an auth-error message. This is the
    ...
    const lowScopeHome = mkdtempSync(join(tmpdir(), 'inbrain-thin-client-lowscope-'));
    try {
