 ...
 *
 * Pre-fix: `inbrain search`, `inbrain query`, `inbrain get` on PGLite
 ...
 * INBRAIN_HOME tempdir, seeds a brain with 2 pages, runs each verb
 * with a hard timeout, and asserts exit 0. Without the drain helper
 * in cli.ts, every variant would time out.
 *
 * Bonus assertion: `inbrain serve --http` (a daemon) MUST stay alive
...
const SHIM_PATH = join(BIN_CACHE, 'inbrain-pglite-exit-shim.sh');
...
beforeAll(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-pglite-exit-'));
  repoSourceDir = mkdtempSync(join(tmpdir(), 'inbrain-pglite-exit-src-'));

  // Seed a tiny git repo with 2 markdown pages so `inbrain sync` has
  ...
  runEnv = { ...process.env, INBRAIN_HOME: tmpHome };
  ...
  if (initResult.status !== 0) {
    throw new Error(
      `inbrain init failed (code=${initResult.status}):\n` +
        `STDOUT:\n${initResult.stdout}\n` +
        `STDERR:\n${initResult.stderr}`,
    );
  }
  ...
  if (syncResult.status !== 0) {
    throw new Error(
      `inbrain sync failed (code=${syncResult.status}):\n` +
        `STDOUT:\n${syncResult.stdout}\n` +
        `STDERR:\n${syncResult.stderr}`,
    );
  }
...
describe('v0.41.8.0 — PGLite CLI read commands exit cleanly (#1247/#1269/#1290)', () => {
  test('inbrain search "foxtrot" exits 0 within 15s', async () => {
  ...
  test('inbrain get returns a page body and exits 0 within 15s', async () => {
    const { code, stdout, stderr, durationMs } = await runWithTimeout(
      ['get', 'alpha'],
      15_000,
    );
  ...
  test('inbrain query without --no-expand exits 0 within 15s (no API key)', async () => {
...
describe('v0.42.20.0 — inbrain capture (CLI_ONLY) exits cleanly + frees the lock (#1762)', () => {
...
describe('v0.41.8.0 — daemon survival (regression guard for narrow force-exit)', () => {
  test('inbrain serve --http stays alive past the timeout window', async () => {
    ...
    if (!wasAlive) {
      throw new Error(
        `inbrain serve --http exited within 3s (code=${earlyCode}). ` +
