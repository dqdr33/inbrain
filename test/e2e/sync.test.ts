 ...
 * markdown files, run inbrain sync, verify pages appear in the database.
...
function createTestRepo(): string {
  const dir = mkdtempSync(join(tmpdir(), 'inbrain-sync-e2e-'));
 ...
 * ~/.inbrain/sync-failures.jsonl so running E2E on a developer machine
 * doesn't trash their local sync state.
 */
describeE2E('E2E: sync --skip-failed structured summary loop (v0.22.12, issue #500)', () => {
  let repoPath: string;
  const realFailuresPath = join(homedir(), '.inbrain', 'sync-failures.jsonl');
  let savedFailuresContent: string | null = null;

  beforeAll(async () => {
    await setupDB();

    // Save+clear the real ~/.inbrain/sync-failures.jsonl so the test starts from
    ...
    repoPath = mkdtempSync(join(tmpdir(), 'inbrain-skipfailed-e2e-'));
  ...
  afterAll(async () => {
    ...
    if (savedFailuresContent !== null) {
      mkdirSync(join(homedir(), '.inbrain'), { recursive: true });
  ...
  test('bounded auto-skip: poison file blocks then auto-skips, advancing the bookmark', async () => {
    ...
    const { loadSyncFailures } = await import('../../src/core/sync.ts');
    const engine = getEngine();
    const prevThreshold = process.env.INBRAIN_SYNC_AUTOSKIP_AFTER;
    process.env.INBRAIN_SYNC_AUTOSKIP_AFTER = '2';
    try {
      const beforeCommit = await engine.getConfig('sync.last_commit');
      ...
      if (prevThreshold === undefined) delete process.env.INBRAIN_SYNC_AUTOSKIP_AFTER;
      else process.env.INBRAIN_SYNC_AUTOSKIP_AFTER = prevThreshold;
    }
  });
