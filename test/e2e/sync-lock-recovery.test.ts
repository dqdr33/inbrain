...
beforeAll(async () => {
  if (skip) return;
  tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-lock-recovery-e2e-'));
  await setupDB();
});
...
beforeEach(async () => {
  ...
  repoDir = mkdtempSync(join(tmpdir(), 'inbrain-lock-recovery-repo-'));
  ...
  const eng = getEngine();
  try { await (eng as any).sql`DELETE FROM inbrain_cycle_locks WHERE id LIKE 'inbrain-sync:%'`; } catch { /* */ }
});
...
describeE2E('v0.41.6.0 — sync lock recovery scenarios', () => {
  ...
  test('lock-busy error message includes PID + hostname + age + --break-lock hint', async () => {
    ...
    const eng = getEngine();
    const lockKey = 'inbrain-sync:default';
  ...
  test('--break-lock with TTL-expired row clears the lock', async () => {
    ...
    await (eng as any).sql`
      ...
      VALUES ('inbrain-sync:default', 99999, ${hostname()}, NOW() - INTERVAL '1 hour', NOW() - INTERVAL '30 minutes')
    `;
    ...
    const snap = await inspectLock(eng, 'inbrain-sync:default');
  ...
  test('--break-lock with alive local PID refuses with --force-break-lock hint', async () => {
    ...
    await (eng as any).sql`
      INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at)
      VALUES ('inbrain-sync:default', ${process.pid}, ${hostname()}, NOW(), NOW() + INTERVAL '30 minutes')
    `;
    ...
    // Cleanup.
    await (eng as any).sql`DELETE FROM inbrain_cycle_locks WHERE id = 'inbrain-sync:default'`;
  });
  ...
  test.skip('pipe through `head -5` exits cleanly, next sync runs without lock-busy', async () => {
    // Run `inbrain sync ... | head -5` via shell.
