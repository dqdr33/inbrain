...
beforeEach(async () => {
  await truncateAll();
  brainDir = mkdtempSync(join(tmpdir(), 'inbrain-issue-972-'));
}, 15_000);
...
describe('issue #972 — FS-source (inbrain extract links default)', () => {
...
// ─── DB-source path (inbrain extract links --source db) ──────────────────

describe('issue #972 — DB-source (inbrain extract links --source db)', () => {
