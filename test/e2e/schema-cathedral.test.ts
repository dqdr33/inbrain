...
describe('v0.39 T22a — custom-pack across consumers', () => {
  ...
  test('runReviewCandidates surfaces disk-derived candidates (D3 contract)', async () => {
    ...
    // Active pack derives from inbrain-base which doesn't have `newkind` → in_active_pack: false
...
describe('v0.39 T22d — artifact-type routing', () => {
  ...
  test('validateManifestByKind rejects cross-kind manifests', () => {
    expect(() => validateManifestByKind('schemapack', { api_version: 'inbrain-skillpack-v1' })).toThrow();
    expect(() => validateManifestByKind('skillpack', { api_version: 'inbrain-schema-pack-v1' })).toThrow();
  });
});
