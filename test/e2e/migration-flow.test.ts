 ...
 * `inbrain init --migrate-only`) through Phase G (completed.jsonl append),
...
// Module-level PATH shim. The orchestrator shells out to `inbrain init
// --migrate-only` and `inbrain jobs smoke`. On source-install test envs
// there's no `inbrain` on $PATH. Install a tiny bash shim that `exec`s
...
if (!SKIP) {
  ...
  fakeBinDir = mkdtempSync(join(tmpdir(), 'inbrain-e2e-bin-'));
  const shim = join(fakeBinDir, 'inbrain');
  writeFileSync(
    shim,
...
function freshTempHome(label: string) {
  const dir = mkdtempSync(join(tmpdir(), `inbrain-e2e-migration-${label}-`));
  // preferences.ts's inbrainDir() returns `$HOME/.inbrain` when INBRAIN_HOME
  // is unset. Test fixtures write to `$dir/.inbrain/...`, so set HOME only
  // and clear any inherited INBRAIN_HOME (which would route prefs to $dir
  // directly, no .inbrain suffix).
  process.env.HOME = dir;
  delete process.env.INBRAIN_HOME;
  // Seed config so Phase A's `inbrain init --migrate-only` has a target.
  mkdirSync(join(dir, '.inbrain'), { recursive: true });
  writeFileSync(
    join(dir, '.inbrain', 'config.json'),
...
beforeAll(() => {
  ...
  origGbrainHome = process.env.INBRAIN_HOME;
});
...
afterAll(() => {
  ...
  if (origGbrainHome === undefined) delete process.env.INBRAIN_HOME;
  else process.env.INBRAIN_HOME = origGbrainHome;
...
describeE2E('E2E: v0.11.0 orchestrator against live Postgres', () => {
  test('fresh install flow: schema → smoke → prefs → host-rewrite → completed', async () => {
    ...
    const prefsPath = join(tmp, '.inbrain', 'preferences.json');
  ...
  test('host rewrite: builtin handlers auto-rewritten, non-builtins queued as JSONL TODOs', async () => {
    ...
    // Fixture: AGENTS.md + cron/jobs.json with a mix of inbrain-builtin and
    ...
    expect(cronAfter.jobs[0].cmd).toContain('inbrain jobs submit sync');
    ...
    expect(agentsMdAfter).toContain('inbrain:subagent-routing v0.11.0');
    expect(agentsMdAfter).toContain('skills/conventions/subagent-routing.md');

    // JSONL TODO file written under ~/.inbrain/migrations/.
    const jsonlPath = join(tmp, '.inbrain', 'migrations', 'pending-host-work.jsonl');
  ...
  test('resumable: partial run → orchestrator re-run → complete', async () => {
    ...
    mkdirSync(join(tmp, '.inbrain', 'migrations'), { recursive: true });
    writeFileSync(
      join(tmp, '.inbrain', 'migrations', 'completed.jsonl'),
      JSON.stringify({
        version: '0.11.0',
        status: 'partial',
