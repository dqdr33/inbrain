/**
 * E2E: the self-upgrade marker actually fires on a real `inbrain` invocation.
 *
 ...
 * agent (Claude Code / Codex / OpenClaw) keys off to run the inbrain-upgrade skill.
 *
...
let home: string;
let inbrainDir: string;
const repoRoot = process.cwd();

beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'inbrain-marker-'));
  inbrainDir = join(home, '.inbrain');
...
function writeCache(line: string): void {
  writeFileSync(join(inbrainDir, 'last-update-check'), line + '\n'); // mtime = now → fresh
}
...
function runGbrain(mode: string | undefined): { stdout: string; stderr: string; code: number } {
  ...
  delete env.INBRAIN_SKIP_STARTUP_HOOKS;
  env.INBRAIN_HOME = home;
  if (mode === undefined) delete env.INBRAIN_SELF_UPGRADE_MODE;
  else env.INBRAIN_SELF_UPGRADE_MODE = mode;
...
describe('self-upgrade marker on a real invocation', () => {
  test('notify mode + fresh upgrade_available cache → emits UPGRADE_AVAILABLE on stderr', () => {
    ...
    expect(stderr).toContain('Run: inbrain self-upgrade');
  });
  ...
  test('active snooze for the version → no marker (notify mode honors snooze)', () => {
    ...
    writeFileSync(join(inbrainDir, 'update-snoozed'), `0.99.0 1 ${Date.now()}\n`);
    const { stderr } = runGbrain('notify');
    expect(stderr).not.toContain('UPGRADE_AVAILABLE');
  });

  test('JUST_UPGRADED breadcrumb → one-time confirmation on stderr, then cleared', () => {
    const breadcrumb = join(inbrainDir, 'just-upgraded-from');
  ...
  test('--quiet suppresses the marker', () => {
    ...
    delete env.NODE_ENV;
    env.INBRAIN_HOME = home;
    env.INBRAIN_SELF_UPGRADE_MODE = 'notify';
