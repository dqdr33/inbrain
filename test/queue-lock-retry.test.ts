...
const AUDIT_DIR = join(tmpdir(), `inbrain-queue-lock-retry-${process.pid}-${Date.now()}`);
// Fast retry + isolated audit dir so the test doesn't sleep ~1s or pollute ~/.inbrain.
const FAST_ENV = {
  INBRAIN_BULK_RETRY_BASE_MS: '1',
  INBRAIN_BULK_RETRY_MAX_MS: '2',
  INBRAIN_AUDIT_DIR: AUDIT_DIR,
};
