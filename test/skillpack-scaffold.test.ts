 ...
 *     cumulative-slugs receipt, no .inbrain-skillpack.lock file
 */
...
function scratchGbrain(opts: { withPairedSource?: boolean } = {}): GbrainFixture {
  const root = mkdtempSync(join(tmpdir(), 'sp-scaffold-inbrain-'));
  created.push(root);
...
describe('runScaffold — happy path', () => {
  ...
  it('copies paired source files declared in frontmatter `sources:`', () => {
    const { inbrainRoot } = scratchGbrain({ withPairedSource: true });
    const ws = scratchWorkspace();
  ...
  it('--all (skillSlug: null) installs every bundled skill', () => {
    ...
    runScaffold({ inbrainRoot, targetWorkspace: ws, skillSlug: null });
...
describe('runScaffold — refuses to overwrite (user owns the files)', () => {
  it('re-running is idempotent (every file skipped_existing)', () => {
    ...
    runScaffold({ inbrainRoot, targetWorkspace: ws, skillSlug: 'book-mirror' });
    const second = runScaffold({ inbrainRoot, targetWorkspace: ws, skillSlug: 'book-mirror' });
...
describe('runScaffold — partial-state policy (F-CDX-6)', () => {
  it('skill dir exists but paired source missing → copies the paired source only', () => {
    ...
    // Now scaffold (inbrain bundle has the paired source declared); the
...
describe('runScaffold — error paths', () => {
  it('unknown skill slug → ScaffoldError(unknown_skill)', () => {
    ...
    try {
      runScaffold({ inbrainRoot, targetWorkspace: ws, skillSlug: 'does-not-exist' });
...
describe('runScaffold — IRON-RULE regressions (R1, R2)', () => {
  it('R1: never writes managed-block markers to RESOLVER.md/AGENTS.md', () => {
    ...
    expect(resolver).not.toContain('inbrain:skillpack:begin');
    expect(resolver).not.toContain('inbrain:skillpack:end');
    expect(resolver).not.toContain('inbrain:skillpack:manifest');
  });

  it('R2: never writes a .inbrain-skillpack.lock file', () => {
    const { inbrainRoot } = scratchGbrain();
    const ws = scratchWorkspace();

    runScaffold({ inbrainRoot, targetWorkspace: ws, skillSlug: 'book-mirror' });

    expect(existsSync(join(ws, '.inbrain-skillpack.lock'))).toBe(false);
    // Lock should also not exist anywhere in the workspace tree.
    expect(readdirSync(ws)).not.toContain('.inbrain-skillpack.lock');
  });
