...
describe('7-tier resolution (D13)', () => {
  test('default to inbrain-base when nothing set', () => {
    expect(resolveActivePackName({ remote: false })).toEqual({ pack_name: 'inbrain-base', source: 'default' });
  });
...
describe('loadPackFromString end-to-end', () => {
  test('YAML round-trip', () => {
    const yaml = `api_version: inbrain-schema-pack-v1
name: minimal
version: 0.1.0
...
describe('candidate-audit', () => {
  test('isAuditVerbose respects env var', async () => {
    await withEnv({ INBRAIN_SCHEMA_AUDIT_VERBOSE: undefined }, async () => {
      expect(isAuditVerbose()).toBe(false);
    });
    await withEnv({ INBRAIN_SCHEMA_AUDIT_VERBOSE: '1' }, async () => {
