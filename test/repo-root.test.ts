...
describe('findRepoRoot', () => {
  ...
  function seedRepo(dir: string): void {
    ...
    writeFileSync(join(dir, 'src', 'cli.ts'), '// inbrain marker\n');
  }
  ...
  it('D-CX-4: $OPENCLAW_WORKSPACE wins over repo-root walk when explicitly set', () => {
    // Prior priority (shadow bug): walking up from cwd found inbrain's
  ...
  it('W1: both RESOLVER.md and AGENTS.md present — RESOLVER.md wins', () => {
    // Policy: when both exist at the same location, inbrain-native wins.
    const workspace = scratch();
    const skillsDir = join(workspace, 'skills');
    mkdirSync(skillsDir, { recursive: true });
    require('fs').writeFileSync(
  ...
  // -----------------------------------------------------------------------
  // v0.31.7 #128 adaptation: tier-0 INBRAIN_SKILLS_DIR + read-only
  // install-path fallback. Locked in eng-review D3 + D5.
  // -----------------------------------------------------------------------

  it('v0.31.7 D3-1: tier-0 $INBRAIN_SKILLS_DIR valid path returns env_explicit source', () => {
    const cwd = scratch();
    const explicit = scratch();
    seedSkillsDir(explicit);
    const found = autoDetectSkillsDir(cwd, { INBRAIN_SKILLS_DIR: explicit });
  ...
  it('v0.31.7 D3-2: tier-0 invalid path falls through to lower tiers', () => {
    // INBRAIN_SKILLS_DIR points to a directory with no resolver file. Must
  ...
  it('v0.31.7 D3-4: autoDetectSkillsDirReadOnly install-path walk finds bundled skills', () => {
    ...
    // import.meta.url (the inbrain module's install path) and finds the
    ...
    // We're inside the inbrain repo when running the test, so the install
  ...
  it('v0.31.7 D3-6: AUTO_DETECT_HINT documents tier-0 INBRAIN_SKILLS_DIR', () => {
    // Hint string is what users see when auto-detect fails — must list the
    // tier-0 explicit override so they know to set it.
    expect(AUTO_DETECT_HINT).toContain('$INBRAIN_SKILLS_DIR');
  ...
  it('v0.31.7 D3-7: AUTO_DETECT_HINT_READ_ONLY adds install-path tier', () => {
    ...
    // user can understand what inbrain found and where.
    ...
    expect(AUTO_DETECT_HINT_READ_ONLY).toContain('$INBRAIN_SKILLS_DIR');
  ...
  // requiring a resolver file or inbrain-shape `src/cli.ts`.
  ...
  it('v0.31.7 D5 regression guard: autoDetectSkillsDir does NOT install-path-fallback', () => {
    ...
    // callers running from `~` would silently retarget the bundled inbrain
    ...
    // the inbrain repo. In either case, NEVER 'install_path' on the shared
