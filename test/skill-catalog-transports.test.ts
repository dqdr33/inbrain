 ...
 *
 * Config is driven via the DB plane (engine.setConfig) — the plane `inbrain
 * config set` writes and the plane the gate reads first. INBRAIN_HOME is pinned
...
describe('list_skills over dispatch', () => {
  ...
  test('DB-plane disable wins (inbrain config set mcp.publish_skills false)', async () => {
    await withEnv({ INBRAIN_HOME: home }, async () => {
