 ...
 *
 * Background: inbrain ships an "embedded latest schema" blob
...
const REQUIRED_BOOTSTRAP_COVERAGE: ForwardReference[] = [
  ...
  // SCHEMA_SQL replay creates the index. Powers `inbrain extract --stale` + the
...
const COLUMN_EXEMPTIONS = new Set<string>([
  ...
  // inbrain think trajectory injection — all defensively skip
  ...
  // Schema-pack codegen (scripts/generate-inbrain-base.ts) consumes the value
