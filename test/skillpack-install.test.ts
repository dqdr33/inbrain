...
function scratchGbrain(): { inbrainRoot: string; skillsDir: string } {
  const root = mkdtempSync(join(tmpdir(), 'skillpack-inbrain-'));
  created.push(root);
  ...
  return { inbrainRoot: root, skillsDir };
}
...
describe('findGbrainRoot', () => {
  it('walks up to find openclaw.plugin.json + src/cli.ts', () => {
    ...
    const nested = join(inbrainRoot, 'a', 'b', 'c');
    mkdirSync(nested, { recursive: true });
    expect(findGbrainRoot(nested)).toBe(inbrainRoot);
  });
  it('returns null when no inbrain root above', () => {
    expect(findGbrainRoot('/tmp/definitely-not-a-inbrain-repo-XYZ')).toBeNull();
  });
});
...
describe('loadBundleManifest', () => {
  it('loads + validates a valid manifest', () => {
    const { inbrainRoot } = scratchGbrain();
    const m = loadBundleManifest(inbrainRoot);
  ...
  it('throws BundleError on malformed JSON', () => {
    const { inbrainRoot } = scratchGbrain();
    writeFileSync(join(inbrainRoot, 'openclaw.plugin.json'), '{ nope');
    expect(() => loadBundleManifest(inbrainRoot)).toThrow(BundleError);
  });
});
...
describe('enumerateBundle (D-CX-10 dependency closure)', () => {
  it('includes skill files AND shared_deps for a single-skill install', () => {
    const { inbrainRoot } = scratchGbrain();
    const m = loadBundleManifest(inbrainRoot);
    const entries = enumerateBundle({ inbrainRoot, skillSlug: 'alpha', manifest: m });
  ...
  it('throws BundleError for unknown skill slug', () => {
    ...
    const m = loadBundleManifest(inbrainRoot);
    expect(() =>
      enumerateBundle({ inbrainRoot, skillSlug: 'nope', manifest: m }),
    ).toThrow(BundleError);
  });
  it('enumerates everything when skillSlug is undefined (--all)', () => {
    const { inbrainRoot } = scratchGbrain();
    const m = loadBundleManifest(inbrainRoot);
    const entries = enumerateBundle({ inbrainRoot, manifest: m });
    const targets = entries.map(e => e.relTarget).sort();
...
describe('buildManagedBlock + updateManagedBlock', () => {
  it('builds a block with all installed slugs as rows', () => {
    const m = loadBundleManifest(scratchGbrain().inbrainRoot);
    const block = buildManagedBlock(m, ['alpha', 'beta']);
    expect(block).toContain('inbrain:skillpack:begin');
    expect(block).toContain('inbrain:skillpack:end');
    expect(block).toContain('`skills/alpha/SKILL.md`');
    expect(block).toContain('`skills/beta/SKILL.md`');
  });
  it('appends block when none exists', () => {
    const block = buildManagedBlock(loadBundleManifest(scratchGbrain().inbrainRoot), ['alpha']);
    const updated = updateManagedBlock('# AGENTS\n\nSome prose.\n', block);
    expect(updated).toContain('inbrain:skillpack:begin');
...
describe('planInstall + applyInstall', () => {
  ...
  it('D-CX-11: concurrent install attempt fails with lock_held', () => {
    ...
    writeFileSync(join(workspace, '.inbrain-skillpack.lock'), '99999');
    const opts = {
      inbrainRoot,
      targetWorkspace: workspace,
      targetSkillsDir: skillsDir,
  ...
  it('D-CX-11: --force-unlock works when lock mtime is sub-ms ahead of Date.now (Linux fs jitter)', () => {
    ...
    const { inbrainRoot } = scratchGbrain();
    const { workspace, skillsDir } = scratchTarget();
    const lockFile = join(workspace, '.inbrain-skillpack.lock');
  ...
  it('managed block is written atomically (tmp then rename)', () => {
    ...
    expect(resolver).toContain('inbrain:skillpack:begin');
    expect(resolver).toContain('inbrain:skillpack:end');
...
describe('managed-block receipt + cumulative semantics (v0.19)', () => {
  ...
  it('install alpha, then install beta (separately) → both rows survive AND receipt lists both', () => {
    ...
    const alphaOpts = { inbrainRoot, targetWorkspace: workspace, targetSkillsDir: skillsDir, skillSlug: 'alpha' };
    applyInstall(planInstall(alphaOpts), alphaOpts);

    const betaOpts = { inbrainRoot, targetWorkspace: workspace, targetSkillsDir: skillsDir, skillSlug: 'beta' };
    applyInstall(planInstall(betaOpts), betaOpts);
  ...
  it('install --all then remove a slug from bundle and re-install --all → removed slug pruned silently', () => {
    ...
    const allOpts = { inbrainRoot, targetWorkspace: workspace, targetSkillsDir: skillsDir, skillSlug: null };
    ...
    const pluginManifest = JSON.parse(
      readFileSync(join(inbrainRoot, 'openclaw.plugin.json'), 'utf-8'),
    );
    ...
    try {
      const allOpts2 = { inbrainRoot, targetWorkspace: workspace, targetSkillsDir: skillsDir, skillSlug: null };
  ...
  it('user hand-adds an unknown row → preserved on reinstall AND stderr warning fires', () => {
    ...
    // unambiguously within inbrain's managed block.
    const path = join(skillsDir, 'RESOLVER.md');
    const orig = readFileSync(path, 'utf-8');
    const endMarker = '<!-- inbrain:skillpack:end -->';
   ...
   * inbrain-written before the receipt feature existed). Receipt is
  ...
  it('pre-v0.19 fence (no receipt) → clean rebuild, receipt now present, no warnings', () => {
    ...
    writeFileSync(
      ...
      [
        ...
        '',
        '<!-- Installed by inbrain 0.18.2 — do not hand-edit between markers. -->',
        '',
        '| Trigger | Skill |',
        '|---------|-------|',
        '| "alpha" | `skills/alpha/SKILL.md` |',
    ...
    try {
      const opts = { inbrainRoot, targetWorkspace: workspace, targetSkillsDir: skillsDir, skillSlug: 'beta' };
      applyInstall(planInstall(opts), opts);
    } finally {
      console.error = origErr;
    }
...
describe('diffSkill', () => {
  it('reports missing files', () => {
    ...
    const diffs = diffSkill(inbrainRoot, 'alpha', skillsDir);
