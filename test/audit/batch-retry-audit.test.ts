...
//
// Hermetic: uses INBRAIN_AUDIT_DIR env override via the withEnv helper so
// the test never touches the user's ~/.inbrain/audit/. Each test gets a fresh
...
describe('pruneOldBatchRetryAuditFiles — codex H-8 actual pruning', () => {
  ...
  test('no-op when audit dir does not exist (ENOENT)', async () => {
    // Isolate INBRAIN_AUDIT_DIR at a guaranteed-nonexistent path (CLAUDE.md R1).
    // Pre-fix this called pruneOld with no override, so on a real dev machine it
    // walked ~/.inbrain/audit — which may already hold this-week's batch-retry
    ...
    await withEnv({ INBRAIN_AUDIT_DIR: missing }, async () => {
