 ...
 *
 * Hermetic via `withEnv` for `INBRAIN_AUDIT_DIR` override; tmpdir per
...
function tmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'inbrain-audit-writer-test-'));
}
...
describe('resolveAuditDir', () => {
  it('honors INBRAIN_AUDIT_DIR override', async () => {
    const dir = makeDir();
    await withEnv({ INBRAIN_AUDIT_DIR: dir }, async () => {
      expect(resolveAuditDir()).toBe(dir);
    });
  });

  it('falls back to inbrainPath("audit") when override is unset', async () => {
    await withEnv({ INBRAIN_AUDIT_DIR: undefined }, async () => {
  ...
  it('treats whitespace-only override as unset', async () => {
    await withEnv({ INBRAIN_AUDIT_DIR: '   ' }, async () => {
      const resolved = resolveAuditDir();
...
describe('createAuditWriter — log()', () => {
  ...
  it('best-effort: write failure stderr-warns but does not throw', async () => {
    ...
    // file at `${root}/blocker`, then point INBRAIN_AUDIT_DIR at
    ...
    try {
      await withEnv({ INBRAIN_AUDIT_DIR: badDir }, async () => {
