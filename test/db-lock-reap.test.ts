...
beforeEach(async () => {
  await engine.executeRaw(`DELETE FROM inbrain_cycle_locks`);
});
...
async function lockIds(): Promise<string[]> {
  const rows = await engine.executeRaw<{ id: string }>(`SELECT id FROM inbrain_cycle_locks ORDER BY id`);
  return rows.map(r => r.id);
}

describe('reapDeadHolderLocks', () => {
  test('reaps same-host dead-PID sync + cycle locks (regardless of TTL)', async () => {
    await seedLock('inbrain-sync:src-a', 900001, LOCAL, OLD_S, /*ttlFuture*/ true);  // dead, TTL NOT expired
    await seedLock('inbrain-cycle', 900002, LOCAL, OLD_S, false);                     // dead, TTL expired
    await seedLock('inbrain-cycle:src-b', 900003, LOCAL, OLD_S, true);                // dead, TTL NOT expired

    const { reaped, reapedIds } = await reapDeadHolderLocks(engine, killSeam(new Set()));
    expect(reaped).toBe(3);
    expect(reapedIds.sort()).toEqual(['inbrain-cycle', 'inbrain-cycle:src-b', 'inbrain-sync:src-a']);
    expect(await lockIds()).toEqual([]);
  });

  test('keeps a live same-host holder', async () => {
    await seedLock('inbrain-sync:src-live', process.pid, LOCAL, OLD_S);
    const { reaped } = await reapDeadHolderLocks(engine, killSeam(new Set([process.pid])));
    expect(reaped).toBe(0);
    expect(await lockIds()).toEqual(['inbrain-sync:src-live']);
  });

  test('keeps a dead holder still within the PID-reuse grace window', async () => {
    await seedLock('inbrain-sync:src-young', 900010, LOCAL, YOUNG_S);
    const { reaped } = await reapDeadHolderLocks(engine, killSeam(new Set()));
    expect(reaped).toBe(0);
    expect(await lockIds()).toEqual(['inbrain-sync:src-young']);
  });

  test('keeps a cross-host holder (cannot probe a remote PID)', async () => {
    await seedLock('inbrain-sync:src-xhost', 900011, 'a-different-host', OLD_S);
    const { reaped } = await reapDeadHolderLocks(engine, killSeam(new Set()));
    expect(reaped).toBe(0);
    expect(await lockIds()).toEqual(['inbrain-sync:src-xhost']);
  });

  test('NEVER reaps a non-sync/cycle namespace, even with a dead PID (blast radius)', async () => {
    await seedLock('inbrain-election:leader', 900020, LOCAL, OLD_S);
    await seedLock('some-other-lock', 900021, LOCAL, OLD_S);
    const { reaped } = await reapDeadHolderLocks(engine, killSeam(new Set()));
    expect(reaped).toBe(0);
    expect(await lockIds()).toEqual(['inbrain-election:leader', 'some-other-lock']);
  });
  ...
  test('mixed set: reaps only the eligible rows', async () => {
    await seedLock('inbrain-sync:dead', 900030, LOCAL, OLD_S);          // reap
    await seedLock('inbrain-sync:live', process.pid, LOCAL, OLD_S);     // keep (live)
    await seedLock('inbrain-cycle:xhost', 900031, 'other', OLD_S);      // keep (cross-host)
    await seedLock('inbrain-election:x', 900032, LOCAL, OLD_S);         // keep (namespace)
    const { reaped, reapedIds } = await reapDeadHolderLocks(engine, killSeam(new Set([process.pid])));
    expect(reaped).toBe(1);
    expect(reapedIds).toEqual(['inbrain-sync:dead']);
    expect(await lockIds()).toEqual(['inbrain-cycle:xhost', 'inbrain-election:x', 'inbrain-sync:live']);
  });
});
...
describe('deleteLockRowExact (snapshot-matched, TOCTOU defense)', () => {
  test('no-op when acquired_at does not match; deletes when it does', async () => {
    await seedLock('inbrain-sync:exact', 900040, LOCAL, OLD_S);
    const snap = await inspectLock(engine, 'inbrain-sync:exact');
    ...
    const miss = await deleteLockRowExact(engine, 'inbrain-sync:exact', 900040, wrong);
    expect(miss.deleted).toBe(false);
    expect(await lockIds()).toEqual(['inbrain-sync:exact']);

    // The matching snapshot deletes.
    const hit = await deleteLockRowExact(engine, 'inbrain-sync:exact', 900040, snap!.acquired_at);
    expect(hit.deleted).toBe(true);
    expect(await lockIds()).toEqual([]);
  });

  test('no-op when holder_pid does not match', async () => {
    await seedLock('inbrain-sync:pidguard', 900050, LOCAL, OLD_S);
    const snap = await inspectLock(engine, 'inbrain-sync:pidguard');
    const res = await deleteLockRowExact(engine, 'inbrain-sync:pidguard', 111111, snap!.acquired_at);
    expect(res.deleted).toBe(false);
    expect(await lockIds()).toEqual(['inbrain-sync:pidguard']);
  });
});
