 ...
 *
 * The `inbrain integrations install <recipe-id>` subcommand copies the
 * recipe's bundle into the operator's host repo. This test pins:
 *   - happy-path copy + manifest write
 *   - SHA-256 computed per file matches inbrain-side source
 *   - .inbrain-source.json shape (no upstream_repo field per D11-A)
 *   - path-traversal rejection (absolute, ..)
 *   - refusal to install into inbrain itself (or its parent)
...
function makeScratchRepo(opts: { withGit?: boolean; withAgentsMd?: boolean } = {}): string {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'inbrain-install-test-'));
...
describe('installRecipeIntoHostRepo — happy path', () => {
  ...
  it('writes .inbrain-source.json with the documented shape', async () => {
    await installRecipeIntoHostRepo('agent-voice', { target: scratch });
    const manifestPath = join(scratch, 'services/voice-agent/.inbrain-source.json');
  ...
  it('does NOT carry an upstream_repo field (D11-A privacy invariant)', async () => {
    ...
    const manifest = JSON.parse(
      readFileSync(join(scratch, 'services/voice-agent/.inbrain-source.json'), 'utf8'),
    );
  ...
  it('appends resolver rows to AGENTS.md when present', async () => {
    ...
    expect(agentsMd).toContain('inbrain:agent-voice:resolver-rows');
...
describe('installRecipeIntoHostRepo — dry-run', () => {
  it('writes nothing in dry-run mode', async () => {
    ...
    expect(existsSync(join(scratch, 'services/voice-agent/.inbrain-source.json'))).toBe(false);
  });
});
...
describe('refreshRecipeIntoHostRepo — D3-A refresh mode', () => {
  ...
  it('default refresh (no --auto) preserves locally-modified files', async () => {
    ...
    const manifest = JSON.parse(readFileSync(join(scratch, 'services/voice-agent/.inbrain-source.json'), 'utf8'));
    const entry = manifest.files.find((f: { target: string; sha256: string }) => f.target === 'services/voice-agent/code/server.mjs');
    expect(entry).toBeDefined();
    // The recorded SHA should be the new (operator-edited) hash, not the original inbrain SHA.
  });
  ...
  it('--auto take-theirs overwrites locally-modified files', async () => {
    ...
    // File should be restored to inbrain-side content.
    expect(readFileSync(modPath, 'utf8')).toBe(beforeContent);
  });

  it('writes a transaction journal at .inbrain-source.refresh.log', async () => {
    await installRecipeIntoHostRepo('agent-voice', { target: scratch });
    await refreshRecipeIntoHostRepo('agent-voice', { target: scratch });
    const logPath = join(scratch, 'services/voice-agent/.inbrain-source.refresh.log');
