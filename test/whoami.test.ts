...
describe('whoami op contract', () => {
  ...
  test('oauth transport returns full client identity', async () => {
    ...
    expect(result.client_id).toBe('inbrain_cl_abc');
  ...
  test('legacy transport (token name as clientId, no inbrain_cl_ prefix)', async () => {
    const auth: AuthInfo = {
      token: 'legacy-token',
...
describe('whoami op metadata', () => {
  ...
  test('not localOnly (must work over HTTP MCP for gstack /setup-inbrain)', () => {
