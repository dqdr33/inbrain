/**
 * Tests for `inbrain init --mcp-only` — thin-client setup branch.
 *
 ...
 *
 * Each test sets `INBRAIN_HOME` to a fresh tempdir so the config write is
 * isolated and we can inspect the resulting `~/.inbrain/config.json` without
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'inbrain-init-mcp-only-'));
  discoveryStatus = 200;
  tokenStatus = 200;
  mcpStatus = 200;
});
...
async function run(args: string[], extraEnv: Record<string, string | undefined> = {}): Promise<RunResult> {
  ...
  delete env.INBRAIN_REMOTE_ISSUER_URL;
  delete env.INBRAIN_REMOTE_MCP_URL;
  delete env.INBRAIN_REMOTE_CLIENT_ID;
...
function configPath(): string { return join(tmp, '.inbrain', 'config.json'); }

describe('inbrain init --mcp-only — happy path', () => {
  test('writes remote_mcp config and creates NO local DB', async () => {
    ...
    expect(existsSync(join(tmp, '.inbrain', 'brain.pglite'))).toBe(false);
  ...
  test('env-var-supplied secret is NOT persisted to config file', async () => {
    const r = await run([
      ...
      '--mcp-url', `http://127.0.0.1:${port}/mcp`,
      '--oauth-client-id', 'cid',
    ], { INBRAIN_REMOTE_CLIENT_SECRET: 'env-secret' });
    expect(r.exitCode).toBe(0);
    const cfg = JSON.parse(readFileSync(configPath(), 'utf-8'));
    expect(cfg.remote_mcp).toBeDefined();
...
describe('inbrain init --mcp-only — required-flag errors', () => {
...
describe('inbrain init --mcp-only — pre-flight smoke failures', () => {
...
describe('inbrain init re-run guard', () => {
  function seedThinClientConfig() {
    mkdirSync(join(tmp, '.inbrain'), { recursive: true });
  ...
  test('default `inbrain init` (no flags) refuses when remote_mcp is set', async () => {
  ...
  test('`inbrain init --pglite` refuses when remote_mcp is set', async () => {
    seedThinClientConfig();
  ...
  test('`inbrain init --mcp-only` (no --force) refuses when remote_mcp is already set', async () => {
    seedThinClientConfig();
    const r = await run([
      'init', '--mcp-only', '--json',
      '--issuer-url', `http://127.0.0.1:${port}`,
  ...
  test('`inbrain init --mcp-only --force` overwrites existing thin-client config', async () => {
    seedThinClientConfig();
    const r = await run([
