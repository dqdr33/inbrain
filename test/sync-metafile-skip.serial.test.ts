 ...
 * an older inbrain version (back when isSyncable() didn't filter `log.md`)
 * or via a direct put_page was being deleted on every subsequent
 * `inbrain sync --skip-failed` because the cleanup loop at
...
describe('#1433 — re-sync preserves previously-indexed metafile pages', () => {
  ...
  beforeEach(async () => {
    ...
    repoPath = mkdtempSync(join(tmpdir(), 'inbrain-metafile-'));
    gitInit(repoPath);
  ...
  test('log.md indexed via direct putPage survives re-sync after edit', async () => {
    ...
    // inbrain version or via a hand-rolled put_page call. This is the
