 ...
 *     ~/.inbrain/migrations/pending-host-work.jsonl
 *
...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-v0_14_0-'));
  // INBRAIN_HOME is the parent dir; configDir() appends '.inbrain' itself.
  process.env.INBRAIN_HOME = tmpHome;
});

afterEach(() => {
  if (originalGbrainHome !== undefined) process.env.INBRAIN_HOME = originalGbrainHome;
...
describe('Bug 5 — Phase B host-work entry dedup', () => {
  test('first run writes the entry, second run is a skip', async () => {
    ...
    const hostPath = join(tmpHome, '.inbrain', 'migrations', 'pending-host-work.jsonl');
