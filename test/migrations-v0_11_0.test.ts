...
beforeEach(() => {
  ...
  tmp = mkdtempSync(join(tmpdir(), 'inbrain-v0_11_0-test-'));
...
describe('cron manifest rewrite — inbrain builtins only', () => {
  test('rewrites `agentTurn` entries whose skill is a inbrain builtin', () => {
    const dir = join(tmp, '.claude');
    ...
    expect(after.jobs[0].cmd).toContain('inbrain jobs submit extract');
    expect(after.jobs[0]._inbrain_migrated_by).toBe('v0.11.0');
    expect(after.jobs[1].kind).toBe('shell');
    expect(after.jobs[1].cmd).toContain('inbrain jobs submit backlinks');
  });
