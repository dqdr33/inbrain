...
//
// Coverage: pack_upgrade_available fires on inbrain-base brain;
...
beforeEach(async () => {
  ...
  // inbrain-base / inbrain-base-v2 and findPackSuccessors silently returns
...
describe('checkPackUpgradeAvailable', () => {
  it('fires on inbrain-base brain with inbrain-base-v2 available', async () => {
    // Default active pack is inbrain-base; inbrain-base-v2 declares
    // migration_from: {pack: inbrain-base, version: "1.x"}.
    ...
    expect(result.check.message).toContain('inbrain-base-v2');
    ...
    expect(result.remediations[0].params.target_pack).toBe('inbrain-base-v2');
  });
...
describe('checkTypeProliferation (D16 pack-aware ratio)', () => {
  ...
  it('warns when distinct types exceed declared+5', async () => {
    // inbrain-base declares 24 types. warn threshold = 29.
    const types: string[] = [];
