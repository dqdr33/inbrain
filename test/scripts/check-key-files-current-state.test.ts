...
function run(extraEnv: Record<string, string> = {}) {
  return spawnSync("bash", [SCRIPT], {
    encoding: "utf8",
    env: { ...process.env, INBRAIN_DOC_GUARD_ROOT: root, ...extraEnv },
  });
}
...
describe("check-key-files-current-state.sh", () => {
  ...
  it("FAILS when CLAUDE.md exceeds the size cap", () => {
    ...
    const r = run({ INBRAIN_CLAUDE_MD_MAX_BYTES: "90000" });
    expect(r.status).toBe(1);
  ...
  it("size cap is configurable via env", () => {
    ...
    expect(run({ INBRAIN_CLAUDE_MD_MAX_BYTES: "1000" }).status).toBe(1);
    expect(run({ INBRAIN_CLAUDE_MD_MAX_BYTES: "10000" }).status).toBe(0);
  });
