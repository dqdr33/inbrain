...
function runLintIn(content: string | null): { exitCode: number; stderr: string } {
  const dir = mkdtempSync(join(tmpdir(), 'inbrain-pii-lint-'));
  try {
...
`);
  ...
  test('inbrain-ai/brain private repo reference is flagged', () => {
    const r = runLintIn(`Context: findings resolved in inbrain-ai/brain.\n`);
    expect(r.exitCode).toBe(1);
    expect(r.stderr).toContain('inbrain-ai/brain');
  });
...
`);
  ...
  test('multiple PII patterns reported together with exit 1', () => {
    const r = runLintIn(`
Context: findings from inbrain-ai/brain.
...
`);
  ...
  test('--help exits 1 with usage text', () => {
    const dir = mkdtempSync(join(tmpdir(), 'inbrain-pii-lint-help-'));
    try {
      spawnSync('git', ['init', '-q'], { cwd: dir });
