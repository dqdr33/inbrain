...
/** Build a tempdir scoped to this test run; honored by INBRAIN_AUDIT_DIR. */
function withTempDirs<T>(fn: (dirs: { brainDir: string; auditDir: string }) => Promise<T>): Promise<T> {
  ...
  return withEnv({ INBRAIN_AUDIT_DIR: auditDir }, async () => {
    try {
...
| 2 | Loves jazz | preference | 0.9 | private | medium | 2020-01-01 |  | OH |  |`,
      ...
      const canonicalMd = readMd(brainDir, 'people/alice-example');
      expect(canonicalMd).toContain('<!--- inbrain:facts:begin -->');
      expect(canonicalMd).toContain('<!--- inbrain:facts:end -->');
      // Must NOT regress to two-dash form
      expect(canonicalMd).not.toContain('<!-- inbrain:facts:begin -->');
    });
  });
...
describe('runExtractFacts — phantom-redirect integration', () => {
  ...
  test('P1: cap enforcement via INBRAIN_PHANTOM_REDIRECT_LIMIT', async () => {
    await withTempDirs(async ({ brainDir }) => {
      ...
      // Cap at 2 per cycle
      await withEnv({ INBRAIN_PHANTOM_REDIRECT_LIMIT: '2' }, async () => {
        const result = await runExtractFacts(engine, { sourceId: 'default', brainDir });
...
describe('lock contention (C4)', () => {
  test('lock_busy when another holder has it → pass skipped, audit entry, retries next cycle', async () => {
    await withTempDirs(async ({ brainDir }) => {
      // Manually claim the inbrain-sync lock with a future TTL
      await engine.executeRaw(
        `INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at)
         VALUES ('inbrain-sync', 9999, 'other-host', now(), now() + interval '1 hour')`,
      );
      ...
      const lockBefore = await engine.executeRaw<{ holder_pid: number }>(
        `SELECT holder_pid FROM inbrain_cycle_locks WHERE id='inbrain-sync'`,
      );
      expect(lockBefore[0].holder_pid).toBe(9999);

      // Cleanup
      await engine.executeRaw(`DELETE FROM inbrain_cycle_locks WHERE id='inbrain-sync'`);
    });
  });
});
...
describe('phantom-audit module', () => {
  ...
  test('write failure does not throw (best-effort)', async () => {
    await withEnv({ INBRAIN_AUDIT_DIR: '/dev/null/cannot-mkdir/this-path' }, async () => {
      const { logPhantomEvent } = await import('../src/core/facts/phantom-audit.ts');
