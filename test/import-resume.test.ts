 ...
 *
 * Test isolation:
 *   - `INBRAIN_HOME` env override via `withEnv` so we NEVER touch the real
 *     `~/.inbrain/import-checkpoint.json`. Pre-v0.33.2 this file did exactly
...
let workspace: string;        // INBRAIN_HOME target — `${workspace}/.inbrain/` holds the checkpoint file
let inbrainHomeDir: string;    // Resolves to `${workspace}/.inbrain` — the actual checkpoint dir
let cpPath: string;           // The checkpoint file path inside inbrainHomeDir
...
beforeEach(async () => {
  ...
  workspace = mkdtempSync(join(tmpdir(), 'inbrain-import-resume-home-'));
  // INBRAIN_HOME is the parent dir; configDir() appends '.inbrain' itself.
  // The checkpoint lives at `${workspace}/.inbrain/import-checkpoint.json`.
  inbrainHomeDir = join(workspace, '.inbrain');
  mkdirSync(inbrainHomeDir, { recursive: true });
  cpPath = join(inbrainHomeDir, 'import-checkpoint.json');
  brainDir = mkdtempSync(join(tmpdir(), 'inbrain-import-resume-brain-'));
});
...
describe('runImport checkpoint resume — v0.33.2 path-based', () => {
  test('old positional checkpoint gets discarded with stderr log', async () => {
    await withEnv({ INBRAIN_HOME: workspace }, async () => {
