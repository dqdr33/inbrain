...
describe('F11 held-out path helpers', () => {
  test('capturesDir honors INBRAIN_HOME', async () => {
    await withEnv({ INBRAIN_HOME: tmp }, async () => {
      const dir = capturesDir();
      expect(dir).toBe(path.join(tmp, '.inbrain', 'skillopt-captures'));
    });
  });
  ...
  test('capturePath returns per-skill-per-run JSONL path', async () => {
    await withEnv({ INBRAIN_HOME: tmp }, async () => {
      ...
      expect(p).toBe(path.join(tmp, '.inbrain', 'skillopt-captures', 'my-skill', 'run-123.jsonl'));
    });
  });
});
