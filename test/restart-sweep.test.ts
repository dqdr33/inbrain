 ...
 * file tests scope `INBRAIN_HOME` to a per-test tmpdir so they don't
 * touch the developer's real `~/.inbrain/`.
 */
...
describe('loadAlerted', () => {
  test('returns empty Map when alerted.json does not exist', async () => {
    ...
    await withEnv({ INBRAIN_HOME: stateRoot, OPENCLAW_TELEGRAM_GROUP: '-100' }, async () => {
      const Detector = await loadDetector();
      const d = new Detector();
...
// ─── INBRAIN_HOME state path override (NEW, D2 verification) ────────────

test('INBRAIN_HOME env override redirects state file to env-pointed dir', async () => {
  const customRoot = makeStateDir();
  await withEnv({ INBRAIN_HOME: customRoot, OPENCLAW_TELEGRAM_GROUP: '-100' }, async () => {
    const Detector = await loadDetector();
    const d = new Detector();
