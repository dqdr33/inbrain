...
function buildPackFixture(root: string, opts: FixtureOptions = {}): SkillpackManifest {
  ...
  if (opts.withBootstrap) {
    ...
    writeFileSync(
      ...
      '1. agent: inbrain put_page wiki/example\n2. show user: "Pack installed."\n',
    );
  }
  return manifest;
}
...
describe('runScaffoldThirdParty — happy path', () => {
  ...
  test('displays bootstrap.md when declared and present', async () => {
    ...
    expect(result.bootstrap.text).toContain('agent: inbrain put_page wiki/example');
  });
});

describe('runScaffoldThirdParty — inbrain version gate', () => {
  test('rejects when current version is below inbrain_min_version', async () => {
    freshSandbox();
    buildPackFixture(packDir, { manifestOverrides: { inbrain_min_version: '99.0.0' } });
    const resolved = resolveSource(packDir);
    ...
    try {
      ...
      expect((err as ScaffoldThirdPartyError).code).toBe('inbrain_version_too_old');
    }
  });

  test('accepts exact-version match', async () => {
    freshSandbox();
    buildPackFixture(packDir, { manifestOverrides: { inbrain_min_version: '0.36.0' } });
    const resolved = resolveSource(packDir);
