...
function makeTempDir(): string {
  return mkdtempSync(join(tmpdir(), 'inbrain-cme-cli-'));
}
...
describe('cross-modal-eval CLI helpers', () => {
  ...
  test('INBRAIN_HOME isolates receipts under <home>/.inbrain/eval-receipts', async () => {
    const dir = makeTempDir();
    try {
      await withEnv({ INBRAIN_HOME: dir }, async () => {
        const { inbrainPath } = await import('../src/core/config.ts');
        const path = inbrainPath('eval-receipts');
        expect(path.startsWith(dir)).toBe(true);
        expect(path.endsWith('eval-receipts')).toBe(true);
        expect(path.includes('.inbrain')).toBe(true);
      });
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
