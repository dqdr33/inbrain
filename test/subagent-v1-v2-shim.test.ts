 ...
 *
 * The crash-replay reconciliation key in v0.38+ is a inbrain-owned UUID v7
 * (ordinal + inbrain_tool_use_id columns added in migration v81). Pre-v0.38
 ...
 *   - loadPriorToolsV2: v1 rows (ordinal=NULL, inbrain_tool_use_id=NULL)
...
describe('loadPriorToolsV2 (D5 — synthesizes stable keys for v1 rows)', () => {
  ...
  it('uses inbrain_tool_use_id as stable key for v2 rows', async () => {
    ...
    const inbrainId = '01987654-3210-7000-8000-000000000001';
    ...
    expect(rows[0].stableKey).toBe(inbrainId);
    expect(rows[0].status).toBe('complete');
    expect(rows[0].output).toBe('results');
  });

  it('synthesizes a legacy-prefixed stable key for v1 rows (ordinal NULL, inbrain_tool_use_id NULL)', async () => {
    const job = await engine.executeRaw<{ id: number }>(
      `INSERT INTO minion_jobs (name, status, data, queue, priority, created_at)
       VALUES ('subagent', 'active', '{}'::jsonb, 'default', 0, now())
       RETURNING id`,
  ...
  it('returns v1 + v2 rows side-by-side with both stable-key shapes resolving', async () => {
    ...
    const jobId = job[0].id;
    // v1 row: ordinal NULL, inbrain_tool_use_id NULL
    await engine.executeRaw(
      `INSERT INTO subagent_tool_executions
         (job_id, message_idx, tool_use_id, tool_name, input, status, schema_version, output)
    ...
    // v2 row: ordinal + inbrain_tool_use_id populated
    const inbrainId = '01987654-3210-7000-8000-000000000002';
    await engine.executeRaw(
      `INSERT INTO subagent_tool_executions
         (job_id, message_idx, tool_use_id, tool_name, input, status,
          schema_version, ordinal, inbrain_tool_use_id, output)
    ...
    expect(keys).toContain(inbrainId);
  });
