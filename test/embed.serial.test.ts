...
afterEach(() => {
  delete process.env.INBRAIN_EMBED_CONCURRENCY;
  delete process.env.INBRAIN_EMBED_TIME_BUDGET_MS;
});
...
describe('runEmbed --all (parallel)', () => {
  test('runs embedBatch calls concurrently across pages', async () => {
    ...
    process.env.INBRAIN_EMBED_CONCURRENCY = '10';
  ...
  // the worker (and lets the cycle's finally release inbrain_cycle_locks)
  ...
  test('respects INBRAIN_EMBED_CONCURRENCY=1 (serial)', async () => {
    ...
    process.env.INBRAIN_EMBED_CONCURRENCY = '1';
  ...
  test('skips pages whose chunks are all already embedded when --stale', async () => {
    ...
    process.env.INBRAIN_EMBED_CONCURRENCY = '5';
...
describe('runEmbedCore --dry-run never calls the embedding model', () => {
  ...
  test('non-dry-run path reports accurate embedded count (regression guard)', async () => {
    ...
    process.env.INBRAIN_EMBED_CONCURRENCY = '2';
...
describe('embedAllStale wall-clock budget end-to-end (D3 + D3a)', () => {
  test('INBRAIN_EMBED_TIME_BUDGET_MS=N cuts the outer loop short on stuck workers', async () => {
    const { runEmbedCore } = await import('../src/commands/embed.ts');
    ...
    process.env.INBRAIN_EMBED_TIME_BUDGET_MS = '100';
    process.env.INBRAIN_EMBED_CONCURRENCY = '1';
