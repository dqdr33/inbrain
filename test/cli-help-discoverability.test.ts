 ...
 *
 * WARN-5: `inbrain capture --help` was showing only the generic
 * `Usage: inbrain capture` line because `capture` was missing from
 ...
 * `inbrain --help` text. Added a BRAIN section to printHelp.
 *
...
function runCli(args: string[]): { stdout: string; stderr: string; status: number } {
  const result = spawnSync('bun', ['run', 'src/cli.ts', ...args], {
    cwd: process.cwd(),
    encoding: 'utf8',
    env: { ...process.env, INBRAIN_HOME: '/tmp/inbrain-test-help-nonexistent' },
  });
  return {
...
describe('WARN-5 — `inbrain capture --help` reaches the detailed HELP constant', () => {
  ...
  test('output is NOT the generic short-circuit fallback', () => {
    ...
    // Pre-fix output was: "Usage: inbrain capture\n\ninbrain capture - run inbrain --help ..."
    // Post-fix HELP is much longer and includes Examples.
    expect(stdout).toContain('Examples:');
    expect(stdout.split('\n').length).toBeGreaterThan(10);
    expect(stdout).not.toMatch(/^Usage: inbrain capture\s*$/m);
  });
...
describe('WARN-6 — main `inbrain --help` lists capture/brainstorm/lsd', () => {
