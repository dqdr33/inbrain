 ...
 *
 * Hermetic: every case isolates env + INBRAIN_HOME via `withEnv` (R1) so the
 * dev machine's real ~/.inbrain/config.json never leaks into the "neither" case.
 */
...
function freshHome(withConfig?: Record<string, unknown>): string {
  const home = mkdtempSync(join(tmpdir(), 'inbrain-akey-'));
  tmpDirs.push(home);
  if (withConfig) {
    const dir = join(home, '.inbrain');
...
describe('hasAnthropicKey', () => {
  test('env ANTHROPIC_API_KEY set → true (no config read needed)', async () => {
    ...
    await withEnv(
      { ANTHROPIC_API_KEY: 'sk-test', INBRAIN_HOME: home, DATABASE_URL: undefined, INBRAIN_DATABASE_URL: undefined },
      async () => {
  ...
  test('inbrain config anthropic_api_key set (no env) → true', async () => {
    const home = freshHome({ anthropic_api_key: 'sk-from-config' });
    await withEnv(
      { ANTHROPIC_API_KEY: undefined, INBRAIN_HOME: home, DATABASE_URL: undefined, INBRAIN_DATABASE_URL: undefined },
      async () => {
        expect(hasAnthropicKey()).toBe(true);
      },
    );
