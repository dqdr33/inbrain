 ...
 * (C1 #6). probeChatModel adds the key layer via hasAnthropicKey (env OR inbrain config
 * file — also gateway-config-independent). Non-Anthropic providers pass the probe (lazy
 * key check deferred to gateway.chat).
 *
 * Hermetic: key-sensitive cases isolate env + INBRAIN_HOME via withEnv (R1) so the dev
 * machine's real ~/.inbrain/config.json never leaks in.
 */
...
function emptyHome(): string {
  const d = mkdtempSync(join(tmpdir(), 'inbrain-probe-'));
  tmpDirs.push(d);
  return d;
}
afterEach(() => {
...
// No-key env: ANTHROPIC_API_KEY unset + INBRAIN_HOME pointed at an empty dir so the
// config-file branch of hasAnthropicKey finds nothing.
const noKeyEnv = () => ({ ANTHROPIC_API_KEY: undefined, INBRAIN_HOME: emptyHome() });
const withKeyEnv = () => ({ ANTHROPIC_API_KEY: 'sk-test', INBRAIN_HOME: emptyHome() });
