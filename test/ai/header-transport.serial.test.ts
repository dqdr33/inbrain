...
const SYNTHETIC_EMBED_RECIPE: Recipe = {
  ...
  default_headers: {
    'HTTP-Referer': 'https://inbrain.ai',
    'X-OpenRouter-Title': 'inbrain',
    'X-Title': 'inbrain',
  },
...
describe('transport-level header sweep (v0.37.2.0)', () => {
  test('1. embed — SDK applies Authorization + default_headers on every request', async () => {
    ...
    expect(h['http-referer'], 'HTTP-Referer must reach the wire').toBe('https://inbrain.ai');
    expect(h['x-openrouter-title'], 'X-OpenRouter-Title must reach the wire').toBe('inbrain');
    expect(h['x-title'], 'X-Title (back-compat) must reach the wire').toBe('inbrain');
  });
  ...
  test('2. chat — SDK applies Authorization + default_headers on every request', async () => {
    ...
    expect(h['http-referer']).toBe('https://inbrain.ai');
    expect(h['x-openrouter-title']).toBe('inbrain');
    expect(h['x-title']).toBe('inbrain');
  });
  ...
  test('3. rerank — manual HTTP path applies Authorization + default_headers', async () => {
    ...
    expect(h['http-referer'], 'rerank: HTTP-Referer must reach the wire').toBe('https://inbrain.ai');
    expect(h['x-openrouter-title']).toBe('inbrain');
    expect(h['x-title']).toBe('inbrain');
