...
describe('inspectLock', () => {
  test('returns null when no row exists', async () => {
    const snap = await inspectLock(engine, 'inbrain-sync:test-source');
    expect(snap).toBeNull();
  });

  test('returns LockSnapshot shape after tryAcquireDbLock', async () => {
    const handle = await tryAcquireDbLock(engine, 'inbrain-sync:default');
    expect(handle).not.toBeNull();
    const snap = await inspectLock(engine, 'inbrain-sync:default');
    expect(snap).not.toBeNull();
    expect(snap!.id).toBe('inbrain-sync:default');
  ...
  test('ttl_expired=true after the TTL has elapsed', async () => {
    ...
    await (engine as any).db.query(
      `INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at)
       VALUES ($1, $2, $3, NOW() - INTERVAL '1 hour', NOW() - INTERVAL '30 minutes')`,
      ['inbrain-sync:stale', 99999, 'old-host'],
    );
    const snap = await inspectLock(engine, 'inbrain-sync:stale');
    expect(snap).not.toBeNull();
...
describe('listStaleLocks', () => {
  test('returns empty when no stale rows exist', async () => {
    const handle = await tryAcquireDbLock(engine, 'inbrain-sync:fresh');
    expect(handle).not.toBeNull();
  ...
  test('returns only rows where ttl_expires_at < NOW()', async () => {
    ...
    const handle = await tryAcquireDbLock(engine, 'inbrain-sync:still-live');
    expect(handle).not.toBeNull();
    await (engine as any).db.query(
      `INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at)
       VALUES ($1, $2, $3, NOW() - INTERVAL '1 hour', NOW() - INTERVAL '30 minutes')`,
      ['inbrain-sync:stale-A', 11111, 'host-A'],
    );
    await (engine as any).db.query(
      `INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at)
       VALUES ($1, $2, $3, NOW() - INTERVAL '2 hours', NOW() - INTERVAL '1 hour')`,
      ['inbrain-sync:stale-B', 22222, 'host-B'],
    );

    const stale = await listStaleLocks(engine);
    const ids = stale.map(s => s.id).sort();
    expect(ids).toEqual(['inbrain-sync:stale-A', 'inbrain-sync:stale-B']);
  ...
  test('orders by acquired_at ascending', async () => {
    await (engine as any).db.query(
      `INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at)
       VALUES ($1, $2, $3, NOW() - INTERVAL '1 hour', NOW() - INTERVAL '30 minutes')`,
      ['inbrain-sync:newer-stale', 11111, 'h1'],
    );
    await (engine as any).db.query(
      `INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at)
       VALUES ($1, $2, $3, NOW() - INTERVAL '5 hours', NOW() - INTERVAL '4 hours')`,
      ['inbrain-sync:older-stale', 22222, 'h2'],
    );

    const stale = await listStaleLocks(engine);
    // Older-stale was acquired first → ordered first.
    expect(stale[0].id).toBe('inbrain-sync:older-stale');
    expect(stale[1].id).toBe('inbrain-sync:newer-stale');
  });
});

describe('deleteLockRow', () => {
  test('deletes the row + RETURNING returns it when (id, pid) matches', async () => {
    const handle = await tryAcquireDbLock(engine, 'inbrain-sync:to-delete');
    expect(handle).not.toBeNull();
    const result = await deleteLockRow(engine, 'inbrain-sync:to-delete', process.pid);
    expect(result.deleted).toBe(true);
    // Row should be gone.
    const snap = await inspectLock(engine, 'inbrain-sync:to-delete');
    expect(snap).toBeNull();
  ...
  test('returns deleted=false when row was already cleared (race)', async () => {
    ...
    await (engine as any).db.query(
      ...
      ['inbrain-sync:race-target', 11111, 'h1'],
    );
    await (engine as any).db.query(
      `DELETE FROM inbrain_cycle_locks WHERE id = $1`,
      ['inbrain-sync:race-target'],
    );
    const result = await deleteLockRow(engine, 'inbrain-sync:race-target', 11111);
  ...
  test('refuses to delete when pid does not match (preserves cross-host safety)', async () => {
    await (engine as any).db.query(
      `INSERT INTO inbrain_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at)
       VALUES ($1, $2, $3, NOW(), NOW() + INTERVAL '30 minutes')`,
      ['inbrain-sync:wrong-pid', 11111, 'h1'],
    );
    const result = await deleteLockRow(engine, 'inbrain-sync:wrong-pid', 22222);
    expect(result.deleted).toBe(false);
    // Row should still exist.
    const snap = await inspectLock(engine, 'inbrain-sync:wrong-pid');
    expect(snap).not.toBeNull();
  ...
  test('atomic shape: single round trip (RETURNING in same statement)', async () => {
    ...
    const handle = await tryAcquireDbLock(engine, 'inbrain-sync:atomic-test');
    expect(handle).not.toBeNull();
    const r = await deleteLockRow(engine, 'inbrain-sync:atomic-test', process.pid);
    expect(r.deleted).toBe(true);
    // Calling again is a no-op (idempotent).
    const r2 = await deleteLockRow(engine, 'inbrain-sync:atomic-test', process.pid);
