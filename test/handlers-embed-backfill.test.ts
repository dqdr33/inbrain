...
beforeEach(async () => {
  ...
  await engine.executeRaw(`DELETE FROM inbrain_cycle_locks WHERE id LIKE 'inbrain-embed-backfill:%'`);
});
...
describe('embed-backfill handler — D2 lock contract', () => {
  test('IRON-RULE: second call returns already_in_progress when lock is held', async () => {
    ...
    const lock = await tryAcquireDbLock(engine, 'inbrain-embed-backfill:default', 60);
  ...
  test('different sources do not contend on each other locks', async () => {
    ...
    const lockA = await tryAcquireDbLock(engine, 'inbrain-embed-backfill:default', 60);
