 ...
 *
 * Real in-memory PGLite (canonical block, R3+R4). INBRAIN_HOME is pointed at an
 ...
 * independent of the developer's real ~/.inbrain config.
 */
...
const EMPTY_HOME = mkdtempSync(join(tmpdir(), 'inbrain-xa-backlog-home-'));
...
describe('computeExtractAtomsBacklogCheck (issue #1678)', () => {
  it('OK with no backlog', async () => {
    const check = await withEnv({ INBRAIN_HOME: EMPTY_HOME }, () =>
