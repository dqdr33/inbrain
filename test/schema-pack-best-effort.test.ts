...
beforeEach(() => {
  ...
  _resetPackLocatorForTests();
  tmpDir = mkdtempSync(join(tmpdir(), 'inbrain-best-effort-test-'));
});
...
describe('loadActivePackBestEffort', () => {
  it('returns ResolvedPack when load succeeds (default bundled inbrain-base)', async () => {
    // No locator override → defaults to bundled inbrain-base resolution.
    await withEnv({ INBRAIN_HOME: tmpDir, INBRAIN_SCHEMA_PACK: undefined }, async () => {
      ...
      expect(result?.manifest.name).toBe('inbrain-base');
    });
  });

  it('returns null when the resolved pack is not on disk', async () => {
    __setPackLocatorForTests(() => null);
    await withEnv({ INBRAIN_HOME: tmpDir, INBRAIN_SCHEMA_PACK: 'never-installed' }, async () => {
      const result = await loadActivePackBestEffort(fakeCtx());
  ...
  it('returns null when the pack file is corrupt', async () => {
    ...
    await withEnv({ INBRAIN_HOME: tmpDir, INBRAIN_SCHEMA_PACK: 'corrupt-pack' }, async () => {
      const result = await loadActivePackBestEffort(fakeCtx());
      expect(result).toBeNull();
    });
  });
