...
describe('CLI structure', () => {
  ...
  // rejected `inbrain reindex` with "Unknown command: reindex" before the
...
describe('CLI dispatch integration', () => {
  test('--version outputs version', async () => {
    ...
    const stdout = await new Response(proc.stdout).text();
    await proc.exited;
    expect(stdout.trim()).toMatch(/^inbrain \d+\.\d+\.\d+/);
  });
  ...
  test('per-command --help prints usage without DB connection', async () => {
    ...
    const stdout = await new Response(proc.stdout).text();
    const exitCode = await proc.exited;
    expect(stdout).toContain('Usage: inbrain get');
  ...
  test('upgrade --help prints usage without running upgrade', async () => {
    ...
    const stdout = await new Response(proc.stdout).text();
    const exitCode = await proc.exited;
    expect(stdout).toContain('Usage: inbrain upgrade');
    expect(exitCode).toBe(0);
  });
  ...
  test('sync --help prints sync-specific usage block without running sync (v0.37 D.4)', async () => {
    ...
    // CLI_ONLY_SELF_HELP so `inbrain sync --help` reaches runSync's own
    ...
    const home = mkdtempSync(join(tmpdir(), 'inbrain-cli-help-'));
    try {
      ...
      const stderr = await new Response(proc.stderr).text();
      const exitCode = await proc.exited;
      expect(stdout).toContain('Usage: inbrain sync');
      ...
      expect(existsSync(join(home, '.inbrain', 'config.json'))).toBe(false);
      expect(exitCode).toBe(0);
    } finally {
  ...
  test('doctor --help short-circuits CLI-only dispatch without diagnostics', async () => {
    ...
    try {
      ...
      const stderr = await new Response(proc.stderr).text();
      const exitCode = await proc.exited;
      expect(stdout).toContain('Usage: inbrain doctor');
  ...
  test('init --help short-circuits CLI-only dispatch without writing config', async () => {
    ...
    try {
      ...
      const stdout = await new Response(proc.stdout).text();
      const exitCode = await proc.exited;
      expect(stdout).toContain('Usage: inbrain init');
      expect(existsSync(join(home, '.inbrain', 'config.json'))).toBe(false);
      expect(exitCode).toBe(0);
    } finally {
      rmSync(home, { recursive: true, force: true });
  ...
  test('--help prints global help', async () => {
    ...
    expect(stdout).toContain('inbrain <command>');
    expect(exitCode).toBe(0);
  });
