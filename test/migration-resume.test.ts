 ...
 * stomp the real ~/.inbrain/migrations/completed.jsonl.
 */
...
const originalGbrainHome = process.env.INBRAIN_HOME;

beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-migration-resume-'));
  // preferences.ts's inbrainDir() returns `$HOME/.inbrain` when INBRAIN_HOME
  // is unset. Set HOME only; clear any inherited INBRAIN_HOME so the test
  // body matches the migrations dir at `$tmpHome/.inbrain/migrations/`.
...
afterEach(() => {
  ...
  if (originalGbrainHome) process.env.INBRAIN_HOME = originalGbrainHome;
