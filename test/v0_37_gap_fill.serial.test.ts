 ...
 *  - ZEROENTROPY_API_KEY env merge into InbrainConfig (Lane C.3)
...
describe('Lane B — init precedence chain (CLI > env > existing file > default)', () => {
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-v37-b-'));
    origHome = process.env.INBRAIN_HOME;
    process.env.INBRAIN_HOME = tmpHome;
  });

  afterEach(() => {
    rmSync(tmpHome, { recursive: true, force: true });
    if (origHome === undefined) delete process.env.INBRAIN_HOME;
    else process.env.INBRAIN_HOME = origHome;
  });
  ...
  test('configureGatewayWithMergedPrecedence honors CLI > env > file > gateway-default', async () => {
    ...
    const dotinbrain = join(tmpHome, '.inbrain');
    require('fs').mkdirSync(dotinbrain, { recursive: true });
    writeFileSync(join(dotinbrain, 'config.json'), JSON.stringify({
      engine: 'pglite',
      database_path: join(dotinbrain, 'brain.pglite'),
    ...
    await withEnv(
      { INBRAIN_EMBEDDING_MODEL: 'openai:text-embedding-3-small', INBRAIN_EMBEDDING_DIMENSIONS: '768' },
      async () => {
        ...
        rg1();
        cg1({
          embedding_model: process.env.INBRAIN_EMBEDDING_MODEL ?? 'voyage:voyage-3-large',
          embedding_dimensions: parseInt(process.env.INBRAIN_EMBEDDING_DIMENSIONS!, 10),
          env: { ...process.env },
        });
...
// ─────────────────────────────────────────────────────────────────────
// Lane C.3 — ZEROENTROPY_API_KEY env merge into InbrainConfig
// ─────────────────────────────────────────────────────────────────────
describe('Lane C.3 — env ZEROENTROPY_API_KEY merges into loadConfig', () => {
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-v37-c-'));
    origHome = process.env.INBRAIN_HOME;
    process.env.INBRAIN_HOME = tmpHome;
...
describe('reinit-pglite — backup + reinit', () => {
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-v37-reinit-'));
    origHome = process.env.INBRAIN_HOME;
    process.env.INBRAIN_HOME = tmpHome;
    ...
    require('fs').mkdirSync(join(dotinbrain, 'brain.pglite'), { recursive: true });
    writeFileSync(join(dotinbrain, 'brain.pglite', 'placeholder'), 'stub');
  });
