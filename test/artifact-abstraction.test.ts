...
describe('v0.39 T14 — artifact abstraction', () => {
  test('detectArtifactKind by extension', () => {
    expect(detectArtifactKind('/tmp/foo.inbrain-schema')).toBe('schemapack');
    expect(detectArtifactKind('/tmp/foo.inbrain-skillpack')).toBe('skillpack');
    expect(detectArtifactKind('/tmp/foo.tar.gz')).toBe(null);
  });

  test('targetDirForKind routes to distinct subdirectories', () => {
    expect(targetDirForKind('schemapack', '/home/u/.inbrain')).toBe('/home/u/.inbrain/schema-packs');
    expect(targetDirForKind('skillpack', '/home/u/.inbrain')).toBe('/home/u/.inbrain/skillpacks');
  });

  test('validateManifestByKind: schemapack happy path', () => {
    expect(() => validateManifestByKind('schemapack', { api_version: 'inbrain-schema-pack-v1' })).not.toThrow();
  });

  test('validateManifestByKind: skillpack happy path', () => {
    expect(() => validateManifestByKind('skillpack', { api_version: 'inbrain-skillpack-v1' })).not.toThrow();
  });

  test('validateManifestByKind: rejects wrong api_version', () => {
    expect(() => validateManifestByKind('schemapack', { api_version: 'inbrain-skillpack-v1' })).toThrow(/api_version/);
    expect(() => validateManifestByKind('skillpack', { api_version: 'inbrain-schema-pack-v1' })).toThrow(/api_version/);
  });
