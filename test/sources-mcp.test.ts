...
const FAKE_GIT_DIR = join(tmpdir(), `inbrain-sources-mcp-test-${process.pid}`);
...
function ctxRemote(scopes: string[]): OperationContext {
  const auth: AuthInfo = {
    token: 'inbrain_at_xxx',
    clientId: 'inbrain_cl_test',
...
describe('sources_* handlers — happy path', () => {
  test('sources_add (with --url) clones, INSERTs, returns row', async () => {
    await withEnv({ INBRAIN_HOME, PATH: fakePath() }, async () => {
...
describe('sources_* scope enforcement (simulates serve-http gate)', () => {
  ...
  test('gstack /setup-inbrain Path 4 token (read + sources_admin) covers everything', () => {
...
describe('sources_add — remote callers ignore path/clone_dir overrides', () => {
  test('remote sources_admin: clone_dir override is silently ignored', async () => {
    await withEnv({ INBRAIN_HOME, PATH: fakePath() }, async () => {
      ...
      const row = (await op.handler(ctx, {
        id: 'attack-clone-dir',
        url: 'https://github.com/example/repo',
        clone_dir: '/etc/inbrain-pwned',  // attacker-supplied
      })) as any;
      // Clone landed at the SAFE default, not /etc/inbrain-pwned.
      expect(row.local_path).not.toBe('/etc/inbrain-pwned');
      expect(row.local_path).toContain('clones/attack-clone-dir');
      // /etc/inbrain-pwned was never written.
      expect(existsSync('/etc/inbrain-pwned')).toBe(false);
    });
  });
  ...
  test('local CLI caller (ctx.remote=false) keeps clone_dir override', async () => {
    await withEnv({ INBRAIN_HOME, PATH: fakePath() }, async () => {
      ...
      const customDir = join(INBRAIN_HOME, 'custom-clones', 'local-override');
