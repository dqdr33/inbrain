...
describe('generateSystemdUnit', () => {
  const unit = generateSystemdUnit('/home/u/.inbrain/autopilot-run.sh');
  ...
  test('runs the given wrapper path', () => {
    expect(unit).toContain('ExecStart=/home/u/.inbrain/autopilot-run.sh');
  });
});
...
describe('autopilot self-upgrade static-shape regressions', () => {
  ...
  test('the silent channel does swap-only, never a blocking full post-upgrade in the tick', () => {
    expect(AUTOPILOT_SRC).toContain("execSync('inbrain upgrade --swap-only'");
    // The tick must not invoke the (up-to-30-min) post-upgrade inline.
    expect(AUTOPILOT_SRC).not.toContain("execSync('inbrain post-upgrade'");
  });
