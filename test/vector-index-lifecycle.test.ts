...
describe('checkActiveBuild', () => {
  ...
  test('Postgres with an active build returns the row', async () => {
    const fakeEngine = {
      kind: 'postgres' as const,
      executeRaw: async () => [
        { pid: 12345, query: 'CREATE INDEX CONCURRENTLY idx_chunks_embedding ON ...', application_name: 'inbrain' },
      ],
    } as never;
    const r = await checkActiveBuild(fakeEngine, 'idx_chunks_embedding');
    expect(r.active).toBe(true);
    expect(r.pid).toBe(12345);
    expect(r.application_name).toBe('inbrain');
  });
...
describe('isSupabaseAutoMaintenance', () => {
  ...
  test('false for inbrain', () => {
    expect(isSupabaseAutoMaintenance({ active: true, application_name: 'inbrain-worker' })).toBe(false);
  });
    ...
    const fakeEngine = {
      ...
      executeRaw: async (sql: string) => {
        if (sql.includes('pg_stat_activity')) {
          return [{ pid: 555, query: 'CREATE INDEX zombie_idx_a ...', application_name: 'inbrain' }];
        }
        if (sql.includes('pg_index')) {
