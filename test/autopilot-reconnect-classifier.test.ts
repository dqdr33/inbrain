...
describe('classifyReconnectError (#1162)', () => {
  ...
  test('auth failure → unrecoverable (creds don\'t fix themselves)', () => {
    expect(classifyReconnectError(new Error('password authentication failed for user "inbrain"'))).toBe('unrecoverable');
  ...
  test('no brain configured → unrecoverable', () => {
    expect(classifyReconnectError(new Error('No brain configured. Run: inbrain init'))).toBe('unrecoverable');
  });
...
describe('generateLaunchdPlist (#1162)', () => {
  test('plist contains ThrottleInterval=60', () => {
    const plist = generateLaunchdPlist('/Users/me/.inbrain/autopilot-run.sh', '/Users/me');
  ...
  test('plist writes StandardErrorPath under the home dir (#1162 — error visibility)', () => {
    ...
    expect(plist).toContain('/Users/alice/.inbrain/autopilot.err');
  });
});
