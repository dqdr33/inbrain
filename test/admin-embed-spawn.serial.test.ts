/**
 * v0.36.1.x #1090: admin embed E2E — spawns `inbrain serve --http` from a
 ...
 * github:inbrain-ai/inbrain`) hit 404 on /admin because the user never
...
async function spawnServer(): Promise<ServeProc> {
  const home = mkdtempSync(join(tmpdir(), 'inbrain-admin-embed-'));
  ...
  const proc = Bun.spawn(
    ...
    ],
    {
      ...
      env: {
        ...
        HOME: home,
        INBRAIN_HOME: home,
        INBRAIN_ADMIN_BOOTSTRAP_TOKEN: bootstrapToken,
...
describe('admin embed E2E — /admin served from embedded manifest (v0.36.1.x #1090)', () => {
  test('GET /admin/ returns 200 with the React SPA shell HTML', async () => {
    ...
    try {
      ...
      // The actual admin/dist/index.html declares <title>Inbrain Admin</title>
