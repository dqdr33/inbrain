...
describe('check-resolvable — unit: resolveSkillsDir', () => {
  ...
  it('v0.31.7: empty cwd falls back to install-path (finds bundled skills/)', () => {
    ...
    // install-path fallback then walks up from the inbrain module's own
    ...
    try {
      ...
      // Install-path fallback succeeds when test runs inside the inbrain repo.
      expect(r.error).toBeNull();
      expect(r.dir).toMatch(/\/skills$/);
  ...
  it('finds skills via cwd_walk_up when cwd is inside a repo (no --skills-dir)', () => {
    // Running from this test file — we're inside the real inbrain repo.
    // v0.33 added the cwd_walk_up tier ahead of repo_root, so the same
    // skills/ dir is matched via the broader (no inbrain-shape gate)
...
describe('inbrain check-resolvable CLI — integration', () => {
  ...
  it('prints usage and exits 0 on --help', () => {
    ...
    expect(r.stdout).toContain('inbrain check-resolvable');
  ...
  // Without this gate, `cd ~ && inbrain check-resolvable --fix` would silently
  // mutate SKILL.md files in the bundled inbrain repo via autoFixDryViolations.
  ...
  it('v0.31.7 D6: --fix refuses when source is install_path', () => {
    ...
    try {
      ...
      const r = spawnSync('bun', ['run', CLI, 'check-resolvable', '--fix'], {
        cwd: empty,
        env: { ...process.env, OPENCLAW_WORKSPACE: '', INBRAIN_SKILLS_DIR: '' },
        encoding: 'utf-8',
      });
      expect(r.status).toBe(1);
      expect(r.stderr).toContain('install-path fallback');
      expect(r.stderr).toContain('refused');
      expect(r.stderr).toMatch(/INBRAIN_SKILLS_DIR|OPENCLAW_WORKSPACE|--skills-dir/);
    } finally {
      rmSync(empty, { recursive: true, force: true });
    }
  });
