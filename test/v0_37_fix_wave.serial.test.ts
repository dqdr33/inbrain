...
describe('v0.37 Lane B — init paths', () => {
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-v37-test-'));
  ...
  test('B.4 / T-3: loadConfigFileOnly ignores env overrides', async () => {
    ...
    require('fs').mkdirSync(join(tmpHome, '.inbrain'), { recursive: true });
    ...
    process.env.INBRAIN_EMBEDDING_MODEL = 'voyage:voyage-3-large';
    process.env.INBRAIN_EMBEDDING_DIMENSIONS = '2048';
    ...
    delete process.env.INBRAIN_EMBEDDING_MODEL;
    delete process.env.INBRAIN_EMBEDDING_DIMENSIONS;
...
describe('v0.37 Lane C.3 — ZE key reaches buildGatewayConfig', () => {
  ...
  test('InbrainConfig type includes zeroentropy_api_key field (TS compile guard)', async () => {
...
// Deferred-TODO ship: inbrain reinit-pglite
describe('v0.37 deferred TODO shipped — inbrain reinit-pglite', () => {
  ...
  test('embeddingMismatchMessage PGLite branch recommends `inbrain reinit-pglite`', async () => {
    ...
    expect(msg).toContain('inbrain reinit-pglite --embedding-model zeroentropyai:zembed-1 --embedding-dimensions 1280');
