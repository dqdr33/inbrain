...
// inbrain-unify lock held; verify-step thresholds.
...
describe('runUnifyTypes', () => {
  describe('preflight', () => {
    it('refuses target pack with no mapping_rules', async () => {
      // inbrain-base has no mapping_rules
      await expect(runUnifyTypes(ctxOf(), {
        target_pack: 'inbrain-base',
        apply: false,
  ...
  describe('apply (full lifecycle)', () => {
    it('runs all 4 phases + active-pack flip (D13)', async () => {
      ...
      expect(result.pack_identity_after).toContain('inbrain-base-v2');
