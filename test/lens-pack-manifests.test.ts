...
//   - inbrain-everything unions all three lens packs' contributions
...
describe('v0.41 T4: all 4 bundled lens packs parse cleanly', () => {
  for (const name of PACK_NAMES) {
    test(`${name}.yaml parses via parseSchemaPackManifest without error`, () => {
      ...
      expect(pack.api_version).toBe('inbrain-schema-pack-v1');
    });
  }
});
...
describe('v0.41 T4: inbrain-creator manifest shape', () => {
  const pack = loadPack('inbrain-creator');

  test('extends inbrain-base', () => {
    expect(pack.extends).toBe('inbrain-base');
  });
...
describe('v0.41 T4: inbrain-investor manifest shape', () => {
  const pack = loadPack('inbrain-investor');

  test('extends inbrain-base + borrows deal/person/company/yc', () => {
    expect(pack.extends).toBe('inbrain-base');
    const borrowEntry = pack.borrow_from.find((b) => b.pack === 'inbrain-base');
...
describe('v0.41 T4: inbrain-engineer manifest shape', () => {
  const pack = loadPack('inbrain-engineer');

  test('extends inbrain-base + borrows code/project', () => {
    expect(pack.extends).toBe('inbrain-base');
    const borrowEntry = pack.borrow_from.find((b) => b.pack === 'inbrain-base');
...
describe('v0.41 T4: inbrain-everything meta-pack shape', () => {
  const pack = loadPack('inbrain-everything');

  test('extends inbrain-investor (chain head)', () => {
    expect(pack.extends).toBe('inbrain-investor');
  });

  test('borrows from inbrain-creator + inbrain-engineer', () => {
    const borrowedPacks = pack.borrow_from.map((b) => b.pack).sort();
    expect(borrowedPacks).toEqual(['inbrain-creator', 'inbrain-engineer']);
  });

  test('borrows atom from creator and learning from engineer', () => {
    const creatorBorrow = pack.borrow_from.find((b) => b.pack === 'inbrain-creator');
    expect(creatorBorrow?.types).toContain('atom');
    const engineerBorrow = pack.borrow_from.find((b) => b.pack === 'inbrain-engineer');
