 ...
 *
 * After the fix:
 *   - failures append to ~/.inbrain/sync-failures.jsonl (with dedup)
 *   - incremental + full-sync + import git-continuity paths gate the
 *     sync.last_commit advance on "no failures"
 *   - `inbrain sync --skip-failed` acknowledges the current set
 *   - `inbrain doctor` surfaces unacknowledged failures
 *
...
// Point HOME at a tmpdir so we don't stomp the real ~/.inbrain/sync-failures.jsonl
let tmpHome: string;
const originalHome = process.env.HOME;

beforeEach(async () => {
  tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-sync-failures-'));
...
describe('Bug 9 — doctor surfaces sync failures', () => {
  test('doctor source contains sync_failures check', async () => {
    ...
    expect(source).toContain("'inbrain sync --skip-failed'");
  });
...
describe('Bug 9 — sync.ts CLI flag wiring', () => {
  ...
  test('runSync acks pre-existing unacked failures up-front when --skip-failed is set', async () => {
    ...
    // runs `inbrain sync --skip-failed` to clear the log gets a no-op —
...
// recorded before the code field existed (~/.inbrain/sync-failures.jsonl
...
describe('acknowledgeSyncFailures — backfill on legacy entries', () => {
  test('backfills code on entries that predate the code field', async () => {
    ...
    // ~/.inbrain/sync-failures.jsonl row that exists on real upgrades.
