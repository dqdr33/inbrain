...
const ASSETS: ReleaseAsset[] = [
  { name: 'inbrain-darwin-arm64', url: 'https://example.com/darwin-arm64' },
  { name: 'inbrain-linux-x64', url: 'https://example.com/linux-x64' },
];

describe('expectedAssetName / resolvePlatformAsset', () => {
  test('maps the two published targets', () => {
    expect(expectedAssetName('darwin', 'arm64')).toBe('inbrain-darwin-arm64');
    expect(expectedAssetName('linux', 'x64')).toBe('inbrain-linux-x64');
  });
...
async function withTmp<T>(fn: (dir: string) => Promise<T>): Promise<T> {
  const dir = mkdtempSync(join(tmpdir(), 'inbrain-binup-'));
  try {
    return await fn(dir);
  } finally {
...
describe('runBinarySelfUpdate', () => {
  test('happy path: stages, smokes, atomically replaces the target', async () => {
    await withTmp(async (dir) => {
      ...
      expect(res.asset).toBe('inbrain-darwin-arm64');
  ...
  test('unsupported platform → unsupported_platform, target untouched', async () => {
    await withTmp(async (dir) => {
      const target = join(dir, 'inbrain.exe');
