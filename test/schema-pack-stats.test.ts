...
beforeEach(async () => {
  ...
  _resetPackLocatorForTests();
  tmpDir = mkdtempSync(join(tmpdir(), 'inbrain-stats-test-'));
});
...
function seedTinyPack(packName: string, types: Array<{ name: string; prefix: string }>): void {
  ...
  let body = `api_version: inbrain-schema-pack-v1\nname: ${packName}\nversion: 1.0.0\ndescription: ""\ninbrain_min_version: 0.38.0\nextends: null\nborrow_from: []\npage_types:\n`;
...
describe('runStatsCore — empty brain', () => {
  it('reports coverage:1.0 (vacuous truth)', async () => {
    await withEnv({ INBRAIN_SCHEMA_PACK: undefined }, async () => {
...
describe('runStatsCore — dead-prefix detection', () => {
  it('flags pack-declared prefixes with zero matching pages', async () => {
    await withEnv({ INBRAIN_HOME: tmpDir, INBRAIN_SCHEMA_PACK: 'tiny' }, async () => {
      seedTinyPack('tiny', [
  ...
  it('returns empty dead_prefixes when pack load fails', async () => {
    await withEnv({ INBRAIN_SCHEMA_PACK: 'never-installed' }, async () => {
