 ...
 * semantics (e.g. someone hardcoding 'inbrain-sync' elsewhere would no longer match
...
describe('syncLockId (per-source lock helper)', () => {
  test('default source returns inbrain-sync:default', () => {
    expect(syncLockId('default')).toBe('inbrain-sync:default');
  });

  test('non-default source returns inbrain-sync:<id>', () => {
    expect(syncLockId('zion-brain')).toBe('inbrain-sync:zion-brain');
    expect(syncLockId('media-corpus')).toBe('inbrain-sync:media-corpus');
  });

  test('IRON-RULE: SYNC_LOCK_ID back-compat alias resolves to syncLockId(default)', () => {
    expect(SYNC_LOCK_ID).toBe(syncLockId('default'));
    expect(SYNC_LOCK_ID).toBe('inbrain-sync:default');
  });
