...
// database connection' fired loudly after every inbrain capture. Per CQ1
// suppress via typed access (instanceof InbrainError && .problem) NOT
...
describe('writeFactsAbsorbLog — WARN-4 disconnected-engine suppression (CQ1 + CV13)', () => {
  ...
  // the same InbrainError shape src/core/db.ts:151-158 throws.
    ...
    return {
      engine: {
        logIngest: async () => {
          ...
          const { InbrainError } = await import('../src/core/types.ts');
  ...
  test('InbrainError problem="No database connection" SUPPRESSED after first occurrence', async () => {
  ...
  test('InbrainError with DIFFERENT problem still warns loudly (not the suppressed class)', async () => {
    const { _resetFactsAbsorbDisconnectedFlagForTests } = await import('../src/core/facts/absorb-log.ts');
    _resetFactsAbsorbDisconnectedFlagForTests();
    ...
    try {
      const { InbrainError } = await import('../src/core/types.ts');
      const otherError = new InbrainError(
  ...
  test('non-InbrainError (plain Error) still warns loudly', async () => {
    const { _resetFactsAbsorbDisconnectedFlagForTests } = await import('../src/core/facts/absorb-log.ts');
    _resetFactsAbsorbDisconnectedFlagForTests();
