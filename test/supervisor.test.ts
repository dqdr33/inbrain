...
const TEST_PID_FILE = '/tmp/inbrain-supervisor-test.pid';
...
function makeHarness(name: string, workerBody: string): IntegrationHarness {
  const tmpRoot = join(tmpdir(), `inbrain-sup-test-${name}-${process.pid}-${Date.now()}`);
...
function readAudit(auditDir: string) {
  const origEnv = process.env.INBRAIN_AUDIT_DIR;
  process.env.INBRAIN_AUDIT_DIR = auditDir;
  try {
    return readSupervisorEvents();
  } finally {
    if (origEnv === undefined) delete process.env.INBRAIN_AUDIT_DIR;
    else process.env.INBRAIN_AUDIT_DIR = origEnv;
  }
}
...
describe('MinionSupervisor', () => {
  ...
  describe('integration: env-var inheritance regression (codex #9 / eng #8)', () => {
    it('strips inherited INBRAIN_ALLOW_SHELL_JOBS when allowShellJobs=false, even if parent has it set', async () => {
      const outFile = join(tmpdir(), `inbrain-sup-env-${process.pid}-${Date.now()}.txt`);
      ...
      const h = makeHarness('env-strip-outfile', `printf '%s\\n' "\${INBRAIN_ALLOW_SHELL_JOBS-UNSET}" > "$OUT_FILE" ; exit 1`);

      try {
        const sup = spawnSupervisor(h, {
          OUT_FILE: outFile,
          INBRAIN_ALLOW_SHELL_JOBS: '1',  // parent has it
    ...
    it('DOES pass INBRAIN_ALLOW_SHELL_JOBS to child when allowShellJobs is true', async () => {
      const outFile = join(tmpdir(), `inbrain-sup-env-ok-${process.pid}-${Date.now()}.txt`);
      try { unlinkSync(outFile); } catch { /* may not exist */ }
  ...
  describe('integration: INBRAIN_SUPERVISED env var (v0.22.14)', () => {
    it('sets INBRAIN_SUPERVISED=1 on spawned worker child', async () => {
      const outFile = join(tmpdir(), `inbrain-sup-supervised-${process.pid}-${Date.now()}.txt`);
      try { unlinkSync(outFile); } catch { /* may not exist */ }

      // exit 1 required post-D1/D2 to trip SUP_MAX_CRASHES=1; clean exits
      // no longer count toward the crash limit.
      const h = makeHarness('supervised-env', `printf '%s\n' "\${INBRAIN_SUPERVISED-UNSET}" > "$OUT_FILE" ; exit 1`);
  ...
  describe('integration: --max-rss spawn args (v0.21, auto-sized v0.41.39.0)', () => {
    it('passes an explicit --max-rss through to the spawned worker', async () => {
      const outFile = join(tmpdir(), `inbrain-sup-maxrss-${process.pid}-${Date.now()}.txt`);
      try { unlinkSync(outFile); } catch { /* may not exist */ }
    ...
    it('auto-sizes --max-rss when no explicit cap is given', async () => {
      const outFile = join(tmpdir(), `inbrain-sup-maxrss-auto-${process.pid}-${Date.now()}.txt`);
      try { unlinkSync(outFile); } catch { /* may not exist */ }
