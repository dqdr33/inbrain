...
// trust gate (remote=true rejects per-call opt), inbrain-base loads from
...
describe('loadActivePack boundary helper', () => {
  beforeAll(() => {
    // Tests use the real bundled inbrain-base for tier-7 default; per-test
  ...
  test('default resolution loads inbrain-base from bundled path', async () => {
    _resetPackCacheForTests();
    const pack = await loadActivePack({ cfg: null, remote: false });
    expect(pack.manifest.name).toBe('inbrain-base');
  ...
  test('tier-2 env var INBRAIN_SCHEMA_PACK wins over tier-6 home config', async () => {
    await withEnv({ INBRAIN_SCHEMA_PACK: 'env-pack' }, async () => {
      const result = resolveActivePackNameOnly({
  ...
  test('tier-7 default falls back to inbrain-base when nothing set', () => {
    const result = resolveActivePackNameOnly({ cfg: null, remote: false });
    expect(result.pack_name).toBe('inbrain-base');
  ...
  test('injected locator overrides default disk path', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'inbrain-test-pack-'));
    const yamlPath = join(dir, 'pack.yaml');
    writeFileSync(yamlPath, `api_version: inbrain-schema-pack-v1
name: injected-pack
version: 0.1.0
description: test
extends: null
