...
describe('resolvePrepare', () => {
  afterEach(() => {
    delete process.env.INBRAIN_PREPARE;
  });
  ...
  test('INBRAIN_PREPARE=false overrides everything', () => {
    process.env.INBRAIN_PREPARE = 'false';
    expect(
      resolvePrepare('postgresql://user:pass@host:5432/db?prepare=true'),
    ).toBe(false);
  });

  test('INBRAIN_PREPARE=true overrides auto-detect on 6543', () => {
    process.env.INBRAIN_PREPARE = 'true';
    expect(resolvePrepare('postgresql://user:pass@host:6543/db')).toBe(true);
  });

  test('INBRAIN_PREPARE=0 is falsy', () => {
    process.env.INBRAIN_PREPARE = '0';
    expect(resolvePrepare('postgresql://user:pass@host:6543/db')).toBe(false);
  });
