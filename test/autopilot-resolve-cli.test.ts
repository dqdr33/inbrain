 ...
 * caused the bug. The canonical resolution is the `inbrain` shim on PATH.
 */
...
describe('resolveGbrainCliPath', () => {
  test('returns a non-empty string or throws with a clear install hint', () => {
    ...
    try {
      ...
      // Machine without inbrain on PATH and no compiled binary: throw is
  ...
  test('shim on PATH wins over argv[1]=cli.ts', () => {
    // If `which inbrain` resolves (most dev machines), the resolver should
    ...
    try {
      const path = resolveGbrainCliPath();
      // On a machine where `which inbrain` resolves, path ends in /inbrain.
  ...
  test('accepts argv[1]=/inbrain when shim is absent (compiled binary)', () => {
    // If the machine has neither shim nor compiled exec, but argv[1]
    // happens to be a literal /inbrain path (direct invocation), accept it.
    const origArg1 = process.argv[1];
    process.argv[1] = '/usr/local/bin/inbrain';
    try {
      const path = resolveGbrainCliPath();
      // On a machine with `which inbrain`, we get the shim. On a machine
      // without, argv[1] fallback fires. Either way the result is valid.
      expect(path.endsWith('/inbrain') || path.endsWith('\\inbrain.exe')).toBe(true);
    } finally {
      process.argv[1] = origArg1;
    }
  });
