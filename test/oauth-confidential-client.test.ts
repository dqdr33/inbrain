 ...
 * presented_secret` plaintext compare. inbrain stores SHA-256 hashes,
...
import { InbrainOAuthProvider } from '../src/core/oauth-provider.ts';
...
beforeAll(async () => {
  ...
  provider = new InbrainOAuthProvider({ sql: sqlQueryForEngine(engine) });
});
