 ...
 * subprocesses real `inbrain doctor` against a synthesized 60K-file brain.
 *
...
describe('doctor frontmatter_integrity — structural rendering (source-grep)', () => {
  test('source contains INBRAIN_DOCTOR_FM_TIMEOUT_MS handling', () => {
    expect(DOCTOR_SOURCE).toContain('INBRAIN_DOCTOR_FM_TIMEOUT_MS');
  });
  ...
  test('source renders NOT SCANNED per skipped source with remediation hint', () => {
    ...
    expect(DOCTOR_SOURCE).toContain('inbrain frontmatter validate');
  });
...
describe('doctor frontmatter_integrity — load-bearing render strings', () => {
  ...
  test('source uses fallback hint pointing at INBRAIN_DOCTOR_FM_TIMEOUT_MS on partial', () => {
    // The fix hint when partial: raise the timeout OR run validate directly.
    const partialHintMatch = DOCTOR_SOURCE.includes('Raise INBRAIN_DOCTOR_FM_TIMEOUT_MS');
