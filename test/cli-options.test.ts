...
describe('cli.ts global-flag stripping (integration)', () => {
  ...
  test('inbrain --progress-json --version works (global flag stripped before dispatch)', () => {
    const res = spawnSync('bun', [CLI, '--progress-json', '--version'], {
      encoding: 'utf-8',
      env: { ...process.env, NO_COLOR: '1' },
    });
    expect(res.status).toBe(0);
    expect(res.stdout).toContain('inbrain ');
  });

  test('inbrain --quiet --progress-interval=500 version works (flags interleaved, all stripped)', () => {
...
describe('CLI integration: progress streams to the right channel', () => {
  ...
  test('inbrain --progress-json --version emits only the version on stdout', () => {
    ...
    expect(res.status).toBe(0);
    expect(res.stdout.trim()).toMatch(/^inbrain /);
    // No JSON progress object should end up on stdout.
    expect(res.stdout).not.toContain('"event":"start"');
  });

  test('inbrain --quiet skillpack-check returns exit code with no stdout', () => {
