...
beforeEach(() => {
  ...
  delete process.env.INBRAIN_BRAIN_ID;
});
...
describe('resolveBrainId — priority order', () => {
  test('explicit flag beats everything else', () => {
    process.env.INBRAIN_BRAIN_ID = 'from-env';
    const dir = mktmp();
    writeFileSync(join(dir, '.inbrain-mount'), 'from-dotfile\n');
...
describe('resolveBrainId — dotfile behavior', () => {
  test('walks up to find .inbrain-mount in ancestor', () => {
    const parent = mktmp();
    const nested = join(parent, 'deep/nested/dir');
    mkdirSync(nested, { recursive: true });
    writeFileSync(join(parent, '.inbrain-mount'), 'luther\n');
  ...
  test('skips malformed dotfile, keeps walking', () => {
    ...
    writeFileSync(join(parent, '.inbrain-mount'), '!!!invalid!!!\n');
    writeFileSync(join(grandparent, '.inbrain-mount'), 'valid-id\n');
    expect(resolveBrainId(null, child, noMounts)).toBe('valid-id');
  });

  test('accepts "host" in dotfile (explicit opt-out of mount routing)', () => {
    const dir = mktmp();
    writeFileSync(join(dir, '.inbrain-mount'), `${HOST_BRAIN_ID}\n`);
  ...
  test('trims whitespace + only uses first line', () => {
    const dir = mktmp();
    writeFileSync(join(dir, '.inbrain-mount'), '  yc-media  \n# comment line\n');
...
describe('resolveBrainId — validation', () => {
  ...
  test('invalid INBRAIN_BRAIN_ID value throws', () => {
    process.env.INBRAIN_BRAIN_ID = 'UPPER';
