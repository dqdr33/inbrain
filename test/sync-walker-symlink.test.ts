 ...
 *
 * Closes the bug class where `inbrain sync --strategy code` could hang on
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'inbrain-walker-'));
});
...
describe('collectSyncableFiles symlink + cycle hardening', () => {
  test('1. self-referencing symlink does not loop', async () => {
    await withEnv({ INBRAIN_EMBEDDING_MULTIMODAL: undefined }, () => {
  ...
  test('6. multimodal preservation under markdown strategy (codex C5)', async () => {
    ...
    await withEnv({ INBRAIN_EMBEDDING_MULTIMODAL: 'true' }, () => {
