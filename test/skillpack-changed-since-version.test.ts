 ...
 * the git-aware filter that backs `inbrain skillpack reference --all --since
 * <version>`. Builds a fixture git repo on the fly and tags it to exercise
 * the version-resolution + commit-walking path.
 *
 * Pins:
 *   - returns null when inbrainRoot is not a git checkout
...
describe('changedSlugsSinceVersion', () => {
  it('returns null when inbrainRoot is not a git checkout', () => {
