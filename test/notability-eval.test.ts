 ...
 * default. Operators with a private fixture (`~/.inbrain/eval/
 * notability-real.jsonl`, mined + hand-confirmed via `inbrain notability-
 * eval`) can opt into the larger run by setting INBRAIN_NOTABILITY_EVAL_REAL=1.
 *
...
describe('JSONL utilities', () => {
  ...
  test('default paths resolve under ~/.inbrain/eval/', () => {
    expect(defaultMiningOutPath()).toContain('.inbrain/eval/notability-mining-candidates.jsonl');
    expect(defaultReviewOutPath()).toContain('.inbrain/eval/notability-real.jsonl');
  });
});
