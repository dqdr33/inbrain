...
describe('resolveLockRenewalKnobs', () => {
  ...
  test('case 12b — valid env values parse cleanly', () => {
    const knobs = resolveLockRenewalKnobs({
      INBRAIN_LOCK_RENEWAL_MAX_FAILURES: '5',
      INBRAIN_LOCK_RENEWAL_CALL_TIMEOUT_MS: '15000',
      INBRAIN_LOCK_RENEWAL_SAFETY_MARGIN_MS: '8000',
    }, 30_000);
  ...
  test('case 12c — bad env (abc/-5/0/1.5) falls back to default with single stderr warn', async () => {
    ...
    try {
      ...
      let knobs = resolveLockRenewalKnobs({ INBRAIN_LOCK_RENEWAL_MAX_FAILURES: 'abc' }, 30_000);
      expect(knobs.maxFailuresForAudit).toBe(3);

      knobs = resolveLockRenewalKnobs({ INBRAIN_LOCK_RENEWAL_MAX_FAILURES: '-5' }, 30_000);
      expect(knobs.maxFailuresForAudit).toBe(3);

      knobs = resolveLockRenewalKnobs({ INBRAIN_LOCK_RENEWAL_MAX_FAILURES: '0' }, 30_000);
      expect(knobs.maxFailuresForAudit).toBe(3);

      knobs = resolveLockRenewalKnobs({ INBRAIN_LOCK_RENEWAL_MAX_FAILURES: '1.5' }, 30_000);
      expect(knobs.maxFailuresForAudit).toBe(3);

      // Despite 4 bad invocations, only ONE stderr warn per env-name fired.
      const warnLines = captured.filter((c) => c.includes('INBRAIN_LOCK_RENEWAL_MAX_FAILURES'));
  ...
  test('case 12d — different env names warn independently', async () => {
    ...
    try {
      _resetKnobWarningsForTests();
      resolveLockRenewalKnobs({
        INBRAIN_LOCK_RENEWAL_MAX_FAILURES: 'bad1',
        INBRAIN_LOCK_RENEWAL_CALL_TIMEOUT_MS: 'bad2',
        INBRAIN_LOCK_RENEWAL_SAFETY_MARGIN_MS: 'bad3',
      }, 30_000);
