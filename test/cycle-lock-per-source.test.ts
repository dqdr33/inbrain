 ...
 *   - back-compat for undefined sourceId (legacy 'inbrain-cycle')
 *   - per-source lock IDs (`inbrain-cycle:<id>`)
...
describe('cycleLockIdFor', () => {
  test('returns legacy inbrain-cycle for undefined sourceId (back-compat)', () => {
    expect(cycleLockIdFor()).toBe('inbrain-cycle');
    expect(cycleLockIdFor(undefined)).toBe('inbrain-cycle');
  });

  test('returns inbrain-cycle:<source_id> for valid kebab IDs', () => {
    expect(cycleLockIdFor('default')).toBe('inbrain-cycle:default');
    expect(cycleLockIdFor('portfolio')).toBe('inbrain-cycle:portfolio');
    expect(cycleLockIdFor('a')).toBe('inbrain-cycle:a');
    expect(cycleLockIdFor('alpha-beta-gamma')).toBe('inbrain-cycle:alpha-beta-gamma');
  });
  ...
  test('produces DISTINCT lock IDs for different sources', () => {
    ...
    expect(a).not.toBe(b);
    expect(a).not.toBe('inbrain-cycle');
    expect(b).not.toBe('inbrain-cycle');
  });
