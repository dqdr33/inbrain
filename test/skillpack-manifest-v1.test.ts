...
describe('validateSkillpackManifest — required fields', () => {
  ...
  test('rejects unknown api_version', () => {
    const bad = { ...VALID_MANIFEST, api_version: 'inbrain-skillpack-v99' as 'inbrain-skillpack-v1' };
    try {
      validateSkillpackManifest(bad);
      throw new Error('should have thrown');
    } catch (err) {
...
describe('validateSkillpackManifest — field shape rules', () => {
  ...
  test('rejects inbrain_min_version that is not semver', () => {
    try {
      validateSkillpackManifest({ ...VALID_MANIFEST, inbrain_min_version: 'latest' });
      throw new Error('should have thrown');
    } catch (err) {
      expect((err as SkillpackManifestError).detail?.field).toBe('inbrain_min_version');
    }
  });
