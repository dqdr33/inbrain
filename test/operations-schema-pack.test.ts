...
beforeEach(async () => {
  ...
  _resetPackLocatorForTests();
  tmpDir = mkdtempSync(join(tmpdir(), 'inbrain-ops-schema-test-'));
  auditDir = mkdtempSync(join(tmpdir(), 'inbrain-ops-schema-audit-'));
});
...
function seedPack(packName: string): string {
  ...
  writeFileSync(path, `api_version: inbrain-schema-pack-v1
name: ${packName}
version: 1.0.0
description: ""
...
describe('get_active_schema_pack', () => {
  it('returns identity packet for the bundled inbrain-base pack', async () => {
    await withEnv({ INBRAIN_HOME: tmpDir, INBRAIN_SCHEMA_PACK: undefined }, async () => {
