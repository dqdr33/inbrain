...
describe('resolveWorkerConcurrency (v0.19.1 H3): clamp + validation', () => {
  ...
  test('flag-unset env=8 → 8', () => {
    expect(resolveWorkerConcurrency([], { INBRAIN_WORKER_CONCURRENCY: '8' } as NodeJS.ProcessEnv)).toBe(8);
  });
  test('flag=2 env=8 → 2 (flag wins)', () => {
    expect(resolveWorkerConcurrency(['--concurrency', '2'], { INBRAIN_WORKER_CONCURRENCY: '8' } as NodeJS.ProcessEnv)).toBe(2);
  });
  test('both unset → 1', () => {
    expect(resolveWorkerConcurrency([], {} as NodeJS.ProcessEnv)).toBe(1);
  });
  test('garbage env "foo" → clamped to 1 (H3)', () => {
    expect(resolveWorkerConcurrency([], { INBRAIN_WORKER_CONCURRENCY: 'foo' } as NodeJS.ProcessEnv)).toBe(1);
  });
  test('env=0 → clamped to 1 (H3 — prevents silent wedge)', () => {
    expect(resolveWorkerConcurrency([], { INBRAIN_WORKER_CONCURRENCY: '0' } as NodeJS.ProcessEnv)).toBe(1);
  });
  test('env=-5 → clamped to 1 (H3)', () => {
    expect(resolveWorkerConcurrency([], { INBRAIN_WORKER_CONCURRENCY: '-5' } as NodeJS.ProcessEnv)).toBe(1);
  });
});
...
describe('backpressure-audit (v0.19.1 Q1): JSONL on coalesce', () => {
  test('logBackpressureCoalesce writes one JSONL line per coalesce', async () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'inbrain-audit-'));
    const prev = process.env.INBRAIN_AUDIT_DIR;
    process.env.INBRAIN_AUDIT_DIR = tmp;
    try {
      ...
      if (prev === undefined) delete process.env.INBRAIN_AUDIT_DIR;
      else process.env.INBRAIN_AUDIT_DIR = prev;
...
describe('MinionWorker: self-health-check', () => {
  test('health check is active when INBRAIN_SUPERVISED is not set', async () => {
    // Save and clear the env var
    const saved = process.env.INBRAIN_SUPERVISED;
    delete process.env.INBRAIN_SUPERVISED;
    ...
    try {
      ...
      if (saved !== undefined) process.env.INBRAIN_SUPERVISED = saved;
      else delete process.env.INBRAIN_SUPERVISED;
    }
  }, 10_000);

  test('health check is skipped when INBRAIN_SUPERVISED=1', async () => {
    const saved = process.env.INBRAIN_SUPERVISED;
    process.env.INBRAIN_SUPERVISED = '1';
