 ...
 * `~/.inbrain`. `withEnv` covers the INBRAIN_AUDIT_DIR escape hatch.
 */
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'inbrain-budget-test-'));
...
describe('BudgetTracker.reserve', () => {
  ...
  test('v0.41.20.0: slash-prefix anthropic/claude-* under --max-cost does NOT no_pricing throw (THE FIX)', () => {
    ...
    // running `inbrain brainstorm --judge-model anthropic/claude-sonnet-4-6
