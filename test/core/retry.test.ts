...
// Minimal InbrainError shape mirrors the typed problem/detail fields from
// db.ts:getConnection so the retry-matcher extension recognizes it.
class FakeInbrainError extends Error {
  problem: string;
  detail: string;
...
describe('isRetryableConnError extension (v0.41.2.1, re-exported from retry.ts)', () => {
  test('InbrainError with problem="No database connection" is retryable', () => {
    const err = new FakeInbrainError('No database connection', 'connect() has not been called');
    expect(isRetryableConnError(err)).toBe(true);
  });

  test('InbrainError with other problem is NOT retryable', () => {
    const err = new FakeInbrainError('Schema mismatch', 'expected vector(1536), got vector(1024)');
...
describe('withRetry primitive — v0.41.2.1 back-compat contract', () => {
  ...
  test('retries on InbrainError "No database connection"; second succeeds', async () => {
    let calls = 0;
    const result = await withRetry(
      async () => {
        calls++;
        if (calls === 1) throw new FakeInbrainError('No database connection', 'connect() has not been called');
        return 'ok';
      },
      { delayMs: 0 },
    );
...
describe('resolveBulkRetryOpts env-override (D3 cherry-pick, codex M-10/M-12)', () => {
  ...
  test('all 3 vars set: overrides apply', () => {
    const out = resolveBulkRetryOpts({
      INBRAIN_BULK_MAX_RETRIES: '5',
      INBRAIN_BULK_RETRY_BASE_MS: '2000',
      INBRAIN_BULK_RETRY_MAX_MS: '15000',
    });
  ...
  test('INBRAIN_BULK_MAX_RETRIES=0: accepted (debug-mode disable)', () => {
    const out = resolveBulkRetryOpts({ INBRAIN_BULK_MAX_RETRIES: '0' });
    expect(out.maxRetries).toBe(0);
  });

  test('INBRAIN_BULK_MAX_RETRIES=negative: throws with paste-ready hint', () => {
    expect(() => resolveBulkRetryOpts({ INBRAIN_BULK_MAX_RETRIES: '-1' }))
      .toThrow(/INBRAIN_BULK_MAX_RETRIES.*>= 0.*Fix: export/);
  });

  test('INBRAIN_BULK_MAX_RETRIES=non-int: throws', () => {
    expect(() => resolveBulkRetryOpts({ INBRAIN_BULK_MAX_RETRIES: '3.5' }))
      .toThrow(/INBRAIN_BULK_MAX_RETRIES/);
    expect(() => resolveBulkRetryOpts({ INBRAIN_BULK_MAX_RETRIES: 'foo' }))
      .toThrow(/INBRAIN_BULK_MAX_RETRIES/);
  });

  test('INBRAIN_BULK_RETRY_BASE_MS=0 or negative: throws (delays must be > 0)', () => {
    expect(() => resolveBulkRetryOpts({ INBRAIN_BULK_RETRY_BASE_MS: '0' }))
      .toThrow(/> 0/);
    expect(() => resolveBulkRetryOpts({ INBRAIN_BULK_RETRY_BASE_MS: '-100' }))
      .toThrow(/> 0/);
  });

  test('INBRAIN_BULK_RETRY_MAX_MS < base: throws', () => {
    expect(() => resolveBulkRetryOpts({
      INBRAIN_BULK_RETRY_BASE_MS: '5000',
      INBRAIN_BULK_RETRY_MAX_MS: '3000',
    })).toThrow(/>= INBRAIN_BULK_RETRY_BASE_MS=5000/);
  });

  test('empty string env values treated as unset', () => {
    const out = resolveBulkRetryOpts({ INBRAIN_BULK_MAX_RETRIES: '' });
