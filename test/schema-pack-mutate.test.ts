...
function seedPack(packName: string, format: 'json' | 'yaml', initial?: Partial<SchemaPackManifest>): string {
  // INBRAIN_HOME=/tmp/x → inbrainPath('schema-packs', 'mine') = /tmp/x/.inbrain/schema-packs/mine
  const dir = join(tmpDir, '.inbrain', 'schema-packs', packName);
...
beforeEach(() => {
  ...
  tmpDir = mkdtempSync(join(tmpdir(), 'inbrain-mutate-test-'));
  auditDir = mkdtempSync(join(tmpdir(), 'inbrain-mutate-audit-'));
  lockDir = mkdtempSync(join(tmpdir(), 'inbrain-mutate-locks-'));
});
...
describe('locateMutablePackFile — bundled guard', () => {
  it('rejects inbrain-base with PACK_READONLY + fork hint', () => {
    expect(() => locateMutablePackFile('inbrain-base')).toThrow(SchemaPackMutationError);
    try { locateMutablePackFile('inbrain-base'); } catch (e) {
      const err = e as SchemaPackMutationError;
      expect(err.code).toBe('PACK_READONLY');
      expect(err.message).toContain('inbrain schema fork');
    }
  });

  it('rejects inbrain-recommended with PACK_READONLY', () => {
    try { locateMutablePackFile('inbrain-recommended'); } catch (e) {
      expect((e as SchemaPackMutationError).code).toBe('PACK_READONLY');
    }
  });

  it('BUNDLED_PACK_NAMES export contains all bundled packs', () => {
    expect(BUNDLED_PACK_NAMES.has('inbrain-base')).toBe(true);
    expect(BUNDLED_PACK_NAMES.has('inbrain-recommended')).toBe(true);
    // v0.42 (T22): inbrain-base-v2 joins the bundled set.
    expect(BUNDLED_PACK_NAMES.has('inbrain-base-v2')).toBe(true);
    expect(BUNDLED_PACK_NAMES.size).toBe(3);
  });

  it('rejects inbrain-base-v2 with PACK_READONLY (bundled guard)', () => {
    try { locateMutablePackFile('inbrain-base-v2'); } catch (e) {
      expect((e as SchemaPackMutationError).code).toBe('PACK_READONLY');
    }
  });
});
...
describe('addTypeToPack', () => {
  it('appends a new type to JSON pack and writes atomically', async () => {
    await withEnv({ INBRAIN_HOME: tmpDir, INBRAIN_AUDIT_DIR: auditDir }, async () => {
...
describe('YAML round-trip', () => {
  it('mutating a YAML pack preserves YAML format and reparses cleanly', async () => {
    await withEnv({ INBRAIN_HOME: tmpDir, INBRAIN_AUDIT_DIR: auditDir }, async () => {
      const dir = join(tmpDir, '.inbrain', 'schema-packs', 'yaml-pack');
      mkdirSync(dir, { recursive: true });
      ...
      const yamlBody = `api_version: inbrain-schema-pack-v1
name: yaml-pack
version: 1.0.0
description: ""
inbrain_min_version: 0.38.0
