...
describe('LocalStorage', () => {
  ...
  beforeAll(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'inbrain-storage-test-'));
...
describe('LocalStorage path traversal', () => {
  test('blocks upload path traversal via ../', async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'inbrain-traversal-'));
    try {
...
describe('createStorage', () => {
  test('creates LocalStorage for backend: local', async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'inbrain-factory-test-'));
    try {
