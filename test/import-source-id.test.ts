/**
 * v0.37.7.0 #1167 — `inbrain import --source-id <id>` routes to a brain source.
 *
 * Pre-fix, `inbrain import --source dept-x ./pages` silently fell back to
...
describe('import --source-id (#1167)', () => {
  ...
  beforeEach(async () => {
    ...
    scratchDir = mkdtempSync(join(tmpdir(), 'inbrain-import-src-'));
  ...
  test('--source-id value is NOT treated as a positional dir arg', async () => {
    ...
    // Should NOT throw "Usage: inbrain import <dir>..." because scratchDir
