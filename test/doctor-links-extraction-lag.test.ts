...
describe('links_extraction_lag doctor check', () => {
  ...
  test('>100 pages, all un-extracted → warn (>20%)', async () => {
    ...
    expect(c.message).toContain('inbrain extract --stale');
  ...
  test('INBRAIN_EXTRACTION_LAG_FAIL_PCT opts into hard fail', async () => {
    await seedPages(120);
    await withEnv({ INBRAIN_EXTRACTION_LAG_FAIL_PCT: '50' }, async () => {
      const c = await checkLinksExtractionLag(engine);
  ...
  test('INBRAIN_EXTRACTION_LAG_WARN_PCT raises the warn bar', async () => {
    ...
    await withEnv({ INBRAIN_EXTRACTION_LAG_WARN_PCT: '5' }, async () => {
