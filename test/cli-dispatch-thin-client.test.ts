 ...
 *
 * When `~/.inbrain/config.json` has `remote_mcp` set, 9 commands are refused
 ...
 *
 * Strategy: seed `~/.inbrain/config.json` with remote_mcp set in a tempdir
 * `INBRAIN_HOME`, then spawn `inbrain <cmd>` and assert (a) exit code 1,
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'inbrain-cli-dispatch-'));
});
...
describe('thin-client dispatch guard refuses DB-bound commands', () => {
  ...
  for (const args of refusedCommands) {
    test(`refuses \`inbrain ${args.join(' ')}\` with pinpoint hint`, async () => {
      ...
      // host" message. Hint format: "`inbrain <cmd>` is not routable. <hint>"
      expect(r.stderr).toContain(`inbrain ${args[0]}`);
...
describe('thin-client dispatch guard does NOT refuse safe commands', () => {
  ...
  // does NOT appear.
  test('`inbrain --version` works on thin-client install', async () => {
    seedThinClientConfig();
    const r = await run(['--version']);
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain('inbrain');
    expect(r.stderr).not.toContain('thin client');
  });

  test('`inbrain --help` works on thin-client install', async () => {
    seedThinClientConfig();
...
describe('thin-client doctor routes to runRemoteDoctor', () => {
  test('`inbrain doctor` runs remote checks (not DB-bound checks) when remote_mcp is set', async () => {
    seedThinClientConfig();
...
describe('regression — local config still passes through normally', () => {
  test('local PGLite config does NOT trigger thin-client guard for `sync`', async () => {
    // Seed a local PGLite config (no remote_mcp). `inbrain sync` shouldn't
