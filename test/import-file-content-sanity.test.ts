...
/** Wrap an importFromContent call with INBRAIN_HOME + INBRAIN_AUDIT_DIR
 *  pointed at fresh tempdirs so config and audit writes don't leak
 *  between tests or pollute the developer's real ~/.inbrain. */
async function withIsolatedHome<T>(fn: () => Promise<T>): Promise<T> {
  inbrainHomeDir = mkdtempSync(join(tmpdir(), 'cs-gate-home-'));
  auditDir = mkdtempSync(join(tmpdir(), 'cs-gate-audit-'));
  try {
    return await withEnv({
      INBRAIN_HOME: inbrainHomeDir,
      INBRAIN_AUDIT_DIR: auditDir,
    }, fn);
  } finally {
    rmSync(inbrainHomeDir, { recursive: true, force: true });
...
describe('importFromContent — kill-switch bypass', () => {
  test('INBRAIN_NO_SANITY=1 lets junk through with bypass audit + stderr', async () => {
    const inbrainHomeDirLocal = mkdtempSync(join(tmpdir(), 'cs-bypass-home-'));
    const auditDirLocal = mkdtempSync(join(tmpdir(), 'cs-bypass-audit-'));
    try {
      await withEnv({
        INBRAIN_HOME: inbrainHomeDirLocal,
        INBRAIN_AUDIT_DIR: auditDirLocal,
        INBRAIN_NO_SANITY: '1',
      }, async () => {
        const content = `---
title: 'Attention Required! | Cloudflare'
...
junk body`;
      ...
      rmSync(inbrainHomeDirLocal, { recursive: true, force: true });
