...
// These tests exercise the filesystem walker that feeds `inbrain import`.
...
describe('collectMarkdownFiles — symlink containment', () => {
  ...
  beforeEach(() => {
    ...
    root = mkdtempSync(join(tmpdir(), 'inbrain-walker-root-'));
    secretDir = mkdtempSync(join(tmpdir(), 'inbrain-walker-secret-'));
  });
