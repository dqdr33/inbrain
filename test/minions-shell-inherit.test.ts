...
describe('deriveEnvKey', () => {
  test('database_url → INBRAIN_DATABASE_URL (override, less ambiguous)', () => {
    expect(deriveEnvKey('database_url')).toBe('INBRAIN_DATABASE_URL');
  });
...
describe('resolveInheritValue', () => {
  test('returns string when field exists and is a non-empty string', () => {
    const cfg: InbrainConfig = { engine: 'postgres', database_url: 'postgresql://x' };
  ...
  test('returns undefined when field is empty string', () => {
    const cfg: InbrainConfig = { engine: 'postgres', database_url: '' };
  ...
  test('returns undefined when field is non-string (e.g. object)', () => {
    const cfg = { engine: 'postgres', remote_mcp: { issuer_url: 'x' } } as unknown as InbrainConfig;
...
describe('integration: deriveEnvKey + resolveInheritValue work together', () => {
  ...
  test.each([
    ['database_url', 'INBRAIN_DATABASE_URL', 'postgresql://x'],
