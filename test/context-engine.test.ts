/**
 * Tests for the inbrain-context OpenClaw context engine.
 *
 * Validates:
...
import { createInbrainContextEngine, ENGINE_ID, ENGINE_NAME, __resetSdkLoadStateForTests } from '../src/core/context-engine.ts';
...
function makeWorkspace(opts: WorkspaceOpts = {}) {
  const dir = mkdtempSync(join(tmpdir(), 'inbrain-ce-test-'));
...
describe('inbrain-context engine', () => {
  let tmpDir: string;
  ...
  it('has correct engine info', () => {
    ...
    const engine = createInbrainContextEngine({ workspaceDir: tmpDir });
  ...
  it('handles missing workspace files gracefully', async () => {
    tmpDir = mkdtempSync(join(tmpdir(), 'inbrain-ce-test-'));
