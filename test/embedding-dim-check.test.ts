 ...
 *
 * Pairs with `inbrain init` and `inbrain doctor`'s loud-failure paths. Validates
 * that:
...
describe('embeddingMismatchMessage', () => {
  ...
  test('PGLite branch uses wipe-and-reinit, not ALTER COLUMN', () => {
    ...
    expect(msg).toContain('inbrain init --pglite --embedding-model zeroentropyai:zembed-1 --embedding-dimensions 1280');
  ...
  test('PGLite branch falls back to default database path when omitted', () => {
    ...
    // Default falls back to inbrainPath('brain.pglite').
    expect(msg).toMatch(/mv .+brain\.pglite .+brain\.pglite\.bak/);
  });

  test('PGLite branch must NOT recommend `inbrain config set embedding_model` (no-op after Lane C.2)', () => {
    const msg = embeddingMismatchMessage({
      currentDims: 1536,
      requestedDims: 1280,
      requestedModel: 'zeroentropyai:zembed-1',
    ...
    // The pre-v0.37 recipe pointed at `inbrain config set embedding_model X`
    // which is a no-op after C.2. Recipe must point at init instead.
    expect(msg).not.toContain('inbrain config set embedding_model');
    expect(msg).not.toContain('inbrain config set embedding_dimensions');
  });
});
