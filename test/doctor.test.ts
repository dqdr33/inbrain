...
describe('doctor command', () => {
  ...
  test('frontmatter_integrity subcheck added in v0.22.4', async () => {
    ...
    expect(src).toContain('inbrain frontmatter validate');
  });
  ...
  // chose --fast with INBRAIN_DATABASE_URL / config-file URL present".
  test('getDbUrlSource reflects INBRAIN_DATABASE_URL env var', async () => {
    const { getDbUrlSource } = await import('../src/core/config.ts');
    const orig = process.env.INBRAIN_DATABASE_URL;
    const origAlt = process.env.DATABASE_URL;
    try {
      process.env.INBRAIN_DATABASE_URL = 'postgresql://test@localhost/x';
      expect(getDbUrlSource()).toBe('env:INBRAIN_DATABASE_URL');
      delete process.env.INBRAIN_DATABASE_URL;
      process.env.DATABASE_URL = 'postgresql://test@localhost/x';
      expect(getDbUrlSource()).toBe('env:DATABASE_URL');
    } finally {
      if (orig === undefined) delete process.env.INBRAIN_DATABASE_URL;
      else process.env.INBRAIN_DATABASE_URL = orig;
  ...
  test('doctor --fast emits source-specific message when URL present', async () => {
    ...
    expect(source).toContain('INBRAIN_DATABASE_URL');
  });

  // v0.12.2 reliability wave — doctor detects JSONB double-encode + truncated
  // bodies and points users at the standalone `inbrain repair-jsonb` command.
  ...
  test('doctor source contains jsonb_integrity and markdown_body_completeness checks', async () => {
    ...
    expect(source).toContain('inbrain repair-jsonb');
  });
  ...
  test('RLS check raises status=fail with quoted-identifier remediation SQL', async () => {
    ...
    // Severity upgraded from 'warn' to 'fail' so `inbrain doctor` exits 1 on gaps.
  ...
  test('RLS check reads pg_description and recognizes the INBRAIN:RLS_EXEMPT escape hatch', async () => {
    const source = await Bun.file(new URL('../src/commands/doctor.ts', import.meta.url)).text();
    const rlsBlock = source.slice(
      source.indexOf('// 5. RLS'),
      source.indexOf('// 6. Schema version'),
    ...
    expect(rlsBlock).toContain('INBRAIN:RLS_EXEMPT');
  ...
  // The graph_coverage WARN message used to suggest stale verbs (`inbrain
  // link-extract` / `inbrain timeline-extract`) that were removed in v0.16
  // when extraction was consolidated into `inbrain extract <links|timeline|all>`.
  // PR #376 (FUSED-ID) flagged the stale hint; PR #536 (mayazbay) replaced it
  // with the canonical `inbrain extract all`. Pin the user-facing copy so a
  // future edit can't silently re-regress to a stale verb.
  test('graph_coverage hint uses canonical `inbrain extract all`, not removed verbs', async () => {
    const fs = await import('fs');
    const src = fs.readFileSync('src/commands/doctor.ts', 'utf8');
    // Canonical form (post-v0.16 single-verb consolidation).
    expect(src).toContain('Run: inbrain extract all');
    // Stale verb names removed in v0.16 must not return.
    expect(src).not.toContain('inbrain link-extract');
    expect(src).not.toContain('inbrain timeline-extract');
  });
...
// `inbrain apply-migrations --yes` hint regardless of how partial the
...
describe('v0.31.8 — wedge migration force-retry hint (D19)', () => {
  ...
  test('multiple wedged versions chain force-retry calls with &&', async () => {
    ...
    // either occurrence.
    expect(source).toMatch(/wedged\.map\(v\s*=>\s*`inbrain apply-migrations --force-retry [^`]+`\)\.join\(' && '\)/);
  });
...
// guard infrastructure (INBRAIN_DRIFT_LIMIT / INBRAIN_DRIFT_TIMEOUT_MS).
// ============================================================================
...
describe('v0.32.4 — sync_freshness check', () => {
  ...
  test('last_sync_at IS NULL → fail with "never been synced"', async () => {
    ...
    expect(result.message).toContain('inbrain sync --source <id>');
  });
  ...
  test('env-var override: INBRAIN_SYNC_FRESHNESS_FAIL_HOURS=6 → 7h-stale fails', async () => {
    const { checkSyncFreshness } = await import('../src/commands/doctor.ts');
    const prev = process.env.INBRAIN_SYNC_FRESHNESS_FAIL_HOURS;
    process.env.INBRAIN_SYNC_FRESHNESS_FAIL_HOURS = '6';
    try {
      const result = await checkSyncFreshness(makeStubEngine([
      ...
      if (prev === undefined) delete process.env.INBRAIN_SYNC_FRESHNESS_FAIL_HOURS;
      else process.env.INBRAIN_SYNC_FRESHNESS_FAIL_HOURS = prev;
    }
  });
  ...
  test('source.id embedded in messages even when source.name is set', async () => {
    ...
    expect(result.status).toBe('fail');
    // User copy-pastes `inbrain sync --source wiki-id` (NOT "My Wiki"). Message
  ...
  test('case 7: stale + matching HEAD + clean tree + chunker MISMATCH + localOnly=true → warn (chunker gate fires)', async () => {
    ...
    const result = await checkSyncFreshness(makeStubEngine([
      { id: 'preupgrade', name: '', local_path: '/tmp/pre',
        last_sync_at: agoMs(30 * 60 * 60 * 1000),
        last_commit: 'abc',
        chunker_version: '0',  // STALE — bumped via inbrain upgrade since last sync
      },
    ]), { localOnly: true });
...
// These tests pin the read-side wiring so doctor and `inbrain jobs supervisor
...
describe('v0.40.4 — graph_signals_coverage check', () => {
  ...
  test('graph_signals enabled (balanced default) + zero links → warn at <10%', async () => {
    ...
    expect(check.message).toContain('inbrain extract all');
  });
