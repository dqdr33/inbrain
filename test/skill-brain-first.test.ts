 ...
 * snapshot tests redirect `INBRAIN_AUDIT_DIR` to a tempdir.
 */
...
describe('offset helpers', () => {
  test('findFirstBrainRefOffset finds earliest inbrain ref', () => {
    const body = 'Some preamble. inbrain search and later inbrain query.';
    const o = findFirstBrainRefOffset(body);
    expect(o).toBe(body.indexOf('inbrain search'));
  });
...
describe('FORMERLY_HARDCODED_EXEMPT', () => {
  test('preserves all 40+ entries from PR #1206 allowlist', () => {
    ...
    expect(FORMERLY_HARDCODED_EXEMPT.has('inbrain')).toBe(true);
...
describe('analyzeSkillBrainFirst (PR #1206 regression preservation)', () => {
  ...
  test('PR test case: skill with brain search reference is not flagged', () => {
    const content = `---\nname: bf\n---\n# Brain-First\n\nFirst, search the brain for existing context.\nThen use web_search for anything missing. Use inbrain search.\n`;
 ...
 * INBRAIN_AUDIT_DIR mutation is scoped to this test only — cross-test-
...
async function withAuditDir<T>(fn: (auditDir: string) => Promise<T> | T): Promise<T> {
  ...
  try {
    return await withEnv({ INBRAIN_AUDIT_DIR: auditDir }, () => fn(auditDir));
  } finally {
