 ...
 * Powers `inbrain sources current` so users can verify both the
...
describe('resolveSourceWithTier — tier 3 (dotfile)', () => {
  ...
  beforeEach(() => {
    scratchDir = mkdtempSync(join(tmpdir(), 'inbrain-tier-dotfile-'));
  });
  afterEach(() => {
    rmSync(scratchDir, { recursive: true, force: true });
  });

  test('.inbrain-source dotfile in CWD returns tier=dotfile', async () => {
    writeFileSync(join(scratchDir, '.inbrain-source'), 'team-alpha\n');
    ...
    expect(result.detail).toBe('.inbrain-source');
  });
...
describe('resolveSourceWithTier — priority assertion', () => {
  test('flag wins over env wins over dotfile wins over default', async () => {
    ...
    await withEnv({ INBRAIN_SOURCE: 'env-src' }, async () => {
