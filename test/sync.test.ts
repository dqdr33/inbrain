...
describe('pruneDir', () => {
  ...
  test('blocks ops (inbrain operational dir)', () => {
    expect(pruneDir('ops')).toBe(false);
  });

  test('blocks *.raw sidecar dirs (inbrain convention)', () => {
    expect(pruneDir('.raw')).toBe(false);
...
describe('pathToSlug', () => {
  ...
  test('handles nested paths', () => {
    expect(pathToSlug('projects/inbrain/spec.md')).toBe('projects/inbrain/spec');
  });
...
describe('performSync dry-run never writes', () => {
  ...
  beforeEach(async () => {
    ...
    repoPath = mkdtempSync(join(tmpdir(), 'inbrain-sync-dryrun-'));
...
describe('#1970: unreachable last_commit bookmark recovery', () => {
  ...
  function mkRepo(files: Record<string, string>): string {
    const dir = mkdtempSync(join(tmpdir(), 'inbrain-1970-'));
    repos.push(dir);
