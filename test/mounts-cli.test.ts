 ...
 * touch the user's real ~/.inbrain/mounts.json.
 */
function withFakeHome<T>(fn: (mountsPath: string) => T): T {
  ...
  try {
    ...
    const mountsPath = join(home, '.inbrain', 'mounts.json');
...
describe('runMounts — end-to-end add/list/remove', () => {
  // These rely on HOME redirection to isolate from the real ~/.inbrain/.
 ...
 *
 * v0.40.3.0: ALSO sets INBRAIN_MOUNTS_PATH because libuv caches homedir()
...
async function withFakeHomeAsync<T>(fn: (mountsPath: string) => Promise<T>): Promise<T> {
  const home = mktmp('fake-home-');
  const prev = process.env.HOME;
  const prevMounts = process.env.INBRAIN_MOUNTS_PATH;
  process.env.HOME = home;
  mkdirSync(join(home, '.inbrain'), { recursive: true });
  const mountsPath = join(home, '.inbrain', 'mounts.json');
  process.env.INBRAIN_MOUNTS_PATH = mountsPath;
  try {
    return await fn(mountsPath);
  } finally {
    if (prev !== undefined) process.env.HOME = prev;
    else delete process.env.HOME;
    if (prevMounts !== undefined) process.env.INBRAIN_MOUNTS_PATH = prevMounts;
    else delete process.env.INBRAIN_MOUNTS_PATH;
  }
}
