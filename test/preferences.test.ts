...
beforeEach(() => {
  ...
  tmp = mkdtempSync(join(tmpdir(), 'inbrain-prefs-test-'));
  // preferences.ts's inbrainDir() returns `$HOME/.inbrain` when INBRAIN_HOME
  // is unset. Test fixtures write to `$tmp/.inbrain/...`, so set HOME only
  // and clear INBRAIN_HOME — setting INBRAIN_HOME would route prefs to $tmp
  // directly (no .inbrain suffix), which doesn't match the fixture layout.
...
describe('loadPreferences', () => {
  ...
  test('parses existing JSON file', () => {
    ...
    writeFileSync(
      join(tmp, '.inbrain', 'preferences.json'),
  ...
  test('throws on malformed JSON so callers can surface it', () => {
    mkdirSync(join(tmp, '.inbrain'), { recursive: true });
    writeFileSync(join(tmp, '.inbrain', 'preferences.json'), '{not json');
...
describe('savePreferences', () => {
  ...
  test('creates ~/.inbrain directory if missing', () => {
    // Confirm .inbrain doesn't exist yet
    expect(existsSync(join(tmp, '.inbrain'))).toBe(false);
    savePreferences({ minion_mode: 'off' });
    expect(existsSync(join(tmp, '.inbrain'))).toBe(true);
  });
  ...
  test('cleans up temp directory used for atomic write', () => {
    savePreferences({ minion_mode: 'off' });
    const inbrainDir = join(tmp, '.inbrain');
    ...
    const entries = readdirSync(inbrainDir);
...
describe('loadCompletedMigrations', () => {
  ...
  test('tolerates malformed lines with a warning, continuing past them', () => {
    const dir = join(tmp, '.inbrain', 'migrations');
