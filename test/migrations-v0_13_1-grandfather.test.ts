...
// Run the phase with an isolated INBRAIN_HOME so the rollback log lands in a
...
async function gf(home: string, opts: Partial<OrchestratorOpts> = {}) {
  ...
  return withEnv({ INBRAIN_HOME: home }, () => phaseCGrandfather(engine, full));
}
...
describe('#1581 phaseCGrandfather (chunked, source-safe)', () => {
  ...
  test('rollback log carries source identity', async () => {
    ...
    // configDir() appends '.inbrain' to INBRAIN_HOME.
    const logPath = join(home, '.inbrain', 'migrations', 'v0_13_1-rollback.jsonl');
