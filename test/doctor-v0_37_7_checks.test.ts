 ...
 *   INBRAIN_HOME surfaces a PID-safe hint.
 *
...
describe('checkSourceRoutingHealth (#1167)', () => {
  ...
  test('non-default source with zero pages → warn (the #1167 fingerprint)', async () => {
    ...
    expect(r.message).toMatch(/inbrain sources current/);
  });
});
...
describe('checkAutopilotLockScope (#1226)', () => {
  test('no lockfile → ok', async () => {
    ...
    await withEnv({ INBRAIN_HOME: sandbox, HOME: sandbox }, async () => {
  ...
  test('stale lock outside INBRAIN_HOME → warn with PID-safe hint', async () => {
    const home = mkdtempSync(join(tmpdir(), 'doctor-lock-home-'));
    const inbrainHome = mkdtempSync(join(tmpdir(), 'doctor-lock-inbrain-'));
    mkdirSync(join(home, '.inbrain'), { recursive: true });
    writeFileSync(join(home, '.inbrain', 'autopilot.lock'), '99999');

    await withEnv({ HOME: home, INBRAIN_HOME: inbrainHome }, async () => {
      const r = checkAutopilotLockScope();
      expect(r.status).toBe('warn');
    ...
    rmSync(inbrainHome, { recursive: true, force: true });
  });
});
