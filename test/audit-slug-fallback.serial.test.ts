 ...
 * Uses INBRAIN_AUDIT_DIR override pointed at a tmpdir for hermeticity (the
...
beforeAll(() => {
  auditDir = mkdtempSync(join(tmpdir(), 'inbrain-slug-fallback-audit-'));
  savedEnv = process.env.INBRAIN_AUDIT_DIR;
  process.env.INBRAIN_AUDIT_DIR = auditDir;
});

afterAll(() => {
  if (savedEnv === undefined) delete process.env.INBRAIN_AUDIT_DIR;
  else process.env.INBRAIN_AUDIT_DIR = savedEnv;
