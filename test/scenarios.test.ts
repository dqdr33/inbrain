...
const ORIG_ROOT = process.env.INBRAIN_CLAW_SCENARIOS_DIR;
let root: string;

beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'scenarios-'));
  process.env.INBRAIN_CLAW_SCENARIOS_DIR = root;
});

afterEach(() => {
  if (ORIG_ROOT !== undefined) process.env.INBRAIN_CLAW_SCENARIOS_DIR = ORIG_ROOT;
  else delete process.env.INBRAIN_CLAW_SCENARIOS_DIR;
