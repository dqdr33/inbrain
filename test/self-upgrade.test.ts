...
async function withTmpHome<T>(fn: () => T | Promise<T>): Promise<T> {
  const dir = mkdtempSync(join(tmpdir(), 'inbrain-selfupgrade-'));
  try {
    return await withEnv({ INBRAIN_HOME: dir, INBRAIN_SELF_UPGRADE_MODE: undefined }, fn);
  } finally {
...
describe('snooze', () => {
  ...
  test('corrupt snooze file → null', async () => {
    await withTmpHome(async () => {
      ...
      mkdirSync(inbrainPath(), { recursive: true });
      writeFileSync(inbrainPath('update-snoozed'), 'garbage not three fields here');
      expect(readSnooze()).toBeNull();
    });
  });
});
...
describe('cache', () => {
  ...
  test('corrupt cache file → null (fail-open)', async () => {
    await withTmpHome(async () => {
      ...
      const { inbrainPath } = await import('../src/core/config.ts');
      mkdirSync(inbrainPath(), { recursive: true });
      writeFileSync(inbrainPath('last-update-check'), 'EVIL not a marker');
      expect(readUpdateCache()).toBeNull();
    });
  });
});
...
describe('resolveSelfUpgradeMode', () => {
  test('defaults to notify', async () => {
    await withEnv({ INBRAIN_SELF_UPGRADE_MODE: undefined }, () => {
  ...
  test('env overrides config', async () => {
    await withEnv({ INBRAIN_SELF_UPGRADE_MODE: 'off' }, () => {
