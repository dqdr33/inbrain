 ...
 * holding a .inbrain-source pin — process.cwd() is process-global and races
 * with parallel files. Drives the real runCodeCallers / runCodeCallees through
 * their process.cwd()-based resolution, asserting:
 *   - a .inbrain-source pin resolves on a multi-source brain (no exit 2)
...
function pinnedDir(sourceId: string): string {
  const dir = mkdtempSync(join(tmpdir(), 'inbrain-pin-cli-'));
  writeFileSync(join(dir, '.inbrain-source'), `${sourceId}\n`);
  return dir;
}
...
describe('code-callers / code-callees — .inbrain-source pin (CLI wiring)', () => {
  test('pin resolves on a multi-source brain: no exit 2, output names the pinned source', async () => {
    ...
    process.chdir(dir);
    try {
      const callers = await withEnv({ INBRAIN_SOURCE: undefined }, () =>
      ...
      const callees = await withEnv({ INBRAIN_SOURCE: undefined }, () =>
  ...
  test('no pin + no flag + multi-source → exit 2 (ambiguous preserved)', async () => {
    await addSource('repo-a', '/fake/a');
    await addSource('repo-b', '/fake/b');
    const dir = mkdtempSync(join(tmpdir(), 'inbrain-nopin-cli-'));
    process.chdir(dir);
    try {
      const callers = await withEnv({ INBRAIN_SOURCE: undefined }, () =>
        capture(() => runCodeCallers(engine, ['someSym', '--no-json'])));
  ...
  test('explicit --source overrides (even with a conflicting pin)', async () => {
    ...
    process.chdir(dir);
    try {
      const { logs, exitCode } = await withEnv({ INBRAIN_SOURCE: undefined }, () =>
  ...
  test('A4: JSON envelope carries source_id + scope (resolved pin)', async () => {
    ...
    process.chdir(dir);
    try {
      const { logs } = await withEnv({ INBRAIN_SOURCE: undefined }, () =>
  ...
  test('A4: --all-sources → source_id null, scope "all"', async () => {
    await addSource('repo-a', '/fake/a');
    await addSource('repo-b', '/fake/b');
    const dir = mkdtempSync(join(tmpdir(), 'inbrain-all-cli-'));
    process.chdir(dir);
    try {
  ...
  test('A4: sole_non_default tier emits the stderr nudge', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'inbrain-sole-cli-'));
    process.chdir(dir);
    try {
      const { errs, exitCode } = await withEnv(
        { INBRAIN_SOURCE: undefined, INBRAIN_NO_SOLE_NON_DEFAULT_NUDGE: undefined },
        () => capture(() => runCodeCallers(engine, ['someSym', '--json'])));
      expect(exitCode).toBeNull();
  ...
  test('A4: dotfile-pin tier does NOT emit the sole_non_default nudge', async () => {
    ...
    process.chdir(dir);
    try {
      const { errs } = await withEnv({ INBRAIN_SOURCE: undefined }, () =>
        capture(() => runCodeCallers(engine, ['someSym', '--json'])));
  ...
  test('A5: zero-result implicit scope appends the "try --all-sources" hint', async () => {
    ...
    try {
      // human output
      const human = await withEnv({ INBRAIN_SOURCE: undefined }, () =>
        capture(() => runCodeCallers(engine, ['someSym', '--no-json'])));
      expect(human.logs.join('\n')).toContain('Try --all-sources');

      // JSON hint field
      const jsonRun = await withEnv({ INBRAIN_SOURCE: undefined }, () =>
  ...
  test('bad .inbrain-source pin → exit 2 with JSON error envelope', async () => {
    await addSource('repo-a', '/fake/a');
    await addSource('repo-b', '/fake/b');
