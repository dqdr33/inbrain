/**
 * Tests for src/core/skillpack/harvest.ts — the host→inbrain lift.
 *
 * Pins:
 *   - happy path: skill files + paired sources land in inbrain's tree
...
function scratchGbrain(): string {
  const root = mkdtempSync(join(tmpdir(), 'sp-h-inbrain-'));
  created.push(root);
  mkdirSync(join(root, 'src', 'commands'), { recursive: true });
...
describe('runHarvest — happy path', () => {
  it('copies a clean skill into inbrain, updates manifest', () => {
    ...
    expect(existsSync(join(inbrainRoot, 'skills', 'my-fork-skill', 'SKILL.md'))).toBe(true);
    expect(result.manifestUpdated).toBe(true);

    const manifest = JSON.parse(readFileSync(join(inbrainRoot, 'openclaw.plugin.json'), 'utf-8'));
  ...
  it('copies paired source files declared in host frontmatter', () => {
    ...
    expect(existsSync(join(inbrainRoot, 'src', 'commands', 'my-fork-skill.ts'))).toBe(true);
  });
});
...
describe('runHarvest — error paths', () => {
  it('host_skill_missing when --from points at the wrong place', () => {
    ...
    try {
      runHarvest({ slug: 'nonexistent', hostRepoRoot: hostRoot, inbrainRoot });
  ...
  it('slug_collision when inbrain already has skills/<slug>/ — without --overwrite-local', () => {
    const hostRoot = scratchHost();
    const inbrainRoot = scratchGbrain();
    mkdirSync(join(inbrainRoot, 'skills', 'my-fork-skill'), { recursive: true });
    writeFileSync(join(inbrainRoot, 'skills', 'my-fork-skill', 'SKILL.md'), '# already here\n');
  ...
  it('symlinks in host skill dir are rejected (D13 security gate)', () => {
    ...
    // Nothing landed.
    expect(existsSync(join(inbrainRoot, 'skills', 'my-fork-skill'))).toBe(false);
  });
});
...
describe('runHarvest — privacy linter integration (T7)', () => {
  it('default Wintermute pattern triggers rollback (no manifest update)', () => {
    ...
    // Rollback: nothing in inbrain tree.
    expect(existsSync(join(inbrainRoot, 'skills', 'my-fork-skill'))).toBe(false);
...
describe('addToBundleManifest', () => {
  it('adds a new slug, sorts skills array, idempotent', () => {
    ...
    expect(addToBundleManifest(inbrainRoot, 'new-skill')).toBe(true);
    expect(addToBundleManifest(inbrainRoot, 'new-skill')).toBe(false); // idempotent
