...
describe('sync-parallel: PGLite + concurrency=4 (T4)', () => {
  ...
  beforeEach(async () => {
    ...
    repoPath = mkdtempSync(join(tmpdir(), 'inbrain-sync-par-'));
  });
...
describe('sync-parallel: bookmark gate under concurrency request (T1)', () => {
  ...
  beforeEach(async () => {
    ...
    await engine.connect({});
    await engine.initSchema();
    repoPath = mkdtempSync(join(tmpdir(), 'inbrain-sync-gate-'));
  });
...
describe('sync-parallel: head-drift gate (CODEX-3)', () => {
  ...
  beforeEach(async () => {
    ...
    await engine.connect({});
    await engine.initSchema();
    repoPath = mkdtempSync(join(tmpdir(), 'inbrain-sync-drift-'));
  });
...
describe('sync-parallel: writer lock prevents reentrance (CODEX-2)', () => {
  ...
  beforeEach(async () => {
    ...
    await engine.connect({});
    await engine.initSchema();
    repoPath = mkdtempSync(join(tmpdir(), 'inbrain-sync-lock-'));
  });
  ...
  test('two parallel performSync calls in same process: second waits or fails fast', async () => {
    ...
    if (secondError) {
      ...
      expect(msg).toMatch(/Another sync is in progress|lock|inbrain-sync/i);
    }
  });
});
