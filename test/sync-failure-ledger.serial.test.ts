...
const originalThreshold = process.env.INBRAIN_SYNC_AUTOSKIP_AFTER;

beforeEach(async () => {
  tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-ledger-'));
  process.env.HOME = tmpHome;
  delete process.env.INBRAIN_SYNC_AUTOSKIP_AFTER;
...
afterEach(() => {
  ...
  if (originalThreshold === undefined) delete process.env.INBRAIN_SYNC_AUTOSKIP_AFTER;
  else process.env.INBRAIN_SYNC_AUTOSKIP_AFTER = originalThreshold;
...
describe('#5 + #6 applySyncFailureGate orchestration', () => {
  test('advance_then_autoskip after threshold; advance runs before auto-skip mark', async () => {
    process.env.INBRAIN_SYNC_AUTOSKIP_AFTER = '3';
  ...
  test('a throwing advance() marks nothing auto_skipped (atomicity)', async () => {
    process.env.INBRAIN_SYNC_AUTOSKIP_AFTER = '1';
    const { applySyncFailureGate, loadSyncFailures } = await L();
...
describe('resolveAutoSkipThreshold', () => {
  test('default 3, env override, 0 disables, invalid → default', async () => {
    ...
    process.env.INBRAIN_SYNC_AUTOSKIP_AFTER = '5';
    expect(resolveAutoSkipThreshold()).toBe(5);
    process.env.INBRAIN_SYNC_AUTOSKIP_AFTER = '0';
    expect(resolveAutoSkipThreshold()).toBe(0);
    process.env.INBRAIN_SYNC_AUTOSKIP_AFTER = 'nonsense';
    expect(resolveAutoSkipThreshold()).toBe(3);
  });
});
