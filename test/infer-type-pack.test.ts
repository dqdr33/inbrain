...
//
// Parity gate: `inferTypeFromPack(path, inbrain-base)` must produce
// IDENTICAL output to the legacy `inferType(path)` for every known
// path prefix. If this drifts, inbrain-base.yaml is out of sync with
// the INBRAIN_BASE_PATH_PREFIXES table in markdown.ts.
//
// Extension test: a user pack adding `paper: { path_prefixes:
// ['papers/'] }` must route `papers/foo.md` to 'paper' (the pack
// declaration), bypassing the inbrain-base fallback.
...
const INBRAIN_BASE_PATH = join(import.meta.dir, '../src/core/schema-pack/base/inbrain-base.yaml');

// Representative paths covering every inbrain-base path-prefix entry.
...
describe('inferTypeFromPack (T7a) — inbrain-base parity', () => {
  test('parity: every known path maps to the same type via pack as via legacy', () => {
    const pack = loadPackFromFile(INBRAIN_BASE_PATH);
  ...
  test('user pack extends inbrain-base with researcher type', () => {
  ...
  test('pack with empty page_types falls back to inbrain-base defaults', () => {
    // E.g. a pack mid-construction with no page_types declared — should
    // not crash, should match inbrain-base behavior.
