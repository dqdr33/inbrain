...
describe('search visibility (soft-deleted pages hidden from searchKeyword)', () => {
  ...
  test('searchKeyword hides soft-deleted pages', async () => {
    ...
    await engine.putPage('people/nora', {
      type: 'note' as any,
      title: 'Nora',
      compiled_truth: 'inbrainquantum signature term occurs here',
      timeline: '',
      frontmatter: {},
    });
    await engine.putPage('people/oscar', {
      type: 'note' as any,
      title: 'Oscar',
      compiled_truth: 'inbrainquantum signature term occurs here too',
      timeline: '',
      frontmatter: {},
    });
    // Force chunk creation so search has something to index.
    await engine.upsertChunks('people/nora', [
      { chunk_index: 0, chunk_text: 'inbrainquantum signature term occurs here', chunk_source: 'compiled_truth' as any },
    ]);
    await engine.upsertChunks('people/oscar', [
      { chunk_index: 0, chunk_text: 'inbrainquantum signature term occurs here too', chunk_source: 'compiled_truth' as any },
    ]);

    const before = await engine.searchKeyword('inbrainquantum');
    expect(before.length).toBe(2);

    await engine.softDeletePage('people/nora');
    const after = await engine.searchKeyword('inbrainquantum');
  ...
  test('searchKeyword hides pages from archived sources', async () => {
    ...
    await engine.executeRaw(
      `INSERT INTO content_chunks (page_id, chunk_index, chunk_text, chunk_source) VALUES ($1, 0, 'inbrainsemaphore unique term', 'compiled_truth')`,
      [pageRows[0].id],
    );
    // Trigger should populate search_vector via the schema trigger.
    const before = await engine.searchKeyword('inbrainsemaphore');
    ...
    const after = await engine.searchKeyword('inbrainsemaphore');
