...
describe('isEvalCaptureEnabled / isEvalScrubEnabled (CONTRIBUTOR_MODE-gated)', () => {
  ...
  const origMode = process.env.INBRAIN_CONTRIBUTOR_MODE;
  const restore = () => {
    if (origMode === undefined) delete process.env.INBRAIN_CONTRIBUTOR_MODE;
    else process.env.INBRAIN_CONTRIBUTOR_MODE = origMode;
  };

  test('defaults to OFF when config is missing AND CONTRIBUTOR_MODE unset', () => {
    delete process.env.INBRAIN_CONTRIBUTOR_MODE;
    try {
  ...
  test('CONTRIBUTOR_MODE=1 turns capture on without any config', () => {
    process.env.INBRAIN_CONTRIBUTOR_MODE = '1';
    try {
  ...
  test('CONTRIBUTOR_MODE=anything-else does NOT turn capture on (strict equality)', () => {
    process.env.INBRAIN_CONTRIBUTOR_MODE = 'true';
    try {
      expect(isEvalCaptureEnabled(null)).toBe(false);
    } finally { restore(); }
    process.env.INBRAIN_CONTRIBUTOR_MODE = 'yes';
    try {
      expect(isEvalCaptureEnabled(null)).toBe(false);
    } finally { restore(); }
    process.env.INBRAIN_CONTRIBUTOR_MODE = '';
    try {
      expect(isEvalCaptureEnabled(null)).toBe(false);
    } finally { restore(); }
  });
