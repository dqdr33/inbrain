 ...
 *
 * Uses INBRAIN_AUDIT_DIR to isolate writes into a temp dir per test.
 */
...
let tmpDir: string;
const originalEnv = process.env.INBRAIN_AUDIT_DIR;

beforeAll(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'inbrain-synopsis-audit-test-'));
  process.env.INBRAIN_AUDIT_DIR = tmpDir;
});

afterAll(() => {
  if (originalEnv === undefined) delete process.env.INBRAIN_AUDIT_DIR;
  else process.env.INBRAIN_AUDIT_DIR = originalEnv;
