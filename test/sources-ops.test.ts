...
const FAKE_GIT_DIR = join(tmpdir(), `inbrain-sources-ops-test-${process.pid}`);
const INBRAIN_HOME = join(FAKE_GIT_DIR, 'inbrain-home');
// inbrainPath() appends `.inbrain` to INBRAIN_HOME, so the actual clone root the
// production code resolves to is $INBRAIN_HOME/.inbrain/clones/. Tests that
// hand-craft path fixtures must use this, NOT $INBRAIN_HOME/clones/.
const CLONE_ROOT = join(INBRAIN_HOME, '.inbrain', 'clones');
...
beforeEach(async () => {
  ...
  // Reset INBRAIN_HOME fixtures between tests
  rmSync(INBRAIN_HOME, { recursive: true, force: true });
  mkdirSync(INBRAIN_HOME, { recursive: true });
  setMode('ok');
});

// Run every test with INBRAIN_HOME pointing at our fixture dir AND fake git
// in PATH. Passed via withEnv so other test files in the shard don't see
// it leak.
async function withEnv2<T>(fn: () => Promise<T>): Promise<T> {
  return withEnv(
    { INBRAIN_HOME, PATH: fakePath() },
    fn,
  );
}
...
describe('removeSource — clone-cleanup', () => {
  test('removes clone IFF managed (local_path under $INBRAIN_HOME/clones/ + remote_url set)', async () => {
    await withEnv2(async () => {
      const row = await addSource(engine, {
        id: 'cleanup-yes',
        remoteUrl: 'https://github.com/example/repo',
  ...
  test('does NOT remove clone for user-supplied --path (no remote_url)', async () => {
    await withEnv2(async () => {
      const userPath = join(INBRAIN_HOME, 'user-managed-fixture');
  ...
  test('symlink-target-OUTSIDE-clones: realpath confinement foils escape', async () => {
    await withEnv2(async () => {
      ...
      const target = join(INBRAIN_HOME, 'sensitive-fixture');
...
describe('getSourceStatus', () => {
  ...
  test('clone_state = "not-applicable" for path-only source (no remote)', async () => {
    await withEnv2(async () => {
      const userPath = join(INBRAIN_HOME, 'na-fixture');
      mkdirSync(userPath, { recursive: true });
...
describe('recloneIfMissing — refuses to delete an unowned working tree (#1881)', () => {
  ...
  test('sync-shape: same refusal surfaces before any filesystem op', async () => {
    await withEnv2(async () => {
      ...
      await expect(recloneIfMissing(engine, 'ghost')).rejects.toThrow(/unmanaged_path|not a clone inbrain created/);
    });
  });
});
...
describe('recloneIfMissing — symlink TOCTOU + EXDEV-safe swap', () => {
  ...
  test('owned no-git clone reclones; no .inbrain-reclone-* / .old-* residue left', async () => {
    await withEnv2(async () => {
      const row = await addSource(engine, {
        id: 'swap-clean',
        remoteUrl: 'https://github.com/example/repo',
      ...
      const residue = readdirSync(CLONE_ROOT).filter(
        (e) => e.startsWith('.inbrain-reclone-') || e.includes('.old-'),
      );
...
describe('isPathContained', () => {
  // Use a sandbox dir, not INBRAIN_HOME (which has the .inbrain quirk).
  const SANDBOX = join(tmpdir(), `inbrain-isPathContained-${process.pid}`);
  beforeEach(() => {
