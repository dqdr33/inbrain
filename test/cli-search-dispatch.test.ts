/**
 * T5 — `inbrain search` dispatch reconcile. Subprocess test against the real
 * cli.ts entrypoint:
...
function run(args: string[], home: string) {
  const r = spawnSync('bun', ['run', 'src/cli.ts', ...args], {
    cwd: process.cwd(),
    encoding: 'utf8',
    env: { ...process.env, INBRAIN_HOME: home, DATABASE_URL: '', INBRAIN_DATABASE_URL: '' },
    timeout: 45_000,
  });
...
function withHome(fn: (home: string) => void) {
  const home = mkdtempSync(join(tmpdir(), 'inbrain-search-dispatch-'));
  mkdirSync(join(home, '.inbrain'), { recursive: true });
  writeFileSync(join(home, '.inbrain', 'config.json'), JSON.stringify({ engine: 'pglite' }));
  try { fn(home); } finally { rmSync(home, { recursive: true, force: true }); }
}

describe('T5 — inbrain search dispatch', () => {
