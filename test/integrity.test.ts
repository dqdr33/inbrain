/**
 * inbrain integrity tests — pure regex + frontmatter-extract paths.
 *
...
describe('findBareTweetHits', () => {
  test('catches "tweeted about X" without URL', () => {
    const hits = findBareTweetHits('Garry tweeted about AI safety last week.', 'people/inbrain-ai');
  ...
  test('skips line that already has a tweet URL', () => {
    const line = 'As he tweeted about YC (https://x.com/inbrain-ai/status/123456).';
  ...
  test('does NOT trigger on already-cited "via X/handle" form', () => {
    const hits = findBareTweetHits('Mentioned via X/inbrain-ai earlier.', 'people/x');
    expect(hits).toEqual([]);
  });
...
describe('extractXHandleFromFrontmatter', () => {
  test('reads x_handle', () => {
    expect(extractXHandleFromFrontmatter({ x_handle: 'inbrain-ai' })).toBe('inbrain-ai');
  });

  test('reads twitter', () => {
    expect(extractXHandleFromFrontmatter({ twitter: 'inbrain-ai' })).toBe('inbrain-ai');
  });

  test('reads twitter_handle', () => {
    expect(extractXHandleFromFrontmatter({ twitter_handle: 'inbrain-ai' })).toBe('inbrain-ai');
  });
  ...
  test('returns null when no handle key is present', () => {
    expect(extractXHandleFromFrontmatter({ name: 'Inbrain Team' })).toBeNull();
  });
