...
// UnrecoverableError when INBRAIN_ALLOW_SHELL_JOBS !== '1'. That's the
...
beforeAll(async () => {
  prevAllowShellJobs = process.env.INBRAIN_ALLOW_SHELL_JOBS;
...
afterAll(async () => {
  ...
  if (prevAllowShellJobs === undefined) delete process.env.INBRAIN_ALLOW_SHELL_JOBS;
  else process.env.INBRAIN_ALLOW_SHELL_JOBS = prevAllowShellJobs;
});
...
describe('shell handler: spawn', () => {
  ...
  test('argv with bogus binary → Error (retryable)', async () => {
    const p = shellHandler(makeCtx({ argv: ['inbrain-nonexistent-binary-xyz'], cwd: '/tmp' }));
...
describe('shell-audit: write', () => {
  ...
  beforeEach(() => {
    ...
    process.env.INBRAIN_AUDIT_DIR = tmpDir;
  });
  afterAll(() => {
    delete process.env.INBRAIN_AUDIT_DIR;
  });

  test('INBRAIN_AUDIT_DIR env override resolves to the custom dir', () => {
  ...
  test('writes a JSONL line; creates dir if missing', () => {
    ...
    process.env.INBRAIN_AUDIT_DIR = inner;
    logShellSubmission({
  ...
  test('write failure (EACCES) is non-blocking', () => {
    ...
    process.env.INBRAIN_AUDIT_DIR = '/dev/null/not-a-dir';
