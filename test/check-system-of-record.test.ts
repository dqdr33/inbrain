...
function runGate(cwd: string): { code: number; stdout: string; stderr: string } {
  // INBRAIN_SCAN_ROOT pins the gate's scan directory to our fake repo.
  ...
  // failed under shard-concurrency load) into the real inbrain repo and
  ...
  const r = spawnSync('bash', [SCRIPT_PATH], {
    ...
    encoding: 'utf-8',
    timeout: 30_000,
    env: { ...process.env, INBRAIN_SCAN_ROOT: cwd },
  });
  return {
    code: r.status ?? -1,
    stdout: r.stdout ?? '',
...
async function reconcile(engine: BrainEngine) {
  await engine.insertFact({ fact: 'ok' } as any, { source_id: 'default' }); // inbrain-allow-direct-insert: this is the canonical reconcile path
}
`,
        'utf-8',
      );
  ...
  test('allow-list comment on a DIFFERENT line does NOT exempt the call', () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        `// inbrain-allow-direct-insert: misplaced comment on the wrong line
import type { BrainEngine } from './engine.ts';
