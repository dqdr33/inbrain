/**
 * v0.39 subprocess smoke for `inbrain doctor --json`.
 *
 ...
 * via `INBRAIN_SKIP_SUBPROCESS_TESTS=1` for fast-loop budget control.
 *
...
const SKIP = process.env.INBRAIN_SKIP_SUBPROCESS_TESTS === '1';

function makeGbrainShim(): { binDir: string; cleanup: () => void } {
  const binDir = mkdtempSync(join(tmpdir(), 'inbrain-shim-doctor-'));
...
describe('inbrain doctor --json subprocess smoke (D10/CMT-2)', () => {
  test.skipIf(SKIP)('exits 0 on freshly-initialized PGLite brain; JSON envelope is well-formed', async () => {
    const home = mkdtempSync(join(tmpdir(), 'inbrain-doctor-smoke-'));
