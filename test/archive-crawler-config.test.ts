 ...
 * `archive-crawler.scan_paths:` is explicitly set in inbrain.yml.
 * These tests pin every gate in that contract:
 *   - missing inbrain.yml -> missing_section
 *   - inbrain.yml without the section -> missing_section
...
function writeYaml(content: string): string {
  const path = join(workdir, 'inbrain.yml');
...
describe('loadArchiveCrawlerConfig — D12 missing_section', () => {
  ...
  it('throws missing_section when inbrain.yml does not exist', () => {
  ...
  it('throws missing_section when inbrain.yml exists but has no archive-crawler section', () => {
    writeYaml('storage:\n  db_tracked:\n    - originals/\n');
    expect(() => loadArchiveCrawlerConfig(workdir)).toThrow(ArchiveCrawlerConfigError);
    try {
      loadArchiveCrawlerConfig(workdir);
