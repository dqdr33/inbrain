...
// pointing INBRAIN_AUDIT_DIR at a tempdir and writing synthetic events.
...
describe('checkBatchRetryHealth — ok states', () => {
  test('no events in window = ok', async () => {
    await withEnv({ INBRAIN_AUDIT_DIR: tmpDir }, async () => {
...
describe('checkBatchRetryHealth — warn states (codex H-9 thresholds)', () => {
  test('>=3 exhausted from same site in 24h = warn', async () => {
    await withEnv({ INBRAIN_AUDIT_DIR: tmpDir }, async () => {
      ...
      expect(check.message).toContain('INBRAIN_BULK_MAX_RETRIES');
    });
  });
...
describe('checkBatchRetryHealth — codex M-10 env validation at doctor time', () => {
  test('invalid INBRAIN_BULK_MAX_RETRIES surfaces at doctor time with paste-ready hint', async () => {
    await withEnv({ INBRAIN_AUDIT_DIR: tmpDir, INBRAIN_BULK_MAX_RETRIES: '-1' }, async () => {
      const check = await checkBatchRetryHealth(stubEngine);
      expect(check.status).toBe('warn');
      expect(check.message).toContain('INBRAIN_BULK_*');
      expect(check.message).toContain('export INBRAIN_BULK_MAX_RETRIES');
    });
  });

  test('valid INBRAIN_BULK_MAX_RETRIES=0 (debug-mode disable) is accepted', async () => {
    await withEnv({ INBRAIN_AUDIT_DIR: tmpDir, INBRAIN_BULK_MAX_RETRIES: '0' }, async () => {
      const check = await checkBatchRetryHealth(stubEngine);
