...
describe('loadConfig env database URL precedence', () => {
  test('DATABASE_URL switches an existing PGLite file config to Postgres', async () => {
    const home = mkdtempSync(join(tmpdir(), 'inbrain-config-env-'));
    try {
      // Pre-seed: PGLite file config in this isolated INBRAIN_HOME.
      await withEnv(
        { INBRAIN_HOME: home, INBRAIN_DATABASE_URL: undefined, DATABASE_URL: undefined },
        () => {
      ...
      await withEnv(
        {
          INBRAIN_HOME: home,
          INBRAIN_DATABASE_URL: undefined,
          DATABASE_URL: 'postgres://user:pass@example.test:5432/inbrain',
        },
        () => {
          const cfg = loadConfig();
          expect(cfg?.engine).toBe('postgres');
          expect(cfg?.database_url).toBe('postgres://user:pass@example.test:5432/inbrain');
  ...
  test('INBRAIN_DATABASE_URL beats DATABASE_URL (operator override)', async () => {
    const home = mkdtempSync(join(tmpdir(), 'inbrain-config-env-'));
    try {
      await withEnv(
        { INBRAIN_HOME: home, INBRAIN_DATABASE_URL: undefined, DATABASE_URL: undefined },
      ...
      await withEnv(
        {
          INBRAIN_HOME: home,
          INBRAIN_DATABASE_URL: 'postgres://win:win@inbrain.test:5432/db',
          DATABASE_URL: 'postgres://lose:lose@other.test:5432/db',
        },
        () => {
          const cfg = loadConfig();
          expect(cfg?.engine).toBe('postgres');
          expect(cfg?.database_url).toBe('postgres://win:win@inbrain.test:5432/db');
        },
      );
    } finally {
      rmSync(home, { recursive: true, force: true });
      ...
      await withEnv(
        {
          INBRAIN_HOME: home,
          INBRAIN_DATABASE_URL: undefined,
          DATABASE_URL: 'postgres://only:env@inbrain.test:5432/db',
        },
        () => {
          // No saveConfig() — no file present at all.
          const cfg = loadConfig();
          expect(cfg?.engine).toBe('postgres');
          expect(cfg?.database_url).toBe('postgres://only:env@inbrain.test:5432/db');
        },
      );
    } finally {
      rmSync(home, { recursive: true, force: true });
