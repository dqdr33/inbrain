...
beforeAll(async () => {
  // v0.37.10.0: doctorReportRemote reads from ~/.inbrain audit files
  ...
  // INBRAIN_HOME to a tempdir so audit reads return empty.
  tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-doctor-remote-'));
  priorHome = process.env.INBRAIN_HOME;
...
afterAll(async () => {
  ...
  if (priorHome === undefined) delete process.env.INBRAIN_HOME;
  else process.env.INBRAIN_HOME = priorHome;
