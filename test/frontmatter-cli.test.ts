...
describe('inbrain frontmatter CLI (B4)', () => {
  let tmp: string;
  ...
  test('validate --fix writes centralized backup and rewrites in place', () => {
    const f = join(tmp, 'broken.md');
    const inbrainHome = join(tmp, 'home');
    const original = `${fence}\ntype: concept\ntitle: "P "I" L"\n${fence}\n\nbody`;
    writeFileSync(f, original);
    const { stdout, code } = runCli(['validate', f, '--fix'], { INBRAIN_HOME: inbrainHome });
    expect(code).toBe(0);
    expect(existsSync(f + '.bak')).toBe(false);
    expect(stdout).toContain('centralized backups');
    const backupDir = join(inbrainHome, '.inbrain', 'backups', 'frontmatter');
  ...
  test('validate --fix succeeds on a non-git path (no dirty-tree guard)', () => {
    ...
    const inbrainHome = join(tmp, 'home');
    writeFileSync(f, `${fence}\ntype: concept\ntitle: "A "B" C"\n${fence}\n\nbody`);
    const { code } = runCli(['validate', f, '--fix'], { INBRAIN_HOME: inbrainHome });
    expect(code).toBe(0);
    expect(existsSync(f + '.bak')).toBe(false);
    expect(existsSync(join(inbrainHome, '.inbrain', 'backups', 'frontmatter'))).toBe(true);
  });
  ...
  test('generate --fix skips catch-all note writes unless explicitly included', () => {
    ...
    const skipped = runCli(['generate', tmp, '--fix', '--json'], { INBRAIN_HOME: inbrainHome });
    ...
    const included = runCli(['generate', tmp, '--fix', '--json', '--include-catch-all'], { INBRAIN_HOME: inbrainHome });
