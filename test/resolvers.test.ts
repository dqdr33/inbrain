...
describe('x_handle_to_tweet resolver', () => {
  ...
  test('zero candidates → confidence 0', async () => {
    ...
    const r = await xHandleToTweetResolver.resolve({
      input: { handle: 'inbrain-ai', keywords: 'nothing matches' },
      context: makeCtx(),
    });
  ...
  test('single strong match → confidence >= 0.8 (auto-repair bucket)', async () => {
    ...
    globalThis.fetch = (async () => new Response(JSON.stringify({
      data: [
        { id: '123', text: 'talking about building inbrain today', created_at: '2026-04-18T00:00:00Z' },
      ],
    }), { status: 200, headers: { 'content-type': 'application/json' } })) as unknown as typeof fetch;
    const r = await xHandleToTweetResolver.resolve({
      input: { handle: 'inbrain-ai', keywords: 'building inbrain' },
      context: makeCtx(),
    });
    expect(r.confidence).toBeGreaterThanOrEqual(0.8);
    expect(r.value.url).toBe('https://x.com/inbrain-ai/status/123');
  ...
  test('single weak-match → confidence in 0.5-0.8 review range', async () => {
    ...
    const r = await xHandleToTweetResolver.resolve({
      input: { handle: 'inbrain-ai', keywords: 'inbrain knowledge runtime specific terms' },
      context: makeCtx(),
    });
  ...
  test('many ambiguous candidates → confidence < 0.5 (skip bucket)', async () => {
    ...
    const r = await xHandleToTweetResolver.resolve({
      input: { handle: 'inbrain-ai', keywords: 'completely different signal words unlikely to match' },
      context: makeCtx(),
    });
  ...
  test('strips X operators from keyword input (injection defense)', async () => {
    ...
    await xHandleToTweetResolver.resolve({
      input: { handle: 'inbrain-ai', keywords: 'from:evil_user lang:ja to:someone normal words' },
      context: makeCtx(),
    });
    ...
    expect(query).toContain('from:inbrain-ai');
