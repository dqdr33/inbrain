/**
 * v0.41.16.0 — `inbrain conversation-parser` debug CLI tests.
 *
 * Pins:
...
describe('runConversationParser — help', () => {
  test('--help prints usage', async () => {
    ...
    expect(text).toContain('Usage: inbrain conversation-parser');
  ...
  test('no subcommand prints help', async () => {
    ...
    expect(cap.out.join('')).toContain('Usage: inbrain conversation-parser');
  });
});
