...
describe('resolveSourceId priority 2 — INBRAIN_SOURCE env', () => {
  test('wins over dotfile / registered-path / default', async () => {
    ...
    process.env.INBRAIN_SOURCE = 'env-wins';
    try {
...
describe('resolveSourceId priority 3 — .inbrain-source dotfile walk-up', () => {
  let tmpdirPath: string;

  beforeEach(() => {
    tmpdirPath = mkdtempSync(join(tmpdir(), 'inbrain-resolver-test-'));
  });
  afterEach(() => {
  ...
  test('finds dotfile in CWD', async () => {
    writeFileSync(join(tmpdirPath, '.inbrain-source'), 'gstack\n');
  ...
  test('walks up ancestors to find dotfile', async () => {
    writeFileSync(join(tmpdirPath, '.inbrain-source'), 'wiki\n');
  ...
  test('ignores dotfile with invalid content', async () => {
    writeFileSync(join(tmpdirPath, '.inbrain-source'), 'INVALID!\n');
    const engine = makeStub(['default'], [], null);
    const id = await resolveSourceId(engine, null, tmpdirPath);
