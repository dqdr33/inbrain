...
describe('isSupabasePoolerUrl', () => {
  ...
  test('rejects self-hosted on standard port', () => {
    expect(isSupabasePoolerUrl('postgresql://u:p@localhost:5432/inbrain_test')).toBe(false);
  });
...
describe('readKillSwitchEnv', () => {
  ...
  beforeEach(() => { original = process.env.INBRAIN_DISABLE_DIRECT_POOL; });
  afterEach(() => {
    if (original === undefined) delete process.env.INBRAIN_DISABLE_DIRECT_POOL;
    else process.env.INBRAIN_DISABLE_DIRECT_POOL = original;
  });

  test('false when unset', () => {
    delete process.env.INBRAIN_DISABLE_DIRECT_POOL;
    expect(readKillSwitchEnv()).toBe(false);
  });

  test('true when "1"', () => {
    process.env.INBRAIN_DISABLE_DIRECT_POOL = '1';
    expect(readKillSwitchEnv()).toBe(true);
  });

  test('true when "true"', () => {
    process.env.INBRAIN_DISABLE_DIRECT_POOL = 'true';
    expect(readKillSwitchEnv()).toBe(true);
  });

  test('false for any other value', () => {
    process.env.INBRAIN_DISABLE_DIRECT_POOL = '0';
    expect(readKillSwitchEnv()).toBe(false);
    process.env.INBRAIN_DISABLE_DIRECT_POOL = 'false';
    expect(readKillSwitchEnv()).toBe(false);
  });
});
...
describe('resolveDirectPoolSize', () => {
  let original: string | undefined;
  beforeEach(() => { original = process.env.INBRAIN_DIRECT_POOL_SIZE; });
  afterEach(() => {
    if (original === undefined) delete process.env.INBRAIN_DIRECT_POOL_SIZE;
    else process.env.INBRAIN_DIRECT_POOL_SIZE = original;
  });

  test('default to 3', () => {
    delete process.env.INBRAIN_DIRECT_POOL_SIZE;
  ...
  test('explicit overrides env', () => {
    process.env.INBRAIN_DIRECT_POOL_SIZE = '5';
  ...
  test('rejects invalid env values', () => {
    process.env.INBRAIN_DIRECT_POOL_SIZE = 'abc';
    expect(resolveDirectPoolSize()).toBe(DEFAULT_DIRECT_POOL_SIZE);
    process.env.INBRAIN_DIRECT_POOL_SIZE = '0';
    expect(resolveDirectPoolSize()).toBe(DEFAULT_DIRECT_POOL_SIZE);
    process.env.INBRAIN_DIRECT_POOL_SIZE = '999';
    expect(resolveDirectPoolSize()).toBe(DEFAULT_DIRECT_POOL_SIZE);
  });
});
...
describe('ConnectionManager — describeMode + dual-pool routing', () => {
  ...
  beforeEach(() => {
    originalKillSwitch = process.env.INBRAIN_DISABLE_DIRECT_POOL;
    delete process.env.INBRAIN_DISABLE_DIRECT_POOL;
  });
  afterEach(() => {
    if (originalKillSwitch === undefined) delete process.env.INBRAIN_DISABLE_DIRECT_POOL;
    else process.env.INBRAIN_DISABLE_DIRECT_POOL = originalKillSwitch;
  });
...
describe('ConnectionManager — parent inheritance (A2)', () => {
  test('child inherits kill-switch from parent', () => {
    const original = process.env.INBRAIN_DISABLE_DIRECT_POOL;
    try {
      process.env.INBRAIN_DISABLE_DIRECT_POOL = '1';
