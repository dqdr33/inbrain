// v0.38 inbrain-base byte-for-byte equivalence gate (T5 + T25 CI-blocking).
//
// inbrain-base.yaml MUST reproduce pre-v0.38 hardcoded behavior exactly.
...
// validation time).
//
// If this test fails, inbrain-base.yaml drifted from the source-of-truth
// constants. Either the YAML needs to be updated to match a deliberate
// constant change, or the constant change is unintentional and inbrain-base
...
const BASE_PATH = join(import.meta.dir, '../../src/core/schema-pack/base/inbrain-base.yaml');

describe('inbrain-base v0.38 parity gate', () => {
  test('every ALL_PAGE_TYPES seed appears in inbrain-base page_types', () => {
  ...
  test('person + company are the only expert_routing default types', () => {
    ...
    // inbrain-base must reproduce this default; user packs opt in others
  ...
  test('alias graph is EMPTY by default (E8 codex F8)', () => {
    // inbrain-base ships with NO alias edges so existing search behavior
  ...
  test('codegen --check passes in process', () => {
    ...
    const pack = loadPackFromFile(BASE_PATH);
    expect(pack.name).toBe('inbrain-base');
