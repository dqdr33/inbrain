 ...
 * `inbrain serve` as a non-daemon (or any other intentional long-
...
describe('shouldForceExitAfterMain — daemon survival gate', () => {
  ...
  test('returns false even when global flags precede `serve`', () => {
    ...
    // flag position. This is the load-bearing case for `inbrain --quiet serve`.
  ...
  test('substring match avoidance: `serves` is NOT `serve`', () => {
    // Future-proofing against a `inbrain serves-foo` subcommand being
