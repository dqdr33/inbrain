 ...
 * tests guard the three suppression flags (--json, INBRAIN_NO_CODE_MODEL_NUDGE,
...
describe('runReindexCode — nudge integration (dry-run path reaches nudge)', () => {
  ...
  beforeAll(async () => {
    prevNudgeEnv = process.env.INBRAIN_NO_CODE_MODEL_NUDGE;
    delete process.env.INBRAIN_NO_CODE_MODEL_NUDGE;
  ...
  afterAll(async () => {
    ...
    resetGateway();
    if (prevNudgeEnv === undefined) delete process.env.INBRAIN_NO_CODE_MODEL_NUDGE;
    else process.env.INBRAIN_NO_CODE_MODEL_NUDGE = prevNudgeEnv;
  }, 30_000);
  ...
  test('INBRAIN_NO_CODE_MODEL_NUDGE=1 → nudge suppressed', async () => {
    configureGateway({
      embedding_model: 'openai:text-embedding-3-large',
      embedding_dimensions: 1536,
      env: { OPENAI_API_KEY: 'sk-test' },
    });
    process.env.INBRAIN_NO_CODE_MODEL_NUDGE = '1';
    captureStreams();
    try {
      await runReindexCode(engine, { dryRun: true });
    } finally {
