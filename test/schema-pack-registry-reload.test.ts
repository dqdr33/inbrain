...
beforeEach(() => {
  _resetPackCacheForTests();
  tmpDir = mkdtempSync(join(tmpdir(), 'inbrain-registry-test-'));
});
...
describe('tryCachedPack — TTL gate', () => {
  ...
  it('respects INBRAIN_PACK_STAT_TTL_MS env override', async () => {
    await withEnv({ INBRAIN_PACK_STAT_TTL_MS: '0' }, async () => {
  ...
  it('falls back to default TTL when env override is invalid', async () => {
    await withEnv({ INBRAIN_PACK_STAT_TTL_MS: 'not-a-number' }, async () => {
