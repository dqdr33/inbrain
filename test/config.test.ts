...
describe('loadConfig — legacy provider+model migration (v0.36.1.x #1086)', () => {
  test('translates {provider, model} to embedding_model', async () => {
    ...
    const tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-cfg-test-'));
    try {
...
describe('configDir — INBRAIN_HOME Windows path acceptance (v0.36.1.x #1019)', () => {
  test('relative paths are rejected with an absolute-path error', async () => {
    const { withEnv } = await import('./helpers/with-env.ts');
    await withEnv({ INBRAIN_HOME: 'relative/path' }, async () => {
  ...
  test("'..' segments rejected on POSIX-style absolute paths", async () => {
    const { withEnv } = await import('./helpers/with-env.ts');
    await withEnv({ INBRAIN_HOME: '/tmp/foo/../bar' }, async () => {
      const { configDir } = await import('../src/core/config.ts');
...
describe('loadConfig — INBRAIN_MAX_MARKUP_RATIO env (v0.42 #1699)', () => {
  async function withHomeAndEnv(env: Record<string, string | undefined>, fn: (cfg: unknown) => void) {
    const { mkdtempSync, mkdirSync, writeFileSync, rmSync } = await import('fs');
    const { join } = await import('path');
    const { tmpdir } = await import('os');
    const { withEnv } = await import('./helpers/with-env.ts');
    const tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-cfg-mk-'));
    try {
      mkdirSync(join(tmpHome, '.inbrain'), { recursive: true });
      writeFileSync(join(tmpHome, '.inbrain', 'config.json'), JSON.stringify({ engine: 'pglite', database_path: '/tmp/x' }));
      await withEnv({ INBRAIN_HOME: tmpHome, ...env }, async () => {
        const { loadConfig } = await import('../src/core/config.ts');
        fn(loadConfig());
      });
    } finally {
  ...
  test('valid ratio in (0,1] is parsed onto content_sanity', async () => {
    await withHomeAndEnv({ INBRAIN_MAX_MARKUP_RATIO: '0.7' }, (cfg) => {
  ...
  test('out-of-range value (>1) is ignored', async () => {
    await withHomeAndEnv({ INBRAIN_MAX_MARKUP_RATIO: '1.5' }, (cfg) => {
