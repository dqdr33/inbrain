...
const FAKE_GIT_DIR = join(tmpdir(), `inbrain-git-remote-test-${process.pid}`);
...
describe('parseRemoteUrl — happy path', () => {
  test('accepts plain https URL', () => {
    const r = parseRemoteUrl('https://github.com/inbrain-ai/dummy.git');
    expect(r.url).toBe('https://github.com/inbrain-ai/dummy.git');
...
describe('parseRemoteUrl — CGNAT 100.64/10 (Tailscale)', () => {
  test('rejected by default', async () => {
    await withEnv({ INBRAIN_ALLOW_PRIVATE_REMOTES: undefined }, async () => {
      try {
  ...
  test('accepted with INBRAIN_ALLOW_PRIVATE_REMOTES=1', async () => {
    await withEnv({ INBRAIN_ALLOW_PRIVATE_REMOTES: '1' }, async () => {
