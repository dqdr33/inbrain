 ...
 * ignored the .inbrain-source pin. The new helper runs the full 7-tier chain
 ...
 * with no .inbrain-source, so the resolver falls through to the DB tiers. */
function cleanCwd(): string {
  return mkdtempSync(join(tmpdir(), 'inbrain-scoped-clean-'));
}
...
describe('resolveScopedSourceOrThrow', () => {
  test('single-source brain: returns default via seed_default tier (no throw)', async () => {
    const cwd = cleanCwd();
    try {
      const r = await withEnv({ INBRAIN_SOURCE: undefined }, () =>
  ...
  test('.inbrain-source pin resolves on a multi-source brain (THE bug)', async () => {
    await addSource('repo-a', '/fake/a');
    await addSource('repo-b', '/fake/b');
    const cwd = mkdtempSync(join(tmpdir(), 'inbrain-scoped-pin-'));
    writeFileSync(join(cwd, '.inbrain-source'), 'repo-a\n');
    try {
      const r = await withEnv({ INBRAIN_SOURCE: undefined }, () =>
        resolveScopedSourceOrThrow(engine, cwd));
  ...
  test('no pin + no signal + multi-source → multiple_sources_ambiguous', async () => {
    ...
    try {
      await withEnv({ INBRAIN_SOURCE: undefined }, () =>
        resolveScopedSourceOrThrow(engine, cwd));
    } catch (e) {
      caught = e;
    } finally {
  ...
  test('INBRAIN_SOURCE env tier wins on a multi-source brain', async () => {
    await addSource('repo-a', '/fake/a');
    await addSource('repo-b', '/fake/b');
    const cwd = cleanCwd();
    try {
      const r = await withEnv({ INBRAIN_SOURCE: 'repo-b' }, () =>
        resolveScopedSourceOrThrow(engine, cwd));
  ...
  test('bad .inbrain-source pin (nonexistent source) throws a resolver user error', async () => {
    await addSource('repo-a', '/fake/a');
    await addSource('repo-b', '/fake/b');
    const cwd = mkdtempSync(join(tmpdir(), 'inbrain-scoped-badpin-'));
    writeFileSync(join(cwd, '.inbrain-source'), 'does-not-exist\n');
    let caught: unknown = null;
    try {
      await withEnv({ INBRAIN_SOURCE: undefined }, () =>
        resolveScopedSourceOrThrow(engine, cwd));
