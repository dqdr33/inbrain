...
beforeEach(() => {
  ...
  tmp = mkdtempSync(join(tmpdir(), 'inbrain-doctor-minions-test-'));
});
...
describe('inbrain doctor — half-migrated Minions detection', () => {
  test('filesystem: partial completed.jsonl entry with no matching complete → FAIL', () => {
    // Seed ~/.inbrain/migrations/completed.jsonl with a single status:"partial"
    // entry — the classic signal the stopgap ran but apply-migrations didn't.
    const migrationsDir = join(tmp, '.inbrain', 'migrations');
    ...
    expect(minions!.message).toContain('inbrain apply-migrations --yes');
  ...
  test('regression: fresh install with schema-applied DB but no prefs must NOT fail', () => {
    // CI regression. `inbrain init` against Postgres applies schema v7 but
    ...
    // "inbrain doctor exits 0 on healthy DB"): the old "schema v7+ no
  ...
  test('filesystem: stale partial superseded by newer complete → NO warning (forward-progress override)', () => {
    ...
    // long-running inbrain installs accumulate partial entries from
  ...
  test('human output: prints MINIONS HALF-INSTALLED loud banner', () => {
    ...
    expect(result.stdout).toContain('inbrain apply-migrations --yes');
  });
});
