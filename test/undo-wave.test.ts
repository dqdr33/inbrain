...
describe('undoWave — happy path', () => {
  ...
  test('resolved_by filter: UPDATE takes scoped to wave-applied resolutions only', async () => {
    ...
    // resolved_by parameter is $2 = 'inbrain:grade_takes' (default label)
    expect(updateCall!.params[1]).toBe('inbrain:grade_takes');
  });
  ...
  test('custom resolvedByLabel is honored', async () => {
    ...
    await undoWave(engine, { waveVersion: 'v0.36.1.0', resolvedByLabel: 'inbrain:grade_takes-custom' });
    const updateCall = sqls.find(s => s.sql.includes('UPDATE takes'));
    expect(updateCall!.params[1]).toBe('inbrain:grade_takes-custom');
  });
});
