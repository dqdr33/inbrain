...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-upgrade-checkpoint-test-'));
  originalHome = process.env.INBRAIN_HOME;
  process.env.INBRAIN_HOME = tmpHome;
});

afterEach(() => {
  if (originalHome === undefined) delete process.env.INBRAIN_HOME;
  else process.env.INBRAIN_HOME = originalHome;
...
describe('writeCheckpoint + loadCheckpoint round-trip', () => {
  ...
  test('loadCheckpoint returns null for malformed JSON', () => {
    ...
    writeCheckpoint(cp);
    // Corrupt the file. inbrainPath resolves to INBRAIN_HOME/.inbrain/<file>.
    const path = join(tmpHome, '.inbrain', 'upgrade-checkpoint.json');
