 ...
 *
 * Hermetic — drives the file system + INBRAIN_HOME + HOME envs directly via
 ...
 *   - INBRAIN_HOME override outside any worktree — ok
 */
...
beforeEach(() => {
  scratch = join(tmpdir(), `inbrain-doctor-hw-${Date.now()}-${Math.random().toString(36).slice(2)}`);
 ...
 *  INBRAIN_HOME, capture stdout AND prevent runDoctor's `process.exit(N)` from
...
describe('home_dir_in_worktree doctor check', () => {
  test('inbrain home outside any worktree → ok', async () => {
    // scratch/.inbrain — no parent has a .git, scratch IS our fake $HOME
    const home = scratch;
    const inbrainParent = home;
    const check = await getCheck('home_dir_in_worktree', {
      HOME: home,
      INBRAIN_HOME: inbrainParent,
    });
    expect(check).toBeDefined();
    expect(check!.status).toBe('ok');
  });

  test('inbrain home inside dir-style .git worktree → warn', async () => {
    // scratch/home/myrepo/.git/    (directory)
    // scratch/home/myrepo/.inbrain/ ← inbrain home is inside the worktree
  ...
  test('inbrain home inside .git-AS-FILE linked worktree → warn (F4)', async () => {
  ...
  test('walk terminates at $HOME — .git ABOVE $HOME does NOT trigger warn (F4)', async () => {
    ...
    // scratch/home/.inbrain/  (no worktree below $HOME)
  ...
  test('INBRAIN_HOME override pointing outside any worktree → ok', async () => {
    // Real $HOME might be inside a worktree, but the user pointed
    // INBRAIN_HOME at a clean location. Doctor should report ok.
    const home = scratch;
