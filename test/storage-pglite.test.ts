 ...
 * inbrain.yml load → inbrain storage status → soft-warn message present →
...
beforeEach(async () => {
  tmp = mkdtempSync(join(tmpdir(), 'inbrain-pglite-test-'));
...
describe('Storage tiering on PGLite — full lifecycle (D8 + D4)', () => {
  ...
  test('getStorageStatus loads inbrain.yml and reports tier counts', async () => {
    try {
      writeGbrainYml();
  ...
  test('end-to-end: inbrain.yml + putPage + storage status + .gitignore', async () => {
    try {
      writeGbrainYml();
      await engine.putPage('people/alice', { type: 'person', title: 'Alice', compiled_truth: '', timeline: '' });
      await engine.putPage('media/x/tweet-1', { type: 'concept', title: 'T1', compiled_truth: '', timeline: '' });
      ...
      expect(gitignore).toContain('# Auto-managed by inbrain');
