 ...
 *
 * Strategy: spin up a tiny in-process HTTP server that mimics `inbrain serve --http`
...
import type { InbrainConfig } from '../src/core/config.ts';
...
function makeConfig(overrides: Partial<NonNullable<InbrainConfig['remote_mcp']>> = {}): InbrainConfig {
  return {
    engine: 'postgres',
    remote_mcp: {
...
describe('collectRemoteDoctorReport', () => {
  ...
  test('missing client_secret entirely — fails before any HTTP call', async () => {
    ...
    await withEnv({ INBRAIN_REMOTE_CLIENT_SECRET: undefined }, async () => {
      ...
      expect(creds.message).toContain('INBRAIN_REMOTE_CLIENT_SECRET');
  ...
  test('missing remote_mcp on config — fails config_integrity', async () => {
    reset();
    const config: InbrainConfig = { engine: 'postgres' };
    const report = await collectRemoteDoctorReport(config);
    expect(report.status).toBe('fail');
  ...
  test('env var INBRAIN_REMOTE_CLIENT_SECRET overrides config-file secret', async () => {
    reset();
    await withEnv({ INBRAIN_REMOTE_CLIENT_SECRET: 'env-supplied-secret' }, async () => {
...
describe('runUpgradeDriftCheck', () => {
  ...
  test('major drift, no prior state → warn with auto-upgrade fix hint', async () => {
    ...
    const tmpHome = mkdtempSync(join(tmpdir(), 'inbrain-doctor-drift-'));
    try {
      await withEnv({ INBRAIN_HOME: tmpHome }, async () => {
        ...
        expect(result.message).toContain('Run `inbrain upgrade`');
  ...
  test('major drift with prior_failed state → warn with manual-install fix hint', async () => {
    ...
    try {
      ...
      const stateDir = join(tmpHome, '.inbrain');
      ...
      await withEnv({ INBRAIN_HOME: tmpHome }, async () => {
        ...
        expect(result.message).toContain('Prior `inbrain upgrade` did not advance');
        expect(result.message).toContain('https://github.com/inbrain-ai/inbrain/releases');
        expect(result.message).not.toContain('Run `inbrain upgrade`');
