...
description: Skill that runs inbrain search before external lookup
triggers:
  - "look up a topic"
---
...
reference (inbrain search) appears strictly before the first external
reference (web_search), so the analyzer accepts it without requiring
the canonical callout.

## Workflow

1. Run `inbrain search "topic"` to find existing brain pages.
