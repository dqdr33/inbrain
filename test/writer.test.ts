...
describe('Scaffolder', () => {
  test('tweetCitation builds canonical form', () => {
    const c = tweetCitation({ handle: 'inbrain-ai', tweetId: '1234567890', dateISO: '2026-04-18' });
    expect(c).toBe('[Source: [X/inbrain-ai, 2026-04-18](https://x.com/inbrain-ai/status/1234567890)]');
  });

  test('tweetCitation strips leading @', () => {
    const c = tweetCitation({ handle: '@inbrain-ai', tweetId: '1', dateISO: '2026-04-18' });
    expect(c).toContain('X/inbrain-ai');
    expect(c).not.toContain('@inbrain-ai');
  });
  ...
  test('tweetCitation rejects non-numeric tweet id', () => {
    expect(() => tweetCitation({ handle: 'inbrain-ai', tweetId: 'abc' })).toThrow(ScaffoldError);
  });

  test('tweetCitation rejects bad date format', () => {
    expect(() => tweetCitation({ handle: 'inbrain-ai', tweetId: '1', dateISO: '2026/04/18' })).toThrow(ScaffoldError);
  });
...
describe('citation validator', () => {
  ...
  test('passes paragraph with [Source: ...]', async () => {
    const findings = await run('Alice was a founder [Source: X/inbrain-ai, 2026-04-18].');
...
\`\`\``;
  ...
  test('ignores inline code within paragraph (but paragraph still needs citation)', async () => {
    const compiled = 'Alice shipped `inbrain` last week.';
    const findings = await run(compiled);
    expect(findings).toHaveLength(1);
  });
