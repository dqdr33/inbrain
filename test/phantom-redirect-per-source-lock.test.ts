 ...
 *
 * Pre-v0.40 phantom-redirect acquired the bare `inbrain-sync` lock, blocking
 * every concurrent sync brain-wide. After D16, phantom acquires
 * `inbrain-sync:<sourceId>` — same-source sync still serializes; cross-source
...
describe('phantom-redirect lock contract', () => {
  ...
  test('helper sanity: syncLockId returns inbrain-sync:<source> shape', () => {
