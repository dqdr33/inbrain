 ...
 * INBRAIN_HOME=<tmp> for hermeticity — see test/inbrain-home-isolation.test.ts
...
const ORIG_HOME = process.env.INBRAIN_HOME;
const ORIG_RUN_ID = process.env.INBRAIN_FRICTION_RUN_ID;
let tmp: string;

beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'friction-test-'));
  process.env.INBRAIN_HOME = tmp;
  delete process.env.INBRAIN_FRICTION_RUN_ID;
});

afterEach(() => {
  process.env.INBRAIN_HOME = ORIG_HOME;
  if (ORIG_RUN_ID !== undefined) process.env.INBRAIN_FRICTION_RUN_ID = ORIG_RUN_ID;
...
describe('activeRunId', () => {
  ...
  test('reads INBRAIN_FRICTION_RUN_ID', () => {
    process.env.INBRAIN_FRICTION_RUN_ID = 'my-run';
    try {
...
describe('renderer', () => {
  ...
  test('redact strips homedir and cwd from message + cwd field', () => {
    ...
    logFriction({
      runId: 'run-red',
      phase: 'p',
      message: `error at ${home}/.inbrain/foo and ${fakeCwd}/bar.ts`,
    });
    const md = renderReport('run-red', { format: 'md', redact: true });
    expect(md).not.toContain(home + '/.inbrain');
...
describe('redactEntry pure function', () => {
  test('replaces homedir occurrences', () => {
    ...
    const e: FrictionEntry = {
      ...
      message: `${home}/secret/file.txt`, source: 'claw', cwd: '/cwd', inbrain_version: 'test',
    };
