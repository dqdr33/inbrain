/**
 * Tests for `inbrain eval replay` (v0.25.0).
 *
 * Three layers:
...
describe('inbrain eval replay — happy path', () => {
...
describe('inbrain eval replay — failure modes', () => {
  ...
  test('schema_version === 2 (future) is rejected with helpful message', async () => {
    await withTmp(async (dir) => {
      ...
      process.exit = origExit;
      expect(code).toBe(1);
      expect(stderr.toLowerCase()).toContain('upgrade inbrain');
    });
  });
