...
// ─────────────────────────────────────────────────────────────────
//
// The base schema shipped 8 inbrain-managed public tables without RLS
// enabled (access_tokens, mcp_request_log, minion_inbox,
// minion_attachments, subagent_messages, subagent_tool_executions,
// subagent_rate_leases, inbrain_cycle_locks). Migration v12 created
...
describe('migration v35 — auto_rls_event_trigger structural guards', () => {
  ...
  test('backfill exemption regex matches the doctor.ts contract', () => {
    const v35 = MIGRATIONS.find(m => m.version === 35);
    const sql = ((v35?.sqlFor as any)?.postgres ?? '') as string;
    // doctor.ts:418 EXEMPT_RE = /^INBRAIN:RLS_EXEMPT\s+reason=\S.{3,}/
    // The plpgsql side must use the same pattern (via ~) so the two surfaces
    // honor identical exemptions.
    expect(sql).toMatch(/'\^INBRAIN:RLS_EXEMPT\\s\+reason=\\S\.\{3,\}'/);
  });
...
// ─────────────────────────────────────────────────────────────────
// resolvePoolSize — INBRAIN_POOL_SIZE env override
// ─────────────────────────────────────────────────────────────────
//
// Guards the Bug 2 fix: users on constrained poolers (Supabase port 6543)
// must be able to cap the pool size via INBRAIN_POOL_SIZE. The default
...
describe('resolvePoolSize — env var + explicit override', () => {
  ...
  const original = process.env.INBRAIN_POOL_SIZE;

  afterAll(() => {
    if (original === undefined) delete process.env.INBRAIN_POOL_SIZE;
    else process.env.INBRAIN_POOL_SIZE = original;
  });

  test('returns 10 default when unset and no explicit override', () => {
    delete process.env.INBRAIN_POOL_SIZE;
    expect(resolvePoolSize()).toBe(10);
  });

  test('reads INBRAIN_POOL_SIZE as an integer', () => {
    process.env.INBRAIN_POOL_SIZE = '2';
    expect(resolvePoolSize()).toBe(2);
    process.env.INBRAIN_POOL_SIZE = '5';
    expect(resolvePoolSize()).toBe(5);
  });

  test('ignores invalid INBRAIN_POOL_SIZE values', () => {
    process.env.INBRAIN_POOL_SIZE = 'not-a-number';
    expect(resolvePoolSize()).toBe(10);
    process.env.INBRAIN_POOL_SIZE = '0';
    expect(resolvePoolSize()).toBe(10);
    process.env.INBRAIN_POOL_SIZE = '-1';
    expect(resolvePoolSize()).toBe(10);
  });
  ...
  test('explicit argument wins over env + default', () => {
    ...
    process.env.INBRAIN_POOL_SIZE = '7';
    expect(resolvePoolSize(3)).toBe(3);
  });
});
...
describe('PR #356 — 57014 catch path emits actionable 4-part diagnostic', () => {
  test('runMigrations surfaces SQLSTATE 57014 with fix + verify steps', async () => {
    ...
    // INBRAIN_MIGRATE_BACKOFF_MS=0 in test env to skip the 5s/15s wait
    ...
    const original = process.env.INBRAIN_MIGRATE_BACKOFF_MS;
    process.env.INBRAIN_MIGRATE_BACKOFF_MS = '0';
    ...
    if (original === undefined) delete process.env.INBRAIN_MIGRATE_BACKOFF_MS;
    else process.env.INBRAIN_MIGRATE_BACKOFF_MS = original;
    ...
    expect(joined).toContain('inbrain doctor --locks');
    expect(joined).toContain('inbrain apply-migrations --yes');
    expect(joined).toContain('Verify:');
    expect(joined).toContain('inbrain doctor');
...
describe('migration v48 — takes_weight_round_to_grid (v0.32)', () => {
  ...
  test('uses transaction:false (codex review #2 — non-blocking, idempotent via WHERE)', () => {
    ...
    // from a long transaction so other inbrain processes can interleave.
    const v48 = MIGRATIONS.find(m => m.version === 48);
...
describe('migration v49 — eval_takes_quality_runs (v0.32)', () => {
  ...
  // DB-authoritative receipts table for `inbrain eval takes-quality`.
...
// resolveSessionTimeouts — INBRAIN_*_TIMEOUT env overrides
// ─────────────────────────────────────────────────────────────────
//
...
describe('resolveSessionTimeouts — env var overrides', () => {
  ...
  const origStatement = process.env.INBRAIN_STATEMENT_TIMEOUT;
  const origIdleTx = process.env.INBRAIN_IDLE_TX_TIMEOUT;
  const origCheck = process.env.INBRAIN_CLIENT_CHECK_INTERVAL;
  ...
  afterAll(() => {
    ...
    restore('INBRAIN_STATEMENT_TIMEOUT', origStatement);
    restore('INBRAIN_IDLE_TX_TIMEOUT', origIdleTx);
    restore('INBRAIN_CLIENT_CHECK_INTERVAL', origCheck);
  });

  const resetEnv = () => {
    delete process.env.INBRAIN_STATEMENT_TIMEOUT;
    delete process.env.INBRAIN_IDLE_TX_TIMEOUT;
    delete process.env.INBRAIN_CLIENT_CHECK_INTERVAL;
  };
  ...
  test('env vars override the defaults', () => {
    resetEnv();
    process.env.INBRAIN_STATEMENT_TIMEOUT = '10min';
    process.env.INBRAIN_IDLE_TX_TIMEOUT = '30s';
    process.env.INBRAIN_CLIENT_CHECK_INTERVAL = '15s';
    const t = resolveSessionTimeouts();
  ...
  test("'0' disables a specific GUC", () => {
    resetEnv();
    process.env.INBRAIN_STATEMENT_TIMEOUT = '0';
    const t = resolveSessionTimeouts();
  ...
  test("'off' disables a specific GUC", () => {
    resetEnv();
    process.env.INBRAIN_IDLE_TX_TIMEOUT = 'off';
    const t = resolveSessionTimeouts();
    expect(t.statement_timeout).toBe('5min');
  ...
  async function tryResolve(
    ...
    try {
      await engine.executeRaw(
        ...
        [quality, outcome, 'inbrain:test', pageId, rowNum],
      );
      return { ok: true };
    } catch (err) {
...
describe('migrate v81 — pages_provenance_columns', () => {
  ...
  test('v81 does NOT create any index (provenance is admin-surface only)', () => {
    ...
    // surface only (admin SPA Sources tab + inbrain doctor
...
// rows. The migration is the substrate behind v0.40.2.0's `inbrain think`
