 ...
 *   4. The legacy `CONCURRENCY = parseInt(process.env.INBRAIN_EMBED_CONCURRENCY...)`
...
describe('embed.ts → worker-pool migration (T3)', () => {
  ...
  test('preserves INBRAIN_EMBED_CONCURRENCY default of 20 (codex #13)', () => {
    ...
    expect(EMBED_SOURCE).toMatch(
      /parseInt\(process\.env\.INBRAIN_EMBED_CONCURRENCY\s*\|\|\s*['"]20['"]/,
    );
  });
