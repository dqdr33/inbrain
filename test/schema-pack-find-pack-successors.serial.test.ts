...
// semver descending compare, transitive walking (inbrain-base@1.x →
// inbrain-base-v2 via bundled packs).
...
describe('findPackSuccessors (against bundled packs)', () => {
  it('finds inbrain-base-v2 as successor of inbrain-base@1.0.0', async () => {
    const successors = await findPackSuccessors('inbrain-base', '1.0.0');
    expect(successors.length).toBe(1);
    expect(successors[0].manifest.name).toBe('inbrain-base-v2');
    expect(successors[0].manifest.migration_from?.pack).toBe('inbrain-base');
    expect(successors[0].manifest.migration_from?.version).toBe('1.x');
  });

  it('returns empty array when no successor declared', async () => {
    // inbrain-base-v2 itself has no successor declared
    const successors = await findPackSuccessors('inbrain-base-v2', '1.0.0');
