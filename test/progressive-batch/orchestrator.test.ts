 ...
 * `INBRAIN_AUDIT_DIR=<tempdir>` to isolate from the user's real audit
...
// Use a per-test tempdir for audit writes via INBRAIN_AUDIT_DIR.
function makeAuditEnv(): Record<string, string> {
  return {
    INBRAIN_AUDIT_DIR: mkdtempSync(join(tmpdir(), 'pb-audit-')),
    INBRAIN_PROGRESSIVE_BATCH_AUTO: '1',
    INBRAIN_PROGRESSIVE_BATCH_DISABLED: undefined as unknown as string,
    INBRAIN_PROGRESSIVE_BATCH_STAGES: undefined as unknown as string,
  };
}
...
describe('resolveStages', () => {
  test('env override wins', async () => {
    await withEnv({ INBRAIN_PROGRESSIVE_BATCH_STAGES: '5,50,500' }, async () => {
  ...
  test('policy when env unset', async () => {
    await withEnv({ INBRAIN_PROGRESSIVE_BATCH_STAGES: undefined as unknown as string }, async () => {
...
describe('runProgressiveBatch — D21 honest behavior preservation', () => {
  test('INBRAIN_PROGRESSIVE_BATCH_DISABLED=1 skips ramp; goes to full', async () => {
    await withEnv(
      {
        ...makeAuditEnv(),
        INBRAIN_PROGRESSIVE_BATCH_DISABLED: '1',
      },
      async () => {
        const { onStageReport, reports } = collectReports();
        const result = await runProgressiveBatch(
