...
let provider: InbrainOAuthProvider;
...
beforeAll(async () => {
  ...
  provider = new InbrainOAuthProvider({ sql, tokenTtl: 60, refreshTtl: 300 });
...
describe('generateToken', () => {
  test('produces prefixed random hex', () => {
    const token = generateToken('inbrain_cl_');
    expect(token).toStartWith('inbrain_cl_');
    expect(token).toHaveLength('inbrain_cl_'.length + 64); // 32 bytes = 64 hex chars
  });
...
describe('client registration', () => {
  test('registerClientManual creates a client', async () => {
    ...
    expect(clientId).toStartWith('inbrain_cl_');
    expect(clientSecret).toStartWith('inbrain_cs_');
...
describe('client credentials', () => {
  ...
  test('valid exchange returns access token', async () => {
    ...
    expect(tokens.access_token).toStartWith('inbrain_at_');
...
describe('verifyAccessToken', () => {
  ...
  test('expired token is rejected', async () => {
    ...
    const expiredToken = generateToken('inbrain_at_');
  ...
  test('NULL expires_at is treated as expired (fail-closed)', async () => {
    ...
    const nullExpiryToken = generateToken('inbrain_at_');
  ...
  test('legacy access_tokens fallback works', async () => {
    ...
    const legacyToken = generateToken('inbrain_');
...
describe('authorization code flow', () => {
  test('code issuance and exchange', async () => {
    ...
    expect(redirectUrl).toContain('code=inbrain_code_');
  ...
  test('expired code is rejected', async () => {
    ...
    const expiredCode = generateToken('inbrain_code_');
...
describe('redirect_uri validation (DCR)', () => {
  test('http://localhost is allowed (loopback exception)', async () => {
    ...
    expect(result.client_id).toStartWith('inbrain_cl_');
  });
...
describe('F2/F3 refresh hardening', () => {
  test('wrong client cannot burn another client refresh token', async () => {
    ...
    expect(rotated.access_token).toStartWith('inbrain_at_');
  ...
  // implies it. gstack /setup-inbrain Path 4 needs this to work.
...
describe('F5 verifyAccessToken / client_credentials column probes', () => {
  test('non-schema SQL failures are not swallowed by client credentials soft-delete probe', async () => {
    ...
    const failingProvider = new InbrainOAuthProvider({ sql: fakeSql as any });

    await expect(
      failingProvider.exchangeClientCredentials('inbrain_cl_fake', 'secret', 'read'),
...
describe('F12 dcrDisabled constructor option', () => {
  test('clientsStore omits registerClient when dcrDisabled=true', () => {
    const dcrOff = new InbrainOAuthProvider({ sql, dcrDisabled: true });
  ...
  test('clientsStore exposes registerClient when dcrDisabled is false/unset', () => {
    const dcrOn = new InbrainOAuthProvider({ sql });
  ...
  test('registerClientManual still works on dcrDisabled providers (CLI path)', async () => {
    ...
    expect(result.clientId).toStartWith('inbrain_cl_');
    expect(result.clientSecret).toStartWith('inbrain_cs_');
  });
});
...
describe('PKCE DCR public-client gate (#909)', () => {
  ...
  test('default auth_method (omitted) still issues a client_secret', async () => {
    ...
    expect(result.client_id).toStartWith('inbrain_cl_');
    expect(result.client_secret).toStartWith('inbrain_cs_');
  });
  ...
  test('PKCE flow end-to-end: public client /authorize then /token, no secret needed', async () => {
    ...
    const code = new URL(redirectUrl).searchParams.get('code')!;
    expect(code).toMatch(/^inbrain_code_/);
...
describe('v0.41.3 DCR validator (T5)', () => {
  ...
  test('DCR accepts "none" → public PKCE client', async () => {
    ...
    expect(reg.client_id).toStartWith('inbrain_cl_');
  ...
  test('DCR accepts "client_secret_basic" — codex F3 regression', async () => {
    ...
    expect(reg.client_id).toStartWith('inbrain_cl_');
    expect(reg.client_secret).toStartWith('inbrain_cs_');
  });
});
