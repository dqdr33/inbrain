 ...
 * (`inbrain-cycle:<source_id>`) would by themselves let two PGLite cycles
 * for different sources run concurrently — which would corrupt the
 * single-writer invariant.
 *
 * Defense: cycle.ts acquires the GLOBAL file lock (`~/.inbrain/cycle.lock`,
...
beforeAll(async () => {
  // INBRAIN_HOME isolation so the file lock at ~/.inbrain/cycle.lock doesn't
  // collide with the dev's real inbrain. resetPgliteState would wipe the
  // config table so we don't use it here.
  inbrainHome = mkdtempSync(join(tmpdir(), 'inbrain-pglite-lock-'));
  process.env.INBRAIN_HOME = inbrainHome;
...
beforeEach(async () => {
  await engine.executeRaw('DELETE FROM inbrain_cycle_locks').catch(() => {});
  ...
  const lockPath = join(inbrainHome, '.inbrain', 'cycle.lock');
  if (existsSync(lockPath)) {
    try { unlinkSync(lockPath); } catch { /* best-effort */ }
  }
  brainDir = mkdtempSync(join(tmpdir(), 'inbrain-cycle-pglite-ord-'));
});
...
describe('PGLite cycle: file lock + per-source DB lock ordering', () => {
  test('global file lock acquired during PGLite cycle (codex P0-C invariant)', async () => {
    ...
    await runCycle(engine, {
      ...
      yieldBetweenPhases: async () => {
        ...
        if (!lockFileExisted) {
          lockFileExisted = existsSync(join(inbrainHome, '.inbrain', 'cycle.lock'));
        }
      },
    });
    expect(lockFileExisted).toBe(true);
    // Lock file released on exit
    expect(existsSync(join(inbrainHome, '.inbrain', 'cycle.lock'))).toBe(false);
  });
  ...
  test('two PGLite cycles for DIFFERENT sources serialize (P0-D regression)', async () => {
    ...
    writeFileSync(
      join(inbrainHome, '.inbrain', 'cycle.lock'),
  ...
  test('file-lock release-on-failure: if DB acquire fails, file lock is released', async () => {
    ...
    // state.
    await seed('gamma');
    const lockId = 'inbrain-cycle:gamma';
    await engine.executeRaw(
    ...
    await runCycle(engine, {
      ...
      yieldBetweenPhases: async () => {
        ...
        const rows = await engine.executeRaw<{ id: string }>(
          `SELECT id FROM inbrain_cycle_locks WHERE id LIKE 'inbrain-cycle%'`,
        );
        if (rows.length > 0) dbLockRowSeen = rows[0];
      },
    });
    expect(dbLockRowSeen).not.toBeNull();
    expect(dbLockRowSeen!.id).toBe('inbrain-cycle:epsilon');
  });
  ...
  test('subsequent cycle after clean exit can acquire both locks', async () => {
    ...
    const dbRows = await engine.executeRaw<{ id: string }>(
      `SELECT id FROM inbrain_cycle_locks WHERE id = 'inbrain-cycle:zeta'`,
    );
