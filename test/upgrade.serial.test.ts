...
describe('detectInstallMethod heuristic (source analysis)', () => {
  ...
  test('checks node_modules before binary', () => {
    ...
    const binaryIdx = source.indexOf("endsWith('/inbrain')");
  ...
  test('bun-link signal walks .git/config for inbrain-ai/inbrain match', () => {
    expect(source).toContain('function detectBunLink');
    expect(source).toContain('INBRAIN_GITHUB_REPO');
  ...
  test('bun global upgrade passes cwd to bun update', () => {
    ...
    expect(source).toContain("execFileSync('bun', ['update', 'inbrain'], { cwd: bunGlobalRoot");
  });
...
describe('resolveBunGlobalRoot', () => {
  ...
  test('uses canonical ~/.bun/install/global when present', () => {
    const home = mkdtempSync(join(tmpdir(), 'inbrain-upgrade-home-'));
    try {
  ...
  test('falls back to the package root above node_modules/inbrain', () => {
    const home = mkdtempSync(join(tmpdir(), 'inbrain-upgrade-home-'));
    const globalRoot = mkdtempSync(join(tmpdir(), 'inbrain-upgrade-global-'));
    try {
      delete process.env.BUN_INSTALL;
      process.env.HOME = home;
      const cliPath = join(globalRoot, 'node_modules', 'inbrain', 'src', 'cli.ts');
...
describe('post-upgrade behavior (post v0.12.0 merge)', () => {
  ...
  test('--help prints usage', async () => {
    ...
    const exitCode = await proc.exited;
    expect(exitCode).toBe(0);
    expect(stdout).toContain('Usage: inbrain post-upgrade');
  });
});
