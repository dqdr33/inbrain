 ...
 * class where `inbrain sync` without --source silently routed to source_id
...
describe('#1434 — sole_non_default tier', () => {
  ...
  test('does NOT fire when INBRAIN_SOURCE env is set (tier 2 wins)', async () => {
    const engine = makeStub([
      { id: 'default', local_path: null },
      { id: 'studiovault', local_path: '/Users/india/vault' },
    ]);
    await withEnv({ INBRAIN_SOURCE: 'default' }, async () => {
      const result = await resolveSourceWithTier(engine, null, '/tmp');
      expect(result.source_id).toBe('default');
...
describe('formatSoleNonDefaultNudge', () => {
  test('returns canonical nudge string in default env', async () => {
    await withEnv({ INBRAIN_NO_SOLE_NON_DEFAULT_NUDGE: undefined }, async () => {
      expect(formatSoleNonDefaultNudge('studiovault')).toBe(
        "[inbrain] routing to source 'studiovault' (sole non-default source registered; pass --source to override).",
      );
    });
  });

  test('returns null when INBRAIN_NO_SOLE_NON_DEFAULT_NUDGE=1 suppresses', async () => {
    await withEnv({ INBRAIN_NO_SOLE_NON_DEFAULT_NUDGE: '1' }, async () => {
      expect(formatSoleNonDefaultNudge('studiovault')).toBeNull();
    });
  });

  test('any value other than literal "1" does NOT suppress', async () => {
    await withEnv({ INBRAIN_NO_SOLE_NON_DEFAULT_NUDGE: 'true' }, async () => {
