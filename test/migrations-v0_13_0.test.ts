 ...
 * handlers must shell out to the bare string `inbrain`, NOT to
...
describe('v0.13.0 — Frontmatter relationship indexing migration', () => {
  ...
  test('source does NOT build commands from a INBRAIN constant (Bug 1 regression)', () => {
    // Earlier revisions used `const INBRAIN = process.execPath` and built
    // commands as `${INBRAIN} extract ...`. The constant was the vector.
    const src = readFileSync(SRC_PATH, 'utf-8');
    expect(src).not.toMatch(/const\s+INBRAIN\s*=/);
    expect(src).not.toMatch(/\$\{INBRAIN\}/);
  });

  test('phases use in-process schema + bare `inbrain` subprocess (v0.41.37.0 #1605)', () => {
    const src = readFileSync(SRC_PATH, 'utf-8');
    // Schema bring-up is now IN-PROCESS (was execSync('inbrain init
    // --migrate-only'), which died with getaddrinfo ENOTFOUND on Windows).
    expect(src).toContain('runMigrateOnlyCore()');
    expect(src).not.toContain("execSync('inbrain init --migrate-only'");
    // Backfill extract goes through the stderr-capturing wrapper (still bare
    // `inbrain` so the canonical shim on PATH wins).
    expect(src).toContain("runGbrainSubprocess('inbrain extract links --source db --include-frontmatter'");
    // Stats readback still shells out (reads stdout); bare inbrain.
    expect(src).toContain("execSync('inbrain call get_stats'");
  });
