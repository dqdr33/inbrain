...
beforeEach(() => {
  tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'inbrain-skillpack-load-'));
});
...
function makePluginDir(opts: {
  ...
  const manifest: Record<string, unknown> = {
    ...
    version: '1.0.0',
    plugin_version: opts.pluginVersion ?? 'inbrain-plugin-v1',
  };
  if (opts.sources && opts.sources.length > 0) {
    manifest.ingestion_sources = opts.sources.map((s) => ({
      kind: s.kind,
      module: s.module ?? './source.js',
      api_version: s.api_version ?? 'inbrain-ingestion-source-v1',
      default_config: s.default_config,
      permissions: s.permissions,
    }));
  }
  fs.writeFileSync(path.join(dir, 'inbrain.plugin.json'), JSON.stringify(manifest));
  if (opts.sources && opts.sources.length > 0) {
...
describe('loadSkillpackSources — discovery', () => {
  test('empty INBRAIN_PLUGIN_PATH returns empty result', async () => {
  ...
  test('rejectIfNotAbsolute helper rejects remote URLs (defense-in-depth)', () => {
    // INBRAIN_PLUGIN_PATH is colon-separated paths ($PATH style), so URL
  ...
  test('directory with no inbrain.plugin.json is silently skipped (not a warning)', async () => {
...
describe('loadSkillpackSources — manifest validation', () => {
  ...
  test('rejects missing name field', async () => {
    ...
    fs.writeFileSync(
      path.join(dir, 'inbrain.plugin.json'),
      JSON.stringify({
        plugin_version: 'inbrain-plugin-v1',
        ingestion_sources: [{ kind: 'x', module: './source.js', api_version: 'inbrain-ingestion-source-v1' }],
      }),
    );
    const result = await loadSkillpackSources({ envPath: dir });
...
describe('loadSkillpackSources — source declaration validation', () => {
  test('rejects missing kind field', async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: 'p',
        plugin_version: 'inbrain-plugin-v1',
        ingestion_sources: [{ module: './source.js', api_version: 'inbrain-ingestion-source-v1' }],
      }),
    );
    const result = await loadSkillpackSources({ envPath: dir });
  ...
  test('rejects array-shaped default_config', async () => {
    const err = __testing.validateDeclaration({
      kind: 'x',
      module: './source.js',
      api_version: 'inbrain-ingestion-source-v1',
...
describe('loadSkillpackSources — api_version compatibility', () => {
  ...
  test('api_version mismatch fails loudly with upgrade hint', async () => {
    const dir = makePluginDir({
      sources: [{ kind: 'stub', api_version: 'inbrain-ingestion-source-v99' }],
    });
    const result = await loadSkillpackSources({
      envPath: dir,
      _import: async () => ({ default: makeFactoryFn() }),
...
describe('loadSkillpackSources — env var path', () => {
  test('reads INBRAIN_PLUGIN_PATH from process.env when envPath is not passed', async () => {
    const dir = makePluginDir({ sources: [{ kind: 'env-stub' }] });
    await withEnv({ INBRAIN_PLUGIN_PATH: dir }, async () => {
      const result = await loadSkillpackSources({
        _import: async () => ({ default: makeFactoryFn() }),
      });
      expect(result.sources).toHaveLength(1);
