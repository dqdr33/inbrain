...
describe('skillify-check CLI', () => {
  test('text mode runs against a known-skilled file', () => {
    // publish is one of the inbrain commands with SKILL.md + tests +
...
function runWithPath(opts: { path: string }): { stdout: string; stderr: string } {
  ...
  // PATH is what skillify-check's own `spawnSync('inbrain', ...)` will search.
...
describe('skillify-check ↔ inbrain check-resolvable wiring', () => {
  ...
  test('loud failure when inbrain binary is not on PATH (no silent pass)', () => {
    const emptyPath = mkdtempSync(join(tmpdir(), 'no-inbrain-'));
    created.push(emptyPath);
    const r = runWithPath({ path: emptyPath });
    // Loud warning must appear on stderr.
    expect(r.stderr).toContain('inbrain check-resolvable not runnable');
  ...
  test('happy path: synthetic inbrain on PATH returns ok=true, gate passes', () => {
    const fakeBinDir = mkdtempSync(join(tmpdir(), 'fake-inbrain-'));
    created.push(fakeBinDir);
    const fakeBin = join(fakeBinDir, 'inbrain');
    writeFileSync(
      fakeBin,
      `#!/bin/sh
cat <<'JSON'
...
`,
    ...
    expect(r.stderr).not.toContain('inbrain check-resolvable not runnable');
    const parsed = JSON.parse(r.stdout);
    const gate = parsed[0].items.find((i: any) => i.name === 'check-resolvable gate');
    expect(gate).toBeDefined();
