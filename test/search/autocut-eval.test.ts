 ...
 * sibling inbrain-evals repo and without any API key. It measures the EXACT
 ...
 * inbrain-evals PrecisionMemBench run. What this gate DOES guarantee, in CI:
...
describe('autocut eval gate (D2 — precision lift without recall regression)', () => {
  ...
  const LIFT_FLOOR = envFloor('INBRAIN_AUTOCUT_EVAL_PRECISION_LIFT_FLOOR', 0.15);
  const RECALL_FLOOR = envFloor('INBRAIN_AUTOCUT_EVAL_RECALL_FLOOR', 0.9);
  const RECALL_REGRESSION_TOLERANCE = envFloor('INBRAIN_AUTOCUT_EVAL_RECALL_TOLERANCE', 0.1);
