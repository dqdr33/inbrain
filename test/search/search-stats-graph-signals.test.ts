/**
 * v0.40.4.0 — `inbrain search stats` graph_signals section.
 *
 * Pins:
 ...
 *
 * Hermetic via PGLite + withEnv for INBRAIN_AUDIT_DIR.
 */
...
// withEnv-wrapped INBRAIN_AUDIT_DIR is honored by the audit writer's
...
function mkTmp(): string {
  const d = fs.mkdtempSync(path.join(os.tmpdir(), 'inbrain-graph-stats-test-'));
  tmpDirs.push(d);
  return d;
}
