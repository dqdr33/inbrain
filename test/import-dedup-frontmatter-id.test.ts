 ...
 *
 * Bug class (infiniteGameExp): `inbrain import /vault/Subdir/` then
 * `inbrain import /vault/` re-ingested the same files under different
...
beforeEach(async () => {
  ...
  tmpRoot = mkdtempSync(join(tmpdir(), 'inbrain-dedup-fm-'));
});
...
describe('#1309 — overlapping-ingest-roots dedup', () => {
  ...
  test('different frontmatter.id (and therefore different content_hash) imports both, no dedup signal', async () => {
    // inbrain's content_hash includes the frontmatter (minus captured_at /
