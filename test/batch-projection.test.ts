...
describe('projectBatch', () => {
  ...
  test('raise_cap_hint fires when lease is binding AND a 4x raise meaningfully helps', () => {
    ...
    expect(p.raise_cap_hint).toContain('INBRAIN_ANTHROPIC_MAX_INFLIGHT');
  });
...
describe('formatProjection', () => {
  ...
  test('raise-cap hint surfaces inline', () => {
    const s = formatProjection({
      ...
      effective_concurrency: 8,
      cold_start: false,
      raise_cap_hint: 'raise INBRAIN_ANTHROPIC_MAX_INFLIGHT to 32 to finish in ~3min',
    });
    expect(s).toContain('raise INBRAIN_ANTHROPIC_MAX_INFLIGHT to 32');
  });
});
...
describe('shouldPromptAtThreshold', () => {
  ...
  test('env var INBRAIN_BATCH_PROMPT_THRESHOLD_USD lowers the prompt floor', async () => {
    await withEnv({ INBRAIN_BATCH_PROMPT_THRESHOLD_USD: '1' }, async () => {
      expect(
        shouldPromptAtThreshold({
          total_duration_ms: 60_000,
          total_cost_usd: 1.50,
