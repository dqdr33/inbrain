 ...
 * the readback path used by `inbrain agent logs`. No real engine; purely
 * filesystem.
 */
...
let tmpDir: string;
const savedAuditDir = process.env.INBRAIN_AUDIT_DIR;
...
afterAll(() => {
  if (savedAuditDir === undefined) delete process.env.INBRAIN_AUDIT_DIR;
  else process.env.INBRAIN_AUDIT_DIR = savedAuditDir;
...
describe('logSubagentHeartbeat', () => {
  ...
  test('best-effort: write failure does not throw', () => {
    ...
    process.env.INBRAIN_AUDIT_DIR = bogus;
    try {
