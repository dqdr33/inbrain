 ...
 * surface behind INBRAIN_HTTP_CORS_ORIGIN with default-deny.
 *
...
describe('parseCorsAllowlistOAuth', () => {
  test('unset → null (default-deny posture)', async () => {
    await withEnv({ INBRAIN_HTTP_CORS_ORIGIN: undefined }, async () => {
      expect(parseCorsAllowlistOAuth()).toBeNull();
    });
  });

  test('empty string → null', async () => {
    await withEnv({ INBRAIN_HTTP_CORS_ORIGIN: '' }, async () => {
      expect(parseCorsAllowlistOAuth()).toBeNull();
    });
  });
  ...
  test('single origin → Set of one', async () => {
    await withEnv({ INBRAIN_HTTP_CORS_ORIGIN: 'https://claude.ai' }, async () => {
  ...
  test('comma-separated origins → Set of N', async () => {
    await withEnv({ INBRAIN_HTTP_CORS_ORIGIN: 'https://claude.ai,https://chatgpt.com,https://my.app' }, async () => {
      const set = parseCorsAllowlistOAuth();
  ...
  test('whitespace around values is trimmed', async () => {
    await withEnv({ INBRAIN_HTTP_CORS_ORIGIN: ' https://a.app , https://b.app ' }, async () => {
      const set = parseCorsAllowlistOAuth();
