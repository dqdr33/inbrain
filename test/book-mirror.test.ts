...
async function runCli(args: string[]): Promise<{
  ...
  // v0.34.1 (#876): set INBRAIN_HOME to a fresh tempdir so the test isn't
  // sensitive to the developer's local ~/.inbrain/config.json. Pre-fix this
  ...
  // depending on the local DB state. With INBRAIN_HOME pointing at an
  ...
  const tmpHome = await import('os').then(os =>
    import('fs').then(fs => fs.mkdtempSync(`${os.tmpdir()}/inbrain-test-`)),
  );
  try {
    const proc = Bun.spawn(
      ['bun', 'run', 'src/cli.ts', 'book-mirror', ...args],
      {
        ...
        stdout: 'pipe',
        stderr: 'pipe',
        env: { ...process.env, DATABASE_URL: '', INBRAIN_HOME: tmpHome },
      },
    );
...
describe('inbrain book-mirror — CLI registration', () => {
...
describe('inbrain book-mirror — source file invariants', () => {
