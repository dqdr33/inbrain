 ...
 *   - --yes override skips
 *   - INBRAIN_NO_PROBE_PROMPT=1 env var skips
...
describe('maybePromptForCostBeforeProbe', () => {
  ...
  test('INBRAIN_NO_PROBE_PROMPT=1 skips entirely', async () => {
    await withEnv({ INBRAIN_NO_PROBE_PROMPT: '1' }, async () => {
  ...
  test('INBRAIN_PROBE_PROMPT_GRACE_SECONDS env overrides the 10s default', async () => {
    await withEnv({ INBRAIN_PROBE_PROMPT_GRACE_SECONDS: '0' }, async () => {
      let waitedSeconds = -1;
      const report: ProbeReport = mkSeedReport('1');
      await writeRunRow(engine, report, 100);
