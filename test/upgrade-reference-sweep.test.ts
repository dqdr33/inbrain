 ...
 * after `inbrain upgrade` so an operator/agent doesn't have to manually
 * run `inbrain skillpack reference --all`.
 *
 * Pins:
 *   - INBRAIN_SKIP_REFERENCE_SWEEP=1 short-circuits silently
 *   - no detected workspace → silent no-op
 *   - workspace == inbrain repo (dev mode) → silent no-op
 ...
 * spawning a full `inbrain upgrade` subprocess. We swap the cwd via
...
const INBRAIN_ROOT = process.cwd(); // tests run from inbrain repo root

describe('postUpgradeReferenceSweep', () => {
  it('INBRAIN_SKIP_REFERENCE_SWEEP=1 short-circuits silently', async () => {
    await withEnv({ INBRAIN_SKIP_REFERENCE_SWEEP: '1' }, async () => {
      const ws = scratchHostWithSkill('book-mirror', { drift: true });
      await postUpgradeReferenceSweep({ inbrainRoot: INBRAIN_ROOT, targetWorkspace: ws });
      expect(logs.join('\n')).toBe('');
    });
  });

  it('zero drift (skills present but identical) → silent', async () => {
    await withEnv({ INBRAIN_SKIP_REFERENCE_SWEEP: undefined }, async () => {
  ...
  it('drift detected → prints header + per-skill summary + footer hints', async () => {
    await withEnv({ INBRAIN_SKIP_REFERENCE_SWEEP: undefined }, async () => {
      ...
      expect(out).toContain('inbrain skillpack reference <slug>');
      expect(out).toContain('_AGENT_README.md');
      expect(out).toContain('INBRAIN_SKIP_REFERENCE_SWEEP');
    });
  });

  it('dev-mode guard: workspace IS inbrain → silent', async () => {
    await withEnv({ INBRAIN_SKIP_REFERENCE_SWEEP: undefined }, async () => {
      // Pass inbrain repo as both inbrainRoot AND targetWorkspace.
      await postUpgradeReferenceSweep({
        inbrainRoot: INBRAIN_ROOT,
        targetWorkspace: INBRAIN_ROOT,
      });
      expect(logs.join('\n')).not.toContain('Skillpack reference sweep');
    });
  });
  ...
  it('errors swallowed silently — never blocks post-upgrade', async () => {
    ...
    await withEnv({ INBRAIN_SKIP_REFERENCE_SWEEP: undefined }, async () => {
      await postUpgradeReferenceSweep({
        inbrainRoot: '/dev/null/no-bundle-here',
