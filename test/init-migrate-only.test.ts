/**
 * Tests for `inbrain init --migrate-only` — the schema-only primitive used by
 ...
 * missing config must fail loudly with a clear "run inbrain init first" error.
 *
...
function run(args: string[]): { exitCode: number; stdout: string; stderr: string } {
  // Strip DATABASE_URL / INBRAIN_DATABASE_URL from the subprocess env. The
...
beforeEach(() => {
  ...
  tmp = mkdtempSync(join(tmpdir(), 'inbrain-init-migrate-only-test-'));
});
...
describe('inbrain init --migrate-only — error paths', () => {
  test('errors with clear message when no config exists', () => {
    ...
    expect(existsSync(join(tmp, '.inbrain', 'config.json'))).toBe(false);
  });
...
describe('inbrain init --migrate-only — happy path with PGLite config', () => {
  test('applies schema against existing PGLite config; does NOT modify config.json', () => {
    ...
    const dbPath = join(inbrainDir, 'brain.pglite');
    const configPath = join(inbrainDir, 'config.json');
