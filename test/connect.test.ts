...
describe('normalizeMcpUrl', () => {
  ...
  test('a non-/mcp base path errors and suggests the full URL', () => {
    const r = normalizeMcpUrl('https://brain.example.com/inbrain');
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error).toMatch(/\/inbrain\/mcp/);
  });
...
describe('validateToken', () => {
  test('accepts a normal token', () => {
    expect(validateToken('inbrain_abc123').ok).toBe(true);
  });
  test('rejects empty', () => {
...
describe('resolveToken', () => {
  ...
  test('install mode without token errors with a inbrain auth create hint', () => {
    const r = resolveToken({ tokenFlag: null, env: null, mode: 'install' });
    expect(r.kind).toBe('error');
    if (r.kind === 'error') {
      expect(r.error).toMatch(/inbrain auth create/);
...
describe('isValidName', () => {
  test('accepts conservative identifiers', () => {
    expect(isValidName('inbrain')).toBe(true);
...
describe('argv + command string', () => {
  test('claude argv shape', () => {
    expect(buildClaudeMcpAddArgv({ name: 'inbrain', url: 'https://h/mcp', headerToken: 'TOK' })).toEqual([
      'mcp', 'add', 'inbrain', '-t', 'http', 'https://h/mcp', '-H', 'Authorization: Bearer TOK',
    ]);
  });
  test('codex argv shape — env-var bearer, no token in argv', () => {
    expect(buildCodexMcpAddArgv({ name: 'inbrain', url: 'https://h/mcp', envVar: ENV_VAR })).toEqual([
      'mcp', 'add', 'inbrain', '--url', 'https://h/mcp', '--bearer-token-env-var', ENV_VAR,
    ]);
  });
  test('command string single-quotes the header (paste-safe)', () => {
    const cmd = cmdString('claude', buildClaudeMcpAddArgv({ name: 'inbrain', url: 'https://h/mcp', headerToken: 'TOK' }));
    expect(cmd).toBe("claude mcp add inbrain -t http https://h/mcp -H 'Authorization: Bearer TOK'");
  });
  test('a token with shell metacharacters cannot trigger command substitution on paste', () => {
    const cmd = cmdString('claude', buildClaudeMcpAddArgv({ name: 'inbrain', url: 'https://h/mcp', headerToken: 'inbrain_$(touch /tmp/pwned)`x`' }));
    // Single-quoted → the $() and backticks are inert literals, not double-quoted.
    expect(cmd).toContain("'Authorization: Bearer inbrain_$(touch /tmp/pwned)`x`'");
...
describe('redactToken', () => {
  ...
  test('null token still scrubs Bearer-shaped values (defense in depth)', () => {
    ...
    expect(redactToken('failed: Bearer inbrain_xyz123', null)).toBe(`failed: Bearer ${REDACTED}`);
  });
...
describe('buildConnectBlock', () => {
  test('claude-code with a literal token inlines it + learn instruction', () => {
    const block = buildConnectBlock({ agent: 'claude-code', name: 'inbrain', url: 'https://h/mcp', token: 'TOK' });
    expect(block).toContain("claude mcp add inbrain -t http https://h/mcp -H 'Authorization: Bearer TOK'");
  ...
  test('claude-code without a token emits a placeholder + replace hint', () => {
    const block = buildConnectBlock({ agent: 'claude-code', name: 'inbrain', url: 'https://h/mcp', token: null });
    expect(block).toContain(PLACEHOLDER_TOKEN);
    expect(block).toMatch(/inbrain auth create/);
  });
  test('generic agent emits URL + header lines, no claude command', () => {
    const block = buildConnectBlock({ agent: 'generic', name: 'inbrain', url: 'https://h/mcp', token: 'TOK' });
  ...
  test('codex emits the codex command + env-var export, token only in export', () => {
    const block = buildConnectBlock({ agent: 'codex', name: 'inbrain', url: 'https://h/mcp', token: 'TOK' });
    expect(block).toContain('codex mcp add inbrain --url https://h/mcp --bearer-token-env-var INBRAIN_REMOTE_TOKEN');
    expect(block).toContain('export INBRAIN_REMOTE_TOKEN=TOK');
    // the codex command itself must not carry the token
    expect(block).toMatch(/codex mcp add[^\n]*$/m);
    expect(block).toContain(LEARN_INSTRUCTION);
    expect(block).toMatch(/reads the token from \$INBRAIN_REMOTE_TOKEN/);
  });
  test('codex single-quotes a metachar token in the export line', () => {
    const block = buildConnectBlock({ agent: 'codex', name: 'inbrain', url: 'https://h/mcp', token: 'inbrain_$(x)`y`' });
    expect(block).toContain("export INBRAIN_REMOTE_TOKEN='inbrain_$(x)`y`'");
  });
  test('perplexity emits GUI connector steps with URL + token, no CLI command', () => {
    const block = buildConnectBlock({ agent: 'perplexity', name: 'inbrain', url: 'https://h/mcp', token: 'TOK' });
...
describe('buildJson', () => {
  test('redacts the token by default; claude has a command', () => {
    const j = buildJson({ url: 'https://h/mcp', name: 'inbrain', agent: 'claude-code', token: 'SeKrEt9', showToken: false });
  ...
  test('--show-token reveals the literal token', () => {
    const j = buildJson({ url: 'https://h/mcp', name: 'inbrain', agent: 'claude-code', token: 'SeKrEt9', showToken: true });
  ...
  test('no token → placeholder, token_present false', () => {
    const j = buildJson({ url: 'https://h/mcp', name: 'inbrain', agent: 'claude-code', token: null, showToken: false });
  ...
  test('codex command carries the env-var name, never the token (even with --show-token)', () => {
    const j = buildJson({ url: 'https://h/mcp', name: 'inbrain', agent: 'codex', token: 'SeKrEt9', showToken: true });
    expect(j.command).toContain('--bearer-token-env-var INBRAIN_REMOTE_TOKEN');
  ...
  test('perplexity has no runnable command', () => {
    const j = buildJson({ url: 'https://h/mcp', name: 'inbrain', agent: 'perplexity', token: 'TOK', showToken: false });
...
describe('OAuth helpers', () => {
  ...
  test('perplexity oauth block: issuer + client id/secret, no bearer header', () => {
    const block = buildConnectBlock({
      agent: 'perplexity', name: 'inbrain', url: 'https://h/mcp', token: null,
      oauth: { issuer: 'https://h', clientId: 'inbrain_cl_x', clientSecret: 'inbrain_cs_y' },
    });
    expect(block).toMatch(/Settings.+Connectors/);
    expect(block).toContain('Issuer URL:    https://h');
    expect(block).toContain('Client ID:     inbrain_cl_x');
    expect(block).toContain('Client Secret: inbrain_cs_y');
  ...
  test('generic oauth block emits the OAuth fields', () => {
    const block = buildConnectBlock({
      agent: 'generic', name: 'inbrain', url: 'https://h/mcp', token: null,
      oauth: { issuer: 'https://h', clientId: 'inbrain_cl_x', clientSecret: 'inbrain_cs_y' },
    });
    expect(block).toContain('Issuer URL:    https://h');
    expect(block).toContain('Client ID:     inbrain_cl_x');
  ...
  test('oauth block placeholders a missing secret', () => {
    const block = buildConnectBlock({
      agent: 'perplexity', name: 'inbrain', url: 'https://h/mcp', token: null,
      oauth: { issuer: 'https://h', clientId: 'inbrain_cl_x', clientSecret: null },
    });
  ...
  test('buildJson oauth: redacts the secret by default, exposes issuer + scopes', () => {
    const j = buildJson({
      url: 'https://h/mcp', name: 'inbrain', agent: 'perplexity', token: null, showToken: false,
      oauth: { issuer: 'https://h', clientId: 'inbrain_cl_x', clientSecret: 'SeKrEt9' }, scopes: 'read',
    });
    expect(j.auth).toBe('oauth');
    expect(j.issuer_url).toBe('https://h');
    expect(j.client_id).toBe('inbrain_cl_x');
  ...
  test('buildJson oauth --show-token reveals the secret', () => {
    const j = buildJson({
      url: 'https://h/mcp', name: 'inbrain', agent: 'perplexity', token: null, showToken: true,
      oauth: { issuer: 'https://h', clientId: 'inbrain_cl_x', clientSecret: 'SeKrEt9' },
    });
...
function installDeps(over: Partial<ConnectDeps> = {}): ConnectDeps {
  return {
    ...
    env: () => undefined, // tests control the env; real INBRAIN_REMOTE_TOKEN must not leak in
    registerOAuthClient: () => ({ ok: true, clientId: 'inbrain_cl_minted', clientSecret: 'inbrain_cs_minted' }),
    ...over,
  };
}
...
describe('runConnect --install', () => {
  test('happy path: adds server, verifies, prints learn instruction', async () => {
    const r = await runWithExitCapture(
      ['https://brain.example.com/mcp', '--token', 'inbrain_tok', '--install', '--yes'],
      installDeps(),
    );
    expect(r.exitCode).toBeUndefined();
    expect(r.err.join('\n')).toMatch(/Added MCP server 'inbrain'/);
    expect(r.err.join('\n')).toMatch(/Verified/);
  ...
  test('probe failure warns + exits 1 + never echoes the token', async () => {
    const r = await runWithExitCapture(
      ['https://brain.example.com/mcp', '--token', 'inbrain_secret', '--install', '--yes'],
      installDeps({ probe: async () => ({ ok: false, reason: 'auth', message: 'HTTP 401 for inbrain_secret' }) }),
    );
    expect(r.exitCode).toBe(1);
    const all = [...r.out, ...r.err].join('\n');
    expect(all).toMatch(/did not verify \(auth\)/);
    expect(all).not.toContain('inbrain_secret');
  ...
  test('--force remove failure aborts + redacts the token', async () => {
    const r = await runWithExitCapture(
      ['https://brain.example.com/mcp', '--token', 'inbrain_secret', '--install', '--yes', '--force'],
      installDeps({
        runBinary: (_b, argv) => (argv[1] === 'remove'
          ? { code: 1, stdout: '', stderr: 'remove failed near inbrain_secret' }
    ...
    const r = await runWithExitCapture(
      ...
      installDeps({
        runBinary: (_b, argv) => (argv[1] === 'add'
          ? { code: 1, stdout: '', stderr: 'add blew up with inbrain_secret' }
  ...
  test('--agent codex --install runs the codex CLI and hints the env var', async () => {
    ...
    const r = await runWithExitCapture(
      ['https://brain.example.com/mcp', '--token', 'inbrain_tok', '--install', '--yes', '--agent', 'codex'],
      installDeps({
        runBinary: (binary, argv) => { calls.push({ binary, argv }); return { code: argv[1] === 'get' ? 1 : 0, stdout: '', stderr: '' }; },
        env: () => undefined, // INBRAIN_REMOTE_TOKEN not set → expect the export hint
      }),
    );
    expect(r.exitCode).toBeUndefined();
    ...
    expect(add?.argv).toEqual(['mcp', 'add', 'inbrain', '--url', 'https://brain.example.com/mcp', '--bearer-token-env-var', 'INBRAIN_REMOTE_TOKEN']);
    expect(JSON.stringify(add?.argv)).not.toContain('inbrain_tok');
    expect(r.err.join('\n')).toMatch(/export INBRAIN_REMOTE_TOKEN/);
    expect(r.err.join('\n')).toMatch(/Verified/);
  });

  test('--agent codex --install skips the env hint when INBRAIN_REMOTE_TOKEN already matches', async () => {
    const r = await runWithExitCapture(
      ['https://brain.example.com/mcp', '--token', 'inbrain_tok', '--install', '--yes', '--agent', 'codex'],
      installDeps({ env: (n) => (n === 'INBRAIN_REMOTE_TOKEN' ? 'inbrain_tok' : undefined) }),
    );
    expect(r.exitCode).toBeUndefined();
...
describe('runConnect print mode', () => {
  test('prints the block to stdout with the literal token', async () => {
    const r = await runWithExitCapture(
      ['https://brain.example.com/mcp', '--token', 'inbrain_tok'],
      installDeps(),
    );
    expect(r.exitCode).toBeUndefined();
    expect(r.out.join('\n')).toContain("claude mcp add inbrain -t http https://brain.example.com/mcp -H 'Authorization: Bearer inbrain_tok'");
  });

  test('--json redacts the token', async () => {
    const r = await runWithExitCapture(
      ['https://brain.example.com/mcp', '--token', 'inbrain_secret', '--json'],
      installDeps(),
    );
    const j = JSON.parse(r.out.join('\n'));
    expect(j.token_redacted).toBe(true);
    expect(r.out.join('\n')).not.toContain('inbrain_secret');
  });

  test('--help prints command-specific HELP, no exit', async () => {
    const r = await runWithExitCapture(['--help'], installDeps());
    expect(r.exitCode).toBeUndefined();
    expect(r.out.join('\n')).toMatch(/inbrain connect/);
  });
  ...
  test('--agent codex print mode emits the codex block', async () => {
    const r = await runWithExitCapture(['https://brain.example.com/mcp', '--token', 'inbrain_tok', '--agent', 'codex'], installDeps());
    expect(r.exitCode).toBeUndefined();
    const out = r.out.join('\n');
    expect(out).toContain('codex mcp add inbrain --url https://brain.example.com/mcp --bearer-token-env-var INBRAIN_REMOTE_TOKEN');
    expect(out).toContain('export INBRAIN_REMOTE_TOKEN=inbrain_tok');
  });

  test('--agent perplexity print mode emits GUI connector steps', async () => {
    const r = await runWithExitCapture(['https://brain.example.com/mcp', '--token', 'inbrain_tok', '--agent', 'perplexity'], installDeps());
    expect(r.exitCode).toBeUndefined();
...
describe('runConnect --oauth', () => {
  test('perplexity --oauth with BYO client id/secret prints the OAuth connector block', async () => {
    const r = await runWithExitCapture(
      ['https://brain.example.com/mcp', '--agent', 'perplexity', '--oauth', '--client-id', 'inbrain_cl_x', '--client-secret', 'inbrain_cs_y'],
      installDeps(),
    );
    expect(r.exitCode).toBeUndefined();
    const out = r.out.join('\n');
    expect(out).toContain('Issuer URL:    https://brain.example.com');
    expect(out).toContain('Client ID:     inbrain_cl_x');
    expect(out).toContain('Client Secret: inbrain_cs_y');
  });
  ...
  test('perplexity --oauth --register mints a client via the host and prints it', async () => {
    const r = await runWithExitCapture(
      ...
      installDeps(), // registerOAuthClient → inbrain_cl_minted / inbrain_cs_minted
    );
    expect(r.exitCode).toBeUndefined();
    const out = r.out.join('\n');
    expect(out).toContain('Client ID:     inbrain_cl_minted');
    expect(out).toContain('Client Secret: inbrain_cs_minted');
  });
  ...
  test('perplexity --oauth --register --json redacts the secret by default', async () => {
    const r = await runWithExitCapture(
      ...
      installDeps({ registerOAuthClient: () => ({ ok: true, clientId: 'inbrain_cl_x', clientSecret: 'inbrain_cs_secret' }) }),
    );
    const j = JSON.parse(r.out.join('\n'));
    expect(j.auth).toBe('oauth');
    expect(j.client_secret).toBe(REDACTED);
    expect(r.out.join('\n')).not.toContain('inbrain_cs_secret');
  });
  ...
  test('--oauth with only --client-id fails (needs both)', async () => {
    const r = await runWithExitCapture(
      ['https://brain.example.com/mcp', '--agent', 'perplexity', '--oauth', '--client-id', 'inbrain_cl_x'],
      installDeps(),
    );
    expect(r.exitCode).toBe(1);
  ...
  test('register failure surfaces the manual register-client command', async () => {
    ...
    expect(r.exitCode).toBe(1);
    expect(r.err.join('\n')).toMatch(/inbrain auth register-client/);
  });
