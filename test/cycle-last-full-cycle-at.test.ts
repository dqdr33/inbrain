...
let brainDir: string;
// Per-test INBRAIN_HOME isolation: cycle's PGLite path acquires a file
// lock at `~/.inbrain/cycle.lock` (no sourceId scope). Without isolating
// INBRAIN_HOME per test, parallel inbrain processes on the same machine
...
// body in `withEnv({INBRAIN_HOME: <unique tmp>})` so the file lock path
// becomes per-test.
...
beforeEach(async () => {
  ...
  brainDir = mkdtempSync(join(tmpdir(), 'inbrain-cycle-lfca-'));
  inbrainHome = mkdtempSync(join(tmpdir(), 'inbrain-cycle-lfca-home-'));
});
...
describe('runCycle last_full_cycle_at exit hook', () => {
  test('per-source cycle with status=ok writes timestamp', async () => {
    await withEnv({ INBRAIN_HOME: inbrainHome }, async () => {
