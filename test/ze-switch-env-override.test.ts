...
describe('detectEnvOverride (pure)', () => {
  ...
  test('triggered:false when env matches target exactly', () => {
    const w = detectEnvOverride('zeroentropyai:zembed-1', 1280, {
      INBRAIN_EMBEDDING_MODEL: 'zeroentropyai:zembed-1',
      INBRAIN_EMBEDDING_DIMENSIONS: '1280',
    });
    expect(w.triggered).toBe(false);
  });

  test('triggered:true on INBRAIN_EMBEDDING_MODEL mismatch (1 var)', () => {
    const w = detectEnvOverride('zeroentropyai:zembed-1', 1280, {
      INBRAIN_EMBEDDING_MODEL: 'openai:text-embedding-3-large',
    });
    expect(w.triggered).toBe(true);
    expect(w.vars).toHaveLength(1);
    expect(w.vars[0].name).toBe('INBRAIN_EMBEDDING_MODEL');
    expect(w.vars[0].current).toBe('openai:text-embedding-3-large');
    expect(w.vars[0].target).toBe('zeroentropyai:zembed-1');
  });

  test('INBRAIN_EMBEDDING_DIMENSIONS string-vs-number comparison works', () => {
    const w = detectEnvOverride('zeroentropyai:zembed-1', 1280, {
      INBRAIN_EMBEDDING_DIMENSIONS: '1536',
    });
    expect(w.triggered).toBe(true);
    expect(w.vars[0].name).toBe('INBRAIN_EMBEDDING_DIMENSIONS');
  ...
  test('NaN dim treated as mismatch (defensive)', () => {
    const w = detectEnvOverride('zeroentropyai:zembed-1', 1280, {
      INBRAIN_EMBEDDING_DIMENSIONS: 'not-a-number',
    });
    expect(w.triggered).toBe(true);
  ...
  test('empty-string env values treated as unset (no false-trigger)', () => {
    const w = detectEnvOverride('zeroentropyai:zembed-1', 1280, {
      INBRAIN_EMBEDDING_MODEL: '',
      INBRAIN_EMBEDDING_DIMENSIONS: '   ',
    });
    expect(w.triggered).toBe(false);
  });
});
...
describe('formatEnvOverrideWarning', () => {
  test('produces ASCII-only box (no Unicode box-drawing per repo D10)', () => {
    const out = formatEnvOverrideWarning({
      triggered: true,
      vars: [{ name: 'INBRAIN_EMBEDDING_MODEL', current: 'openai:x', target: 'zeroentropyai:zembed-1' }],
    });
  ...
  test('every line is ≤ 78 cols (terminal-safe)', () => {
    const out = formatEnvOverrideWarning({
      triggered: true,
      vars: [
        { name: 'INBRAIN_EMBEDDING_MODEL', current: 'openai:text-embedding-3-large', target: 'zeroentropyai:zembed-1' },
        { name: 'INBRAIN_EMBEDDING_DIMENSIONS', current: '1536', target: '1280' },
      ],
    });
  ...
  test('names every var by exact key', () => {
    const out = formatEnvOverrideWarning({
      triggered: true,
      vars: [
        { name: 'INBRAIN_EMBEDDING_MODEL', current: 'a', target: 'b' },
        { name: 'INBRAIN_EMBEDDING_DIMENSIONS', current: 'c', target: 'd' },
      ],
    });
    expect(out).toContain('INBRAIN_EMBEDDING_MODEL');
    expect(out).toContain('INBRAIN_EMBEDDING_DIMENSIONS');
  });
  ...
  test('includes paste-ready `unset` command listing every var', () => {
    ...
    expect(out).toContain('unset INBRAIN_EMBEDDING_MODEL INBRAIN_EMBEDDING_DIMENSIONS');
  });
});
...
describe('applyRetrievalUpgrade env-gate (D9 #7)', () => {
  test('refused on env-triggered apply; ZERO setConfig calls fired', async () => {
    await withEnv({ INBRAIN_EMBEDDING_MODEL: 'openai:text-embedding-3-large' }, async () => {
      ...
      try {
        ...
        if (result.status === 'refused') {
          ...
          expect(result.warning.vars[0].name).toBe('INBRAIN_EMBEDDING_MODEL');
        }
  ...
  test('env not triggered → proceeds silently (no env vars set)', async () => {
    // No env vars at all
    await withEnv(
      { INBRAIN_EMBEDDING_MODEL: undefined, INBRAIN_EMBEDDING_DIMENSIONS: undefined },
      async () => {
        await engine.setConfig('embedding_model', 'openai:text-embedding-3-large');
        for (let i = 0; i < 101; i++) {
  ...
  test('result tagged union shape pinned (TypeScript discriminator + runtime)', async () => {
    await withEnv({ INBRAIN_EMBEDDING_MODEL: 'openai:other-model' }, async () => {
      await engine.setConfig('embedding_model', 'openai:text-embedding-3-large');
      for (let i = 0; i < 101; i++) {
...
describe('resumeRetrievalUpgrade env-gate (D9 #6)', () => {
  test('refused on env-triggered resume; ZERO schema-mutation setConfig calls fired', async () => {
    ...
    await withEnv({ INBRAIN_EMBEDDING_MODEL: 'openai:still-old-model' }, async () => {
      const setConfigSpy: string[] = [];
      const realSetConfig = engine.setConfig.bind(engine);
      (engine as unknown as { setConfig: typeof engine.setConfig }).setConfig =
        async (key: string, val: string) => {
  ...
  test('--ignore-env-override proceeds with resume', async () => {
    ...
    await withEnv({ INBRAIN_EMBEDDING_MODEL: 'openai:still-old' }, async () => {
