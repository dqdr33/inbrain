 ...
 *     not reported (#8).
 *   - Registry path is brain-isolated under inbrainPath (#7).
...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'inbrain-reg-'));
  process.env.INBRAIN_HOME = home;
});
...
// Imported lazily AFTER INBRAIN_HOME is set so inbrainPath resolves to the temp dir.
async function reg() {
...
describe('register + read round trip', () => {
  test('registerWorker writes under inbrainPath; readWorkers returns the live worker', async () => {
    const { registerWorker, readWorkers, workerRegistryDir } = await reg();
    expect(workerRegistryDir()).toBe(join(home, '.inbrain', 'workers'));
