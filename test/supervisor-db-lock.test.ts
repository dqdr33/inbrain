...
beforeEach(async () => {
  await engine.executeRaw(`DELETE FROM inbrain_cycle_locks WHERE id LIKE 'inbrain-supervisor:%'`);
});

describe('#1849 supervisorLockId', () => {
  test('keys on queue ONLY (DB scoping is physical — the lock row lives in the DB)', () => {
    expect(supervisorLockId('default')).toBe('inbrain-supervisor:default');
    expect(supervisorLockId('shell')).toBe('inbrain-supervisor:shell');
...
describe('#1849 LOCK_HELD path does not strand the pidfile', () => {
  test('the pidfile-cleanup exit listener is installed BEFORE the DB-lock acquire', async () => {
    ...
    const pidFile = join(tmpdir(), `inbrain-sup-stranded-${process.pid}-${Math.random().toString(36).slice(2)}.pid`);
