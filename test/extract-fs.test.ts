/**
 * Tests for `inbrain extract --source fs` (the default, FS-walking path).
 *
...
beforeEach(async () => {
  await truncateAll();
  brainDir = mkdtempSync(join(tmpdir(), 'inbrain-extract-fs-'));
}, 15_000);
...
describe('inbrain extract links --source fs', () => {
...
describe('inbrain extract timeline --source fs', () => {
...
describe('inbrain extract --dir default resolution', () => {
  ...
  test('uses configured sources(local_path) when --dir is not passed', async () => {
    ...
    const otherDir = mkdtempSync(join(tmpdir(), 'inbrain-extract-other-'));
  ...
  test('explicit --dir always wins over configured source', async () => {
    ...
    const decoyDir = mkdtempSync(join(tmpdir(), 'inbrain-extract-decoy-'));
    await (engine as any).db.exec(
