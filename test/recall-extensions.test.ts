/**
 * v0.32 — `inbrain recall` extensions: --since-last-run + --pending + --rollup
...
// body inside withEnv({ INBRAIN_HOME: tmpHome }) so process.env mutations are
// scoped + restored via try/finally instead of leaking across files).
function makeTmpHome(): string {
  return join(tmpdir(), `inbrain-recall-test-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
}
