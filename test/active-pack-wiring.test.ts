...
// parity with the inbrain-base hardcoded behavior when no pack is passed.
//
...
describe('v0.39 T1.5 — parseMarkdown activePack threading', () => {
  test('no activePack passed → falls back to legacy inferType (inbrain-base parity)', () => {
  ...
  test('activePack with custom type → uses pack inference (NOT legacy)', () => {
    ...
    // exist in inbrain-base — proves the pack drives, not the hardcoded table).
  ...
  test('activePack empty → falls back to inbrain-base hardcoded', () => {
    const result = parseMarkdown('# alice', 'people/alice.md', {
