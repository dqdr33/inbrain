...
describe('renderResolutionCommand', () => {
  test('takes_supersede emits inbrain takes supersede with row id', () => {
    const pair = mkIntraPagePair('people/alice', 7);
    const cmd = renderResolutionCommand(pair, 'takes_supersede');
    expect(cmd).toBe('inbrain takes supersede people/alice --row 7');
  });
  ...
  test('dream_synthesize targets the curated entity side', () => {
    ...
    expect(cmd).toBe('inbrain dream --phase synthesize --slug companies/acme');
  });
  ...
  test('takes_mark_debate emits mark-debate with row id', () => {
    ...
    expect(cmd).toBe('inbrain takes mark-debate people/alice --row 12');
  });
...
describe('proposeResolution (classify + render combined)', () => {
  test('intra_page → takes_supersede with paste-ready command', () => {
    ...
    expect(p.resolution_command).toBe('inbrain takes supersede people/alice --row 42');
  });
  ...
  test('cross_slug curated → dream_synthesize on curated slug', () => {
    ...
    expect(p.resolution_command).toBe('inbrain dream --phase synthesize --slug companies/acme');
  });
});
...
describe('pairToFinding', () => {
  test('merges pair + verdict into a finding', () => {
    ...
    expect(finding.resolution_command).toContain('inbrain takes supersede');
