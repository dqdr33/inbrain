/**
 * Tests for `inbrain skillpack-check` — the agent-readable health report.
 *
 * Covers:
 *   - Healthy fresh install → exit 0, healthy:true, actions:[], no DB needed.
 *   - Half-migrated (partial entry in completed.jsonl) → exit 1,
 *     healthy:false, actions includes `inbrain apply-migrations --yes`,
 ...
 * state. DATABASE_URL / INBRAIN_DATABASE_URL stripped so the report runs
...
beforeEach(() => {
  ...
  tmp = mkdtempSync(join(tmpdir(), 'inbrain-skillpack-check-test-'));
});
...
describe('inbrain skillpack-check', () => {
  test('healthy fresh install → exit 0, healthy:true, empty actions', () => {
    ...
    expect(report.summary).toBe('inbrain skillpack healthy');
  ...
  test('half-migrated (partial completed.jsonl) → exit 1, apply-migrations in actions', () => {
    ...
    expect(report.actions).toContain('inbrain apply-migrations --yes');
    expect(report.summary).toContain('inbrain apply-migrations --yes');
