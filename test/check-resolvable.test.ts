...
| "What do we know" | \`skills/query/SKILL.md\` |`;
  ...
  //
  // The list branch was added so `inbrain doctor` no longer reports every
...
- **Important**: read this carefully`;
  ...
  test("[list] D4 negative: convention-violating uppercase names are silently skipped", () => {
    ...
    // The user would notice on the first `inbrain doctor` run and lowercase it.
...
function makeSkillsFixture(files: Record<string, string>): string {
  const dir = mkdtempSync(join(tmpdir(), "inbrain-dry-"));
