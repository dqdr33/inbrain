/**
 * inbrain quarantine CLI (issue #1699) — list / clear / scan.
...
async function withHome<T>(fn: () => Promise<T>): Promise<T> {
  ...
  try {
    return await withEnv({ INBRAIN_HOME: home, INBRAIN_AUDIT_DIR: audit }, fn);
  } finally {
...
describe('inbrain quarantine list', () => {
...
describe('inbrain quarantine clear', () => {
...
describe('inbrain quarantine scan', () => {
