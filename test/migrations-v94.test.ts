...
describe('v0.41.2 R-MIG: take_domain_assignments migration v94', () => {
  ...
  test('composite PK (take_id, domain) rejects duplicate (take, domain) pair', async () => {
    ...
    await engine.executeRaw(
      `INSERT INTO take_domain_assignments (take_id, domain, pack) VALUES ($1, 'deal_success', 'inbrain-investor')`,
      [takeId]
    );
  ...
  test('multi-domain assignment for same take is permitted', async () => {
    ...
    await engine.executeRaw(
      `INSERT INTO take_domain_assignments (take_id, domain, pack) VALUES ($1, 'market_call', 'inbrain-investor')`,
      [takeId]
    );
    const rows = await engine.executeRaw<{ count: number }>(
  ...
  test('CHECK constraint rejects confidence outside [0, 1]', async () => {
    ...
    try {
      await engine.executeRaw(
        `INSERT INTO take_domain_assignments (take_id, domain, pack, confidence) VALUES ($1, 'deal_success', 'inbrain-investor', 1.5)`,
        [takeId]
      );
    } catch {
      threw = true;
    ...
    try {
      await engine.executeRaw(
        `INSERT INTO take_domain_assignments (take_id, domain, pack, confidence) VALUES ($1, 'deal_success', 'inbrain-investor', -0.1)`,
        [takeId]
      );
    } catch {
      threw = true;
  ...
  test('aggregator JOIN direction returns assignments per domain', async () => {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      await engine.executeRaw(
        `INSERT INTO take_domain_assignments (take_id, domain, pack) VALUES ($1, $2, 'inbrain-investor')`,
        [takeId, domain]
      );
    }
