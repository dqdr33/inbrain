...
`inbrain skillpack install`. This directory is NOT bundled. The pattern (in
SKILL.md) ships everywhere; the eval evidence stays in the inbrain repo where
...
From the inbrain repo root with `ANTHROPIC_API_KEY` set:
...
The harness routes through inbrain's gateway, so it inherits inbrain's auth,
...
├── harness-runner.ts                               # TS runner via inbrain gateway
